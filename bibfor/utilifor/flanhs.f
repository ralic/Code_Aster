      REAL*8 FUNCTION FLANHS( NORM, N, A, LDA, WORK )
...
C ======================================================================
C
C     SUBROUTINE LAPACK CALCULANT DES NORMES MATRICIELLES.
C-----------------------------------------------------------------------
C
...
C  =======
C
C  FLANHS  RETURNS THE VALUE OF THE ONE NORM,  OR THE FROBENIUS NORM, OR
...
C  ===========
C
C  FLANHS RETURNS THE VALUE
C
C     FLANHS = ( MAX(ABS(A(I,J))), NORM = 'M' OR 'M'
C              (
...
C          SPECIFIES THE VALUE TO BE RETURNED IN FLANHS AS DESCRIBED
C          ABOVE.
C
C  N       (INPUT) INTEGER
C          THE ORDER OF THE MATRIX A.  N >= 0.  WHEN N = 0, FLANHS IS
C          SET TO ZERO.
C
C  A       (INPUT) REALC8 ARRAY, DIMENSION (LDA,N)
...
C
         ...
         SUM = ONE
         DO 90 J = 1, N
            CALL FLASSQ( MIN( N, J+1 ), A( 1, J ), 1, SCALE, SUM )
   90    CONTINUE
