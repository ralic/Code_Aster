...
C***END PROLOGUE  DERFC
     ...
     1  SQRTPI, XMAX, TXMAX, XSML, Y, RMINEM, RMIREM, CSEVL
      LOGICAL FIRST
      INTEGER INITS
...
C
      ...
      IF (Y.GE.SQEPS) ERFCFO = 1.0D0 - X*(1.0D0 + CSEVL (2.D0*X*X-1.D0,
     1  ERFCS, NTERF))
      GOTO 9999
C
...
C
 30   CONTINUE
      Y = Y*Y
      IF (Y.LE.4.D0) ERFCFO = EXP(-Y)/ABS(X) * (0.5D0 + CSEVL (
     1  (8.D0/Y-5.D0)/3.D0, ERC2CS, NTERC2) )
      IF (Y.GT.4.D0) ERFCFO = EXP(-Y)/ABS(X) * (0.5D0 + CSEVL (
     1  8.D0/Y-1.D0, ERFCCS, NTERFC) )
      IF (X.LT.0.D0) ERFCFO = 2.0D0 - ERFCFO
      GOTO 9999
C
 40   CONTINUE
      CALL UTMESS ('A', 'ERFCFO', 'X TROP GRAND ')
      ERFCFO = 0.D0
C
 9999 CONTINUE
      END
