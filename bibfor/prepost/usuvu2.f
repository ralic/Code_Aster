...
C MODIF PREPOST  DATE 09/10/2002   AUTEUR DURAND C.DURAND 
C ======================================================================
...
C
      IF ( LOI(1:7) .EQ. 'ARCHARD' ) THEN
         IF ( ISUPP .EQ. 1 ) THEN
            WRITE(IFIRES,1000)
            CALL USUPU2( NBPT,NBPAIR,COEF,ANG,ISUPP,NBINST,
     &                         TEMPS,PUUSU2,
     &                         VUSTUB,VUSOB,PUS,PMOYE,POURPU,POUPRE)
...
C
      ELSEIF ( LOI(1:8) .EQ. 'KWU_EPRI' ) THEN
         ...
         IF ( IRET .EQ. 10 ) THEN
            CALL UTMESS('F','USURE','MODELE NON VALIDE.')
         ENDIF
         DO 20 I = 1,NBINST
...
C
      ELSEIF ( LOI(1:6) .EQ. 'EDF_MZ' ) THEN
         ...
         IF ( XD .GT. 1.D0 ) THEN
            IRET = 10
            CALL UTMESS('I','USURE',' SEUIL / V0  > 1 ' )
            CALL UTMESS('I','USURE',' ***** ARRET DU CALCUL ***** ' )
            GOTO 9999
         ENDIF
         X1 = ( 1.D0 - XD ) / XN
         DO 30 I = 1,NBINST
