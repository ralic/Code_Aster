      SUBROUTINE IMRNMV(IFM,NOMSDZ,OPTIOZ,NBNO,LISNOZ,NBCMP,LISCMZ,
     &                  NBCHIF,EPS)
...
C     IMRNMV  -- IMPRESSION DE LA MATR_ASSE DE NOM NOMSD, REELLE,
C                NON-SYMETRIQUE ET STOCKEE MORSE
...
C             -----------------------------------------
      DO 100 IBLOC = INDBLO*NBBLOC + 1,INDBLO*NBBLOC + NBBLOC
...
C ---    DE LA MATRICE :
C        -------------
        JBLOC = IBLOC + ISIBLO*NBBLOC
        CALL JEVEUO(JEXNUM(VALE,JBLOC),'L',ISBLOC)
...
C ---       POSITION DU TERME DIAGONAL DE LA LIGNE
C ---       (COLONNE) IEQUA DANS LE BLOC :
C           ----------------------------
...
C ---       IMPRESSION DE LA LIGNE (COLONNE) COURANTE SELON LE
C ---       GRAIN 'VALEUR' :
C           -------------
                CALL IMPFR1(NOMNO1,NOMCM1,LONLI2,ZK8(IDNOMN),
     &                      ZK8(IDNOMC),ZR(IDVALL),NBCHIF,IFM)

              END IF
            END IF
          END IF

C ---    REACTUALISATION DE L'INDICE DE DEBUT DE LIGNE (COLONNE)
