      SUBROUTINE USOBE1 ( DIMOBS, VOLOBS, OBSUSE, RCRAY, RCARTE, SECT,
...
C ======================================================================
C-----------------------------------------------------------------------
C     CALCULE LES VOLUMES USES D'OBSTACLE (GUIDAGE ENCO_1)
C ----------------------------------------------------------------------
...
C     ---- FIN DES COMMUNS JEVEUX --------------------------------------
      ...
      INTEGER  N, NC, NCO, IRAYO, ITHET, IREFE
...
C
         IF ( PARUSU(I,5) .EQ. 1.D0 ) THEN
            ...
            DO 152 J = 1 , IPAS(I,1)
               ...
               L = L + 1
               IF ( (DIF1.GE.0.D0) .AND. (PARUSU(I,1).LT.90.D0) ) THEN
                  OBSUSE(2*L-1) = THETA
                  OBSUSE(2*L)   = RHO + DIF1
               ELSE
                  OBSUSE(2*L-1) = THETA
...
C
            ...
            DO 154 J = 1 , IPAS(I,2)
               ...
               L = L + 1
               IF ( (DIF2.GE.0.D0) .AND. (PARUSU(I,3).GT.270.D0) ) THEN
                  OBSUSE(2*L-1) = THETA
                  OBSUSE(2*L)   = RHO + DIF2
               ELSE
                  OBSUSE(2*L-1) = THETA
