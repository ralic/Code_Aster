...
C
      ...
      CHARACTER*8 CHMAT,RESU,MODELE,CARELE,MAILLA,CHARGE
      CHARACTER*80 TITRE
      CHARACTER*16 CONCEP,NOMCMD
      CHARACTER*19 LIGREL,CART1
      LOGICAL LEXI
C ----------------------------------------------------------------------
C
      CALL JEMARQ()
...
C       -- CHAM_MATER :
C       ----------------
        CALL GETVID('CONCEPT','CHAM_MATER',IOCC,1,1,CHMAT,N1)
        IF (N1.EQ.1) THEN
          IF (.NOT.LEXI) THEN
...
C       -- CARA_ELEM :
C       ----------------
        CALL GETVID('CONCEPT','CARA_ELEM',IOCC,1,1,CARELE,N1)
        IF (N1.EQ.1) THEN
          IF (.NOT.LEXI) THEN
...
C       -- CHARGE :
C       ----------------
        CALL GETVID('CONCEPT','CHARGE',IOCC,1,1,CHARGE,N1)
        IF (N1.EQ.1) THEN
          IF (.NOT.LEXI) THEN
