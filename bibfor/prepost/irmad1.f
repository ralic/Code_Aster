      SUBROUTINE IRMAD1 ( FICH,VERSIO,NBNO,PRNO,NUEQ,NEC,DG,NCMPMX,
     +            ITYPE,NSTAT,CHAMNO,NOMGD,NOMCMP,NOMSYM,NOMNOE,NUMNOE)
      IMPLICIT REAL*8 (A-H,O-Z)
C
      INTEGER       IFI,NBNO,PRNO(*),NUEQ(*),NEC,DG(*),NCMPMX,NUMNOE(*)
      INTEGER       VERSIO, ITYPE, NSTAT
      CHARACTER*(*) NOMGD,NOMCMP(*),NOMNOE(*),FICH,NOMSYM,CHAMNO(*)
C--------------------------------------------------------------------
C            CONFIGURATION MANAGEMENT OF EDF VERSION
C MODIF PREPOST  DATE 06/06/96   AUTEUR ACBHHLB L.VIVAN 
C ======================================================================
...
C     ------------------------------------------------------------------
      ...
      LOGICAL      LTABL(50), AFAIRE, EXISTE, EXISDG
C
C  --- INITIALISATIONS ----
C
      CALL JEMARQ()
...
C
      ...
      ELSE
         CALL UTMESS('F','IRMAD1','VRAIMENT DESOLE')
      ENDIF
C
...
C ---- BOUCLE SUR LES DIVERSES GRANDEURS SUPERTAB ----
 ...
 20   CONTINUE
C
      IF ( IMPRE .GT. 1 ) CALL UTMESS('F','IRMAD1','IL Y A UN BUG')
C
