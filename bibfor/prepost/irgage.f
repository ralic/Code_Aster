...
C MODIF PREPOST  DATE 17/04/2000   AUTEUR JMBHH01 J.M.PROIX 
C ======================================================================
...
C     --- RECONNAISSANCE, DECOMPTE DES COMPOSANTES DE LA GRANDEUR ASTER
      ...
      DO 9 ICMAE=1,NBCOMP
        ...
        ELSE
          NBVARI= NBVARI+1
          IF (NBVARI.GT.500) CALL UTMESS('F','IRGAGE','LE'
     &    //' NOMBRE DE CMPS DE LA GRANDEUR DU CHAMP DEPASSE 500'
     &    //' AUGMENTER LA DIMENSION DU TABLEAU IVARI')
          IVARI(NBVARI)= ICOMP
        ENDIF
 9    CONTINUE
      IF(L2COMP) CALL UTMESS('A','IRGAGE','UNE MEME COMPOSANTE EST '//
     +     'DONNEE 2 FOIS SOUS LE MOT-CLE NOM_CMP LORS DE '//
     +     'L''IMPRESSION AU FORMAT ENSIGHT PAR IMPR_RESU')
C
