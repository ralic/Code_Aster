...
C
 ...
 40      CONTINUE
         L = L + 1
         IF ( TUBUSE(2*L-1).LT.PARUSU(I,1)) GOTO 40
         THETA = TUBUSE(2*L-1)
 42      CONTINUE
...
C --------- ON RECHERCHE LE THETA JUSTE AU DESSUS DE ANGDEB
 ...
 44         CONTINUE
            J = J - 1
            IF (TUBUSE(2*J-1).GT.PARUSU(I,1)) GOTO 44
            J = J + 1
            THETA =TUBUSE (2*J-1)
C
 43         CONTINUE
...
C
 ...
 45            CONTINUE
               J = J + 1
               IF (TUBUSE(2*J-1).LT.PARUSU(I,2)) THEN
                  THETA =TUBUSE (2*J-1)
                  P = AI1*THETA - BI1    
                  P = MIN ( P , PM ) 
                  TUBUSE(2*J) = TUBUSE(2*J) - P 
                  GOTO 45
               ELSEIF (TUBUSE(2*J-1).LT.PARUSU(I,3)) THEN
                  THETA =TUBUSE (2*J-1)
...
C ------------ ON RECHERCHE LE THETA JUSTE AU DESSUS DE ANGDEB
 ...
 49            CONTINUE
               J = J - 1
               IF (TUBUSE(2*J-1).GT.PARUSU(I,1)) GOTO 49
               J = J + 1
               THETA =TUBUSE (2*J-1)
C
 50            CONTINUE          
