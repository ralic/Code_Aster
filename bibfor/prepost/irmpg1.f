...
C
        CALL MFGAUI ( IDFIMD, IAUX,
     &                SAUX64, TYPGEL, NBREPL, NDIM2, CODRET )
C
        IF ( CODRET.NE.0 ) THEN
          CALL CODENT ( CODRET,'G',SAUX08 )
          CALL U2MESK('F','MED_50',1,SAUX08)
        ENDIF
C
        IF ( NIVINF.GT.1 ) THEN
...
C
        IF ( SAUX64(1:LGNOFA).EQ.SAUX16(1:LGNOFA) ) THEN
C
C
          IF ( TYPGEL.EQ.TYPGEO .AND. NBREPL.EQ.NBREPG ) THEN
C
            CALL MFGAUL ( IDFIMD, RAUX1, RAUX2, RAUX3,
     &                    EDFUIN, SAUX64, CODRET )
C
            IF ( CODRET.NE.0 ) THEN
              SAUX08='MFGAUL  '
              CALL U2MESG('F','DVP_97',1,SAUX08,1,CODRET,0,0.D0)
...
C      NOLOPG(26:32) = COMPTEUR AU DELA DE 1
C      LES MANQUES INTERNES SONT COMPLETES PAR DES '_'
C
C     EXEMPLE : HE8_____FPG8
C               QU4_____FPG4 __________________3
C               12345678901234567890123456789012
C
C 3.1.1 ==> MISE A BLANC
C
      NOLOPG =
     >'                                                                '
C      1234567890123456789012345678901234567890123456789012345678901234
C
C 3.1.2 ==> INSERTION DU NOM DE LA FAMILLE
C
...
C
  ...
  312 CONTINUE
C
C 3.1.3 ==> INSERTION DU NOMBRE DE SOUS-POINTS SI > 1
C
      IF ( NBSP.GT.1 ) THEN
  ...
  313   CONTINUE
      ENDIF
C
C 3.1.4 ==> INSERTION DU COMPTEUR AU DELA DE 1
C
      KAUX = KAUX + 1
      IF ( KAUX.GT.0 ) THEN
...
C
      ENDIF
C
      CALL MFGAUE ( IDFIMD, TYPGEO, REFCOO, EDFUIN,
     &              NBREPG, GSCOO, WG, NOLOPG, NDIM, CODRET )
C
      IF ( CODRET.NE.0 ) THEN
        SAUX08='MFGAUE  '
        CALL U2MESG('F','DVP_97',1,SAUX08,1,CODRET,0,0.D0)
