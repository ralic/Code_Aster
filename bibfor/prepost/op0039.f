...
C     ------------------------------------------------------------------
      ...
      CHARACTER*16 NOMCMD,K16BID,FORMR,FICH,K16NOM
...
C
C     -------------------------------------------
C     --- TRAITEMENT DE LA COMMANDE IMPR_RESU ---
C     -------------------------------------------
      LMOD   = .FALSE.
      MODELE = ' '
      CALL GETVID ( ' ', 'MODELE'  ,0,1,1, MODELE, N )
      IF ( N .NE. 0 ) LMOD= .TRUE.
C
      NUMEMO = 0
      VERSIO = 5
...
C        --- FORMAT ---
         ...
         IF(FORM.EQ.'CASTEM') THEN
           ...
           IF(IRET.EQ.0) THEN
             CALL WKVECT('&&OP0039.LAST','V V I',8,JLAST)
           ENDIF
         ENDIF
C
C        --- FICHIER ---
...
C            (RMQUE: UNIQUEMENT INTERESSANT POUR FORMAT 'RESULTAT')
         ...
         ENDIF
C
C        --- VERSION D'ECRITURE DU FICHIER IDEAS ----
         VERSIO = 5
...
C
 ...
 10   CONTINUE
      IF(LCASTS) THEN
         ...
         ENDIF
         CALL JEDETR ( '&&OP0039.LAST'   )
         CALL JEDETR ( '&&OP0039.LIGREL' )
         CALL JEDETR ( '&&OP0039.NOM_MODELE' )
      ENDIF
      ...
      CALL JEEXIN ( '&&IRCHML.CONNECINVERSE  ', N1 )
      IF ( N1 .NE. 0 ) CALL JEDETR( '&&IRCHML.CONNECINVERSE  ' )
 9999 CONTINUE
      CALL JEDEMA()
      END
