...
C
      ...
      INTEGER NUMEMO, NBMODL, NMAIL, NCHAR, NCHAR2
      INTEGER IUTIL, IDEBU
...
C
      ...
      CHARACTER*8  LERESU,NOPASE,NOMGD,RESMED
      CHARACTER*16 NOMCMD,K16BID,FORMR,FICH,TYRES
      CHARACTER*19 NOCH19,KNUM,RESU19
      CHARACTER*24 NOMJV
      CHARACTER*24 VALK(6)
      CHARACTER*24 NORECG,CORRN,CORRM,NOVCMP
      CHARACTER*64 K64BID
      CHARACTER*80 TITRE
C
...
C          RESURE : NOCC SD_RESULTAT "RESTREINT"
        ...
        CORRN='&&OP0039.CORRN'
        CORRM='&&OP0039.CORRM'
        CALL RDTMAI(NOMA,NOMARE,'V',CORRN,CORRM,0,0)
...
C            - ON RECUPERE LA LISTE DES NOMS DONNEE PAR L'UTILISATEUR
     ...
     &                                         ZK16(JNOSY-1+ISY))
               ...
               NCHAR2=LXLGUT(NOPASE)
               IF(NCHAR2.NE.0)THEN
                 DO 130 I=1,16-NCHAR
                     ZK80(JNCMED+ISY-1)(8+NCHAR+I:8+NCHAR+I)='_'
 130             CONTINUE
                 ZK80(JNCMED+ISY-1)(25:24+NCHAR2)=NOPASE(1:NCHAR2)
               ENDIF
   13        CONTINUE
           ENDIF
...
C          - ECRITURE DU CONCEPT LERESU SUR FICHIER FICH AU FORMAT FORM
          IF ( FORM(1:4).EQ.'MED' ) THEN
            IF ( LNCMED ) THEN
            CALL IREMED(LERESU,RESU,NOPASE,IFI,NBNOSY,
     &                  ZK16(JNOSY),NBCMDU,ZK80(JNCMED),PARTIE,NBORDR,
     &                  ZI(JORDR),LRESU,MODELE,NBNOT,ZI(JNUNOT),
     &                  NBMAT,ZI(JNUMA),NBCMP,ZK8(JCMP),LVARIE)
            ELSE
            CALL IREMED(LERESU,RESU,NOPASE,IFI,NBNOSY,
     &                  ZK16(JNOSY),NBCMDU,' ',PARTIE,NBORDR,
     &                  ZI(JORDR),LRESU,MODELE,NBNOT,ZI(JNUNOT),
     &                  NBMAT,ZI(JNUMA),NBCMP,ZK8(JCMP),LVARIE)
            ENDIF
          ELSE
     ...
     &        NBORDR,ZI(JORDR),LRESU,'RESU',IOCC,MODELE,CECR,
