...
C MODIF PREPOST  DATE 02/12/2003   AUTEUR PBADEL P.BADEL 
C ======================================================================
...
C
      ...
      INTEGER     IVAL,IRET,K,ITRMA,IGR,NBTROU,ITBMA,LMA,NBVAL
...
C     --- RECUPERATION DU NUMERO DE MAILLE----
   ...
   11     CONTINUE
          ...
          DO 13 IGR=1,NGROUP
            ...
            IF (IRET.EQ.0) THEN
              CALL UTDEBM('A','PRCOCH',' ON NE TROUVE PAS LE GROUPE')
              CALL UTIMPK('S',' ',1,GROUP(IGR))
              CALL UTFINM
            ELSE
  ...
  100     CONTINUE
          IF (NBTROU.EQ.0) THEN
            CALL UTMESS('F','PRCOCH','ON NE TROUVE PAS DE MAILLE'
...
10      CONTINUE              
        ...
        IF (NBVAL.EQ.0) THEN
          CALL UTMESS('F','PRCOCH','ON NE TROUVE PAS DE VALEURS')
        ENDIF
...
C     --- RECUPERATION DU NUMERO DE NOEUDS----
     ...
     &                K8BID,IRET)
          CALL WKVECT('&&PRCOCH.INDIC_NOEUD','V V I',NBMA,ITRMA)
          DO 111 K=1,NBMA
            ZI(ITRMA-1+K)=0
  111     CONTINUE
 ...
 1000     CONTINUE
          IF (NBTROU.EQ.0) THEN
            CALL UTMESS('F','PRCOCH','ON NE TROUVE PAS DE NOEUDS'
     &                     //'DANS LES GROUPES FOURNIS')
          ENDIF
