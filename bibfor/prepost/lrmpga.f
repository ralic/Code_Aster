...
C MODIF PREPOST  DATE 16/05/2011   AUTEUR PELLET J.PELLET 
...
C
      ...
      INTEGER NUNO,NUMDT,NPR,N
      INTEGER IOPT, IMOD, JMOD, IGRD, IADGD, NEC
      REAL*8  DT,PREC2,RBID
...
C
      DO 38 ITYG=1,NTYGEO
         CALL MFNPDT(IDFIMD,NOCHMD,NPDT,ZK16(JUNIT),ZK16(JCOMP),IRET)
         IF(NPDT.GT.0)THEN
           DO 39 IDT=1,NPDT
             CALL MFPDTI(IDFIMD,NOCHMD,IDT,NUMDT,NUNO,DT,IRET)
             IF(IINST.EQ.0)THEN
                 IF(NUNO.EQ.NUMORD .OR. NUMDT.EQ.NUMPT)GOTO 37
             ELSE
                 IF(CRIT(1:4).EQ.'RELA')THEN
                   PREC2=ABS(PREC*INST)
                   IF(ABS(DT-INST).LE.PREC2) GOTO 37
                 ELSEIF(CRIT(1:4).EQ.'ABSO')THEN
                   IF(ABS(DT-INST).LE.PREC) GOTO 37
                 ENDIF
             ENDIF
 39        CONTINUE
 37        CONTINUE
           CALL MFNNOP(IDFIMD,NOCHMD,EDMAIL,TYGEO(ITYG),NUMDT,NUNO,
     &                 1,NOMPRF,EDCOMP,NPR,NOMLOC,NGAULU,N,IRET)
           IF ( N.NE.0 ) THEN
              NBTYEL=NBTYEL+1
