...
C
      ...
      CHARACTER*1 SAUX01
      CHARACTER*8 SAUX08,K8B,FAPG,ELREF,NOMTM,TYELE(NTYGEO)
      CHARACTER*8 TYPMA(NTYGEO)
...
C  == 1.3. A PARTIR DU NOM DU CHAMP MED ET DE L'INDICE DU PAS DE TEMPS,
...
C      - UN MARQUEUR DE CORRESPONDANCE : ZI(JNGAOK)
...
C
          IF ( NIVINF.GT.1 ) THEN
            IF ( NGAULU.GT.0 ) THEN
              WRITE (IFM,2002) TYGEO(ITYG),NGAULU,TYELE(ITYG),NOMLOC
            ENDIF
          ENDIF
C
   13 CONTINUE
...
C  == 1.4. LECTURE DU NOMBRE DE LOCALISATIONS PRESENTES DANS LE FICHIER
...
C            SI LA LOCALISATION POUR LE CHAMP N'A TOUJOURS PAS ETE
C            TROUVEE DANS CELLES DU MODELE
...
C             COMPARAISON DES COORDONNEES DES PG ASTER/MED
              ...
              CALL LRVCPG(IDFIMD,NBPG,ZI(JNGALU+J-1),NOMTM,
     &                    ZI(JTYPGE+J-1),ELREF,ZK8(JTYELM+J-1),FAPG,
     &                    NLOC,ZI(JPERM),NUTYMA,CODRET)
              NBPGM = ZI(JNGALU+J-1)
