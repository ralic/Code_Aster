      SUBROUTINE IRMAIL ( FORM, IFI, VERSIO, NOMA, LMOD, NOMO, NIVE,
     &                    INFMAI )
C
      IMPLICIT NONE
C     ------------------------------------------------------------------
C            CONFIGURATION MANAGEMENT OF EDF VERSION
C MODIF PREPOST  DATE 15/09/2008   AUTEUR MEUNIER S.MEUNIER 
C ======================================================================
...
C
      ...
      INTEGER JNOMAI, JNONOE, JPERM, JPERM2, JPOIN
...
C       - TRAITEMENT DU FORMAT ECHANGE DE DONNEES 'MED'
     ...
     &      NOMA,ZI(JTYPM),ZK80(JTITR),NBTITR,NBGRN,
...
C       - TRAITEMENT DU FORMAT 'CASTEM'
     ...
     &    NOMA,ZI(JTYPM),LMOD,NBGRN,ZK8(JNOGN),NBGRM,ZK8(JNOGM),NIVE)
C
...
C         ON REGARDE SI LE MAILLAGE EST UN MAILLAGE GMSH (LGMSH)
        ...
        IF(IRET.NE.0) THEN
          ...
          ENDIF
        ENDIF
        CALL IRMGMS(IFI,NDIM,NBNOE,NOMA,NBGRM,ZK8(JNONOE),
     &              LGMSH,VERSIO)
C
...
C       - SOUS PROGRAMME : TRAITER LES ADHERENCES SUPERTAB
     ...
     &  MAXNOD,LMOD,NOMA,NBGRN,ZK8(JNOGN),NBGRM,ZK8(JNOGM),
...
C       - LE SOUS-PROGRAMME IRADHE TRAITE LES ADHERENCES ENSIGHT
        ...
        CALL JEVEUO('&&IRADHE.PERMUTA','L',JPERM2)
        CALL JELIRA('&&IRADHE.PERMUTA','LONMAX',LON1,K1BID)
        MAXNOD=ZI(JPERM2-1+LON1)
        IF(NBGRN.NE.0) THEN
          LGNOMA=LXLGUT(NOMA)
          CALL U2MESK('A','PREPOST2_76',1,NOMA(1:LGNOMA))
        ENDIF
        CALL IRMAEN(IFI,NOMA,NDIM,NBNOE,ZR(JCOOR),NBMAI,ZI(JCONX),
     &       ZK8(JNOMAI),ZK8(JNONOE),ZI(JPOIN),
     &       ZI(JTYPM),ZI(JTYPL),ZK8(JNOMAE),ZI(JPERM2),
