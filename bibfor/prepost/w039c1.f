...
C          POUR KZONE < IZONE :
   ...
   10     CONTINUE
C         -- IZONE == KZONE :
          ZI(JZONES-1+IZONE)=KZONE
          GOTO 30

   20   CONTINUE
        CALL W039C2(IZONE,JVALE,JDESC,NOMGD,IFM,IFR)
   30 CONTINUE
...
C     ------------------------------------------------------
      ...
      DO 40,IMA=1,NBMA
        ...
        IF (IZONE.GT.0) THEN
          IF (ZI(JZONES-1+IZONE).NE.0)IZONE=ZI(JZONES-1+IZONE)
          CALL CESEXI('C',JCESD,JCESL,IMA,1,1,1,IAD)
          CALL ASSERT(IAD.LE.0)
          ZL(JCESL-1-IAD)=.TRUE.
          ZR(JCESV-1-IAD)=DBLE(IZONE)
        ENDIF
   40 CONTINUE
