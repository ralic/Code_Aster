...
!
    exituy = .false.
    call dismoi('NOM_MODELE',carael,'CARA_ELEM',&
                ibid,nomo,'F',iret)
    call dismoi('EXI_TUYAU',nomo,'MODELE',&
                ibid,saux03,'F',iret)
...
!
    if (exituy) then
        ...
        call jeveuo(sdcarm//'.CARORIEN  .CESC', 'L', vk8=ocesc)
        nbcmp = zi(jocesd+1)
        icmpo1 = indik8(ocesc,'GAMMA',1,nbcmp)
        icmpo2 = indik8(ocesc,'GAMMA2',1,nbcmp)
        icmpo3 = indik8(ocesc,'GAMMA3',1,nbcmp)
        icmpo4 = indik8(ocesc,'GAMMA4',1,nbcmp)
        nbgamm = 4
        if( icmpo4.eq.0 ) nbgamm = 3
    endif
!
...
!
        ...
        if (exituy) then
            call cesexi('C', jocesd, jocesl, ima, 1,&
                        1, icmpo1, iad)
            zr(jorima(ityp)+(nbgamm-1)*nmatyp(ityp)) = ocesv(iad)
            call cesexi('C', jocesd, jocesl, ima, 1,&
                        1, icmpo2, iad)
            zr(jorima(ityp)+(nbgamm-1)*nmatyp(ityp)+1) = ocesv(iad)
            call cesexi('C', jocesd, jocesl, ima, 1,&
                        1, icmpo3, iad)
            zr(jorima(ityp)+(nbgamm-1)*nmatyp(ityp)+2) = ocesv(iad)
            if( nbgamm.eq.4 ) then
                call cesexi('C', jocesd, jocesl, ima, 1,&
                            1, icmpo4, iad)
                if (iad .gt. 0) then
                    zr(jorima(ityp)+(nbgamm-1)*nmatyp(ityp)+3) = ocesv(iad)
                else
                    zr(jorima(ityp)+(nbgamm-1)*nmatyp(ityp)+3) = 0.d0
                endif
            endif
        endif
        if (exituy) then
            ...
            call cesexi('C', jpcesd, jpcesl, ima, 1,&
                        1, icmpep, iad)
            if (iad .gt. 0) then
                zr(jrmax(ityp)+nmatyp(ityp)-1) = pcesv(iad)
            else
...
!         -- ATTRIBUTS VARIABLE, ICI RMIN ET RMAX
            if (nbsect .ne. 0) then
                valr(1) = zr(jrmin(ityp))-zr(jrmax(ityp))
                valr(2) = zr(jrmin(ityp))
                call as_mmhraw(idfimd, nomamd, nvtyge, atepai, nmatyp(ityp),&
                               valr, codret)
                if (codret .ne. 0) then
                    saux08='mmhraw'
