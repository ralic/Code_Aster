...
C
  ...
  212 CONTINUE
C
C     ON TRAITE LES TRIA7,QUAD9,PENTA18,HEXA27 EN OUBLIANT LES NOEUDS DU
C     CENTRE ET LES SEG4 EN OUBLIANT LES 2 NOEUDS CENTRAUX
C     - TRIA7
      LNOCEN=.FALSE.
...
C
        ITYP   = TYPMA(IMA)
C       ON TRAITE LES TRIA7,QUAD9,PENTA18,HEXA27 EN OUBLIANT
...
C
          CALL MFCONE ( FID, NOMAMD, NDIM, ZI(JCNXMA(ITYP)),
...
C
          CALL MFNOME ( FID, NOMAMD, ZK16(JNOMMA(ITYP)), NMATYP(ITYP),
...
C
          CALL MFNUME ( FID, NOMAMD, ZI(JNUMMA(ITYP)), NMATYP(ITYP),
     &                  EDMAIL, TYPGEO(ITYP), CODRET )
          IF ( CODRET.NE.0 ) THEN
            SAUX08='MFNUME  '
            CALL U2MESG('F','DVP_97',1,SAUX08,1,CODRET,0,0.D0)
