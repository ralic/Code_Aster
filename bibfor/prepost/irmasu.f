...
CCC
     ...
     +                      'INEXISTANT DANS IDEAS, CONVERTI EN TRIA6')
        ...
        ELSEIF ( NOMTM .EQ. 'QUAD9' ) THEN
           IF (IQUA9.EQ.0) CALL UTMESS('I','IRMASU','QUAD9 ELEMENT '//
     +                      'INEXISTANT DANS IDEAS, CONVERTI EN QUAD8')
           IQUA9 = 1
           NNOE = NNOE - 1
        ELSEIF ( NOMTM .EQ. 'SEG4' ) THEN
           IF (ISEG4.EQ.0) CALL UTMESS('I','IRMASU','SEG4 ELEMENT '//
     +                      'INEXISTANT DANS IDEAS, CONVERTI EN SEG2')
           ISEG4 = 1
           NNOE = NNOE - 2
CJMP
...
C       - ELEMENTS NON DISPONIBLES DANS IDEAS
     ...
     &          //' DANS IDEAS')
        ...
        IF((ICOD2.GT.10000).OR.(ICOD2.LT.0))THEN
          CALL UTMESS('F','IRMASU', 'CODE INVALIDE : ICOD2')
        END IF
        IF(ICOD1.NE.0.AND.ICOD2.NE.0) THEN
          IF(LMASU) THEN
            CALL LXLIIS(NOMAI(IMA)(3:8),IMAS,IER)
            DO 751 J=1,NNOE
              CALL LXLIIS(NONOE(NODSUP(J))(3:8),NODSUP(J),IER)
 751        CONTINUE
          ELSE
            IMAS = IMA
          ENDIF
...
C     --- TRAITEMENT DES GROUPES DE NOEUDS
      DO 752 IGN = 1,NBGRN
        ...
        DO 755 JN=1,NBN
          ...
          IF(LMASU) THEN
            CALL LXLIIS(NONOE(ZI(JNOGR-1+JN))(3:8),ZI(JNOGR-1+JN),IER)
          ENDIF
  755   CONTINUE
...
C           - SI MAILLAGE IDEAS RECUPERATION DU NUMERO DE MAILLE
            CALL LXLIIS(NOMAI(ZI(JMAGR-1+NBM2))(3:8),
     +                                     ZI(JMAGR-1+NBM2),IER)
          ENDIF
  756   CONTINUE
        IF(NBM2.NE.0) THEN
