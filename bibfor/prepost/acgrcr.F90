...
!
    ...
    rayon = .false.
    if ( (praccr(24) .eq. 1) .or. (praccr(25) .eq. 1) .or. &
          (praccr(32) .eq. 1) ) then
        rayon = .true.
    endif
...
!
    ...
    if (crepst) then
        ...
        call acplcr(nbvec, jvectn, jvectu, jvectv, nbordr,&
                  ...
                  jvecno, jnorma,rayon, jresun, jdgama,jgamma, &
                  jepnma,jdenma )
    ...
    if (crepsp) then
        ...
        call acplcr(nbvec, jvectn, jvectu, jvectv, nbordr,&
                  kwork, sompgw, jrwork, tspaq, ipg, dectau, nommet, &
                  jvecno, jnorma,rayon,jresun, jdgpma,jgapma, &
                  jeppma,jdepma  )
...
30      continue
    !
        ...
        if (gcmax .gt. epsilo) then
            ...
            if( ( abs(grcrma(2)-grcrma(1)) .lt. epsilo ) .and. &
                (i .ne. mnmax(1)) ) then
                grcrma(2) = gcmax
                mnmax(2) = i
            endif
