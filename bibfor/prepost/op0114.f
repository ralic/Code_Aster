...
C     ----- RECUPERATION DES AMORTISSEMENTS -----
      ...
      IF (NA.NE.0) THEN
         ...
         IF (NBAMOR.GT.NBMODE) THEN
            CALL UTDEBM('F','OP0114','TROP D''AMORTISSEMENTS MODAUX')
...
CCC      CALL JEVEUO(VALE,'L',JAMOR)
      ...
      IF (NBAMOR.NE.NBMODE) THEN
       CALL UTDEBM('F','OP0114','IL MANQUE DES AMORTISSEMENTS MODAUX')
         CALL UTIMPI('L','   NOMBRE D''AMORTISSEMENT : ',1,NBAMOR)
         CALL UTIMPI('L','   NOMBRE DE MODE : ',1,NBMODE)
         CALL UTFINM( )
      ENDIF
...
C     --- ECRITURE MATRICE MASSE DIAGONALE ---
      ...
      CALL JEVEUO(ZK24(JREFA-1+2)(1:14)//'.SLCS.SCDI','L',JDIA)
      CALL JEVEUO(ZK24(JREFA-1+2)(1:14)//'.SLCS.SCBL','L',JABL)
      CALL JEVEUO(ZK24(JREFA-1+2)(1:14)//'.SLCS.SCDE','L',JSCDE)
      NBBLOC = ZI(JSCDE+3-1)
      VALE(1:8) = MASSE
      IDDL = 0
      DO 30 IB = 1,NBBLOC
