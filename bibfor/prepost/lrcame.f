...
C       RECUPERE LE NOMBRE DE MAILLES DE TYPE TYGEOM
        CALL MFNEMA ( IDFIMD, NOMAMD, EDCONN, EDMAIL, TYGEOM, EDNODA,
     &                NMATYP, CODRE2 )
C
        IF ( CODRE2.EQ.0 ) THEN
C
...
C
          ...
          ENDIF
          CALL MDCHIN ( NOFIMD, IDFIMD, NOCHMD, TYPENT,
     &                  TYGEOM, PREFIX, NPAS, CODRET )
C
          IF ( NPAS.NE.0 ) THEN
...
C
 ...
 5    CONTINUE
      ...
      IF (TYPECH(1:4).EQ.'NOEU') THEN
        CALL CNSCRE(NOMAAS, NOMGD, NCMPRF, ZK8(JNOCMP), 'V', CHAMES)
C
     ...
     &                 ZK8(JNOCMP),NPGMA,-1,-NCMPRF)
        ELSEIF (TYPECH(1:4).EQ.'CART')THEN
          CALL CESCRE ( 'V', CHAMES, 'ELEM', NOMAAS, NOMGD, NCMPRF,
     &                 ZK8(JNOCMP),-1,-1,-NCMPRF)
        ELSEIF (TYPECH(1:4).EQ.'ELNO'.OR.TYPECH(1:4).EQ.'ELEM')THEN
          CALL CESCRE ( 'V', CHAMES, TYPECH, NOMAAS, NOMGD, NCMPRF,
     &                 ZK8(JNOCMP),-1,-1,-NCMPRF)
        ELSE
          CALL ASSERT(.FALSE.)
