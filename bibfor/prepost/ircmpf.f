...
C
      ...
      IF ( CODRET.NE.0 ) THEN
        ...
        CALL UTMESS ('F',NOMPRO,'MED: ERREUR EFOUVR NUMERO '//SAUX08)
      ENDIF
C
C====
...
C====
C
      CALL EFNPRO ( IDFIMD, NBPROF, CODRET )
      IF ( CODRET.NE.0 ) THEN
        CALL UTDEBM ( 'A', NOMPRO, 'FICHIER ' )
        CALL UTIMPK ( 'S', 'MED : ', 1, NOFIMD )
        CALL UTIMPI ( 'L', 'IDENTIFIANT NUMERO ', 1, IDFIMD )
        CALL UTIMPI ( 'L', 'ERREUR EFNPRO NUMERO ', 1, CODRET )
        CALL UTIMPI ( 'L', 'NBPROF ', 1, NBPROF )
        CALL UTFINM ()
        CALL CODENT ( CODRET,'G',SAUX08 )
        CALL UTMESS ('F',NOMPRO,'MED: ERREUR EFNPRO NUMERO '//SAUX08)
      ENDIF
C
      IF ( NIVINF.GT.1 ) THEN
...
C
      IF ( NBPROF.NE.0 ) THEN
        CALL WKVECT ( NTNOPF, 'V V K32', NBPROF, ADNOPF )
      ENDIF
C
C====
...
C
        CALL EFPROI ( IDFIMD, IAUX, NOPROF, LGPROF, CODRET )
        IF ( CODRET.NE.0 ) THEN
          CALL UTDEBM ( 'A', NOMPRO, 'FICHIER ' )
          CALL UTIMPK ( 'S', 'MED : ', 1, NOFIMD )
          CALL UTIMPI ( 'L', 'PROFIL NUMERO ', 1, IAUX )
          CALL UTIMPI ( 'L', 'ERREUR EFPROI NUMERO ', 1, CODRET )
          CALL UTFINM ()
...
C
          CALL EFPFLL ( IDFIMD, ZI(ADPROF), NOPROF, CODRET )
          IF ( CODRET.NE.0 ) THEN
            CALL UTDEBM ( 'A', NOMPRO, 'PROFIL ' )
            CALL UTIMPK ( 'S', 'MED : ', 1, NOPROF )
...
C
      ...
      IF ( CODRET.NE.0 ) THEN
        CALL CODENT ( CODRET,'G',SAUX08 )
        CALL UTMESS ('F',NOMPRO,'MED: ERREUR EFFERM NUMERO '//SAUX08)
      ENDIF
C
C====
...
C
      IF ( NRPRTY.EQ.0 ) THEN
C
C 6.1. ==> OUVERTURE FICHIER MED EN MODE 'ECRITURE'
...
C          IL SERA DU TYPE 'PROFIL_00000000000000000000000N'
...
C
        DO 62 , IAUX = 1 , NBPROF + 1
          CALL CODENT( IAUX , 'D0' , SAUX24  )
C                   12345678
          NOPROF = 'PROFIL__'//SAUX24
CGN        WRITE (IFM,*) 'TEST DU NOM DE PROFIL : ',NOPROF
          DO 621 , JAUX = 0 , NBPROF-1
CGN        WRITE (IFM,*) '... COMPARAISON AVEC  : ',ZK32(ADNOPF+JAUX)
            IF ( NOPROF.EQ.ZK32(ADNOPF+JAUX) ) THEN
              GOTO 62
            ENDIF
  621     CONTINUE
          GOTO 622
...
CGN 1789  FORMAT(10I5)
 ...
 6301     FORMAT('. PROFIL A CREER : NOM      = ',A,
     ...
     >         /,'   1ERE ET DERNIERE VALEURS = ',2I8)
        ...
        IF ( CODRET.NE.0 ) THEN
          CALL CODENT ( CODRET,'G',SAUX08 )
          CALL UTMESS ('F',NOMPRO,'MED: ERREUR EFPFLE NUMERO '//SAUX08)
        ENDIF
C
