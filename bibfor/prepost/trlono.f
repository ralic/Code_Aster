      SUBROUTINE TRLONO ( NBVEC, JVECTN, JVECTU, JVECTV, NBORDR, KWORK,
     &                    SOMNOW, JRWORK, TSPAQ, INO, JVECNO )
      IMPLICIT   NONE
      INTEGER    NBVEC, JVECTN, JVECTU, JVECTV, NBORDR, KWORK
      INTEGER    SOMNOW, JRWORK, TSPAQ, INO, JVECNO
...
C     KWORK   : IN  : KWORK = 0 ON TRAITE LE 1ER NOEUD DU PAQUET DE
C                               NOEUDS ;
C                     KWORK = 1 ON TRAITE LE IEME (I>1) NOEUD DU PAQUET
C                               NOEUDS.
...
C                     ATTACHES A CHAQUE NOEUD DU <<PAQUET>> DE NOEUDS.
C     TSPAQ   : IN  : TAILLE DU SOUS-PAQUET DU <<PAQUET>> DE NOEUDS
C                     COURANT.
C     INO     : IN  : Ieme NOEUD DE LA LISTE.
C     JVECNO  : IN  : ADRESSE DU VECTEUR DE TRAVAIL CONTENANT 
...
C
         DO 20 IORDR=1, NBORDR
            ADRS = (IORDR-1)*TSPAQ + KWORK*SOMNOW*6 + (INO-1)*6
...
C PROJECTION DU vect_TAU SUR LES VECTEURS u ET v DU REPERE LOCAL
            ...
            N = N + 1
            ZR( JVECNO + (N-1)*2 )     = CUTAU
            ZR( JVECNO + (N-1)*2 + 1 ) = CVTAU
