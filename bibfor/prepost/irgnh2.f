      SUBROUTINE IRGNH2 ( IFI, NBORDR, COORD, CONNEX, POINT, NOCMP, 
     +                    NJVHEX, NBHEX, CNSC, CNSL, CNSV, CNSD )
      IMPLICIT NONE
C
...
C
      INTEGER      IHEX, IMA, IPOIN, LISTNO(8), J, JCNSC, JCNSL, JCNSV,
     +             JCNSD, NCMP, K, JHEX, IOR, INOE
      REAL*8       VALE
C     ------------------------------------------------------------------
C
      CALL JEMARQ()
...
C
      DO 10 IHEX = 1 , NBHEX
         ...
         LISTNO(7) = CONNEX(IPOIN+6)
         LISTNO(8) = CONNEX(IPOIN+7)

         DO 12 J = 1 , 3          
            WRITE(IFI,1000) (COORD(3*(LISTNO(INOE)-1)+J),INOE=1,8)
 12      CONTINUE
C
         DO 14 IOR = 1 , NBORDR
