...
C
      INTEGER      VALI, IUNIFI, NREF, IREFC, IREFR, IMIN, IBID
...
C
           IF (ZK24(LPROL).EQ. 'FONCT_C') THEN
             CALL FOINTC('E',LAFONC,0,' ',VALPU(1),RESURE,RESUIM,IRET)
 ...
 69              CONTINUE
     ...
     +                       VALI,VALR,VALC,EPSI,CRIT,IFIC,SSIGNE)
             ...
             ENDIF
           ELSE
             CALL FOINTE ('E',LAFONC,NBPU,NOMPU,VALPU,VALR,IRET)
             IF ( OUINON .EQ. 'OUI' ) THEN
               K12 = NOMPU(NBPU)
               TESTOK = ' OK '
...
C
         CALL FOINTC('E',SPECTR,1,K8B,XPARA,RESURE,RESUIM,IER)
         IF (IER.NE. 0) THEN
           TESTOK = 'NOOK'
           TEXTE = ' PB INTERPOLATION. VOIR MESSAGE CI-DESSUS'
           WRITE(IFIC,1300) TESTOK, TEXTE
