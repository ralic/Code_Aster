...
C ----------------------------------------------------------------------
C !! EN CAS DE MODIFICATION DE CE SS-PGME, PENSER A IRDEER,IRDEEC,IRDRER
...
C       PORTEES PAR LES NOEUDS. SINON ON NE LES IMPRIMERA PAS.
      DO 70 ISCA=1,NBVSMX
        ...
        IF(NBCMPT.GT.0) THEN
          ...
          LGCOMP=LXLGUT(NOCMP)
          IF(.NOT.LRESU) THEN
            CALL UTMESS('A','IRDREC',' IMPR_RESU FORMAT ENSIGHT :'//
...
C        CALL U2MESK('A','PREPOST2_42', 2 ,VALK)
          ELSE
            CALL UTMESS('A','IRDREC',' IMPR_RESU FORMAT ENSIGHT :'//
     ...
     &            'LES NOEUDS DU MAILLAGE ET NE SERA PAS IMPRIMEE')
C        CALL U2MESK('A','PREPOST2_43', 3 ,VALK)
          ENDIF
        ENDIF
  70  CONTINUE
C
...
C        CALL U2MESK('A','PREPOST2_44', 4 ,VALK)
          ELSE
             CALL UTMESS('A','IRDREC',' IMPR_RESU FORMAT ENSIGHT :'//
     &            ' AUCUNE DES TROIS COMPOSANTES '//
     &            NOCMP1(1:LGCMP1)//' '//NOCMP2(1:LGCMP2)//' '//
     ...
     &            ' N''EST PORTEE PAR LES NOEUDS DU MAILLAGE ET LE'//
     &            ' VECTEUR ENSIGHT CORRESPONDANT NE SERA PAS IMPRIME')
C        CALL U2MESK('A','PREPOST2_45', 5 ,VALK)
          ENDIF
        ENDIF
  80  CONTINUE
C
  ...
  22      CONTINUE
     ...
     &           ' CAS='//VKORD(1:LGNTXT)
          ...
          DO 24 INO=1,NBNO
            IVAL=(INO-1)*NCMP
            INUVAL=INUVAL+1
            VALR8R(INUVAL)=DBLE(VALE(IVAL+ICMPAS))
            VALR8I(INUVAL)=DIMAG(VALE(IVAL+ICMPAS))
            IF(INUVAL.EQ.6) THEN
  ...
  32      CONTINUE
     ...
     &           ' CAS='//VKORD(1:LGNTXT)
          ...
          DO 34 INO=1,NBNO
            ...
            INUVAL=INUVAL+1
            IF(.NOT.LNULV1) THEN
              VALR8R(3*(INUVAL-1)+1)=DBLE(VALE(IVAL+ICMPA1))
              VALR8I(3*(INUVAL-1)+1)=DIMAG(VALE(IVAL+ICMPA1))
            ELSE
              VALR8R(3*(INUVAL-1)+1)=0.0D0
              VALR8I(3*(INUVAL-1)+1)=0.0D0
            ENDIF
            IF(.NOT.LNULV2) THEN
              VALR8R(3*(INUVAL-1)+2)=DBLE(VALE(IVAL+ICMPA2))
              VALR8I(3*(INUVAL-1)+2)=DIMAG(VALE(IVAL+ICMPA2))
            ELSE
              VALR8R(3*(INUVAL-1)+2)=0.0D0
              VALR8I(3*(INUVAL-1)+2)=0.0D0
            ENDIF
            IF(.NOT.LNULV3) THEN
              VALR8R(3*(INUVAL-1)+3)=DBLE(VALE(IVAL+ICMPA3))
              VALR8I(3*(INUVAL-1)+3)=DIMAG(VALE(IVAL+ICMPA3))
            ELSE
...
C           - ON ECRIT LES FICHIERS VIDES DE VALEURS QUI SUIVENT
 ...
 205          CONTINUE
  ...
  30    CONTINUE
      ...
      GOTO 999
 101  CONTINUE
      CALL UTMESS('E','IRDREC','PROBLEME A L''OUVERTURE DU'//
     &     ' FICHIER DE VALEURS ENSIGHT '//FIEN(1:LGFICH)//
     &     ' POUR L''IMPRESSION DE LA PARTIE REELLE DU CHAM_GD '//
     &     NOSY16(1:LGCH16))
C        CALL U2MESK('E','PREPOST2_26', 2 ,VALK)
 102  CONTINUE
      CALL UTMESS('E','IRDREC','PROBLEME A L''OUVERTURE DU'//
     &     ' FICHIER DE VALEURS ENSIGHT '//FIEN(1:LGFICH)//
     &     ' POUR L''IMPRESSION DE LA PARTIE RELLE DU CHAMP '//
     &     NOSY16(1:LGCH16)//' DU CONCEPT'//NOMSD(1:LGCONC))
C        CALL U2MESK('E','PREPOST2_27', 3 ,VALK)
      GOTO 999
 103  CONTINUE
      CALL UTMESS('E','IRDREC','PROBLEME A L''OUVERTURE DU'//
     &     ' FICHIER DE VALEURS ENSIGHT '//FIEN(1:LGFICH)//
     &     ' POUR L''IMPRESSION DE LA PARTIE IMAGINAIRE DU CHAM_GD '//
     &     NOSY16(1:LGCH16))
C        CALL U2MESK('E','PREPOST2_28', 2 ,VALK)
 104  CONTINUE
      CALL UTMESS('E','IRDREC','PROBLEME A L''OUVERTURE DU'//
     &     ' FICHIER DE VALEURS ENSIGHT '//FIEN(1:LGFICH)//
     &     ' POUR L''IMPRESSION DE LA PARTIE IMAGINAIRE DU CHAMP '//
     &     NOSY16(1:LGCH16)//' DU CONCEPT'//NOMSD(1:LGCONC))
C        CALL U2MESK('E','PREPOST2_29', 3 ,VALK)
      GOTO 999
 998  CONTINUE
