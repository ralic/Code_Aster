     ...
     &                  NUMMAI,ALIAS ,NOMMAI,NDIM  ,NBNOM ,
     &                  COORMA)
C     
...
C --- TYPE DE LA MAILLE
C
      IF  (ZI(JTYPMA+ZTYPM*(POSMAI-1)+1-1).EQ.1) THEN
        TYPMAI = 'MAIT'
      ELSEIF  (ZI(JTYPMA+ZTYPM*(POSMAI-1)+1-1).EQ.-1) THEN
        TYPMAI = 'ESCL'
      ELSE
        CALL ASSERT(.FALSE.)
      ENDIF
...
C      
      CALL CFNBEN(DEFICO,POSMAI,'MAIL',NBNOM ,IBID  )
