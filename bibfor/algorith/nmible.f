...
C
      INTEGER      IFM,NIV    
      INTEGER      MMITGO,MMITCA,MMITFR,IBID
      LOGICAL      ISFONC,LBOUCF,LBOUCG,LBOUCC,LAPINI
      LOGICAL      CFDISL,LTFCM,LXFCM,LCTCC,LBID
      REAL*8       TPSGEO,R8BID
C
C ----------------------------------------------------------------------
C 
...
C
      ...
      ENDIF
C
C --- COMPORTEMENT DIFFERENT EN REAC_GEOM='SANS' ENTRE DISCRET ET 
C --- CONTINU
C
      IF ((.NOT.LBOUCG).AND.LCTCC) THEN
        IF (ITERAT.EQ.0) THEN
          LAPINI = .TRUE.
        ENDIF
      ENDIF      
...
C  
      IF (LBOUCC) THEN  
        ...
        CALL NMNBLE(NUMINS,MODELE,NOMA  ,NUMEDD,SDDYNA,
     &              SDDISC,DEFICO,RESOCO,VALINC,SOLALG)
      ENDIF
C
 9999 CONTINUE
C
