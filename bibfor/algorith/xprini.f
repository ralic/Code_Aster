     ...
     &                  CNSLT,CNSGLS,NOESOM,NORESI,VCN,GRLR,LCMIN)
      ...
      CHARACTER*19   CNSLN,CNSLT,CNSGLS,NOESOM,NORESI,CNXINV
...
C        GRILLE  : .TRUE. SI MODEL EST UNE GRILLE AUXILIAIRE
C                  .FALSE. SI MODEL N'EST PAS UNE GRILLE AUXILIAIRE
...
C
C    EN PLUS, SI GRILLE=.FALSE. ET SI LA METHODE UPWIND EST UTILISEE,
C    ON A ON SORTIE LES OBJETS SUIVANTES:
...
C
C    SI GRILLE=.FALSE.
C        LCMIN   : LONGUEUR DE LA PLUS PETITE ARETE DE LA GRILLE
C
C     ------------------------------------------------------------------
...
C   RECUPERATION DE LA METHODE DE REINITIALISATION A EMPLOYER
      ...
      IF ((METHOD.EQ.'SIMPLEXE').AND.(.NOT.GRILLE)) THEN
