...
C 
      ...
      INTEGER      INTEGR,DIM
      INTEGER      CFMMVD,ZTABF,ZRESU,ZPERC,ZCMCF,ZMAES,ZNOES
      ...
      CHARACTER*24 TABFIN,NDIMCO,NOESCL,CONTNO,JEUCON
      CHARACTER*24 JEUSUR,CARACF,MAESCL,NOZOCO
      INTEGER      JTABF ,JDIM  ,JNOESC,JNOCO ,JJEU
...
C 
      ...
      CALL JEEXIN(SCONEL(1:8)//'.LISTE_RESU',IRET)
      IF (IRET.EQ.0) THEN
        GOTO 999
      ENDIF      
C
...
C
      ...
      DO 10 IZONE = 1,NBZONE
        IF (ZR(JCMCF+ZCMCF*(IZONE-1)+1).EQ.2.D0) THEN
...
C
 ...
 710  CONTINUE 
C
C --- ASSEMBLAGE DES SECONDS MEMBRES DU AU CONTACT
...
C
C --- ASSEMBLAGE DES SECONDS MEMBRES DU AU FROTTEMENT
C --- POUR RECUPERER LES FORCES NODALES DE FROTTEMENT
C
      CALL ASSVEC('V',FFROT,1,'&&MMCFRO',LICOEF,NUMEDD,' ','ZERO',1)
...
C
     ...
     &              ERR(3).LE.EPS) THEN
                  IF (.NOT.ZL(JPREMI-1+NUNOE)) THEN
                    CALL ASSERT(ZL(JDEPDL-1+NDD1*(NUNOE-1)+1))
                    CALL ASSERT(ZL(JDEPDL-1+NDD1*(NUNOE-1)+2))
                    CALL ASSERT(ZL(JDEPDL-1+NDD1*(NUNOE-1)+3))
                    ZL(JPREMI-1+NUNOE) = .TRUE.
                    ZL(JCNSLR-1+ZRESU*(NUNOE-1)+1) = .TRUE.
...
C
                    IZONE = ZI(JZOCO-1+INO)
                    IF (ZR(JCMCF+ZCMCF*(IZONE-1)+5).EQ.3.D0) THEN
                      IF (LAGSF.GE.0.999D0) THEN
C
C --- LE NOEUD EST GLISSANT
C
