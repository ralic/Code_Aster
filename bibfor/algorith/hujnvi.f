...
C ======================================================================
C  ----------------------------------------------------------------
C  HUJEUX   : DONNE NOMBRE CONTARINTES ET NOMBRE VARIABLES DE 
C             HUJEUX
C  ----------------------------------------------------------------
C  IN  
...
C CALL HUJNVI(MOD,IBID,IBID,NVI)
...
C - D_PLAN AXIS
        ELSEIF (MOD(1:6).EQ.'D_PLAN'.OR.MOD(1:4).EQ.'AXIS') THEN
            NDTLOC = 4
            NDILOC = 3

      
C - C_PLAN
        ELSEIF (MOD(1:6).EQ.'C_PLAN') THEN
          CALL U2MESS('F','COMPOR1_4')


C - 1D
        ELSEIF (MOD(1:2).EQ.'1D') THEN
          CALL U2MESS('F','COMPOR1_4')
C - 1D
        ELSE 
