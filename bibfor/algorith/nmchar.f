...
C --- FIN DECLARATIONS NORMALISEES JEVEUX ------------------------------
      ...
      INTEGER      JCNFE,  JCNLA,  JVITKM, IONDP,  TYPESE, ITERAT, JFEPI
      INTEGER      JDIPI,  JCNCI,  JPREC,  IRET2
...
C      CORRECTION POUR TENIR COMPTE DU TYPE DIRICHLET DIFFERENTIEL
        ...
        IF ((IALGO.EQ.4).AND.(FOPREC)) THEN
          ...
          IF (IRET.NE.0) THEN
            CALL JEVEUO(DIDOLD(1:19) // '.VALE','L',JPREC)
            CALL JEVEUO(CNDIDO(1:19) // '.VALE','E',JDIDO)
            CALL JELIRA(CNDIDO(1:19) // '.VALE','LONMAX',NEQ,K8BID)
            CALL DAXPY(NEQ,COEFML,ZR(JPREC),1,ZR(JDIDO),1)
          ENDIF
        ENDIF
C -- DEPLACEMENTS IMPOSES PILOTES                            ---> CNDIPI
        ...
        IF ((IALGO.EQ.4).AND.(FOPREC)) THEN
          ...
          IF (IRET.NE.0) THEN
            CALL JEVEUO(DIPILD(1:19) // '.VALE','L',JPREC)
            CALL JEVEUO(CNDIPI(1:19) // '.VALE','E',JDIPI)
            CALL JELIRA(CNDIPI(1:19) // '.VALE','LONMAX',NEQ,K8BID)
            CALL DAXPY(NEQ,COEFML,ZR(JPREC),1,ZR(JDIPI),1)
          ENDIF
        ENDIF
...
C -- CHARGEMENTS MECANIQUES PILOTES                         ---> CNFEPI
     ...
     &               VEFEPI, ' ')
        ...
        IF ((IALGO.EQ.4).AND.(FOPREC)) THEN
          ...
          IF (IRET.NE.0) THEN
            CALL JEVEUO(FEPILD(1:19) // '.VALE','L',JPREC)
            CALL JEVEUO(CNFEPI(1:19) // '.VALE','E',JFEPI)
            CALL JELIRA(CNFEPI(1:19) // '.VALE','LONMAX',NEQ,K8BID)
            CALL DAXPY(NEQ,COEFML,ZR(JPREC),1,ZR(JFEPI),1)
          ENDIF
        ENDIF
...
C -- CONDITIONS CINEMATIQUES IMPOSEES  (AFFE_CHAR_CINE) ---> CNCINE
        ...
        IF ((IALGO.EQ.4).AND.(FOPREC)) THEN
          ...
          IF (IRET.NE.0) THEN
            CALL JEVEUO(CINELD(1:19) // '.VALE','L',JPREC)
            CALL JEVEUO(CNCINE(1:19) // '.VALE','E',JCNCI)
            CALL JELIRA(CNCINE(1:19) // '.VALE','LONMAX',NEQ,K8BID)
            CALL DAXPY(NEQ,COEFML,ZR(JPREC),1,ZR(JCNCI),1)
          ENDIF
        ENDIF
...
C -- CHARGEMENTS FORCES FLUIDES SUR LES GRAPPES          ---> CNFEDO
     ...
     +                             VITPLU, ACCPLU, INST(2), CNFEDO )
        ...
        IF ((IALGO.EQ.4).AND.(FOPREC)) THEN
          ...
          IF (IRET.NE.0) THEN
            CALL JEVEUO(FEDOLD(1:19) // '.VALE','E',JPREC)
            CALL JEVEUO(CNFEDO(1:19) // '.VALE','E',JCNFE)
            CALL JELIRA(CNFEDO(1:19) // '.VALE','LONMAX',NEQ,K8BID)
            CALL DAXPY(NEQ,COEFML,ZR(JPREC),1,ZR(JCNFE),1)
          ENDIF
        ENDIF
