...
C
      ...
      INTEGER         NDIMF,NBNOMX,NNOC,NNOS
      INTEGER         I,J,K,NNOF,IPOIDF,IVFF,IDFDEF,JCOR1D,JCOR2D
      LOGICAL         ISMALI
      CHARACTER*24    COOR1D,COOR2D
      CHARACTER*8     K8BID
      INTEGER         DDLH,NFE,DDLS,DDLM
      REAL*8          HE,FE(4),DGDGL(4,3)
      REAL*8          XE(3),F(3,3),DFDIC(27,3)
      REAL*8          EPS(6),GRAD(3,3)
      INTEGER         IBID,IBID2,IBID3
...
C --- COORDONNÉES DES SOMMETS DE LA FACETTE DANS LE REPERE GLOBAL NDIM
      COOR2D='&&JACFF.COOR2D'
      CALL WKVECT(COOR2D,'V V R',NPTF*NDIM,JCOR2D)
      DO 10 I=1,NPTF
        DO 11 J=1,NDIM
          ZR(JCOR2D-1+(I-1)*NDIM+J)=ZR(JINTER-1+NDIM*(CFACE(IFA,I)-1)+J)
 11     CONTINUE 
 10   CONTINUE 
...
C     EN QUADRATIQUE 2D
        KSIB=1.D0
        CALL ABSCVF(NDIM,JCOR2D,KSIB,NORMAB)
        ZR(JCOR1D-1+1)=0.D0
        ZR(JCOR1D-1+2)=0.D0
        ZR(JCOR1D-1+3)=NORMAB
        ZR(JCOR1D-1+4)=0.D0
        ZR(JCOR1D-1+5)=NORMAB/2
        ZR(JCOR1D-1+6)=0.D0
        SEG(1)=0.D0
        SEG(2)=NORMAB
        SEG(3)=NORMAB/2
...
C --- CALCUL DE JAC EN 1D    
      K = (IPG-1)*NNOF
      CALL DFDM1D(NNOF,ZR(IPOIDF-1+IPG),ZR(IDFDEF+K),ZR(JCOR1D),RBID,
...
C --- COORDONNEES REELLES 2D DU POINT DE GAUSS
      KSIG2(1)=KSIG1
      KSIG2(2)=0.D0
      CALL REEREL(ELC,NNOF,NDIM,JCOR2D,KSIG2,G)
...
C     CALCUL DES FF DE L'ÉLÉMENT PARENT EN CE POINT DE GAUSS
      ...
      CALL REEREF(ELREFP,NNO,NNOS,IGEOM,G,IBID,.FALSE.,NDIM,HE,DDLH,
     &     NFE,DDLS,DDLM,FE,DGDGL,CINEM,XE,FFP,DFDI,F,EPS,GRAD)
...
C     CALCUL DES FF DE L'ÉLÉMENT DE CONTACT EN CE POINT DE GAUSS
     ...
     &     DDLH,NFE,DDLS,DDLM,FE,DGDGL,CINEM,XE,FFPC,DFDIC,F,EPS,GRAD)
