...
C
      ...
      IF ( NBAMOR.NE.0 .AND. NBAMOR.NE.NBMODE ) THEN
       CALL UTMESS('E','VRIALE','LE NOMBRE D''AMORTISSEMENT MODAUX '//
...
C
      ...
      IF ( EXCMOD.EQ.'OUI' .AND. NBMODE.NE.NINDEX ) THEN
       CALL UTMESS('A','VRIALE','IL N Y A PAS LE MEME NOMBRE DE MODES'//
...
C
      IF ( NBINDI .EQ. 0 ) THEN
         ...
         IF ( NBCMPI .NE. NBCMPJ ) THEN
            CALL UTMESS('E','VRIALE','IL FAUT AUTANT DE CMP EN I ET J')
         ENDIF
         IF ( NBCMPI .NE. NBINDI ) THEN
          CALL UTMESS('E','VRIALE','IL FAUT AUTANT DE CMP QUE DE NOEUD')
         ENDIF
      ENDIF
      IF ( NBINDJ .NE. NBINDI ) THEN
       CALL UTMESS('E','VRIALE','IL FAUT AUTANT D''INDICES EN I ET J')
      ENDIF
      NINDEX = -NBINDI
C
...
C
      IF ( (GRAEXC.EQ.'SOUR_PRESS').OR.(GRAEXC.EQ.'SOUR_FORCE')) THEN
         IF ( NNOEEX .NE. 2*NINDEX ) THEN
            CALL UTMESS('E','VRIALE','AVEC SOUR_PRESS ET SOUR_FORCE '//
     &                     'IL FAUT DEUX POINTS/DDLS D''APPLICATION')
         ENDIF
      ELSEIF ((NAPEXC.NE.NINDEX).AND.(EXCMOD.EQ.'NON')) THEN
       CALL UTMESS('E','VRIALE','MAUVAIS ACCORD ENTRE NOMBRE D APPUIS'//
...
C
      ...
      IF ( NNOEEX .NE. NCMPEX ) THEN
         CALL UTMESS('E','VRIALE', 'IL FAUT AUTANT DE NOM DE '//
...
C
      ...
      IF ((GRAEXC.EQ.'DEPL_R').AND.(NMOST1.EQ.0).AND.(NVASEX.EQ.0))THEN
         CALL UTMESS('E','VRIALE',' VOUS AVEZ OUBLIE DE PRECISER '//
     &                          'LE MODE STATIQUE')
      ELSEIF ( (GRAEXC.NE.'DEPL_R').AND.(NMOST1.NE.0) ) THEN
         CALL UTMESS('E','VRIALE',' MODE STATIQUE NON- NECESSAIRE')
      ENDIF
C
C---FREMIN < FREMAX
C
      ...
      IF ( IBID .NE. 0 ) THEN
         ...
         IF ( FREMIN .GE. FREMAX ) THEN
            CALL UTMESS('E','VRIALE','LA FREQUENCE MINI DOIT ETRE '//
