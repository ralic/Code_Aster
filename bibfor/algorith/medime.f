...
C
      CALL JEEXIN(MEDIRI(1:8)//'.LISTE_RESU',IRET)
      IF (IRET.EQ.0) THEN
        CALL MEMARE('V',MEDIRI,MODELE(1:8),' ',' ','RIGI_MECA')
        CALL WKVECT(MEDIRI(1:8)//'.LISTE_RESU','V V K24',NCHAR,JMED)
      ELSE
        CALL JEVEUO(MEDIRI(1:8)//'.LISTE_RESU','E',JMED)
      END IF
C
C --- CHAMPS DE SORTIE
C
...
C
     ...
     &                                    NBOUT,LCHOUT,LPAOUT,'V')
          ...
          END IF
   10   CONTINUE
        CALL JEECRA(MEDIRI(1:8)//'.LISTE_RESU','LONUTI',ILIRES,K8BID)
      END IF
C      
   20 CONTINUE
C
