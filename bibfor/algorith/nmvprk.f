...
C        NFS=NOMBRE DE FAMILLES DE SYSTEMES DE GLISSEMENT MAXIMUM
      ...
      REAL*8 COTHE(NMAT),DCOTHE(NMAT),PGL(3,3),SIGI(6),EPSD(9)
...
C
     ...
     &            NHSR,NUMHSR)
C
      IF (OPT.EQ.'RIGI_MECA_TANG') GOTO 9000
C
...
C     INITIALISATIONS PARTICULIERES POUR CERTAINES LOIS
      ...
      CALL LCRKIN(NDIM,OPT,COMP,MATERF,NBCOMM,NMAT,MOD,NVI,
...
C --  CALCUL DES CONTRAINTES
      ...
      IF ((LOI(1:8).EQ.'MONOCRIS').AND.(COMP(3)(1:4).EQ.'SIMO')) THEN
         CALL LCRKSG(COMP,NVI,VINF,EPSD,DETOT,NMAT,COEL,SIGI)
      ELSE
C     CALCUL DES CONTRAINTES SUIVANT QUE LE MATERIAU EST ENDOMMAGE 
         IF (LOI(1:9).EQ.'VENDOCHAB')  E=E*(1.0D0-VINF(9))
         IF (LOI(1:8).EQ.'HAYHURST')   E=E*(1.0D0-VINF(11))
   ...
   14 CONTINUE
      ...
      CALL LCDPEQ (VIND, VINF,COMP,NBCOMM,CPMONO,NMAT,NVI,SIGI,
     &                DETOT,EPSD,MATERF)
