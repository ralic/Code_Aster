...
C
      CALL MTDSCR(RAID)
      CALL JEVEUO(RAID(1:19)//'.&INT','L',LMAT)
C
C --- PROJECTION MODES-RAIDEUR-MODES
...
C
        ...
        CALL R8COPY(NEQ,ZR(LLCHAM),1,ZR(LTVEC2),1)
        CALL JELIBE(CHAMVA)
...
C
        ZR(LDK0II+KTRIAN) = R8DOT(NEQ,ZR(LTVEC1),1,ZR(LTVEC2),1)
        KTRIAN = KTRIAN + 1
        DO 20 J=I-1,1,-1
...
10    CONTINUE
C
C --- PRODUIT RAIDEUR  DEFORMEES DROITES
C
C --- REQUETTES MATRICES A REMPLIR
C
      CALL JENONU(JEXNOM(REPMAT,'K0JJ'),IBID)
...
C
        ZR(LDK0JJ+KTRIAN) = R8DOT(NEQ,ZR(LTVEC1),1,ZR(LTVEC2),1)
C
...
C
        DO 65 J=1,NBMOD
           ...
           CALL JELIBE(CHAMVA)
           CALL ZERLAG(ZR(LTVEC2),NEQ,ZI(IDDEEQ))
           XPROD= R8DOT(NEQ,ZR(LTVEC1),1,ZR(LTVEC2),1)
...
C
     ...
     &             ZR(LDKPJA),NBDDR,NBDAX)
      ENDIF
C
C --- PRODUIT RAIDEUR  DEFORMEES GAUCHES
C
...
C
      ENDIF
C
C --- PRODUIT RAIDEUR - DEFORMEES AXE
C
C --- REQUETTE DU TABLEAU A REMPLIR (MATRICE STOCKEE PLEINE)
C
      IF(NBDAX.GT.0) THEN
...
C -----(RECUPERATION EN LECTURE DE K0AA)
C
...
C
      CALL MTDSCR(MASS)
      CALL JEVEUO(MASS(1:19)//'.&INT','L',LMAT)
C
C --- PROJECTION MODES-MASSE-MODES
C     PAS INDISPENSABLE MAIS LA METHODE RESTE EXACTE SI LES
C     MODES DU SECTEUR ONT MAL CONVERGE
C
C --- REQUETTES MATRICES A REMPLIR
...
C ----- PRODUIT AVEC MODES
C
        ZR(LDM0II+KTRIAN) = R8DOT(NEQ,ZR(LTVEC1),1,ZR(LTVEC2),1)
        KTRIAN = KTRIAN + 1
        DO 45 J=I-1,1,-1
           CALL DCAPNO(BASMOD,'DEPL    ',J,CHAMVA)
           CALL JEVEUO(CHAMVA,'L',LLCHAM)
...
40    CONTINUE
C
C --- PROJECTION MASSE-DEFORMEES DROITES
C
C --- REQUETTE DES TABLEAUX A REMPLIR (MATRICE STOCKEE PLEINE)
C
      CALL JENONU(JEXNOM(REPMAT,'M0IJ'),IBID)
...
C
      ...
      DO 110 I=1,NBDDR
C
C --- PROJECTION MATRICE DEFORMEES DROITES
C
        IORD=ZI(LTORD+I-1)
        CALL DCAPNO(BASMOD,'DEPL    ',IORD,CHAMVA)
        CALL JEVEUO(CHAMVA,'L',LLCHAM)
...
C ----- CALCUL TERME DIAGONAL
C
        ZR(LDM0JJ+KTRIAN) = R8DOT(NEQ,ZR(LTVEC1),1,ZR(LTVEC2),1)
C
C ----- MULTIPLICATION PAR MODES PROPRES
C
        DO 120 J=1,NBMOD
...
120     CONTINUE
C
C ----- MUTIPLICATION PAR LES DEFORMEES DROITES (HORS TERMES DIAGONAUX)
C
        KTRIAN = KTRIAN + 1
        DO 130 J=I-1,1,-1
          IORD=ZI(LTORD+J-1)
...
130     CONTINUE
C
C ----- MUTIPLICATION PAR LES DEFORMEES AXE
C
        IF(NBDAX.GT.0) THEN
          DO 135 J=1,NBDAX
            IORD=ZI(LTORA+J-1)
...
C
     ...
     &             ZR(LDMPJA),NBDDR,NBDAX)
      ENDIF
C
C --- PROJECTION MASSE-DEFORMEES GAUCHES
C
C --- REQUETTE SUR TABLEAUX A REMPLIR (MATRICES STOCKEES PLEINES)
C
...
C
      DO 140 I=1,NBDGA
C
C ----- PROJECTION MATRICE DEFORMEES GAUCHES
C
        IORD=ZI(LTORG+I-1)
        CALL DCAPNO(BASMOD,'DEPL    ',IORD,CHAMVA)
        CALL JEVEUO(CHAMVA,'L',LLCHAM)
...
150     CONTINUE
C
C ----- MUTIPLICATION PAR LES DEFORMEES DROITES
C
        DO 160 J=1,NBDDR
          IORD=ZI(LTORD+J-1)
          CALL DCAPNO(BASMOD,'DEPL    ',IORD,CHAMVA)
...
160     CONTINUE
C
C ----- MULTIPLICATION PAR DEFORMEES GAUCHES (HORS TERMES DIAGONAUX)
C
        DO 170 J=1,I-1
          IORD=ZI(LTORG+J-1)
          CALL DCAPNO(BASMOD,'DEPL    ',IORD,CHAMVA)
...
170     CONTINUE
C
C ----- MULTIPLICATION PAR DEFORMEES AXE
C
        IF(NBDAX.GT.0) THEN
          DO 175 J=1,NBDAX
            IORD=ZI(LTORA+J-1)
...
C
      ENDIF
C
C --- PROJECTION MASSE-DEFORMEES AXE
C
C --- REQUETTE SUR TABLEAUX A REMPLIR (MATRICES STOCKEES PLEINES)
C
      IF(NBDAX.GT.0) THEN
...
C
        DO 180 I=1,NBDAX
C
C ------- PROJECTION MATRICE DEFORMEES AXE
C
          IORD=ZI(LTORA+I-1)
          CALL DCAPNO(BASMOD,'DEPL    ',IORD,CHAMVA)
          CALL JEVEUO(CHAMVA,'L',LLCHAM)
