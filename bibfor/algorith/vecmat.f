...
C    ----------------------------------------------------------------
C           FAMI   :  FAMILLE DE POINT DE GAUSS
C           KPG    :  NUMERO DE POINT DE GAUSS
...
C     PAR RCVALB) A FOINT1
C
C --- CALCUL DE IPIF: DANS RCVALB ON A
...
C RCVALB DANS LA SOUS ROUTINE DONNANT LES VALEURS DES DERIVEES DES
...
C
     ...
     &               ' ', 0.D0, 3,
     &                  NOMC(4),  MATERD(1,2),  CERR(4), 2)
         CALL RCVALB(FAMI,KPG,KSP,'-',JMAT,' ','VENDOCHAB',   0,
     &               ' ', 0.D0, 5,
     &                  NOMC(7),  MATERD(4,2),  CERR(7), 2)
         IF (MATCST.EQ.'NAP') THEN
            MATERD(9,2)=0.0D0
         ELSE
            CALL RCVALB(FAMI,KPG,KSP,'-',JMAT,' ','VENDOCHAB',   0,
     &                ' ', 0.D0, 1,
     &                   NOMC(12),  MATERD(9,2),  CERR(12), 2)
         ENDIF
C
