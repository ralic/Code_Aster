...
C
      ...
      INTEGER      MMAIT,AMAIT,NMAIT,STATUE
      INTEGER      POSMAE,POSMIN,NPTE,NFACE,NVIT,IAD,NARET
      REAL*8       GEOM(3),KSIPC1,KSIPC2,WPC
      REAL*8       T1MIN(3),T2MIN(3),XIMIN,YIMIN,R8BID
      REAL*8       JEUMIN,COOR(3),NORM(3),NOOR,RLST
      CHARACTER*8   ALIAS
...
C        CALL MMINFP(IZONE ,DEFICO,K24BLA,'SEUIL_INIT',
C     &              IBID  ,LAMBDA,K24BID,LBID  )
...
C 
        ...
        STATUE = ZI(JMAESC+ZMESX*(IMAE-1)+4)
C
...
C
            CALL XMREPT(CHSGM,CHSLO,CHSAI,NDIM,GEOM,JMAESC,STATUE,
     &                   MMAIT,AMAIT,NMAIT)
C
...
C
            CALL XMRLST(NOMA,CHSLT,POSMIN,COOR,RLST)
C
            ZR(JTABF+ZTABF*NTPC+ZTABF*(IPC-1)+31) = RLST
C
            IF (PREMIE) THEN
C
...
C
              CALL XMRLST(NOMA,CHSLT,POSMAE,COOR,RLST)
C
              ZR(JTABF+ZTABF*NTPC+ZTABF*(IPC-1)+30) = RLST
C
...
C --- SEUIL_INIT
C 
C              ZR(JTABF+ZTABF*NTPC+ZTABF*(IPC-1)+14) = LAMBDA
            ENDIF  
C
