      SUBROUTINE NMCPEL (NDIM,TYPMOD,IMATE,COMPOR,CRIT,TEMP,TREF,
...
C IN  HYDR    : HYDRATATION A L'INSTANT DU CALCUL
C IN  SECH    : SECHAGE A L'INSTANT DU CALCUL
...
C DECLARATION PARAMETRES D'APPELS
      ...
      REAL*8             CRIT(3), TEMP , TREF , HYDR , SECH
      REAL*8             EPS(6), SIG(6), VI(*), DSIDEP(6,6)
      CHARACTER*2  VALRET
      CHARACTER*16 PHENOM
      REAL*8       ENERGI(2)
...
C - ORTHOTROPIE OU ISOTROPIE TRANSVERSE LINEAIRE
     ...
     &    (PHENOM(1:6).EQ.'ELAS_O'.OR.PHENOM(1:6).EQ.'ELAS_I')) THEN
        IF ( COMPOR(1)(1:5) .EQ. 'ELAS ')THEN
          CALL NMORTH (NDIM,  TYPMOD, IMATE, COMPOR, PHENOM,
     &                 TEMP, TREF, OPTION, EPS, SIG, DSIDEP)
        ELSE
          CALL UTMESS('F','NMCOMP_01','PAS D''ORTHOTROPIE NON LINE')
        ENDIF
