...
C
      CALL GETVTX('INCREMENT','VITE_MIN',1,IARG,1,VVAR,NV)
      DO 10 I=1,NEQ
        VMIN(I) = 1.D-15
 10   CONTINUE
C
...
C
      CALL GETVR8('INCREMENT','COEF_MULT_PAS'   ,1,IARG,1,CMP,N1)
C
C     --- COEFFICIENT DE DIVISION DU PAS DE TEMPS ---
C
      CALL GETVR8('INCREMENT','COEF_DIVI_PAS'   ,1,IARG,1,CDP,N1)
C
C     --- COEFFICIENT DETERMINANT DT MIN (=DT INIT * CPMIN) --
C
      CALL GETVR8('INCREMENT','PAS_MINI'   ,1,IARG,1,DTMIN,N1)
C
C     --- COEFFICIENT DETERMINANT DT MIN (=DT INIT * CPMIN) --
C
      IF (N1.EQ.0) THEN
        CALL GETVR8('INCREMENT','PAS_LIMI_RELA'   ,1,IARG,1,CPMIN,N1)
        DTMIN = DTI * CPMIN
      ENDIF
C
C     --- NOMBRE DE POINTS PAR PERIODE
C
      CALL GETVIS('INCREMENT','NB_POIN_PERIODE',1,IARG,1,NPER,N1)
C
C     --- NMAX_REDUC_PAS
C
      CALL GETVIS('INCREMENT','NMAX_ITER_PAS',1,IARG,1,NRMAX,N1)
C
C
      VALI (1) = NPER
      VALI (2) = NRMAX
