...
C TOLE CRP_21
      ...
      CHARACTER*19 MAPREC
      CHARACTER*24 MODELE,COMPOR,NUMEDD,NUMEDE,VALMOI,DEPMOI
...
C --- FIN DECLARATIONS NORMALISEES JEVEUX ------------------------------
      ...
      INTEGER NEQ,LONDE,JCRR,JCRK,IRET,IBID,NOCC,N1, IFONC
      INTEGER DERNIE
      REAL*8 INSTAM,R8BID
      COMPLEX*16 CBID
      ...
      CHARACTER*19 LIGREL
      CHARACTER*24 K24BLA,SIGMOI,VARMOI,COMMOI,TEXTE
C ----------------------------------------------------------------------
...
C -- INCONNUES DE TYPE VARIABLES NON LOCALES
      ...
      IF (MODEDE.NE.' ') THEN
        ...
        CALL GCNCON('_',NUMEDE(1:8))
        CALL RSNUME(RESULT,'VARI_NON_LOCAL',NUPOSS)
        CALL NUMER2(NUPOSS,1,LIGREL,'DDL_NLOC',SOLVDE,'VG',NUMEDE)
...
C -- PREPARATION DE LA STRUCTURE DE DONNEE "EVOL_NOLI"
      ...
      CALL NMNOLI(RESULT,COMPOR,PARTPS,CRITNL,DERNIE,DEPMOI,SIGMOI,
     &            VARMOI,VARDEM,LAGDEM,CMD,VITPLU,ACCPLU,NBMODS,DEPENT,
     &            VITENT,ACCENT)
...
C -- SD_IMPR : FONCTIONNALITES A AFFICHER
      CALL WKVECT('&&OP0070.IMPR.FONC','V V L',3,IFONC)
      ZL(IFONC  ) = FONACT(1)
      ZL(IFONC+1) = FONACT(2)
      ZL(IFONC+2) = FONACT(3)
