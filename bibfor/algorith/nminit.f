      SUBROUTINE NMINIT(RESULT,MODELE,NUMEDD,MATE  ,
     ...
     &                  ZFON  ,FONACT,CNVFRE,PARCON,PARCRI,
...
C
      ...
      CHARACTER*13 INPSCO
      CHARACTER*14 PILOTE
      CHARACTER*19 SOLVEU,NURO,PARTPS,CRITNL
...
C
      INTEGER      NEQ,JCRR,JCRK,IRET,IBID,N1
...
C
      IF (NDYNLO(SDDYNA,'DYNAMIQUE')) THEN
        ...
        ELSE
          OPMASS='MASS_MECA'
        ENDIF
        CALL NDDOET(RESULT,MODELE,NUMEDD,MATE,
     &              COMPOR,CARELE,MEMASS,OPMASS,MASSE,
     &              MEDIRI,LISCHA,INSTAM,COMMOI,DEPMOI,
     &              VITPLU,ACCPLU,SIGMOI,VARMOI,MAPREC,
     &              SOLVEU,CARCRI,VARDEM,LAGDEM,PARTPS,
     &              NBPASE,INPSCO,DEFICO,DEPOLD,PILOTE)
      ELSE
        CALL NMDOET(RESULT,MODELE,COMPOR,CARELE,
     &              INSTAM,DEPMOI,SIGMOI,VARMOI,VARDEM,
     &              LAGDEM,NBPASE,INPSCO,DEPOLD,NUMEDD,
     &              NEQ   ,PILOTE)           
      END IF
C
...
C
     ...
     &              'ABSOLU',DERNIE,1,IBID)
      END IF
      CALL DIINIT(INSTAM,PARTPS,DERNIE,RESULT,NBOBSE,
     &            NUINS0,LOBSER,NUOBSE,NOMTAB,MAILL2,
     &            NBOBAR,LISINS(1:19),LISOBS,MATE,CARELE,
     &            SDSUIV,SDDYNA)
C
...
C --- CREATION DE LA SD POUR ARCHIVAGE DES INFORMATIONS DE CONVERGENCE
C
      CALL NMNOLI(COMPOR,PARTPS,CRITNL,DERNIE,VARDEM,
     &            LAGDEM,VITPLU,ACCPLU,DEPENT,VITENT,
     &            ACCENT,NBPASE,INPSCO,MODELE,MATE  ,
     &            CARELE,LISCH2,RESOCO,SDDYNA)
...
C -- FONCTIONNALITES ACTIVEES
      ...
      CALL NMFONC(ZFON,PARCRI,SOLVEU,MODELE,DEFICO,DEFICU,
     &            LISCHA,FONACT)
...
C -- CREATION SD POUR METHODE CONTACT CONTINU
...
C -- CREATION DE LA SD AFFICHAGE
      ...
      CALL IMPINI(SDIMPR,SDSUIV,ZFON,FONACT)
