...
C      RECUPERATION DES PHASES METALLURGIQUES
     ...
     &        PHASM(I),IRET1)
         IF (IRET1.EQ.1) PHASM(I)=0.D0
         CALL RCVARC(' ',ZIRC(I),'+',FAMI,KPG,KSP,
     &        PHASP(I),IRET1)
         IF (IRET1.EQ.1) PHASP(I)=0.D0
 1     CONTINUE
       ...
       CALL RCVALB(FAMI,KPG,KSP,'+',IMAT,' ','ELAS_META',0,' ',0.D0,6,
...
C 4 RECUPERATION CARACTERISTIQUES (PLASTIQUE)
     ...
     &              0.D0,3,NOMRES(5),VALRES(5),CODRET(5),'F ')
