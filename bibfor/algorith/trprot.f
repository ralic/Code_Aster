     ...
     &                  NUM,NU,MA,MATE,MOINT,MOFLUI,ILIRES,K,ICOR)
...
C     --- FIN DES COMMUNS JEVEUX ------------------------------------
      ...
      REAL*8        R8BID,TGEOM(6),TMIN,EPSI,CONST(2)
      ...
      CHARACTER*8   MOFLUI,MOINT,MA,K8BID,MAFLUI
...
C VERIFICATION SUPPLEMENTAIRE
      ...
      CALL JEMARQ()
           IF (MODEL.EQ.'AX') THEN
           CALL UTMESS('F','TRPROT', 'ON NE SAIT PAS ENCORE TRAITER'//
...
C CHANGEMENT DE VALEUR POUR DX ET DY (OU DZ)
        ...
        CALL DISMOI('F','NOM_NUME_DDL',NOMCHA(1:19),
     +              'CHAM_NO',IBID,NUDDL,IERD)
        CALL DISMOI('F','PROF_CHNO',NOMCHA(1:19),
     +              'CHAM_NO',IBID,PCHNO,IERD)
     ...
     +              'MAILLAGE',NBNOE,K8BID,IERD)
        ...
        CALL DISMOI('F','NOM_GD',NUDDL,'NUME_DDL',IBID,GD,IERD)
...
C----ON RESOUT L EQUATION DE LAPLACE
           ...
           CALL RESOUD(MA,MAPREC,CHFLU,SOLVEU,' ','V',CHSOL,CRITER)
