...
C 0.1. ==> ARGUMENTS
      ...
      CHARACTER*24  CNCHCI,MODELE,CARELE,FOMULT,NUMEDD,STYPSE
...
C 0.3. ==> VARIABLES LOCALES
      ...
      INTEGER      JCHAR,JINF,NCHAR,NCHA,NH,IERR,JTP,NUMCHT,NUMCHY,
     &             IRET,IAUX,JAUX,TYPCUM,NUMCHS
      REAL*8       TIME,ALPHA
     ...
     &             CHAMEL,TEMP,BLAN24,K24BID,CHTIME,CHNUMC,CHFREQ,
     ...
     &             DERITE,CHHYDR,CHSECH,CHSREF
      LOGICAL      ASS1ER,EXITIM,LBID
      COMPLEX*16   CALPHA
...
C          SIMPLIFIER LA PROGRAMMATION.
     ...
     >                  CHAMEL, LIGRMO, BASE, CHHYDR, CHSECH,CHSREF,
     >                  K24BID,K24BID,
...
C====
      ...
      ENDIF
      IF (TYPESE.EQ.0) THEN    
        CALL DETRSD('VECT_ELEM',VELAPL(1:8))
        CALL DETRSD('CHAMP_GD',CHLAPL(1:8))
      ENDIF
      IF ((TYPESE.LE.0) .OR. (TYPESE.GE.5)) THEN                
      ...
      ENDIF
      IF ((TYPESE.EQ.-1) .OR. (TYPESE.EQ.3)) THEN               
        CALL DETRSD('VECT_ELEM',VECOMP(1:8))
        CALL DETRSD('CHAMP_GD',CHCOMP(1:8))
      ENDIF      
      IF (TYPESE.EQ.0) THEN
        IF (NUMCHT.GT.0.OR.NUMCHY.GT.0.OR.NUMCHS.GT.0) THEN
