...
C -------------- FIN  DECLARATIONS  NORMALISEES  JEVEUX ----------------
      ...
      PARAMETER (ZCONV=6)
      LOGICAL CONJUG,GCPC
      INTEGER IFM,NIV,IZONE,NZOCO,JTOLE,FREQ,ISMAEM
      INTEGER II,ITER,ILIAC,JRESU,JDECAL
      ...
      REAL*8 AJEU,VAL,R8PREM,R8VIDE,DDOT,TOLE,RELA
...
C       CRITERE_PRECOND = RELA*EPSI
        RELA = MIN(RELA,ZR(JTOLE+ZTOLE*(IZONE-1)+6))
C       TYPE DE PRECONDITIONNEUR
        IF (ZI(JCONV+ZCONV*(IZONE-1)+4).EQ.1) PRECON = 'DIRICHLET'
C       TYPE DE RECHERHCE LINEAIRE
        IF (ZI(JCONV+ZCONV*(IZONE-1)+5).EQ.1) SEARCH = 'NON_ADMISSIBLE'
   10 CONTINUE
...
C----PRECONDITIONNEMENT DIRICHLET
     ...
     &              ZR(JAPCOE),ZI(JAPDDL),ZI(JAPPTR),ZI(JLIAC),LMAT,
     &              ZR(JSGRAP),ZR(JSGPRE))
