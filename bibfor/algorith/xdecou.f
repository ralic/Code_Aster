...
C         RECUP COOR GLOBALES
  ...
  310     CONTINUE
C           CALCUL DES FF
          CALL REEREF(ELP,NNOP,IBID,IGEOM,A,IBID,LBID,NDIM,
...
C         INTERPOLATION LSN
  ...
  10      CONTINUE
        ...
        ELSE
C         CALCUL DES FF
          CALL REEREF(ELP,NNOP,IBID,IGEOM,B,IBID,LBID,NDIM,
     &              RBID,IBID,IBID,IBID,IBID,IBID,IBID,RBID,RBID,'NON',
     &              RBID2,FF,RBID,RBID,RBID,RBID)
C         INTERPOLATION LSN
          LSNB=0
...
C           ON AJOUTE A LA LISTE LE POINT A
            CALL XAJPIN(JPTINT,PTMAX,IPT,INS,A,LONREF,JAINT,0,NA,0.D0)
          ENDIF
          IF (LSNB.EQ.0) THEN
C           ON AJOUTE A LA LISTE LE POINT B
          CALL XAJPIN(JPTINT,PTMAX,IPT,INS,B,LONREF,JAINT,0,NB,0.D0)
          ENDIF
...
C           INTERPOLATION DES COORDONNEES DE C
 ...
 130        CONTINUE
            IF (NFISS.GE.2) THEN
C         CALCUL DES FF
              CALL REEREF(ELP,NNOP,IBID,IGEOM,C,IBID,LBID,NDIM,
...
C           ON AJOUTE A LA LISTE LE POINT C
          CALL XAJPIN(JPTINT,PTMAX,IPT,IBID,C,LONREF,JAINT,IA,0,0.D0)
          ENDIF
        ENDIF
 100  CONTINUE
