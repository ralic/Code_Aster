...
C     IN THIS ITERATION (ONE SINGLE PROPA_FISS CALL)
      CALL GETVID(' ','LISTE_FISS',1,IARG,0,FISLST,NUMLST)
      NUMLST = NUMLST*(-1)
      CALL WKVECT('&&XPRCYC.LISTFISS','V V K8',NUMLST,JLISTE)
      CALL GETVID(' ','LISTE_FISS',1,IARG,NUMLST,ZK8(JLISTE),IBID)
...
C        MATERIAL
         CALL GETVTX('LOI_PROPA','LOI',1,IARG,1,LOI,IBID)
         IF (LOI.EQ.'PARIS') THEN
             CALL GETVR8('LOI_PROPA','C',1,IARG,1,C,IBID)
             CALL GETVR8('LOI_PROPA','N',1,IARG,1,N,IBID)
             CALL GETVR8('LOI_PROPA','M',1,IARG,1,M,IBID)
             CALL GETVR8('LOI_PROPA','E',1,IARG,1,MATE,IBID)
             CALL GETVR8('LOI_PROPA','NU',1,IARG,1,MATNU,IBID)
         ENDIF

C        RETRIEVE THE LIST OF THE SIF TABLES
         CALL GETVID(' ','TABLE',1,IARG,0,TABLE,NUMTAB)
         NUMTAB = NUMTAB*(-1)
         CALL WKVECT('&&XPRCYC.SIF','V V K8',NUMTAB,JSIF)
         CALL GETVID(' ','TABLE',1,IARG,NUMTAB,ZK8(JSIF),IBID)

C        CHECK IF THE NB_POINT_FOND OPTION HAS BEEN USED
         CALL GETVIS(' ','NB_POINT_FOND',1,IARG,0,NPF,IBID)
         IF (IBID.EQ.0) THEN
C           DEACTIVATE THE FLAG FOR THE NB_POINT_FOND OPTION
            NBPTFO = .FALSE.
            CHLINE = .FALSE.
...
C           OPTION
            ...
            CALL GETVIS(' ','NB_POINT_FOND',1,IARG,NPF,ZI(JNPF),IBID)
...
C        CHECK IF THE OPTION COMP_LINE HAS BEEN USED
         CALL GETVR8('COMP_LINE','COEF_MULT_MAXI',1,IARG,1,CMAX,IBID)
         IF (IBID.EQ.0) THEN
...
C           ACTIVATE THE FLAG FOR THE COMP_LINE OPTION
            CHLINE=.TRUE.
            CALL GETVR8('COMP_LINE','COEF_MULT_MINI',1,IARG,1,CMIN,IBID)
