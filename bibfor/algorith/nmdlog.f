...
C
      ...
      REAL*8 GN(3,3),LAMB(3),LOGL(3),RBID,TBID(6)
      REAL*8 DEF(2*NDIM,NNO,NDIM),PFF(2*NDIM,NNO,NNO)
      REAL*8 DSIDEP(6,6),PK2(6),PK2M(6)
...
C     ATTENTION DFF NON CALCULE. PB SI MONOCRISTAL
      CALL LCEGEO(NNO,NPG,IW,IVF,IDFF,GEOMI ,TYPMOD,COMPOR,NDIM,DFF,
     &            DEPLM ,DEPLD ,ELGEOM)
...
C---     CALCUL DE F_N, F_N+1 PAR RAPPORT A GEOMI GEOM INITIAL
        ...
        CALL PRELOG(NDIM,LGPG,VIM(1,G),GN,LAMB,LOGL,FM,FP,EPSML,DEPS,TN,
     &              RESI,COD(G))
        ...
        CALL NMCOMP(FAMI,G,1,NDIM,TYPMOD,MATE,COMPOR,CRIT,INSTM,INSTP,
     &              6,EPSML,DEPS,6,TN,VIM(1,G),OPTION,ANGMAS,10,
     &              ELGEOM(1,G),TP,VIP(1,G),36,DTDE,1,RBID,COD(G))
...
C        TEST SUR LES CODES RETOUR DE LA LOI DE COMPORTEMENT
        ...
        CALL POSLOG(RESI,RIGI,TN,TP,FM,LGPG,VIP(1,G),NDIM,FP,G,DTDE,
     &              SIGM(1,G),CPLAN,FAMI,MATE,INSTP,ANGMAS,GN,LAMB,
