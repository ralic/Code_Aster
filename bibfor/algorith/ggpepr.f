      SUBROUTINE GGPEPR(S,DPC,TEMP,FLUPHI,VALDRP,TTAMAX,THETA,
     *                  DEUXMU,PREC,NITER,G,DGDST,DGDEV)
...
C ======================================================================
       IMPLICIT NONE
       REAL*8   S,DPC,TEMP,FLUPHI,VALDRP,TTAMAX,DEUXMU,PREC
       REAL*8   THETA
...
C     VALDRP:R: PARAMETRE R_P
C     TTAMAX:R: PARAMETRE THETA_MAX
C     THETA :R: PARAMETRE DU SCHEMA D'INTEGRATION (0.5 OU 1)
C                  THETA = 0.5 -> SEMI-IMPLICITE
C                  THETA = 1.0 -> IMPLICITE
C     PREC  :R: PRECISION DE LA RESOLUTION EN TEMPS(ROUTINE TPSEPR)
C     NITER :I: NOMBRE ITERATIONS DE LA RESOLUTION EN TEMPS
C               (ROUTINE TPSCYR)
...
C---------------------------------------------------------------
C            DANS LE CAS DE LA LOI DE L'EPRI,
...
C
      IF (S.EQ.0.D0.OR.DPC.EQ.0.D0)THEN
        ...
        GO TO 99
      ELSE
        CALL TPSEPR(TPS,S,DPC,TEMP,FLUPHI,VALDRP,TTAMAX,PREC,NITER)
        CALL FGDEPR(TPS,S,TEMP,FLUPHI,VALDRP,TTAMAX,F1,FP1,FS1,F2,
