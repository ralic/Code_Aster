...
! ======================================================================
    ...
    real(kind=8) :: bmph, ums, phids, rinstp
...
! ======================================================================
    ...
    real(kind=8) :: dpvpt, dpvpl, biot, k0, cs, alpha0, alpliq, cliq
    real(kind=8) :: cp11, cp12, sat, dsatp1, mamolv, em
    real(kind=8) :: r, rho0, csigm, alp11, alp12, rho12
    real(kind=8) :: eps
...
! ======================================================================
    ...
    real(kind=8) :: rbid50, rbid51, rbid52, rbid53, rbid54, rbid55
    real(kind=8) :: rbid45, rbid46, rbid47, rbid48, rbid49, rbid56
    real(kind=8) :: rbid57, rbid58
    real(kind=8) :: r3bid(6)
    real(kind=8) :: m11m, m12m, coeps, pinf, dp2, cp21, cp22, rho21
    real(kind=8) :: rho22, dpad, signe
!
    logical :: net, bishop
!
! =====================================================================
...
! =====================================================================
    call thmrcp('SATURATI', imate, thmc, meca, hydr,&
                ther, rbid1, rbid2, rbid3, rbid4,&
                rbid5, rbid50, pvp-p1, rbid41, rbid6,&
                rbid7, rbid8, rbid10, rbid51, rbid52,&
                rbid53, rbid54, rbid41, sat, dsatp1,&
                rbid14, rbid15, rbid16, rbid17, rbid18,&
                rbid19, rbid20, rbid21, rbid22, rbid23,&
...
! =====================================================================
            call viporo(nbvari, vintm, vintp, advico, vicphi,&
                        phi0, depsv, alpha0, dt, dp1-dpvp,&
                        dpvp, signe, sat, cs, biot,&
                        phi, phim, retcom)
        endif
        if (emmag) then
...
! --- CAR ON MULTIPLIE DANS VIPORO PAR -1) ----------------------------
! ======================================================================
            congep(adcote) = congep(adcote) + calor(alpha0,k0,t,dt, depsv,dp1-dpvp,dpvp,signe,alp&
                             &11,alp12, coeps)
        endif
    endif
! =====================================================================
...
! ======================================================================
            dsde(adcome+6,addep1)=dsde(adcome+6,addep1) + dspdlq(biot,&
            sat,dpvpl)
! ======================================================================
! --- CALCUL DES DERIVEES DE SIGMAP PAR RAPPORT A LA TEMPERATURE -------
! ======================================================================
            if (yate .eq. 1) then
                dsde(adcome+6,addete) = dsde(adcome+6,addete) + dsipdt(biot,sat,dpvpt)
            endif
! ======================================================================
...
! ======================================================================
            dsde(adcp11,addete) = dsde(adcp11,addete) + dmwdt2(rho11, alp11,phids,sat,bmph,cs,dpv&
                                  &pt)
...
! --- POUR LES AUTRES CAS ----------------------------------------------
! ======================================================================
        dsde(adcp11,addep1) = dsde(adcp11,addep1) + dmwp1v(rho11, phids,sat,bmph,cs,dpvpl,phi,cli&
                              &q)
        dsde(adcp12,addep1) = dsde(adcp12,addep1) + dmvpp1(rho11, rho12,phids,ums,bmph,cs,dpvpl,s&
                              &at,phi,pvp)
    endif
! =====================================================================
30  continue
