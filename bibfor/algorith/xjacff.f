...
C
      ...
      REAL*8          EPS(6),GRAD(3,3),COOR2D(6)
...
C     CALCUL DE JAC EN 2D
      CALL DFDM2D(NNOF,IPG,IPOIDF,IDFDEF,COOR2D,RBID1,RBID2,JAC)
...
C --- COORDONNEES DE REFERENCE 2D DU POINT DE GAUSS
      CALL REEREG('S',ELC,NNOF,COOR2D,XG,NDIMF,KSIG,IBID)
...
C         ESSAI AVEC LE PROJETE DE OY
      ...
      CALL REEREF(ELREFP,AXI,NNO,NNO,ZR(IGEOM),G,IBID,.FALSE.,
     &             NDIM,HE, RBID, RBID,
...
C     CALCUL DES FF DE L'ÉLÉMENT DE CONTACT EN CE POINT DE GAUSS
      ...
      CALL REEREF(ELREFC,AXI, NNOC,NNOC,ZR(IGEOM),G,IBID,.FALSE.,
     &             NDIM,HE,RBID, RBID,
     &             IBID,IBID,DDLH,NFE,DDLS,DDLM,FE,DGDGL,CINEM,
