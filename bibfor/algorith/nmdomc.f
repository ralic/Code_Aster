...
C
      IF(MODELE.EQ.' ') THEN
        CALL GETVID(' ','MODELE'    ,0,IARG,1,MODE,N1)
        MODELE = MODE
      ENDIF
...
C
      ...
      DO 100 ICH=1,NCHAR
        CALL GETVID('EXCIT','CHARGE',ICH,IARG,1,ZK24(IALICH+ICH-1),N1)
C
C ----- TYPES DE CHARGES UTILISEES
C
C        CALL GETVTX('EXCIT','TYPE_CHARGE',ICH,1,1,TYPCHA,N1)
        TYPCHA='FIXE' 
C
...
C
        CALL GETVID('EXCIT','FONC_MULT_C',ICH,IARG,1,NOMFCT,N1)
        CALL GETVID('EXCIT','FONC_MULT',ICH,IARG,1,NOMFCT,N11)
        IF ((N1.EQ.0).AND.(N11.EQ.0)) THEN
           CALL CODENT( ICH , 'D0' , KNUM  )
           NOMFCT = '&&NC'//KNUM
           CALL ASSERT(LXLGUT(NOMFCT).LE.24)
           CALL GETVC8('EXCIT','COEF_MULT_C',ICH,IARG,1,CCOEF,N2)
           IF ( N2 .EQ. 0 ) THEN
              CALL GETVR8('EXCIT','COEF_MULT',ICH,IARG,1,COEF,N3)
