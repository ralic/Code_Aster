      SUBROUTINE XLAGSP(ALGOLA,DIMENS,NOMA,CNSLT,CNSLN,FISS)
...
C     ------------------------------------------------------------------
      ...
      IF (DIMENS.EQ.3) CALL WKVECT('&&XLAGSP.TABINT','V V R',3*MXAR,
     &                              JTAB2)
      IF (DIMENS.EQ.2) CALL WKVECT('&&XLAGSP.TABINT','V V R',2*MXAR,
     &                              JTAB2)
...
C         NOUVELLE ARETE
          ...
          DO 130 I=1,DIMENS
              A(I)=ZR(JCOOR-1+DIMENS*(NUNOA-1)+I)
              B(I)=ZR(JCOOR-1+DIMENS*(NUNOB-1)+I)
              C(I)=A(I)-LSNA/(LSNB-LSNA)*(B(I)-A(I))
              ZR(JTAB2-1+DIMENS*(NBARTO-1)+I)=C(I)
 130      CONTINUE
...
C       CREATION DES LISTES DES RELATIONS DE LIAISONS ENTRE LAGRANGE
        CALL XRELL1(ZI(JTAB1),DIMENS,NBARTO,ZR(JTAB2),ZI(JPINO),NBPINO,
     &              FISS)
...
C       CREATION DES LISTES DES RELATIONS DE LIAISONS ENTRE LAGRANGE
        CALL XRELL2(NBARTO,DIMENS,ZI(JTAB1),ZR(JTAB2),FISS)
