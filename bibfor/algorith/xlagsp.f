      SUBROUTINE XLAGSP(NDIM  ,NOMA  ,FISS  ,ALGOLA)
...
C
      ...
      CHARACTER*19 CNSBAS
      INTEGER      IFM,NIV
      CHARACTER*19 LTNO,LNNO,GRLTNO,GRLNNO
...
C
      IF (NBARTO.GT.0) THEN
        CALL CNSCNO(CNSBAS,' ','NON','G',FISS//'.BASCO')
      ENDIF  
C
C --- CREATION DES LISTES DES RELATIONS DE LIAISONS ENTRE LAGRANGE
C 
      CALL XLAGSC(NDIM  ,NBNO  ,NBARTO,MXAR  ,FISS  ,
...
C      
      IF (NIV.GE.2) THEN
        ...
        CALL UTIMSD(IFM,-1,.TRUE.,.TRUE.,
     &              FISS(1:8)//'.CONTACT.LISEQ',1,' ')
        CALL UTIMSD(IFM,-1,.TRUE.,.TRUE.,
     &              FISS(1:8)//'.CONTACT.LISRL',1,' ')
        CALL UTIMSD(IFM,-1,.TRUE.,.TRUE.,
     &              FISS(1:8)//'.CONTACT.LISCO',1,' ')  
      ENDIF
