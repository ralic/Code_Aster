...
C
 ...
 12       CONTINUE
        ELSE
          CALL GETVIS(MOTFAC,'LIST_CAS',IOCC,IARG,0,IBID,NC)
          NC=-NC
          IF (NC.LT.2) THEN
     ...
     &                                                   NC,' ')
          CALL JEVEUO(JEXNUM('&&ASENAP.LISTCAS',IOCC),'E',JCAS)
          CALL GETVIS(MOTFAC,'LIST_CAS',IOCC,IARG,NC,ZI(JCAS),NC)
          NCAS =NCAS+NC
        ENDIF
 10   CONTINUE
...
C
      ...
      DO 20 IOCC =1, NBOCC
        CALL GETVTX(MOTFAC,'TYPE_COMBI',IOCC,IARG,1,CTYP,NC)
...
C
      ...
      DO 30 ICAS =1, NOCAS
        CALL GETVIS(MOTFAC,'NUME_CAS',ICAS,IARG,1,NUCAS,NC)
C
C INITIALISATION DU DEPLACEMENT DE  NOEUD_REFE
C
C
C -- STOCKAGE MODE STATIQUE DU NUME_CAS TRAITE
         CALL GETVID(MOTFAC,'MODE_STAT',ICAS,IARG,1,STAT,NS)
...
C -- STOCKAGE DES DIRECTIONS D''ANCRAGE
 ...
 36       CONTINUE
           CALL GETVR8(MOTFAC,'DX',ICAS,IARG,1,DX,NX)
           CALL GETVR8(MOTFAC,'DY',ICAS,IARG,1,DY,NY)
           CALL GETVR8(MOTFAC,'DZ',ICAS,IARG,1,DZ,NZ)
C
          DO 38 INO =1,NBNO
