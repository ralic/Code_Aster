     ...
     &  TREF,HYDRM,HYDRP,SECHM,SECHP,SREF,ANGMAS,
     &  DDLM,DDLD,SIGM,LGPG,VIM,SIGP,VIP,MATR,VECT,CODRET,DFDI2)
...
C TOLE CRP_21
       ...
       REAL*8  HYDRM(NPG),HYDRP(NPG),SECHM(NNO1),SECHP(NNO1),SREF
...
C IN  SECHM   : SECHAGE AUX NOEUDS A L'INSTANT PRECEDENT
C IN  SECHP   : SECHAGE AUX NOEUDS A L'INSTANT DE CALCUL
C IN  SREF    : SECHAGE DE REFERENCE
C IN  HYDRM   : HYDRATATION AUX PTS GAUSS A L'INSTANT PRECEDENT
C IN  HYDRP   : HYDRATATION AUX PTS GAUSS A L'INSTANT DE CALCUL
...
C ----------------------------------------------------------------------
      ...
      INTEGER IBID,NDIMSI,NDDL,G,COD(27),N,I,M,J,KL,PQ,OS,KK
      INTEGER IU(3,27),IE(6,8)
      REAL*8  RBID,RAC2,LC,C,DEPLM(3,27),DEPLD(3,27),TM,TP,DFDI1(27,3)
      REAL*8  R,WG,EPSGM(6,2),EPSGD(6,2),GEPSM(6,3),GEPS(6,3),F(3,3)
      REAL*8  B(6,3,27),DE(6),TAMPON(10),SIGMA(6),DSIDEP(6,6,2),T1,T2
      REAL*8  P(6,6),SM,SP,SIGMAM(6),PERT
      REAL*8  DDOT
...
C      VARIABLES DE COMMANDE
        ...
        SM = DDOT(NNO1,VFF1(1,G),1,SECHM,1)
        SP = DDOT(NNO1,VFF1(1,G),1,SECHP,1)
...
C      LOI DE COMPORTEMENT
        ...
        CALL NMCOMP(NDIM,TYPMOD,MATE,COMPOR,CRIT,INSTAM,INSTAP,
     &          TM,TP,TREF,HYDRM(G),HYDRP(G),SM,SP,SREF,
     &          RBID,RBID,EPSGM,EPSGD,SIGM(1,G),
     &          VIM(1,G),OPTION,RBID,RBID,IBID,RBID,RBID,
     &          RBID,RBID,ANGMAS,P,
