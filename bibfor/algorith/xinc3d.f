      SUBROUTINE  XINC3D(POUM,NNOP,IPOIDS,IVF,DDLH,NFE,DDLC,IGEOM,
...
C            CONFIGURATION MANAGEMENT OF EDF VERSION
C MODIF ALGORITH  DATE 10/07/2007   AUTEUR MARKOVIC D.MARKOVIC 
C ======================================================================
...
C         DEBUT DE LA ZONE MEMOIRE DE SIG et VI CORRESPONDANTE
          IF (NDIM .EQ. 3) THEN
             NPG=15
             NCMPS=6
             IDEBS=(NSEMAX*NPG*(IT-1)+NPG*(ISE-1))*NCMPS+1
             IDEBV=(NSEMAX*NPG*(IT-1)+NPG*(ISE-1))*LGPG+1

             CALL XKIN3D(POUM,ELREFP,NDIM,COORSE,IGEOM,HE,DDLH,DDLC,NFE,
     &                   INSTAM,INSTAP,DEPLP,SIGM(IDEBS),VIP(IDEBV),
     &                   BASLOC,NNOP,NPG,TYPMOD,OPTION,IMATE,COMPOR,
     &                   LGPG,CRIT,DEPL,LSN,LST,
     &                   SIG(IDEBS),VI(IDEBV),MATUU,VECTU,CODRET)
          ELSE
            NPG=3
            NCMPS=4  
            IDEBS=(NSEMAX*NPG*(IT-1)+NPG*(ISE-1))*NCMPS+1
            ...
            CALL XKIN2D(POUM,ELREFP,NDIM,COORSE,IGEOM,HE,DDLH,DDLC,NFE,
     &                 INSTAM,INSTAP,DEPLP,SIGM(IDEBS),VIP(IDEBV),
     &                 BASLOC,NNOP,NPG,TYPMOD,OPTION,IMATE,COMPOR,LGPG,
     &                 CRIT,DEPL,LSN,LST,SIG(IDEBS),
     &                 VI(IDEBV),MATUU,VECTU,CODRET)
