...
C  IN  : MASSE   : MATRICE DE MASSE DU MODELE COMPLET
C  IN  : RAIDE   : MATRICE DE RAIDEUR DU MODELE COMPLET
C  IN  : NEQ     : NOMBRE D'EQUATIONS DU NUME_DDL COMPLET
...
C  IN  : SWITCH  : 1 SI ON DOIT RELEVER LES MODES SUR TOUTE LA SST
C                  0 SI ON NE CONSERVE QUE LES MODES SUR L'INTERFACE
C
C-----------------------------------------------------------------------
C
C
...
C-- VARIABLES DE LA ROUTINE
     ...
     &             LMATMA,IRET,NBVECT,IBID,DECAL,JWORK,LDELG,
     &             NO,NBSST,LINDIN,COEFF,LVP,LINTRF,INFO
      REAL*8       TEMP,PI,RBID,NORM,DDOT,LAMBDA,COMLIN(2),SWORK(1),
     &             RAND,MAX,ABS
...
C-- POUR L'EXPANSION : ON NE GARDE EN SORTIE QUE LE SEV SUR L'INTERFACE
      IF (SWITCH .EQ. 0) THEN
...
C-- MISE A 0 DES DDL DE LAGRANGE
  ...
  200 CONTINUE
      ...
      CALL MRMULT('ZERO',LMATM,ZR(LMAKRY),ZR(LMATRM),NSEKRY,.TRUE.)
      CALL JEVEUO(RAIDE(1:19)//'.&INT','L',LMATK)
      CALL MRMULT('ZERO',LMATK,ZR(LMAKRY),ZR(LMATRK),NSEKRY,.TRUE.)
