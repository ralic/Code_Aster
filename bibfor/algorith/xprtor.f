     ...
     &    VCN,VCND,CNSLN,GRLN,CNSLT,GRLT,TORE,RADTOR,RADIMP,
     &    CNSDIS,DISFR,CNSBL,NODCAL,ELECAL,LIGGRD,VCNT,VCNDT)
      IMPLICIT NONE

      CHARACTER*8    METHOD,MODEL,NOMA,FISPRE,FISS
      CHARACTER*19   CNSLN,GRLN,CNSLT,GRLT,VCN,VCND,NODCAL,ELECAL,
     &               CNSDIS,DISFR,CNSBL,VCNT,VCNDT,CNXINV,LIGGRD
      LOGICAL        TORE
...
C      VCND   = METHODE UPWIND SEULEMENT. VOIR XPRCNU.F POUR LA
C               DESCRIPTION DE CET OBJET.
...
C      VCNDT  = METHODE UPWIND SEULEMENT. VECTEUR DISTANCE NODALE TORE.
C               VOIR XPRCNU.F POUR LA DESCRIPTION DE CETTE OBJET.
...
C     GENERAL PURPOSE
      INTEGER      IBID,IRET,I,J,K,JDIME,NDIM,IFM,NIV,ELDIM
      CHARACTER*8  K8B
      CHARACTER*1  K1BID
      REAL*8       MESERR(2)
...
C     TORUS
     ...
     &             JCONX1,JCONX2,NOCUR,NUMELM,JMAI,ITYPMA,JLISEL,NBMA,
...
C     NODAL CONNECTION TABLE
      INTEGER      JVCN,JVCND,JVCNT,JVCNDT,JLISOL
      CHARACTER*24 LISOLD
...
C        IF THE AUXILIARY GRID IS NOT USED, THE LOGICAL LIST OF THE
C        NODES INSIDE THE NEW TORUS MUST BE CREATED
         IF(FISPRE.NE.FISS) THEN
            CALL WKVECT(FISS//'.TORE','G V L',NNODGR,JNOCAL)
...
C        TORUS OF THE ACTUAL CRACK
         LISOLD='&&OP0010.TORE'
         CALL JEDUPO(FISPRE//'.TORE','V',LISOLD,.FALSE.)
         CALL JEVEUO(LISOLD,'L',JLISOL)

C        CREATE OR RETREIVE THE LOGICAL LIST FOR THE TORUS OF THE
C        PROPAGATED CRACK
         IF(FISPRE.NE.FISS) THEN
            CALL WKVECT(FISS//'.TORE','G V L',NNODGR,JLISNO)
         ELSE
...
C        CREATE THE VECTORS FOR THE NODAL CONNECTION TABLE OF THE TORUS
         CALL WKVECT(VCNT,'V V I',6*NNODTO,JVCNT)
         CALL WKVECT(VCNDT,'V V R',6*NNODTO,JVCNDT)
