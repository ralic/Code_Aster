     ...
     &                  VITKM1, ACCKM1, VITPLU, ACCPLU, IALGO)
...
C -------------- FIN  DECLARATIONS  NORMALISEES  JEVEUX ----------------
      ...
      REAL*8 PHI
      INTEGER I,IC,ICOMP,IRAN(3),IALGO
C ----------------------------------------------------------------------
      CALL JEMARQ()
...
C -- MISES A JOUR
      IF ((.NOT.REAROT).AND.(IALGO.EQ.6)) THEN
        DO 10 I=1,NEQ
         ZR(JVITP+I-1) = ZR(JVITKM+I-1) + UNDEMI*COEVIT*ZR(JACCP+I-1)
     &                   + UNDEMI*COEVIT*ZR(JACCKM+I-1)
  10    CONTINUE 
        ELSEIF ((.NOT.REAROT).AND.(IALGO.EQ.7)) THEN
         DO 20 I=1,NEQ
          ZR(JVITP+I-1) = ZR(JVITKM+I-1) + COEVIT*ZR(JACCKM+I-1)
  20    CONTINUE
        ELSE
