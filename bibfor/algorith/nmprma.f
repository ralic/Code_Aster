...
C
      ...
      LOGICAL      LDYNA,LAMOR,LSUIV,LCRIGI,LCFINT,LEXPL,LPRMO
      LOGICAL      NDYNLO,ISFONC
      LOGICAL      LBID,LASSE
      LOGICAL      LSHIMA
      REAL*8       R8BID,COEFB(2),NDYNRE,COEMAS
      ...
      CHARACTER*24 LIMAB(2)
      CHARACTER*4  TYPCSB(2)
...
C
      ...
      LSHIMA = .FALSE.
      IF (LDYNA) THEN
        COEMAS = NDYNRE(SDDYNA,'COEF_MASS_SHIFT')
        IF (ABS(COEMAS).GT.R8PREM()) LSHIMA = .TRUE.
      ENDIF
C
C --- INITIALISATIONS
C
...
C      IF (IRET.EQ.0.AND.LDYNA) 
C     & WRITE(6,*) 'LCRIGI2 ',LCRIGI,' LPRMO ',LPRMO,' LSHIMA ',LSHIMA 
      IF (LDYNA.AND.LEXPL.AND.IRET.EQ.0.AND.LSHIMA)  
     & LCRIGI=.TRUE.
      IF (LCRIGI) THEN
        IF (IRET.EQ.0) THEN
         LASSE=.TRUE.
     ...
     &              VEELEM,LDCCVG,CODERE)
      ...
      IF (IRET.EQ.0 .AND. LDYNA) THEN
        IF (LSHIMA) THEN 
         ...
         LIMAB(2) = MEASSE(1)
         IF (LEXPL) THEN
          CALL MTCMBL(2,TYPCSB,COEFB,LIMAB,MEASSE(2),' ',' ','ELIM=')
         ELSE
          CALL MTCMBL(2,TYPCSB,COEFB,LIMAB,MEASSE(2),'LAGR',' ','ELIM=')
         ENDIF
        ENDIF
      END IF
