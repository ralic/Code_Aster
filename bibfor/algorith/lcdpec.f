...
C     ----------------------------------------------------------------
      ...
      REAL*8   RHOSAT,PHISAT,DZ,ROLOOP(12),FIVOID(12),SDP,DPS(30)
      CHARACTER*16 NOMFAM,COMP(*),NECOUL
...
C           TAU      : SCISSION REDUITE TAU=SIG:MUS
     ...
     &               TAU(NS+IS),MUS,MSNS)
            CALL LCMMLC(NMAT,NBCOMM,CPMONO,NFS,NSG,HSR,NSFV,NSFA,IFA,
     &       NBSYS,IS,DT,NVI,VIND,YD,DY,ITMAX,TOLER,MATERF,EXPBP,
     &       TAU(NS+IS),DALPHA,DGAMMA,DP,CRIT,SGNS,RP,IRET)
  71     CONTINUE
         NS=NS+NBSYS
         NSFA=NSFA+NBSYS
         NSFV=NSFV+NBSYS*3
