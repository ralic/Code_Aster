...
C
      NBMA = 0
      CALL GETVTX ( MOTFAC, MCMAIL, 1,1,0, K8BID, NBMA )
      IF (NBMA.LT.0) THEN
        NBMA = -NBMA
        CALL WKVECT('&&CYC110.NOM.MA','V V K8',NBMA,LTNMMA)
        CALL GETVTX(MOTFAC,MCMAIL,1,1,NBMA,ZK8(LTNMMA),NBID)
      END IF
C
C-------TRAITEMENT DES GROUPES DE MAILLES EN ENTREE---------------------
C
      NBUF =0
      CALL GETVTX ( MOTFAC, MCGRM, 1,1,0, K8BID, NBGR )
      IF (NBGR.LT.0) THEN
        NBGR = -NBGR
        CALL WKVECT('&&CYC110.NOM.GRMA','V V K8',NBGR,LTNMGR)
        CALL GETVTX(MOTFAC,MCGRM,1,1,NBGR,ZK8(LTNMGR),NBID)
