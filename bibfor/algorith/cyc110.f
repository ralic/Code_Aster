...
C
      NBMA = 0
      CALL GETVID ( MOTFAC, MCMAIL, 1,1,0, K8BID, NBMA )
      IF (NBMA.LT.0) THEN
        NBMA = -NBMA
        CALL WKVECT('&&CYC110.NOM.MA','V V K8',NBMA,LTNMMA)
        CALL GETVID(MOTFAC,MCMAIL,1,1,NBMA,ZK8(LTNMMA),NBID)
      END IF
C
C-------TRAITEMENT DES GROUPES DE MAILLES EN ENTREE---------------------
C
      NBUF =0
      CALL GETVID ( MOTFAC, MCGRM, 1,1,0, K8BID, NBGR )
      IF (NBGR.LT.0) THEN
        NBGR = -NBGR
        CALL WKVECT('&&CYC110.NOM.GRMA','V V K8',NBGR,LTNMGR)
        CALL GETVID(MOTFAC,MCGRM,1,1,NBGR,ZK8(LTNMGR),NBID)
        CALL COMPMA(MAILLA,NBGR,ZK8(LTNMGR),NBUF)
      END IF
C
C-----------CAS DE LA RESTITUTION DU MAILLAGE EN ENTIER-----------------
C
      CALL GETVTX ( MOTFAC, 'TOUT', 1,1,0, K8BID, IOCTOU )
      IF (IOCTOU.LT.0) THEN
        IOCTOU = 1
