subroutine dtmprep_noli_revi(sd_dtm_, sd_nl_, icomp)
    implicit none
! ----------------------------------------------------------------------
! ======================================================================
...
! ======================================================================
! dtmprep_noli_revi : prepare the calculations for a localized nonlinearity
!                     of type : RELA_EFFO_VITE. This routine adds one or more 
...
!   --- 1.2 - Case with double (or triple) projections (sub-structuring case)
        ...
        if (iret .eq. 0) call utmess('F', 'ALGORITH5_64')
...
!             # Si le GROUP_NO contient plus d'un noeud
    ...
    call nlsav(sd_nl, _FV_FONCT, 1, iocc=i, kscal=fonc)
