      SUBROUTINE XSTANO(NOMA,LISNO,NMAFIS,JMAFIS,CNSLT,CNSLN,STANO)
      IMPLICIT NONE 
...
C ======================================================================
C                      TROUVER LES MAILLES OU LSN CHANGE DE SIGNE
C
C     ENTREE
C         NOMA
C         CNSLN,CNSLT 
...
C
      INTEGER         INO,IMAE,IN,AR(12,2),IA,I
      ...
      REAL*8          LSNC,LSTC,LSN,A(3),B(3),C(3),R8MAEM
      CHARACTER*8     TYPMA
      CHARACTER*19    MAI
      CHARACTER*32    JEXATR
      LOGICAL         MARE
C ----------------------------------------------------------------------
      ...
      LISNO = '&&XSTANO.LISTE_NO_ENRICH'
      CALL RELIEM(' ',NOMA,'NU_NOEUD',' ',1,1,
     &                     'GROUP_MA_ENRI','GROUP_MA',LISNO,NBNOE)
...
C               AUCUN POINT DE L'ARETE N'A LSN = 0,ALORS ON RETIENT RIEN
                NRIEN=NRIEN+1
              ENDIF
              IF (NRIEN.EQ.NBAR) CALL UTMESS('E','OP0137','AUCUNE '//
     &                               'ARETE SUR LAQUELLE LSN S ANNULE')
 212        CONTINUE
C           BOUCLE SUR LES NOEUDS DE LA MAILLE RETENUE
            DO 213 IN=1,NBNOMA
              NUNO=ZI(JCONX1-1+ZI(JCONX2+NMAABS-1)+IN-1)
...
C       TESTS SI IL Y A EU UNE MAILLE DE VOISINAGE TROUVÉE
        IF (PARE.LT.NMAFIS) THEN
          IF ((MINLSN*MAXLSN.LT.0).AND.(MAXLST.LE.0))        ENR1=1
          IF ((MINLSN*MAXLSN.LE.0).AND.(MINLST*MAXLST.LE.0)) ENR2=2
        ENDIF
        ENR=ENR1+ENR2
