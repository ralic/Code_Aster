...
C ---> DEFINITION DU DOMAINE POTENTIEL DE MECANISMES ACTIFS
        IF(DEBUG)WRITE(6,*)'HUJPOT --- VIND = ',(VIND(I),I=1,50)
     ...
     &     NEGMUL, IRET, SUBD, LOOP, NSUBD)
         ...
         DO 49 I = 1, NDI
           IF(SIGF(I).GT.PTRAC)THEN
             NOCONV=.TRUE.
             IF(DEBUG)WRITE(6,*)'SOL EN TRACTION'
           ENDIF
 49      CONTINUE
     ...
     &               GOTO 500
         IF (NOCONV) THEN
           IF(DEBUG)WRITE(6,*)' PROBLEME AVEC NOCONV ' 
           IRET = 1
           GOTO 9999
         ENDIF         
         IF(IRET.EQ.1)GOTO 9999
...
C --- PREDICTEUR MIS A JOUR TENANT COMPTE DE L'ETAT PRECEDEMMENT OBTENU
            ...
            IF(MAJ.LT.10)THEN
              ...
              GOTO 100
            ELSE      
              IF(DEBUG)WRITE(6,*)'SOLUTION EXPLICITE'
              GOTO 45
            ENDIF            
          ENDIF
