...
C
      ...
      INTEGER      IBID,IRET,MMITGO,IOCC,NOCC,I
      REAL*8       R8VIDE,R8B,PASMIN
      REAL*8       RESIGR
      REAL*8       DIINST,INSTAM,INSTAP
      CHARACTER*16 K16BLA,NOMDIV(2),EVEN
      INTEGER      JCRI,JCRR,IDIV
      ...
      LOGICAL      LBID
      LOGICAL      LRESI,LELTC,LBOUCL,MAXNOD
      INTEGER      IRELA,IMAXI,IREFE,INNODA
      INTEGER      IFM,NIV
      CHARACTER*8  NODDLM,K8B,KSTO
C
C ----------------------------------------------------------------------
C
      CALL JEMARQ()
...
C
      CALL ASSERT( LICCVG(5).EQ.0 .OR.
     &             LICCVG(5).EQ.1 .OR.
     &             LICCVG(5).EQ.3 )

      CALL UTDIDT('L',SDDISC,'LIST',IBID,'ALGO_DECOUPE',R8B,IBID,KSTO)
C     ECHEC SSI MATRICE SINGULIERE ET STOP_SINGULIER ='DECOUPE'
      ECHEQU = LICCVG(5).EQ.1 .AND. KSTO.EQ.'DECOUPE'
...
C
          CALL IMPSDR(SDIMPR,'BOUC_NOEU',K16BLA,R8B   ,IBID)
...
C
      ...
      IF (CBORST) THEN
         IF (CONVER) THEN
           CALL IMPSDR(SDIMPR,'ITER_DEBO',' DE BORST...    ',R8B,IBID)
...
C        A T'ON LE DROIT DE FAIRE DES ITERATIONS SUPPLEMENTAIRES
         CALL NMLERR(SDDISC,'L','ITERSUP',ITERAT,R8B,IBID)
C        IBID=3  ON A LE DROIT, DONC PAS DE MESSAGE D'ERREUR
C        IBID<>3 ON A PAS LE DROIT, DONC MESSAGE
         IF (IBID .NE. 3) THEN
