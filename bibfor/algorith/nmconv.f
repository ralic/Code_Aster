...
C
      LOGICAL      LCTCD,LCTCC,LIMPEX,LRELI,DIVRES,LNKRY
      LOGICAL      ISFONC
      LOGICAL      ERROR
      ...
      LOGICAL      CTCFIX
      LOGICAL      CBORST,BORCVG,CVNEWT,CTDCVG,CTCCVG
      LOGICAL      MAXNOD
      INTEGER      NBITER,ITESUP,CTCITE
      INTEGER      IFM,NIV
...
C
     ...
     &            SDDYNA,SDIMPR,MATASS,NUMINS,CONV  ,
     &            RESIGR,ETA   ,COMREF,VALINC,VEASSE,
...
C
      IF (LCTCD) THEN
        CALL CFCONV(NOMA  ,DEFICO,RESOCO,SOLALG,CVNEWT,
     &              CTCFIX,CTDCVG,GEONOE,GEOVAL)
      ...
      ENDIF
C
C --- AFFICHAGE ATTENTE POINT FIXE
C
      IF (CTCFIX) THEN
...
C
      IF (LIMPEX) THEN
        CONVER = .TRUE.
      ELSE
        CONVER = CVNEWT.AND.BORCVG.AND.CTDCVG.AND.CTCCVG
      ENDIF
C
...
C --- EVALUATION DE LA DIVERGENCE DU RESIDU -> DIVRES
C
      CALL NMDIVR(SDDISC,ITERAT,DIVRES)
...
C
      IF (ERROR.OR.ITEMAX.OR.DIVRES) THEN
...
C
      CALL NMERGE(SDERRO,'SET','ITX',ITEMAX)
C
C --- SAUVEGARDE ERREUR DIVERGENCE RESIDU
C
      CALL NMERGE(SDERRO,'SET','RES',DIVRES)
C
C --- MISE A JOUR DE L'INDICATEUR DE SUCCES SUR LES ITERATIONS DE NEWTON
C
      CALL NMADEV(SDDISC,CONVER,ITERAT,ITEMAX,DIVRES,ERROR)
C
C --- CALCUL CRITERE DE CONVERGENCE POUR NEWTON-KRYLOV (FORCING-TERM)
C
      IF (LNKRY) THEN
        CALL DISMOI('F','SOLVEUR',MATASS,'MATR_ASSE',IBID,SOLVEU,IRET)
