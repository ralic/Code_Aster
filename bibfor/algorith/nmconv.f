...
C
      ...
      LOGICAL      LBID   
      LOGICAL      LDIRI,ISDIRI,LRESI,LCTCG    
...
C
C --- PREDICTION DEPL_CALCULE OU EXTRAPOL
C --- PAS DE CALCUL DE CONVERGENCE SI PAS DE CL DIRICHLET
C          
      IF ((METHOD(5) .EQ. 'EXTRAPOL').OR.
...
C
      CALL IMPSDR(SDIMPR,'ITER_NEWT',K16BLA,R8BID,ITERAT)
C
...
C
      IF (ITERAT.EQ.0) THEN
        CONV(10) = 0
      ENDIF
      CALL IMPSDR(SDIMPR,'RELI_ITER',K16BLA,R8BID,INT(CONV(10)))
...
C
      ...
      IF (IRET.GT.0) THEN
        ...
        CALL IMPSDR(SDIMPR,'ITER_FETI',K16BLA,R8BID,ZI(JCRI))
        R8BID = DBLE(ZI(JCRI))
