...
C
      ...
      IF(IER.EQ.0) THEN
        CALL UTDEBM ('E','COMPMA','LE MAILLAGE MAIL NE CONTIENT PAS
     &DE GROUP_MA')
        CALL UTIMPK('L','MAIL= ',1,MAILLA)
        CALL UTFINM
      ENDIF
C
...
C
        IF(NUM.EQ.0) THEN
          CALL UTDEBM('E','COMPMA','LE GROUP_MA GP N''EXISTE PAS DANS
     & LE MAILLAGE MAIL')
          CALL UTIMPK('L','MAIL=',1,MAILLA)
          CALL UTIMPK('L','GP=',1,NOMCOU)
          CALL UTFINM
        ENDIF
C
