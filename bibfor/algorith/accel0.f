...
C
      INTEGER      IERR,NEQ,LONCH,JCHMP,JCGMP,JCHTP,
     &             JDIRP,JACC0,I,IRET,ISLVI,ISTOP,JTP, IECPCO
...
C     -- DECOMPOSITION OU CALCUL DE LA MATRICE DE PRECONDITIONNEMENT
      ...
      ISTOP = ZI(ISLVI-1+3)
      IF (ISTOP.EQ.1) ZI(ISLVI-1+3) = 2
      ISTOP = ZI(ISLVI-1+3)
      
      IF (IECPCO.NE.0) THEN
 
        CALL JEVEUO ( ACCMOI(1:19)//'.VALE', 'L', JACC0)
        CALL JELIRA ( ACCMOI(1:19)//'.VALE','LONMAX',LONCH,K1B)
  
        VARINT =0.D0
        DO 25 I = 0,LONCH-1
          VARINT =  VARINT + ABS(ZR(JACC0+I))
25    CONTINUE     

        IF (VARINT .NE. 0.D0) THEN       
        CALL UTMESS('F','ACCEL0',
     &   'ON NE TRAITE PAS LES CAS OU A0 EST NON NULLE')
        ELSE
        CALL NULVEC(ACCMOI)
        GOTO 9999
        ENDIF      
      ...
      IF (IERR.EQ.2.AND.ISTOP.EQ.2) THEN
        CALL NULVEC(ACCMOI)
        CALL UTMESS('A','ACCEL0',
     &   'MATRICE MASSE NON INVERSIBLE : ACCELERATION INITIALE NULLE')
        CALL JEVEUO(SOLVEU//'.SLVI','E',ISLVI)
        ZI(ISLVI-1+3) = 1
        GOTO 9999
      ENDIF
      CALL JEVEUO(SOLVEU//'.SLVI','E',ISLVI)
      IF (ISTOP.EQ.2) ZI(ISLVI-1+3) = 1
