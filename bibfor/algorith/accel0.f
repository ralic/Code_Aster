...
C
      ...
      CHARACTER*19 ACCMOI,FEXMOI
      CHARACTER*8  K8BID
      INTEGER      IFM,NIV,I,IFEXMO,ICNDON
      LOGICAL      LENER,ISFONC
C
C ----------------------------------------------------------------------
C
      CALL JEMARQ()
...
C
     ...
     &            SDDYNA,SDSTAT,SDTIME,SDDISC,DEFICO,
     &            RESOCO,MEELEM,MEASSE,MAPREC,MATASS,
     &            FACCVG)
      IF (FACCVG.EQ.2) THEN
        CALL NULVEC(ACCMOI)
...
C
C --- AJOUT DE LA FORCE EXTERIEURE FICTIVE QUI ENGENDRE
C --- CETTE ACCELERATION INITIALE AU VECTEUR DES FORCES 
C --- EXTERIEURES INITIALES POUR LE CALCUL DES ENERGIES
C
      IF (LENER) THEN
        CALL JEVEUO(FEXMOI//'.VALE','E',IFEXMO)
        CALL JEVEUO(CNDONN//'.VALE','L',ICNDON)
        DO 10 I=1,NEQ
          ZR(IFEXMO-1+I)=ZR(IFEXMO-1+I)+ZR(ICNDON-1+I)
10      CONTINUE
      ENDIF
C
