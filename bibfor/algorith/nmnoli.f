      SUBROUTINE NMNOLI(COMPOR,PARTPS,CRITNL,DERNIE,VARDEM,
     &                  LAGDEM,VITPLU,ACCPLU,DEPENT,VITENT,
     &                  ACCENT,NBPASE,INPSCO,MODELE,MATE  ,
     &                  CARELE,LISCHA,RESOCO,SDDYNA)
C
C            CONFIGURATION MANAGEMENT OF EDF VERSION
C MODIF ALGORITH  DATE 04/04/2007   AUTEUR ABBAS M.ABBAS 
C ======================================================================
C COPYRIGHT (C) 1991 - 2001  EDF R&D                  WWW.CODE-ASTER.ORG
C THIS PROGRAM IS FREE SOFTWARE; YOU CAN REDISTRIBUTE IT AND/OR MODIFY
C IT UNDER THE TERMS OF THE GNU GENERAL PUBLIC LICENSE AS PUBLISHED BY
C THE FREE SOFTWARE FOUNDATION; EITHER VERSION 2 OF THE LICENSE, OR
C (AT YOUR OPTION) ANY LATER VERSION.

C THIS PROGRAM IS DISTRIBUTED IN THE HOPE THAT IT WILL BE USEFUL, BUT
C WITHOUT ANY WARRANTY; WITHOUT EVEN THE IMPLIED WARRANTY OF
C MERCHANTABILITY OR FITNESS FOR A PARTICULAR PURPOSE. SEE THE GNU
C GENERAL PUBLIC LICENSE FOR MORE DETAILS.

C YOU SHOULD HAVE RECEIVED A COPY OF THE GNU GENERAL PUBLIC LICENSE
C ALONG WITH THIS PROGRAM; IF NOT, WRITE TO EDF R&D CODE_ASTER,
C    1 AVENUE DU GENERAL DE GAULLE, 92141 CLAMART CEDEX, FRANCE.
C ======================================================================
C RESPONSABLE MABBAS M.ABBAS
C
      IMPLICIT NONE
      INTEGER DERNIE,NBPASE
      CHARACTER*8 NOPASE
      CHARACTER*13 INPSCO
      CHARACTER*19 PARTPS,CRITNL, LISCHA,SDDYNA
      CHARACTER*24 COMPOR,VARDEM,LAGDEM
      CHARACTER*24 VITPLU,ACCPLU,DEPENT,VITENT,ACCENT
      CHARACTER*24 MODELE,MATE,CARELE
      CHARACTER*24 RESOCO
C 
C ----------------------------------------------------------------------
C
C ROUTINE MECA_NON_LINE (SD EVOL_NOLI)
C
C PREPARATION DE LA SD EVOL_NOLI
C      
C ----------------------------------------------------------------------
C
C
C IN  COMPOR : COMPORTEMENT
C IN  PARTPS : SD DISC_INST
C IN  CRITNL : INFORMATIONS RELATIVES A LA CONVERGENCE
C IN  DERNIE : NUMERO DU DERNIER NUMERO ARCHIVE (OU 0 SI NOUVEAU)
C IN  DEPMOI : DEPLACEMENT DE L'ETAT INITIAL
C IN  SIGMOI : CONTRAINTES DE L'ETAT INITIAL
C IN  VARMOI : VARIABLES INTERNES DE L'ETAT INITIAL
C IN  VARDEM : VARIABLES NON LOCALES DE L'ETAT INITIAL
C IN  LAGDEM : MULTIPLICATEURS DE LAGRANGE NON LOCAUX INITIAUX
C IN  NBPASE : NOMBRE DE PARAMETRES SENSIBLES
C IN  INPSCO : SD CONTENANT LISTE DES NOMS POUR SENSIBILITE
C IN  RESOCO : SD DE TRAITEMENT NUMERIQUE DU CONTACT
C IN  SDDYNA : SD DEDIEE A LA DYNAMQIUE
C      
C ----------------------------------------------------------------------
C
      INTEGER      NUMARC,DINUAR,NRPASE,IAUX,JAUX
      INTEGER      IFM,NIV
      INTEGER      NUMINS
      LOGICAL      FORCE,INCR,NDYNLO
      CHARACTER*8  RESULT
      CHARACTER*24 VALINI(8),K24BLA
      CHARACTER*24 DEPMOI,SIGMOI,VARMOI
      CHARACTER*19 CNSINR      
C      
C ----------------------------------------------------------------------
C
      CALL INFNIV(IFM,NIV)
C
C --- INITIALISATIONS
C
      K24BLA = ' '
C
C ---  NOUVEAU CONCEPT
C
      IF (DERNIE.EQ.0) THEN
C          
C --- NOM DES CHAMPS
C      
        NUMARC = DINUAR(PARTPS)
        DO 10 NRPASE = NBPASE,0,-1
          IAUX = NRPASE
          JAUX = 3
          CALL PSNSLE(INPSCO,IAUX,JAUX,RESULT)
          JAUX = 5
          CALL PSNSLE(INPSCO,IAUX,JAUX,DEPMOI)
          JAUX = 9
          CALL PSNSLE(INPSCO,IAUX,JAUX,SIGMOI)
          JAUX = 11
          CALL PSNSLE(INPSCO,IAUX,JAUX,VARMOI)
          IF (NDYNLO(SDDYNA,'DYNAMIQUE')) THEN
            JAUX = 14
            CALL PSNSLE(INPSCO,IAUX,JAUX,VITPLU)
            JAUX = 16
            CALL PSNSLE(INPSCO,IAUX,JAUX,ACCPLU)
            JAUX = 18
            CALL PSNSLE(INPSCO,IAUX,JAUX,DEPENT)
            JAUX = 20
            CALL PSNSLE(INPSCO,IAUX,JAUX,VITENT)
            JAUX = 22
            CALL PSNSLE(INPSCO,IAUX,JAUX,ACCENT)
          END IF
C          
C --- CREATION DE LA SD EVOL_NOLI
C
          CALL RSCRSD(RESULT,'EVOL_NOLI',100)
C
C --- ARCHIVAGE DE L'ETAT INITIAL
C
          IF (NUMARC.EQ.0) THEN
C
            CALL AGGLOM(DEPMOI,SIGMOI,VARMOI,K24BLA,VARDEM,LAGDEM,
     &                  K24BLA,K24BLA,6,VALINI)            
C
C --- CREATION D'UN VALE_CONT INITIAL VIERGE
C
            CALL CFMXR0(RESOCO,MODELE,CNSINR)

            CALL NMIMPR('IMPR','ARCH_INIT',' ',0.D0,0)
            IF (NRPASE.GT.0) THEN
              IAUX = NRPASE
              JAUX = 1
              CALL PSNSLE(INPSCO,IAUX,JAUX,NOPASE)
              WRITE (IFM,*)
     &          'ARCHIVAGE DES CHAMPS DERIVES PAR RAPPORT A '//NOPASE
            END IF
C            
            NUMINS = 0
            FORCE  = .TRUE.
            INCR   = NRPASE.EQ.0            
C
            CALL NMARCH(RESULT,NUMINS,PARTPS,FORCE ,COMPOR,
     &                  CRITNL,VALINI,CNSINR,VITPLU,ACCPLU,
     &                  DEPENT,VITENT,ACCENT,INCR  ,MODELE,
     &                  MATE  ,CARELE,LISCHA,SDDYNA)

          END IF

   10   CONTINUE


C  --  CONCEPT REENTRANT

      ELSE

C      SUPPRESSION DES NUMEROS D'ARCHIVAGE A ECRASER
        NUMARC = DINUAR(PARTPS)
        DO 20 NRPASE = 0,NBPASE
          IAUX = NRPASE
          JAUX = 3
          CALL PSNSLE(INPSCO,IAUX,JAUX,RESULT)
          CALL RSRUSD(RESULT,NUMARC)
   20   CONTINUE


      END IF


      END
