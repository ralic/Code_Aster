...
C ----------------------------------------------------------------------
C
      INTEGER       I,NBCHOC,NBREDE,NBREVI
...
C
      CALL GETVID(' ','MATR_AMOR',0,1,1,AMOGEN,NAGEN)
      CALL GETVR8('AMOR_MODAL','AMOR_REDUIT',1,1,0,R8BID,NARED)
...
C
      IF (METHOD(1:4).EQ.'ITMI') THEN
        CALL GETVID('SCHEMA_TEMPS','BASE_ELAS_FLUI',1,1,0,K8BID,NBASFL)
        ...
        CALL GETVTX('SCHEMA_TEMPS','ETAT_STAT',1,1,1,OUINON,IBID)
        IF (OUINON(1:3).EQ.'OUI') THEN
          CALL GETVR8('SCHEMA_TEMPS','TS_REG_ETAB',1,1,0,R8BID,NTS)
        ...
        CALL GETVTX('CHOC','NOEUD_2',1,1,0,K8BID,N1)
...
C
      ...
      KF = 0
      DO 20 I=1,NBEXC
        CALL GETVID('EXCIT','VECT_ASSE',I,1,0,K8BID,NM)
        IF (NM .NE. 0) THEN
          KF = KF+1
        ENDIF
 20   CONTINUE
