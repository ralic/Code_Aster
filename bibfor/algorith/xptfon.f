...
C
      INTEGER         IN,IMA,IFT,I,J,IBID,IRET,NDIM,INO,IPT,K,IFQ
...
C         VÉRIFICATION SI CE POINT A DÉJÀ ÉTÉ TROUVÉ
          DO 612 J=1,IN
            ...
            IF (PADIST(NDIM,P,M).LT.(LONCAR*PREC)) GOTO 200
 612      CONTINUE
...
C         ET DE LA BASE LOCALE (GRADIENT DE LSN ET LST)
          DO 556 K=1,NDIM
            ZR(JFON-1+4*(IN-1)+K)          =   M(K)
            ZR(JBAS-1+2*NDIM*(IN-1)+K)     = GLN(K)
            ZR(JBAS-1+2*NDIM*(IN-1)+K+NDIM)= GLT(K)
 556      CONTINUE
