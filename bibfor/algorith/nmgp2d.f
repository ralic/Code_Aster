       SUBROUTINE NMGP2D(NNO, NPG, POIDSG, VFF, DFDE, DFDK, GEOMI,
...
C.......................................................................
      ...
      REAL*8  GEOMM(2,9), GEOMP(2,9), DFDN
...
C     CALCUL DE F POUR DT  =>  DF, RM  ET DFDIM
        CALL NMGEOM(2,NNO,AXI,GRAND,GEOMM,KPG,POIDSG(KPG),
     &              VFF(1,KPG),DFDE,DFDN,DFDK,DDEPL,POIDS,DFDIM,
     &              DF,EPS,RM)

C     CALCUL DE DFDIP, RP ET POIDS EN T+
        CALL NMGEOM(2,NNO,AXI,GRAND,GEOMP,KPG,POIDSG(KPG),
...
C - LOI DE COMPORTEMENT
     ...
     &            ELGEOM,SIGP(1,KPG),VIP(1,KPG),DSIDEP,COD(KPG))
...
C        TERMES COMPLEMENTAIRES EN AXISYMETRIQUE
 ...
 720        CONTINUE
            DO 740 N = 1,NNO
              MIAX3(N) = DSIDEP(3,3,3)*VFF(N,KPG)/(RM*RP)
 740        CONTINUE
            DO 800 M = 1,NNO
              DO 810 N = 1,NNO
                DO 820 A = 1,2
                  MATNS(A,N,1,M) = MATNS(A,N,1,M)
     &                           + POIDS * MIAX1(A,N) * VFF(M,KPG)
 820            CONTINUE
 810          CONTINUE
              DO 830 N = 1,NNO
                DO 840 A = 1,2
                  MATNS(1,M,A,N) = MATNS(1,M,A,N)
     &                           + POIDS * MIAX2(A,N) * VFF(M,KPG)
 840            CONTINUE
 830          CONTINUE
              DO 850 N = 1,NNO
                MATNS(1,M,1,N) = MATNS(1,M,1,N)
     &                         + POIDS * MIAX3(N) * VFF(M,KPG)
 850          CONTINUE
 800        CONTINUE
          END IF
        ENDIF
...
C        TERMES COMPLEMENTAIRES EN AXISYMETRIQUE
          IF (AXI) THEN
            DO 190 N = 1,NNO
              VECTU(1,N) = VECTU(1,N) + POIDS *SIGMP(3)*VFF(N,KPG)/RP
 190        CONTINUE
          ENDIF
        ENDIF
