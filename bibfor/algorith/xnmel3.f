      SUBROUTINE  XNMEL3(POUM,ELREFP,NDIM,COORSE,IGEOM,HE,NFH,DDLC,
     &                   DDLM,NFE,BASLOC,NNOP,NPG,TYPMOD,OPTION,
     &                   IMATE,COMPOR,LGPG,CRIT,IDEPL,LSN,LST,IDECPG,
     &                   SIG,VI,MATUU,IVECTU,CODRET,NFISS,FISNO)

      IMPLICIT NONE
      INCLUDE 'jeveux.h'
      INTEGER       NDIM,IGEOM,IMATE,LGPG,CODRET,NNOP,NPG,NFH,DDLC,NFE
      INTEGER       IDECPG,NFISS,FISNO(NNOP,NFISS),IVECTU,IDEPL
      CHARACTER*(*) POUM
...
C       JUSTE POUR CALCULER LES FF
        CALL REEREF(ELREFP,.FALSE.,NNOP,NNOPS,ZR(IGEOM),XG,IDEPL,GRDEPL,
     &              NDIM,HE,RBID,RBID,FISNO,NFISS,NFH,NFE,DDLS,
     &              DDLM,FE,DGDGL,'NON',XE,FF,DFDI,F,EPS,GRAD)
...
C       ET CALCUL DE FF, DFDI, ET EPS
     ...
     &              GRDEPL,NDIM,HE,RBID,RBID,FISNO,NFISS,NFH,NFE,DDLS,
     &              DDLM,FE,DGDGL,'OUI',XE,FF,DFDI,F,EPS,GRAD)

C       SI OPTION 'RIGI_MECA', ON INITIALISE À 0 LES DEPL
        ELSEIF ( OPTION .EQ. 'RIGI_MECA') THEN

          CALL REEREF(ELREFP,.FALSE.,NNOP,NNOPS,ZR(IGEOM),XG,IDEPL,
     &                .FALSE.,NDIM,HE,RBID,RBID,FISNO,NFISS,NFH,NFE,
     &                DDLS,-1,FE,DGDGL,'INI',XE,FF,DFDI,F,EPS,GRAD)
...
C
          DO 180 N=1,NNOP
            ...
            DO 181 I=1,DDLD
              DO 182 KL=1,6
                ZR(IVECTU-1+NN+I)=ZR(IVECTU-1+NN+I)+
     &            DEF(KL,N,I)*SIGMA(KL)*JAC
 182          CONTINUE
 181        CONTINUE
 180      CONTINUE
C
