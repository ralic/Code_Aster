      SUBROUTINE VTGPLK ( GEOMI, ALPHA, DEPLA, BASE, GEOMF )
      IMPLICIT   NONE
...
C ======================================================================
C
C     BUT: CREER UN CHAMP DE VITESSE SUR LA GEOMETRIE
C          C'EST UNE UTILISATION DEVIEE DE LA ROUTINE VTGPLD
C
C          IL Y A BCP DE CHOSES INUTILES, MAIS JE GARDE EN ATTENDANT
C          MIEUX
C          M. KHAM 
C
C          ON FAIT:
C            X +ALPHA*VX --> ALPHA*VX
C            Y +ALPHA*VY --> ALPHA*VY
C            Z +ALPHA*VZ --> ALPHA*VZ
C     IN:
C         GEOMI : CHAM_NO(GEOM_R) : CHAMP "BIDON"
...
C
      ...
      IF (LDIM/3.NE.NBNO) CALL U2MESS('F','ALGORITH_19')
