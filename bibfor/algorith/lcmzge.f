...
C ======================================================================
      ...
      REAL*8             SIG(6), VIP(3), DSIDPT(12,6), TM, TP, TREF
C ----------------------------------------------------------------------
...
C ----------------------------------------------------------------------
      ...
      LOGICAL     RIGI, RESI, ELAS, PROG, CPLAN
      CHARACTER*2 CODRET(6)
      CHARACTER*8 NOMRES(6) 
      INTEGER     NDIMSI, NPERM, NITJAC, TRIJ, ORDREJ
      INTEGER     I,J,K,L
      REAL*8      EPSM(6), EPSRM(6), DEPS(6), DEPSR(6), TRR(6)
      REAL*8      VECPER(3,3), EPSPR(3), DSIDEP(6,6), EPSPLU(6)
      REAL*8      DSIDPR(6,6), COPLAN, RTEMPC, RTEMPT
      REAL*8      RAC2, EPS(6), D , TOL, TOLDYN, TR(6),TU(6)
      REAL*8      VECPE(3,3), EPSP(3), JACAUX(3), EPSR(6)
      REAL*8      EPSEQ, LAMBDA, DEUXMU, ALPHA
      REAL*8      SIGEL(6), TREPS, TMP1, ALPHAT, EPST(3), TRSIG
      REAL*8      DC, DT, AC, AT, BC, BT, BETA, EPSD0, SIGELP(3)
      REAL*8      E, NU, COEF, TMAXM, TEMP, TMAX, EPSE(6), EPSER(6)
      REAL*8      VALRES(6)
      REAL*8      R8DOT
      REAL*8      EPSTIL, KRON(6)
...
C    A FAIRE EVOLUER L'ENDOMMAGEMENT)
      ...
      DO 35 K=1,NDIMSI
        ...
        EPSER(K) = EPSR(K) - ALPHA * (TEMP - TREF) * KRON(K)
35    CONTINUE 
...
C -     CALCUL DES CONTRAINTES ELASTIQUES (repère principal)
...
C -- CONTRIBUTION ELASTIQUE
        ...
        DO 110 K = 1,3
          DO 120 L = 1,3
            DSIDEP(K,L) = (1-D)*LAMBDA
120       CONTINUE
110     CONTINUE
        DO 130 K = 1,NDIMSI
          DSIDEP(K,K) = DSIDEP(K,K) + (1-D)*DEUXMU
130     CONTINUE
...
180       CONTINUE        
          DO  190 I=1,6
            DO 200 J=1,6
              DSIDPR(I,J) = - COEF * SIGEL(I)* EPSPLU(J)  
200         CONTINUE
190       CONTINUE
        ENDIF         
