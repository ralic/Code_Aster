...
C     PARAMETER       ( NPROPS = 200) A CAUSE DE FTNCHK
      PARAMETER       ( NPROPS = 197)
      INTEGER         CODREL(NPROPS),NEPS,NSIG,NDSDE,IADZI,IAZK24
      REAL*8          CRIT(*),ANGMAS(*),TM,TP,TREF,PROPS(NPROPS+3)
      REAL*8          INSTAM,INSTAP,DROT(3,3),DSTRAN(6)
...
C     LECTURE DES PROPRIETES MATERIAU (MOT-CLE UMAT DE DEFI_MATERIAU)
      ...
      IF ((NIV.GE.2).AND.(IDBG.EQ.1)) THEN
         WRITE(IFM,*)' '
         WRITE(IFM,*)'AVANT APPEL UMAT, COEF MATERIAU=',NOMRES
         IDBG=0
      ENDIF
...
C CAS DES GRANDES DEFORMATIONS : ON VEUT F- ET F+
     ...
     &        OPTION(1:9) .EQ. 'FULL_MECA'     ) THEN
         ...
         ENDIF         
         CALL R8INIR(NTENS, 0.D0, STRAN, 1)
