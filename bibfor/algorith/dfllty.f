...
C --- LECTURE METHODE
C
      CALL GETVTX(MOTFAC,'METHODE',1,1,1,METLIS,IBID)
C   
...
C
      CALL GETVID(MOTFAC,'LIST_INST',1,1,1,LISINS,N1)
C
...
C
      IF (METLIS.EQ.'AUTO')THEN
        ...
        CALL GETVTX('ADAPTATION','MODE_CALCUL_TPLUS',NADAPT,1,1,
     &              MODETP,IBID)
...
C 
      IF (METLIS.EQ.'AUTO') THEN
C        
        CALL GETVR8(MOTFAC,'PAS_MAXI' ,1,1,1,PASMAX,IRET)
...
C         PASMIN = CELLE DE VAL_MIN DE PAS_MINI (DEFI_LIST_INST.CAPY)
        ...
        ENDIF
C
        CALL GETVR8(MOTFAC,'PAS_MINI'    ,1,1,1,PASMIN,IRET)
        IF (PASMIN.GT.DTMIN) CALL U2MESS('F','DISCRETISATION_1')      
C
        CALL GETVIS(MOTFAC,'NB_PAS_MAXI' ,1,1,1,NBPAMX,IRET)   
C
        ZR(JLINR-1+2) = PASMIN
        ZR(JLINR-1+3) = PASMAX
        ZR(JLINR-1+4) = NBPAMX
