...
C
      ...
      LOGICAL   TEST,LINSTI
      CHARACTER*8  K8BID,MASGEN,RIGGEN
...
C     EVALUATION DE LA CONDITION DE COURANT EN EXPLICITE
      IF (CMD(1:15).EQ.'DYNA_TRAN_EXPLI') THEN
        ...
        ELSE
          CALL GETVID('PROJ_MODAL','MASS_GENE',1,1,1,MASGEN,NM)
          CALL GETVID('PROJ_MODAL','RIGI_GENE',1,1,1,RIGGEN,NR)
C  IL FAUT LA PRESENCE DES DEUX MATRICES GENERALISEES POUR
C  CALCULER LA CONDITION DE COURANT       
          IF (NM.NE.0.AND.NR.NE.0) CALL PASCOM(MASGEN,RIGGEN)
        ENDIF
      ENDIF
