      SUBROUTINE ACNORM(NDIM,NNOCO,NOMA,NEWGEO,DEFICO)
C ======================================================================
...
C OUT  NORMCO : NORMALE EN TOUS LES NOEUDS DE CONTACT
C OUT TANGCO : TANGENTES EN TOUS LES NOEUDS DE CONTACT
...
C              POUR LA ZONE N : METHCO(1+5*(N-1)+1) = 4 SI
...
C ---------------- FIN DECLARATIONS NORMALISEES JEVEUX -----------------
      ...
      INTEGER JCOOR,JMACO,JMANO,JPOMA,JNORM,JDES,JZOCO,JDIR,JMETH
      INTEGER POSNO,POSMA,ITYP,NUTYP,N1,NO(9),NUMA,IZONE,JTANG
      INTEGER INO,J,JDEB,JFIN,K,IATYMA,NTPOI1,NTSEG2,NTSEG3
      INTEGER JTGDEF
      REAL*8 RBID,XNORM(3),VECSEG(3),COORMA(27),XTANG(6),VECNOR(3)
      CHARACTER*1 K1BID
      CHARACTER*24 NOZOCO,CONTMA,MANOCO,PMANO,NORMCO,DIRCO,METHCO
      CHARACTER*24 TANGCO,TANDEF
...
C ----- RECUPERATION D'ADRESSES JEVEUX
      ...
      CALL JEVEUO(NORMCO,'E',JNORM)
      CALL JEVEUO(TANGCO,'E',JTANG)
...
C --- UTILISATION D'UNE DIRECTION FIXE SI NECESSAIRE
        ...
        IF (ZI(JMETH+9* (IZONE-1)+1).EQ.4) THEN
...
C   ---   CALCUL DES NORMALES
   ...
   40     CONTINUE
        ...
        CALL R8COPY(3,XNORM,1,ZR(JNORM+3* (POSNO-1)),1)
        CALL R8COPY(6,XTANG,1,ZR(JTANG+6* (POSNO-1)),1)
