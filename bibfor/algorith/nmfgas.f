...
C-----------------------------------------------------------------------
      ...
      REAL*8  CRIT(*)
      REAL*8  XLONG0
      REAL*8  COEFFL(NCOEFF)
      REAL*8  TMOINS,TPLUS,EFFNOM,PM(3)
      REAL*8  UGL(NEQ),PGL(3,3)
      REAL*8  KLS(NBT),EFFNOC,FLC,PP(3)
      CHARACTER*(*) FAMI
C ----------------------------------------------------------------------
C
...
C
      EXTERNAL NMCRI4
      REAL*8   NMCRI4, UL(12)
      REAL*8     FGRAND,DEPGRD,RIGELA,DEPTHE,EXPON
      REAL*8     PRECI,BA,BB,FA,CORREC,XRIG,SIGP,TABS,R8T0
      REAL*8     E,EM,TEMPM,TEMPP
...
C --- COEFFICIENTS DE FLUAGE ET GRANDISSEMENT
      CALL VERIFM(FAMI,NPG,1,'T',ICODMA,'ELAS',1,DEPTHE,IRET)

      CALL MOYTEM(FAMI,NPG,1,'+',TEMPP,IRET)
      IF (IRET.EQ.1) CALL U2MESS('F','CALCULEL_31')
      CALL MATELA(ICODMA,' ',ITEMP,TEMPP,E,NU)

      CALL MOYTEM(FAMI,NPG,1,'-',TEMPM,IRET)
      IF (IRET.EQ.1) CALL U2MESS('F','CALCULEL_31')
      CALL MATELA(ICODMA,' ',ITEMP,TEMPM,EM,NUM)
      YOU    = E
      ...
      CALL GRANAC(FAMI,1,1,ICODMA,'        ','LEMAITRE_IRRA   ',
     +                              IRRAP,IRRAM,TEMPM,TEMPP,FGRAND)
      PP(3)  = PM(3)+FGRAND
...
C      FA0 = FA
      ...
      IF (SIGE.NE.0.D0) THEN
         ...
         NITMAX = INT(CRIT(1))
         CALL ZEROFR(1,'BRENT',NMCRI4,BA,BB,PRECI,NITMAX,DP,IRET,ITER)
