...
CJMP  CETTE FORMULE NE FONCTIONNE QUE POUR THETA=1 OU 0.5
      ...
      IF (SDT.GT.0.D0) THEN
        SDT = ((SDT**COEFFL(5))+COEFFL(7)*COEFFL(2))**COEFFL(1)
      ELSEIF (SDT.EQ.0.D0) THEN
        SDT = (COEFFL(7)*COEFFL(2))**COEFFL(1)
      ENDIF
      IF (COEFFL(6).LT.0.D0) THEN
        CALL UTMESS('F','LEMAITRE1D','PHI_ZERO < 0')
      ENDIF
      IF (SDT.LT.0.D0) THEN
        CALL UTMESS('F','LEMAITRE1D','FLUENCE DECROISSANTE(PHI<0)')
      ENDIF
      IF (SDT.GT.0.D0) THEN
        SDT = EXPON*THETA*(TPLUS-TMOINS)/SDT
      ELSE
        CALL UTMESS('F','LEMAITRE1D','DENOMINATEUR NUL (L/K=0)')
      ENDIF
C
      BA = 0.D0
      FA = NMCRI4(BA)
