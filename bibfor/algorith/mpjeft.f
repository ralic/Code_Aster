...
C  ELEMENT VOLUMIQUE / ELEMENT SURFACIQUE / ELEMENT LINEIQUE
      ...
      ELSE IF (NCAS.EQ.5) THEN
        CALL PJ5DCO('TOUT',MODEL1,MODEL2,0,0,0,0,' ',' ',CORRES)
      ELSE

        CALL UTMESS('A','MPJEFT','CAS NON PREVU'//
     &     ' ON INITIALISE LA CORRESPONDANCE AU NOEUD LE PLUS PROCHE')
C
...
C
        IF (NBNLIS .NE. NBNMES) THEN
          CALL UTMESS('F','MPJFTE','NB NOEUDS MESURE SUPERIEUR'//
     &                 ' NB NOEUDS CALCUL')
        ENDIF
...
C        ----------------------------------------------
     ...
     &                MOTCLE,TYMOCL,'&&PJEFTE.LINONU1',NBNO1)
          IF (NBNO1 .EQ. 0) THEN
            CALL UTMESS('F','MPJFTE','NOEU_CALCUL NON TROUVE')
          ENDIF
...
C        ----------------------------------------------
     ...
     &                MOTCLE,TYMOCL,'&&PJEFTE.LINONU2',NBNO2)
          IF (NBNO2 .EQ. 0) THEN
            CALL UTMESS('F','MPJFTE','NOEU_MESURE NON TROUVE')
          ENDIF
...
C        ----------------------------------------------
          IF (NBNO1 .NE. NBNO2) THEN
            CALL UTMESS('F','MPJFTE','NOMBRE DE NOEUDS DIFFERENT')
          ENDIF
          IF (NBNO1 .GT. 1) THEN
            CALL UTMESS('F','MPJFTE','TRAITEMENT MANUEL '//
     &                  'CORRESPONDANCE : UN COUPLE A LA FOIS')
          ENDIF
