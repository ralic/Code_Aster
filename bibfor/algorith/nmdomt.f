...
C
      ...
      ELSEIF (METHOD(1) .EQ. 'IMPL_EX') THEN
        CALL GETVIS('IMPL_EX','REAC_INCR',1,IARG,1,REINCR,IRET)
        IF (REINCR.LT.0) THEN
          CALL ASSERT(.FALSE.)
        ELSE
          PARMET(1) = REINCR
        ...
        CALL GETVTX('IMPL_EX','PREDICTION',1,IARG,1,METHOD(5),IRET)
...
C
     ...
     &             IRET)
        ...
        PARMET(10) = ITMP
        CALL GETVIS('RECH_LINEAIRE','ITER_LINE_CRIT',1,IARG,1,ITMP,IRET)
        PARMET(12) = ITMP
        CALL GETVR8('RECH_LINEAIRE','PAS_MINI_CRIT',1,IARG,1,PARMET(13),
     &             IRET)
