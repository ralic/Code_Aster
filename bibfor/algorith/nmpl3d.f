...
C
     ...
     &              DEPLM,.TRUE.,POIDS,DFDI,F,EPS,R)
C
C       CALCUL DE DEPS
C
        CALL NMGEOM(3,NNO,.FALSE.,GRAND,GEOM,KPG,IPOIDS,IVF,IDFDE,
     &              DEPLP,.FALSE.,POIDS,DFDI,F,DEPS,R)
C
...
C
          DO 230 N=1,NNO
            VECTU(1,N)=VECTU(1,N)+ POIDS*
     &       (DEF(1,N,1)*SIGMA(1)+DEF(4,N,1)*SIGMA(4)+
     &        DEF(5,N,1)*SIGMA(5))
            VECTU(2,N)=VECTU(2,N)+ POIDS*
     &       (DEF(2,N,2)*SIGMA(2)+DEF(4,N,2)*SIGMA(4)+
     &        DEF(6,N,2)*SIGMA(6))
            VECTU(3,N)=VECTU(3,N)+ POIDS*
     &       (DEF(3,N,3)*SIGMA(3)+DEF(5,N,3)*SIGMA(5)+
     &        DEF(6,N,3)*SIGMA(6))
 230      CONTINUE
C
          DO 310 KL=1,3
