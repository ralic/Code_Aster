...
C
      ...
      REAL*8  B(6,3,27),DE(6),SIGMA(6),DSIDEP(6,6,2),T1,T2,PIN(6,6)
      REAL*8  P(6,6),SIGMAM(6),EPSRSS(6),SIGELL(6),DIST(NNO2,2)
      REAL*8  Z(3,3),W(3),WORK(9),BARY(NDIM),BARYO(NDIM),SCAL(3)
      REAL*8  DIRR(NDIM)
C
C ----------------------------------------------------------------------
C
...
C
          NUMAV=LIVOIS(KVOIS)
          CALL TECAC2('OOO',NUMAV,'PVARIMP','L',1,VIVOIS,IRET)
...
C IL FAUDRA PREVOIR DE POUVOIR SORTIR DU DOMAINE GEOMETRIQUE ADMIS
C  AVEC UN CODRET = 2
C
C
C
