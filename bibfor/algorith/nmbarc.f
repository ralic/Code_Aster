...
C     -- INVERSE DE HHB
        CALL R8INIR(6*6,0.D0,HHBM,1)
           DO 3004 K=1,6
               HHBM(K,K)=1.D0
 3004  CONTINUE
        CALL MGAUSS('NFVP',HHB,HHBM,6,6,6,DET,IRET)
...
C     -- INVERSE DE LA SOMME DES TERMES DEVIATORIQUE ET HYDROSTATIQUE
        CALL R8INIR(6*6,0.D0,DEVHYM,1)
           DO 3012 K=1,6
               DEVHYM(K,K)=1.D0
 3012  CONTINUE
        CALL MGAUSS('NFVP',DEVHYD,DEVHYM,6,6,6,DET,IRET)
...
C     9.7.13-- INVERSE DE LA MATRICE T
       CALL R8INIR(6*6,0.D0,VV,1)
           DO 108 K=1,6
               VV(K,K)=1.D0
 108  CONTINUE
       CALL MGAUSS('NFVP',T,VV,6,6,6,DET,IRET)
...
C     9.9.2-- INVERSE DE LA MATRICE VHH
       CALL R8INIR(6*6,0.D0,VVH,1)
           DO 390 K=1,6
               VVH(K,K)=1.D0
 390  CONTINUE
       CALL MGAUSS('NFVP',VHH,VVH,6,6,6,DET,IRET)
