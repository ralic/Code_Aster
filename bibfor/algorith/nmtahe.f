...
C      PREDICTION ELASTIQUE
        DP = 0.D0
        XI = 1.D0
        CALL NMTASP(NDIMSI,CRIT,MAT,SIGEL,VIM,EPM,DP,SP,XI,F)

C      CHARGE
        IF (F.GT.0.D0) THEN

C        CALCUL DE DP : EQUATION SCALAIRE F=0 AVEC  0 < DP < DPMAX
          CALL NMTADP(NDIMSI,CRIT,MAT,SIGEL,VIM,EPM,DP,SP,XI,G)
...
C            ACTUALISATION
 ...
 200        CONTINUE
            CALL UTMESS('F','NMTAHE', 'PSEUDO CHARGE: PAS CONVERGE')
 210        CONTINUE
...
C          CALCUL DE XI : EQUATION SCALAIRE F=0 AVEC  0 < XI < 1
            CALL NMTAXI(NDIMSI,CRIT,MAT,SIGEL,VIM,EPM,DP,SP,XI,G)
...
C              ACTUALISATION
 ...
 300          CONTINUE
              CALL UTMESS('F','NMTAHE','PSEUDO-DECHARGE: PAS CONVERGE')
 310          CONTINUE
            END IF
