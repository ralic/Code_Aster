      SUBROUTINE COPNOR(NOMA  ,DEFICO,FFORME,POSMAI,KSI1  ,
     &                  KSI2  ,TAU1  ,TAU2)
C     
...
C
      ...
      CHARACTER*24 NOMACO,TANINI     
      INTEGER      JNOMA,JTGINI
...
C
      CALL CFNBEN(NOMA  ,DEFICO,POSMAI,'MAIL',NBENT ,
     &            JDECNO)
      ...
      DO 10 INO = 1,NNO
        ...
        DO 11 IDIM = 1,3
          VECTA1(3*(INO-1)+IDIM) = ZR(JTGINI+6*(POSNO -1)+IDIM-1)   
          VECTA2(3*(INO-1)+IDIM) = ZR(JTGINI+6*(POSNO -1)+3+IDIM-1)
 11     CONTINUE                     
   10 CONTINUE
C
...
C --- NODALES
C
      CALL MMCOOR(ALIAS ,NNO   ,NDIM  ,VECTA1,FFORME,
     &            KSI1  ,KSI2  ,TAU1  )
      IF (NDIM.EQ.3) THEN
        CALL MMCOOR(ALIAS ,NNO   ,NDIM  ,VECTA2,FFORME,
     &              KSI1  ,KSI2  ,TAU2  )    
      ENDIF 
C
      CALL JEDEMA()
