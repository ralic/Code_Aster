     ...
     &                  OLD,OLDIA,FMOD,FMRES,FTMP,MTMP1,MTMP6,
     &                  TESTC,ITFORN,INEWTO,TOLN)
      IMPLICIT NONE
C-----------------------------------------------------------------------
...
C ---------
     ...
     &          VITG(*), DEPG(*), VITG0(*), DEPG0(*), OLD(9,*)
      INTEGER   OLDIA(*)
      REAL*8    FMOD(*), FMRES(*), FTMP(*), MTMP1(NP1,*), MTMP6(3,*)
...
C ROUTINES EXTERNES
C -----------------
C     EXTERNAL  CALJAC, CALRES, DEIMPJ, DISBUT, FTEST1, GLOLOC, INITMA,
C    &          INITVE, LOCGLO, PRLGMA, PROJAC, PROJGM, PROJMG
C
C-------------------   DEBUT DU CODE EXECUTABLE    ---------------------
C
C  0. INITIALISATIONS
C     ---------------
      CALL INITMA(NP1,NP1,CMOD)
      CALL INITMA(NP1,NP1,KMOD)
      CALL INITVE(NP1,FMOD)
      CALL INITVE(3,FLOC)
      CALL INITVE(NP1,FMRES)
C
C --- SI INEWTO = 1, LA ROUTINE APPELANTE EST NEWTON, LES CALCULS SONT
C --- EFFECTUES DANS LA CONFIGURATION A TC0. SI INEWTO = 0, LA ROUTINE
C --- APPELANTE EST ALITMI, LES CALCULS SONT EFFECTUES SELON LE
...
C        (DEPLACEMENTS)
         ...
         CALL PROJMG(NP1,NP2,IC,NBMCD,PHII,DEPG0,XGLO0)
C        (VITESSES)
         CALL PROJMG(NP1,NP2,IC,NBMCD,PHII,VITG,VGLO)
         CALL PROJMG(NP1,NP2,IC,NBMCD,PHII,VITG0,VGLO0)
C
...
C
C  ----- DETERMINATION DU POINT D'IMPLICITATION DU JACOBIEN
C
         TESTCV = 0
         CALL DEIMPJ(ITESTC,ITEST0,TETAJ,TESTCV)
C
C  ----- MDCHOE EST APPELE PAR NEWTON : TESTCV = ITEST0
C        ----------------------------------------------
...
C
C  1.4.2.   CALCUL DES FORCES NON-LINEAIRES RESIDUELLES
C           -------------------------------------------
...
C           ----------------
            ...
            CALL PROJGM(NP1,NP2,IC,NBMCD,PHII,FGLO,FTMP)
            DO 20 J = 1, NBMCD
...
C           --------------------
            CALL INITVE(NP1,FTMP)
            CALL PROJGM(NP1,NP2,IC,NBMCD,PHII,FGLRES,FTMP)
            DO 30 J = 1, NBMCD
               FMRES(J) = FMRES(J) + FTMP(J)
  30        CONTINUE
C
C  1.4.6.   PASSAGE REPERE LOCAL -> GLOBAL (FORCES NON-LINEAIRES)
C           -----------------------------------------------------
            CALL PRLGMA(JACOBK,SINA,COSA,SINB,COSB,SING,COSG,JACOBK)
            CALL PRLGMA(JACOBC,SINA,COSA,SINB,COSB,SING,COSG,JACOBC)
C
C  1.4.7.   CONVERSION DDLS PHYSIQUES -> DDLS GENERALISES
C           (FORCES NON-LINEAIRES)
C           ----------------------
            CALL PROJAC(NP1,NP2,NBM,IC,PHII,JACOBC,MTMP1,MTMP6)
            DO 40 K = 1, NBM
               DO 41 J = 1, NBM
                  CMOD(J,K) = CMOD(J,K) + MTMP1(J,K)
  41           CONTINUE
  40        CONTINUE
            CALL PROJAC(NP1,NP2,NBM,IC,PHII,JACOBK,MTMP1,MTMP6)
            DO 50 K = 1, NBM
               DO 51 J = 1, NBM
  ...
  50        CONTINUE
C
C  1.5.  REMPLISSAGE DU TABLEAU OLD LORSQU'IL N'Y A PAS CHOC
C        SUR LA BUTEE IC
C        ---------------
         ELSE
C
