...
C
      ...
      CHARACTER*8     TYPMA,ELRESE(2),ELREFP
      LOGICAL         CUT
...
C     (1) RECHERCHE D'UN NOEUD PIVOT (LSN NON NULLE)
      IF(I.NE.NX.AND.TABLS(I).NE.0.AND.I.LT.NNOSE)THEN
        DO 30 K=I+1,NNOSE
C     (2) PRODUIT DE CE PIVOT PAR LES AUTRES LSN
          IF(TABLS(I)*TABLS(K).LT.0.D0) CUT=.TRUE.
 30     CONTINUE
      ELSE
        I=I+1
        GO TO 1
...
C           ON AJOUTE A LA LISTE LE POINT A
            CALL XAJPIN(PINTER,PTMAX,IPI,INS,A,LONGAR,
     &                                      AINTER,0,NA,0.D0)
          END IF
...
C           ON AJOUTE A LA LISTE LE POINT B
            CALL XAJPIN(PINTER,PTMAX,IPI,INS,B,LONGAR,
     &                                    AINTER,0,NB,LONGAR)
          END IF
...
C         PETITE TOLERANCE SUR LSNM CAR VALEUR CALCULEE
          ...
          IF (ABS(LSNM-LSNB).LE.R8PREM().OR.ABS(D).LE.CRILSN) THEN
            ...
            IF (CUT) THEN
              CALL XAJPIN(PINTER,PTMAX,IPI,INM,M,LONGAR,
     &                                     AINTER,IA,0,ALPHA)
            ELSEIF (.NOT.CUT) THEN
              CALL XAJPIN(PINTER,PTMAX,IPI,INM,M,LONGAR,
     &                                     AINTER,0,NM,ALPHA)
