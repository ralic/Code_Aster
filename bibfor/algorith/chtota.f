     ...
     &           VEFONO,INSTAM,VAFONO,VEBUDI,VABUDI,VECHTP,CNCHTP)
...
C TOLE CRP_21
      ...
      CHARACTER*24 DEPMOI,DEPLAS,DEPDEL,SIGMA,COM,COM2
      REAL*8 R8BID(3),INSTAM
      INTEGER LONCH
      REAL*8 VCHTOT(*)
C ----------------------------------------------------------------------
...
C --------- FIN  DECLARATIONS  NORMALISEES  JEVEUX ---------------------
      INTEGER IBID,JFO,JFONO,JBU,JBU2,JBUDI,JTM,JCHTM,NH,NBVEC,I,J
      CHARACTER*8 K8BID
      CHARACTER*24 VEFONO,VAFONO,VEBUDI,VABUDI,VECHTP,CNCHTP
      CHARACTER*24 CHVARC
      COMPLEX*16 CBID
      DATA CHVARC /'&&CHTOTA.VARC'/
      CALL JEMARQ()

      NH = 0
      VEBUDI = '&&VEBDEP.LISTE_RESU'
C     -- JP : J'AI L'IMPRESSION QUE COM N'EST PAS ENCORE CALCULE
C             => J'EN FAIT 1 TEMPORAIRE : COM2.
      COM2='&&CHTOTA.COMMOI'
      CALL NMVCLE(MODELE,MATE,CARELE,LISCHA,INSTAM,COM2)
      CALL NMVCEX('TOUT',COM2,CHVARC)
      FNOEVO=.FALSE.
      CALL VEFNME(MODELE,SIGMA,CARELE,DEPMOI,DEPDEL,VEFONO,MATE,
     &            COMPOR,NH,FNOEVO,R8BID,' ',CHVARC,' ',LISCHA)
      CALL ASASVE(VEFONO,NUMEDD,'R',VAFONO)

      CALL VEBUME(MODELE,DEPLAS,LISCHA,VEBUDI)
      CALL ASASVE(VEBUDI,NUMEDD,'R',VABUDI)

      CALL VECTME(MODELE,LISCHA,CARELE,MATE,COMPOR,COM2,VECHTP)
