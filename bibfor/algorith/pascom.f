      SUBROUTINE PASCOM(MASGEN,RIGGEN)
...
C
C IN  MASGEN   : MASSE GENERALISEE
C IN  RIGGEN   : RIGIDITE GENERALISEE
C ----------------------------------------------------------------------
C
...
C
      ...
      INTEGER      NBMA,IMA,IAD,JPAS,NBDT,JREPE,JREFAM,JREFAK
      INTEGER      JSCDEM,JSCDEK,JDESC,NBMODE,NBSTOM,NBSTOK
      INTEGER      JMASG,JRAIG
      REAL*8       DTCOU,VALEUR,PHI,DT,R8DEPI
      CHARACTER*8  K8BID,STOCFL,MASGEN,RIGGEN
...
C     --- RECUPERATION DE LA BASE MODALE ET NOMBRE DE MODES ---
      ...
      DTCOU = R8DEPI() / SQRT(ABS(ZR(JRAIG)/ZR(JMASG)))
      DO 21 I = 1,NBMODE - 1
       DT = R8DEPI() / SQRT(ABS(ZR(JRAIG+I)/ZR(JMASG+I)))
