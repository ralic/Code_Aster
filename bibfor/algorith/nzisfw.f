...
C 2 RECUPERATION DES CARACTERISTIQUES (ELAS ET DILATATION)
     ...
     &             0.D0,6,NOMRES,VALRES,CODRET, FB2 )
...
C 4 RECUPERATION CARACTERISTIQUES (PLASTIQUE)
     ...
     &             NOMRES(6),FMEL,CODRET(6),'  ')
       ...
       CALL RCVALB(FAMI,KPG,KSP,'+',IMAT,' ','ELAS_META',0,' ',0.D0,5,
     ...
     &    COMPOR(1)(1:9).EQ.'META_V_IL' )THEN
        ...
        CALL RCVALB(FAMI,KPG,KSP,'+',IMAT,' ','META_ECRO_LINE',0,
     &              ' ',0.D0,5,NOMRES(7),VALRES(7),CODRET(7),'F ')
...
C 5 RECUPERATION CARACTERISTIQUES (RESTAURATION ECROU DE METALLO)
 ...
 200   CONTINUE
     ...
     &     COMPOR(1)(1:16) .EQ. 'META_V_INL_PT_RE'  ) THEN
        ...
        NOMRES(7) ='F3_C_THETA'
        NOMRES(8) ='F4_C_THETA'
        CALL RCVALB(FAMI,KPG,KSP,'-',IMAT,' ','META_RE',0,'  ',0.D0,8,
...
C 8 RECUPERATION CARACTERISTIQUES (PLAS DE TRANSFORMATION)
     ...
     &    COMPOR(1)(1:16) .EQ. 'META_V_INL_PT_RE') THEN
       ...
       CALL RCVALB(FAMI,KPG,KSP,'+',IMAT,' ','META_PT',0,' ',0.D0,4,
     &             NOMRES,VALRES,CODRET, FB2 )
       DO 20 I=1,4
        KPT (I) = VALRES(I)
 20    CONTINUE
