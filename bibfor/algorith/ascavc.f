...
C MODIF ALGORITH  DATE 23/07/2007   AUTEUR SALMONA L.SALMONA 
C ======================================================================
...
C----------------------------------------------------------------------
      ...
      INTEGER ICHCI,IBID,IFM,NIV,NEQ,IEQ,IDLCI,ICHDLC
      CHARACTER*8 NEWNOM,KBID
...
C     -- S'IL Y A DES CHARGES CINEMATIQUES :
      ELSE
        ...
        CALL WKVECT(VCI2//'.DLCI','V V I',NEQ,IDLCI)
        DO 20 ICHAR = 1,NCHTOT
          ...
          IF (ICINE.LT.0) THEN
            ...
            CALL CALVCI(CHAMNO,NUMEDD,1,ZK24(IDCHAR-1+ICHAR),INST,'V')
            CALL JEVEUO(CHAMNO//'.DLCI','L',ICHDLC)
C --- COMBINAISON DES DLCI (OBJET CONTENANT DES 0 OU DES 1), LES 1
C --- ETANT POUR LES DDL CONTRAINT
C --- LE RESTE DE L OBJECT VCI2 EST CREE PAR ASCOVA
            DO 30 IEQ=1,NEQ
              ZI(IDLCI-1+IEQ)=MAX(ZI(IDLCI-1+IEQ),ZI(ICHDLC-1+IEQ))
   30       CONTINUE
          END IF
   20   CONTINUE
