...
C TOLE CRP_21
      ...
      CHARACTER*24 MODELE,NUMEDD,MATE,CARELE,COMREF,COMPOR,LISTE
...
C                      'CONTACT_FR' : TANT QU'ADHERENCE CONTACT / NEWTON
...
C -------------- FIN  DECLARATIONS  NORMALISEES  JEVEUX ----------------
      ...
      INTEGER REINCR,REITER,IERR,IECPCO,IMAT,JJJ,NBNO,ICONTA,IBID
      REAL*8 INSTAM,INSTAP,DIINST,PASMIN
      REAL*8 UN,COEF2(3)
      CHARACTER*8 NOMDDL,TLIMAT(2),MATEL,MATGEO,LISCMP(3),LISNO(2)
      CHARACTER*16 METCOR,METPRE
      CHARACTER*19 MATASS,MAPREC
      CHARACTER*24 DEPMOI,K24BID,K24BLA,SIGMOI
      CHARACTER*24 MERIGI,MESUIV,VERESI,VEDIRI
      CHARACTER*24 MASSE,LIMAT(3)
      CHARACTER*4 TYPMAT,TYPE(3)
      INTEGER IACHAR,JINFC,IACHA2,I,NBCHAR,NBMAT,IRET,LISBID,ILIMA
...
C -- SI FROTTEMENT       ->MATRIX(1) = '&&NMASFR.MATANG'
      ...
      IF (PHASE.EQ.'CONTACT_FR') THEN
        CALL FROTTE(ITERAT,RESOCO,DEFICO,MATRIX)
        GOTO 9999
      END IF
...
C SI CONTACT/FROTTEMENT, ON FORCE LE REASSEMBLAGE
        CALL CFDISC(DEFICO,RESOCO,ICONTA)

        IF ((ABS(ICONTA).EQ.2)) THEN
             REASMA = .TRUE.   
        ENDIF
...
C -- CALCUL DE LA MATRICE MATASS
          ...
          CALL MTDEFS(MATASS,RIGID,'V',TYPMAT)
...
C          IF (PHASE.NE.'PREDICTION' .OR. METPRE.NE.'EXTRAPOL') THEN
     ...
     &                  INSTAM,COMPOR,CARCRI)
            CALL ASCOMA(MESUIV,NUMEDD,LISCHA//'.FCHA',INSTAP,MATASS,
     &                  SOLVEU,LISCHA)
          END IF
