...
C
      INTEGER      CFDISI
      INTEGER      JCOOR,JDES
      INTEGER      POSNO,POSMA,N1,NO(9),NUMA,IZONE
      INTEGER      NUTYP
      INTEGER      INO,J,JDEB,JFIN,K
      ...
      CHARACTER*24 NORINI,TANDEF,TANPOU
      INTEGER      JNRINI,JTGDEF,JPOUDI
      CHARACTER*4  MATYP
C
C ----------------------------------------------------------------------
C
...
C   ---   VALEUR DE VECT_ORIE_POU

            IF (CFDISI(DEFICO,'VECT_ORIE_POU',IZONE).EQ.1) THEN
              CALL JEVEUO(TANDEF,'L',JTGDEF)
              XTANG(1) = ZR(JTGDEF+3* (IZONE-1))
              XTANG(2) = ZR(JTGDEF+3* (IZONE-1)+1)
              XTANG(3) = ZR(JTGDEF+3* (IZONE-1)+2)
            ELSE IF (CFDISI(DEFICO,'VECT_ORIE_POU',IZONE).EQ.2) THEN
              CALL JEVEUO(TANPOU,'L',JPOUDI)
              XTANG(1) = ZR(JPOUDI+3* (IZONE-1))
              XTANG(2) = ZR(JPOUDI+3* (IZONE-1)+1)
              XTANG(3) = ZR(JPOUDI+3* (IZONE-1)+2)
            END IF
...
C   ---   CALCUL DES NORMALES
            IF ((NDIM.EQ.3) .AND. (MATYP.EQ.'POI1')) THEN
              ...
              XNORM(2) = 0.D0
              XNORM(3) = 0.D0
            ELSE IF ((NDIM.EQ.3) .AND. ((MATYP.EQ.'SEG2').OR.
     &               (MATYP.EQ.'SEG3'))) THEN
   ...
   40     CONTINUE
        ...
        CALL DCOPY(3,XNORM,1,ZR(JNRINI + 3* (POSNO-1)),1)
