...
C ---------------- FIN DECLARATIONS NORMALISEES JEVEUX -----------------
      ...
      REAL*8        RBID
      COMPLEX*16    CBID
      CHARACTER*8   LPAIN(12),LPAOUT(3),LICMP(2),KBID
      CHARACTER*19  LIGREL,CICOCA,CINDOO,LST,LCHIN(12),LCHOUT(3)
      CHARACTER*19  CMEMCO,PINTER,AINTER,CFACE,LONCHA
C ----------------------------------------------------------------------
...
C     POUR L'INDICATEUR DE CONTACT COURANT
...
C     CRÉATION DU CHAMP ELEM SIMPLE DE SORTIE À 12*5 SS PG
C     POUR L'INDICATEUR DE MEMOIRE DE CONTACT 
      CMEMCO = '&&XMMBCA.MEMCON'
      
      CALL CESCRE('V',CMEMCO,'ELEM',MA,'DCEL_I',2,LICMP,-1,-1,-2)
      CALL JEVEUO(CMEMCO//'.CESD','L',JCESD)
      CALL JEVEUO(CMEMCO//'.CESL','E',JCESL)
      CALL JEVEUO(CMEMCO//'.CESV','E',JCESV)
      DO 11,IMA = 1,NBMA
        CALL CESEXI('C',JCESD,JCESL,IMA,1,1,1,IAD)
        IF (IAD.GE.0) CALL U2MESS('F','CALCULEL_2')
        ZL(JCESL-1-IAD) = .TRUE.
...
C     CREATION DES LISTES DES CHAMPS IN ET OUT
      ...
      LPAIN(11) = 'PGLISS'
      LCHIN(11) = DEFICO(1:16)//'.GL'
      LPAIN(12) = 'PMEMCON'
      LCHIN(12) = DEFICO(1:16)//'.MC'
      ...
      CALL CALCUL('S','XCVBCA',LIGREL,12,LCHIN,LPAIN,3,
     &                                   LCHOUT,LPAOUT,'V')
...
C     ON COPIE CMEMCO DANS DEFICO.MC
      CALL COPISD('CHAMP_GD','V',LCHOUT(3),DEFICO(1:16)//'.MC')
