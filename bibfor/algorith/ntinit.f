     ...
     &                  NBPASE, INPSCO,VHYDR,CHLAPM,CHLAQM,NUMFIN,
...
C                      PAR LE MOT-CLE 'TEMP_INIT' DU PB STD.
...
C 0.3. ==> VARIABLES LOCALES
      ...
      INTEGER NEQ,NUMIMP(2),IRET,IERR,NRPASE,INITPR,IAUX,JAUX,NIV,IFM,
     &        JCRR, JCRK
      LOGICAL LLIN
...
C OUT LNUAR  : LOGIQUE INDIQUE SI ARCHIVAGE PAR INSTANTS OU NUMORDRE
...
C OUT NUMIMP(1) : NUMERO D'ORDRE DU CALCUL PRECEDENT SI INITPR = 3
...
C OUT HYDRIN : CHAMP D HYDRATATION INITIALE
          CALL NTDOET(MODELE,LOSTAT,INITPR,RESUIN,NUMIMP(1),TEMPIN,
     &                HYDRIN)
...
C VRAI STATIONNAIRE (.NOT.LOSTAT) OU IL Y'A RIEN A FAIRE
          ...
          IF (INITPR.EQ.3) THEN
            ...
            CALL PSRENC ( RESUIN, NOPASE, RESUID, IRET )
            IF ( IRET.NE.0 ) THEN
              VALI = IRET
              CALL U2MESG('A', 'ALGORITH13_72',0,' ',1,VALI,0,0.D0)
               VALK(1) = RESUIN
               VALK(2) = NOPASE
...
C STATIONNAIRE ET POUR UN TRANSITOIRE DEBUTANT PAR UNE PHASE STATIO.
          IF (.NOT.LOSTAT) THEN
            CALL NTDOED(INITPR,RESUID,NUMIMP(1),TEMPIN)
          ENDIF
...
C 3.4. ==> LISTE DES INSTANTS DE CALCUL ET DIVERS COMPTEURS
      ...
      CALL NTINST ( LNUAR, LOSTAT, LIEVOL, LISARC, EXCARC, NUMIMP,
     &              NBGRPA, IDEB, JDEB, NUMORD, TPSNP1 )
      NUMFIN = NUMIMP(2)
