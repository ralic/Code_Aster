...
C MODIF ALGORITH  DATE 10/11/1999   AUTEUR VABHHTS J.PELLET 
C ======================================================================
...
C     ----------------------------------------------
      IF ( NPILOT .GE. NEQ ) THEN
        CALL UTMESS('F','RUNGEK','PB2, VARIABLES DE PILOTAGES')
      ENDIF
C     ----------------------------------------------
...
308       CONTINUE
        ...
        IF ( COMPTE .GT. COMPTM ) THEN
          CALL UTMESS ('F','POUTRE','RK41. ERREUR D INTEGRATION DANS'//
     &            ' RUNGE-KUTTA. TROP D ITERATION.')
        ENDIF
        ERREUR =  ABS( DVRINT(1) )
        DO 310, I = 2 , NPILOT
...
310     CONTINUE
        IF ( (ERREUR .LT. 1.0D-30).AND.(ENCORE) ) THEN
          CALL UTMESS ('F','POUTRE','RK42. ERREUR D INTEGRATION DANS.'//
     &            ' RUNGE-KUTTA.')
        ENDIF
      IF ( ENCORE ) GOTO 100
      ERREUR = 0.D0
