...
C
      ...
      INTEGER RACCK
      INTEGER NNEX,INI4,IBID,VALRET
      REAL*8 XIMIN,YIMIN,T1MIN(3),T2MIN(3),GEOM(3),LAMBDA,XPG,YPG,HPG
      REAL*8 DIR(3),NORME1,TOLEOU,NMIN(3),JEUMIN
...
C
      ...
      DO 20 IMA = 1,NTMA
C
C --- INFOS GENERIQUES SUR LA ZONE DE CONTACT
C    
...
C              
        ...
        DO 10 INI = 1,NBN
          IF (NIV.GE.2) THEN
            WRITE (IFM,*) '<CONTACT> <> COORDONNEES POINT DE CONTACT' 
          ENDIF        
C
...
C 
          CALL COPCOG(NOMA,DEFICO,NEWGEO,POSMA,XPG,YPG,GEOM)
C
...
C          
C --- STOCKAGE DES VALEURS DANS LA CARTE
C  
          IF (NIV.GE.2) THEN
...
C              
          IF (INI .EQ. NBN .AND. NNEX .EQ. NBN) THEN
            DO 11 INI4 = 1,NBN
              ZR(JTABF+ZTABF*NTPC+ZTABF*(INI4-1)+22) = 1.D0
 11         CONTINUE
          ELSE
            DO 12 INI4 = 1,NBN
              ZR(JTABF+ZTABF*NTPC+ZTABF*(INI4-1)+22) = 0.D0
 12         CONTINUE
          END IF
C
