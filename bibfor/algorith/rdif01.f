...
C       ----------------------------------------------------------------
     ...
     &          NBPHAS,NFS,IRET,ITMAX,NSG,NHSR,NUMHSR(*),NEPS
        ...
        CHARACTER*(*) FAMI
        CHARACTER*3 MATCST
        REAL*8 E, NU, ALPHA,PGL(3,3),TOLER,X, DTIME,COEL(NMAT)
        REAL*8 COTHE(NMAT),DCOTHE(NMAT),COEFF(NMAT),DCOEFF(NMAT)
        REAL*8 SIGI(6),EPSD(6),DETOT(6),COEFT(NMAT),XM
        REAL*8 VINI(NVI),DVIN(NVI),HSR(NSG,NSG,NHSR)
...
C         PAS DE VARIATION DES COEF AVEC LA TEMPERATURE        
          NCOE=NBCOMM(NMAT,3)
          XM=0.D0
          CALL COEFFT(NCOE,COTHE,COEFF,DCOTHE,DCOEFF,XM,DTIME,COEFT,
