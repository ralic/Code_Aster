...
C
     ...
     +             IAK11,IAK1,IAM11,IAM1,NDDEXT,NBMTOT,NBMDEF,
...
C On recupere le liste des noeuds presents dans interf_dyna
      ...
      NDDEXT=(ZI(IADESC+1)*(ZI(IADESC+1)+1)/2)

      CALL BMNBMD(BASMOD,'TOUT',NBMTOT)
      CALL BMNBMD(BASMOD,'DEFORMEE',NBMDEF)
...
C     CREATION DU .KP_EE
C **********************
      CALL WKVECT(NOMRES//'.KP_EE','G V R',NDDEXT,IAK1)
      CALL WKVECT(NOMRES//'.MP_EE','G V R',NDDEXT,IAM1)
      CALL JEVEUO(NOMRES//'.MAEL_RAID_VALE','L',IAK11)
      CALL JEVEUO(NOMRES//'.MAEL_MASS_VALE','L',IAM11)
      CALL DCOPY(NDDEXT,ZR(IAK11),1,ZR(IAK1),1)
      CALL DCOPY(NDDEXT,ZR(IAM11),1,ZR(IAM1),1)
      CALL JEEXIN(NOMRES//'.MAEL_AMOR_VALE',IRET)
      IF (IRET.GT.0) THEN
        CALL WKVECT(NOMRES//'.AP_EE','G V R',NDDEXT,IAA1)
        CALL JEVEUO(NOMRES//'.MAEL_AMOR_VALE','L',IAA11)
        CALL DCOPY(NDDEXT,ZR(IAA11),1,ZR(IAA1),1)
