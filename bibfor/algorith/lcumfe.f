...
C TOLE CRP_20
      ...
      CHARACTER*(*)    FAMI
      REAL*8           TINSTM,TINSTP, EPSTM(12), DEPST(12)
      REAL*8           SIGM(6), SIGP(6),VIM(25),VIP(25)
      REAL*8           DSIDPT(6,6,2),PROJ(6,6)

C COUPLAGE UMLV MAZARS EN NON LOCAL
C---&s---1---------2---------3---------4---------5---------6---------7--
...
C IN  EPSTM   : EFORMATION A L INSTANT MOINS
C IN  DEPST   : INCREMENT DE DEFORMATION TOTALE
...
C      REAL*8  VALPAM(3),VALPAP(3),SIGM(6),DEPS(6)
      REAL*8  CMAT(15),DEP(6,6),DEPM(6,6)
...
C MODIFI 20 OTT 2008 - M.B.
      REAL*8   TMAXP, TMAXM, YOUNM, XNUM, EPSM(6), DEPS(6)
...
C  LA DEPENDENCE DES PARAMETRES PAR RAPPORT A LA TEMPERATURE
C  CHANGE PAR RAPPORT A LA LOI D ENDOMMAGEMENT
...
C      ELSE
C     IF (OPTION(2).EQ.'ENDO_ISOT_BETON') THEN
C
C       ENDIF
...
C
C      IF ((OPTION(2).EQ.'MAZARS') .OR.
C     &    (OPTION(2).EQ.'ENDO_ISOT_BETON')) THEN
...
C    CALCUL DE LA DEFORMATION ELASTIQUE AU TEMP M
...
C        IF (OPTION(2).EQ.'MAZARS') THEN
        CALL LCUMEF(OPTION,DEP,DEPM,AN,BN,CN,EPSM,EPSRM,EPSRP,
...
C
C  MISE A JOUR DE L ENDOMMAGEMENT ET DES SIGMA POUR MAZARS
C_________________________________________________________


C        IF (OPTION(2).EQ.'MAZARS') THEN

        CALL  LCMZGE (FAMI,KPG,KSP,NDIM, TYPMOD, IMATE,
     &                   EPSTM,DEPST, VIM (22),
     &                   'RAPH_COUP       ', SIGP, VIP,  DSIDPT, PROJ)
C        ENDIF
...
C
C      IF (OPTION(2).EQ.'MAZARS') THEN
C  MB: LA MATRICE TANGENTE CALCULEE EST CELLE DU COMPORTEMENT DE MAZARS
C      I.E. LA CONTRIBUTION DU FLUAGE N EST PAS CONSIDEREE
