...
C ---------------- FIN DECLARATIONS NORMALISEES JEVEUX -----------------
      ...
      INTEGER JCMCF,IM,ITYP,NTPC,IMA,NBN,JMETH,MET,IN,JDECNO,JJEU
      CHARACTER*24 CONTMA,MAESCL,PNOMA,CARACF,NDIMCO
      CHARACTER*24 TABFIN,PSURMA,PZONE,METHCO,NOESCL,PSURNO,JEUCON
C
C ----------------------------------------------------------------------
C
...
C RECUPERONS D'ABORD LE NOMBRE TOTALE DES MAILLES DE CONTACT.
      ...
      DO 10 IZONE = 1,NZOCO
        ...
        IF ((MET.NE.6) .AND. (MET.NE.8)) GOTO 10
...
C SIMPSON2
          ...
          ELSEIF (TYCO .EQ. 6) THEN
C NEWTON COTES
            IF (NUTYP .EQ. 2) ZI(JMAESC+3*NTMA+3*IM) = 4
            IF (NUTYP .EQ. 4) ZI(JMAESC+3*NTMA+3*IM) = 4
            IF (NUTYP .EQ. 7) ZI(JMAESC+3*NTMA+3*IM) = 4
            IF (NUTYP .EQ. 9) ZI(JMAESC+3*NTMA+3*IM) = 4
            IF (NUTYP .EQ. 12) ZI(JMAESC+3*NTMA+3*IM) = 16
            IF (NUTYP .EQ. 14) ZI(JMAESC+3*NTMA+3*IM) = 16
            IF (NUTYP .EQ. 16) ZI(JMAESC+3*NTMA+3*IM) = 16
          ELSEIF (TYCO .EQ. 7) THEN
C NEWTON COTES1
            IF (NUTYP .EQ. 2) ZI(JMAESC+3*NTMA+3*IM) = 5
            IF (NUTYP .EQ. 4) ZI(JMAESC+3*NTMA+3*IM) = 5
            IF (NUTYP .EQ. 7) ZI(JMAESC+3*NTMA+3*IM) = 6
            IF (NUTYP .EQ. 9) ZI(JMAESC+3*NTMA+3*IM) = 6
            IF (NUTYP .EQ. 12) ZI(JMAESC+3*NTMA+3*IM) = 25
            IF (NUTYP .EQ. 14) ZI(JMAESC+3*NTMA+3*IM) = 25
            IF (NUTYP .EQ. 16) ZI(JMAESC+3*NTMA+3*IM) = 25
          ELSEIF (TYCO .EQ. 8) THEN
C NEWTON COTES2
            IF (NUTYP .EQ. 2) ZI(JMAESC+3*NTMA+3*IM) = 10
            IF (NUTYP .EQ. 4) ZI(JMAESC+3*NTMA+3*IM) = 10
            IF (NUTYP .EQ. 7) ZI(JMAESC+3*NTMA+3*IM) = 42
            IF (NUTYP .EQ. 9) ZI(JMAESC+3*NTMA+3*IM) = 42
            IF (NUTYP .EQ. 12) ZI(JMAESC+3*NTMA+3*IM) = 100
            IF (NUTYP .EQ. 14) ZI(JMAESC+3*NTMA+3*IM) = 100
            IF (NUTYP .EQ. 16) ZI(JMAESC+3*NTMA+3*IM) = 100
          ELSE
            CALL JXABOR
          END IF
 20     CONTINUE
...
C CREATION DU TABLEAU TABFIN
      ...
      IF (IRET .GT. 0) THEN
        CALL JEDETR(DEFICO(1:16)//'.TABFIN')
      END IF
      CALL WKVECT(TABFIN,'G V R',21*NTPC+1,JTABF)
      CALL JEEXIN(DEFICO(1:16)//'.JEUCON',IRET)
      IF (IRET .GT. 0) THEN
        CALL JEDETR(DEFICO(1:16)//'.JEUCON')
      END IF
