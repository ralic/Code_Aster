...
C
      ...
      INTEGER LGBLOC, JAMOG, IAMOG, IDIFF, I2, IE, JMODG, JRAID
      INTEGER IAM, IBLOC, IFREQ, IEQ, INOM, IAUX, JAUX, IER
      INTEGER LCC, LFON, LADRVE, LTYPVE, LPUISS, LPHASE, L, JBASM
      INTEGER IATMAR, IATMAM, IATMAT , LREFE, LDYNAM, LSECMB
      INTEGER ICOEF, NDECI, ISINGU, NPVNEG, ICODE, JAMO2, JMACR
      INTEGER IADR, LVALE, LINST, IRET, IRESOL, LADPA,JORD

      INTEGER       LMAT(4),LPRO,NBORD,JCARA,JMODL,JMATE
      ...
      CHARACTER*24  MODELE,CARELE,MATE,CHARGE,FOMULT,INFOCH,VACOMP
...
C --- STOCKAGE : MODELE,CARA_ELEM,CHAM_MATER
      ...
      ELSE
         CALL JEVEUO(CONCEP//'           .REFD','L',LREFE)
         CALL JEVEUO(ZK24(LREFE)(1:8)//'           .LIME','L',JMODG)
         IF(ZK8(JMODG).EQ.'        ')THEN
           CALL JEVEUO(ZK24(LREFE)(1:8)//'           .REFA','L',JMODG)
           CALL JEVEUO(ZK24(JMODG)(1:8)//'           .MODL','L',JMODL)
           IF(ZK8(JMODL).NE.'        ')THEN
             MODELE=ZK8(JMODL)
             CALL JEVEUO(ZK24(JMODG)(1:8)//'           .MATE','L',JMATE)
             CHMAT=ZK8(JMATE)
             CALL JEVEUO(ZK24(JMODG)(1:8)//'           .CARA','L',JCARA)
             CARAEL=ZK8(JCARA)
             GOTO 44
           ELSE
