        SUBROUTINE LMAMAT ( FAMI,KPG,KSP,MOD,IMAT,NMAT,
...
C       ----------------------------------------------------------------
        ...
        REAL*8          EPSI
        CHARACTER*8     MOD,  NOM , NOMC(34) , TYPMA
...
C
     ...
     1                 5,NOMC(1),  MATERD(1,1),  CERR(1), BL2 )
          ...
          CALL RCVALB(FAMI,KPG,KSP,'-',IMAT,' ',    'LMARC',0,
     1                ' ', 0.D0,29,NOMC(6),  VMTP1, CERR(6) , FB2 )
C
          DO 11 I = 1 , 13
          MATERD(I,2) = VMTP1(I)
 11       CONTINUE
...
C --    CONSTRUCTION DES MATRICES D ANISOTROPIE A TEMPF (T)
          CALL LMAANI( MOD, VMTP1(14) , MATERD(16,1) )
          CALL LMAANI( MOD, VMTP1(18) , MATERD(52,1) )
          CALL LMAANI( MOD, VMTP1(22) , MATERD(16,2) )
          CALL LMAANI( MOD, VMTP1(26) , MATERD(52,2) )
C
C -     RECUPERATION MATERIAU A TEMPF (T+DT)
C
          CALL RCVALB(FAMI,KPG,KSP,'+',IMAT,' ','ELAS',0,' ',
     1                0.D0,5,NOMC(1),MATERF(1,1),CERR(1),BL2 )
          IF ( CERR(3) .NE. 'OK' ) MATERF(3,1) = 0.D0
          IF ( CERR(4) .NE. 'OK' ) MATERF(4,1) = 0.D0
          IF ( CERR(5) .NE. 'OK' ) MATERF(5,1) = 0.D0
          CALL RCVALB(FAMI,KPG,KSP,'+',IMAT,' ', 'LMARC',  0,  ' ',
     1                0.D0,29,  NOMC(6),  VMTP2,         CERR(6) , FB2 )
C
          DO 21 I = 1 , 13
          MATERF(I,2) = VMTP2(I)
 21       CONTINUE
...
C --    CONSTRUCTION DES MATRICES D ANISOTROPIE A TEMPF (T+DT)
          CALL LMAANI( MOD, VMTP2(14) , MATERF(16,1) )
          CALL LMAANI( MOD, VMTP2(18) , MATERF(52,1) )
          CALL LMAANI( MOD, VMTP2(22) , MATERF(16,2) )
          CALL LMAANI( MOD, VMTP2(26) , MATERF(52,2) )
C
C -     MATERIAU CONSTANT ?
C
        MATCST = 'OUI'
