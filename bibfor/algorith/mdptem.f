     ...
     &       PARCHO,NOECHO,DT,DTS,DTU,DTMAX,DTMIN,TINIT,TFIN,NBPAS,INFO,
     &       IER,LISINS)
      IMPLICIT NONE
      INCLUDE 'jeveux.h'
     ...
     &                   PARCHO(NBCHOC,*),DPLMOD(NBCHOC,NBMODE,*)
      REAL*8             DT, TINIT, TFIN, DTMAX, DTS, DTU, DTMIN
...
C ----------------------------------------------------------------------
      ...
      INTEGER       VALI
      REAL*8        KNORM, KTANG, R8BID,R8PREM
      REAL*8        VALR(3)
...
C      
C     VERIFICATION DE PRESENCE DU PAS SI ADAPT OU RUNGE-KUTTA
...
C
      ...
      ELSEIF ( METHOD .EQ. 'NEWMARK' ) THEN
         DTP = DTS / 10.D0
         DT = MIN( DTP , DTU )
      ELSEIF (METHOD(1:5).EQ.'RUNGE') THEN
         DT = DTU
...
C
 ...
 9999 CONTINUE
C
C     SI LA METHODE N'EST PAS ADAPT, RUNGE-KUTTA OU ITMI: 
...
C     DE LA LISTE D'ARCHIVAGE DANS DYARCH.F
 ...
 30       CONTINUE
      ...
      ENDIF    
C       
C     GESTION DU PAS MAXIMAL POUR SCHEMA ADAPT OU RUNGE-KUTTA
      IF (METHOD(1:5).EQ.'ADAPT'.OR.(METHOD(1:5).EQ.'RUNGE')) THEN
     ...
     &      CALL U2MESG('A','DYNAMIQUE_16',0,' ',0,0,1,DTMAX)
      ...
      ENDIF
C
C     GESTION DU PAS MINI POUR SCHEMA RUNGE-KUTTA
      IF (METHOD(1:5).EQ.'RUNGE') THEN
          CALL GETVR8('INCREMENT','PAS_MINI',1,IARG,1,R8BID,N6)
          IF ( N6 .NE. 0 ) THEN
             CALL GETVR8('INCREMENT','PAS_MINI',1,IARG,1,DTMIN,N6)
          ELSE
