...
C
      ...
      CHARACTER*8  K8BID
      CHARACTER*19 NUMDDL,PROFNO, CHAMNO
      CHARACTER*24 INDIRF,LILI,PRNO,DEEQ,NUEQ
      INTEGER      IDEC(NBCPMX),LLREFA,NBEC,NEC
C
C-----------------------------------------------------------------------
C
      CALL JEMARQ()
...
C
      CALL DISMOI('F','NOM_GD ',NUMDDL,'NUME_DDL',IBID,K8BID,IRET)      
      CALL DISMOI('F','NB_CMP_MAX',K8BID,'GRANDEUR',NBCMP,K8BID,IRET)
      CALL DISMOI('F','NUM_GD_SI',NUMDDL,'NUME_DDL',IBID,K8BID,IRET)
      NEC  = NBEC(IBID)
      IF (NEC.GT.10) THEN
        CALL UTMESS('F',PGC,
