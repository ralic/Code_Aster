...
C-------------------------------------------------------------
      REAL*8     FPRIM, FPLAS, DFPRIM,DPU(5),DPNEW,FPNEW, TMP
...
C     AMELIORATION DES BORNES PAR ESTMATION DE DP^VP
      TMP = SINH(FPLAS/SIGM0)
      TMP = (TMP + ABS(TMP) ) / 2.D0
      DP = DT*EPSI0* (TMP)**COEFM 
...
C------CALCUL DE DP : EQUATION SCALAIRE FPRIM = 0 AVEC DPMIN< DP < DPMAX
     ...
     &      THEN
        ...
        X(4)=DP
  100   CONTINUE
        CALL UTMESS ('F','CALCDP','DETERMINATION DE DP - ABSENCE DE
     &      CONVERGENCE : AUGMENTER ITER_INTE_MAXI')
