...
C     ------------------------------------------------------------------
C      ADAPTATION - DECODAGE DE LA MACRO-COMMANDE - PHASE 04
C      --           -              -                      --
C      ECRITURE DE LA COMMANDE D'EXECUTION DE HOMARD
C     ------------------------------------------------------------------
C
      IMPLICIT NONE
C
...
C
      LVHOMA = LGCAR(28)
      VHOMAR(1:LVHOMA) = TABCAR(28)(1:LVHOMA)
C
      LREPHC = LGCAR(29)
...
C
      IF ( NIVINF.GE.2 ) THEN
        WRITE(IFM,*) 'VERSION DE HOMARD :'
        WRITE(IFM,*) VHOMAR(1:LVHOMA)
        WRITE(IFM,*) 'LOGICIEL DE LANCEMENT DE HOMARD :'
        WRITE(IFM,*) PROCHO(1:LPROCH)
        WRITE(IFM,*) 'REPERTOIRE POUR HOMARD :'
...
C
C 2.3. ==> LES ARGUMENTS DE FONCTIONNEMENT GENERAL
C
...
C
      CALL SMFMCF ( CODRET )
C
C 2.3.3. ==> VERSION DE HOMARD
C
      CALL SMDMCF ( 'ARGUMENT', CODRET )
C
      CALL PUTVTX ( 'NOM_PARA', 1, VHOMAR, LVHOMA, CODRET)
C
      CALL SMFMCF ( CODRET )
C
C 2.3.4. ==> ARGUMENTS SUIVANTS : CE SONT DES ENTIERS A CONVERTIR
C            EN CHAINE DE CARACTERE
C      0. NIVEAU D'INFORMATION
C     PUIS LES 5 NUMEROS D'UNITES LOGIQUE
C
      DO 234 , IAUX = 0 , 5
C
C 2.3.4.1. ==> PASSAGE DES ENTIERS EN CARACTERE
C
        IF ( IAUX.EQ.0 ) THEN
          JAUX = NIVINF
          LAUX = JAUX
...
C
        IF ( JAUX.EQ.0 ) THEN
          ...
          KAUX = 1
        ELSE
          CALL CODENT ( LAUX, 'G', PARAM )
          KAUX = LXLGUT(PARAM)
        ENDIF
C
C 2.3.4.2. ==> ECRITURE DES PARAMETRES
C
        CALL SMDMCF ( 'ARGUMENT', CODRET )
C
        CALL PUTVTX ( 'NOM_PARA', 1, PARAM, KAUX, CODRET)
...
C
  234 CONTINUE
C
C 2.3.5. ==> TYPE D'EXECUTABLE UTILISE
C
      IF ( TYPEXE.EQ.1 ) THEN
C
      CALL SMDMCF ( 'ARGUMENT', CODRET )
