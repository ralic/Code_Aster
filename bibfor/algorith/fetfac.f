...
C   IN/OUT IFETF:  IN : ADR. OBJ. JEV. .FETF POUR MODES CORPS RIGIDES
C      IN   BASE:  K1 : BASE SUR LAQUELLE EST CREE LA MATR_ASSE
C   -------------------------------------------------------------------
C     ASTER INFORMATIONS:
C       21/05/04 (OB): CREATION.
C----------------------------------------------------------------------
...
C DECLARATION PARAMETRES D'APPELS
      INTEGER      LMAT,IDD,NBSD,NPREC,NBSDF,IFETF
      CHARACTER*1  BASE
...
C DECLARATION VARIABLES LOCALES
      ...
      CHARACTER*19 VEMOCR,VEINPN
      CHARACTER*24 NOMSDF,NOMSDP,NOMSDA,NOMSDR
...
C CREATION STRUCTURES DE DONNEES POUR FETI AU PREMIER SOUS-DOMAINE
      IF (IDD.EQ.1) THEN
C MATASS.FETF
        NOMSDF=MATASS//'.FETF'
        ...
        IF (NIV.GE.3) THEN 
          ...
          WRITE (IFM,*)'<FETI/FETFAC> CREATION OBJET JEVEUX ',NOMSDF 
        ENDIF
...
C MATASS.FETP/.FETR     
     ...
     &    'VARIABLE',NBSD)
        ...
        IF (NIV.GE.3) THEN
          WRITE (IFM,*)'<FETI/FETFAC> CREATION OBJET JEVEUX ',NOMSDP 
          WRITE (IFM,*)'<FETI/FETFAC> CREATION OBJET JEVEUX ',NOMSDR
          WRITE(IFM,*)'DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD'
          WRITE(IFM,*)
        ENDIF   
      ENDIF
...
C RANGEMENT DANS LES STRUCTURES DE DONNEES      
      IF (NBMOCR.EQ.0) THEN
C PAS DE MODES DE CORPS RIGIDE
        ZI(IFETF+IDD-1)=-1
      ELSE
...
C MONITORING
      ...
      IF (NIV.GE.3) THEN
        WRITE(IFM,*)
        WRITE(IFM,*)'DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD'
        WRITE(IFM,*)'<FETI/FETFAC> SOUS-DOMAINE: ',NOMSD
        WRITE(IFM,*)'DE NUMERO: ',IDD,' ET TAILLE: ',NEQ
        WRITE(IFM,*)'<FETI/FETFAC> REMPLISSAGE OBJETS JEVEUX ',
     &              MATAS(1:19)
        WRITE(IFM,*)
        WRITE(IFM,*)'<FETI/FETFAC> NBRE MODES DE CORPS RIGIDES ',
     &              NBMOCR
        IF (NIV.GE.4)
     &    CALL UTIMSD(IFR,2,.FALSE.,.TRUE.,MATAS(1:19),1,' ')     
        IF (NBMOCR.NE.0)
     &    WRITE(IFM,*)'<FETI/FETFAC> SOUS-DOMAINE FLOTTANT N ',NBSDF
