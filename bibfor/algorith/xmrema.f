     ...
     &                 IZONE,GEOM,JMAESC,MMAIT,AMAIT,NMAIT,STATUE,
     &                 POSMAE,IFACE,POSMIN,IFAMIN,
...
C
      ...
      REAL*8       GEOM(3)
      INTEGER      MMAIT,NMAIT,AMAIT,STATUE
      INTEGER      POSMAE,IFACE,POSMIN,IFAMIN
...
C
      ...
      INTEGER POSMA,NUNOIN,NUNOG,NUGLA,NUGLB,NBNOTT(3)
      INTEGER N1,N2,NBNOS,NTMA
      INTEGER INO,NIVERR
      INTEGER I,J,K,IA,IMA,NUMPI(NDIM),NFACM,IFACM
      INTEGER AR(12,2),NBAR,NA,NB,NUNOA,NUNOB
      REAL*8  JEU,TAU1(3),TAU2(3),NRESE
      REAL*8  CFDISR,TOLEOU,EPSMAX
      REAL*8  COORMA(27),XI,YI,JEUMIN
...
C --- SI LE POINT DE CONTACT EST SUR UNE ARRÊTE
        IF (AMAIT.GT.0) THEN
...
C              
            IF (JEU.LT.JEUMIN) THEN
              ...
              JEUMIN = JEU
              IPROJM = IPROJ
C --- ON FORCE L'APPARIEMENT SI POSMIN EST H C-T
              IF (STATUM.EQ.3.AND.IPROJM.EQ.2) IPROJM = 1
              DO 40 K = 1,3
                T1MIN(K) = TAU1(K)
                T2MIN(K) = TAU2(K)
  40          CONTINUE
...
C
        DO 240 I=1,NDIM       
          CALL CESEXI('S',JCSD3,JCSL3,POSMAE,1,1,(IFACE-1)*NDIM+I,IAD)
          CALL ASSERT(IAD.GT.0)
          NUMPI(I) = ZI(JCSV3-1+IAD)
 240    CONTINUE
        DO 250 I=1,NDIM
          DO 260 J=1,NDIM
            CALL CESEXI('S',JCSD1,JCSL1,POSMAE,1,1,
     &                            NDIM*(NUMPI(I)-1)+J,IAD)
            CALL ASSERT(IAD.GT.0)
            COORMA(3*(I-1)+J)=ZR(JCSV1-1+IAD)
 260      CONTINUE
...
C
        CALL MMPROJ(ALIAS,NDIM,NDIM,COORMA,GEOM,
     &              ITEMAX,EPSMAX,TOLEOU,DIRAPP,R3BID,
     &              XIMIN,YIMIN,T1MIN,T2MIN,IPROJM,NIVERR)
C
C --- ECHEC DE NEWTON
C      
        IF (NIVERR.EQ.1) THEN
