      SUBROUTINE NMCRSU(SDDISC,LISINS,PARCRI,NOMCMD)
C
...
C       ON A DONNE UNE LISTE DE R8 : SOUS-DECOUPAGE INTERDIT
        NOCC = 1
        CALL WKVECT(SDDISC//'.EEVR','V V R',  NOCC*LEEVR,JEEVR)
        CALL WKVECT(SDDISC//'.EEVK','V V K16',NOCC*LEEVK,JEEVK)
        CALL WKVECT(SDDISC//'.ESUR','V V R'  ,NOCC*LESUR,JESUR)
C       EVEN = ERREUR ET METHODE = 'AUCUNE'
        IOCC = 1
...
C     RECHERCHE DU N° D'OCCURENCE DE L'EVENEMENT 'ERREUR'
      CALL UTDIDT('L',SDDISC,'ECHE',IBID,'IOCC_ERREUR',R8B,IDIV,K8B)
C
