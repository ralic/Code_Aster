       SUBROUTINE  NIGP2D ( NNO1, NNO2, NPG1, POIDSG, VFF1, VFF2,
     &                      DFDE1, DFDK1, DFDIM, DFDIP, GEOM, 
...
C TOLE CRP_21
       ...
       REAL*8         DFDE1(*), DFDK1(*), DFDIM(NNO1,2), DFDIP(NNO1,2)
...
C     CALCUL DE F EN T-  =>  FM
     ...
     &              VFF1(1,KPG),DFDE1,RBID,DFDK1,DEPLM,POIDS0,DFDIM,
     &              FM,EPS,RM)

C     CALCUL DE F POUR DT  =>  DF, RM  ET DFDIM
        CALL NMGEOM(2,NNO1,AXI,GRAND,GEOMM,KPG,POIDSG(KPG),
     &              VFF1(1,KPG),DFDE1,RBID,DFDK1,DDEPL,POIDS,DFDIM,
     &              DF,EPS,RM)

C     CALCUL DE DFDIP, RP ET POIDS EN T+
        CALL NMGEOM(2,NNO1,AXI,GRAND,GEOMP,KPG,POIDSG(KPG),
     &              VFF1(1,KPG),DFDE1,RBID,DFDK1,DEPLM,POIDS,DFDIP,
     &              FBID,EPS,RP)
...
C     CALCUL DE F ET DF POUR LA LOI DE COMPORTEMENT
        ...
        CALL R8COPY(9,FM,1,FMLDC,1)
        CALL R8SCAL(9,TETJM,FMLDC,1)
        CALL R8COPY(9,DF,1,DFLDC,1)
        CALL R8SCAL(9,TETJD,DFLDC,1)
...
C      CALCUL DE LA TRACE ET DU DEVIATEUR DE SIGC
        IF (RESI) THEN
          CALL R8COPY(4,SIGPC,1,SIGC,1)
        ELSE
          CALL R8COPY(4,SIGMC,1,SIGC,1)
        END IF
...
C        TERMES COMPLEMENTAIRES EN AXISYMETRIQUE
     ...
     &            -TIERS*(TETJD)**2 * VFF1(N,KPG) * VFF1(M,KPG)*
     &                                  TINT(3,3) /RP**2
     &            + TETJD**2*VFF1(N,KPG)/RM*PROJ(3,3,3)*VFF1(M,KPG)/RP
     &            )           
                DO 157 I = 1,2
                  KUU(1,N,I,M) =  KUU(1,N,I,M) + POIDS * (
     &             VFF1(N,KPG) /RP * SIGDF(I,M) 
     &             -TIERS*(TETJD)**2 * VFF1(N,KPG) /RP * SIGDF2(I,M) 
     &             +TETJD**2 * VFF1(N,KPG)/RM * 
     &                  ( PROJ(IND(I,1),3,3)*DFDIP(M,1) +
     &                    PROJ(IND(I,2),3,3)*DFDIP(M,2))
     &                  )
     
                  KUU(I,N,1,M) =  KUU(I,N,1,M) + POIDS * (
     &              DFDIP(N,I) * SIGEQ(3) * VFF1(M,KPG)/RP 
     &              -TIERS*(TETJD)**2 * DFDIP(N,I)*
     &                             TINT(3,3) * VFF1(M,KPG)/RP
...
C        TERME K_UP  
     ...
     &                   POIDS * DFDIP(N,I) * VFF2(M,KPG)
 110          CONTINUE
C        TERMES COMPLEMENTAIRES EN AXISYMETRIQUE
              IF (AXI) THEN
                KUA(1,N,1,M) = KUA(1,N,1,M) + 
     &                   POIDS * VFF1(N,KPG) / RP * VFF2(M,KPG)        
              END IF
 111        CONTINUE
 112      CONTINUE
...
C        TERME K_UG 
     ...
     &                SIGDF2(I,N)  * VFF2(M,KPG)                      
 129          CONTINUE
C        TERMES COMPLEMENTAIRES EN AXISYMETRIQUE
              IF (AXI) THEN
                KUA(1,N,2,M) = KUA(1,N,2,M) + POIDS*TETJD**2 *TIERS/
     &            (1.D0+GP)*TINT(3,3)*VFF1(N,KPG)/RP * VFF2(M,KPG)
              END IF 
 126        CONTINUE
 130      CONTINUE
...
C        CALCUL DE FINT_U
 ...
 189        CONTINUE
     ...
     &                       SIGEQ(3) * VFF1(N,KPG)/RP
            END IF
 190      CONTINUE
 
C        CALCUL DE FINT_P ET FINT_G
          DO 191 N = 1, NNO2
            TMP = (JP - 1.D0 - GP)*VFF2(N, KPG)
            FINTA(1,N) = FINTA(1,N) + TMP*POIDS0
            FINTA(2,N) = FINTA(2,N) + POIDS0 * SIGP(5,KPG) *VFF2(N,KPG)
 191      CONTINUE
