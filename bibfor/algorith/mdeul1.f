...
C
      ...
      ENDIF
      IF (NBNLI.NE.0 .AND. NBPAL.EQ.0 ) THEN
C      IF (NBCHOC.NE.0  ) THEN
         CALL WKVECT('&&MDEUL1.SCHOR','V V R8',NBNLI*14,JCHOR)
C        INITIALISATION POUR LE FLAMBAGE
         CALL JEVEUO(NOMRES//'           .VINT','E',JVINT)
         CALL R8INIR(NBNLI,0.D0,ZR(JVINT),1)
      ENDIF
      IF (NBREDE.NE.0) THEN
...
C
      CALL MDINIT(BASEMO,NEQGEN,NBNLI,ZR(JDEPL),ZR(JVITE),ZR(JVINT),
     &            IRET, TINIT)
      IF (IRET.NE.0) GOTO 9999
      IF (NBNLI.GT.0.AND.NBPAL.EQ.0) THEN
         CALL DCOPY(NBNLI,ZR(JVINT),1,ZR(JCHOR+13*NBNLI),1)
      ENDIF
C
...
C
     ...
     &            ZR(JACCE),
     &           ISTO2,NBNLI,ZR(JCHOR),NBSCHO, ISTO3,NBREDE,ZR(JREDR),
...
C
     ...
     &               R8BID5,NBNLI,NBCHOC,LOGCHO,DPLMOD,PARCHO,NOECHO,
     &               ZR(JCHOR), NBREDE,DPLRED,PARRED,FONRED,
     &               ZR(JREDR),ZI(JREDI),NBREVI,DPLREV,FONREV,
     &               TEMPS,NOFDEP,NOFVIT,NOFACC,NBEXCI,PSIDEL,MONMOT,
     &               NBRFIS,FK,DFK,ANGINI,
...
C
     ...
     &                  ZR(JVITE),ZR(JACCE),ISTO2,NBNLI,ZR(JCHOR),
...
C
         ...
         CALL UTTCPR('CPU.MDEUL1',4,TPS1)
          ...
          IF (MAX(RINT1,N100*TPS1(4)).GT.(RINT2*TPS1(1))) THEN
           CALL MDSIZE (NOMRES,ISTO1,NEQGEN,LPSTO,NBNLI,NBREDE)
...
C
 9999 CONTINUE
      ...
      ENDIF
      IF (NBNLI.NE.0) CALL JEDETR('&&MDEUL1.SCHOR')
      IF (NBREDE.NE.0) THEN
