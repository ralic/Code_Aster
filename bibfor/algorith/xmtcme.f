      SUBROUTINE XMTCME(OPTION,MODELE,DEPMOI,DEPDEL,RESOCO,
     &                  MEXFTE)     
C
...
C OUT MEXFTE : MATR_ELEM DE CONTACT OU FROTTEMENT
C
...
C
      CALL DETRSD('MATR_ELEM',MEXFTE)           
C
C --- PREPARATION DES MATRICES ELEMENTAIRES 
C
      CALL MEMARE('V',MEXFTE,MODELE,' ',' ','RIGI_MECA')      
C
C --- CALCUL DU MATR_ELEM 
C 
      LPAOUT(1) = 'PMATUUR'
      LCHOUT(1) = MEXFTE(1:15)//'.M01'
      LPAOUT(2) = 'PMATUNS'
      LCHOUT(2) = MEXFTE(1:15)//'.M02'
...
C
     ...
     &              NBOUT ,LPAOUT,LCHOUT)
      ENDIF 
C     
      CALL REAJRE(MEXFTE,LCHOUT(1),'V')
      CALL REAJRE(MEXFTE,LCHOUT(2),'V')
C
      CALL JEDEMA()
      END
