...
C  MISE A JOUR DU RESIDU ET RESIDU PROJETE
              IAUX1=IDDFRO+NBI
              CALL DGEMV('N',NBI4,NB4,1.D0,ZR(IAUX1),NBI4,ZR(IMSMR),
     &                   1,0.D0,ZR(IR1),1)
              CALL DAXPY(NBI4,-1.D0,ZR(IR1),1,ZR(IRR),1)
              K24BID(1:4)='VIDE'
     ...
     &              LSTOGI,INFOFE,IREX,IPRJ,NBPROC,RANG,K24BID)
              CALL DAXPY(NBI4,-1.D0,ZR(IR2),1,ZR(IRG),1)
...
C PRODUIT MATRICE-VECTEUR (AI*VI)T * SOMME PRECEDENTE
              IAUX1=IDDFRO+NBI
              CALL DGEMV('T',NBI4,NB4,1.D0,ZR(IAUX1),NBI4,
     &                   ZR(IMSMR2),1,0.D0,ZR(IMSMR3),1)
C PRODUIT MATRICE-VECTEUR VIT * GK
              IAUX1=IDDRO+NBI
              CALL DGEMV('T',NBI4,NB4,1.D0,ZR(IAUX1),NBI4,
     &                   ZR(IR1),1,0.D0,ZR(IMSMR4),1)
C THETAI
              DO 45 J=1,NBDDSM
                J1=J-1
...
C PRODUIT MATRICE-VECTEUR VI * THETAI ET MISE A JOUR STOCKAGE
              IAUX1=IDDRO+NBI
              CALL DGEMV('N',NBI4,NB4,1.D0,ZR(IAUX1),NBI4,
     &                   ZR(IMSMR3),1,1.D0,ZR(IMSMR1),1)
              CALL JELIBE('&&FETI.PS.'//K8BID1)
              CALL JELIBE('&&FETI.DD.'//K8BID1)
              CALL JELIBE('&&FETI.FIDD.'//K8BID1)
            ENDIF
...
C MISE A JOUR VECTEUR SOLUTION
          CALL DAXPY(NBI4,1.D0,ZR(IMSMR1),1,ZR(IR2),1)
