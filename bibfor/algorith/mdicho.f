     ...
     +                    NBCHTO,NBCHOC,NBSISM,PARCHO,NOECHO )
      IMPLICIT REAL*8 (A-H,O-Z)
      INTEGER             NBSTOC,NBCHOC,NBCHTO,NBSISM
...
C IN  : NBCHTO : DIMENSION DES TABLEAUX (NBCHOC + NBSISM)
C IN  : NBCHOC : NOMBRE DE NOEUDS DE CHOC
C IN  : NBSISM : NOMBRE DE DISPOSITIFS ANTI-SISMIQUES
...
C
      ...
      IF (NIMPR.EQ.0) THEN
         CALL GETVTX('IMPRESSION','TOUT',1,1,1,NIVEAU,NIMPR)
         IF (NIVEAU(1:3).NE.'OUI') GOTO 9999
         NIVEAU='TOUT_LOC'
      ENDIF
      CALL GETVR8('IMPRESSION','INST_INIT',1,1,1,DEBUT,NDEB)
      CALL GETVR8('IMPRESSION','INST_FIN',1,1,1,FIN,NFIN)
...
C        --- TAUX DE RECONSTITUTION ---
     ...
     +     'AU NOEUD DE CHOC:',NOECHO(ICHO,1),':',PARCHO(ICHO,47)
         IF (TYPEOB(1:2).EQ.'BI') 
     +      WRITE(IFR,1070)
     +     'TAUX DE RECONSTITUTION DE LA SOLUTION STATIQUE',
     +     'AU NOEUD DE CHOC:',NOECHO(ICHO,5),':',PARCHO(ICHO,48)
         ENDIF
C
