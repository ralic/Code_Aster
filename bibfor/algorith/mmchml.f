...
C
      ...
      INTEGER      ICOMPL,IFROTT,IUSURE
      REAL*8       COEFFF,KWEAR,HWEAR,COMPEN,COMPEV,ASPERI
...
C         MAILLE TARDIVE ZI(JLIEL-1+IEL) < 0
          ...
          IFROTT = MMINFI(DEFICO,'FROTTEMENT_ZONE'  ,IZONE )
          COEFFF = MMINFR(DEFICO,'COEF_COULOMB'     ,IZONE ) 
          ICOMPL = MMINFI(DEFICO,'COMPLIANCE'       ,IZONE )
          ASPERI = MMINFR(DEFICO,'COMPL_ASPERITE'   ,IZONE )
          COMPEN = MMINFR(DEFICO,'COMPL_E_N'        ,IZONE )
          COMPEV = MMINFR(DEFICO,'COMPL_E_V'        ,IZONE )
     ...
     &                COEFCR)
          CALL CFMMCO(DEFICO,RESOCO,IZONE,'COEF_REGU_FROT','L',
     &                COEFFR)     
          CALL CFMMCO(DEFICO,RESOCO,IZONE,'COEF_PENA_CONT','L',
     &                COEFCP)
          CALL CFMMCO(DEFICO,RESOCO,IZONE,'COEF_PENA_FROT','L',
     &                COEFFP)  
          CALL CFMMCO(DEFICO,RESOCO,IZONE,'COEF_STAB_CONT','L',
     &                COEFCS)
          CALL CFMMCO(DEFICO,RESOCO,IZONE,'COEF_STAB_FROT','L',
     &                COEFFS)
...
C         REMPLISSAGE EFFECTIF
          ...
          ZR(JVALV-1+16) = IFORM
          ZR(JVALV-1+17) = COEFCR
          ZR(JVALV-1+18) = COEFCS
          ZR(JVALV-1+19) = COEFCP
          ZR(JVALV-1+20) = IFROTT
          ZR(JVALV-1+21) = COEFFF
          ZR(JVALV-1+22) = COEFFR
          ZR(JVALV-1+23) = COEFFS
          ZR(JVALV-1+24) = COEFFP
          ZR(JVALV-1+25) = ICOMPL
          ZR(JVALV-1+26) = ASPERI
          ZR(JVALV-1+27) = COMPEN
          ZR(JVALV-1+28) = COMPEV 
          ZR(JVALV-1+29) = IUSURE
          ZR(JVALV-1+30) = KWEAR
          ZR(JVALV-1+31) = HWEAR
