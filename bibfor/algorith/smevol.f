...
C
   ...
   10 CONTINUE
      ...
      IF (TEST.EQ.0) THEN
        ...
        CALL MECACT('V',CHFTRC,'LIGREL',LIGRMO,'ADRSJEVN',2,NOMCM2,VALI,
     &              R8B,CBID,K8B)
      ELSE
        CHFTRC = ' '
      END IF
...
C ----------------
        NUM0 = ZI(JORDR)
        CALL RSEXCH('F',TEMPER, 'TEMP', NUM0, TEMPE, IRET )
...
C
     ...
     &                'V','OUI')
        ...
        CALL RSEXCH(' ',TEMPER,'META_ELNO',NUM0,NOMCH,IRET)
...
C ----------------
        NUM1 = ZI(JORDR+1)
        CALL RSEXCH('F',TEMPER,'TEMP', NUM1,TEMPE,IRET)
...
C
     ...
     &                'V','OUI')
        ...
        CALL RSEXCH(' ',TEMPER, 'META_ELNO', NUM1, NOMCH, IRET )
        CALL COPISD('CHAMP_GD','G','&&SMEVOL.PHAS_META1',NOMCH(1:19))
...
C
        CALL RSEXCH('F',TEMPER, 'TEMP', NUM1, TEMPA, IRET )
        CALL RSEXCH('F',TEMPER, 'TEMP', NUM2, TEMPE, IRET )
        CALL RSEXCH('F',TEMPER, 'META_ELNO', NUM2, PHASIN, IRET )
        CALL RSEXCH('F',TEMPER, 'TEMP', NUM3, TEMPI, IRET )
...
C ----- STOCKAGE DU CHAMP DANS LA S D EVOL_THER
        ...
        CALL RSEXCH(' ',TEMPER, 'META_ELNO', NUM3, NOMCH, IRET )
