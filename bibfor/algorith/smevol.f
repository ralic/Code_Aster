...
C --- ET STOCKAGE DU CHAMP INITIAL DANS LA S D EVOL_THER (PAS 0 ET 1)
      ...
      IF (NUMPHA.EQ.0) THEN
        ...
        IF (OPTION(1:14).EQ.'META_ELGA_TEMP') THEN
          OPTIO2 = 'META_INIT'
          LPAOUT(1) = 'PPHASOU'
     ...
     &                'V')
          ...
          CALL RSEXCH(TEMPER,'META_ELGA_TEMP', NUM0, NOMCH, IRET)
          CALL COPISD('CHAMP_GD','G','&&SMEVOL.PHAS_META1',NOMCH(1:19))
          CALL RSNOCH(TEMPER,'META_ELGA_TEMP',NUM0,' ')
          WRITE(IFM,1010) 'META_ELGA_TEMP', NUM0, INST0

        ELSE IF (OPTION(1:14).EQ.'META_ELNO_TEMP') THEN
          OPTIO2 = 'META_INIT_ELNO'
          LPAOUT(1) = 'PPHASOU'
          LCHOUT(1) = '&&SMEVOL.PHAS_META1'
          LPAOUT(2) = 'PPHASNOU'
          LCHOUT(2) = '&&SMEVOL.PHAS_META2'

          CALL COPISD('CHAM_ELEM_S','V',COMPOR,LCHOUT(1))
          CALL COPISD('CHAM_ELEM_S','V',COMPOR,LCHOUT(2))
     ...
     &                'V')
          ...
          CALL COPISD('CHAMP_GD','G','&&SMEVOL.PHAS_META2',NOMCH(1:19))
        ...
        IF (OPTION(1:14).EQ.'META_ELGA_TEMP') THEN
          ...
          CALL CALCUL('S',OPTIO2,LIGRMO,4,LCHIN,LPAIN,1,LCHOUT,LPAOUT,
     &                'V')
          CALL RSEXCH ( TEMPER, 'META_ELGA_TEMP', NUM1, NOMCH, IRET )
          CALL COPISD('CHAMP_GD','G','&&SMEVOL.PHAS_META1',NOMCH(1:19))
          CALL RSNOCH ( TEMPER, 'META_ELGA_TEMP', NUM1, ' ' )
          WRITE(IFM,1010) 'META_ELGA_TEMP', NUM1, INST1
...
C
        IF ( ZI(JORDR+IORD-1) .LT. NUMPHA ) GOTO 20
C
...
C
        ...
        IF (IRET.GT.0) THEN
          ...
          CALL UTFINM
        END IF
        CALL RSEXCH ( TEMPER, 'META_ELGA_TEMP', NUM2, PHASIN, IRET )
        IF (IRET.GT.0) THEN
          CALL UTDEBM('F','SMEVOL','LE CHAMP DE "META_ELGA_TEMP" '//
     +                          'N''EXISTE PAS')
          CALL UTIMPI('S',' POUR LE NUMERO D''ORDRE ',1,NUM2)
          CALL UTFINM
        END IF
...
C ------ STOCKAGE DU CHAMP DANS LA S D EVOL_THER
          ...
          CALL RSEXCH ( TEMPER, 'META_ELGA_TEMP', NUM3, NOMCH, IRET )
          CALL COPISD('CHAMP_GD','G','&&SMEVOL.PHAS_META3',NOMCH(1:19))
          CALL RSNOCH ( TEMPER, 'META_ELGA_TEMP', NUM3, ' ' )
          WRITE(IFM,1010) 'META_ELGA_TEMP', NUM3, INST2
        ...
        ELSE IF (OPTION(1:14).EQ.'META_ELNO_TEMP') THEN
          ...
          CALL CALCUL('S',OPTION,LIGRMO,8,LCHIN,LPAIN,2,LCHOUT,LPAOUT,
     &                'V')
...
C ----- STOCKAGE DU CHAMP DANS LA S D EVOL_THER
          ...
          CALL COPISD('CHAMP_GD','G','&&SMEVOL.PHAS_META4',NOMCH(1:19))
