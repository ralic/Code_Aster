...
C
      ...
      CHARACTER*24 JEUSUP,METHCO,JEUINI,PENAL,JEUPOU,JEUCOQ
      INTEGER      JJSUP,JMETH,JDIM,JPENA,JJPOU,JJCOQ
...
C
          CALL U2MESS('F','ALGORITH10_22')
C
        ELSEIF (APPAR .EQ. 3) THEN
C
          CALL U2MESS('F','ALGORITH10_23')
C
         END IF
C
...
C
          ZR(JFRO-1+K) = ZR(JCMCF+12*(IZONE-1)+4)
          ZR(JPENA-1+2*K-1) = ZR(JCMCF+12*(IZONE-1)+2)
          ZR(JPENA-1+2*K) = ZR(JCMCF+12*(IZONE-1)+3)
          ZR(JCOMA-1+K) = ZR(JCMCF+12*(IZONE-1)+6)
 60     CONTINUE
C
...
C
      ...
      IF (NESCL .GT. NESMAX) THEN
        CALL U2MESS('F','ALGORITH10_24')
      END IF
      IF (ZI(JAPPTR+NESCL) .GT. 30*NESMAX) THEN
        CALL U2MESS('F','ALGORITH10_25')
      END IF
