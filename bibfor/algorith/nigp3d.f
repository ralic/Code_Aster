       SUBROUTINE  NIGP3D ( NNO1, NNO2, NPG1, POIDSG, VFF1, VFF2,
     &                     DFDE1, DFDN1, DFDK1, DFDIM, DFDIP, GEOM, 
...
C TOLE CRP_21
       ...
       REAL*8        DFDE1(*), DFDN1(*), DFDK1(*), DFDIM(NNO1,3)
...
C     CALCUL DE F EN T-  =>  FM
        CALL NMGEOM(3,NNO1,.FALSE.,GRAND,GEOM,KPG,POIDSG(KPG),
     &              VFF1(1,KPG),DFDE1,DFDN1,DFDK1,DEPLM,POIDS0,DFDIM,
     &              FM,EPS,RBID)
C     CALCUL DE F POUR DT  =>  DF ET DFDIM
        CALL NMGEOM(3,NNO1,.FALSE.,GRAND,GEOMM,KPG,POIDSG(KPG),
     &              VFF1(1,KPG),DFDE1,DFDN1,DFDK1,DDEPL,POIDS,DFDIM,
     &              DF,EPS,RBID)
C     CALCUL DE DFDIP ET POIDS EN T+
        CALL NMGEOM(3,NNO1,.FALSE.,GRAND,GEOMP,KPG,POIDSG(KPG),
     &              VFF1(1,KPG),DFDE1,DFDN1,DFDK1,DEPLM,POIDS,DFDIP,
     &              FBID,EPS,RBID)
...
C      CALCUL DE LA TRACE ET DU DEVIATEUR DE SIGC
        IF (RESI) THEN
          CALL R8COPY(6,SIGPC,1,SIGC,1)
        ELSE
          CALL R8COPY(6,SIGMC,1,SIGC,1)
        END IF
...
C        CALCUL DE FINT_P ET FINT_G
          DO 291 N = 1, NNO2
            ...
            FINTA(2,N) = FINTA(2,N) + POIDS0 * SIGP(7,KPG) *VFF2(N,KPG)
 291      CONTINUE
