...
C
      ...
      CHARACTER*24 DEPPLU
      CHARACTER*19 DDEPLA,DEPDEL,CNCINE
      CHARACTER*19 MATASS
      INTEGER      CTCCVG(2)
C      
C ----------------------------------------------------------------------
...
C
     ...
     &               DEPPLU,DEPDEL,CTCFIX)
      ELSE IF (IMETH.EQ.0) THEN
         CALL ALGOCO(DEFICO,RESOCO,MATASS,LMAT  ,LDSCON,
     &               NOMA  ,DDEPLA,DEPPLU,CTCCVG)
      ELSE IF (IMETH.EQ.1) THEN
         CALL ALGOCL(DEFICO,RESOCO,MATASS,LMAT  ,LDSCON,
     &               NOMA  ,DEPPLU,REAGEO,REAPRE,DDEPLA,
     &               CTCCVG,CTCFIX)
      ELSE IF (IMETH.EQ.2) THEN
         CALL FRO2GD(DEFICO,RESOCO,LMAT  ,LDSCON,NOMA  ,
     &               DEPPLU,REAGEO,REAPRE,DEPDEL,DDEPLA,
     &               CTCCVG)
      ELSE IF (IMETH.EQ.3) THEN
         CALL FROPGD(DEFICO,RESOCO,LMAT  ,LDSCON,NOMA  ,
     &               DDEPLA,DEPPLU,RESIGR,REAGEO,REAPRE,
     ...
     &               DEPDEL,CTCCVG)
      ELSE IF (IMETH.EQ.5) THEN
         CALL FROGDP(DEFICO,RESOCO,LMAT  ,LDSCON,NOMA  ,
     &               DDEPLA,DEPPLU,RESIGR,REAPRE,DEPDEL,
     &               CTCFIX)
      ELSE IF (IMETH.EQ.7) THEN
         CALL ALGOGL(DEFICO,RESOCO,MATASS,LMAT  ,LDSCON,
     &               NOMA  ,DDEPLA,DEPPLU,CTCCVG)
      ELSE IF (IMETH.EQ.9) THEN
         CALL ALGOCG(DEFICO,RESOCO,LMAT  ,NOMA  ,CNCINE,
     &               DDEPLA,CTCCVG)
      ELSE
         CALL ASSERT(.FALSE.)
      ENDIF
