...
C
      CHARACTER*6      PGC
      CHARACTER*8 NOMRES,TYPINT,BASMOD
      CHARACTER*14 OPTION
...
C
      CALL JEVEUO(NOMRES//'      .CYCL.TYPE','L',LLTYP)
      TYPINT=ZK8(LLTYP)
C
C-----------------RECUPERATION DU NOMBRE DE SECTEURS--------------------
C
      CALL JEVEUO(NOMRES//'      .CYCL.NBSC','L',LLNBS)
      NBSEC=ZI(LLNBS)
...
C
      ...
      NBDIA1=-NBDIA1
      CALL GETVTX('CALCUL','TOUT_DIAM',1,1,0,' ',NBDIA2)
      NBDIA2=-NBDIA2
C
      IF(NBDIA1.EQ.0.AND.NBDIA2.EQ.0) THEN
          CALL UTDEBM('F',PGC,' CALCUL CYCLIQUE:'//
     &'AUCUN NOMBRE DE DIAMETRE NODAUX DEMANDE')
          CALL UTFINM
      ENDIF
      IF(NBDIA2.GT.0) THEN
...
C
     ...
     &'CALCUL DU NOMBRE DE DIAMETRES MODAUX DEMANDE IMPOSSIBLE')
          CALL UTIMPI('L','NOMBRE DE DIAMETRE DEMANDE -->',1,IDIA)
          CALL UTFINM
        ENDIF
 30   CONTINUE
C
       IF(ICOMP.LT.NBDIA)  THEN
         CALL UTDEBM('I',PGC,'CALCUL DES MODES PROPRES LIMITE A')
         CALL UTIMPI('L',' NOMBRE DE DIAMETRES MAXIMUM -->',1,
     &MAXDIA)
         CALL UTFINM
       ENDIF
C
...
C
      CALL WKVECT(NOMRES//'      .CYCL.DIAM','G V I',NBDIA*2,LDNBD)
C
      DO 40 I=1,NBDIA
...
C
      ...
      IF(OPTION.EQ.'PLUS_PETITE'.OR.OPTION.EQ.'CENTRE') THEN
        IF(NBLIF.GT.1) THEN
          CALL UTDEBM('F',PGC,
     &'LISTE DE FREQUENCES INCOMPATIBLE AVEC OPTION')
          CALL UTIMPI('L','NOMBRE DE FREQUENCES -->',1,NBLIF)
...
C
      CALL JEVEUO(NOMRES//'      .CYCL.DESC','L',LLNUM)
      NBMOS=ZI(LLNUM)
      NBDDR=ZI(LLNUM+1)
      NBDAX=ZI(LLNUM+2)
      NBMCAL=ZI(LLNUM+3)
...
C
      IF(NBDAX.GT.0) THEN
        CALL JEVEUO(NOMRES//'      .CYCL.NUIN','L',LLNUM)
        NUMA=ZI(LLNUM+2)
...
C
      NTAIL=NBMCAL*NBDIA
      CALL WKVECT(NOMRES//'     .CYCL.FREQ','G V R',NTAIL,LDFRE)
      NTAIL=NBDIA*NBMCAL*NBDDG
      CALL WKVECT(NOMRES//'     .CYCL.CMODE','G V C',NTAIL,LDMOC)
C
...
C
     ...
     &' RESOLUTION DU PROBLEME GENERALISE COMPLEXE')
        CALL UTIMPI('L','     NOMBRE DE MODES DYNAMIQUES: ',1,NBMOS)
        CALL UTIMPI('L','     NOMBRE DE DDL DROITE: ',1,NBDDR)
        IF(NBDAX.GT.0) THEN
          CALL UTIMPI('L','     NOMBRE DE DDL AXE: ',1,NBDAX)
          CALL UTIMPI('L','            DONT CYCLIQUES: ',1,NBDAX0)
          CALL UTIMPI('L','            DONT NON CYCLIQUES: ',1,NBDAX1)
        ENDIF
        CALL UTIMPI('L','     DIMENSIOM MAX DU PROBLEME: ',1,NBDDG)
        CALL UTFINM
C
C
...
C
     ...
     &ZC(LTZV1),ZC(LTZV2),ZC(LTZV3),ZR(LTRV1),ZR(LTRV2),
