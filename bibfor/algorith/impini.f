      SUBROUTINE IMPINI(IMPRCO,SUIVCO,ZFON,FONACT)
C
...
C CETTE ROUTINE PREPARE LA SD IMPRCO = '&&OP0070.IMPR.'
C
C
C IN IMPRCO : NOM DE LA SD POUR L'AFFICHAGE
C IN SUIVCO : NOM DE LA SD POUR LE SUIVI
C IN ZFON   : LOBNGUEUR DU VECTEUR FONACT
...
C
      ZK24(JIMPSU-1+1) = SUIVCO
      SUIVNB = SUIVCO(1:14)//'NBSUIV'
...
C
      IF (NBOCC.EQ.0) THEN
        IOCC           = 0
        ZI(JIMPIN-1+6) = 0
        CALL IMPSTD(IMPRCO(1:14),IMPTMP(1:14),FONACT,INFCMP,NBSUIV,
...
C
        ELSE IF (TYPM(1:7).EQ.'MINIMUM') THEN
          ZI(JIMPIN-1+6) = 2
          CALL IMPMIN(IMPRCO(1:14),IMPTMP(1:14),FONACT,INFCMP,NBSUIV,
     &                ZTIT,ZDEF,ZFON,MOTFAC,IOCC)
C
C --- PERSONNALISE
C
        ELSE
          ZI(JIMPIN-1+6) = 1
          CALL IMPPER(IMPRCO(1:14),IMPTMP(1:14),FONACT,INFCMP,NBSUIV,
     &                ZTIT,ZDEF,ZFON,MOTFAC,IOCC,TYPM(1:9))
        ENDIF
 10   CONTINUE
