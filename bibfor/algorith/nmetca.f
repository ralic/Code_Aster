      SUBROUTINE NMETCA (MODELE, MATE  , TBGRCA, INST  ,
     &                   NUMARC, MAILLA, SIGMAP, SIGMAM )
...
C                   INST(2) : DELTAT
C      IN  NUMARC : NUMERO COURANT D'ARCHIVAGE
C      IN  MAILLA : MAILLAGE SOUS-TENDU PAR LE MAILLAGE
C      IN  SIGMAP : CONTRAINTES A L'INSTANT ACTUEL
...
C 0.1. ==> ARGUMENTS
      ...
      INTEGER      NUMARC
      CHARACTER*8  MAILLA
      CHARACTER*24 MODELE, MATE, SIGMAP, SIGMAM
      REAL*8       TBGRCA(3), INST(5)
...
C
      ...
      REAL*8       INSTAP, DELTAT,RCMP(2),SOMME(1)
      REAL*8       LONGC, PRESC
      REAL*8       R8BID
      REAL*8       TABERR(NPARA)
C
