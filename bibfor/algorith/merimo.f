...
C --- FIN DECLARATIONS NORMALISEES JEVEUX ------------------------------
      INTEGER NCHAR,JCHAR,JINF,ICHA,JMER,JRES,IRET,IFISS
      INTEGER JDIR,IBID,JFISS
      INTEGER NBRES,NDIR,NBRIG,NUMDI
      REAL*8 ITER
      CHARACTER*8 LPAOUT(6),LPAIN(50),K8BID,NOMCHA,NOMGD
      CHARACTER*16 OPTINT
      CHARACTER*24 LIGRMO,LIGRCH,LCHOUT(6),LCHIN(50)
...
C    LECTURE DES VARIABLES DE COMMANDE EN T- ET T+ ET VAL. DE REF
      ...
      CALL NMVCEX('CORR',COMMOI,CORMOI)
      ...
      CALL NMVCEX('CORR',COMPLU,CORPLU)
...
C     -- ON TESTE LA NATURE DU CHAMP DE TEMPERATURE :
      ...
      CALL CALCUL('S',OPTION,LIGRMO,50,LCHIN,LPAIN,6,LCHOUT,LPAOUT,'V')
        ZK24(JMER) = LCHOUT(1)
      ...
      ELSE IF (OPTION(1:10).EQ.'RIGI_MECA ') THEN
        ...
        LPAIN(13) = 'PTEMPSR'
      CALL CALCUL('S',OPTION,LIGRMO,50,LCHIN,LPAIN,1,LCHOUT,LPAOUT,'V')
        ZK24(JMER) = LCHOUT(1)
        ZK24(JMER+1) = LCHOUT(2)
        NBRIG = 2
        CALL JEECRA(MERIGI,'LONUTI',NBRIG,K8BID)
      ...
      CALL CALCUL('S',OPTION,LIGRMO,50,LCHIN,LPAIN,5,LCHOUT,LPAOUT,'V')
        ZK24(JMER) = LCHOUT(1)
        ZK24(JMER+1) = LCHOUT(2)
        NBRIG = 2
        CALL JEECRA(MERIGI,'LONUTI',NBRIG,K8BID)
      ...
      ELSE IF (OPTION(1:9).EQ.'RAPH_MECA') THEN
        CALL CALCUL('S',OPTION,LIGRMO,50,LCHIN,LPAIN,4,
     &              LCHOUT(3),LPAOUT(3),'V')
        NBRES = 1
        ZK24(JRES+NBRES-1) = LCHOUT(3)
        CALL JEECRA(RESIDU,'LONUTI',NBRES,K8BID)
