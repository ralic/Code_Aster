      SUBROUTINE XLS3D(CALLST,GRILLE,JLTSV,JLTSL,JLNSV,JLNSL,NBNO,JCOOR,
...
C     DES MAILLES DE FISSURE
 ...
 5    CONTINUE
C
C     VERIFICATION DE L'ORIENTATION DES MAILLES DE LA FISSURES
      SENS='&&XLS3D.ORI_MAFIS'
      CALL XORIMA(NBMAF,JDLIMA,JCONX1,JCONX2,JCOOR,SENS)
...
C           ON VERIFIE QUE CETTE NORMALE EST ORIENTEE COMME LA
C           PRECENDENTE, A PART POUR LE 1ER TRIANGLE DE CHAQUE MAILLE
C           CAR XORIMA VERIFIE LA NORMAL DU 1ER TRIANGLE DE CHQ MAILL 
            IF (ITRI.GT.1)THEN
...
C           ON GARDE CETTE NORMALE COMME REF POUR LE TRIANGLE SUIVANT
            DO 213 I=1,3
              VNREF(I)=VN(I)
 213        CONTINUE
...
C             POUR RECUPERER UN 3EME POINT (SOMMET) DE LA MAILLE
...
C             CALCUL DE LA NORMALE A LA MAILLE
              CALL PROVEC(AB,AC,VN)
              CALL NORMEV(VN,NORME)
...
C             ON GARDE CETTE NORMALE COMME REF POUR LE SEG SUIVANT
              VNREF(1)=VN(1)
              VNREF(2)=VN(2)
              VNREF(3)=VN(3)
...
C             ON RAMENE M SUR LES BORDS S'IL LE FAUT
...
C             MISE EN MEMOIRE DE LSN=PM.N POUR LE SEG LE PLUS PROCHE
