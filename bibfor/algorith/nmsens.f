...
C MODIF ALGORITH  DATE 04/02/2004   AUTEUR CAMBIER S.CAMBIER 
C ======================================================================
...
C -------------- FIN  DECLARATIONS  NORMALISEES  JEVEUX ----------------
      ...
      INTEGER JVITPS,JACCPS
      LOGICAL LBID
      REAL*8  R8BID,CODONN(2)
      CHARACTER*6 NOMPRO
      PARAMETER ( NOMPRO = 'NMSENS' )
      CHARACTER*8  RESULT,NOPASE
      CHARACTER*14 K14BID
      CHARACTER*19 K19BID,CNDONN(2)
      CHARACTER*24 K24BID
...
C -- CAS DYNAMIQUE
     ...
     &            K24BID,K24BID)
      ...
      CODONN(1) =  1
      CODONN(2) =  1
      CALL NMRESO (K14BID, 2     , CODONN, CNDONN, 0,
...
C -- MISE A JOUR DES CHAMPS VITESSE ET ACCELARATION DERIVES
      ...
      IF (CMD(1:4).EQ.'DYNA') THEN
        ...
        CALL R8AXPY(NEQ, COEVIT, ZR(JDEPLS), 1, ZR(JVITPS), 1)

        CALL JEVEUO(ACCPLU(1:19)//'.VALE','E',JACCPS )
        CALL R8AXPY(NEQ, COEACC, ZR(JDEPLS), 1, ZR(JACCPS), 1)
      END IF           

C -- MISE A JOUR DU CHAMP DE DEPLACEMENT DERIVE

      CALL JEVEUO(DEPMOS(1:19)//'.VALE','L',JDEPMS)
      CALL R8AXPY(NEQ, 1.D0, ZR(JDEPMS), 1, ZR(JDEPLS), 1)


C -- INTEGRATION DU COMPORTEMENT DERIVE

      CALL METYSE(NBPASE, INPSCO, NOPASE, TYPESE, STYPSE)
C
      IF (TYPESE.EQ.2) THEN
        CALL UTMESS('F',NOMPRO,'DERIVEE PAR RAPPORT A UN PARAMETRE'
     &   //' DE TYPE 2(CHARGEMENT DE TYPE DIRICHLET) NON DISPONIBLE')
      END IF
C
...
C    ARCHIVAGE
     ...
     &             IBID,K24BID,K24BID,K24BID,NRPASE.EQ.NBPASE)
      ELSE
        CALL NMARCH (RESULT, NUMINS, PARTPS, .FALSE., COMPOR,
     &             K19BID, VALPLU, K19BID, CMD ,VITPLU,ACCPLU ,
     &             NBMODS,DEPENT,VITENT,ACCENT,NRPASE.EQ.NBPASE)      
      END IF
      WRITE(IFM,*) 
