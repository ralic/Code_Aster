      SUBROUTINE PREGCP(RESOCO,NEQ   ,NBLIAI,TOLE  ,EPSIPC,
...
C       GRAD=A(AC,:)*UU-SSGRAD(AC)
     ...
     &              GRAD(ILIAC))
        GRAD(ILIAC) = GRAD(ILIAC) - SSGRAD(ILIAI)
   30 CONTINUE
C
...
C
      ...
      DO 40 ILIAC = 1,NBLIAC
        CONVER = MAX(CONVER,ABS(GRAD(ILIAC)))
   40 CONTINUE
      IF (NIV.GE.2) THEN
...
C
     ...
     &              APDDL(1+JDECAL),ZR(JDDELT))
   50 CONTINUE
C
C --- RESOLUTION
C --- DU=K-1*A(AC,:)'*DIRECH
C
      CALL DCOPY(NEQ,ZR(JDDELT),1,ZR(JSECMB),1)
...
C
      DO 70 ILIAC = 1,NBLIAC
        ILIAI = INLIAC(ILIAC)
        SSGRPR(ILIAI) = SSGRPR(ILIAI) + ALPHA*DIRECH(ILIAC)
   70 CONTINUE

      CALL DAXPY(NEQ,-ALPHA,ZR(JDDELT),1,UU,1)
