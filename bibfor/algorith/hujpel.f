        SUBROUTINE HUJPEL ( ETATD,MOD,CRIT,IMAT,MATERF,ANGMAS,
...
C       ----------------------------------------------------------------
        INTEGER         NVI, IMAT, IRET
        REAL*8          MATERF(22,2),SIGD(6),SIGF(6),ANGMAS(3)
        REAL*8          VIND(*),VINF(*),DEPS(6),CRIT(*)
        CHARACTER*8     MOD
        CHARACTER*7     ETATD
C
        INTEGER         I
        REAL*8          ZERO,UN,R8VIDE,BID66(6,6)
        LOGICAL         REORIE
        PARAMETER      (ZERO = 0.D0)
        PARAMETER      (UN   = 1.D0)
C       ----------------------------------------------------------------
...
C --- ORIENTATION DES CONTRAINTES SELON ANGMAS VERS REPERE LOCAL
     ...
     &          .OR. (ANGMAS(3).NE.ZERO)
        ...
        CALL HUJPRE (ETATD,MOD,CRIT,IMAT,MATERF,DEPS,SIGD,
     &                SIGF, VIND, IRET)
