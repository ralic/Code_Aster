...
C    
      CALL TPSVIL(TPS,X,DPC,TSCHEM,FLUPHI,A,B,CTPS,ENER,PREC)
C
C---------------------------------------------------------------
C----CONSTANTES     
      NRJACT = ENER
...
C  
      IF ((1+(CTPS*TPS*FLUPHI)).LE.0.D0) THEN 
         CALL UTMESS('F','AVILOG_1','ERREUR LOG NEGATIF OU NUL')
      ENDIF
...
C----CALCUL DE G1-----------------------------------------------
C
      G1 = A*EXP(-NRJACT/(TSCHEM+273.15D0))*X      
C
C----CALCUL DE G2-----------------------------------------------
C
      G2 = B*EXP(-NRJACT/(TSCHEM+273.15D0))*X
      G = FP1*G1 + FP2*G2
      G = G*THETA
