      SUBROUTINE ASSVSU(NNO,NNOS,NNOM ,NFACE,
     & GEOM,CRIT,DEPLM,DDEPL,
     & CONGEM,CONGEP,VINTM,VINTP,
     & DEFGEM,DEFGEP,DSDE,
     & MATUU,VECTU,RINSTM,RINSTP,
     & OPTION,IMATE,MECANI,PRESS1,PRESS2,TEMPE,
     & DIMDEF,DIMCON,DIMUEL,NBVARI,
...
C DANS VALCEV DERNIER INDICE  INDIQUE LE NUMERO DE
C FACE DE L ELEMENT MAITRE COMMUN AVEC LE VOISIN CONSIDERE

C DANS VALFAV LE PREMIR INDICE INDIQUE
C UN NUMERO DE FACE DASN LE VOISIN
C LE DERNIER INDICE  INDIQUE LE NUMERO DE
C FACE DE L ELEMENT MAITRE COMMUN AVEC LE VOISIN CONSIDERE
...
C
      ...
      PARAMETER (MAXDIM=3)
      INTEGER NNOM
      INTEGER NNO,NNOS,IMATE,DIMDEF,DIMCON,DIMUEL
      INTEGER NBVARI,NDDLS,NDDLM,NDDLK,NMEC,NP1,NP2,NDIM,CODRET
...
C
C FW1S(1:MAXFA+1) DERIVEE FLUW / P1_K PUIS P_1,SIGMA
C FW2S(1:MAXFA+1) DERIVEE FLUW / P2_K PUIS P_2,SIGMA
C FW1SV(IFA DE 1A MAXFA) D_FLUW / D_P1_L
C L : VOSIN DE K PAR IFA
C FW2SV((IFA DE 1A MAXFA) D_FLUW / D_P2_L
C L : VOSIN DE K PAR IFA

C FVP1S(1:MAXFA+1) DERIVEE FLUVP / P1_K PUIS P_1,SIGMA
C FVP2S(1:MAXFA+1) DERIVEE FLUVP / P2_K PUIS P_2,SIGMA
C FVP1SV(IFA DE 1A MAXFA) D_FLUVP / D_P1_L
C L : VOSIN DE K PAR IFA
C FVP2SV(IFA DE 1A MAXFA) D_FLUVP / D_P2_L
C L : VOSIN DE K PAR IFA
C
C FAS1S(1:MAXFA+1) DERIVEE FLUAS / P1_K PUIS P_1,SIGMA
C FAS2S(1:MAXFA+1) DERIVEE FLUAS / P2_K PUIS P_2,SIGMA
C FAS1SV(IFA DE 1A MAXFA) D_FLUW / D_P1_L
C L : VOSIN DE K PAR IFA
C FAS2SV(IFA DE 1A MAXFA) D_FLUW / D_P1_L
C L : VOSIN DE K PAR IFA
C
C FAD1S(1:MAXFA+1) DERIVEE FLUAD / P1_K PUIS P_1,SIGMA
C FAD2S(1:MAXFA+1) DERIVEE FLUAD / P2_K PUIS P_2,SIGMA
...
C DE CONTINUITE POUR UNE ARETE EXTERNE
C
C FM1VPS(1:MAXFA+1,NFACE) DERIVEE DE FMVPS / P_K PUIS P_1,SIGMA
C FM2VPS(1:MAXFA+1,NFACE) DERIVEE DE FMVPS / P_K PUIS P_2,SIGMA
C FM1WS(1:MAXFA+1,NFACE) DERIVEE DE FMWS / P_K PUIS P_1,SIGMA
C FM2WS(1:MAXFA+1,NFACE) DERIVEE DE FMWS / P_K PUIS P_2,SIGMA
C FM1ASS(1:MAXFA+1,NFACE) DERIVEE DE FMASS / P_K PUIS P_1,SIGMA
C FM2ASS(1:MAXFA+1,NFACE) DERIVEE DE FMASS / P_K PUIS P_2,SIGMA
C FM1ADS(1:MAXFA+1,NFACE) DERIVEE DE FMADS / P_K PUIS P_1,SIGMA
C FM2ADS(1:MAXFA+1,NFACE) DERIVEE DE FMADS / P_K PUIS P_2,SIGMA
C
C
C**********************************************************************
...
C =====================================================================
     ...
     &        NORMFA(1:MAXDIM,1:MAXFA),VOL,VOLV
...
C
      REAL*8 FW1S(1:MAXFA+1) ,FW2S(1:MAXFA+1)
      REAL*8 FVP1S(1:MAXFA+1),FVP2S(1:MAXFA+1)
      REAL*8 FAS1S(1:MAXFA+1),FAS2S(1:MAXFA+1)
      REAL*8 FAD1S(1:MAXFA+1),FAD2S(1:MAXFA+1)
C
      REAL*8 FW1SV(1:MAXFA) ,FW2SV(1:MAXFA)
      REAL*8 FVP1SV(1:MAXFA),FVP2SV(1:MAXFA)
      REAL*8 FAS1SV(1:MAXFA),FAS2SV(1:MAXFA)
      REAL*8 FAD1SV(1:MAXFA),FAD2SV(1:MAXFA)
C
      REAL*8 FMVPS(1:MAXFA),FMWS(1:MAXFA)
      REAL*8 FMASS(1:MAXFA),FMADS(1:MAXFA)
C
      REAL*8 FM1VPS(1:MAXFA+1,1:MAXFA),FM2VPS(1:MAXFA+1,1:MAXFA)
      REAL*8 FM1WS(1:MAXFA+1,1:MAXFA) ,FM2WS(1:MAXFA+1,1:MAXFA)
...
C
     ...
     &       DCVP2F(1:MAXFA)
C
      REAL*8 MOBWF(0:NVOIMA,1:MAXFA) ,MOADF(0:NVOIMA,1:MAXFA),
     &       MOASF(0:NVOIMA,1:MAXFA) ,MOVPF(0:NVOIMA,1:MAXFA),
     &       DW1F(0:NVOIMA,1:MAXFA)  ,DW2F(0:NVOIMA,1:MAXFA),
     &       DW1FV(0:NVOIMA,1:MAXFA) ,DW2FV(0:NVOIMA,1:MAXFA),
     &       DVP1F(0:NVOIMA,1:MAXFA) ,DVP2F(0:NVOIMA,1:MAXFA),
     &       DVP1FV(0:NVOIMA,1:MAXFA),DVP2FV(0:NVOIMA,1:MAXFA),
     &       DAS1F(0:NVOIMA,1:MAXFA) ,DAS2F(0:NVOIMA,1:MAXFA),
     &       DAS1FV(0:NVOIMA,1:MAXFA),DAS2FV(0:NVOIMA,1:MAXFA),
     &       DAD1F(0:NVOIMA,1:MAXFA) ,DAD2F(0:NVOIMA,1:MAXFA),
     &       DAD1FV(0:NVOIMA,1:MAXFA),DAD2FV(0:NVOIMA,1:MAXFA)
...
C NOUVELLE VARIABLES POUR SUSHI POUR TRAITER LA DIFFUSION PAR MOYENNE
      REAL*8 FCLLS(1:MAXFA),FCLLS1(1:MAXFA+1,1:MAXFA),
     &       FCLLS2(1:MAXFA+1,1:MAXFA),FTGLS(1:MAXFA),
     &       FTGLS2(1:MAXFA+1,1:MAXFA),
     &       FTGLS1(1:MAXFA+1,1:MAXFA)

      REAL*8 FLVPSK(1:MAXFA),FVP1SK(1:MAXFA+1,1:MAXFA),
     &       FVP2SK(1:MAXFA+1,1:MAXFA),
     &       FLASSK(1:MAXFA),FAS1SK(1:MAXFA+1,1:MAXFA),
     &       FAS2SK(1:MAXFA+1,1:MAXFA),
     &       FLADSK(1:MAXFA),FAD1SK(1:MAXFA+1,1:MAXFA),
     &       FAD2SK(1:MAXFA+1,1:MAXFA)

      REAL*8 FLVPSL(1:MAXFA),FVP1SL(1:MAXFA+1,1:MAXFA),
     &       FVP2SL(1:MAXFA+1,1:MAXFA),
     &       FLASSL(1:MAXFA),FAS1SL(1:MAXFA+1,1:MAXFA),
     &       FAS2SL(1:MAXFA+1,1:MAXFA),
     &       FLADSL(1:MAXFA),FAD1SL(1:MAXFA+1,1:MAXFA),
     &       FAD2SL(1:MAXFA+1,1:MAXFA)

      REAL*8 MOYVP(1:MAXFA),MOYVP1(1:MAXFA,1:MAXFA+1,0:1),
     &       MOYVP2(1:MAXFA,1:MAXFA+1,0:1),
     &       MOYAS(1:MAXFA),MOYAS1(1:MAXFA,1:MAXFA+1,0:1),
     &       MOYAS2(1:MAXFA,1:MAXFA+1,0:1),
     &       MOYAD(1:MAXFA),MOYAD1(1:MAXFA,1:MAXFA+1,0:1),
     &       MOYAD2(1:MAXFA,1:MAXFA+1,0:1)

      REAL*8 DL (1:MAXFA,1:MAXFA)
      REAL*8 YSSL (1:MAXDIM,1:MAXFA,1:MAXFA)
      REAL*8 CL (1:MAXFA,1:MAXFA)

      INTEGER FAV,IFAV
      REAL*8  MFACEL(1:MAXFA),DFACEL(1:MAXFA),
     &        XFACEL(1:MAXDIM,1:MAXFA),
     &        NORFAL(1:MAXDIM,1:MAXFA)

      REAL*8   CVPFV(1:MAXFA),CVPFV1(1:MAXFA),CVPFV2(1:MAXFA)
      REAL*8   CADFV(1:MAXFA),CADFV1(1:MAXFA),CADFV2(1:MAXFA)
      INTEGER  NFACEV
      INTEGER  NNOV,NNOSV
      REAL*8   ALPHA
      INTEGER  NUFLOC
...
C ================================================================
     ...
     &                  VOLV,MFACEL,DFACEL,XFACEL,NORFAL,UTICER )
            CALL CACDSU(MAXFA,MAXDIM,ALPHA,
     &                  NDIM,NNOV,NNOSV,NFACEV,AXI,ZR(IGEOMV),
