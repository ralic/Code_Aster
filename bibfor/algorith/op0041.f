...
C
      ...
      INTEGER      NMAEN1,NMAEN2,NMAEN3,IOCC2
...
C
      ...
      IF (IOCC2 .EQ. 0) THEN
        IF (NDIM .EQ. 3) CALL UTMESS('F','OP0041','ORIE_FOND MANQUANT')
      ELSEIF (IOCC2 .EQ. 1) THEN
        IF (NDIM .EQ. 2) CALL UTMESS('F','OP0041',
...
C     SEULEMENT S'IL Y A DES MAILLES DE CONTACT
      IF (NMAEN1+NMAEN2+NMAEN3.GT.0) THEN
        ...
        CALL SDCONX(RHON,CRIT,FROTT,MU,RHOT,SEUIL0,STACO0,INTE,COECH,
     &                                                ALGOLA,IOCCC,FISS)
...
C       MULTIPLICATUERS DE LAGRANGE DE CONTACT
     ...
     &                'PRESSION DE CONTACT'
          CALL XLAGSP(ALGOLA,NDIM,NOMA,CNSLT,CNSLN,FISS)
        ENDIF
