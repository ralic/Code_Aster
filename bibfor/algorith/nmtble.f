      SUBROUTINE NMTBLE(NIVEAU, PREMIE,
     &                  NOMA,   DEFICO, RESOCO, OLDGEO, NEWGEO,
     &                  DEPMOI, DEPGEO, MAXB,   DEPLAM,
     &                  MMITGO, MMITFR, MMITCA,  IXFEM,  
     &                  DEPPLU, INST,   DECOL,  MODELE,
     &                  MAXREL, IMPRCO, USUFIX, VECNOD,
     &                  NEQ, VECNOX, VECNOY,VECNOZ)
...
C
      ...
      INTEGER      NIVEAU
      INTEGER      MAXB(4)
      INTEGER      MMITCA,MMITGO,MMITFR,NEQ
      INTEGER      JVECNO,JVECNX,JVECNY,JVECNZ
      LOGICAL      IXFEM,PREMIE
      CHARACTER*8  NOMA
      CHARACTER*24 VECNOD,VECNOX,VECNOY,VECNOZ
      CHARACTER*24 DEFICO,RESOCO,IMPRCO
...
C IN  MMITGO : ITERATION DE REACTUALISATION GEOMETRIQUE
C                POUR LE CONTACT METHODE CONTINUE
C IN  MMITFR : ITERATION DE POINT FIXE SUR LE SEUIL DE FROTTEMENT
C                POUR LE CONTACT METHODE CONTINUE
C IN  MMITCA : ITERATION DE CONTRAINTE ACTIVE
C                POUR LE CONTACT METHODE CONTINUE
...
C
 101  CONTINUE
      IF (IXFEM) THEN
        CALL XMMBCA(NOMA  ,NOMO  ,DEFICO,RESOCO,OLDGEO,
     &              DEPPLU,DEPMOI,MMCVCA)      
      ELSE
        ...
        CALL MMMBCA(NOMA,DEFICO,OLDGEO,DEPPLU,
     &              INST,MMCVCA,DECOL,USUFIX,ZI(JVECNO),
     &              NEQ,ZI(JVECNX),ZI(JVECNY),ZI(JVECNZ),PREMIE)       
      ENDIF
      CALL MMCONV(NIVEAU,
     &            MAXB,MAXREL,IXFEM,
...
C
 102  CONTINUE
      ...
      ELSE
        CALL REACLM(NOMA  ,DEFICO,DEPPLU,NEWGEO)      
      ENDIF 
      CALL MMCONV(NIVEAU,
     &            MAXB,MAXREL,IXFEM,
     &            DEPLAM,DEPPLU,DEPGEO,DEFICO,IMPRCO,
