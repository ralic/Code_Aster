...
C                POUR LE CONTACT METHODE CONTINUE
C IN  ICONTX : INDICATEUR VALANT TRUE POUR XFEM AVEC CONTACT
...
C TEST FLIP-FLOP: SI CRITERE ATTEINT, ON PASSE A L'ITERATION SUIVANTE
      IF ((NFLIP(3) .GT. 0) .AND. (NFLIP(1) .GE. NFLIP(3))) THEN
        CALL UTDEBM('A','NMTBLE','LE STATUT DU CONTACT ')
        CALL UTIMPI('S','A CHANGE ',1,NFLIP(1))
        CALL UTIMPI('S',' FOIS AU NOEUD ',1,NFLIP(2))
        CALL UTIMPI('L',' PRESENCE POSSIBLE DE FLIP-FLOP !!',0,NFLIP(1))
        CALL UTFINM()
        INCOCA = 1
      ENDIF
