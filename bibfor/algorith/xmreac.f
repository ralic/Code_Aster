      SUBROUTINE XMREAC(NDIM,NNE,NNES,NNM,IGEOM,IDEPM)
...
C
     ...
     &    ZR(IDEPM+(I-1)*(3*NDIM)+J-1)-ZR(IDEPM+(I-1)*(3*NDIM)+J+NDIM-1)
   10   CONTINUE
   20 CONTINUE
     ...
     & ZR(IGEOM+NNE*NDIM+(I-1)*NDIM+J-1)+
     & ZR(IDEPM+NNES*3*NDIM+(NNE-NNES)*NDIM+2*NDIM*(I-1)+J-1)+
     & ZR(IDEPM+NNES*3*NDIM+(NNE-NNES)*NDIM+2*NDIM*(I-1)+J+NDIM-1)
   11   CONTINUE
   21 CONTINUE
C
      CALL JEDEMA()
