...
C IN  ECHCON : .TRUE. SI ECHEC TRAITEMENT DU CONTACT UNILATERAL
C
...
C 
      ...
      LOGICAL      ISFONC,LCTCC,LCTCD,LXFEM,LCONT 
      CHARACTER*24 DDEPLA,DEPDEL,K24BID 
      CHARACTER*8  MODELE 
C
C ----------------------------------------------------------------------
C
...
C
      IF (LCTCC) THEN
        ...
        ELSE
          CALL MMMRES(DEFICO,RESOCO,DEPDEL,NUMEDD,MAILLA,
     &                CNSINR,CNSPER,INST)
        ENDIF 
      ELSEIF (LCTCD) THEN 
        IF (.NOT.ECHCON(1) .AND. .NOT.ECHCON(2)) THEN
          CALL CFRESU(NUMINS,INST  ,MATASS,DEFICO,RESOCO,
