...
C         -- POUR NE PAS PAS CREER UN PROF_CHNO A CHAQUE INSTANT :
          IF (J.EQ.1) THEN
            CHMOD= RESU//'.CHMODELE'
            CALL COPISD ( 'CHAMP_GD', 'V', CHAMP, CHMOD )
            IF ( K24 .EQ. TYPCHF ) THEN
              CALL JELIRA ( CHAMP//'.VALE', 'LONMAX', LG, K8B )
              CALL JEVEUO ( CHAMP//'.VALE', 'L', JNOMF )
              CALL JEVEUO ( CHAMP//'.DEEQ', 'L', JDEEQ )

              CALL JEVEUO ( CHMOD//'.DESC', 'E', IADESC )
              CALL JENONU ( JEXNOM('&CATA.GD.NOMGD',TYPEGD), IGD )
              ZI(IADESC-1+1) = IGD
              CALL JEDETR ( CHMOD//'.VALE' )
              CALL WKVECT ( CHMOD//'.VALE', 'G V R', LG, JC )
            END IF
          END IF

          CALL COPISD ( 'CHAMP_GD', 'G',CHMOD, NOMCH )
