...
C
      ...
      CHARACTER*19  LISCHA,SDNUME,SDPILO,MATASS
...
C
 ...
 20   CONTINUE
      ...
      END IF
C
C --- INTERSECTION AVEC L'INTERVALLE ETA_PILO_*
C      - SI PROJ_BORNE = 'OUI', ON PROJETTE ETA SUR L'INTERVALLE
C      - DANS TOUS LES CAS, LICITE = -1 INDIQUE QU'ON A FRANCHI LES
C        BORNES
C
      DO 50 I = 1, NBEFFE
        IF (BORMAX) THEN
 ...
 50   CONTINUE
C
C --- SELECTION DU PARAMETRE DE PILOTAGE ETAF 
C     S'IL EXISTE DEUX ETA SOLUTIONS :
C        - ON DEMANDE A NMCESE DE CHOISIR      
      IF (NBEFFE.EQ.2) THEN
...
C
C --- CALCUL DU RESIDU POUR LA RECHERCHE LINEAIRE 
C      
      IF (IRECLI) THEN
C ----- CETTE ETAPE EST SAUTEE SI LE RESIDU EST DEJA CALCULE DANS NMCESE
