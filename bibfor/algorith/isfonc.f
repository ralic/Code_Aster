...
C       PILOTAGE           :  PILOTAGE
C       NEWTON_KRYLOV      :  METHODE DE NEWTON_KRYLOV
C       DEBORST            :  ALGORITHME DE DE BORST
C       SOUS_STRUC         :  CALCUL PAR SOUS-STRUCTURATION
C       PROJ_MODAL         :  PROJECTION MODALE
C       IMPLEX             :  METHODE IMPLEX
C       EXPLICITE          :  METHODE EXPLICITE
...
C                             LE CONTACT
C       FROT_NEWTON        :  NEWTON GENERALISE POUR LE CONTACT CONTINU
C                             FROTTEMENT
C       CONT_NEWTON        :  NEWTON GENERALISE POUR LE CONTACT CONTINU
C                             CONTACT
C       GEOM_NEWTON        :  NEWTON GENERALISE POUR LE CONTACT CONTINU
C                             GEOMETRIE
C       CONT_ALL_VERIF     :  CONTACT SANS CALCUL SUR TOUTES LES ZONES
C       LIAISON_UNILATER   :  LIAISON UNILATERALE
C       ELT_CONTACT        :  ELEMENTS DE CONTACT (CONTINU/XFEM)
C       ELT_FROTTEMENT     :  ELEMENTS DE FROTTEMENT (CONTINU/XFEM)
C       DIS_CHOC           :  ELEMENTS DIS_CHOC
C       GD_ROTA            :  ELEMENTS DE STRUCTURE EN GRANDES ROTATIONS
C       XFEM               :  ELEMENTS XFEM
C       EXI_STRX           :  ELEMENTS UTILISANT STRX (PMF)
...
C       MATR_DISTRIBUEE    :  MATRICES DISTRIBUEES
C
C DERNIER NUMERO UTILISE: 56
C
C ---------------------------------------------------------------------
C
      CHARACTER*24 NOMFON
...
C
      ...
      ELSEIF (NOMFON.EQ.'ERRE_TEMPS_THM') THEN
...
C
      ...
      ELSEIF (NOMFON.EQ.'ENERGIE') THEN
        ISFONC = FONACT(50).EQ.1
      ELSEIF (NOMFON.EQ.'PROJ_MODAL') THEN
        ISFONC = FONACT(51).EQ.1
      ELSEIF (NOMFON.EQ.'MATR_DISTRIBUEE') THEN
        ISFONC = FONACT(52).EQ.1
      ELSEIF (NOMFON.EQ.'EXPLICITE') THEN
        ISFONC = FONACT(54).EQ.1
      ELSEIF (NOMFON.EQ.'EXI_STRX') THEN
