...
C
        ...
        IF ( IER .EQ. 0 ) THEN
          CALL UTMESS('A','MODEIM',CHAMNO//'CHAM_NO DEJA EXISTANT')
        ELSE IF ( IER .EQ. 100 ) THEN
          CALL VTCREA(CHAMNO,CREFE,'G','R',NEQR)
        ELSE
          CALL UTMESS('F',PGC,'APPEL ERRONE A RSEXCH')
        ENDIF
C
...
C
        ...
        CALL MRCONL(LMAT,NEQ,'R',ZR(LTCHAM),1)
C
C ----- RESOLUTION EN PLACE
C
        CALL RLDLGG(LMAT,ZR(LTCHAM),CBID,1)
C
