      SUBROUTINE XSELLA(LSN,NBNO,NARZ,TABNOZ,PICKNO,NBPINO)
...
C RESPONSABLE VABHHTS J.PELLET
      ...
      INTEGER       NBNO,NARZ,TABNOZ(3,NARZ),PICKNO(NARZ),NBPINO
      REAL*8        LSN(*)
C
C   CHOIX DE L'ESPACE DES LAGRANGES (VOIR BOOK VI 15/07/05) :
C           - SELECTION DES NOEUDS POUR LES LAGRANGES
C
C   IN
C       LSN    : CHAM_NO DE LA LEVEL SET NORMALE
C       NBNO   : NOMBRE DE NOEUDS DU MAILLAGE
C       NARZ   : NOMBRE D'ARETES COUPEES
C       TABNOZ : TABLEAU DES NOEUDS EXTREMITES ET NOEUD MILIEU

C   OUT
C      PICKNO  : NUMEROS DES NOEUDS SELECTIONNES�
C      NBPINO  : NOMBRE DE NOEUDS SELECTIONNES�
...
C     COMPTEUR DE NOEUDS SELECTIONNES�
      CPTNO=0
...
C       MEILLEURE ARETE : PICK MEILLEUR NOEUD
 ...
 212    CONTINUE
        ...
        NO2=TABDIR(BESTAR,2)
        BESTNO=NO1
        IF ((SCORNO(NO2).GT.SCORNO(NO1)).OR.(SCORNO(NO2).GT.SCORNO(NO1)
     &       .AND.SCORN2(NO2).LT.SCORN2(NO1)))      BESTNO=NO2
        CPTNO=CPTNO+1
