      SUBROUTINE NMRESI(MAILLA,MATE  ,NUMEDD,SDNUME,FONACT,
...
C
        ...
        ENDIF
C
C ----- SI LAGANGIEN DE CONTACT: ON IGNORE LA VALEUR DU RESIDU
C
C        IF (ZI(JNUCO+IEQ-1).EQ.1) THEN
C          IF (ABS(ZR(JFINT+IEQ-1)).GE.VLAGC) VLAGC = ZR(JFINT+IEQ-1)
C          GOTO 20  
C        ENDIF
C
