     ...
     &                  SIGP,VECT,MATR,VIM,VIP,TM,TP,CRIT,
     &                  COMPOR,TYPMOD)
...
C RESPONSABLE LAVERNE J.LAVERNE
      ...
      REAL*8  VECT(NDDL),MATR(NDDL*NDDL),SIGP(2*NDIM-1,NPG)
...
C-----------------------------------------------------------------------
      ...
      REAL*8 CRIT,RBID,R8VIDE,PRESGM,PRESGD,PRESG,TEMP
      REAL*8 COD(NPG)
...
C     CALCUL DES COORDONNEES DES POINTS DE GAUSS
      CALL GEDISC(2,NNO2,NPG,VFF2,GEOM,COOPG)
...
C         PRISE EN COMPTE DE LA PRESSION DE FLUIDE
          SIGP(1,KPG) = SIGP(1,KPG) - PRESG
