...
C====================================================================
      ...
      IF (DERIVL) THEN
        IF (.NOT.ELAS)
     &    CALL UTMESS('F','NMELNL '//OPTION,'SEULE UNE LOI DE '//
...
C CALCUL DE LA DERIVEE LAGRANGIENNE DU EPS EQUIVALENT (PART II)
      IF (DERIVL) THEN
        ...
        ELSE
          CALL UTMESS('A','CALC_DG-NMELNL 1','LE TENSEUR EPSEQ=0 '//
     &      'ON A DONC UNE DERIVEE DEPSEQ TRES GRANDE !')
...
CJMP
          PREC= CRIT(3) * SIGY
          NITER = INT(CRIT(1))
          CALL ZEROFO(NMCRI1,F0,APPROX,PREC,NITER,P)
          IF (LINE) THEN
            RP = SIGY +RPRIM*P
