      SUBROUTINE VECHTH(MODELZ,CHARGZ,INFOCZ,CARELZ,MATEZ,INSTZ,
     &                  CHTNZ,VECELZ)
...
C====
C 4.7 CAS PARTICULIER X-FEM
C====
              CALL EXIXFE(MODELE,IRET)
              IF (IRET.NE.0) THEN
                CALL XVTHIN(MODELE,OPTION,NCHINX,LCHIN,LPAIN,NCHIN)
                LIGCAL = LIGREL(1)
              ELSE
...
C====
C 4.8 LANCEMENT DES CALCULS ELEMENTAIRES OPTION
C====
              IF (NIV.EQ.2) THEN
                WRITE (IFM,*) '     K              :',K
   ...
   30           CONTINUE
              END IF
              CALL CALCUL('S',OPTION,LIGCAL,NCHIN,LCHIN,LPAIN,
...
C=====
C 4.10 FIN BOUCLE SUR LES NBCHMX CL NEUMANN POSSIBLES =======
C=====
