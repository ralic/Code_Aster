...
C  VERIFICATION SUR LE NOMBRE DE POINTS D'INTERSECTION TROUVES
               IF ((TYPMA(1:5).EQ.'TETRA'.AND.NPTINT.GT.4).OR.
     &             (TYPMA(1:5).EQ.'PENTA'.AND.NPTINT.GT.5).OR.
     &             (TYPMA(1:4).EQ.'HEXA'.AND.NPTINT.GT.6)) THEN
                  CALL JENUNO(JEXNUM(NOMMAI,NMAABS),NOMAIL)
                  CALL CODENT(NPTINT,'D',NPTIK2)
                  CALL UTMESS('F','XPRLS0','LES ARETES DE LA MAILLE '
