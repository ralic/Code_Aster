     ...
     &                  NR,MOD,TIMED,TIMEF,INDI,VINS,CODRET)
...
C     INDI   : INDICATEUR MECANIQMES POT. ACTIFS (HUJEUX) 
C     VINS   : VARIABLES INTERNES A T (ORIGINAL - HUJEUX)
C  OUT
C     VINF   :  VARIABLES INTERNES A T+DT
C ----------------------------------------------------------------
      INTEGER  NDT,NVI,NMAT,NDI,NBCOMM(NMAT,3),ITER,ITMAX,NR,CODRET
      INTEGER  NFS,NSG,INDI(7),I
      REAL*8   MATERD(NMAT,2),MATERF(NMAT,2),VINS(NVI),TIMED,TIMEF
      REAL*8   PKC,M13,DTOT,HOOKF(6,6)
...
C        D
  ...
  10     CONTINUE
      ...
      ELSEIF ( LOI(1:6) .EQ. 'HUJEUX')THEN
         CALL HUJLNF(TOLER,NMAT,MATERF,NVI,VIND,VINF,VINS,
     &               NR,YD,YF,SIGD,SIGF,INDI,CODRET)
      ELSE
C        CAS GENERAL :
