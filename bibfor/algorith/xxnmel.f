     ...
     &                   SIG,VI,MATUU,IVECTU,CODRET,NFISS,FISNO)
      ...
      INTEGER       IDECPG,IDEPL,IGEOM,IMATE,IVECTU
...
C       SI OPTION 'RIGI_MECA', ON INITIALISE À 0 LES DEPL
     ...
     &                .FALSE.,NDIM,HE,R,UR,FISNO,NFISS,NFH,NFE,DDLS,-1,
     &                FE,DGDGL,'INI',XE,FF,DFDI,F,EPS,RBID33)
        ENDIF
...
C
            DO 191 I=1,DDLD
              DO 192 M=1,2*NDIM
                ZR(IVECTU-1+NN+I)=
     &                       ZR(IVECTU-1+NN+I) + DEF(M,N,I)*SIGMA(M)*JAC
 192          CONTINUE
 191        CONTINUE
 190      CONTINUE
C
