     ...
     &                     SIGI,NEPS,EPSD, DETOT,COEL,DVIN,
...
C     ------------------------------------------------------------------
      CHARACTER*8 MOD
      CHARACTER*16 NOMFAM,CPMONO(5*NMAT+1)
      REAL*8 DT,DY(6+NSG),EXPBP(NSG),CRIT,SGNS,Q(3,3),EVI(6)
      REAL*8 DEVI(6),MUS(6),NG(3),TAUS,DGAMMA,DALPHA,DP,RP,YD(6+NSG)
      REAL*8 FKOOH(6,6),MATERF(NMAT*2),MSNS(3,3),GAMSNS(3,3),LG(3)
      REAL*8 TOUTMS(NFS,NSG,6),FP(3,3),FP1(3,3)
      INTEGER ITENS,NBFSYS,I,NUVI,IFA,NBSYS,IS,NSFA,NSFV,NS
C     ------------------------------------------------------------------
...
C
    ...
    5 CONTINUE
      ...
      IF ((COEFT(NBCOMM(1,1)).EQ.4).OR.
     &    (COEFT(NBCOMM(1,1)).EQ.5)) THEN
...
C
   ...
   30 CONTINUE
      IF (COMP(3)(1:4).EQ.'SIMO') THEN
         NS=(NVI-15)/3
         CALL DCOPY(9,VINI(6+3*NS+1),1,FP,1)
         CALL PMAT(3,GAMSNS,FP,FP1)      
         CALL DCOPY(9,FP1,1,DVIN(6+3*NS+1),1)
      ENDIF
