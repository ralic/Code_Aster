        SUBROUTINE XXLAG3(FFC   ,IDEPL ,IDEPM ,IDEP0,LACT  ,NDIM  ,
     &                    NNOL  ,PLA   ,LAMB  ,NVEC)
        IMPLICIT NONE
      INCLUDE 'jeveux.h'
...
C OUT LAMB  : LAMBTION DE CONTACT AU POINT DE GAUSS
C OUT LAMB12: LAMBTION DE FROTTEMENT AU POINT DE GAUSS
...
C --- (DEPDEL+DEPMOI)
        ...
        DO 1 I = 1,NNOL
           ...
           DO 2 J=1,NDIM
           LAMB(J) = LAMB(J) + 
     &       FFI * ZR(IDEPL-1+PLI-1+J)
           IF(NVEC.GE.2) THEN
           LAMB(J) = LAMB(J) + 
     &       FFI * ZR(IDEPM-1+PLI-1+J)
            ENDIF
           IF(NVEC.GE.3) THEN
           LAMB(J) = LAMB(J) + 
     &       FFI * ZR(IDEP0-1+PLI-1+J)
            ENDIF
2       CONTINUE
 1    CONTINUE
      END
