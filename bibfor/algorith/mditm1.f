...
C -----------------
     ...
     &              IDEPG, IVITG, IACCG, IACCG0
      INTEGER       IFX, IFXS, ITX, ITXS, IRFX, IIFX, IFXTR, IFX0
...
C
     ...
     &   CALL UTMESS('F','MDITM1','ABSCENCE DE TERME DE FORCAGE '//
     &   'EXTERNE. L''ALGORITHME ITMI N''EST PAS PREVU POUR CALCULER '//
     &   'LA REPONSE LIBRE D''UNE STRUCTURE.')
C
      N2 = NBM + 2
      DTTR = 0.0D0
C
...
C
      ...
      CALL WKVECT('&&MDITM1.REFEXT', 'V V R8', NBF    , IRFX   )
      CALL WKVECT('&&MDITM1.IMFEXT', 'V V R8', NBF    , IIFX   )
...
C
C
      CALL UTMESS('F','MDITM1','ABSCENCE DE NON-LINEARITES DE CHOC. '//
     &            'POUR TRAITER LE REGIME LINEAIRE, PRECISER UNE '//
     &            'NON-LINEARITES DE CHOC AVEC UN JEU IMPORTANT.')
C
C-----------------------------------------------------------------------
...
C
     ...
     &ZR(IFX),ZR(IFXS),ZR(ITX),ZR(ITXS),ZR(IRFX),ZR(IIFX),ZR(IFXTR),
