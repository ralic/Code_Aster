...
C
      ...
      CHARACTER*24 CARCRI
      CHARACTER*24 SDSENS,SDTIME,SDERRO,SDIMPR,SDIETO
...
C
     ...
     &            SDSENS,SDTIME,SDNUME,SDERRO,DEFICO,
     &            RESOCO,RESOCU,PARCON,VALINC,SOLALG,
     &            VEELEM,VEASSE,MEELEM,MEASSE,ERROR ,
     &            MATASS)
C
      IF (ERROR) GOTO 315
C
...
C
      IF ((ACTITE.EQ.0).OR.(ACTITE.EQ.1)) THEN
        GOTO 330
      ENDIF
C
...
C
      ...
      IF (MTCPUI) THEN
        CALL NMERGE('SET','TIN',SDERRO,MTCPUI)
        ACTITE = 3
      ENDIF
C
 330  CONTINUE
...
C
      CALL NMACTN(SDDISC,PARCRI,ITERAT,NUMINS,ITEMAX,
     &            ACTITE,ACTNIV)
C
