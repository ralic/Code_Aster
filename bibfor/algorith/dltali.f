...
C
      ...
      INTEGER LVALE, LACCE
C
      INTEGER      ICODE,JSTD,IEQ,JAUX,NDY,IFEXTM,IFEXTC
      CHARACTER*1  TYPSOL
...
C
      IF (NRORES.EQ.0) THEN
        ...
        IF (NDY.EQ.0) THEN
          ...
          CALL MRMULT('ZERO',IMAT(1),DEP0,'R',FEXTE0,1)
          CALL MRMULT('ZERO',IMAT(2),ACC0,'R',ZR(IFEXTM),1)
          CALL WKVECT('FEXT0C','V V R',NEQ,IFEXTC)
          IF (LAMORT) THEN
            CALL MRMULT('ZERO',IMAT(3),VIT0,'R',ZR(IFEXTC),1)
          ENDIF
          DO 562 IEQ=1,NEQ
