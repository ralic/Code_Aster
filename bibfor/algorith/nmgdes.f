       SUBROUTINE NMGDES (FAMI,KPG,KSP,NDIM,TYPMOD,IMATE,
     &                   COMPOR,CRIT,INSTAM,INSTAP,
     &                   DEPST,SIGM,VIM,OPTION,SIGP,VIP,
     &                   DSIDEP)
...
C
      ...
      REAL*8      HYGRM,HYGRP,HYDRM, HYDRP,SECHM, SECHP, SREF
      REAL*8      THER
      REAL*8      COEFA, COEFB
      LOGICAL     CPLAN
...
C
C     RECUPERATION DE LA TEMPERATURE, DE L HYDRATATION ET DU SECHAGE
...
C
C
      CALL RCVALB (FAMI,KPG,KSP,'-',IMATE,' ','ELAS', 0,NOMPAR,
     &             VALPAM,2,NOMRES,VALRES,CODRET(1), FB2 )
      EM  = VALRES(1)
      NUM = VALRES(2)
C
      CALL RCVALB (FAMI,KPG,KSP,'-',IMATE,' ','ELAS', 0,NOMPAR,
     &             VALPAM,1,NOMRES(3),VALRES(3),CODRET(3), BL2 )
...
C
     ...
     &             VALPAP,2,NOMRES,VALRES,CODRET, FB2 )
      CALL RCVALB (FAMI,KPG,KSP,'+',IMATE,' ','ELAS', 0,NOMPAR,
     &             VALPAP,1,NOMRES(3),VALRES(3),CODRET(3), BL2 )
      IF ( CODRET(3) .NE. 'OK' ) VALRES(3) = 0.D0
      ALPHAP = VALRES(3)
      E      = VALRES(1)
      NU     = VALRES(2)
...
C
      ...
      NOMRES(2)='K_DESSIC'
      CALL RCVALB (FAMI,KPG,KSP,'-',IMATE,' ','ELAS', 0,NOMPAR,
     &             VALPAM,1,NOMRES,VALRES,CODRET, BL2 )
      IF ( CODRET(1) .NE. 'OK' ) VALRES(1) = 0.D0
      BENDOM = VALRES(1)
C
      CALL RCVALB (FAMI,KPG,KSP,'+',IMATE,' ','ELAS', 0,NOMPAR,
     &             VALPAP,1,NOMRES,VALRES,CODRET, BL2 )
      IF ( CODRET(1) .NE. 'OK' ) VALRES(1) = 0.D0
      BENDOP = VALRES(1)
C
      CALL RCVALB (FAMI,KPG,KSP,'-',IMATE,' ','ELAS', 0,NOMPAR,
     &             VALPAM,1,NOMRES(2),VALRES(2),CODRET(2), BL2 )
      IF ( CODRET(2) .NE. 'OK' ) VALRES(2) = 0.D0
      KDESSM = VALRES(2)
C
      CALL RCVALB (FAMI,KPG,KSP,'+',IMATE,' ','ELAS', 0,NOMPAR,
     &             VALPAP,1,NOMRES(2),VALRES(2),CODRET(2), BL2 )
      IF ( CODRET(2) .NE. 'OK' ) VALRES(2) = 0.D0
      KDESSP = VALRES(2)
C
...
C
      NOMRES(1) = 'LAM_VISC'
      CALL RCVALB (FAMI,KPG,KSP,'+',IMATE,' ','BAZANT_FD',0,NOMPAR,
     &             VALPAP,1,NOMRES,VALRES,CODRET, BL2 )
         IF  (CODRET(1) .NE. 'OK')  THEN
             CALL U2MESS('F','ALGORITH7_97')
         ENDIF
         LAMBDA = VALRES(1)
 110  CONTINUE
C
...
C
      ...
      CALL RCVALB(FAMI,KPG,KSP,'-',IMATE,' ','ELAS',0,NOMPAR,
     &            VALPAM,1,NOMRES(1),VALRES(1),CODRET(1), FB2 )
      HYGRM=VALRES(1)
