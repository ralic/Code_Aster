...
C IN  CHSLT  : CHAMP NOEUD SIMPLIFIé CONTENANT LA LA LEVEL SET TANGENTE 
...
C
      REAL*8      FF(8)
      INTEGER     JCONX1,JCONX2,JLST,JMA,ITYPMA,NNGL,NNO,INO
...
C
      ...
      LST = 0.D0
      DO 10 INO=1,NNO
        NNGL=ZI(JCONX1-1+ZI(JCONX2+POSMA-1)+INO-1)
        LST = LST + ZR(JLST-1+NNGL) * FF(INO)
10    CONTINUE
      LST = SQRT(ABS(LST))
C
      CALL JEDEMA()
