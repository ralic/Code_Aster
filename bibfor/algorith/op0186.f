...
C 0.3. ==> VARIABLES LOCALES
     ...
     &        RECHLI,LOBSER,DIDERN,FINPAS,INCR,EVOL
...
C ----------------------------------------------------------------------
      ...
      DATA PARTPS    /'&&OP0186.PARTPS'/
      DATA LISOBS            /'&&OP0186.OBSERVATIO'/
      DATA SDSUIV            /'&&OP0186.SUIV.          '/
C ----------------------------------------------------------------------
...
C --- LECTURE DE L'ETAT INITIAL ET DES DONNEES D'INCREMENTATION
      CALL NTINIT(RESULT,MODELE,INFCHA,CHARGE,INFOCH,SOLVEU,NUMEDD,
     &            PARMER,LOSTAT,EVOL,TIME,PARTPS,NBOBSE,DERNIE,LOBSER,
     &            NUOBSE,NOMTAB,MAILL2,NBOBAR,LISINS,LISOBS,NUINS0,
     &            SDSUIV,NBPASE,INPSCO,VHYDR,K24BID,K24BID,CRITHE)
C --- REPRISE D'UN CALCUL OU ETAT INITIAL NON NUL
      IF (.NOT.LOSTAT) THEN
        CALL NTREP0(PARTPS,DERNIE,VHYDR,NBPASE,INPSCO,
 ...
 210  CONTINUE
       ...
       IF (LOSTAT) THEN
          IF (.NOT.EVOL) THEN
            ...
            KHI=0.D0
          ELSE
            INSTAP=DIINST(PARTPS, NUMORD)
            DELTAT=-1.D150
            THETA=1.D0
            KHI=0.D0
          ENDIF
        ELSE
          IF ((DININS(PARTPS,NUMORD-1)-DININS(PARTPS,NUMORD)).GT.1) THEN
            CALL DIDECO(PARTPS, NUMORD, CRITHE(1:19), -1 , IRET)
            IF (IRET.EQ.0) THEN
              GOTO 210
            ELSE
          ...
          ENDIF
          INSTAP = DIINST(PARTPS, NUMORD)
          DELTAT = INSTAP-DIINST(PARTPS, NUMORD-1)
          THETA=PARMER(1)
          KHI=1.D0
        ENDIF
        PARA(2) = DELTAT
...
C -- DOIT-ON FAIRE UNE OBSERVATION
        IF (.NOT.LOSTAT) CALL LOBS(NBOBSE, NUINS0, LOBSER, INSTAP,
   ...
   30         CONTINUE
     ...
     &        ) '*',ITERAT,TESTR,TESTM,ITERHO,RHO,KREAS,'*'
            ...
            IF (ITEMAX .AND. .NOT.CONVER) THEN
              ...
              IF (.NOT.LOSTAT) THEN
                CALL DIDECO(PARTPS, NUMORD, CRITHE(1:19), 2, IRET)
                IF (IRET.EQ.0)  THEN
...
C        TEMPS RESTANT SUFFISANT ?
            ...
            WRITE (IFM,FMT1)
C -- SUIVI DE DDL
C            CALL DYOBAR(MAILLA,INSTAM,ITERAT,
C     &            K24BID,K24BID,VTEMP,K24BID,
C     &            K24BID,SDIMPR,SDSUIV,
C     &            K24BID,K24BID,K24BID,K24BID,
C     &            K19BID,IBID,IBID,.TRUE.,K24BID)
...
C ======================================================================
            ...
            IF (.NOT.EVOL) THEN
              FINPAS=.TRUE.
            ELSE
              FINPAS = DIDERN(PARTPS, NUMORD)
            ENDIF
            INCR  = NRORES.EQ.NBPASE
C VTEMPM --> VTEMP ET ON ARCHIVE VTEMP
            CALL NTSTOC(PARTPS,LOSTAT,NUMORD,FINPAS,INCR,RESULT,COMPOR,
...
C -- OBSERVATION EVENTUELLE
      ...
      IF ( LOBSER .AND. EVOL) THEN
        CALL DYOBAR(MAILL2,INSTAP,IBID,
     &            K24BID,K24BID,VTEMP,K24BID,
     &            K24BID,K24BID,K24BID,
     &            K24BID,K24BID,K24BID,K19B,
     &            NOMTAB,NBOBAR,NUOBSE,.FALSE.,K19B)
      ENDIF
