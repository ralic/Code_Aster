...
C 0.3. ==> VARIABLES LOCALES
     ...
     &        RECHLI,DIDERN,FINPAS,LEVOL,FORCE,LNONL
      INTEGER PARMEI(2),PARCRI(3),IAUX,JAUX,NUMINS,K,IERD,
...
C
     ...
     &            INPSCO,VHYDR ,SDOBSE,MAILLA,SDCRIT,
     &            TIME  )
      CALL ASSERT(LNONL)
C
      IF (LOSTAT) THEN
...
C --- MATRICE TANGENTE REACTUALISEE POUR UN NOUVEAU DT
     ...
     &      'TANGENTE','*'
          WRITE (IFM,FMT1)
          CALL JELIRA(VTEMPM(1:19)//'.VALE','LONMAX',NEQ,SAUX08)
...
C
          IF (LEVOL) THEN
            CALL NTOBSV(MAILLA,SDOBSE,VTEMP ,NUMINS,INSTAP)
          ENDIF
C
C ------- VERIFICATION SI INTERRUPTION DEMANDEE PAR SIGNAL USR1
C
