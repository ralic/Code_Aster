...
C --- LECTURE DE L'ETAT INITIAL ET DES DONNEES D'INCREMENTATION
     ...
     &            LNUAR,LOSTAT,LIEVOL,LISARC,EXCARC,TIME,NBPASE,INPSCO,
     &            VHYDR,K24BID,K24BID,NUMFIN,NBGRPA,IDEB,JDEB,NUMORD,
     &            TPSNP1,CRITHE)
...
C --- BOUCLE SUR LES NBGRPA "GROS" PAS DE TEMPS (N FOIS LE MEME DELTAT)
      ...
      DO 70 NRGRPA = IDEB,NBGRPA

C --- RECUPERATION DU PAS DE TEMPS ET DES PARAMETRES DE RESOLUTION

        CALL NTITIN(LIEVOL,PARMER,NRGRPA,LOSTAT,DELTAT,NBPTPA,THETA,KHI)
...
C --- ILS ONT TOUS LA MEME LONGUEUR : DELTAT
        ...
        DO 60 NRPTPA = JDEB,NBPTPA

          CALL UTTCPU(1,'DEBUT',4,TPS1)
          NUMORD = NUMORD + 1
          IF (NUMORD.GT.NUMFIN) GO TO 80
...
C (OU ON REMPLACE (DT/DS)- PAR T+)
     ...
     &                  NUMEDD,SOLVEU,IAUX,TIME,TPSTHE,REASVC,REASVT,
...
C SOLUTION: VTEMP= T- ET VTEMPM = T+,1
     ...
     &                  NRGRPA,TIME,LONCH,MATASS,MAPREC,VTEMP,VTEMPM,
...
C DE SENSIBILITE QUI VA SUIVRE
            IF (NBPASE.NE.0 .AND. NRGRPA.GT.0 .AND.
