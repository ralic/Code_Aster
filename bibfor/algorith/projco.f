...
C
      ...
      INTEGER      NDIM,NESCL,NBNO,NBDDL,POSNO,NNOCO
      INTEGER      POSNOE,POSMA
      INTEGER      JDECAL
      INTEGER      JCOOR
      REAL*8       JEU,CMULT,RBID,R6BID(6),OLDJEU,DIST3,DIST4
      ...
      CHARACTER*24 JEUSUP,JEUFO1,JEUFO2,JEUFO3,JEUPOU,NORMCO
      INTEGER      JJSUP,JJFO1,JJFO2,JJFO3,JJPOU,JNORMO
C
C ----------------------------------------------------------------------
C
      CALL INFNIV (IFM,NIV)
...
C        
        DIST3 = 0.D0
        DIST4 = 0.D0
        CALL CFDIST(IZONE,POSNOE,INST,JNOCO,JCOOR,
     &              JJFO1,JJFO2,JJFO3,JJSUP,JJPOU,NNOCO,DIST3,DIST4)
C
...
C
     ...
     &                NBNO,POSNO,NBDDL,ZI(JAPDDL+JDECAL))
          ZR(JAPJEU+IESCL-1) = JEU - ZR(JJSUP+IZONE-1) + DIST3 - DIST4
C
C --- DANS LE CAS DES CONDITIONS THM SUR PRE1 OU PRE2
C --- ON INVERSE LE SIGNE DU SEUIL
C
        IF ((ZI(JCHAM+IZONE-1).EQ.-4).OR.(ZI(JCHAM+IZONE-1).EQ.-5)) THEN
          ZR(JAPJEU+IESCL-1) = JEU + ZR(JJSUP+IZONE-1) + DIST3 + DIST4
        END IF
C
        END IF
