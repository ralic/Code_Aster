...
C    DIMANV = DIMENSION MAX DE LA LISTE DU NOMBRE DE VAR INT EN THM
      ...
      CHARACTER*16 COMPOR(NCMPMA),COMP,COMCOD,RESO,LCOMEL(5),MOCLEF(2)
      CHARACTER*16 TYMATG,TEXTE(2),TXCP
      CHARACTER*8  SDCOMP
      CHARACTER*128 NOMLIB
      CHARACTER*16 NOMSUB
      REAL*8       CARCRI(9),RESI
...
C     CAS PARTICULIER DU MONOCRISTAL
      ...
      IF (COMP(1:4).EQ.'ZMAT') THEN
        CALL GETVIS(MOCLEF(K),'NB_VARI',1,1,1,NBVARI,N1)
        CALL GETVIS(MOCLEF(K),'UNITE',1,1,1,NUNIT,N1)
...
C       ET LA SUBROUTINE DANS KIT9
        CALL GETVTX(MOCLEF, 'LIBRAIRIE',1,1,1,NOMLIB,N1)
        CALL GETVTX(MOCLEF, 'NOM_ROUTINE',1,1,1,NOMSUB,N1)
        DO 30 II = 1,DIMAKI-1
...
C     CRITERES DE CONVERGENCE 
      CALL GETVR8(MOCLEF(K),'RESI_INTE_RELA',1,1,1,RESI,IRET)
      CALL GETVIS(MOCLEF(K),'ITER_INTE_MAXI',1,1,1,ITEINT,IRET)
      IF (K.EQ.1) THEN
         CALL GETVTX(MOCLEF(K),'RESO_INTE',1,1,1,RESO,IRET) 
         CALL GETVIS(MOCLEF(K),'ITER_INTE_PAS' ,1,1,1,ITEPAS,IRET)
      ENDIF
...
C        dans ZR(JVALV+1) on stocke le type de matrice tgte
         CALL GETVTX(MOCLEF(K),'TYPE_MATR_TANG',1,1,1,TYMATG,IRET)
            IF (IRET.EQ.0) THEN
               TYPTGT = 0
            ELSE
               IF (TYMATG.EQ.'PERTURBATION') THEN
                  TYPTGT = 1
                  CALL GETVR8(MOCLEF(K),'VALE_PERT_RELA',1,1,1,
     &                        PERT,IRET)
...
C              Verif que TYMATG est possible pour COMP
     ...
     &           1,1,1,ITDEBO,IRET)
         CALL GETVR8(MOCLEF(K),'RESI_DEBO_MAXI',1,1,1,
     &              RESID,IRET)
         IF (IRET.NE.0) THEN
            RESID=-RESID
         ELSE
            CALL GETVR8(MOCLEF(K),'RESI_DEBO_RELA',1,1,1,
     &              RESID,IRET)
         ENDIF
      ENDIF
      IF (K.EQ.1) THEN
          CALL GETVR8(MOCLEF(K),'PARM_THETA',1,1,1,THETA,IRET)
      ELSE
          THETA=1.D0
      ENDIF
      CARCRI(1)=ITEINT
