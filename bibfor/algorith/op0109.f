...
C
      ...
      TYPCMA = ' '
      CALL GETVTX('COMB_MULT_APPUI','TYPE'      ,1,1,1,TYPCMA,NTY1)
...
C
   ...
   15    CONTINUE
     ...
     &         1,TYPCDI)
         IF (NTY1.NE.0 .OR. NTY2.NE.0) THEN
...
C     ----- CAS DU MULTI-SUPPORT -----
      IF ( .NOT.MONOAP) THEN
         ...
         CALL WKVECT('&&OP0109.TYPE_APP','V V I',NBSUP*3,JCAP)
         CALL ASMSUP(MASSE,MECA,STAT,NBMODE,NEQ,NBSUP,ZI(JNSU),
     +               ZK8(JKNO),ZI(JDIR),PSMO,ZR(JDEP),ZR(JREA),
     +               ZI(JCSU),NUME,ZI(JORDR) )
C
         CALL ASENAP(MASSE,NBSUP,ZI(JNSU),
     +               ZK8(JKNO),ZI(JDIR),ZR(JDEP),ZI(JCAP) )        
      ENDIF
C     --- CALCUL DES REPONSES ---
     ...
     +     ZR(JREA),ZR(JDEP),ZI(JCSU),ZI(JCAP),CORFRE,INFO)
      IF (NDEPL.NE.0) 
     +   CALL ASIMPR(NBSUP,MASSE,ZI(JCSU),ZI(JCAP),ZK8(JKNO))
