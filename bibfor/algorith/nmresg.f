...
C        
     ...
     &                              ZR(J2MEMB),1)
          DO 12 IMODE2 = 1,NBMODE
            ZR(JFMODA+IMODE-1) = ZR(JFMODA+IMODE-1)  
     &        - ZR(JRIGGE+(IMODE2-1)*NBMODE+IMODE-1)*ZR(JDEPGP+IMODE2-1)
     &        - ZR(JAMOGE+(IMODE2-1)*NBMODE+IMODE-1)*ZR(JVITGP+IMODE2-1)
 12       CONTINUE
          DO 15 IFONC = 1,NFONC
            ZR(JFMODA+IMODE-1) = ZR(JFMODA+IMODE-1)  
     &        + ZR(JFORGE+(IFONC-1)*NBMODE+IMODE-1)*ZR(JVALFO+IFONC-1)
 15       CONTINUE
 11     CONTINUE
C
C --- CALCUL DES ACCELERATIONS GENERALISEES
C        
        DO 71 IMODE = 1,NBMODE
...
C   
      ...
      CALL MDGEPH(NEQ,NBMODE,ZR(JBASMO),ZR(JACCG),ZR(JACCP))  
C 
