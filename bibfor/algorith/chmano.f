...
C ---------------- FIN DECLARATIONS NORMALISEES JEVEUX -----------------
      ...
      INTEGER JAPMEM,JAPPAR,JAPPTR,JAPCOE,JAPNOR,JAPJEU,JPDDL,JDEC
      ...
      INTEGER JTANG,JAPJFX,JAPCOF,JMETH,JAPJFY
      REAL*8 NORM(3),COEF(30),OLDJEU,JEU,JEUMIN,R8GAEM
      REAL*8 TANG(6),JEUFX,COFX(30),COFY(30)
      CHARACTER*8 NOM1,NOM2
      CHARACTER*24 APMEMO,APPARI,APPOIN,APCOEF,APNORM,APJEU,APDDL
      CHARACTER*24 APJEFX,APCOFR,TANGCO,DDLCO,PDDL,METHCO,APJEFY
      CHARACTER*24 NDIMCO,CONTNO,CONTMA,MANOCO,PMANO,NOMACO,PNOMA,NORMCO
...
C - CALCUL DE LA PROJECTION SUR LA MAILLE, ET DU JEU MINIMUM
          ...
          CALL PROJEC(IZONE,NDIM,NOMA,NEWGEO,DEFICO,MULNOR,POSNO1,POSMA,
     &                REAPRO,CMULT,NORM,POSNO,COEF,OLDJEU,JEU,DDL,NBNO,
     &                NBDDL,TANG,JEUFX,COFX,COFY)
...
C - SI JEU (AVANT CORRECTION) < JEU MINIMUM, STOCKER LES VALEURS
          ...
          IF (OLDJEU.LT.JEUMIN) THEN
            ...
            ZR(JAPNOR+3* (IESCL-1)) = NORM(1)
            ZR(JAPNOR+3* (IESCL-1)+1) = NORM(2)
            ZR(JAPNOR+3* (IESCL-1)+2) = NORM(3)
            NBDDL1 = ZI(JPDDL+POSNO1) - ZI(JPDDL+POSNO1-1)
            IF (ZI(JMETH+6).EQ.2) THEN
              ZR(JTANG+3* (IESCL-1)) = TANG(1)
              ZR(JTANG+3* (IESCL-1)+1) = TANG(2)
              ZR(JTANG+3* (IESCL-1)+2) = TANG(3)
              DO 10 K = 1,NBDDL1
