...
C ----- REMPLISSAGE DE LA CARTE CARTCF.POINT
        ...
        ZR(JVALV1-1+12) = NPTE
        ZR(JVALV1-1+13) = MMINFR(DEFICO,'COEF_REGU_CONT' ,IZONE )
        ZR(JVALV1-1+14) = MMINFR(DEFICO,'COEF_REGU_FROT' ,IZONE )
        ZR(JVALV1-1+15) = MMINFR(DEFICO,'COEF_COULOMB'   ,IZONE )
        ZR(JVALV1-1+16) = MMINFI(DEFICO,'FROTTEMENT_ZONE',IZONE )
        ...
        CALL NOCART(CPOINT,-3,KBID,'NUM',1,KBID,-IPC,LIGRXF,NCMP1)
C ----- REMPLISSAGE DE LA CARTE CARTCF.PINTER
        DO 10 I=1,NDIM
          DO 20 J=1,NPTE                  
            CALL CESEXI('S',JCESD2,JCESL2,NUMMAE,1,1,NDIM*(J-1)+I,IADE)
...
C ----- REMPLISSAGE DE LA CARTE CARTCF.AINTER
        DO 40 I=1,ZXAIN
          DO 50 J=1,NPTE
            CALL CESEXI('S',JCESD3,JCESL3,NUMMAE,1,1,ZXAIN*(J-1)+I,IADE)
            CALL ASSERT(IADE.GT.0)
...
C ----- REMPLISSAGE DE LA CARTE CARTCF.CCFACE	
        DO 70 I=1,NDIM
          DO 80 J=1,NFACE
            CALL CESEXI('S',JCESD4,JCESL4,NUMMAE,1,1,NDIM*(J-1)+I,IADE)
            CALL ASSERT(IADE.GT.0)
...
C
        IF (NIV.GE.2) THEN
          CALL XMIMP3(IFM,NOMA,IPC,JVALV1,JTABF)
        ENDIF
