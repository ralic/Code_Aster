...
C
      ...
      PARAMETER    (NCMP1=30)
      INTEGER      CFMMVD,ZTABF,ZCMCF,IMAES
      INTEGER      I,J,IPC,K,IZONE,NBPC,IBID,NDIM,NPTE
      INTEGER      JTABF,JCMCF,JDIM,NFACE
      INTEGER      JVALV1,JVALV2,JVALV3,JVALV4
      INTEGER      IADE,IADM
      INTEGER      JNCMP1,JNCMP2,JNCMP3,JNCMP4
      INTEGER      JCESL2,JCESD2,JCESV2
      INTEGER      JCESL3,JCESD3,JCESV3,JCESL4,JCESD4,JCESV4
      CHARACTER*2  CH2
      CHARACTER*8  KBID
...
C --- ON RECUPERE LES NUMEROS DES MAILLES ESCLAVE
        IMAES= NINT(ZR(JTABF+ZTABF*(IPC-1)+1))
C
...
C        ZR(JVALV1-1+12) = ZR(JTABF+ZTABF*(IPC-1)+14)
        ...
        ZR(JVALV1-1+18) = ZR(JTABF+ZTABF*(IPC-1)+30)
        ZR(JVALV1-1+19) = ZR(JTABF+ZTABF*(IPC-1)+16)    
        ZR(JVALV1-1+20) = ZR(JTABF+ZTABF*(IPC-1)+29)
        ZR(JVALV1-1+21) = ZR(JTABF+ZTABF*(IPC-1)+28)
        ZR(JVALV1-1+22) = ZR(JTABF+ZTABF*(IPC-1)+25)
        ZR(JVALV1-1+23) = ZR(JTABF+ZTABF*(IPC-1)+31)
        ZR(JVALV1-1+24) = ZR(JTABF+ZTABF*(IPC-1)+17)
        ZR(JVALV1-1+25) = ZR(JTABF+ZTABF*(IPC-1)+18)
        ZR(JVALV1-1+26) = ZR(JTABF+ZTABF*(IPC-1)+19)
        ZR(JVALV1-1+27) = ZR(JTABF+ZTABF*(IPC-1)+20)
        ZR(JVALV1-1+28) = ZR(JTABF+ZTABF*(IPC-1)+21)
        ZR(JVALV1-1+29) = ZR(JTABF+ZTABF*(IPC-1)+23)
        ZR(JVALV1-1+30) = ZR(JTABF+ZTABF*(IPC-1)+27)
C
...
C
        DO 10 I=1,NDIM
          DO 20 J=1,NPTE                  
            CALL CESEXI('S',JCESD2,JCESL2,IMAES,1,1,NDIM*(J-1)+I,IADE)
            CALL ASSERT(IADE.GT.0)
            ZR(JVALV2-1+NDIM*(J-1)+I)=ZR(JCESV2-1+IADE)
 20       CONTINUE
 10     CONTINUE
...
C
        DO 40 I=1,ZXAIN
          DO 50 J=1,NPTE
            CALL CESEXI('S',JCESD3,JCESL3,IMAES,1,1,ZXAIN*(J-1)+I,IADE)
            CALL ASSERT(IADE.GT.0)
            ZR(JVALV3-1+ZXAIN*(J-1)+I)=ZR(JCESV3-1+IADE)
 50       CONTINUE
 40     CONTINUE          
        CALL NOCART(CAINTE,-3,KBID,'NUM',1,KBID,-IPC,LIGRCF,NCMP3)
C
C --- REMPLISSAGE DE LA CARTE CARTCF.CFACE
C
        DO 70 I=1,NDIM
          DO 80 J=1,NFACE
            CALL CESEXI('S',JCESD4,JCESL4,IMAES,1,1,NDIM*(J-1)+I,IADE)
            CALL ASSERT(IADE.GT.0)
            ZR(JVALV4-1+NDIM*(J-1)+I)=ZI(JCESV4-1+IADE)
 80       CONTINUE
 70     CONTINUE
