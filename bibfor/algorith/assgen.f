      SUBROUTINE  ASSGEN (NOMRES,OPTION,NOMNUM,NOMSTO)
...
C
      ...
      CHARACTER*9  OPTION,RIGOPT,MASOPT,AMOOPT,KSST,LSST
...
C
      CALL JEMARQ()
      CALL WKVECT(NOMRES//'           .REFA','G V K24',4,LDREF)
      CALL JEECRA(NOMRES//'           .REFA','DOCU',BID,'SLCS')
      ZK24(LDREF)='                    '
      ZK24(LDREF+1)=NOMNUM
      ZK24(LDREF+2)=NOMSTO
C
...
C
      CALL JENONU(JEXNOM(NOMNUM//'.LILI',LSST),IBID)
      CALL JELIRA(JEXNUM(NOMNUM//'.PRNO',IBID),'LONMAX',NBLIA,K1BID)
      IF (NBLIA.EQ.1) THEN
        CALL UTMESS('F','ASSGEN',
...
C
     ...
     &'CONSTANT',NBLOC)
      ...
      CALL JEECRA(NOMRES//'           .VALE','DOCU',BID,'MS')
C
...
C
      DO 10 I=1,NBPRNO
        ...
        CALL JELIRA(JEXNUM(NOMNUM//'.PRNO',I),'LONMAX',NTPRNO,K1BID)
        NTPRNO=NTPRNO/2
C
...
C
        ELSEIF(NOMPRN.NE.KSST.AND.OPTION.EQ.RIGOPT) THEN
          CALL JECROC(JEXNOM(TMREP,NOMPRN))
          CALL JENONU(JEXNOM(TMREP,NOMPRN),IBID)
          ...
          CALL JEVEUO(JEXNUM(NOMNUM//'.ORIG',IBID),'L',LLORL)
C
C    BOUCLE SUR LES ELEMENTS DU LIGREL COURANTS
C
          DO 30 J=1,NTPRNO
...
C
        CALL PRASMP(OPTION,NOMSTO,NOMNUM,TMINBL,NOMPRN,MODGEN,TMNOBL,
     &              TMADBL,ZK24(LTNOMB),ZI(LTNUMB),SSMAX)
C
C
C    CAS D'UN LIGREL DE LAGRANGES ET OPTION=RIGI_GENE
C
        CALL PRASML(OPTION,NOMSTO,NOMNUM,TMINBL,NOMPRN,MODGEN,TMNOBL,
...
C
        ...
        CALL JEVEUO(JEXNUM(NOMRES//'           .VALE',IBLO),'E',LDBLO)
C
...
110    CONTINUE
C
        CALL JELIBE(JEXNUM(NOMRES//'           .VALE',IBLO))
C
100   CONTINUE
C
C
