...
C MODIF ALGORITH  DATE 19/11/2007   AUTEUR GENIAUT S.GENIAUT 
C ======================================================================
...
C ======================================================================
      ...
      INTEGER      JDEPV,NOMIL,JCNSVR,JCNSLR,IBID,CFMMVD
      INTEGER      NSOM,NOSOM(2),AR(12,2),NBAR,ZCNS
      CHARACTER*8  FISS,NOMA,K8BID,TYPMA,LICMP(6),LICMPR(27),NOMNO
      ...
      REAL*8       XYZ(3),LAMBDA(3),R8PREM,DIST,COEF,JEU,FF(2)
...
C ------ POUR RECUPERER LES FORCES NODALES DE CONTACT
      ...
      CALL ASSVEC('V',FCONT,1,'&&MMCCON', 1.D0,NUMEDD,' ','ZERO',1)
      ...
      CALL ASSVEC('V',FFROT,1,'&&MMCFROT',1.D0,NUMEDD,' ','ZERO',1)
...
C       BOUCLE SUR LES POINTS D'INTERSECTION
        DO 120 I=1,NINTER
          ...
          CALL CESEXI('S',JCESD2,JCESL2,IMA,1,1,4*(I-1)+3,IAD)
          CALL ASSERT(IAD.GT.0)
          LONGAR =ZR(JCESV2-1+IAD)
          CALL CESEXI('S',JCESD2,JCESL2,IMA,1,1,4*(I-1)+4,IAD)
          CALL ASSERT(IAD.GT.0)
...
C         RECUP DES NUMEROS GLOBAUX DES NOEUDS DE L'ARETE/NOUED SOMMENT
          IF (IN.NE.0) THEN
            NSOM = 1
...
C         OR AVEC X-FEM, L'INTEGRATION SE FAIT AUX POINTS DE GAUSS
C          ZR(JCNSVR-1+ZCNS*(NUNO-1)+1)=0
...
C         RTAX, RTAY, RTAZ, RTGX, RTGY, RTGZ
          ...
          DO 160 J=1,NDIM
            LAGFRO(J) = ZR(JLAGV-1+NDIM*(NUNO-1)+2) * TAU1(J)
            IF (NDIM.EQ.3) LAGFRO(J) = LAGFRO(J) 
     &                + ZR(JLAGV-1+NDIM*(NUNO-1)+3) * TAU2(J)
 160      CONTINUE
