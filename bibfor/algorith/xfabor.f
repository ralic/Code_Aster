      SUBROUTINE XFABOR(NOMA,NUNOA,NUNOB,NUNOC,FABORD)
      IMPLICIT NONE
...
C
      INTEGER         IBID,JMA,NMANOA,NMANOB,NMANOC,JMANOA,JMANOB,JMANOC
      INTEGER         IMAA,IMAB,IMAC,ITYPMA,NUMAA,NUMAB,NUMAC,NBMACO
      CHARACTER*19    MAI,CNXINV
...
C         SI MAILLE NON VOLUMIQUE ON CONTINUE À 110
          IF (TYPMA(1:4).NE.'HEXA'.AND.TYPMA(1:5).NE.'PENTA'.
     &                             AND.TYPMA(1:5).NE.'TETRA') GOTO 110
...
C             SI MAILLE NON VOLUMIQUE ON CONTINUE À 120
              IF (TYPMA(1:4).NE.'HEXA'.AND.TYPMA(1:5).NE.'PENTA'.
     &                             AND.TYPMA(1:5).NE.'TETRA') GOTO 120
C     SI LA MAILLE EST EN COMMUN AUX NOEUDS B ET C (ET A) ,
C     ON A DECOUVERT UNE MAILLE QUE LES TROIS NOEUDS ONT EN COMMUN
