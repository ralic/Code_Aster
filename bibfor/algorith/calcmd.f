...
C ROUTINES EXTERNES
C -----------------
C     EXTERNAL   PRMAMA, TSTJAC, UTMESS,
...
C............. CALCUL DES VALEURS PROPRES DE LA MATRICE MTMP2
               ...
               IF ( IFAIL.NE.0 )
     &            CALL UTMESS('F','CALCMD','ERREUR DANS LE CALCUL '//
     &            'DES VALEURS PROPRES DE LA MATRICE DE RAIDEUR')
C
...
C
                  IF ( IIMA.EQ.1 )
     &               CALL UTMESS('A','CALCMD','VALEURS PROPRES DE LA '//
     &               'MATRICE DE RAIDEUR NON REELLES')
C
                  IF ( INEG.EQ.1 )
     &               CALL UTMESS('A','CALCMD','VALEURS PROPRES DE LA '//
     &               'MATRICE DE RAIDEUR REELLES NEGATIVES')
C
                  GO TO 999
C
               ENDIF
...
C............. DE LA MATRICE MTMP1
     ...
     &                     EPS,IFAIL)
               IF ( IFAIL.NE.0 )
     &            CALL UTMESS('F','CALCMD','ERREUR DANS LA SELECTION '//
     &            'DES VALEURS PROPRES DE LA MATRICE DE RAIDEUR')
C
...
C............. CALCUL DE LA MATRICE DIAGONALE B = (TRANSPOSEE DE VG)*VD
 ...
 31            CONTINUE
     ...
     &                           MTMP1,NP1,N,N,IER)
               IF ( IER.NE.0 )
     &            CALL UTMESS('F','CALCMD','TAILLES DES MATRICES '//
     &            'INCOMPATIBLES POUR CALCUL MATRICE DIAGONALE')
C
