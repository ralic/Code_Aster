      SUBROUTINE XENRCH(NOMA  ,CNSLT ,CNSLN ,CNSEN ,CNSENR, NDIM,
     &                  FISS  ,LISMAE,LISNOE)
C
...
C       ZAPPE TOUT CE QUI N'EST PAS MAILLE PRINCIPALE OU MAILLE DE BORD
C       ATTENTION : QD LES ELEMENT DE BORD EN 2D SERONT DISPO, IL FAUDRA
C       VIRER LE 2EME IF   !!!!!!!!!!!!!
        IF (NDIM.EQ.3.AND.TYPMA(1:3).EQ.'SEG') GOTO 310
C        IF (NDIM.EQ.2.AND.TYPMA(1:3).EQ.'SEG') GOTO 310
...
C--------------------------------------------------------------------
     ...
     &      'DU MAILLAGE EST '//K12//'.'
      WRITE(IFM,*)'NOMBRE DE POINTS DE FOND DE FISUURE',NFON
