...
C MODIF ALGORITH  DATE 05/04/2000   AUTEUR ACBHHCD G.DEVESA 
C ======================================================================
...
C ------------- CALCUL DES DEPLACEMENTS ET VITESSES
         DO 21 IEQ=1, NEQ
              ZR(IVITE1+IEQ-1)=VIT0(IEQ)+DT*ACC0(IEQ)
              ZR(IDEPL1+IEQ-1)=DEP0(IEQ)+DT*ZR(IVITE1+IEQ-1)
 21      CONTINUE             
C
...
C ------------- ARCHIVAGE DU DERNIER INSTANT DE CALCUL
 ...
 78      CONTINUE
      ENDIF
      IF (ISTOP.EQ.1)
     +       CALL UTMESS('S','OP0048','ARRET PAR MANQUE DE TEMPS CPU')
C
