...
C 0.3. ==> VARIABLES LOCALES
      ...
      CHARACTER*8 VECEL,NEWNOM,CARA,NOMCHA
      CHARACTER*8 NOPASE,MATERS,NOMGD
      CHARACTER*16 OPTION
      CHARACTER*19 CHVARC
      CHARACTER*24 MODELE,CARELE,LVECHP
      CHARACTER*24 LIGRMO,CHCHAR,LCH24
      CHARACTER*24 CHGEOM,CHCARA(15),CHTIME,CHTREF,CHCARS(15)
      CHARACTER*24 LCHIN(NCHINX),LCHOUT(NCHOUX),CHTEMP
      CHARACTER*24 DEPLA,CONTRR,TEMPER,DERITE
      CHARACTER*24 VECTTH,MATSEN,MATERI,CHHARM,MATSEM


      REAL*8 TIME

      LOGICAL EXIGEO,EXITIM,ZFAT
...
C      DETERMINATION DE CARA_ELEM DERIVE NON CODE MATSEN
        CALL PSRENC(CARA,NOPASE,MATERS,IRET)
        IF (IRET.EQ.0) THEN
C CAS ELEMENTS DISCRETS
C      TRANSFORMATION EN CARA ELEM DERIVE CODE (RAIDEUR+MASSE)
          NH = 0
          CALL MERIME (MODELE,NCHAR,LCHAR,MATERI,
     >               CARELE,.FALSE.,TIME,MATERS,NH,'G')
          CALL MECHAM(OPTION,MODELE,NCHAR,LCHAR,MATERS,NH,CHGEOM,
     &              CHCARS,CHHARM,ICODE)
...
C------EMISSION D'UN MESSAGE D'ERREUR FATALE DANS CE CAS
   ...
   10     CONTINUE
          IF (ZFAT) CALL UTMESS('F',NOMPRO,'CALCUL DE SENSIBILITE'
     &         //' PAS ENCORE DISPONIBLE POUR LES CHARGEMENTS DE'
     &         //' TYPE EPSI_INIT'  )
        ENDIF
C------
C------
...
C 3.3. ==> LES DONNEES THERMIQUES
        TIME = PARTPS(1)
        CALL MECHTE(MODELE,NCHAR,LCHAR,MATE,EXITIM,TIME,CHTREF,CHTEMP)

C 3.4. ==> LES CHARGEMENTS DE PESANTEUR
        CALL MECHPE(NCHAR,LCHAR,CHCHAR)

C 3.6. ==> LES PARAMETRES TEMPORELS
...
C 4.1. ==> LES VARIABLES DE COMMANDES
        CALL VRCINS(MODELE(1:8),MATE(1:8),CARELE(1:8),TIME,CHVARC)
