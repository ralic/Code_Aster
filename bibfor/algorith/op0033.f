...
C    DIMANV = DIMENSION MAX DE LA LISTE DU NOMBRE DE VAR INT EN THM
      ...
      REAL*8       DEPS(9),SIGM(6),SIGP(6),EPSM(9),EPS(9),VIM2(NVIMAX)
...
C        6 CMP DE EPSI OU 9 CMP DE GRAD DONNEES : PAS BESOIN DE NEWTON 
 ...
 30            CONTINUE
            ...
            ENDIF
            IF (INCELA.EQ.1) THEN
               CALL DCOPY(NBVARI,VIM,1,VIM2,1)
               CALL NMCOMP (FAMI,KPG,KSP,NDIM,TYPMOD,IMATE,COMPOR,
     &         CARCRI,INSTAM,INSTAP,NCMP,EPSM,DEPS,6,SIGM,VIM2,OPT2,
     &         ANG,10,WORK,SIGP,VIP,6*NCMP,DSIDEP,1,RBID,IRET)
     ...
     &           COMPOR,CARCRI,OPTION,EPS,SIGP,VIP,DSIDEP,IRET)
            ELSE
               CALL ASSERT(.FALSE.)
            ENDIF
...
C        INITIALISATION DE L'ALGO DE NEWTON 
         ...
         IF (PRED.EQ.1) THEN
            ...
            IF (INCELA.EQ.1) THEN
               CALL NMCOMP (FAMI,KPG,KSP,NDIM,TYPMOD,IMATE,COMPOR,
     &              CARCRI,INSTAM,INSTAP,6,EPSM,DEPS,6,SIGM,SVIP,OPT2,
     &              ANG,10,WORK,SSIGP,SVIP,36,DSIDEP,1,RBID,IRET)
            ELSEIF (INCELA.EQ.2) THEN
...
C           CALCUL DU RESIDU
     ...
     &             INSTAM,INSTAP,6,EPSM,DEPS,6,SIGM,VIM2,OPTION,
     &             ANG,10,WORK,SIGP,VIP,36,DSIDEP,1,RBID,IRET)
            ELSEIF (INCELA.EQ.2) THEN
...
C           VERIFICATION DES EVENT-DRIVEN
  500       CONTINUE
            CALL PMSTA1(SIGM,SIGP,DEPS,VIM,VIP,NBVARI,NBVITA,IFORTA,
...
C        STOCKAGE EFFECTIF DU RESULTAT DANS LA TABLE
         CALL PMSTAB(SIGM,SIGP,EPSM,DEPS,NBVARI,VIM,VIP,IFORTA,INSTAM,
     &    INSTAP,ITER,NBPAR,NOMPAR,TABLE,VR,IGRAD,VALIMP,IMPTGT,
     &    DSIDEP,NOMVI,NBVITA)
