...
C
     ...
     &         SU, SY , E , EP , PUISS, XNU
      ...
      CHARACTER*8  NOPAR,NOMRE2(4),NOMRE3(10),NOMRE4(2),FAMI,POUM
...
C     LE TYPE D'ECROUISSAGE
      IF ( COMPOR(1) .EQ. 'VMIS_POU_LINE' ) THEN
         NUMLOI = 1
         CALL RCVALB(FAMI,KPG,SPT,POUM,ICODMA,' ','ECRO_LINE',NBPAR,
     &              NOPAR,VALPAR,2, NOMRE4,VALRES,ICODRE,1)
         EP = VALRES(1)
         SY = VALRES(2)
         IF (EP .GE. E) THEN
            VALKM(1) = 'EP'
      ...
      ELSE IF ( COMPOR(1) .EQ. 'VMIS_POU_FLEJOU' ) THEN
         NUMLOI = 2
         CALL RCVALB(FAMI,KPG,SPT,POUM,ICODMA,' ','ECRO_FLEJOU',NBPAR,
     &               NOPAR,VALPAR,4,NOMRE2,VALRES,ICODRE,1)
         EP    = VALRES(1)
         SY    = VALRES(2)
         SU    = VALRES(3)
         PUISS = VALRES(4)
...
C     NP,MEY,CAY,...,MPX
      CALL RCVALB(FAMI,KPG,SPT,POUM,ICODMA,' ','VMIS_POUTRE',NBPAR,
     &           NOPAR,VALPAR,10, NOMRE3,VALRES,ICODRE,1)
      XNP  = VALRES(1)
      XMEY = VALRES(2)
      XMPY = VALRES(3)
      XAY  = VALRES(4)
