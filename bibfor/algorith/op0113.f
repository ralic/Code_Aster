...
C     
      ...
      INTEGER         NMAENR,ITYPEL,ITYXH,ITYXT,ITYXHT,Q(6),NB1
...
C-----------------------------------------------------------------------
      ...
      CALL JENONU(JEXNOM('&CATA.TE.NOMTE','MECA_XH_HEXA8'),ITYXH)
      CALL JENONU(JEXNOM('&CATA.TE.NOMTE','MECA_XT_HEXA8'),ITYXT)
      CALL JENONU(JEXNOM('&CATA.TE.NOMTE','MECA_XHT_HEXA8'),ITYXHT)
      ...
      DO 200 IGR1=1,NGR1
        ...
        DO 210 IEL=1,NBELT
          ...
          IF (NOTYPE.EQ.'MECA_HEXA8') THEN
            IF (ZI(JJ+1).EQ.1)  ZI(JJ+5)=ITYXH
            IF (ZI(JJ+2).EQ.1)  ZI(JJ+5)=ITYXT
            IF (ZI(JJ+3).EQ.1)  ZI(JJ+5)=ITYXHT
            IF (ZI(JJ+4).EQ.1)  ZI(JJ+5)=ITYPEL          
          ELSEIF (NOTYPE.EQ.'MECA_PENTA6') THEN
            IF (ZI(JJ+1).EQ.1.OR. ZI(JJ+2).EQ.1.OR.ZI(JJ+3).EQ.1) THEN
              CALL UTMESS('F','OP0113','PENTA6 PAS ENCORE ENRICHIS')
            ENDIF
            ZI(JJ+5)=ITYPEL
          ELSE
            ZI(JJ+5)=ITYPEL
