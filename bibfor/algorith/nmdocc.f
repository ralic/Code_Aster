...
C    DIMANV = DIMENSION MAX DE LA LISTE DU NOMBRE DE VAR INT EN THM
      ...
      CHARACTER*19 COMPOR
      CHARACTER*8  NOMA,K8B,TYPMCL(2),NOMCMP(*),SDCOMP
...
C         CAS PARTICULIER DE MONOCRISTAL
          IF (COMP(1:8).EQ.'MONOCRIS') THEN
            ...
            IF ((DEFO.EQ.'SIMO_MIEHE').AND.(COMP.EQ.'MONOCRISTAL')) THEN
               CALL GETVTX(MOCLEF(I),'ALGO_INTE',K,1,1,ALGO,N1)
...
C         ET LA SUBROUTINE DANS KIT9
          IF (COMP(1:4).EQ.'UMAT') THEN
            CALL GETVIS(MOCLEF,'NB_VARI',K,1,1,NBVARZ,N1)
            NBVARI=NBVARZ+NBVARI
            CALL GETVTX(MOCLEF, 'LIBRAIRIE',K,1,1,NOMLIB,N1)
            CALL GETVTX(MOCLEF, 'NOM_ROUTINE',K,1,1,NOMSUB,N1)
            DO 30 II = 1,DIMAKI-1
