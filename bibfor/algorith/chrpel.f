...
C     REPERE   IN  K16  : TYPE DE REPERE (UTILISATEUR OU CYLINDRIQUE)
...
C
      ...
      REAL*8       VECTX(3), VECTY(3),  ANGREP(3)
      ...
      CHARACTER*8  LPAIN(4),PAOUT,LICMP(3),KBID
...
C     ON EXCLUT LES MOT-CLES 'NOEUD' ET 'GROUP_NO'
      ...
      IF (IERK.NE.0) THEN
        ...
        CALL GETVTX('AFFE','GROUP_NO',ICHAM,IARG,0,ZK24(IGNO),IRET1)
      ELSE
        IRET1=0
      ENDIF
      IF(IRET0.LT.0)THEN
...
C
     ...
     &        (REPERE(1:15).EQ.'COQUE_UTIL_INTR')) THEN
...
C
        ...
        LICMP(2) = 'BETA'
        LICMP(3) = 'REP'
        CALL MECACT('V',CARTE,'MODELE',MODELE,'CAORIE',3,LICMP,
     +               IBID,ANGREP,CBID,' ')
C
