...
C ----------------------------------------------------------------------
C     ARGUMENTS :
C     CHAMP1   IN  K16  : NOM DU CHAMP A TRAITER (CHAMP OUT)
...
C     TYPE     IN  K16  : TYPE DU CHAMP :'TENS' 'VECT' 'TORS' OU 'COQUE'
C     NOMCH    IN  K16  : NOM DE CHAMP
C     CHAMP0   IN  K16  : NOM DU CHAMP IN
C ----------------------------------------------------------------------
...
C
      ...
      REAL*8       VECTX(3), VECTY(3),  ANGREP(2)
      REAL*8       X(27), Y(27), Z(27)
      REAL*8       XPG(27), YPG(27), ZPG(27)
      COMPLEX*16   VALETC(6),CBID
      CHARACTER*1  K1B
      CHARACTER*3  TSCA
      CHARACTER*8  MA    , K8B, TYPMCL(2), NOMGD, TYCH, PARAM
      CHARACTER*8  LPAIN(4),PAOUT,LICMP(2),KBID
      CHARACTER*16 OPTION,MOTCLE(2),NOMCH2
      CHARACTER*19 CHAMS1,CHAMS0,LIGREL,MANOGA,CANBSP
      CHARACTER*19 CHANGL,CARTE
      CHARACTER*24 MESMAI,CHGEOM,LCHIN(4),CHAOUT
      CHARACTER*24 VALK(3),CHCARA(18)
C
      CALL JEMARQ()
      EPSI = 1.0D-6
      MOTCLE(1) = 'GROUP_MA'
...
C
C ----- DEFINITION ET CREATION DU CHAM_ELEM SIMPLE CHAMS1
C ----- A PARTIR DU CHAM_ELEM CHAMP1
C
...
C
      ...
      CALL RELIEM(' ',MA,'NU_MAILLE','AFFE',ICHAM,2,MOTCLE,TYPMCL,
     &                                                   MESMAI,NBM)
      IF (NBM.GT.0) THEN
        NBMAIL = NBM
...
C        SI LE NOUVEAU REPERE EST DONNE VIA DES VECTEURS
         CALL GETVR8('AFFE','VECT_X',1,1,3,VECTX,IBID)
         IF ((IBID.EQ.3).AND.(NDIM.EQ.3)) THEN
            CALL GETVR8('AFFE','VECT_Y',1,1,3,VECTY,IBID)
            CALL ANGVXY(VECTX,VECTY,ANGNOT)
         ELSE
           IF (NDIM.EQ.3) THEN
            CALL GETVR8('AFFE','ANGL_NAUT',1,1,3,ANGNOT,IBID)
            IF (IBID.NE.3) THEN
               CALL U2MESS('F','ALGORITH2_7')
            ENDIF
           ELSE
            CALL GETVR8('AFFE','ANGL_NAUT',1,1,1,ANGNOT(1),IBID)
            IF (IBID.NE.1) THEN
                           VALR = ANGNOT(1)
...
C REPERE CYLINDRIQUE
     ...
     &                TYCH, IRET)
         IF (NDIM.EQ.3) THEN
            CALL GETVR8('AFFE','ORIGINE',1,1,3,ORIG,IBID)
            IF (IBID.NE.3) THEN
               CALL U2MESS('F','ALGORITH2_8')
            ENDIF
            CALL GETVR8('AFFE','AXE_Z',1,1,3,AXEZ,IBID)
            IF (IBID.EQ.0) THEN
               CALL U2MESS('F','ALGORITH2_9')
            ENDIF
         ELSE
            CALL GETVR8('AFFE','ORIGINE',1,1,2,ORIG,IBID)
            IF (IBID.NE.2) THEN
               CALL U2MESS('A','ALGORITH2_10')
            ENDIF
            CALL GETVR8('AFFE','AXE_Z',1,1,0,AXEZ,IBID)
            IF (IBID.NE.0) THEN
...
C CHAMP COMPLEXE
 ...
 29         CONTINUE
     ...
     &            IBID)
         CALL DETRSD('CHAM_ELEM_S',CHAMS1)
C
      ELSE IF(REPERE.EQ.'COQUE') THEN
        CALL MEGEOM(MODELE,' ',EXIGEO,CHGEOM)
        CALL MECARA(CARELE,EXICAR,CHCARA)
C
C    GENERATION D UN CHAMP D'ANGLES (CARTE CONSTANTE)
C
        ...
        LICMP(1) = 'ALPHA'
        LICMP(2) = 'BETA'
        CALL MECACT('V',CARTE,'MODELE',MODELE,'CAORIE',2,LICMP,
     +               IBID,ANGREP,CBID,' ')
C
C  CREATION D UN CHAM_ELEM D'ANGLES EN LISANT LES ANGL_REP
C
        CHANGL = '&&CHRPEL.ANGL'
        ...
        ELSE IF(TYPE.EQ.'TENS_3D') THEN
          ...
          IF(NOMCH.EQ.'SIGM_ELGA') THEN
            LPAIN(4) = 'PCOGAIN'
            PAOUT = 'PCOGAOUT'
          ELSE IF(NOMCH.EQ.'SIGM_ELNO'.OR.NOMCH.EQ.'SICO_ELNO') THEN
            LPAIN(4) = 'PCONOIN'
            PAOUT = 'PCONOOUT'
        ...
        IF (IRET1.NE.1) THEN
           CALL DISMOI('F','MXNBSP',CHAMP0(1:19),'CHAM_ELEM',
     &                 NBSP,KBID,IRET)
C
C SI LE CHAMP A DEJA ETE EXTRAIT IL FAUT APPELER CESVAR AVEC CE CHAMP
C
           IF(NBSP.EQ.1.OR.NOMCH.EQ.'SICO_ELNO') THEN
        ...
        CALL CALCUL('S',OPTION,LIGREL,4,LCHIN,LPAIN,1,
     &              CHAOUT,PAOUT,'V','OUI')
