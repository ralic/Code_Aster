      SUBROUTINE  PRASML (OPTION,NOMSTO,NOMNUM,TMINBL,NOMPRN,MODGEN,
...
C
      ...
      CHARACTER*9  OPTION,RIGOPT,KSST
...
C
C-----------------------------------------------------------------------
      DATA RIGOPT/'RIGI_GENE'/
      DATA ZERO / 0.0D+00 /
      DATA KSST /'&SOUSSTR'/
C-----------------------------------------------------------------------
C
C    TEST SI ON EST SUR LES LAGRANGES ET SI OPTION=RIGI_GENE
C
      CALL JEMARQ()
      IF(NOMPRN.EQ.KSST.OR.OPTION.NE.RIGOPT) GOTO 9999
C
...
C
      ...
      CALL JENONU(JEXNOM(NOMNUM//'.LILI',NOMPRN),IBID)
      CALL JEVEUO(JEXNUM(NOMNUM//'.PRNO',IBID),'L',LLPRL)
      CALL JENONU(JEXNOM(NOMNUM//'.LILI',KSST),IBID)
...
50          CONTINUE
40        CONTINUE
CALL JEIMPO('MESSAGE',JEXNUM(TMADBL,IBLC),' ','BIDON')
CALL JEIMPO('MESSAGE',JEXNUM(TMNOBL,IBLC),' ','BIDON')
