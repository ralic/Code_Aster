...
C - LECTURE DES CARACTERISTIQUES ELASTIQUES DU MATERIAU (T- ET T+)
      ...
      CALL RCVALA(IMATE,'ELAS',1,'TEMP',TM,2,NOM(1),MATM(1),OK(1),FB2)
      CALL RCVALA(IMATE,'ELAS',1,'TEMP',TM,1,NOM(3),MATM(3),OK(3),BL2)
      ...
      CALL RCVALA(IMATE,'ELAS',1,'TEMP',TP,2,NOM(1),MAT(1),OK(1),FB2)
      CALL RCVALA(IMATE,'ELAS',1,'TEMP',TP,1,NOM(3),MAT(3),OK(3),BL2)
...
C - LECTURE DES CARACTERISTIQUES D'ECROUISSAGE (T+)
      CALL RCVALA(IMATE,'TAHERI',1,'TEMP',TP,8,NOM(4),MAT(4),OK(4),FB2)
...
C LECTURE DES CARACTERISTIQUES DE VISCOSITE (TEMPS +)
     ...
     &            MAT(12),OK(12),BL2)
      ...
      IF (VISCO) THEN
        IF (MAT(12).EQ.0.D0)
     &    CALL UTMESS('F','NMTAMA','VISCOSITE N EGALE A ZERO')
        MAT(12) = 1.D0 / MAT(12)

        IF (MAT(13).EQ.0.D0)
     &    CALL UTMESS('F','NMTAMA','VISCOSITE UN_SUR_K EGALE A ZERO')
