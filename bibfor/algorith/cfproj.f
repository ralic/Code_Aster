...
C
      INTEGER      JDECMA
      INTEGER      IFM,NIV
      INTEGER      IMAM,NTMAM,NBNOM,IBID
      INTEGER      NIVERR,NDIM,NDIMG,CFDISI
      REAL*8       COORMA(27),DIST(3)
      REAL*8       R8GAEM,R8PREM
      CHARACTER*8  ALIAS,NOMMAI,TYPMAM,NOMMAM
      CHARACTER*24 MANOCO
      INTEGER      JMANO
      CHARACTER*4  TYPMAL
      ...
      REAL*8       JEUMIN,JEUL
      INTEGER      POSMIN,POSMAL,NUMMAL,CODRET
...
C
      CALL CFNBEN(DEFICO,POSNOM,'NOEU',NTMAM ,JDECMA)
C
...
C
        ...
        CALL CFCARM(NOMA  ,DEFICO,NEWGEO,POSMAL,TYPMAL,
     &              NUMMAL,ALIAS ,NOMMAI,NDIM  ,NBNOM ,
     &              COORMA)
        IF (TYPMAL.NE.'MAIT') THEN
          CALL ASSERT(.FALSE.)
        ENDIF
C
...
C
     ...
     &              KSI1L ,KSI2L ,TAU1ML,TAU2ML,IPROJ ,
     &              NIVERR)
C
...
C
        CALL MMJEUX(ALIAS ,NBNOM ,NDIM  ,COORMA,KSI1L ,
     &              KSI2L ,COORPT,JEUL  ,DIST)
C
C --- CHOIX DE L'APPARIEMENT SUIVANT LE RESULTAT DE LA PROJECTION
C
        CALL CFCHOI(JEUL  ,JEUMIN,POSMAL,POSMIN,TAU1ML,
     &              T1MIN ,TAU2ML,T2MIN ,KSI1L ,XIMIN ,
     &              KSI2L ,YIMIN ,IPROJ ,IPROJM,DIST  ,
     &              DISTME)
C
   50 CONTINUE
C
...
C
      ...
      JEUPM  = JEUMIN
C
C --- CARACTERISTIQUES MAILLE MAITRE RESULTANTE
C
     ...
     &            NUMMAM,CODRET)
      IF (CODRET.LT.0) THEN
        CALL ASSERT(.FALSE.)
      ENDIF
      CALL MMELTY(NOMA  ,NUMMAM,TYPMAM,IBID  ,IBID  )
...
C
      IF (LPOUTR) THEN
        ...
        IF (NOOR.LE.R8PREM()) THEN
          CALL U2MESK('F','CONTACT3_38',1,NOMMAM)
        ENDIF
      ELSE
        CALL CFORTH(NDIMG ,TAU1M ,TAU2M ,NIVERR)
        IF (NIVERR.EQ.1) THEN
          CALL U2MESG('F','CONTACT3_14',1,NOMMAM,0,0,3,COORPT)
        ELSEIF (NIVERR.EQ.2) THEN
          CALL U2MESG('F','CONTACT3_34',1,NOMMAM,0,0,3,COORPT)
        ENDIF
      ENDIF
C
      CALL JEDEMA()
