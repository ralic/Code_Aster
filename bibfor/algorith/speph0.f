...
C-----------------------------------------------------------------------
     ...
     &        IVARI,NAPEX1,NBMR,IDIM,IMR,NUMOD,IN,NBPF,NBFO1,IF1,
     &        IFOR,IFOI,ICHAM1,ISIP,ICHAM,NBN1,NBN2,LTYPA
      INTEGER I1,LNUMI,LNUMJ,MXVAL,LREFE,LFREQ,LREFES,LFREQS
      REAL*8 R8B,BANDE(2),FREQ1,EPSI
      COMPLEX*16 C16B
      LOGICAL INTMOD,INTPHY,INTDON
      CHARACTER*8 K8B,MODMEC,MODSTA,NOEUD,NOMA,CMP
      CHARACTER*16 MOVREP,OPTCAL,OPTCHA,
     &             TYPCHA,ACCES,TYPMEC,NOCHAM,OPTCH1,MAILLE
      CHARACTER*16 OPTION
      CHARACTER*24 CHAM19,TYPBA
      CHARACTER*24 VALK(3),REFD
      CHARACTER*24 CHNUMI,CHNUMJ,CHFREQ
...
C-----------------------------------------------------------------------
      ...
      EPSI = 0.D0
      CALL RSORAC(MODMEC,'LONUTI',IBID,R8B,K8B,C16B,EPSI,K8B,NBMOD1,1,
     &            NBTROU)
      CALL WKVECT('&&SPEPH0.NUMERO.ORDRE','V V I',NBMOD1,LNUMOR)
      CALL RSORAC(MODMEC,'TOUT_ORDRE',IBID,R8B,K8B,C16B,EPSI,K8B,
...
C     --- RECUPERATION DU NUMERO DU DDL ---
      ...
      CALL RSEXCH('F',MODMEC,OPTCH1,ZI(ILMODE),CHAM19,IRET)
...
C     --- ON NE PREND EN COMPTE QUE LES MODES DYNAMIQUES ---
     ...
     &              NUMOD,1,NBTROU)
        ...
        END IF
        CALL RSEXCH('F',MODSTA,OPTCH1,NUMOD,CHAM19,IRET)
   ...
   60 CONTINUE
      ...
      DO 90 IMR = 1,NBMOD1
        ...
        CALL RSEXCH('F',MODMEC,OPTCH1,NUMOD,CHAM19,IRET)
        CALL JEVEUO(CHAM19(1:19)//'.VALE','L',ISIP)
...
C     --- CREATION DE LA TABLE DE SORTIE ---
  ...
  100 CONTINUE
     ...
     &            ZK8(INOEN),ZK8(ICMPN),NBMR,NBN,NBPF)
