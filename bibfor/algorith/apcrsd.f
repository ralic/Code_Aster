      SUBROUTINE APCRSD(SDAPPA,NBZONE,NTPT  )
C     
...
C
      ...
      CALL WKVECT(APINFI,'V V I',5,JPINFI)
      APINFR = SDAPPA(1:19)//'.INFR'
      CALL WKVECT(APINFR,'V V R',1,JPINFR)
C
C --- CREATION SD INFORMATIONS PAR ZONE
C
      APINZI = SDAPPA(1:19)//'.INZI'
      CALL WKVECT(APINZI,'V V I',11*NBZONE,JPINZI)
      APINZR = SDAPPA(1:19)//'.INZR'
      CALL WKVECT(APINZR,'V V R',5*NBZONE,JPINZR)
C
