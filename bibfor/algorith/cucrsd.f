...
C
      IF (ILIAI.GT.1) THEN
        CALL UTMESS('F','CUCRSD','IL Y A PLUSIEURS '
     &              //'CHARGES CONTENANT DES LIAISONS UNILATERALES ')
      END IF
C
...
C
     ...
     &                CDDL)
          IF (CDDL.EQ.0) THEN
            CALL UTDEBM('F','CUCRSD',' COMPOSANTE INEXISTANTE SUR ')
 ...
 100  CONTINUE  
      IF ((IDDL-1).NE.NBGAU) THEN
        CALL UTMESS('F','CUCRSD','DEBORDEMENT TABLEAU (DVLP)')
      ENDIF
C
