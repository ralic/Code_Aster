      SUBROUTINE NMARCH(RESULT,NUMINS,PARTPS,FORCE ,COMPOR,
...
C
      ...
      INTEGER      NUMINS
      CHARACTER*8  RESULT
      CHARACTER*19 PARTPS,CRITNL,CNSINR,SDDYNA
...
C
      INTEGER      JINST,JCRR,JCRK,JPARA,JARCH
      INTEGER      IRET,K,IE,I15,I16,I19,II,IBID,LPARA(7)
      INTEGER      JDEPEN,JDEPP,JVITEN,JVITP,JACCEN,JACCP
      INTEGER      JIFL,JFFL,JCNSVR
      INTEGER      IFM,NIV,IINF,NIVMPI
      INTEGER      NBPROC,NBPB,NEQ,NUMARC,NBT
      REAL*8       INSTAP,R8BID
      CHARACTER*8  K8BID
      CHARACTER*24 DEPPLU,SIGPLU,VARPLU,VARDEP,LAGDEP,K24VAL
      CHARACTER*24 CHAMP,K24BID,INFOFE
      INTEGER      DIARCH
      REAL*8       DIINST
      LOGICAL      DIINCL,LFETI,NDYNLO
      CHARACTER*19 CNOINR
C
C ----------------------------------------------------------------------
C
...
C
      ...
      IF (IRET.GT.0) THEN
        ...
        ELSE
          NIVMPI=1
        ENDIF
        CALL FETMPI(3,IBID,IFM,NIVMPI,IBID,NBPROC,
     &              K24BID,K24BID,K24BID,R8BID)
...
C      DEPLACEMENTS
        IF (DIINCL(PARTPS,NUMINS,'DEPL',FORCE)) THEN
          CALL RSEXCH(RESULT,'DEPL',NUMARC,CHAMP,IRET)
          IF (IRET.LE.100) THEN
            CALL ASSDE2(DEPPLU)
...
C      CAS DES FORCE_FLUIDE
          ...
          END IF
C      VITESSES
          IF (DIINCL(PARTPS,NUMINS,'VITE',FORCE)) THEN
...
C      ACCELERATIONS
          IF (DIINCL(PARTPS,NUMINS,'ACCE',FORCE)) THEN
...
C      DEPLACEMENTS ABSOLUS
            IF (DIINCL(PARTPS,NUMINS,'DEPL_ABSOLU',FORCE)) THEN
...
C      VITESSES ABSOLUES
            IF (DIINCL(PARTPS,NUMINS,'VITE_ABSOLU',FORCE)) THEN
...
C      ACCELERATIONS ABSOLUES
            IF (DIINCL(PARTPS,NUMINS,'ACCE_ABSOLU',FORCE)) THEN
...
C      CONTRAINTES
        IF (DIINCL(PARTPS,NUMINS,'SIEF_ELGA',FORCE)) THEN
...
C SINON CELA FAUSSE LE NEWTON SUIVANT
              ...
              CALL FETMPI(71,NBPB,IFM,NIVMPI,IBID,IBID,K24VAL,K24BID,
     &                    K24BID,R8BID)
            ENDIF
...
C      VARIABLES INTERNES
        IF (DIINCL(PARTPS,NUMINS,'VARI_ELGA',FORCE)) THEN
...
C      VARIABLES NON LOCALES
        ...
        IF (IRET.NE.0  .AND.  
     &         DIINCL(PARTPS,NUMINS,'VARI_NON_LOCAL',FORCE))THEN
...
C      MULTIPLICATEURS DE LAGRANGE DES CHAMPS NON LOCAUX
        ...
        IF (IRET.NE.0 .AND. DIINCL(PARTPS,NUMINS,'LANL_ELGA',FORCE))THEN
