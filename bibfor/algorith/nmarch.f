     ...
     &                  SDCRIT,CARCRI,DEFICO,RESOCO,VALINC,INCR  ,
...
C
      ...
      CHARACTER*24 CHAMP,NDIMCO
      INTEGER      DIARCH,JDIM,IZONE
      REAL*8       DIINST,THETA,MMINFR
...
C
      ...
      LCONX  = ISFONC(FONACT,'CONT_XFEM')
      LFROX  = ISFONC(FONACT,'FROT_XFEM')
C
...
C
C
C --- SAUVEGARDE CONTACT COHESIF X-FEM
C
        IF (LCONX) THEN
C
...
C
          ...
          IZONE = ZI(JDIM+2-1)
          IF (MMINFR(DEFICO,'RELATION',IZONE).EQ.1.D0) THEN
...
C
C ARCHIVAGE DE THETA DANS TOUS LES CAS
C
        CALL DISMOI ( 'F', 'MODELISATION_THM', MODELE, 'MODELE',
     &              IBID, REPONS, IER )
        IF ((REPONS.EQ.'OUI').OR.(REPONS.EQ.'OUI_P')) THEN
