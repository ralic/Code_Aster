...
C
C --- MISE A JOUR CHAMPS INTERMEDIAIRES (ITERATION NEWTON)
C  
      CALL COPISD('CHAMP_GD','V',DEPPLU,DEPKM1)
      CALL COPISD('CHAMP_GD','V',VITPLU,VITKM1)
      CALL COPISD('CHAMP_GD','V',ACCPLU,ACCKM1)
      CALL COPISD('CHAMP_GD','V',ROMK  ,ROMKM1)             
C
C --- MISES A JOUR VITESSE ET ACCELERATION VITPLU/ACCPLU
C
      ICOMP  = 0
      DO 20 I = 1,NEQ
        ...
        ELSE IF (ZI(INDRO+I-1).EQ.1) THEN
          ICOMP         = ICOMP + 1
          IRAN(ICOMP)   = I
          THETA(ICOMP)  = ZR(JROMK+I-1)
          DELDET(ICOMP) = ZR(JDDEPL+I-1)
          IF (ICOMP.EQ.3) THEN
            ICOMP = 0          
            CALL NDGROT(SDDYNA,VALMOI,VALPLU,POUGD ,THETA ,
     &                  DELDET,IRAN  )
          ENDIF
        ELSE
          CALL ASSERT(.FALSE.)
