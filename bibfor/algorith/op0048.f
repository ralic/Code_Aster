...
C
      ...
      INTEGER IMAT(3), NUME, IAUX, IRET, NIV, IBID, IFM, IONDP, LADPA
      ...
      INTEGER LACCE, I,JORD,NBORD
      REAL*8 RVAL, T0
      CHARACTER*8   K8B, NOMRES, MASSE, RIGID, AMORT, BASENO
      CHARACTER*8   MATREI, MAPREI, MATERI, BLAN8, CARAEL
      CHARACTER*13  INPSCO
      ...
      CHARACTER*24  INFOCH, CRITER, VAPRIN,VACOMP
...
15       CONTINUE
      ...
      IF (NVECA.NE.0.AND.NCHAR.NE.0) THEN
         IF (NCHAR.NE.NONDP) CALL U2MESS('F','ALGORITH9_27')
      ENDIF
...
C  SAUVEGARDE DU CHAMP SOLUTION STANDARD DANS VAPRIN (DEPL)
C  SAUVEGARDE DU CHAMP SOLUTION STANDARD DANS VACOMP (ACCE)
C
      IF (NRPASE.EQ.0) THEN
        JSTD = 0
        JAUX = 4
  ...
  426   CONTINUE
        ...
        DO 427 IEQ = 1, NEQ
          ZR(LACCE-1 +IEQ) = ZR(IACCE0-1 +IEQ)
  427   CONTINUE
        CALL DLTP0(T0)
      END IF
...
C
     ...
     &                CHARGE,INFOCH,FOMULT,NUMEDD,NUME,INPSCO,NBPASE)
      ...
      DO 17 I=1,NBORD
        CALL RSADPA(NOMRES,'E',1,'MODELE',ZI(JORD+I-1),0,LADPA,K8B)
        ZK8(LADPA)=MODELE(1:8)
        CALL RSADPA(NOMRES,'E',1,'CHAMPMAT',ZI(JORD+I-1),0,LADPA,K8B)
        ZK8(LADPA)=MATERI
        CALL RSADPA(NOMRES,'E',1,'CARAELEM',ZI(JORD+I-1),0,LADPA,K8B)
        ZK8(LADPA)=CARAEL
 17   CONTINUE
