...
C             POUR REPÉRER L'ARETE DANS XAINT2
 ...
 150      CONTINUE
C
          IF (LAJPA) CALL XAJPIN(PINTER,PTMAX,IPT,INS,A,LONGAR,
     &                           AINTER,0,NA,0.D0)
          IF (LAJPB) CALL XAJPIN(PINTER,PTMAX,IPT,INS,B,LONGAR,
     &                           AINTER,0,NB,0.D0)
          IF (LAJPC) CALL XAJPIN(PINTER,PTMAX,IPT,IBID,C,LONGAR,
     &                           AINTER,NC,0,ALPHA)
        ENDIF
...
C     RECHERCHE SPECIFIQUE POUR LES ELEMENTS EN FOND DE FISSURE
      IF (ENR(2:2).EQ.'T'.OR.ENR(3:3).EQ.'T') THEN
        CALL XCFACF(PINTER,PTMAX,IPT,AINTER,LSN,LST,IGEOM,NNO,NDIM,
     &                                                    LLIN,TYPMA)
      ENDIF
      NINTER=IPT
