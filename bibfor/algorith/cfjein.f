...
C
      IF (LLAGRC) THEN
        IF (REAGEO) THEN
          DO 100 ILIAI = 1,NTNOE
            ZI(JLIOT+0*NTNOE-1+ILIAI) = 0
            ZI(JLIOT+1*NTNOE-1+ILIAI) = 0
            ZI(JLIOT+2*NTNOE-1+ILIAI) = 0
            ZI(JLIOT+3*NTNOE-1+ILIAI) = 0
 100      CONTINUE
          ZI(JLIOT+4*NTNOE  ) = 0
          ZI(JLIOT+4*NTNOE+1) = 0
          ZI(JLIOT+4*NTNOE+2) = 0
          ZI(JLIOT+4*NTNOE+3) = 0
        ENDIF
      ELSE
        ZI(JLIOT+4*NBLIAI  ) = 0
