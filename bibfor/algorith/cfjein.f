...
C MODIF ALGORITH  DATE 08/02/2010   AUTEUR TARDIEU N.TARDIEU 
C ======================================================================
...
C
      INTEGER      IFM,NIV
      INTEGER      NBLIAI,NEQ,NBDDL,NTNOE,NDIM
      INTEGER      ILIAI,IEQ,IALARM,BTOTAL,LLIAC
      INTEGER      JRESU,JLIAI,JDECAL
      CHARACTER*19 DELTA,DELT0,LIOT,LIAC ,CONVEC
      INTEGER      JDELTA,JDELT0,JLIOT,JLIAC,JVECC
      CHARACTER*24 ATMU,AFMU,RESU0,MU,APJEFX
      INTEGER      JATMU,JAFMU,JMU,JAPJFX
      CHARACTER*24 APPOIN,APDDL,APCOEF,APJEU,CLREAC
      INTEGER      JAPPTR,JAPDDL,JAPCOE,JAPJEU,JCLREA
...
C
      IF (LLAGRC.AND.LCTFD.AND.REAPRE) THEN
        ...
        DO 331 ILIAI = 1,NTNOE
          ZR(JMU+3*NTNOE+ILIAI-1) = 0.D0
          ZR(JMU+2*NTNOE+ILIAI-1) = 0.D0
          ZR(JMU+NTNOE+ILIAI-1)   = 0.D0
          ZR(JMU+ILIAI-1)         = 0.D0
...
C
      IF (LLAGRF.AND.(NDIM.EQ.3)) THEN
        VDIAGM = ZR(JMU+6*NTNOE-1)
        IF (VDIAGM.EQ.R8VIDE()) THEN
          CALL CFDIAG(LMAT  ,VDIAGM)
          ZR(JMU+6*NTNOE-1)  = VDIAGM ** 0.25D0
        ENDIF  
      ENDIF 
...
C  
      BTOTAL = NBLIAC + LLF + LLF1 + LLF2
      IF (LLAGRC.AND.(REAPRE.OR.REAGEO).AND.LLAGRF) THEN
        DO 7 JLIAI = 1, BTOTAL
