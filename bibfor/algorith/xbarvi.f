      SUBROUTINE XBARVI(CHAR,NOMA,NOMO,FISS,FACLON,AINTER)
      IMPLICIT NONE
      CHARACTER*8   CHAR,NOMA,NOMO,FISS
...
C
      ...
      INTEGER AR(12,3),I,IA,NBAR,NLOC(2),NGLO(2),NUNO(2),NEQ,IBID,NO
      ...
      LOGICAL   LMULTI,LCONNE,ANCIEN
      INTEGER   JXC,JINDIC,KK,JGRP,IENR,NMAENR
C
C ----------------------------------------------------------------------
C
      CALL JEMARQ()
...
C
     ...
     &                 NCTA,K8BID)
      ...
      ENDIF
C
C --- LE CONTACT EST-IL SELON L'ANCIENNE FORMULATION (AUX ARETES) ?
C
...
C   
      ...
      CALL JEVEUO(AINTER//'.CESL','L',JCSL2)
...
C
      IF (ANCIEN.OR.(.NOT.LCONNE)) GOTO 41
C
C --- BOUCLE SUR LES GRP
C
      DO 81 KK = 1,3
        IF (ZI(JINDIC-1+2*(KK-1)+1).NE.1) GOTO 81
...
C
      DO 80 KK = 1,3
        IF (ZI(JINDIC-1+2*(KK-1)+1).NE.1) GOTO 80
        CALL JEVEUO(GRP(KK),'L',JGRP)
        NMAENR = ZI(JINDIC-1+2*KK)
C
C --- BOUCLE SUR LES MAILLES DU GROUPE
C
        DO 10 IENR = 1,NMAENR
          ...
          IF (ANCIEN) THEN
C
C --- SI C L'ANCIENNE FORMULATION, TOUTES LES ARETES SONT VITALES
C
            DO 5 PIN = 1,NINTER
              CALL CESEXI('S',JCSD2,JCSL2,IMA,1,1,ZXAIN*(PIN-1)+5,IAD)
              ZR(JCSV2-1+IAD)=1     
 5          CONTINUE        
            GOTO 10
          ENDIF
