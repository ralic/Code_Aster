      SUBROUTINE ECRELT(MAXNOD,NBTYMA,NOMAIL,NBMAIL,MINT,MANT,
     &                  LIMAIL,NBMTOT)
...
C
      ...
      INTEGER NEU2(32),INUM,ITYP,IPROP,IMAT,ICOL,NEUL
      INTEGER L
C
...
C
             DO 12 IN = 1,NEUL
                ...
                CALL CODNOP(CHTAB(IN),PRFNOE,1,2)
                CALL CODENT(NEU2(IN),'G',CHTAB(IN)(3:8))
   12        CONTINUE
C
             IDIV=INT(NEUL/8)
             IREST=MOD(NEUL,8)
