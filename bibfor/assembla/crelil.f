      SUBROUTINE CRELIL(NBMAT,ILIMAT,LILI,BASE,NOMMA,PREF,GD,
...
C     -------------------------------
      DO 100 IMAT = 1,NBMAT
         ...
         IF (IRET1.EQ.0)
     + CALL UTMESS('F','CRELIL','MESSAGE VIDE    ')
...
C    +    .OR.((SUROPT.NE.SUROPS).AND.(SUROPS.NE.' '))) THEN
             CALL UTMESS('F','CRELIL','INCOHERENCE DES MATR_ELEM')
         END IF
         MODELS= MODELE
         SUROPS= SUROPT
C
...
C
         ...
         ELSE
           IF (EXISS1(1:3).EQ.'NON') CALL UTMESS('F','CRELIL',
...
C
      IF (NLILI.EQ.1) THEN
         CALL UTMESS('F','CRELIL',' -  AUCUN LIGREL DANS LES RESUELEM ')
         GOTO 9999
      END IF
101   CONTINUE
C
...
C
         IF (NOMLI(9:15).EQ.'.MODELE') THEN
            ...
            IF (NBMO.GT.1) CALL UTMESS('F','CRELIL','MODELES DIFF.')
         END IF
