...
C-----------------------------------------------------------------------
      ...
      LOGICAL CUMUL,ACREER,LFETI
      CHARACTER*1 BASE1,TYPSCA,K1BID
      CHARACTER*8 MATEL,K8BID,NOGDCO,NOGDSI,NOMSD,MA,EXIELE,MA2
      CHARACTER*8 MO,MO2,NOMACR
      CHARACTER*11 K11B
      CHARACTER*14 NUDEV,K14B,NUM2
      CHARACTER*19 MATDEV,K19B
      CHARACTER*24 K24PRN,KNULIL,KMALIL,KMAREF,RESU,NOMLI,KHCOL
      CHARACTER*24 KADIA,KVALE,KDESC,KTMP1,KTMP2,KCONL
      CHARACTER*24 METHOD,SDFETI,K24B,SDFETS
      INTEGER NMALIL,IMO,ILAGR,ILIMO,NBNO,EPDMS,JPDMS,NBEC
...
C SI FETI ET DOMAINE GLOBAL ON ENREGISTRE QUE LE .REFA
        IF ((LFETI) .AND. (IDD.EQ.0)) GO TO 310
...
C MONITORING
                    IF ((NIV.GE.5) .AND. (LFETI)) WRITE (IFM,
     &                  *) '<FETI/ASSMAM>',IDD,ILIMA,IGR,IEL,NUMA,
     &                  ZL(ILOGI+NUMA-1)
...
C QUESTION DE L'APPARTENANCE DE LA MAILLE NUMA AU SOUS-DOMAINE IDD
                    IF (LFETI) THEN
                      IF (IDD.GT.0) THEN
                        IF (.NOT.ZL(ILOGI+NUMA-1)) GO TO 250
                      END IF
                    END IF
...
C---- LIGREL TARDIF:
C-------------------
                      IF (LFETI) CALL UTMESS('F','ASSMAM',
     &                                       'LE RESUELEM '//RESU//
     &                                      ' CONTIENT UN LIGREL TARDIF'
     &                                       //
...
C MONITORING
  310   CONTINUE
     ...
     &      KMAREF(1:19)
          WRITE (IFM,*)
          WRITE (IFM,1000) COEF
          IF ((NIV.GE.4) .AND. (IDD.NE.0)) CALL UTIMSD(IFM,2,.FALSE.,
     &        .TRUE.,KMAREF(1:19),1,' ')
          IF ((NIV.GE.4) .AND. (IDD.EQ.NBSD)) CALL UTIMSD(IFM,2,.FALSE.,
     &        .TRUE.,MATDEV(1:19),1,' ')
        END IF
