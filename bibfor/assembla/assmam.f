...
C IN  I  NBMAT  : NOMBRE DE MAT_ELEM  DE LA LISTE TLIMAT
C IN  K* TLIMAT : LISTE DES MAT_ELEM
...
C----------------------------------------------------------------------
      ...
      NUDEV = NU
      K19VID='                   '
      IF (DBG) CALL CHEKSD(NU,'sd_nume_ddl',IRET)
...
C---- RECOPIE DE LA LISTE DES MAT_ELE DANS 1 OBJET JEVEUX 
...
C                           WRITE(IFM,*)'ASSMAM, SAUTE LA MAILLE ',NUMA
                            GOTO 300
                          ENDIF
                        ELSE
                          IF (RANG.NE.0) THEN
...
C POUR MONITORING
C      CALL UTIMSD(IFM,2,.FALSE.,.TRUE.,MATDEV,1,' ')
