...
C-----------------------------------------------------------------------
      ...
      REAL*8 C1,C2,TEMPS(6)

      LOGICAL ACREER,CUMUL,DBG,IDDOK,LBID,LFETI,LFETIC,ELLAGR
...
C         -------------------------------------
          CALL WKVECT(MAT19//'.REFA',BASE1//' V K24',10,JREFA)
          ZK24(JREFA-1+1)=MA
          ZK24(JREFA-1+2)=NU14
          ZK24(JREFA-1+8)='ASSE'
          IF (LMASYM) THEN
...
C         -- MISE A L'ECHELLE DES COEF. DE LAGRANGE SI NECESSAIRE :
          IF (ELLAGR.AND.(.NOT.(LFETI.AND.IDD.EQ.0))) THEN
            CALL ASSMA1(MAT19)
          ENDIF
