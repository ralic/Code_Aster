...
C-----------------------------------------------------------------------
      ...
      CHARACTER*8  MATEL,K8BID,NOGDCO,NOGDSI,MA,EXIELE,MA2,MO,MO2,
     &             NOMACR
      CHARACTER*11 K11B
...
C --- NUME_DDL ET DONC MATR_ASSE ETENDU, OUI OU NON ?
     ...
     &    'POUR FETI ',NUDEV(1:14)//'.NUME.REFN'
      ...
      IF (METHOD(1:4).EQ.'FETI') THEN
        ...
        IF (.NOT.ACREER)
     &    CALL JEVEUO(MATDEV//'.FETM','L',IFETM)
...
C     (IREEL CONTIENT LE NOMBRE DE REELS A TRAITER)
     ...
     &                            IDRESL+NCMPEL* (IEL-1),NBLC,KVALM,R,1,
     &                            JVAL1)
                    ELSE IF (TYPE.EQ.2) THEN
                      IF (TYPSCA.EQ.'R') THEN
...
C MONITORING
  310   CONTINUE
     ...
     &    CALL UTIMSD(IFM,2,.FALSE.,.TRUE.,MATDEV,1,' ')
