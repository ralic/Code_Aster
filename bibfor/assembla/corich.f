...
C MODIF ASSEMBLA  DATE 06/09/2000   AUTEUR VABHHTS J.PELLET 
C ======================================================================
...
C     ------------------------------------------------------
      IF (ACTION.EQ.'E') THEN
        IF (ICHIN.EQ.0) CALL UTMESS('F','CORICH','ICHIN = 0 ')
        IF (ICHIN.LT.-2) CALL UTMESS('F','CORICH','ICHIN < -2 ')
        NOMCH = CHAMP(1:19)
...
C     ------------------------------------------------------
      ELSE IF (ACTION.EQ.'S') THEN
        ...
        CALL JENONU(JEXNOM(REPERT,NOMCH),KK)
        IF (KK.EQ.0) THEN
          CALL UTMESS('F','CORICH','S => ICHIN=/0 ')
        ELSE
          CALL JEVEUO(NUMICH,'E',JNUMIC)
          ZI(JNUMIC-1+KK) = 0
        END IF
...
C     ------------------------------------------------------
      ELSE
        CALL UTMESS('F','CORICH','ACTION:E/L/S ')
      END IF
