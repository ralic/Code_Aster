      SUBROUTINE NUEFFE(LLIGR,BASE,NUZ,RENUM,TYPREZ,MOLOC)
      IMPLICIT NONE
...
C----------------------------------------------------------------------
     ...
     &             KNEQUA,DERLI,NUM2,DSCLAG,EXI1,NEWN,OLDN
      ...
      INTEGER IVSUIV,J,J1,JJ,JNULAG,JPRNO,K,KK,L,L1,L2,LONG,N0
      INTEGER N0RE,N1,N1M1RE,N1RE,N2,N21,N3,NBCMP,NBN,NBNL
      INTEGER NBNOM,NBNONU,NBNORE,NCMP,NCOEF,NDDL1,NDDLB
      INTEGER NDDLK,NDDLKK,NEL,NEQUA,NIEL,NIGREL,NIV,NLAG,NMA,NN
...
C     =================================
      ...
      DO 220 I = 1,N
        ...
        IF (ILI.GT.0) THEN
          ...
          IF (NDDL1.EQ.0) THEN
            NDDL1 = NDDL(ILI,NUNOEL,NEC,NCMP,IDPRN1,IDPRN2)
            ZI(IZZPRN(ILI,NUNOEL,2)) = NDDL1
          END IF
          ZI(IZZPRN(ILI,NUNOEL,1)) = IAD
          IAD = IAD + NDDL1
          NCOEF = NCOEF + (NDDL1* (NDDL1+1))/2
        END IF
  220 CONTINUE
...
C       ----------------------------------------------------------------
  ...
  230   CONTINUE
     ...
     &    ' DONT : ',NLAG,' NOEUDS "LAGRANGE"'
      ...
      CALL WKVECT(NU//'.NUME.REFN',BASE//' V K24',2,IDREF)
      ZK24(IDREF) = MAILLA
      ZK24(IDREF+1) = NOMGDS

C ---  CALCUL DU NOMBRE DE COEFFICIENTS :
C       -------------------------------
      DO 300 ILI = 2,NLILI
        NIGREL = ZZNGEL(ILI)
        DO 290 IGREL = 1,NIGREL
          NIEL = ZZNELG(ILI,IGREL)
          DO 280 IEL = 1,NIEL
            IMAIL = ZZLIEL(ILI,IGREL,IEL)

            IF (IMAIL.GT.0) THEN
              NNOE = ZZNBNE(IMAIL)
              DO 250 J = 1,NNOE
                K = ZZCONX(IMAIL,J)
                NDDLK = ZZPRNO(ILIM,K,2)
                DO 240 JJ = J + 1,NNOE
                  KK = ZZCONX(IMAIL,JJ)
                  NDDLKK = ZZPRNO(ILIM,KK,2)
                  NCOEF = NCOEF + NDDLK*NDDLKK
  240           CONTINUE
  250         CONTINUE

            ELSE
              IMAIL = -IMAIL
              NNOE = ZZNSUP(ILI,IMAIL)

              DO 270 J = 1,NNOE
                K = ZZNEMA(ILI,IMAIL,J)
                IF (K.LT.0) THEN
                  K = -K
                  NDDLK = ZZPRNO(ILI,K,2)
                ELSE
                  NDDLK = ZZPRNO(ILIM,K,2)
                END IF
                DO 260 JJ = J + 1,NNOE
                  KK = ZZNEMA(ILI,IMAIL,JJ)
                  IF (KK.LT.0) THEN
                    KK = -KK
                    NDDLKK = ZZPRNO(ILI,KK,2)
                  ELSE
                    NDDLKK = ZZPRNO(ILIM,KK,2)
                  END IF
                  NCOEF = NCOEF + NDDLKK*NDDLK
  260           CONTINUE
  270         CONTINUE
