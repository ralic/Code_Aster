...
C---- RECUPERATION DES MATRICES ELEMENTAIRES ---
      CALL GETVID(' ','MATR_ELEM',0,IARG,0,KBIDON,NBMAT)
      NBMAT = -NBMAT
      LMATEL='&&OP0012.LMATEL'
      CALL WKVECT(LMATEL,'V V K24',NBMAT,JLIMAT)
      CALL GETVID(' ','MATR_ELEM',0,IARG,NBMAT,ZK24(JLIMAT),IBID)


C---- RECUPERATION DES CHARGES CINEMATIQUES ---
      LCHCI='&&OP0012.LCHARCINE'
      CALL GETVID(' ','CHAR_CINE',0,IARG,0,KBIDON,NBCHC)
      NBCHC = -NBCHC
C     -- LES SD_CHAR_XXX PEUVENT CONTENIR UNE SD_CHAR_CINE :
 ...
 1    CONTINUE
      ...
      IF (NBCHC.GT.0) THEN
        ...
        CALL GETVID(' ','CHAR_CINE',0,IARG,NBCHC,ZK24(JLCHCI),ICO)
        DO 3, K=1,NBMAT
          MATEL=ZK24(JLIMAT-1+K)
          CALL JEEXIN(MATEL//'.RECC',IEXI)
          IF (IEXI.GT.0) THEN
...
C---- ASSEMBLAGE PROPREMENT DIT
      SYME = ' '
      CALL GETVTX(' ','SYME',1,IARG,1,SYME,IBID)
      IF (SYME.EQ.'OUI') THEN
