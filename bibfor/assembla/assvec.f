...
C MODIF ASSEMBLA  DATE 12/02/2013   AUTEUR SELLENET N.SELLENET 
C ======================================================================
...
C ---------------------------------------------------------------------
      ...
      CHARACTER*19 K19B,VECAS,VPROF,VECEL,A19,B19,C19,RESU
      CHARACTER*24 METHOD,SDFETI,K24B,SDFETS,KMAILA,K24PRN,KNUEQ,KNULIL,
     &             KVELIL,KVEREF,KVEDSC,NOMLI,KVALE,NOMLOG,NOMLID,
     &             INFOFE,SDFETA
...
C           * LDGREL: .TRUE. : LA PARTITION EST DE TYPE 'GROUP_ELEM'
...
C       -- ICODLA EST L'ENTIER CODE CORRESPONDANT A LA CMP "LAGR"
      ...
      IF (LFETI) THEN
C       -- ADRESSE JEVEUX DE LA LISTE DES NUME_DDL ASSOCIES
C          AUX SOUS-DOMAINES
...
C       ==========================
          DO 240 IMAT=1,NBVEC
            ...
            CALL DISMOI('F','NOM_MODELE',VECEL,'VECT_ELEM',IBID,MO2,
     &                  IERD)
...
C         -----------------------------------
     ...
     &                  IERD)
            IF (NBSSA.GT.0) THEN
              NOMCAS=' '
              CALL DISMOI('F','NB_SM_MAILLA',MO,'MODELE',NBSMA,KBID,
     &                    IERD)
              CALL DISMOI('F','NOM_MAILLA',MO,'MODELE',IBID,MA,IERD)
              CALL JEVEUO(MO//'.MODELE    .SSSA','L',IASSSA)
              CALL SSVALV('DEBUT',NOMCAS,MO,MA,0,JRESL,NCMPEL)
...
C             -- ON N'ASSEMBLE QUE LES SSS VRAIMENT ACTIVES :
     ...
     &                        KBID)
                  CALL SSVALV(' ',NOMCAS,MO,MA,IMA,JRESL,NCMPEL)
                  NOMACR=ZK8(IANMCR-1+IMA)
                  CALL DISMOI('F','NOM_NUME_DDL',NOMACR,
     &                        'MACR_ELEM_STAT',IBID,NUM2,IERD)
   ...
   50                 CONTINUE
     ...
     &                    I1)-1))+ZC(JRESL+IL-1)*RCOEF
   60                 CONTINUE
                    ENDIF
   70             CONTINUE
   80           CONTINUE
   90         CONTINUE
              CALL SSVALV('FIN',NOMCAS,MO,MA,0,JRESL,NCMPEL)
            ENDIF
...
C ELLE APPARTIENT AU GREL IGR DU LIGREL TARDIF ILIMA
     ...
     &                                ILIMNU,MODE,NEC,NCMP,N1,K1,NDDL1,
     &                                ZI(IAPSDL))
                          ...
                          IF (IAD1.EQ.0) THEN
                            ...
                            VALK(2)=VECEL
                            VALK(3)=NUDEV
                            CALL U2MESG('F','ASSEMBLA_41',3,VALK,1,VALI,
     &                                  0,0.D0)
                          ENDIF
                          ...
                          IF (IAD1.GT.NEQUA) THEN
                            ...
                            VALK(1)=RESU
                            VALK(2)=VECEL
                            CALL U2MESG('F','ASSEMBLA_42',2,VALK,3,VALI,
     &                                  0,0.D0)
                          ENDIF
                          ...
                          IF (NDDL1.GT.100) THEN
                            VALI(1)=NDDL1
                            VALI(2)=100
                            CALL U2MESG('F','ASSEMBLA_43',0,' ',2,VALI,
     &                                  0,0.D0)
                          ENDIF
                          ...
                          IF (TYPE.EQ.1) THEN
                            DO 110 I1=1,NDDL1
                              IL=IL+1
                              ZR(JVALE-1+ZI(IANUEQ-1+IAD1+ZI(IAPSDL-1+
     &                          I1)-1))=ZR(JVALE-1+
     &                                  ZI(IANUEQ-1+IAD1+ZI(IAPSDL-1+
     &                                  I1)-1))+ZR(JRESL+(IEL-1)*NCMPEL+
     &                                  IL-1)*R
  110                       CONTINUE
                          ...
                          ELSE
                            DO 120 I1=1,NDDL1
                              IL=IL+1
                              ZC(JVALE-1+ZI(IANUEQ-1+IAD1+ZI(IAPSDL-1+
     &                          I1)-1))=ZC(JVALE-1+
     &                                  ZI(IANUEQ-1+IAD1+ZI(IAPSDL-1+
     &                                  I1)-1))+ZC(JRESL+(IEL-1)*NCMPEL+
     &                                  IL-1)*R
  120                       CONTINUE
                          ENDIF
...
C                     -- N1 : NBRE DE NOEUDS DE LA MAILLE NUMA
     ...
     &                     ZI(ZI(IADNEM+3*(ILIVE-1)+2)+NUMA-1)-1
                        IF (NNOE.NE.N1) THEN
                          ...
                          VALI(3)=N1
                          VALI(4)=NNOE
                          CALL U2MESG('F','ASSEMBLA_44',3,VALK,4,VALI,0,
     &                                0.D0)
                        ENDIF
                        IL=0
                        DO 200 K1=1,NNOE
...
C NOEUD PHYSIQUE
     ...
     &                                    VALI,0,0.D0)
                          ...
                          IF (IAD1.EQ.0) THEN
                            ...
                            VALK(2)=VECEL
                            VALK(3)=NUDEV
                            CALL U2MESG('F','ASSEMBLA_48',3,VALK,1,VALI,
     &                                  0,0.D0)
                          ENDIF
                          IF (IAD1.GT.NEQUA) THEN
                            VALI(1)=N1
                            ...
                            VALK(1)=RESU
                            VALK(2)=VECEL
                            CALL U2MESG('F','ASSEMBLA_49',2,VALK,3,VALI,
     &                                  0,0.D0)
                          ENDIF
                          IF (TYPE.EQ.1) THEN
                            DO 180 I1=1,NDDL1
