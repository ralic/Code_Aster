...
!-----------------------------------------------------------------------
    ...
    integer :: neq, nbnoxfem, size_vect_col, size_vect_raw, nbnomax, maxi_ddl, size_smhc, deca
    ...
    real(kind=8) :: vect_col(size_vect_col), vect_raw(size_vect_raw)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    do jcoll=1,neq
       nunoj=ino_xfem(zi(jdeeq-1+2*(jcoll-1)+1))
...
92        enddo
! NE PAS OUBLIER LE COEFFICIENT DIAGONAL
          do ij=1,ieq_loc(jcoll)
             incr=incr+1
             zi4(jsmhc-1+iadr+incr)=iglob_ddl(maxi_ddl*(nunoj-1)+ij)
             zr(jvale-1+iadr+incr)=tab_mloc(deca*(nunoj-1)+ neq_mloc(nunoj)*(ij-1)+ieq_loc(jcoll))
          enddo
!
       elseif ( is_connec(jcoll) .and. ieq_loc(jcoll) .eq. 0 ) then
          if (jcoll .gt. 1) then
