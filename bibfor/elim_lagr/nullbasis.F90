...
!
  real(kind=8) :: eps
  integer :: ifm, niv
  aster_logical :: debug, is_ok
  mpi_int :: mpicomm
  Vec :: vec_c, c_elim, c_free, cz_elim
  Mat :: z_elim_elim
  IS :: is_elim, is_free
...
! Initialisation de la matrice mat_z à l'identité
...
!
    ...
    ASSERT( ierr == 0 )
    allocate(idelim_c(nc), stat = ierr )
    ASSERT( ierr == 0 )
...
! stockée dans une matrice dense.
           call elg_nllspc( to_aster_int(nnz), valnorm(1:nnz), &
                            nullbasis_of_vec(1:nnz, 1:nnz) )

! On met à zéro les termes diagonaux
           do ii = 1, nnz
...
! une contrainte déjà éliminée.
  ...
  enddo
!
  call ISCreateGeneral(mpicomm, nelim, idelim_c(1:nelim), &
