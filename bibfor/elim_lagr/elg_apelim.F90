...
!--------------------------------!
    ...
    call elg_remplt(c, t, nbeq, clag1, nbnvco, nonu)
...
!       calcul de indred : tableau fortran stocké dans le common
    ...
    allocate(vrow(nbeq))
    do ilig = 1, nbeq
        call MatGetRow(melim(ke)%tfinal, ilig-1, nterm, irow(1), vrow(1), ierr)
        if (nterm .gt. 0) then
            do kterm = 1, nterm
...
!   -----------------------------------------
    call MatGetSubMatrix(ap(kptsc), isphys, isphys, MAT_INITIAL_MATRIX, melim(ke)%matb, ierr)


!   -- Projection T'*(MatB*T) :
!   -----------------------------
    call MatPtAP(melim(ke)%matb, melim(ke)%tfinal, MAT_INITIAL_MATRIX, 1.d0, melim(ke)%kproj, ierr)
