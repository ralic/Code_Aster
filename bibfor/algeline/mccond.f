      SUBROUTINE MCCOND (  LMAT, NEQ2, CVECT, NVECT )
...
C
C     (DE)-CONDITIONNEMENT DU VECTEUR CVECT
      ...
      IF ( IER  .NE.  0 ) THEN
         ...
         IF ( TYPE(ZI(LMAT+3)) .EQ. 'R' ) THEN
            DO 10 IVE = 1, NVECT
               ...
               II = 1
               DO 12 IEQ = 1, NEQC
                  REALVE = DBLE(CVECT(IND+IEQ))*ZR(LCOND+II)
                  RIMAGV = DIMAG(CVECT(IND+IEQ))*ZR(LCOND+II+1)
 ...
 10         CONTINUE
         ELSEIF ( TYPE(ZI(LMAT+3)) .EQ. 'C' ) THEN
            DO 20 IVE = 1, NVECT
               IND = NEQC*(IVE-1)
               DO 22 IEQ = 1, NEQ
                 CVECT(IND + IEQ) = CVECT(IND + IEQ) * ZC(LCOND+IEQ)
 22            CONTINUE
 20         CONTINUE
         ENDIF
      ENDIF
