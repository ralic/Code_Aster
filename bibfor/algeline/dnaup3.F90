...
!
!  NSTA   INTEGER (INPUT) NOMBRE DE DDL STAB
!
!  DDLSTA INTEGER ARRAY OF LENGTH N (INPUT). POSITION DES DDL_STAB
!
!  VSTAB  REAL*8 ARRAY OF LENGTH N (INPUT/OUTPUT). MODE STABILITE
!
!  CSTA   REAL*8 (OUTPUT). VALEUR CRITERE DE STABILITE
!
!  LDYNFA INTEGER (INPUT). DESCRIPTEUR DE L'OPERATEUR TANGENT
!
!  DDLEXC INTEGER ARRAY OF LENGTH N (INPUT). POSITION DDLS IMPOSES
!
!  REDEM  INTEGER (INPUT). NOMBRE DE REDEMARRAGES METHODE SORENSEN
!
! ENDDOC
!-----------------------------------------------------------------------
! BEGINLIB
...
!
    ...
    integer :: ido, info, ishift, ldh, ldq, ldv, mxiter, nsta, n, nev, np
    integer :: neqact
