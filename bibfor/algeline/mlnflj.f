...
C ======================================================================
      ...
      REAL*8     TRAVL(P,NB,*),TRAVU(P,NB,*)
      REAL*8  CL(NB,NB, *),CU(NB,NB, *),ALPHA,BETA
      TRA='N'
      TRB='N'
      ALPHA=-1.D0
      BETA=0.D0
...
C$OMP+SHARED(N,M,P,NMB,NBL,NLB,NB,RESTM,RESTL,CL,CU,TRA,TRB,ALPHA,BETA)
...
C
         ...
         DO 500 IB = KB,NLB
            ...
            CALL DGEMM( TRA,TRB,NB,NB,NBL,ALPHA,FRONTL(IA),N,
     &                  TRAVU(IT,1,NUMPRO), P,BETA,CL(1,1,NUMPRO), NB)
            CALL DGEMM( TRA,TRB,NB,NB,NBL,ALPHA,FRONTU(IA),N,
     &                  TRAVL(IT,1,NUMPRO), P,BETA,CU(1,1,NUMPRO), NB)
C     RECOPIE
...
C              IND = ADPER(K +I1) - DECAL  + NB*(IB-KB-1) +NB - I1
 ...
 500     CONTINUE
         IF(RESTL.GT.0) THEN
            IB = NLB + 1
            IA =   N*(IT-1)  +K + NB*(IB-KB)
            CALL DGEMM( TRA,TRB,RESTL,NB,NBL,ALPHA,FRONTL(IA),N,
     &                  TRAVU(IT,1,NUMPRO),P,BETA,CL(1,1,NUMPRO), NB)
            CALL DGEMM( TRA,TRB,RESTL,NB,NBL,ALPHA,FRONTU(IA),N,
     &                  TRAVL(IT,1,NUMPRO),P,BETA,CU(1,1,NUMPRO), NB)
C           RECOPIE
...
C
         ...
         DO 2500 IB = KB,NLB
            IA = N*(IT-1)  + K + NB*(IB-KB)
            CALL DGEMM( TRA,TRB,NB,NB,NBL,ALPHA,FRONTL(IA),N,
     &                  TRAVU(IT,1,1), P,BETA,CL(1,1,1), NB)
            CALL DGEMM( TRA,TRB,NB,NB,NBL,ALPHA,FRONTU(IA),N,
     &                  TRAVL(IT,1,1), P,BETA,CU(1,1,1), NB)
C     RECOPIE
...
C
         DO 600 IB = KB,NLB
            IA =   N*(IT-1 ) + K + NB*(IB-KB)
            CALL DGEMM( TRA,TRB,NB,RESTM,NBL,ALPHA,FRONTL(IA),N,
     &                  TRAVU(IT,1,1), P,BETA,CL(1,1,1), NB)
            CALL DGEMM( TRA,TRB,NB,RESTM,NBL,ALPHA,FRONTU(IA),N,
     &                  TRAVL(IT,1,1), P,BETA,CU(1,1,1), NB)
C     RECOPIE
...
C     IND = ADPER(K +I1) - DECAL  + NB*(IB-KB-1) +NB - I1
 ...
 600     CONTINUE
         IF(RESTL.GT.0) THEN
            IB = NLB + 1
            IA =   N*(IT-1) + K + NB*(IB-KB)
            CALL DGEMM( TRA,TRB,RESTL,RESTM,NBL,ALPHA,FRONTL(IA),N,
     &                  TRAVU(IT,1,1), P,BETA,CL(1,1,1), NB)
            CALL DGEMM( TRA,TRB,RESTL,RESTM,NBL,ALPHA,FRONTU(IA),N,
     &                  TRAVL(IT,1,1), P,BETA,CU(1,1,1), NB)
C     RECOPIE
