...
C         CREATION D UN .DESC POUR LA MATRASS GENE RESULTAT
          ...
          IF(NBOCC.NE.0) THEN
            CALL GETVID(COMBRC,'MATR_ASSE',1,IARG,1,MATRI1,L)
...
1             CONTINUE
      ...
      DO 10 IOCC = 0,NBOCC - 1
         CALL GETVID(COMBRC,'MATR_ASSE',IOCC+1,IARG,1,ZK8(LNOM+IOCC),L)
C        ON RECHERCHE UNE EVENTUELLE MATRICE NON SYMETRIQUE
   ...
   10 CONTINUE
      ...
      NOMDDL=' '
      CALL GETVTX(' ','SANS_CMP',1,IARG,1,NOMDDL,IBID)
...
C       REMARQUE : POUR PARTIE='IMAG', ON FAIT COEF=-J*COEF
      ...
      DO 25 IOCC = 0,NBOCC - 1
         CALL GETVR8(COMBRC,'COEF_R',IOCC+1,IARG,1,R8VAL,LR)
         IF (LR.EQ.1) THEN
           LCOEFC=.FALSE.
           IF (COMBRC.EQ.'COMB_R') THEN
             PARTIE=' '
             CALL GETVTX(COMBRC,'PARTIE',IOCC+1,IARG,1,PARTIE,IBID)
           ...
           ENDIF
         ELSE
           CALL GETVC8(COMBRC,'COEF_C',IOCC+1,IARG,1,CVAL,LC)
           CALL ASSERT(LC.EQ.1)
