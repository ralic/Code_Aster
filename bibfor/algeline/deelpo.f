...
C MODIF ALGELINE  DATE 26/11/97   AUTEUR D6BHHBQ B.QUINNEZ 
C ======================================================================
...
C
      ...
      IF (IRET.EQ.0) CALL UTMESS('F','DEELPO','LA CARTE DES '//
     &  'CARACTERISTIQUES GEOMETRIQUES DES ELEMENTS DE POUTRE '//
     &  'N EXISTE PAS')
...
C
  ...
  40  CONTINUE
      IF (IASBON.EQ.0) THEN
        ...
        CALL UTMESS('F','DEELPO','CARACTERISTIQUES GEOMETRIQUES '//
     &    'ELEMENTAIRES DE POUTRE NON DEFINIES POUR LA MAILLE '//
     &     NOMAIL)
      ENDIF
C
C
...
C
  ...
  50  CONTINUE
      IF (IRANG1.EQ.0 .OR. IRANG2.EQ.0) CALL UTMESS('F','DEELPO',
     &  'L UNE OU L AUTRE DES COMPOSANTES <R1> ET <R2> N EXISTE PAS '//
     &  'DANS LE CHAMP DE LA GRANDEUR')
C
  ...
  62  CONTINUE
      IF (IRANV1.EQ.0 .OR. IRANV2.EQ.0) CALL UTMESS('F','DEELPO',
     &  'LA SECTION DE L ELEMENT DE POUTRE CONSIDERE N EST PAS '//
     &  'CIRCULAIRE')
C
      R1 = ZR(ICAV+NBCMP*(IASBON-1)+IRANV1-1)
      R2 = ZR(ICAV+NBCMP*(IASBON-1)+IRANV2-1)
      IF (R1.EQ.0.D0 .OR. R2.EQ.0.D0) CALL UTMESS('F','DEELPO',
     &  'RAYON EXTERIEUR NUL A L UNE OU L AUTRE DES EXTREMITES DE '//
     &  'L ELEMENT CONSIDERE')
      TOLR = R8PREM()
      DIFR = DBLE(ABS(R1-R2))
      IF (DIFR.GT.R1*TOLR) CALL UTMESS('F','DEELPO','LE RAYON '//
     &  'EXTERIEUR N EST PAS CONSTANT SUR L ELEMENT CONSIDERE')
C
      PHIE = 2.D0*R1
C
