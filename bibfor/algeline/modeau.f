...
C MODIF ALGELINE  DATE 16/05/2000   AUTEUR KXBADNG T.KESTENS 
C ======================================================================
...
C
     ...
     &            ZR(IWRK2),30,IER,NITQR)
      IF (IER.NE.0) CALL UTMESS('F','MODEAU','NON CONVERGENCE POUR '//
     &                          'LE CALCUL DES MODES EN EAU AU REPOS')
C
      DO 30 IMOD = 1,NBM
        FRE = DBLE(ABS(ZR(IVECW+2*(IMOD-1))))
        FIM = DBLE(ABS(ZR(IVECW+2*(IMOD-1)+1)))
        IF (FIM.GT.(TOLE*FRE)) CALL UTMESS('F','MODEAU','L UNE DES '//
     &    'VALEURS PROPRES DE LA MATRICE DU PROBLEME MODAL '//
     &    'GENERALISE EN EAU AU REPOS N EST PAS REELLE')
...
C
  ...
  41      CONTINUE
          ...
          IF (RMAX.EQ.0.D0) CALL UTMESS('F','MODEAU','CALCUL DES '//
     &      'MODES EN EAU AU REPOS : UNE DES VALEURS PROPRES '//
     &      'OBTENUES EST NULLE')
          DO 42 KMOD = 1,NBM
...
C-------MASSES MODALES
        ...
        MASG(IMOD) = R8DOT(NBM,VECPR(1,IMOD),1,ZR(IVEC),1)
C-------FACTEURS DE PARTICIPATION
        FACT(3*(IMOD-1)+1) = R8DOT(NBM,ZR(IVEC),1,ZR(IFACT),1)
        FACT(3*(IMOD-1)+2) = R8DOT(NBM,ZR(IVEC),1,ZR(IFACT+NBM),1)
        FACT(3*(IMOD-1)+3) = R8DOT(NBM,ZR(IVEC),1,ZR(IFACT+2*NBM),1)
C-------MASSES MODALES AJOUTEES PAR LE FLUIDE
        CALL PMAVEC('ZERO',NBM,ZR(IMAT2),VECPR(1,IMOD),ZR(IVEC))
        MAJ(IMOD) = R8DOT(NBM,VECPR(1,IMOD),1,ZR(IVEC),1)
