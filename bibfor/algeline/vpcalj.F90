...
!
    call vplecs(eigsol,&
                itemax, ibid, ibid, ibid, ibid, ibid, nbvect, ibid, nfreq, nperm,&
                rbid, omecor, rbid, rbid, precdc, rbid, rbid, rbid, rbid, tol, toldyn,&
                rbid,&
                k1bid, k8bid, method, k9bid, k14bid, k14bid, k14bid,k16bid,optiof,k16bid,k16bid,&
                k16bid, typres, k19bid, masse, raide, k19bid,&
                lc, lkr, lns, lpg, lbid)
...
!     ------------------------------------------------------------------
        ...
        call sspace(lmtpsc, lmatra, lmasse, neq, nbvect, nfreq, zi(lprod), itemax, nperm, tol,&
                    toldyn, zr(lvec), zr(lvalpr), nitjac, nitbat, solveu)
        call rectfr(nfreq, nbvect, omeshi, npivot, nblagr, zr(lvalpr), nbvect, zi(lresui),&
                    zr(lresur), mxresf)
        call vpbost(typres, nfreq, nbvect, omeshi, zr(lvalpr), nbvect, vpinf, vpmax, precdc,&
                    method, omecor)
