...
C        TEST DE LA POSITION INITIALES ET DE L'ORIGINE
     ...
     &               DNORM)
         IF (DNORM .LT. 0.D0) THEN
             CALL UTMESS('A','CHVERI','ATTENTION')
             WRITE(INUM,'(I3.3)') IC
...
C
         ...
         IF(DBLE(ABS(XLOC(1))).LT.EPSI) THEN
            CALL UTMESS('A','CHVERI','ATTENTION')
            WRITE(INUM,'(I3.3)') IC
            CALL UTMESS('A','CHVERI','LA NORMALE A L''OBSTACLE FAIT '//
     &                  'UN ANGLE NUL AVEC LE NOEUD DE CHOC DE '//
     &                  'NUMERO ' // INUM // ', AVEC '//
     &                   'L''AXE DU TUBE.')
C
         ELSE IF(DBLE(ABS(XLOC(1))).LT.0.17364818D0) THEN
            ...
            WRITE(INUM,'(I3.3)') IC
            CALL UTMESS('A','CHVERI','LA NORMALE A L''OBSTACLE FAIT '//
     &                  'UN ANGLE INFERIEUR A 10 DEGRES AU NOEUD '//
     &                  'DE CHOC DE NUMERO ' // INUM // ', AVEC '//
     &                   'L''AXE DU TUBE.')
C
         ELSE IF(DBLE(ABS(XLOC(1))).LT.0.70710678D0) THEN
            ...
            WRITE(INUM,'(I3.3)') IC
            CALL UTMESS('A','CHVERI','LA NORMALE A L''OBSTACLE FAIT '//
     &                  'UN ANGLE INFERIEUR A 45 DEGRES AU NOEUD '//
     &                  'DE CHOC DE NUMERO ' // INUM // ', AVEC '//
     &                  'L''AXE DU TUBE.')
C
         ENDIF
