      SUBROUTINE TLDLR8(NOMMAT,HCOL,ADIA,ABLO,NPIVOT,NEQ,NBBLOC,ILDEB,
     +                  ILFIN,EPS)
      IMPLICIT NONE
C
      CHARACTER*(*) NOMMAT
...
C
      ...
      INTEGER LM,ICA,ICB,ICD,IBID,JREFA
      INTEGER IADIGS
C
C     ----- CNJ --------------------------------------------------------
      INTEGER ZI
...
C
C     --- CREATION D'UN TABLEAU INTERMEDIAIRE (D'UNE COLONNE)
C
...
C
  120     CONTINUE
          ...
          ICA = IADIA - ILONG
C
C        --- SAUVEGARDE DE LA COLONNE ET NORMALISATION DE LA LIGNE -----
C
