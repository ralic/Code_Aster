...
C
            CALL MLTAFF(LFRONT(SN),NBA,ADPER,ZR(PPERE),ZR(PFILS),
...
C     ASSEMBLAGE FILS -> FACTOR ( INFERIEURE)
          IF (SN.NE.0) THEN
            ...
            NL = LGSN(SN)
            NBA = NBASS(SN)
            CALL MLTAFP(LFRONT(SN),NBA,ADPER,ZR(ADFACL),ZR(PFILS),
     +                  LOCAL(ADRESS(SN)+NL))
...
C     ASSEMBLAGE FILS -> PERE ( SUPERIEURE)
              CALL MLTAFF(LFRONT(SN),NBA,ADPER,ZR(PPEREU),ZR(PFILSU),
     +                    LOCAL(ADRESS(SN)+NL),P)
...
C     ASSEMBLAGE FILS -> FACTOR ( SUPERIEURE)
              CALL MLTAFP(LFRONT(SN),NBA,ADPER,ZR(ADFACU),ZR(PFILSU),
     +                    LOCAL(ADRESS(SN)+NL))
C     ON DETRUIT LA MATRICE FRONTALE QUI NE SERA PLUS UTILISEE
...
C     FACTORISATION
          IF (TYPSYM.EQ.1) THEN
            ...
            ELSE
              CALL MLTFLM(NB,N,P,ZR(ADFACL),ADPER,T1,AD,EPS,IER,CL)
              IF (IER.NE.0) GO TO 9999
              CALL MLTFMJ(NB,N,P,ZR(ADFACL),ZR(PPERE),ADPER,T1,CL)
            END IF
            IF (M.NE.0) CALL JELIBE(JEXNUM(NOMPIL,SNI))
          ELSE
            CALL JEVEUO(JEXNUM(FACTOL,IB),'E',IFACL)
            ADFACL = IFACL - 1 + DECAL(SNI)
            CALL MLNFLM(NB,N,P,ZR(ADFACL),ZR(ADFACU),ADPER,T1,T2,AD,EPS,
     +                  IER,CL,CU)
            IF (IER.NE.0) GO TO 9999
            IF (M.NE.0) CALL JEVEUO(JEXNUM(NOMPIL,SNI),'E',PPERE) 
            CALL MLNFMG(NB,N,P,ZR(ADFACL),ZR(ADFACU),ZR(PPERE),ADPER,T1,
     %                  CL)
            IF (M.NE.0) THEN 
                CALL JELIBE(JEXNUM(NOMPIL,SNI))
                CALL JEVEUO(JEXNUM(NMPILU,SNI),'E',PPEREU)
            END IF 
            CALL MLNFMG(NB,N,P,ZR(ADFACU),ZR(ADFACL),ZR(PPEREU),ADPER,T1
     +                  ,CL)
            IF (M.NE.0) CALL JELIBE(JEXNUM(NMPILU,SNI))
