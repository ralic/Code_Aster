      SUBROUTINE MULFR8(NOMMAT,NPIVOT,NEQ,NBBLOC,ILDEB,ILFIN,TYPSYM,EPS)
      IMPLICIT NONE
      CHARACTER*(*) NOMMAT
...
C     APPEL A MLTFAS1
      IF (PILMEM) THEN
         CALL JEDETR(NOMPR1)
         CALL MLTFC1(NBLOC,ZI(NCBLOC),ZI(DECAL),ZI(LGBLOC),NBSN,
     +        ZI(SUPND),
...
C     ON ALLOUERA UNE COLLECTION DISPERSEE DANS MLTFCB
         SBLOC=1
         CALL MLTFCB(NBLOC,ZI(NCBLOC),ZI(DECAL),ZI(LGBLOC),NBSN,
     +        ZI(SUPND),
     +        ZI(FILS),ZI(FRERE),ZI(SEQ),ZI(LGSN),ZI(LFRONT),
     +        ZI(ADRESS),ZI(LOCAL),ZI(ADPILE),ZI(NBASS),
