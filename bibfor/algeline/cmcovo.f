...
C     -----  FIN  COMMUNS NORMALISES  JEVEUX  --------------------------
      ...
      INTEGER      JNORN,IMA,N1,N2,N3,NNOAJ,IC,I,IJ,II,IQ4,IT3
      ...
      INTEGER      NBMAI,JGG,NBMAT,JNO,IMA2,JTRAV,KNO
      CHARACTER*1  K1B
      ...
      REAL*8       XX(3),YY(3),ZZ(3),NX,NY,NZ,NT(3),EPS2,SINVEC,COSVEC
...
C     -------------------------------------------
      ...
      DO 10 IMA = 1, NBMA
         ...
         ELSE
            CALL UTMESS('F','CMCOVO','LES MAILLES DOIVENT ETRE DE '//
     &                    ' TYPE QUAD4 OU TRI3 ET NON DE TYPE '//TYPM)
         ENDIF
...
C
     ...
     +              + ZR(JNORM+3*(INO-1)+2)*ZR(JNORN+3*(NUMA-1)+2)
            ...
            IF (ABS(ANGL).GT.90.0D0) THEN
               ...
               CALL UTMESS('A','COQU_VOLU','L''ANGLE AU NOEUD '//NOMG//
     +              ' FORME PAR LE VECTEUR NORMAL DE LA MAILLE '//MA1//
     +              ' ET LE VECTEUR NORMAL DE LA MAILLE '//MA2//
     +      ' EST SUPERIEUR A 90 DEGRES ET VAUT '//KANGL//' DEGRES.')
            ENDIF
 72      CONTINUE
...
C
      DO 40 INO = 1, NBNIN
         ...
         ELSE
            CALL UTDEBM('F','CMCOVO','ERREUR DONNEES')
...
C
      ...
      DO 50 INO = 1 , NBNIN
         ...
         LGNU = LXLGUT(KNUME)
C
         IF (LGNU+LGNO.GT.8) CALL UTMESS('F','CMCOVO',
...
C
      ...
      DO 100 IMA = 1 , NBMA
         ...
         LGND = LXLGUT(KNUME)
         IF (LGND+LGPREF.GT.8) CALL UTMESS('F','CMCOVO',
