      SUBROUTINE MLTDRB(NBLOC,LGBLOC,NCBLOC,DECAL,SEQ,NBSN,NBND,SUPND,
...
C     TOLE CRP_21
      ...
      INTEGER SEQ(NBSN),SUPND(NBSN+1),GLOBAL(*),LGSN(NBSN)
...
C     
      INTEGER DEB1
      INTEGER SNI,K,J,DEB,FIN,NDK,GJ,IFAC,ISM
      INTEGER DEB0,IL0
...
C     RANGEMENT DU TERME DIAGONAL
            ...
            IF (LONG.GT.L) THEN
               P = L
               OPTA=1
C$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(ISM,LARG,DEB) SCHEDULE(STATIC)
               DO 152 ISM=1,NPROC
