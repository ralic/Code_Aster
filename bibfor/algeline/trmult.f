...
C     --- ON NORMALISE LE VECTEUR ---
 ...
 10   CONTINUE
      ...
      IF (XNORM.LT.0.D0) THEN
         CALL UTMESS('F','TRMULT','LE VECTEUR DIRECTEUR EST NUL.')
      ENDIF
      DO 12 I = 1,NBDIR
         DEPL(I) = DEPL(I) / XNORM
 12   CONTINUE
...
C              --- ON RECUPERE LE MODE STATIQUE ASSOCIE AU NOEUD ---
     ...
     +                     CRIT,IORDR,1,NBTROU)
               IF (NBTROU.NE.1) THEN
                  IER = IER + 1
                  CALL UTDEBM('F','TRMULT','PAS DE MODE STATIQUE POUR')
     ...
     +                                  'DEPL_IMPO',IRET)
               IF (IRET.NE.100) THEN
                  IER = IER + 1
                  CALL UTDEBM('F','TRMULT','POUR LES MODES STATIQUES.')
               ...
               IF (IRET.NE.0) THEN
                  IER = IER + 1
                  CALL UTDEBM('E','TRMULT','CHAMP INEXISTANT.')
