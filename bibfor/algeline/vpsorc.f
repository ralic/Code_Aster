...
C GESTION DES FLAGS D'ERREURS
      ...
      ELSE IF (INFO.EQ.2) THEN
        CALL UTMESS('F','VPSORC/ZNAUPD 2','PROBLEME INTERNE ARPACK')
     ...
     &   ' APPLIQUE'
      ...
      ELSE IF (INFO.EQ.-7) THEN
        CALL UTMESS('F','VPSORC/ZNAUPD -7','PROBLEME TAILLE '//
     &    'WORKD/L -> AUGMENTER DIM_SOUS_ESPACE')
      ELSE IF (INFO.EQ.-8) THEN
        CALL UTMESS('F','VPSORC/ZNAUPD -8','PROBLEME INTERNE '//
     &    'LAPACK')
      ELSE IF (INFO.EQ.-9) THEN
        CALL UTMESS('F','VPSORC/ZNAUPD -9','PROBLEME CONSTRUCTION'//
     ...
     &    ' D''ARNOLDI'
      ...
      ELSE IF (INFO.LT.0) THEN
        CALL UTDEBM('F','VPSORC/ZNAUPD -N','INCOHERENCE DE CER'//
...
C GESTION DES MODES CONVERGES
      IF ((NCONV.LT.NFREQ).AND.(IDO.EQ.99)) THEN
        CALL UTDEBM('E','VPSORC/ZNAUPD 0','NOMBRE DE VALEURS PROPRES ')
...
C GESTION DES FLAGS D'ERREURS
        IF (INFO.EQ.1) THEN
          CALL UTMESS('F','VPSORC/ZNEUPD 1','PROBLEME INTERNE '//
     &     'LAPACK')
        ELSE IF (INFO.EQ.-7) THEN
          CALL UTMESS('F','VPSORC/ZNEUPD -7','PROBLEME TAILLE '//
     &     'WORKD/L -> AUGMENTER DIM_SOUS_ESPACE')
        ELSE IF (INFO.EQ.-8) THEN
          CALL UTMESS('F','VPSORC/ZNEUPD -8','PROBLEME INTERNE '//
     &     'LAPACK, ROUTINE FLAHQR (FORME DE SCHUR)')
        ELSE IF (INFO.EQ.-9) THEN
          CALL UTMESS('F','VPSORC/ZNEUPD -9','PROBLEME INTERNE '//
     &     'LAPACK, ROUTINE FTREVC (VECTEURS PROPRES)')
        ELSE IF (INFO.EQ.-14) THEN
          CALL UTMESS('F','VPSORC/ZNEUPD -14','AUCUNE VALEUR PROPRE'//
     &      ' A LA PRECISION REQUISE --> AUGMENTER PREC_SOREN OU '//
     &      'NMAX_ITER_SOREN OU AUGMENTER DIM_SOUS_ESPACE')
        ELSE IF (INFO.LT.0) THEN
          CALL UTDEBM('F','VPSORC/ZNEUPD -N','INCOHERENCE DE CER'//
     &     'TAINS PARAMETRES MODAUX PROPRES A ARPACK')
          CALL UTIMPI('L',' NUMERO D''ERREUR ',1,INFO)
          CALL UTFINM()
        ENDIF
...
C       WRITE(6,*) 'RE: LANDAJ/ FJ INIT',RBID
C   59 CONTINUE
...
C RE-ORTHONORMALISATION SUIVANT IGS PAR RAPPORT A B
      ...
      CALL VPGSKC
     &  (NBEQ, NCONV, VECT, ALPHA, LMASSE, 1, VAUX, DDLEXC, RWORK)
