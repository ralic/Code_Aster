...
C
      ...
      CALL MRMULT('ZERO',LC,YH,U2,1,.FALSE.)
      CALL MRMULT('ZERO',LM,YB,U3,1,.FALSE.)
      CALL MRMULT('ZERO',LM,YH,U4,1,.FALSE.)
C
      IF ( DSR .NE. ZERO ) THEN
...
11       CONTINUE
      ENDIF
C
      CALL MRMULT('CUMU',LK,YB,ZH,1,.FALSE.)
      CALL MRMULT('CUMU',LC,YB,ZB,1,.FALSE.)
C
      DO 20, I = 1, N, 1
         ZH(I) =  ISI*ZH(I)
         ZB(I) =  ISI*ZB(I)
