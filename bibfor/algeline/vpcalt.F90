...
!
    call vplecs(eigsol,&
                ibid, ibid, ibid, nitv, nborto, ibid, nbvect, ibid, nfreq, ibid,&
                rbid, omecor, rbid, rbid, precdc, rbid, prorto, prsudg, rbid, rbid, rbid,&
                rbid,&
                appr, k8bid, method, k9bid, k14bid, k14bid, k14bid, k16bid, optiof, k16bid, k16bid,&
...
!     ------------------------------------------------------------------
        ...
        call vp2ini(lmtpsc, lmasse, lmatra, neq, nbvect, nborto, prorto, zi(lprod), zi(lddl),&
                    zr(ldiagr), zr(lsurdr), zr(lsign), zr( lvec), prsudg, nstoc, omeshi, solveu)
        call vp2trd('G', nbvect, zr(ldiagr), zr(lsurdr), zr(lsign), zr(iadz), nitv, nitqrm)
        call vpreco(nbvect, neq, zr(iadz), zr(lvec))
        call rectfr(nfreq, nbvect, omeshi, npivot, nblagr, zr(ldiagr), nbvect, zi(lresui),&
                    zr(lresur), mxresf)
        call vpbost(typres, nfreq, nbvect, omeshi, zr(ldiagr), nbvect, vpinf, vpmax, precdc,&
                    method, omecor)
        if (typres(1:9) .eq. 'DYNAMIQUE')&
            call vpordi(1, 0, nfreq, zr( lresur+mxresf), zr(lvec), neq, zi(lresui))
...
!     -------  LANCZOS PB QUADRATIQUE REEL SYMETRIQUE ------------------
!     ------------------------------------------------------------------
        call wp2ini(appr, lmasse, lamor, lraide, lmatra, lmtpsc, sigma, zr(iadrh), zr(iadrb),&
                    optiof, prorto, nborto, nbvect, neq, zi(lprod), zi(lddl), zr(ldiagr),&
                    zr(lsurdr), zr(lsign), zr(iadx), zr(iady), solveu)
        call vp2trd('Q', nbvect, zr(ldiagr), zr(lsurdr), zr(lsign), zr(iadz), nitv, nitqrm)
        npivot = nblagr
        nfreq = nfreq / 2
        call wp2vec(appr, optiof, nfreq, nbvect, neq, sigma, zr(iadx), zr(iady), zr(iadz),&
                    2*nbvect, zr(lsurdr), zr(ldiagr), zc(lvec), mxresf, zi(lresui),&
