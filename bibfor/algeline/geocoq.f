...
C
      ...
      IF (NBNOIN.LT.4) CALL UTMESS('F','GEOCOQ','NOMBRE DE NOEUDS'//
     &                             ' INSUFFISANT SUR LA COQUE INTERNE')
  ...
  10  CONTINUE
      ...
      IF (DIFZ.LT.TOLE) CALL UTMESS('F','GEOCOQ','COQUE INTERNE DE '//
     &  'LONGUEUR NULLE')
C
...
C
      ...
      IF (NBNOEX.LT.4) CALL UTMESS('F','GEOCOQ','NOMBRE DE NOEUDS'//
     &                             ' INSUFFISANT SUR LA COQUE EXTERNE')
  ...
  20  CONTINUE
      ...
      IF (DIFZ.LT.TOLE) CALL UTMESS('F','GEOCOQ','COQUE EXTERNE DE '//
     &  'LONGUEUR NULLE')
C
...
C
     ...
     &    DIFZ2.LT.TOLE) CALL UTMESS('F','GEOCOQ','LE DOMAINE DE '//
     &    'RECOUVREMENT DES COQUES INTERNE ET EXTERNE N EXISTE PAS')
C
...
C
      ...
      IF (IRET.EQ.0) CALL UTMESS('F','GEOCOQ','LA CARTE DES '//
     &  'CARACTERISTIQUES GEOMETRIQUES DES ELEMENTS DE COQUE '//
     &  'N EXISTE PAS. IL FAUT PREALABLEMENT AFFECTER CES '//
     &  'CARACTERISTIQUES AUX GROUPES DE MAILLES CORRESPONDANT AUX '//
     &  'COQUES INTERNE ET EXTERNE, PAR L OPERATEUR <AFFE_CARA_ELEM>')

      CALL DISMOI('F','NB_EC','CACOQU','GRANDEUR',NBEC,K8BID,IER)
      IF (NBEC.NE.2) CALL UTMESS('F','GEOCOQ','CE PROGRAMME'
     +    //' SUPPOSE QUE LA GRANDEUR "CACOQU" A 2 ENTIERS CODES')
...
C
      ...
      DO 30 IAS = 1,IASEDI
        ...
        ENDIF
  30  CONTINUE
      IF (IASCQI.EQ.0 .OR. IASCQX.EQ.0) CALL UTMESS('F','GEOCOQ',
     &  'LES CARACTERISTIQUES DES ELEMENTS DE COQUE N ONT PAS ETE '//
     &  'AFFECTEES DISTINCTEMENT A L UN OU(ET) L AUTRE DES GROUPES '//
     &  'DE MAILLES ASSOCIES AUX COQUES INTERNE ET EXTERNE')
C
...
C
  ...
  40  CONTINUE
      IF (IRANG.EQ.0) CALL UTMESS('F','GEOCOQ','LA COMPOSANTE <EP> '//
...
C
  ...
  50  CONTINUE
      IF (IRANV.EQ.0) CALL UTMESS('F','GEOCOQ','PAS D EPAISSEUR '//
     &  'AFFECTEE AUX ELEMENTS DE LA COQUE INTERNE')
C
      EPINT = ZR(IVALE+NBCMP*(IASCQI-1)+IRANV-1)
      IF (EPINT.EQ.0.D0) CALL UTMESS('F','GEOCOQ','EPAISSEUR DE LA '//
     &  'COQUE INTERNE NULLE')
C
      GEOM(6) = EPINT
C
...
C
  ...
  51  CONTINUE
      IF (IRANV.EQ.0) CALL UTMESS('F','GEOCOQ','PAS D EPAISSEUR '//
     &  'AFFECTEE AUX ELEMENTS DE LA COQUE EXTERNE')
C
      EPEXT = ZR(IVALE+NBCMP*(IASCQX-1)+IRANV-1)
      IF (EPEXT.EQ.0.D0) CALL UTMESS('F','GEOCOQ','EPAISSEUR DE LA '//
     &  'COQUE EXTERNE NULLE')
C
      GEOM(7) = EPEXT
C
...
C
      IF (RINT.EQ.0.D0 .OR. REXT.EQ.0.D0) CALL UTMESS('F','GEOCOQ',
     &  'INCOHERENCE DANS LA DEFINITION DE LA CONFIGURATION : LE '//
     &  'RAYON D UNE DES COQUES EST NUL')
C
      GEOM(8) = RINT
      GEOM(9) = REXT
C
      ...
      IF (HMOY.LE.0.D0) CALL UTMESS('F','GEOCOQ','INCOHERENCE DANS LA'//
     & ' DEFINITION DE LA CONFIGURATION : JEU ANNULAIRE NEGATIF OU NUL')
C
      GEOM(1) = HMOY
      GEOM(2) = RMOY
C
