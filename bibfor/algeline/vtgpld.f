...
C
      ...
      IF (KBID(1:6).NE.'GEOM_R') CALL U2MESS('F','CALCULEL_2')
      CALL DISMOI('F','NOM_GD',DEPLA2,'CHAM_NO',IBID,KBID,IERD)
      IF (KBID(1:6).NE.'DEPL_R') CALL U2MESS('F','CALCULEL_8')
C
...
C
      ...
      NUM = ZI(IADESC-1+2)
      NEC = NBEC(IGD)
      IF (NEC.GT.10) CALL U2MESS('F','CATAELEM_14')
...
C     -- ON VERIFIE LE NOM DES CMPS DE LA GRANDEUR DEPL_R:
      ...
      IF (ZK8(IAD-1+1).NE.'DX') CALL U2MESS('F','CALCULEL_10')
      IF (ZK8(IAD-1+2).NE.'DY') CALL U2MESS('F','CATAELEM_11')
      IF (ZK8(IAD-1+3).NE.'DZ') CALL U2MESS('F','CATAELEM_12')
C
