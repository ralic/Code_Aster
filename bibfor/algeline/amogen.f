...
C     -----  FIN  COMMUNS NORMALISES  JEVEUX  --------------------------
      ...
      REAL*8   R8BID,KMIN,VALMIN, KMAX,RK
      INTEGER      IARG

      CALL JEMARQ()

      NOMCMD = 'CALC_AMOR_GENE'
      CALL GETVID(NOMCMD, 'MASS_GENE', 1,IARG,1,MASSE,NBID)
      CALL GETVID(NOMCMD, 'RIGI_GENE', 1,IARG,1,RAID,NBID)
      CALL GETVR8(NOMCMD, 'AMOR_REDUIT', 1,IARG,0, R8BID, N1)
      CALL GETVID(NOMCMD,'LIST_AMOR',1,IARG,0,K8BID,N2)
      ...
      IF (N1.NE.0) THEN
        NBAMOR = -N1
      ELSE
        CALL GETVID(NOMCMD,'LIST_AMOR',1,IARG,1,LISTAM,NLIST)
      ...
      IF (NBAMOR.GT.N) THEN
        ...
        IF (N1.NE.0) THEN
          CALL GETVR8(NOMCMD,'AMOR_REDUIT',1,IARG,N,ZR(JAMOG),NBID)
        ELSE
...
30        CONTINUE
      ...
      ELSE
        CALL WKVECT('&&AMORMA.AMORTI','V V R8',N,JAMOG)
        IF (N1.NE.0) THEN
          CALL GETVR8(NOMCMD,'AMOR_REDUIT',1,IARG,NBAMOR,ZR(JAMOG),NBID)
        ELSE
          CALL JEVEUO(LISTAM//'           .VALE','L',IAMOG)
          DO 40 I = 1,NBAMOR
            ZR(JAMOG+I-1) = ZR(IAMOG+I-1)
