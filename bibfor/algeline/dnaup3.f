     ...
     &     NSTA, DDLSTA, VSTAB, CSTA, LDYNFA, DDLEXC, REDEM)
C---------------------------------------------------------------------
C
...
C  
C  NSTA   INTEGER (INPUT) NOMBRE DE DDL STAB
C
C  DDLSTA INTEGER ARRAY OF LENGTH N (INPUT). POSITION DES DDL_STAB 
C
C  VSTAB  REAL*8 ARRAY OF LENGTH N (INPUT/OUTPUT). MODE STABILITE
C
C  CSTA   REAL*8 (OUTPUT). VALEUR CRITERE DE STABILITE
C
C  LDYNFA INTEGER (INPUT). DESCRIPTEUR DE L'OPERATEUR TANGENT
C
C  DDLEXC INTEGER ARRAY OF LENGTH N (INPUT). POSITION DDLS IMPOSES
C  
C  REDEM  INTEGER (INPUT). NOMBRE DE REDEMARRAGES METHODE SORENSEN
C
C ENDDOC
C-----------------------------------------------------------------------
C BEGINLIB
...
C     %------------------%
      ...
      CHARACTER*1 BMAT
      CHARACTER*2 WHICH
      INTEGER IDO, INFO, ISHIFT, LDH, LDQ, LDV, MXITER, NSTA,
     &  N, NEV, NP, NEQACT
      REAL*8 TOL, ALPHA, CSTA
      INTEGER LDYNFA, REDEM
...
C     %-----------------%
     ...
     &  WORKD(3*N), WORKL( (NEV+NP)*(NEV+NP+3) ), VSTAB(LDV)
...
C     %---------------%
     ...
     &  NP0, NPTEMP, NUMCNV, K, ETAT, IFM, NIV
      REAL*8 RNORM, TEMP, EPS23, BETA
...
C
           ...
           ENDIF
           CALL UNISTA (H,LDH,V,LDV,DDLSTA,N,VSTAB,CSTA,BETA,
     &                  ETAT,LDYNFA,DDLEXC,REDEM)
         ENDIF
C
         IF (IERR .NE. 0) THEN
            INFO = -8
