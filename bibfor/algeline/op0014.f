...
C MODIF ALGELINE  DATE 26/07/2011   AUTEUR DESOZA T.DESOZA 
C ======================================================================
...
C     -----  FIN  COMMUNS NORMALISES  JEVEUX  --------------------------
      ...
      CHARACTER*3 KSTOP,KLAG2,KDJ
      CHARACTER*24 VALK(2)
      CHARACTER*8 MATASS,MATFAC,TYPE,KTYPR,KTYPS,METRES,PRECON,KOOC,
     &            MIXPRE
...
C     ----------------------
      IF (METRES.EQ.'MUMPS') THEN
         ...
         CALL GETVTX(' ','LIBERE_MEMOIRE',1,1,1,KDJ,IBID)
         CALL JEVEUO(SOLVEU//'.SLVI','E',JSLVI)
         CALL JEVEUO(SOLVEU//'.SLVK','E',JSLVK)
         CALL JEVEUO(SOLVEU//'.SLVR','E',JSLVR)
         ZI(JSLVI-1+1)  =NPREC
...
C        ON EST OBLIGE DE MODIFIER DIRECTEMENT MASS
         ...
         IF (PRECON.EQ.'LDLT_INC') THEN
           CALL GETVIS(' ','NIVE_REMPLISSAGE',0,1,1,NIREMP,IBID)
           CALL GETVR8(' ','REMPLISSAGE'     ,0,1,1,FILLIN,IBID)
           ZR(JSLVR-1+3) = FILLIN
           ZI(JSLVI-1+4) = NIREMP
         ELSE IF (PRECON.EQ.'LDLT_SP') THEN
C          CREATION D'UN NOM UNIQUE POUR LA SD SOLVEUR MUMPS 
