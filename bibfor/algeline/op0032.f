...
C     --- KIND OF COMPUTATION : REAL (GEP), DYNAMIC OR BUCKLING    ---
      IF (TYPMOD(1:7).EQ.'GENERAL') THEN
        IF (ZI(LMASSE+3)*ZI(LMASSE+4)*ZI(LRAIDE+3)*ZI(LRAIDE+4).NE.1
     &      .OR.LC) THEN
          TYPMOD(1:13)='MODE_COMPLEXE'
        ELSE
          TYPMOD(1:10)='MODE_FLAMB'
        ENDIF
      ENDIF
      ...
      ELSE IF (TYPMOD(1:13).EQ.'MODE_COMPLEXE') THEN
        ...
        CALPAC(3) = RAYONC     
      ELSE IF (TYPMOD(1:10).EQ.'MODE_FLAMB'.OR.
     &         TYPMOD(1:7).EQ.'GENERAL') THEN
...
C     --- INITIALIZATION JUST IN CASE                           ---
      ...
      IF (TYPMET(1:5).EQ.'STURM') THEN
        ...
        ELSE
          CALL GETVR8('COMPTAGE','SEUIL_CHAR_CRIT',1,IARG,1,FCORIG,L)
        ENDIF
