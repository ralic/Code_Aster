      SUBROUTINE WPSORN
     &  (APPR, OPT, LMASSE, LAMOR, LMATRA, NBEQ,
...
C POUR LE FONCTIONNEMENT GLOBAL    
      INTEGER    I, J
      INTEGER    AU1,AU2,AU3,AU4,AU5,AV
...
C     ---- ALLOCATION DES ZONES DE TRAVAIL ---
      ...
      CALL WKVECT('&&WPSORN.VECTEUR.AUX.U4R','V V R',NBEQ,AU4  )
...
C GESTION DES FLAGS D'ERREURS
      ...
      ELSE IF (INFO.EQ.2) THEN
        CALL UTMESS('F','WPSORN/DNAUPD 2','PROBLEME INTERNE ARPACK')
     ...
     &   ' APPLIQUE'
      ...
      ELSE IF (INFO.EQ.-7) THEN
        CALL UTMESS('F','WPSORN/DNAUPD -7','PROBLEME TAILLE '//
     &    'WORKD/L -> AUGMENTER DIM_SOUS_ESPACE')
      ELSE IF (INFO.EQ.-8) THEN
        CALL UTMESS('F','WPSORN/DNAUPD -8','PROBLEME INTERNE '//
     &    'LAPACK')
      ELSE IF (INFO.EQ.-9) THEN
        CALL UTMESS('F','WPSORN/DNAUPD -9','PROBLEME CONSTRUCTION'//
     ...
     &    ' D''ARNOLDI'
      ...
      ELSE IF (INFO.LT.0) THEN
        CALL UTDEBM('F','WPSORN/DNAUPD -N','INCOHERENCE DE CER'//
...
C GESTION DES MODES CONVERGES
      IF ((NCONV.LT.NFREQ).AND.(IDO.EQ.99)) THEN
        CALL UTDEBM('A','WPSORN/DNAUPD 0','NOMBRE DE VALEURS PROPRES ')
...
C X <- X*DDL_LAGRANGE
   ...
   25   CONTINUE 
          CALL WP2AYL(APPR,LMATRA,LMASSE,LAMOR,SIGMA,DDLEXC,
     +            VAUX(1),VAUX(NBEQ+1),WORKD(IPNTR(1)),
     +            WORKD(IPNTR(1)+NBEQ),
     +            ZR(AU1),ZR(AU2),ZR(AU3),ZR(AU4),ZC(AV),NBEQ)
        DO 30 J= 1,NBEQ
...
C GESTION DES FLAGS D'ERREURS
        IF (INFO.EQ.1) THEN
          CALL UTMESS('F','WPSORN/DNEUPD 1','PROBLEME INTERNE '//
     &     'LAPACK')
        ELSE IF (INFO.EQ.-7) THEN
          CALL UTMESS('F','WPSORN/DNEUPD -7','PROBLEME TAILLE '//
     &     'WORKD/L -> AUGMENTER DIM_SOUS_ESPACE')
        ELSE IF (INFO.EQ.-8) THEN
          CALL UTMESS('F','WPSORN/DNEUPD -8','PROBLEME INTERNE '//
     &     'LAPACK, ROUTINE DLAHQR (FORME DE SCHUR)')
        ELSE IF (INFO.EQ.-9) THEN
          CALL UTMESS('F','WPSORN/DNEUPD -9','PROBLEME INTERNE '//
     &     'LAPACK, ROUTINE DTREVC (VECTEURS PROPRES)')
        ELSE IF (INFO.EQ.-14) THEN
          CALL UTMESS('F','WPSORN/DNEUPD -14','AUCUNE VALEUR PROPRE'//
     &      ' A LA PRECISION REQUISE --> AUGMENTER PREC_SOREN OU '//
     &      'NMAX_ITER_SOREN OU AUGMENTER DIM_SOUS_ESPACE')
        ELSE IF (INFO.LT.0) THEN
          CALL UTDEBM('F','WPSORN/DNEUPD -N','INCOHERENCE DE CER'//
     &     'TAINS PARAMETRES MODAUX PROPRES A ARPACK')
          CALL UTIMPI('L',' NUMERO D''ERREUR ',1,INFO)
          CALL UTFINM()
        ENDIF
...
C RE-ORTHONORMALISATION SUIVANT IGS PAR RAPPORT A B
      CALL WPGSKP
     &  ( NBEQ, NCONV/2, VAUL, ALPHA, LMASSE, LAMOR,
     &   2, ZC(AU5), DDLEXC, WORKV)
