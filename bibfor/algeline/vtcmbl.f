...
C DECLARATION VARIABLES LOCALES
     ...
     &        ICMB,IRET,IVAL,ILIMPI,IFETI,LVALE,ICONST
...
C ON VERIFIE QUE TOUTE LES CHAM_NO DE LA LISTE SONT HOMOGENES
        ...
        ELSE
          IF (NIV.GE.2)WRITE (IFM,*)'<FETI/VTCMBL> CHAM_NO SANS REFE ',
     &        K24B(1:19)
          NBREFE=0
        ENDIF
        IF (NBREFE.NE.4) THEN
          IF (NIV.GE.2)WRITE (IFM,*)
     &        '<FETI/VTCMBL> CHAM_NO NON ETENDU POUR FETI ',K24B(1:19)
        ELSE
     ...
     &          ,K24B(1:19)
          ...
          ELSE
            ...
            METHO1=ZK24(IREFE+2)
            SDFET1=ZK24(IREFE+3)
            IF ((METHO1.NE.METHOD) .OR. (SDFET1.NE.
     &          SDFETI)) CALL U2MESS('F','ALGELINE3_92')
          ENDIF
   10   CONTINUE
      ENDIF
...
C SI FETI CONSTITUTION DE L'OBJET JEVEUX CHPRESS.FETC COMPLEMENTAIRE
            IF ((NBSD.GT.0) .AND. (IDD.EQ.0)) CALL WKVECT(CH19R//FETC,
     &          'V V K24',NBSD,KFETC)
          ELSE
...
C SI FETI CONNEXION A L'OBJET JEVEUX CHPRESS.FETC COMPLEMENTAIRE
            IF ((NBSD.GT.0) .AND. (IDD.EQ.0)) CALL JEVEUO(CH19R//FETC,
     &          'E',KFETC)
          ENDIF
   ...
   80             CONTINUE
                ELSEIF (TYPCST(ICMB)(1:1).EQ.'C') THEN
                  ...
                  DO 90 IVAL=0,NBVALE-1
                    ZC(LVALE+IVAL)=ZC(LVALE+IVAL)+C8CST*ZR(JVALE+IVAL)
   90             CONTINUE
                ENDIF
              ELSE
                IF (TYPCST(ICMB)(1:1).EQ.'R') THEN
  ...
  100             CONTINUE
                ELSEIF (TYPCST(ICMB)(:1).EQ.'C') THEN
                  ...
                  DO 110 IVAL=0,NBVALE-1
                    ZC(LVALE+IVAL)=ZC(LVALE+IVAL)+C8CST*ZC(JVALE+IVAL)
  110             CONTINUE
                ENDIF
              ENDIF
            ENDIF
