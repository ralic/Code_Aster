...
!
    call vplecs(eigsol,&
                ibid, ibid, ibid, ibid, ibid, ibid, nbvect, ibid, nfreq, ibid,&
                rbid, omecor, rbid, rbid, precdc, rbid, rbid, rbid, rbid, rbid, rbid,&
                rbid,&
                k1bid, k8bid, method, k9bid, k14bid, k14bid, k14bid, k16bid, optiof, k16bid,k16bid,&
...
!     ------------------------------------------------------------------
            call vpqzla(typeqz, qrn, iqrn, lqrn, qrar, qrai, qrba, qrvl, lvec, kqrn, lvalpr, nconv,&
                        omecor, ktyp, kqrnr, neqact, ilscal, irscal, optiof, omemin, omemax,&
                        omeshi, zi(lprod), nfreq, lmasse, lraide, lamor, numedd, sigma, icscal,&
                        ivscal, iiscal, bwork, flage)
            call rectfr(nconv, nconv, omeshi, npivot, nblagr, zr(lvalpr), nfreq, zi(lresui),&
                        zr(lresur), mxresf)
            call vpbost(typres, nconv, nconv, omeshi, zr(lvalpr), nfreq, vpinf, vpmax, precdc,&
                        method, omecor)
...
!     ------------------------------------------------------------------
            call vpqzla(typeqz, qrn, iqrn, lqrn, qrar, qrai, qrba, qrvl, lvec, kqrn, lvalpr, nconv,&
                        ...
                        ivscal, iiscal, bwork, flage)
            npivot = nblagr
            call rectfc(nconv, nconv, sigma, npivot, nblagr, zc(lvalpr), nfreq, zi(lresui),&
                        zr(lresur), nfreq)
            call vpbosc(typres, nconv, nconv, sigma, zc(lvalpr), nfreq, vpinf, vpmax, precdc,&
                        method, omecor)
            do imet = 1, nconv
                zi(lresui-1+ mxresf+imet)  = izero
                zr(lresur-1+imet)          = freqom(zr(lresur-1+mxresf+imet))
...
!     ------------------------------------------------------------------
        call vpqzla(typeqz, qrn, iqrn, lqrn, qrar, qrai, qrba, qrvl, lvec, kqrn, lvalpr, nconv,&
                    ...
                    ivscal, iiscal, bwork, flage)
        nfreq=nfreq/2
        call wp4vec(nfreq, nconv, neq, sigma, zc(lvalpr), zc(lvec), mxresf, zi(lresui), zr(lresur),&
                    zi(lprod), zc(lauc), omecor)
        do imet = 1, nfreq
            zi(lresui-1+mxresf+imet)       = izero
            zr(lresur-1+imet)              = freqom(zr(lresur-1+mxresf+imet))
            zk24(lresuk-1+mxresf+imet)     = typeqz
