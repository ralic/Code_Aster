...
C DECLARATION PARAMETRES D'APPELS
      ...
      CHARACTER*24 CRITER,TYREOR,PRECO,SCALIN,STOGI,ACMA,ACSM
...
C DECLARATION VARIABLES LOCALES
     ...
     &             NBPROC,IMSMK,IMSMI,DIMTET,IMSMR,NBDDSM,NBREOA,NBVAL,
     &             NBVAL1,JBID,LBID,ASSMAT,MBID
     ...
     &             K24DDR,K24FIR,K24REX,K24FIV,K24PRJ,K24IPI,K24MS1,
     &             K24MS2
...
C  INTER-VALIDATION DES DONNEES
        IF (NBREOR.GT.NITER) THEN
          NBREOR=NITER
          IF (RANG.EQ.0) WRITE(IFM,1064)NBREOR
        ENDIF
        IF (NBREOR.EQ.0) THEN
...
C----  1.5 DONNEES POUR ACCELERATION ACMA ET ACSM
      ...
      CALL JEVEUO('&FETI.PAS.TEMPS','L',IFETPT)
      NBVAL=ZI(IFETPT)
      NBVAL1=NBVAL-1
      ITPS=ZI(IFETPT+1)
      ASSMAT=ZI(IFETPT+2)
C  INITIALISATION DES VECTEURS D'INDIRECTION SUR LES DD DES PAS DE
C  TEMPS PRECEDENTS
      K24MS1='&FETI.MULTIPLE.SM.K24'
      K24MS2='&FETI.MULTIPLE.SM.IN'
      IF (ITPS.EQ.1) THEN
        CALL WKVECT(K24MS1,'V V K24',NBVAL,IMSMK)
        CALL WKVECT(K24MS2,'V V I',NBVAL,IMSMI)
        DO 5 I=0,NBVAL1
          ZK24(IMSMK+I)='                       '
          ZI(IMSMI+I)=0
    5   CONTINUE
      ELSE
        CALL JEVEUO(K24MS1,'E',IMSMK)
        CALL JEVEUO(K24MS2,'E',IMSMI)
      ENDIF
      LACMA=.FALSE.
      LACSM=.FALSE.
      DIMTET=0
      NBREOA=0
      IF ((REORTH).AND.(NBVAL.GT.1)) THEN
C  INTER-VALIDATION DES DONNEES
        IF (NBREOI.GE.NBVAL) THEN
          NBREOI=NBVAL-1
          IF (RANG.EQ.0) WRITE(IFM,1066)NBREOI      
        ENDIF
        IF (NBREOI.EQ.0) THEN
          NBREOI=MAX(NBVAL/5,5)
          IF (RANG.EQ.0) WRITE(IFM,1067)NBREOI
        ENDIF
...
C  PERIPHERIQUES (SAUF POUR FVAF/FNBN ET FACS QU'AU PREMIER PAS)
     ...
     &              RBID)
        ...
        CALL FETMPI(5,NBSD1,IFM,NIVMPI,IBID,IBID,K24BID,K24BID,K24BID,
     &              RBID)
        K24BID='&FETI.INFO.CPU.ELEM'
        CALL FETMPI(5,NBPROC,IFM,NIVMPI,IBID,IBID,K24BID,K24BID,K24BID,
     &              RBID)
      ENDIF
...
C  UI_SOL = (KI)+ * (FI - RIT*LANDA_SOL) - BI*ALPHAI_SOL   
     ...
     &      LSTOGI,INFOFE,IREX,IPRJ,IFM,IFIV,NBPROC,RANG,K24IRR)
        IF (RANG.EQ.0) ZI(IMSMI-1+ITPS)=0          
        GOTO 200
      ENDIF
...
C  STOCKAGE
          IF (RANG.EQ.0) ZI(IMSMI-1+ITPS)=MIN(ITER+1,NBREOR)
          GOTO 200
        ELSE
          CALL FETCPU(9,TEMPS,INFOFE,RANG,IFM) 
        ENDIF
...
C----  4.2 FORMAT AFFICHAGE
 ...
 1065 FORMAT('! FETI: NB_REORTHO_DD FIXE A MAX(NMAX_ITER/10,5)= ',
     &       I6,' !')
 1066 FORMAT('! FETI: NB_REORTHO_INST FIXE A NBVAL-1= ',I6,' !')
 1067 FORMAT('! FETI: NB_REORTHO_INST FIXE A MAX(NBVAL/5,5)= ',I6,' !')
...
C----  4.3.2 DONNEES DE TYPE REORTHO
      ...
      IF (RANG.EQ.0) THEN
        IF ((.NOT.LACSM).AND.(.NOT.LACMA)) THEN
C  PAS D'ACCELERATION MA OU SM: ON EFFACE TOUT A CHAQUE PAS
          NBDDSM=ZI(IMSMI+ITPS-1)
          IF (NBDDSM.NE.0) THEN
            K8BID1=ZK24(IMSMK+ITPS-1)
            CALL JEDETR('&&FETI.PS.'//K8BID1)
            CALL JEDETR('&&FETI.DD.'//K8BID1)
            CALL JEDETR('&&FETI.FIDD.'//K8BID1)
          ENDIF
        ELSE
C  ACCELERATION MA OU SM
          IF (ITPS.GT.NBREOI) THEN
C  ON EFFACE TOUTES LES DONNEES DE CE PAS DE TEMPS QUI DEPASSENT
C  LA ZONE DE REORTHO
            NBDDSM=ZI(IMSMI+ITPS-1)
            IF (NBDDSM.NE.0) THEN                 
              K8BID1=ZK24(IMSMK+ITPS-1)(1:8)
              CALL JEDETR('&&FETI.PS.'//K8BID1)
              CALL JEDETR('&&FETI.DD.'//K8BID1)
              CALL JEDETR('&&FETI.FIDD.'//K8BID1)
            ENDIF
          ENDIF
C DERNIER PAS DE TEMPS: ON EFFACE TOUTES LES DONNEES REORTHO
          IF (ITPS.EQ.NBVAL) THEN
            DO 250 I=1,NBREOA
              NBDDSM=ZI(IMSMI+I-1)
              IF (NBDDSM.NE.0) THEN               
                K8BID1=ZK24(IMSMK+I-1)(1:8)
                CALL JEDETR('&&FETI.PS.'//K8BID1)
                CALL JEDETR('&&FETI.DD.'//K8BID1)
                CALL JEDETR('&&FETI.FIDD.'//K8BID1)
              ENDIF
...
C----  4.3.3  DONNEES A DETRUIRE EN FIN DE LISTE DE PAS DE TEMPS
     ...
     &    CALL JEDETR('&&FETI.TEST')
        ...
        CALL JEDETR('&FETI.INFO.CPU.ELEM')
        CALL JEDETR('&FETI.PAS.TEMPS')
      ELSE

C----  4.3.4  TEMPS INTERMEDIAIRES: INIT DES VECTEURS DE MONITORING

        CALL JEVEUO('&FETI.INFO.CPU.FACN','E',IBID)
        CALL JEVEUO('&FETI.INFO.CPU.ASSE','E',JBID)
        CALL JEVEUO('&FETI.INFO.CPU.FACS','E',LBID)
        CALL JEVEUO('&FETI.INFO.CPU.ELEM','E',MBID)
        DO 300 I=0,NBSD
          ZR(IBID+I)=0.D0
          ZR(JBID+I)=0.D0
          ZR(LBID+I)=0.D0
