      SUBROUTINE MCMMVZ ( CUMUL,NOMMAT,ADIA,HCOL,NEQ,VECT,XSOL,NBVECT)
...
C             : LA MATRICE EST HERMITIENNE STOCKEE MORSE
C     ------------------------------------------------------------------
C
C
...
C
C---- PREMIERE LIGNE
         XSOL(1,NV) = XSOL(1,NV) + ZC(LMAT)*VECT(1,NV)
C
C---- LIGNES SUIVANTES
         DO 40 I = 2 , NEQ
            KDEB = ADIA(I-1)+1
...
CCDIR$ IVDEP
     ...
     +                                + DCONJG(ZC(LMAT-1+KI))*VECT(I,NV)
               XSOL(I,NV) =XSOL(I,NV) + ZC(LMAT-1+KI)*VECT(HCOL(KI),NV)
 50         CONTINUE
            XSOL(I,NV) =XSOL(I,NV) + ZC(LMAT+KFIN)*VECT(ADIA(I),NV)
 40      CONTINUE
 30   CONTINUE
      CALL JEDEMA()
      END
