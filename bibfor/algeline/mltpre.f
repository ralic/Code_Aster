...
C     DATA NOMCOL/'&&MLTPRE.TABLEAU_COLONNE'/
     ...
     &            NOMP15,NOMP16,NOMP17,NOMP18,NOMP19,NOMP20)
      ...
      CALL JEVEUO(NUZ(1:14)//'.SMOS.SMHC','L',ICOL)
      CALL JEVEUO(NUZ(1:14)//'.SMOS.SMDI','L',ISMDI)
      CALL JEVEUO(NUZ(1:14)//'.SMOS.SMDE','L',JSMDE)
...
C     ET N' EST PAS SURDIMENSIONNEE COMME SMOS.SMHC
   ...
   20 CONTINUE
      CALL JELIBE(NUZ(1:14)//'.SMOS.SMHC')
...
C     COPIE DE SMDI DANS DIAG
   ...
   40 CONTINUE
      CALL JELIBE(NUZ(1:14)//'.SMOS.SMDI')
      ZI(DIAG) = 0
...
C     OPTNUM =2 APPEL A METIS : MULTI LEVEL BISSECTION
      ...
      ELSE
        RENUM2 = RENUM
        CALL U2MESK('F','ALGELINE_91',1,RENUM2)
      END IF
      IF (OPTNUM.EQ.1) THEN
