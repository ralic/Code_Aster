...
C MODIF ALGELINE  DATE 04/05/2009   AUTEUR PELLET J.PELLET 
...
C ======================================================================
      ...
      INTEGER IRET,OPTNUM,IER,JRENU,JSLVK
...
C     DATA NOMCOL/'&&MLTPRE.TABLEAU_COLONNE'/
      ...
      NIVDBG=.FALSE.
      CALL JEMARQ()
      CALL DISMOI('F','NOM_NUME_DDL',MAT19,'MATR_ASSE',IBID,NU,IERD)
      CALL JELIRA(NU//'.SMOS.SMDI','CLAS',IBID,BASE)


C     -- RENUM : RENUMEROTATION SOUHAITEE POUR LA RESOLUTION
      IF (RENUMZ.EQ.' ') THEN
...
C     -- SI LE STOCKAGE STOC_MLTF EXISTE DEJA ET QU'IL CORRESPOND
C        A LA BONNE METHODE DE RENUMEROTATION, ON NE FAIT RIEN.
C        SINON, ON (RE)CONSTRUIT UN STOC_MLTF
C     ------------------------------------------------------------
      CALL JEEXIN(NU//'.MLTF.ADNT',IRET)
      IF (IRET.GT.0) THEN
        CALL JEVEUO(NU//'.MLTF.RENU','L',JRENU)
C       -- RENUM2: RENUMEROTATION ASSOCIEE A .MLTF
        RENUM2=ZK8(JRENU)
...
C         -- IL N'Y A RIEN A FAIRE :
      ...
      CALL MLNMIN(NU,NOMP01,NOMP02,NOMP03,NOMP04,NOMP05,NOMP06,NOMP07,
...
C     ET N' EST PAS SURDIMENSIONNEE COMME SMOS.SMHC
      CALL WKVECT(NOMADI,BASE//' V I ',LMAT,ISMHC)
      DO 20 I = 0,LMAT - 1
        ZI(ISMHC+I) = ZI(ICOL+I)
   20 CONTINUE
...
C------------------------------------------------------------
      ...
      CALL PREML0(NEQ,N2,ZI(DIAG),ZI(ISMHC),ZI(IDELG),ZI(IPRNO),
...
C     VOISINAGE DE TOUS LES DDL RELIES PAR UNE RELATION LINEAIRE
      ...
      CALL PREMLA(NEQ,ZI(DIAG),ZI(ISMHC),LGLIST,NRL,ZI(RL),ZI(DEB),
...
C
      CALL PREML1( NEQ,N2,
     &            ZI(DIAG),ZI(IDELG),ZI(ISMHC),ZI(XADJ2),ZI(ADJNC2),
...
C     NOUV, ANC : RENUMEROTATION ISSUE DU MD ENTRE TOUS LES DDL.
      ...
      CALL PREMLC(NEQ,ZI(DIAG),ZI(ISMHC),ZI(SEQ),ZI(PAREND),ZI(ANC),
...
C     QUI SERONT DANS NOMADI A LA PLACE DE COL
   ...
   90 CONTINUE
      ...
      CALL PREML2(NEQ,ZI(DIAG),ZI(ISMHC),ZI(IDELG),ZI(XADJ1),ZI(ADJNC1),
...
C      CALL JELIBE(NU//'.NUME.DELG')
      ...
      CALL JELIBE(NOMADI)
      IF(NIVDBG) THEN
C     ON APPELLE PRNCHK POUR VERIFIER LA COHERENCE
C     ENTRE L'ARBORESCENCE ET LA RENUMEROTATION
C     EN PARTICULIER A ACTIVER LORS D'UN PROBLEME AVEC METIS
C     COMME ON A DEJA VU AVEC DES ELEMENTS 3D FILAIRES OU DES
C     POUTRES (FICHES 10312 ET 10468)
      CALL PRNCHK(NBSN,ZI(ADRESS),ZI(GLOBAL),ZI(FILS),ZI(FRERE),
