...
C
      ...
      CHARACTER*1  TYPCST(2),TYPMAT(2)
      CHARACTER*8  NOMDDL
      CHARACTER*19 MATMOD
      REAL*8       RNORM, RX1, RX2, CONSTR(4), FR, AM
      COMPLEX*16   XX1, CMPL, XNORM,DCONJG
      CHARACTER*24 NMATR(2),NDYNAM
C     ------------------------------------------------------------------
      DATA        TYPCST/'C','C'/, TYPMAT/'R','R'/
      DATA        NOMDDL /'        '/
C     ------------------------------------------------------------------
C
      CALL JEMARQ()
...
C        --- DU PROBLEME GENERALISE ASSOCIE AU PROBLEME QUADRATIQUE ---
         ...
         DO 30 IM = 1, NBMODE
            ...
            ELSEIF( NORM.EQ.'RIGI_GENE') THEN
               ...
               CONSTR(4) = DIMAG(CMPL)
            ENDIF
            CALL MTCOMB(2,TYPCST,CONSTR,TYPMAT,NMATR,'C',NDYNAM,NOMDDL,
     .                  'V')
            CALL MCMULT('ZERO',LDYNAM,VECPRO(1,IM),'C',ZC(LACC1),1)
            XNORM = DCMPLX(0.0D0,0.0D0)
            DO 31 IEQ = 1, NEQ
