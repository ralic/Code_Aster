...
C
      ...
      CHARACTER*14 NUMDDL
      CHARACTER*19 NOMRC
      CHARACTER*24 REFEBA,FREQBA,MATRIA,GRPNO,COORNO,RCVALK,RCVALR
      CHARACTER*32 JEXNOM
C
...
C
      ...
      IF (IRET.EQ.0) CALL UTMESS('F','MODCOQ','ABSENCE DE RELATION '//
     &  'DE COMPORTEMENT DE TYPE <ELAS> POUR LE MATERIAU CONSTITUTIF '//
     &  'DE LA COQUE INTERNE')
...
C
  ...
  10  CONTINUE
      IF (IOK1.EQ.0 .OR. IOK2.EQ.0 .OR. IOK3.EQ.0) THEN
        CALL UTMESS('F','MODCOQ','ABSENCE D UN OU DE PLUSIEURS '//
     &    'PARAMETRES DE LA RELATION DE COMPORTEMENT <ELAS> POUR LE '//
     &    'MATERIAU CONSTITUTIF DE LA COQUE INTERNE')
      ELSE IF (YOUNG1.EQ.0.D0) THEN
        CALL UTMESS('F','MODCOQ','VALEUR DU MODULE D YOUNG NULLE '//
     &    'POUR LE MATERIAU CONSTITUTIF DE LA COQUE INTERNE')
      ENDIF
C
...
C
  ...
  20  CONTINUE
      IF (IOK1.EQ.0 .OR. IOK2.EQ.0 .OR. IOK3.EQ.0) THEN
        CALL UTMESS('F','MODCOQ','ABSENCE D UN OU DE PLUSIEURS '//
     &    'PARAMETRES DE LA RELATION DE COMPORTEMENT <ELAS> POUR LE '//
     &    'MATERIAU CONSTITUTIF DE LA COQUE EXTERNE')
...
C
          WRITE(KMOD,'(I3)') IMOD
          CALL UTMESS('A','MODCOQ','LES DEUX COQUES INTERNE ET '//
     &      'EXTERNE SONT EN MOUVEMENT POUR LE '//KMOD//'IEME MODE.')
C
          WRITE(IFM,510)
          WRITE(IFM,*)
          ICOQ = 1
