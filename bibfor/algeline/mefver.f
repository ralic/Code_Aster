...
C
         DO 20 I = 2,3
            ...
            IF(NORM.EQ.0.D0) THEN
               CALL UTMESS('F','MEFVER','UN COTE DE L ENCEINTE EST DE'//
     &                                'LONGUEUR NULLE')
            ENDIF
...
C
         ...
         ELSE IF(A(1).GT.A(2).AND.A(2).GT.PI) THEN
            ...
            IND(3) = 2
         ELSE
            CALL UTMESS('F','MEFVER','LES QUATRES SOMMETS DE '//
     &                     'L ENCEINTE NE FORMENT PAS UN RECTANGLE')
         ENDIF
C
         DO 30 I = 1,3
...
C
     ...
     &   CALL UTMESS('F','MEFVER','LES QUATRES SOMMETS DE L ENCEINTE'//
     &                            'NE FORMENT PAS UN RECTANGLE')
C
C
...
C
     ...
     &                  THEN
                   ...
                   CALL UTMESS('F','MEFVER','LE CYLINDRE '// NOTE //
     &                          ' DEBORDE DE L ENCEINTE RECTANGULAIRE')
      ...
      ELSE IF(IENCEI.EQ.1) THEN
         ...
         DO 100 I = 1,NBCYL
            ...
            IF((DIFF+RINT(I)).GT.REXT) THEN
               WRITE(NOTE(1:3),'(I3.3)') I
               CALL UTMESS('F','MEFVER','LE CYLINDRE '// NOTE //
     &                       ' DEBORDE DE L ENCEINTE CIRCULAIRE')
            ENDIF
 100     CONTINUE
      ENDIF
C
