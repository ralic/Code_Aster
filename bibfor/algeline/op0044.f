...
C       JEVEUX      : JEMARQ, WKVECT, JEVEUO, JEDETC, JEDEMA, JEDETR
C       SUPERVISEUR : GETRES, GETVID, GETVTX, GETVIS, GETVR8
C       MESSAGES    : UTMESS, INFMAJ, INFNIV,
C       UTILITAIRES : R8VIDE, R8DEPI, ISNNEM, OMEGA2, MTDEFS, MTDSCR,
C                     R8DEPI
C       VALEURS PROPRES : VPCREA, VPDDL, VPSTUR, VPECST, VPDICH, VPINTE,
C         WP1MUL, WP1INV, VP1PRO, VPWECF, VPPARA, VPCNTL
C     FONCTIONS INTRINSEQUES:
C       SIGN, MAX, MIN, ABS, SQRT.
C     ------------------------------------------------------------------
C     ASTER INFORMATIONS:
C      26/01/2000 TOILETTAGE FORTRAN, IMPLICIT NONE,
C                 MODIF. APPEL VPCNTL POUR VERIF. DE STURM ETENDUE,
C                 MODIF. APPEL WPECF POUR NOUVEL AFFICHAGE,
C                 CHANGEMENT DES APPELS (1.D0-PRECDC) AVEC SIGN,
C                 RAJOUT DE LA DECLARATION DE DCMPLX.
C      13/04/200 RAJOUT DE LA POSSIBLITE D'AVOIR EN ENTREE DES CHARGES
C                CRITIQUES NEGATIVES.
C                DANS LE CAS DU FLAMBEMENT ET POUR L OPTION PROCHE LES
...
C         VIA VP1PRO. MODIFICATION DE L'AFFICHAGE VIA VPWECF.
C-----------------------------------------------------------------------
C
      IMPLICIT NONE
...
C VARIABLES LOCALES
     ...
     &  IDET1, IEME1, IERX, IDET2, IEME2, IEQ, I
     ...
     &  LRESUI, LRESUR, LRESUK, LVALP, LVEC, LPR
      INTEGER MXFREQ, NCRITR, NPREC, NBRSS, NITSEP, NITAJU, NITV,
     &  NFREQR, NFREQ, NCRIT, NBMOD, NA1, NAMORR, NIV, NBCINE,
     &  NEQACT, NFREQB, MXRESF, NDIM, NPARR, NEQ
...
C     --- COMPATIBILITE DES MODES (DONNEES ALTEREES) ---
      CALL VPCREA( 0,MODES, MASSE, AMOR, RAIDE,I)

C     TYPE_RESU : 'DYNAMIQUE' OU 'FLAMBEMENT'
      CALL GETVTX(' ','TYPE_RESU',1,1,1,TYPRES,LTYPRE)
      CALL GETVR8('CALC_FREQ','CHAR_CRIT  ',1,1,0,RBID,NCRITR)
...
C     --- OPTION DES FREQUENCES ET DES MODES  ---
      CALL GETVTX('CALC_MODE','OPTION',1,1,1,OPTIOM,LMO)
      CALL GETVTX('CALC_FREQ','OPTION',1,1,1,OPTIOF,LMF)

C     --- RECUPERATION DES ARGUMENTS POUR LE CALCUL DES FREQUENCES ---
      CALL GETVIS('CALC_FREQ','NMAX_FREQ'       ,1,1,1,MXFREQ,L)
      CALL GETVR8('CALC_FREQ','PREC_SEPARE'     ,1,1,1,TOLSEP,L)
      CALL GETVIS('CALC_FREQ','NMAX_ITER_SEPARE',1,1,1,NITSEP,L)
      CALL GETVR8('CALC_FREQ','PREC_AJUSTE'     ,1,1,1,TOLAJU,L)
      CALL GETVIS('CALC_FREQ','NMAX_ITER_AJUSTE',1,1,1,NITAJU,L)
      CALL GETVR8('CALC_FREQ','SEUIL_FREQ'     ,1,1,1,FCORIG,L)
      CALL GETVR8('CALC_FREQ','PREC_SHIFT'     ,1,1,1,PRECDC,L)
      OMECOR = OMEGA2(FCORIG)

C     --- RECUPERATION DES ARGUMENTS POUR LE CALCUL DES MODES ---
      CALL GETVR8('CALC_MODE','PREC'     ,1,1,1,TOLV ,L)
      CALL GETVIS('CALC_MODE','NMAX_ITER',1,1,1,NITV ,L)
...
C     --- VERIFICATION ET CORRECTION DES DONNEES DE DECOUPAGE ---
     ...
     +           'UNE SEULE VALEUR DONNEE, SEPARATION IMPOSSIBLE')
         ELSEIF ( NBMOD .NE. 0 ) THEN
            ...
            IF (NFREQ .NE. 0) THEN
              CALL GETVR8('CALC_FREQ','FREQ',1,1,NFREQ,ZR(LBORNE),L)
            ELSE
              CALL GETVR8('CALC_FREQ','CHAR_CRIT',1,1,NFREQ,
     &                    ZR(LBORNE),L)
            ENDIF
            ERRF = -1.D0
     ...
     +      DE PROCHE')
      ...
      IF (OPTIOF.EQ.'PROCHE') THEN
         ...
         IF ((NAMORR.NE.0).AND.(NAMORR.NE.NFREQR)) THEN
            CALL UTMESS('E',NOMCMD//' (ERREUR)','NOMBRE DIFFERENT'//
...
C     --- VERIFICATION DES "REFE" ---
      ...
      IF (IRET .GT. 0 ) THEN
         CALL UTMESS('F',NOMCMD//'(ERREUR)','LES MATRICES "'//RAIDE//
     +         '" ET  "'//MASSE//'"  SONT INCOMPATIBLES ENTRE ELLES')
      ENDIF
      IF (LAMOR.NE.0) CALL VRREFE(MASSE,AMOR ,IRET)
      IF (IRET .GT. 0 ) THEN
         CALL UTMESS('F',NOMCMD//'(ERREUR)','LES MATRICES "'//AMOR //
     +        '"  ET  "'//MASSE//'"  SONT INCOMPATIBLES ENTRE ELLES')
      ENDIF
 ...
 4       CONTINUE
     ...
     +           'PRESENCE DE FREQUENCES NEGATIVES DANS LES DONNEES.')
      ...
      IF ( (NCRITR .NE. 0).AND.(NAMORR.EQ.0) ) THEN
         ...
         CALL GETVR8('CALC_FREQ','CHAR_CRIT',1,1,NCRIT,ZR(LBORNE),L)
      ENDIF
...
C     --- LISTE DES AMORTISSEMENTS (CAS QUADRATIQUE) ---
      IF ((NFREQR .NE. 0).AND.(NAMORR.NE.0)) THEN
         ...
         CALL GETVR8('CALC_FREQ','FREQ',     1,1,NFREQ,ZR(LFREQ),L)
         CALL WKVECT(CAMOR,'V V R',NFREQ,LAMORT)
         IF ( NA1 .NE. 0 ) THEN
          CALL GETVR8('CALC_FREQ','AMOR_REDUIT',1,1,NFREQ,ZR(LAMORT),L)
         ENDIF
     ...
     +               IEME1,IERX)
         IF (IERX .NE. 0 ) THEN
            ...
            IF (ICOMP.GT.NBRSS) THEN
               CALL UTMESS('A',NOMCMD,' TROP DE REAJUSTEMENT '//
 ...
 104     CONTINUE
     ...
     +               IEME2,IERX)
         IF (IERX .NE. 0 ) THEN
            ICOMP = ICOMP + 1
            IF (ICOMP.GT.NBRSS) THEN
               CALL UTMESS('A',NOMCMD,'ERREUR TROP DE REAJUSTEMENT'//
     +             'D''UNE BORNE DE L''INTERVALLE DE RECHERCHE.')
            ENDIF
...
C     --- INITIALISATION A UNDEF DE LA STRUCTURE DE DONNEES RESUF --
 ...
 302     CONTINUE
  ...
  310    CONTINUE
      ...
      ELSE
C        --- ERREUR ---
         CALL UTMESS('F',NOMCMD,'"'//OPTIOF//'"   OPTION INCONNUE.')
      ENDIF
