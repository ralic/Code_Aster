...
C VARIABLES LOCALES
     ...
     &  NEQACT, NFREQB, MXRESF, NDIM, NPARR, NEQ, ISLVK, ISLVI, JREFA
      ...
      REAL*8 TOLSEP, TOLAJU, TOLV, ERRF, FCORIG, OMECOR, PRECSH, OMEG,
...
C     --- VERIFICATION ET CORRECTION DES DONNEES DE DECOUPAGE ---
 ...
 2          CONTINUE
      ...
      ENDIF
      NA1=0
      IF(TYPRES.NE.'MODE_FLAMB'.AND.TYPRES.NE.'GENERAL') THEN
...
C         --- CONTROLE DE FREQUENCE NEGATIVE ---
 ...
 4       CONTINUE
      ...
      ENDIF
      IF ( (TYPRES.EQ.'MODE_FLAMB'.OR.TYPRES.EQ.'GENERAL').AND.
     &       (NAMORR.EQ.0) ) THEN
         NCRIT = -NCRITR
