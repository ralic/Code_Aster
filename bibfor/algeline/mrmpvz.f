      SUBROUTINE MRMPVZ( CUMUL,NOMMAT,ADIA,HCOL,ABLO,NEQ,NBBLOC,
     +                                                 VECT,XSOL,NBVECT)
      IMPLICIT REAL*8 (A-H,O-Z)
      CHARACTER*(*)       CUMUL
      INTEGER                  ADIA(*),HCOL(*),ABLO(*),NEQ,NBBLOC,NBVECT
      CHARACTER*(*)            NOMMAT
...
C             : LA MATRICE EST HERMITIENNE STOCKEE PROFIL PAR BLOC
C     ------------------------------------------------------------------
C
C
...
C
            DO 120 NV = 1,NBVECT
               Z8VAL = VECT(IEQUA,NV)
               DO 130 IL = 1, LM
                  XSOL(IV+IL,NV)=XSOL(IV+IL,NV)+Z8VAL*ZC(IA+IL)
  130          CONTINUE
C
               Z8VAL = Z8ZERO
               DO 140 IL = 1,LM
                  Z8VAL = Z8VAL + DCONJG(ZC(IA+IL)) * VECT(IV+IL,NV)
  140          CONTINUE
C
               XSOL(IEQUA,NV) = XSOL(IEQUA,NV) + Z8VAL +
     +                                    ZC(IA+LONG)*VECT(IEQUA,NV)
  120       CONTINUE
  110    CONTINUE
