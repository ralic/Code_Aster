...
C MODIF ALGELINE  DATE 26/03/2012   AUTEUR PELLET J.PELLET 
C ======================================================================
...
C     -----  FIN  COMMUNS NORMALISES  JEVEUX  --------------------------
      ...
      CHARACTER*8 NOMNO,NOMCMP,TYDDL,METRES
      CHARACTER*24 KPIV
      CHARACTER*40 INFOBL,VALK(7)
...
C     ------------------------------------------------------------------
      ...
      IF (METRES.NE.'LDLT' .AND. METRES.NE.'MULT_FRO' .AND.
     &    METRES.NE.'MUMPS') THEN
...
C          DE .VALM DANS .UALF
     ...
     &                  NBBLOC/2,ILDEB,ILFIN1,EPS)
      ...
      ELSEIF (METRES.EQ.'MULT_FRO') THEN
C     ---------------------------------------
        IF (TYPVAR.EQ.1) THEN
...
C        AMONT DS AMUMPH. ON NE SAIT PAS PRECISER ISINGUCONTRAIREMENT A
C        MF/LDLT. ON MET ISINGU=-999 ARBITRAIREMENT)
          NDECI=-999
          ISINGU=-999
          NPIVOT=-999
        ELSE
...
C     -- CALCUL DU CODE RETOUR: IRETZ,NDECIET ISINGU:
C     ------------------------------------------------
...
C     ----------------------------------------------
      ...
      ELSEIF (IRETZ.EQ.0) THEN
        GOTO 20

      ELSEIF (ISTOP.EQ.1) THEN
        IF (IRETZ.EQ.1) THEN
          CODMES='A'
        ELSEIF (IRETZ.EQ.2) THEN
          CODMES='F'
        ELSE
          CALL ASSERT(.FALSE.)
        ENDIF
      ELSEIF (ISTOP.EQ.0) THEN
        CODMES='F'
      ENDIF
      ...
      IF (ISINGU.GT.0) THEN
        CALL RGNDAS(NU,ISINGU,NOMNO,NOMCMP,TYDDL,LIGREL,INFOBL)
        VALK(4)=TYDDL
        IF (TYDDL.EQ.'A') THEN
...
C         -- SI C'EST UN SIMPLE BLOCAGE DE DDL PHYSIQUE :
          VALK(3)=INFOBL
...
C           -- CAS NUME_DDL_GENE :
        ...
        ENDIF
      ENDIF
      CALL U2MESG(CODMES,'FACTOR_10',7,VALK,2,VALI,0,0.D0)
...
C       -- ALARME EVENTUELLE SI LE PIVOT DEVIENT TROP GRAND :
     ...
     &      ' A LA LIGNE:',IEQ4
          WRITE (IFM,*)'<FACTOR> NOMBRE DE DECIMALES PERDUES:',NDECI2
          CALL RGNDAS(NU,IEQ4,NOMNO,NOMCMP,TYDDL,LIGREL,INFOBL)
