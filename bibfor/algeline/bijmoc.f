...
C  IN : IMOD   : INDICE DU MODE CONSIDERE POUR LE PROBLEME FLUIDE
C  IN : JMOD   : INDICE DU SECOND MODE CONSIDERE
...
C                CALCULE LE TERME (I,J) DE LA MATRICE B
...
C
      REAL*8      LONG
      COMPLEX*16  BIJ1,BIJ2,BIJ11,BIJ21,BIJ12,BIJ22
C
C-----------------------------------------------------------------------
C
C-----------------------------------------------------------------------
...
C
     ...
     &                  RKI,TCOEF,S1,S2,ZC(IYSOL))
            CALL BIJSOM(UMOY,RHOF,R1,R2,LONG,CF0,ICOQ,JCOQ,JMOD,NBM,RKI,
     &                  THETAI,THETAJ,TCOEF,ZC(IYSOL),BIJ)
          ELSE
...
C
     ...
     &                  RKI,TCOEF,S1,S2,ZC(IYSOL))
            IF (KI.EQ.K1J) THEN
              CALL BIJSOM(UMOY,RHOF,R1,R2,LONG,CF0,ICOQ,1,JMOD,NBM,RKI,
     &                    THETAI,THET1J,TCOEF,ZC(IYSOL),BIJ1)
            ELSE
              BIJ1 = DCMPLX(0.D0,0.D0)
            ENDIF
            IF (KI.EQ.K2J) THEN
              CALL BIJSOM(UMOY,RHOF,R1,R2,LONG,CF0,ICOQ,2,JMOD,NBM,RKI,
     &                    THETAI,THET2J,TCOEF,ZC(IYSOL),BIJ2)
            ELSE
...
C
     ...
     &                  RK1I,TCOEF,S1,S2,ZC(IYSOL1))
            CALL BIJSOM(UMOY,RHOF,R1,R2,LONG,CF0,1,JCOQ,JMOD,NBM,RK1I,
     &                  THET1I,THETAJ,TCOEF,ZC(IYSOL1),BIJ1)
          ELSE
            BIJ1 = DCMPLX(0.D0,0.D0)
          ENDIF
C
     ...
     &                  RK2I,TCOEF,S1,S2,ZC(IYSOL2))
            CALL BIJSOM(UMOY,RHOF,R1,R2,LONG,CF0,2,JCOQ,JMOD,NBM,RK2I,
     &                  THET2I,THETAJ,TCOEF,ZC(IYSOL2),BIJ2)
          ELSE
            BIJ2 = DCMPLX(0.D0,0.D0)
          ENDIF
C
