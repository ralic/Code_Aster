...
!     DO WHILE (SN.NE.0)
            if (sn .ne. 0) then
                ...
                call mltacp(lfront(sn), nb, adper, zc(adfacl), pile( adpile(sn)),&
                            local(adress(sn)+nl))
                call mltacf(lfront(sn), nb, adper, pile(itemp), pile( adpile(sn)),&
                            local(adress(sn)+nl), p)
                if (typsym .eq. 0) then
                    call mltacp(lfront(sn), nb, adper, zc(adfacu), pile(adpile(sn)+lm2),&
                                local(adress(sn)+nl))
                    call mltacf(lfront(sn), nb, adper, pile(itemp+lm1), pile(adpile(sn)+lm2),&
                                local(adress(sn)+nl), p)
                endif
                sn = frere(sn)
                goto 40
