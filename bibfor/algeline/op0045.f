...
C     NFREQ : NOMBRE DE MODES DEMANDES
      CALL GETVIS('CALC_FREQ','NMAX_FREQ',1,IARG,1,NFREQ,L)

C     --- RECUPERATION PARAM ESPACE REDUIT ---
C     NBVECT, NBVEC2 : DIMENSION/COEF MULTIPLICATEUR DE L'ESPACE REDUIT
      NBVECT = 0
      CALL GETVIS('CALC_FREQ','DIM_SOUS_ESPACE',1,IARG,1,NBVECT,L)
      NBVEC2 = 0
      CALL GETVIS('CALC_FREQ','COEF_DIM_ESPACE',1,IARG,1,NBVEC2,L)
...
C     --- LISTES DES FREQUENCES/CHARGES CRITIQUES ---
      CALL GETVR8('CALC_FREQ','FREQ',1,IARG,0,RBID,NNFREQ)
      CALL GETVR8('CALC_FREQ','CHAR_CRIT',1,IARG,0,RBID,NNCRIT)
      IF (NNFREQ.LT.0) THEN
         NNFREQ = -NNFREQ
         CALL WKVECT(CBORFR,' V V R',NNFREQ,LBORFR)
         CALL GETVR8('CALC_FREQ','FREQ',1,IARG,NNFREQ,ZR(LBORFR),L)
      ELSE
      ...
      IF (NNCRIT.LT.0) THEN
         ...
         CALL GETVR8('CALC_FREQ','CHAR_CRIT',1,IARG,NNCRIT,ZR(LBORCR),L)
      ELSE
...
C     UTILISE SI LMAMOR.NE.0 ET SI METHODE.NE.QZ
      CALL GETVTX('CALC_FREQ','APPROCHE',1,IARG,1,APPR,IBID)
