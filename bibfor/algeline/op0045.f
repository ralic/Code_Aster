...
C     --- CONTROLE DE LA COHERENCE OPTION/FREQ OU CHAR_CRIT ---
      IF (TYPRES .EQ. 'DYNAMIQUE') THEN
        CALL VPVOPT(OPTIOF,TYPRES,NNFREQ,NFREQ,ZR(LBORFR),IERX)
      ELSE
        CALL VPVOPT(OPTIOF,TYPRES,NNCRIT,NFREQ,ZR(LBORCR),IERX)
      ENDIF
...
C        --- CAS SANS AMORTISSEMENT : PROBLEME GENERALISE ---
     ...
     +             NITJAC,NITBAT)
            CALL RECTFR(NFREQ,NBVECT,NEQ,OMESHI,NPIVOT,NBLAGR,
...
C        --- CAS AVEC AMORTISSEMENT : PROBLEME QUADRATIQUE ---
     ...
     &                  ZR(IADY),ZR(IADZ),2*NBVECT,NPIVOT,ZR(LSURDR),
     &                  ZR(LDIAGR),ZC(LVEC),LMASSE,MXRESF,
