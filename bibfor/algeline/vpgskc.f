...
C DECLARATION PARAMETRES D'APPELS
      INTEGER      NBEQ, NCONV, LMATB, TYPEPS, DDLEXC(*)
      COMPLEX*16   VECT(NBEQ,*), VAUX(*)
      REAL*8       ALPHA, DELTA(*)
...
C NORMALISATION DU PREMIER VECTEUR PROPRE
      IF (TYPEPS.NE.0) THEN
        CALL MCMULT('ZERO',LMATB,VECT(1,1),'C',VAUX,1)
      ELSE
        DO 5 I=1,NBEQ
          VAUX(I) = VECT(I,1)
    5   CONTINUE
      ENDIF
      RAUX = DCMPLX(0.D0,0.D0)
      DO 10 I=1,NBEQ
        RAUX = RAUX + VECT(I,1) * VAUX(I) * DDLEXC(I)
   10 CONTINUE
      IF (TYPEPS.EQ.1) THEN
        DELTA(1)= SIGN(1.0D0,MAX(EPS,DBLE(RAUX)))
      ELSE
        DELTA(1) = 1.D0
      ENDIF
      RAUX = DELTA(1) / ZSQRT(RAUX)
      DO 15 I=1,NBEQ
...
C CALCUL (VJ,VI) ET ||VI|| (STEP 1)/ (VJ,VI+) (STEP 2 SI NECESSAIRE)
   ...
   20     CONTINUE
          ...
          IF (TYPEPS.NE.0) THEN
            CALL MCMULT('ZERO',LMATB,VECT(1,I),'C',VAUX,1)
          ELSE
            DO 25 K=1,NBEQ
              VAUX(K) = VECT(K,I)
   25       CONTINUE
...
C PREMIER TEST
     ...
     &      -EPS)).AND.(STEP.LE.2)) THEN
            GOTO 60
          ELSE IF ((STEP.GE.1).AND.(STEP.LT.2)) THEN
            RAUOLD = RAUX
            GOTO 20
   ...
   60     CONTINUE
          IF (TYPEPS.EQ.1) THEN
            DELTA(I)= SIGN(1.0D0,MAX(EPS,DBLE(RAUX)))
          ELSE
            DELTA(I) = 1.D0
          ENDIF
          RAUX = DELTA(I) / ZSQRT(RAUX)
          DO 65 K=1,NBEQ
