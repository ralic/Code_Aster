...
C ----   ET DU TEMPS
C        -----------
        CALL RCVALA(ZI(IMATE),'ELAS',0,NOMPAR,VALPAR,NBV,NOMRES,VALRES,
     &              CODRET,'FM')
        E = VALRES(1)
        NU = VALRES(2)
...
C       RECUPERATION ET AFFECTATION DU VECTEUR EN SORTIE
C       LES NOEUDS 1 2 3 4 DE LA FACE INF PRENNENT LA VALEUR
C       DU PT D INTEGRATION NO 1
        CALL SHBCSF(ZR(ICONT),SIGMA,FSTAB)
        DO 120 I = 1,4
          DO 110 J = 1,NBSIG
            ZR(ICHN+NBSIG* (I-1)+J-1) = SIGMA(J)
  110     CONTINUE
  120   CONTINUE
C       LES NOEUDS 5 6 7 8 DE LA FACE SUP PRENNENT LA VALEUR
C       DU PT D INTEGRATION NO 5
        DO 140 I = 5,8
          DO 130 J = 1,NBSIG
            ZR(ICHN+NBSIG* (I-1)+J-1) = SIGMA(NBSIG*4+J)
  130     CONTINUE
  140   CONTINUE
