...
C  ===========================================
   ...
   20     CONTINUE
   30   CONTINUE
        CALL SHB8PS(IMATE,OPTION,ZR(IGEOM),D,WORK,
     &  WORK(100),RE,WORK(150))
...
C        VECTEUR DES CONTRAINTES AUX POINTS D'INTEGRATION
        CALL SHB8PS(IMATE,OPTION,ZR(IGEOM),D,WORK,ZR(IDEPL),
     &  WORK(150),SIGMA)
C        RECUPERATION ET AFFECTATION DU VECTEUR EN SORTIE
...
C        RECUPERATION DES CONTRAINTES DANS ZR(ICONT)
        ...
        CALL SHB8PS(IBID,OPTION,ZR(IGEOM),WORK(20),
     &  WORK,SIGMA,RE,WORK)
...
C           FORCES DE STABIBILISATION DANS CONTM(7,7+12)
        ...
        CALL SHB8PS(IMATE,SOPTIO,ZR(IGEOM),WORK,WORK(101),
...
C ----       LAG=1 LAGRANGIEN TOTAL (EPS=EPSLIN+EPSNL)
  ...
  170     CONTINUE
          ...
          CALL SHB8PS(IMATE,SOPTIO,ZR(IGEOM),D,WORK,
     &              ZR(IDEPLP),WORK(150),SIGMA)
...
C           NB ITERATIONS MAX AUTORISEES
  ...
  210       CONTINUE
            ...
            CALL SHB8PS(IMATE,SOPTIO,ZR(IGEOM),D,WORK,VARINT,
     &      WORK(150),SIGMA)
            DO 230 IP = 1,NPG
              DO 220 I = 1,2
...
C ----   RIGIDITE ELASTIQUE
          SOPTIO = 'RIGI_MECA       '
          CALL SHB8PS(IMATE,SOPTIO,ZR(IGEOM),D,WORK,WORK(100),
     &    RE,WORK(150))
          IF (ZK16(ICOMPO+2) (1:5).NE.'PETIT') THEN
C ----   RIGIDITE ELASTIQUE + RIGIDITE GEOMETRIQUE
            ...
            IF (OPTION(1:16).EQ.'RIGI_MECA_TANG') THEN
              CALL SHBCSF(ZR(ICONTM),SIGMA,FSTAB)
              CALL SHB8PS(IMATE,SOPTIO,ZR(IGEOM),WORK(20),WORK,SIGMA,
     &        RE,WORK)
...
C ----      ZR(IVECTU) : FORCES INTERNES FIN DE PAS
  ...
  300       CONTINUE
          ...
          SOPTIO = 'FORC_NODA       '
          CALL SHBCSF(ZR(ICONTM),SIGMM,FSTAB)
          CALL SHB8PS(IMATE,SOPTIO,ZR(IGEOM),WORK,UTOT,SIGMP,
     &              FSTAB,ZR(IVECTU))
