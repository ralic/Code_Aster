      SUBROUTINE TURIGI(OPTION,ELREFE,NBRDDL,NVEC,TNVEC,B,K,PASS,KTEMP)
      IMPLICIT NONE
...
C ======================================================================
      ...
      CHARACTER*8  ELREFE
      INTEGER NBRDDL,NC
      REAL*8 NVEC(6,NBRDDL),TNVEC(NBRDDL,6),B(4,NBRDDL)
      REAL*8 K(NBRDDL,NBRDDL)
...
C                      NVEC, TNVEC, B, MASS1,MASS,K : MATRICES
...
C     VARIABLES LOCALES
      ...
      INTEGER IGAU,ICOU,ISECT,I,J,IFF,LORIEN,ICOUD2,MMT
      INTEGER IXF,NPG1,ITABM(8)
...
C          ------------------------------------------------------------
      CALL TECAC2 ('ONN','PTEMPER',8,ITABM,IRET)
      ITEMP=ITABM(1)
      IF (ITEMP.GT.0) THEN
        NBPAR = 1
        NOMPAR = 'TEMP'
...
C  PASSAGE DU REPERE LOCAL AU REPERE GLOBAL
      ...
      IF (ICOUDE.EQ.0) THEN
        CALL KLG(NNO,NBRDDL,PGL,K,PASS,KTEMP)
      ELSE IF (ICOUDE.EQ.1) THEN
        CALL KLGCOU(NNO,NBRDDL,PGL1,PGL2,PGL3,PGL4,K,PASS,KTEMP)
      END IF
