...
C------------FIN  COMMUNS NORMALISES  JEVEUX  --------------------------
      ...
      INTEGER  JGEOM,IPG,INO,JDEPM,ISIG,JSIG,IDENER,NBSP
...
C ---     PASSAGE DES CONTRAINTES DANS LE REPERE INTRINSEQUE :
          CALL COSIRO(NOMTE,'PCONTRR','UI','G',ICONTP,'S')
        ELSE
      ...
      ELSE IF(OPTION(1:4) .EQ. 'ENER') THEN
        ...
        IF (.NOT.DKG) THEN
C ---     PASSAGE DES CONTRAINTES DANS LE REPERE INTRINSEQUE :
          CALL COSIRO(NOMTE,'PCONTPR','UI','G',ICONTP,'S')
        ELSE
...
C     =======================================
 ...
 110       CONTINUE
        ELSE
          NCMP = 1
          NBSP = 1
          CALL PPGAN2 ( JGANO, NBSP, NCMP, ENELT, AUXT)
          CALL PPGAN2 ( JGANO, NBSP, NCMP, ENELM, AUXM)
          CALL PPGAN2 ( JGANO, NBSP, NCMP, ENELF, AUXF)
          DO 120 I = 1, NNOEL
