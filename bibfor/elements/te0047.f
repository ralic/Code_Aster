...
C --- RECUPERATION DES ORIENTATIONS (ANGLES NAUTIQUES -> VECTEUR ANG)
      ...
      CALL TECACH ( .TRUE., .FALSE.,'PCAORIE', 1, LORIEN )
      IF ( LORIEN .EQ. 0 ) THEN
...
C        MATRICE DE RIGIDITE EN REPERE LOCAL
        ...
        IF (OPTION.EQ.'FULL_MECA' .OR. OPTION.EQ.'RAPH_MECA') THEN
          ...
          CALL JEVECH('PVARIPR','E',IVARIP)
C
          CALL TECACH(.TRUE.,.FALSE.,'PTEMPMR',1,ITEMPM)
          CALL TECACH(.TRUE.,.FALSE.,'PTEMPPR',1,ITEMPP)
...
C           SAUF SUIVANT Y LOCAL : FROTTEMENT DE COULOMB
          ...
          CALL DICOUL(OPTION,ZK16(ICOMPO),NNO,NBT,NEQ,NC,ZI(IMATE),
     &                ZR(IGEOM),ULM,DUL,ZR(ICONTM),ZR(JTP),ZR(IVARIM),
...
C --- PARAMETRES EN ENTREE
   ...
   30   CONTINUE
        ...
        CALL TECACH(.TRUE.,.FALSE.,'PVITPLU',1,IVITP)
        IF (IVITP.NE.0) THEN
   ...
   40     CONTINUE
        END IF
        CALL TECACH(.TRUE.,.FALSE.,'PDEPENT',1,IDEPEN)
        IF (IDEPEN.NE.0) THEN
   ...
   50     CONTINUE
        END IF
        CALL TECACH(.TRUE.,.FALSE.,'PVITENT',1,IVITEN)
        IF (IVITEN.NE.0) THEN
...
C --- RELATION DE COMPORTEMENT DE CHOC
        ...
        CALL DICHOC(NBT,NEQ,NNO,NC,ZI(IMATE),ULM,DUL,ULP,ZR(IGEOM),PGL,
     &              ZR(JINST),ZR(ICONTM),KLV,KTY2,DULY,DVL,DPE,DVE,FOR2,
     &              FOR3,VARMO,VARPL)
...
C           SAUF SUIVANT Y LOCAL : ELASTOPLASTIQUE VMIS_ISOT_TRAC
        ...
        CALL DIGOUJ(OPTION,ZK16(ICOMPO),NNO,NBT,NEQ,NC,ZI(IMATE),ULM,
