...
C          SUR DES FACES D'ELEMENTS ISOPARAMETRIQUES 3D ET 3D_MIXTE
C
...
C
          II = 4*I-3
C
          ZC(IVECTT+II-1) = ZC(IVECTT+II-1) + JAC *
     &    ZR(IPOIDS+IPG-1) * ZC(IVITN+IPG-1) * ZR(IVF+LDEC+I-1)
     &    *RHO
203       CONTINUE
        ELSE
C
          DO 103 I=1,NNO
          ZC(IVECTT+I-1) = ZC(IVECTT+I-1) + JAC *
     &    ZR(IPOIDS+IPG-1) * ZC(IVITN+IPG-1) * ZR(IVF+LDEC+I-1)
     &    *RHO
103       CONTINUE
        ENDIF
