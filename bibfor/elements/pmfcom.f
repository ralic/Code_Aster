...
C     ------------------------------------------------------------------
     ...
     &     'EP_SUR_E','A1_PM','A2_PM','ELAN','A6_PM','C_PM','A_PM'/
      ...
      COMPO=COMPOR(2)
      ALGO=COMPOR(3)
C      PRINT*,'PMFCOM ',MATERI,COMPO,ALGO,TEMPM,TEMPP
C -- PARTIE ELASTIQUE DU COMPORTEMENT (TEMPERATURE MOINS)
      CALL MATELA(ICDMAT,MATERI,1,TEMPM,E,NU,ALPHA)
...
C ---   ON RECUPERE LES PARAMETRES MATERIAU
        ...
        VALPAR = 0.D0
        NBRES = 9
        CALL RCVALA(ICDMAT,MATERI,'LABORD_1D',NBPAR,NOMPAR,VALPAR,NBRES,
...
C ---   BOUCLE COMPORTEMENT SUR CHAQUE FIBRE
   ...
   20   CONTINUE
      ELSE IF (COMPO.EQ.'PINTO_MENEGOTTO') THEN
        ...
        NOMPAR = '  '
        VALPAR = 0.D0
        CALL RCVALA(ICDMAT,MATERI,'PINTO_MENEGOTTO',NBPAR,NOMPAR,VALPAR,
   ...
   30   CONTINUE
        DO 40 I = 1,NF
          IVARI = NBVALC* (I-1) + 1
          CALL NM1DPM('RIGI',KPG,I,OPTION,NBVALC,ALPHA,13,CSTPM,
     &                CONTM(I),VARIM(IVARI),DEFP(I),VARIP(IVARI),
     &                SIGF(I),MODF(I))
   40   CONTINUE
...
C --- CARACTERISTIQUES ELASTIQUES A TMOINS
      ...
      VALPAR = TEMPM
      CALL RCVALA(ICDMAT,MATERI,'ELAS',NBPAR,NOMPAR,VALPAR,1,'E',EM,
     & CODRES,ARRET)
      CALL RCVALA(ICDMAT,MATERI,'ELAS',NBPAR,NOMPAR,VALPAR,1,'ALPHA',
...
C --- CARACTERISTIQUES ELASTIQUES A TPLUS
      ...
      VALPAR = TEMPP
      CALL RCVALA(ICDMAT,MATERI,'ELAS',NBPAR,NOMPAR,VALPAR,1,'E',EP,
     &            CODRES,ARRET)
      CALL RCVALA(ICDMAT,MATERI,'ELAS',NBPAR,NOMPAR,VALPAR,1,'ALPHA',
     &            ALPHAP,CODRES,RETOUR)
      IF (CODRES(1).NE.'OK') ALPHAP = 0.D0
        IF (COMPO.EQ.'VMIS_CINE_LINE') THEN
          DO 50 I = 1,NF
            IVARI = NBVALC* (I-1) + 1
        CALL NM1DCI('RIGI',KPG,I,ICDMAT,EM,EP,ALPHAM,ALPHAP,CONTM(I)
     &       ,DEFP(I),VARIM(IVARI),OPTION,MATERI,SIGF(I),VARIP(IVARI),
     &       MODF(I))
   50     CONTINUE
        ELSE IF (COMPO.EQ.'VMIS_ISOT_LINE') THEN
          DO 60 I = 1,NF
            IVARI = NBVALC* (I-1) + 1
        CALL NM1DIS('RIGI',KPG,I,ICDMAT,EM,EP,ALPHAM,ALPHAP,CONTM(I)
     & ,DEFP(I),VARIM(IVARI),OPTION,COMPO,MATERI,SIGF(I),VARIP(IVARI),
     &   MODF(I))
   60     CONTINUE
