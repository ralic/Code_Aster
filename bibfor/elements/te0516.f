...
C     NPG : NOMBRE DE POINTS DE GAUSS
      ...
      REAL*8 DEFAM(6),DEFAP(6),DUP(6),XUGM(6),XDM(3),DGAMMA
...
C     NOMBRE DE COMPOSANTES D'EFFORTS GENERALISES
      NCOMP2 =  7
C     NOMBRE DE COMPOSANTES DE DEFORMATION     = 7
...
100   CONTINUE
      ...
      IF ( REACTU ) THEN
C       INCREMENT DE DEPLACEMENT
        DO 115 I = 1,3
          DUP(I)=ZR(IDEPLP-1+I)
          DUP(I+3)=ZR(IDEPLP-1+I+7)
115     CONTINUE
C       CALCUL DU VECTEUR XLOCAL EN TEMPS T+
        DO 110 I = 1,3
...
C       CALCUL DU VECTEUR XLOCAL EN TEMPS T-
        DO 116 I = 1,3
          XUGM(I) = UTG(I) + ZR(IGEOM-1+I)-DUP(I)
          XUGM(I+3) = UTG(I+7) + ZR(IGEOM-1+I+3)-DUP(I+3)
116     CONTINUE
...
C       CORRECTION DE GAMMA
        ...
        ANG1(3) = GAMMA1
        IF (VECTEU)THEN
            ZR(ISTRXP+16-1) = ALFA1
            ZR(ISTRXP+17-1) = BETA1
            ZR(ISTRXP+18-1) = GAMMA1 - (TET1+TET2)/2.D0
        ENDIF
...
C       LES DEFORMATIONS SONT STOCKES A PARTIR DE LA 8 POSISITION
