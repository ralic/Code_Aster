...
C     NPG : NOMBRE DE POINTS DE GAUSS
      ...
      REAL*8 KLV(DIMKLV),WORK(NC,2*NC),CORRM,CORRP
      REAL*8 CO(NPG),EPSM
      ...
      INTEGER ITREF,I,ITEMP,ITEMPM,ITEMPP,JCRET
      ...
      INTEGER NCARFI,NBFIB,JACF,JTAB(7),IVARMP,CODRET,ICORRM,ICORRP
...
C
      ...
      IF (JTAB(7).NE. (NBFIB+NCOMP)) CALL UTMESS('F','TE0946','STOP1')
      ICONTM = JTAB(1)

      CALL TECACH('OON','PVARIMR',7,JTAB,IRET)
      IF (JTAB(7) .NE. (NBFIB+NCOMP)) CALL UTMESS('F','TE0946','STOP2')
      IVARIM = JTAB(1)

      IF (VECTEU) THEN
         CALL TECACH('OON','PVARIMP',7,JTAB,IRET)
         IF (JTAB(7).NE.(NBFIB+NCOMP)) CALL UTMESS('F','TE0946','STOP3')
         IVARMP = JTAB(1)
      ENDIF
...
C --- DEFORMATIIONS MOINS ET INCREMENT DE DEFORMATION POUR CHAQUE FIBRE
      CALL WKVECT('&&TE0516.DEFMFIB','G V R8',NBFIB,JDEFM)
      CALL WKVECT('&&TE0516.DEFPFIB','G V R8',NBFIB,JDEFP)
...
C --- MODULE ET CONTRAINTES SUR CHAQUE FIBRE (COMPORTEMENT)
      CALL WKVECT('&&TE0516.MODUFIB','G V R8',NBFIB,JMODFB)
      CALL WKVECT('&&TE0516.SIGFIB' ,'G V R8',NBFIB,JSIGFB)
      CALL WKVECT('&&TE0516.VARFIB' ,'G V R8',NBFIB*NBVALC,JVARFB)
...
310     CONTINUE
        ...
        IF ((ALPHAP.NE.0.D0).AND.(ITEMP.NE.0) ) THEN
            F = ALPHAM*(TEMM-ZR(ITREF))
           DF = ALPHAP*(TEMP-ZR(ITREF))-ALPHAM*(TEMM-ZR(ITREF))
            EPS(1) =  EPS(1) -  F
           DEPS(1) = DEPS(1) - DF
        ENDIF
...
C         ENDIF
        ...
        CALL PMFCOM(OPTION,ZK16(ICOMPO),NBFIB,ZR(ITREF),TEMM,TEMP,
     &            E,ALPHAP,ZI(IMATE),NBVALC,
     &            ZR(IVARIM+NBFIB*NBVALC*(KP-1)),
     &            ZR(IVARMP+NBFIB*NBVALC*(KP-1)),
     &            ZR(ICONTM+(NBFIB+NCOMP)*(KP-1)),
     &            ZR(JDEFM),ZR(JDEFP),
     &            ZR(JMODFB),ZR(JSIGFB),ZR(JVARFB),CODREP,
     &            EPSM,CORRM,CORRP)
