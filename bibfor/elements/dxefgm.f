...
C
     ...
     +    NOMTE(1:8).EQ.'MEDKTG3 ' ) THEN
        CALL DKTEDG(NOMTE,XYZL,OPTION,PGL,DEPL,EFFG,MULTIC,GRILLE)
C
      ELSE IF (NOMTE(1:8).EQ.'MEDSTR3 ') THEN
        CALL DSTEDG(NOMTE,XYZL,OPTION,PGL,DEPL,EFFG)
C
      ELSE IF (NOMTE(1:8).EQ.'MEDKQU4 '.OR.
     +         NOMTE(1:8).EQ.'MEDKQG4 ') THEN
        CALL DKQEDG(NOMTE,XYZL,OPTION,PGL,DEPL,EFFG)
C
      ELSE IF (NOMTE(1:8).EQ.'MEDSQU4 ') THEN
        CALL DSQEDG(NOMTE,XYZL,OPTION,PGL,DEPL,EFFG)
C
      ELSE IF (NOMTE(1:8).EQ.'MEQ4QU4 ') THEN
        CALL Q4GEDG(NOMTE,XYZL,OPTION,PGL,DEPL,EFFG)
C
      ELSE
        CALL UTMESS('F','DXEFGM','LE TYPE D''ELEMENT : '//NOMTE(1:8)//
     +              'N''EST PAS PREVU.')
      END IF
C
      END
