      SUBROUTINE INI080(NOMTE,CODE,NMAX,ITABL,K24TAB,NVAL)
...
C    ON CONSTRUIT 3 TABLEAUX POUR CHAQYE ELEMENT (MEC3QU9H,MEC3TR7H)

C    DESI='&INEL.'//NOMTE(1:8)//'.DESI' CONTIENT :
C                MEC3QU9H    MEC3TR7H
C    NBN1        8           6
C    NBN2        9           7
C    NPGSR       4           3
...
C   DESR='&INEL.'//NOMTE(1:8)//'.DESR' (LONGUEUR 2100) CONTIENT :
C ======================================================================
...
C DEB ------------------------------------------------------------------
      ...
      IF (NOMTE(1:8).EQ.'MEC3QU9H' .OR. NOMTE(1:8).EQ.'MEGRC3Q9') THEN
        ...
        NPGSN = 9
        NSO = 4
      ELSE IF (NOMTE(1:8).EQ.'MEC3TR7H' .OR.
     &         NOMTE(1:8).EQ.'MEGRC3T7') THEN
        ...
        NSO = 3
      ELSE
        CALL UTMESS('F','INI080_1',' ELEMENT NON PREVU')
      END IF
...
C     PARTIR DE LEURS VALEURS AUX POINTS DE GAUSS
        ...
        DO 150 M = 1,3
          ...
          DO 140 L = 1,8
            ...
            KOMPT = KOMPT + 1
            LL = 8* (KOMPT-1)
            CALL FCESND(NOMTE(1:8),1,XI1,XI2,XI3,'LI',VFESND)
            DO 130 K = 1,8
              I4 = 1260 + LL + K
  ...
  150   CONTINUE
        ...
        DO 170 L = 1,8
          ...
          XI1 = ZR(LZR-1+I1)
          XI2 = ZR(LZR-1+I2)
          CALL FCESND(NOMTE(1:8),0,XI1,XI2,XI3,'LI',VFESND)
          LL = 4* (L-1)
          DO 160 K = 1,4
            I3 = 1452 + LL + K
...
C     PTS DE GAUSS AUX NOEUDS PAR MOINDRES CARRES
        ...
        CALL WKVECT('&INEL.'//NOMTE//'.B','G V R',LJMAS,JMAS)

        CALL MAMAGI(NOMTE(1:8),ZR(LZR),ZR(JMAS))
