...
C     -----  FIN  COMMUNS NORMALISES  JEVEUX  --------------------------
      ...
      PARAMETER (NDDL=12,NL=NDDL*NDDL)
      PARAMETER (NBRES=3)
      ...
      REAL*8 E, RHO, KGB(NDDL,NDDL) , VG(NDDL*NDDL)
...
C        CALL UTPVLG(NNO,NC,PGL,KLV,ZR(LMAT))
        CALL UTPVLG(NNO*NNO*NC,NC,PGL,KLV,ZR(LMAT))
C      do ijkl=1,144 
C         ZR(LMAT-1+ijkl)=KLV(ijkl)
C      enddo
C        CALL UTBTAB('ZERO',NNO*NC,NNO*NC,PGL,KLV,WK,KGB)
C        DO II=1,NDDL
C           DO KK=1,NDDL
C              ZR(LMAT)=KGB(II,KK)
C           ENDDO
