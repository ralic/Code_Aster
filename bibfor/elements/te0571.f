...
C    DIMENSIONS CARACTERISTIQUES DE L'ELEMENT
      ...
      NNO  = ZI(JIN)
      NPG  = ZI(JIN+2)
      IF (NNO.GT.3) CALL UTMESS('F','TE0571','MATNS() SOUS-DIMENSIONNE')
      IF (NPG.GT.4) CALL UTMESS('F','TE0571','PR() SOUS-DIMENSIONNE')
...
C    CALCUL EFFECTIF DU SECOND MEMBRE
      CALL NMPR2D(AXI,1,NNO,NPG,ZR(IPOIDS),ZR(IVF),ZR(IDF),ZR(IGEOM),
     &            PR,ZR(IRES),MATNS)
