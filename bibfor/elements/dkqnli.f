...
C     ------------------------------------------------------------------
C     CALCUL DES OPTIONS NON LINEAIRES POUR L'ELEMENT DE PLAQUE DKQ
...
C --------------------------------------------------------------------
      ...
      INTEGER NBCOU,NPG,NC,NPGH,NZ,ITABP(8),ITABM(8)
...
C     ------------------ PARAMETRAGE QUADRANGLE ------------------------
      INTEGER LDETJ,LJACO,LTOR,LQSI,LETA,LWGT,JTAB(7),COD,JNBSPI
      PARAMETER (NPG=4)
      PARAMETER (NC=4)
      PARAMETER (LDETJ=1)
      PARAMETER (LJACO=2)
...
C     CONTRAINTE 2D : SIXX,SIYY,SIZZ,SQRT(2)*SIXY
      NBCON = 6
C     NBVAR : NOMBRE DE VARIABLES INTERNES (2D) LOI DE COMPORT
...
C         CALL R8MULR(9,POIDS,DM2,1)
          CALL UTBTAB('CUMU',3,2*NNO,DM2,BM,WORK,MEMB)

C         -- FLEXION :
C         ------------
          CALL R8COPY(9,DF,1,DF2,1)
C         CALL R8MULR(9,POIDS,DF2,1)
          CALL UTBTAB('CUMU',3,3*NNO,DF2,BF,WORK,FLEX)

C         -- COUPLAGE:
C         ------------
          CALL R8COPY(9,DMF,1,DMF2,1)
C         CALL R8MULR(9,POIDS,DMF2,1)
          CALL UTCTAB('CUMU',3,3*NNO,2*NNO,DMF2,BF,BM,WORK,MEFL)
        END IF
