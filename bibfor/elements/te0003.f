...
C MODIF ELEMENTS  DATE 17/09/2002   AUTEUR BOITEAU O.BOITEAU 
C ======================================================================
...
C DECLARATION VARIABLES LOCALES
      INTEGER ICARAC,NNO,NPG1,IFF,IPOIDS,IVF,IDFDE,IDFDK,IGEOM,IFLUM,
     &        IFLUP,ISOUR,IMATE,ICHARG,IVOIS,IERRE,I,ITEMM,ITEMP,J,
     &        KP,K,ICODE,I1,NDIM,IJ,I2,MCELD,MCELV,PCELD,PCELV,IAUX1,
     &        IAVAF,NCMPF,IAVAH,NCMPH,NARET,NSOMM,INO,JNO,
     &        MNO,TYPMC,TYPMV,IMAV,IRET,IAPTMA,IENTF,IENTH,IENTT,
     &        IAVAT,NCMPT,NBSV,NBNV,JAD,JADV,
     &        IGREL,IEL,IAVALP,IAVALM,IAREPE,IFM,NIV,NIVEAU,
     &        IFON(3),NBPAR,NBFPG,NBPG(10),IDFDN,K3,ITYP,NDEGRE,
     &        NSOMM2,IDFDX,IDFDX2,IDFDY,IDFDY2,IPG,NPGF,NPGF2,
     &        NOE(9,6,3),TABNIV(20),IJEVEO
      REAL*8 R8PREM,INSOLD,INST,VALTHE,AUX,
     &       RHOCP,DFDX(27),DFDY(27),POIDS,R,VALFP(9),VALFM(9),R8CART,
     &       VALPAR(4),VALSP,VALSM,VALUNT,TERBUF,TEMPM,TEMPP,
     &       FLUXM(3),FLUXP(3),FFORME,DELTAT,R8MIEM,PREC,OVFL,DER(6),
     &       FLURP,FLURM,UNSURR,X3,Y3,XN(9),YN(9),ZN(9),TERM22,
     &       JAC(9),HK,HF,ZRJNO1,ZRJNO2,ZRINO1,ZRINO2,POINC1,POINC2,
     &       VALHP(9),VALHM(9),VALTP(9),VALTM(9),R8CAR1,
     &       ERTABS,ERTREL,TERMNO,TERMVO,TERMSA,TERMFL,TERMEC,TERMV1,
     &       TERMV2,TERMS1,TERMS2,TERMF1,TERMF2,TERME1,TERME2,JACOB,
     &       UNSURD,R8BID,RHOCPM,RHOCPP,DFDZ(27),X,Y,Z,
     &       POIDS1(9),POIDS2(9),POIDS3(9)
      CHARACTER*2 CODRET,FORMV
      CHARACTER*3 NOMT3
      CHARACTER*4 EVOL
      CHARACTER*8 MA,NOMPAR(4),TYPMAC,TYPMAV,K8CART,K8CAR1,ELREFE,
     &            ELREF2
      CHARACTER*16 PHENOM
...
C INITIALISATION DIMENSION DE L'ESPACE DE TRAVAIL/LLUMPE OU PAS
      NOMT3 = NOMTE(5:7)
      IF ((NOMT3.EQ.'TR3').OR.(NOMT3.EQ.'QU4').OR.(NOMT3.EQ.'TR6')
     &   .OR.(NOMT3.EQ.'QU8').OR.(NOMT3.EQ.'QU9')) THEN
        NDIM = 2
        L2D = .TRUE.
        LLUMPE = .FALSE.
      ELSE IF ((NOMT3.EQ.'TL3').OR.
     &   (NOMT3.EQ.'QL4').OR.(NOMT3.EQ.'TL6').OR.
     &   (NOMT3.EQ.'QL8').OR.(NOMT3.EQ.'QL9')) THEN
        NDIM = 2
        L2D = .TRUE.
        LLUMPE = .TRUE.
      ELSE
...
C UN PT DE GAUSS ET UN NOEUD DONNE
   ...
   10   CONTINUE
        ...
        IVF    = IPOIDS + NPG1
C EN 3D, C'EST L'INVERSE, ON LES RANGE A SUIVRE
        IDFDE  = IVF    + NPG1*NNO
        IDFDK  = IDFDE  + 1
        IDFDN  = IDFDK  + 1
      ENDIF
...
C FIN DE LECTURE DE LA CARTE ETENDUE: LECTURE VECT '&&RESTHE.JEVEUO'
      ...
      CALL TECACH(.TRUE.,.FALSE.,'PSOURCR',1,ISOUR)
...
C CALCUL DU DIAMETRE HK DE L'ELEMENT K
      CALL UTHK(NOMTE,IGEOM,HK,NDIM,NOE,NSOMM,ITYP,INO,TABNIV(3),
     &          IFM)
...
C RECHERCHE DE LA VALEUR DE RHO*CP EN LINEAIRE ET EN NON-LINEAIRE
     ...
     &                CODRET,'FM')
          IF (CODRET.NE.'OK')
     &      CALL UTMESS('F','TE0003','! PB RCVALA RHOCP !')
...
C CALCUL L'ORIENTATION DE LA MAILLE
        CALL UTJAC(L2D,IGEOM,IDFDE+K3,IDFDK+K3,IDFDN+K3,TABNIV(5),
     &             IFM,NNO,JACOB)
...
C SOURCE FONCTION (R,Z,TEMPS) (VALSP/M S+/- AU POINT DE GAUSS)
  ...
  110     CONTINUE
          ...
          VALPAR(NBPAR) = INST
          CALL FOINTE('FM',ZK8(ISOUR),NBPAR,NOMPAR,VALPAR,VALSP,
     &                 ICODE)
          IF (LTHETA) THEN
            VALPAR(NBPAR) = INSOLD
            CALL FOINTE('FM',ZK8(ISOUR),NBPAR,NOMPAR,VALPAR,VALSM,
     &                   ICODE)
...
C INTERPOLATION DERIVEE CHAMP D'ENTHALPIE IFON(1) A T- ET T+
            ...
            IF (TABNIV(6).EQ.2)  
     &        WRITE(IFM,*)'RHOCPP/M ',RHOCPP,RHOCPM
          ELSE
C CAS LINEAIRE
...
C IMPRESSIONS NIVEAU 2 POUR DIAGNOSTIC...
          IF (TABNIV(6).EQ.2)
     &       WRITE(IFM,*)'TERMVO TEMPP/M/DELTAT',TEMPP,TEMPM,
     &                   DELTAT
        ENDIF
...
C TRAITEMENT PARTICULIER DU A L'AXI (PART III)
        ...
        IF (L2D) THEN
          ...
          IF (TABNIV(6).EQ.2) 
     &      WRITE(IFM,*)'TERMVO FLUXP',FLUXP(1),FLUXP(2)
          IF (LAXI) THEN
            TERBUF = TERBUF - VALTHE*UNSURR*FLURP
            IF (TABNIV(6).EQ.2) 
     &        WRITE(IFM,*)'TERMVO FLURP/R',FLURP*UNSURR,R
          ENDIF
          IF (LTHETA) THEN
            TERBUF = TERBUF - VALUNT*(FLUXM(1)+FLUXM(2))
            IF (TABNIV(6).EQ.2) 
     &        WRITE(IFM,*)'TERMVO FLUXM',FLUXM(1),FLUXM(2)
            IF (LAXI) THEN
              TERBUF = TERBUF - VALUNT*UNSURR*FLURM
              IF (TABNIV(6).EQ.2)
     &          WRITE(IFM,*)'TERMVO FLURM',FLURM*UNSURR
            ENDIF
          ENDIF
        ELSE
          TERBUF = TERBUF - VALTHE*(FLUXP(1)+FLUXP(2)+FLUXP(3))
          IF (TABNIV(6).EQ.2) WRITE(IFM,*)'TERMVO FLUXP',
     &      FLUXP(1),FLUXP(2),FLUXP(3)
          IF (LTHETA) THEN
            TERBUF = TERBUF - VALUNT*(FLUXM(1)+FLUXM(2)+FLUXM(3))
            IF (TABNIV(6).EQ.2) WRITE(IFM,*)'TERMVO FLUXM',
...
C CALCUL FINAL DU TERME VOLUMIQUE ET DU TERME SOURCE DE NORMALISATION
      ...
      IF (TABNIV(7).EQ.2) 
     &  WRITE(IFM,*)'---> TERMVO/TERMV1',TERMVO,TERMV1
...
C RECUPERATION DE SES DERIVEES DE FONCTIONS DE FORME (EN 3D ONLY).
      CALL UTVOIS(TYPMAC,L2D,LMAJ,NARET,NSOMM,NSOMM2,POINC1,
...
C LES FACES DE TYPE 2.
  ...
  206       CONTINUE
          ENDIF
          CALL UTNO3D(IFM,TABNIV(8),NSOMM,INO,ITYP,IGEOM,XN,YN,ZN,
...
C CALCUL DU TERME D'ERREUR ET DE NORMALISATION ASSOCIE
              CALL UTERFL(NDIM,IFLUP,IFLUM,INO,MNO,JNO,NSOMM,JAC,
     &                    TERM22,AUX,LTHETA,VALTHE,VALUNT,TABNIV(10),
     &                    IFM,XN,YN,ZN,VALFP,VALFM,ITYP,NOE)
            ENDIF
...
C TEMPERATURE IMPOSEE FONCTION (VALTP/M(I)=TEXT+/-  INO/JNO/MNO)
            ...
            IF (TABNIV(12).EQ.2) WRITE(IFM,*)'TERMEC COEH_F ',
     &        K8CART,' / ',K8CAR1
...
C CALCUL DU TERME D'ERREUR ET DE NORMALISATION ASSOCIE
              CALL UTEREC(NDIM,IFLUP,IFLUM,INO,MNO,JNO,NSOMM,JAC,
     &             TERM22,AUX,LTHETA,VALTHE,VALUNT,TABNIV(13),IFM,
     &             XN,YN,ZN,
...
C TEMPERATURE IMPOSEE CONSTANT (VALTP(I)=TEXT+  INO/JNO/MNO VALTM(I)=0)
            ...
            IF (TABNIV(12).EQ.2) WRITE(IFM,*)'TERMEC COEH_R',
     &        R8CART,R8CAR1
...
C---------------------------------
     ...
     &    (FORMV.EQ.'TL').OR.(FORMV.EQ.'QL')))
     &    .OR.(.NOT.L2D.AND.((FORMV.EQ.'HE').OR.(FORMV.EQ.'PE')
     &    .OR.(FORMV.EQ.'TE')))) THEN
...
C DANS ZR DE LA CMP 1 DU PT 1 DE LA MAILLE 1 DU GREL IEL
          ...
          IF (LTHETA)
     &      IAVALM = MCELV-1 + ZI(MCELD-1+ZI(MCELD-1+4+IGREL)+8)

C CALCUL DU TERME D'ERREUR ET DE NORMALISATION ASSOCIE
          CALL UTERSA(NDIM,IFLUP,IFLUM,INO,MNO,JNO,IVOIS,MA,IEL,
     &            NBNV,NBSV,IAVALP,IAVALM,NSOMM,JAC,LTHETA,VALTHE,
     &            VALUNT,TABNIV(15),IFM,ITYP,XN,YN,ZN,TERM22,AUX,
     &            JAD,JADV,NOE)
