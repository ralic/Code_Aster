...
C ======================================================================
C
C CALCUL DES OPTIONS SIEF_ELGA_DEPL SIGM_ELNO_DEPL 
C                    EFGE_ELNO_DEPL EPSI_ELNO_DEPL 
C  POUR LES COQUE_3D
C
C ======================================================================
...
C --------- FIN  DECLARATIONS  NORMALISEES  JEVEUX --------------------
      ...
      INTEGER NBCOU,JCOU,IMOY,IRET,IRET1,IRET2,IRET3,JSIG,JTEM
      ...
      REAL*8 TEMPGA(27),FORTHI(1)
      REAL*8 YOUNG,NU,ALPHA,EPAIS
      REAL*8 EPSI(5),XI3,P1XI3,P2XI3,P3XI3,TINF,TMOY,TSUP
...
C
      ...
      HIC = UN/NBCOU
      ZMIN = -0.5D0
      CALL WKVECT('&&VDXSIG.SIG','V V R',162*NBCOU,JSIG)
      CALL WKVECT('&&VDXSIG.EPS','V V R',162*NBCOU,JEPS)
      CALL WKVECT('&&VDXSIG.TEMP','V V R',27*NBCOU,JTEM)
C
...
C
          ...
          IF ( (IRET1+IRET2+IRET3) .EQ. 0 ) THEN
            ...
            IF (IRET.EQ.1) THEN
              ...
              ELSE
                ZR(JTEM-1+KWGT)=TMOY*P1XI3+TINF*P2XI3+TSUP*P3XI3
                ZR(JTEM-1+KWGT)=ZR(JTEM-1+KWGT)-TREF
              ENDIF
          ELSE
            INDITH=-1
          ENDIF
C
     ...
     &                INDITH,ALPHA,ZR(JTEM),ZR(JEPS),ZR(JSIG),VECTT)
C
          KPGS = KPGS+1
          K1=6*((INTSN-1)*NPGE*NBCOU+NPGE*(ICOU-1)+INTE - 1)
          DO 35 I=1,6
            SIGMPG(K1+I) = ZR(JSIG-1+I+6*(KPGS-1))
 35       CONTINUE
C
 200    CONTINUE
 100  CONTINUE
...
C
 ...
 110  CONTINUE
C
      CALL JEDETR('&&VDXSIG.SIG')
      CALL JEDETR('&&VDXSIG.EPS')
      CALL JEDETR('&&VDXSIG.TEMP')
      END
