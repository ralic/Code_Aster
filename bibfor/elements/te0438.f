...
C     ------------------------------------------------------------------
      ...
      ELSE IF (OPTION(11:14) .EQ. 'SIGM' )  THEN
          NCEQ = 6
      ENDIF
      IF (ELREFE .NE. 'TETRI10 '.AND.ELREFE .NE. 'HEXI20  '
     &    .AND. ELREFE .NE. 'PENTI15 ') THEN
        CALL UTMESS('F','TE0438','ELEMENT:'//NOMTE//
     &       'NON IMPLANTE')
      ENDIF
...
C     -----------------------------------
      ...
      ELSE IF ( OPTION(11:14) .EQ. 'SIGM' )  THEN
         ...
         DO 200 INO = 1,NNO
            IDCP = (INO-1) * NCEQ
            CALL FGEQUI(ZR(ICONT+(INO-1)*7),'SIGM',3,EQNO(IDCP+1))
 200     CONTINUE

       ELSE
         CALL UTMESS('F','TE0438','OPTION INCONNUE : '//OPTION)
