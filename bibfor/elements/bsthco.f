...
CCC      CALL JEVECH('PCOMPOR','L',ICOMPO)
      CALL TECAC2 ('NNN','PCOMPOR',8,ITAB,IRET)
      ICOMPO = ITAB(1)
      IF ( ICOMPO .EQ. 0 ) THEN
         NBCOU = 1
      ELSE
...
C     -- SI LA TEMPERATURE EST UNE FONCTION DE 'INST' ET 'EPAIS':
        CALL TECACH(.TRUE.,.FALSE.,'PTEMPEF',1,ITEMPF)
        IF (ITEMPF.GT.0) THEN
          TEMPNO = .FALSE.
          NBPAR = 1
          NOMPAR = 'TEMP'
...
C           ----------------------------------------------
            CALL HSJ1MS(ZR(JGEOM),INTE,INTSR,ZR(LZR),EPAIS,VECTG,VECTT,
     +                  HSFM,HSS,HSJ1M,HSJ1S)
C
C ---       CALCUL POUR L'INTEGRATION REDUITE DES PARTIES MEMBRANE
C ---       BTDM ET CISAILLEMENT BTDS DE LA MATRICE B :
C           -----------------------------------------
            CALL BTDMSR(NB1,NB2,ZR(JGEOM),KSI3S2,INTSR,ZR(LZR),EPAIS,
...
C ---       [HSJ1FX] = [HSF]*[JM1]  :
C           ----------------------
            CALL HSJ1F(ZR(JGEOM),INTE,INTSN,ZR(LZR),EPAIS,VECTG,VECTT,
     +                 HSF,KWGT,HSJ1FX,WGT)
C
...
C           --------------------
            CALL BTDFN(1,NB1,NB2,ZR(JGEOM),KSI3S2,INTSN,ZR(LZR),EPAIS,
     +                 VECTPT,HSJ1FX,BTDF)
C
