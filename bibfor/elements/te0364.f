...
C
      ...
      INTEGER NNE,NNM,NDDL,NDIM,INDCO,INADH,INDM
...
C  --- PAS DE CONTACT
        ...
        IF (INDCO .EQ. 0) THEN
          ...
          DO 60 I = 1,NNE
            DO 50 J = 1,NNE
              MMAT((2*NDIM)*(I-1)+NDIM+1,(J-1)*(2*NDIM)+NDIM+1) =
     &          -PDS*FFE(J)*FFE(I)*JAC/COEFCA
 50         CONTINUE
 60       CONTINUE
...
C    -------------------------------------------------------
          ...
          DO 90 I = 1,NNE
            DO 80 J = 1,NNE
              DO 70 K = 1,NDIM
                MMAT((2*NDIM)*(I-1)+NDIM+1,(2*NDIM)*(J-1)+K) =
     &            -PDS*FFE(I)*FFE(J)*JAC*NORM(K)
                MMAT((2*NDIM)*(J-1)+K,(2*NDIM)*(I-1)+NDIM+1) =
     &            MMAT((2*NDIM)*(I-1)+NDIM+1,(2*NDIM)*(J-1)+K)
 70           CONTINUE
 80         CONTINUE
 90       CONTINUE
...
C    ------------------------------------------------------
          ...
          DO 120 I = 1,NNE
            DO 110 J = 1,NNM
              DO 100 K = 1,NDIM
                MMAT((2*NDIM)*(I-1)+NDIM+1,(2*NDIM)*NNE+NDIM*(J-1)+K) =
     &            PDS * FFE(I) * FFM(J) * JAC * NORM(K)
                MMAT((2*NDIM)*NNE+NDIM*(J-1)+K,(2*NDIM)*(I-1)+NDIM+1) =
     &            MMAT((2*NDIM)*(I-1)+NDIM+1,(2*NDIM)*NNE+NDIM*(J-1)+K)
 100          CONTINUE
 110        CONTINUE
 120      CONTINUE
...
C    -----------------------------------------------------------
          ...
          DO 160 I = 1,NNE
            DO 150 J = 1,NNE
              DO 140 K = 1,NDIM
                DO 130 L = 1,NDIM
                  MMAT((2*NDIM)*(I-1)+L,(2*NDIM)*(J-1)+K) =
     &              COEFCA * PDS * FFE(I) * NORM(L) * FFE(J) * JAC *
     &              NORM(K)
 130            CONTINUE
 140          CONTINUE
 150        CONTINUE
...
C    --------------------------------------------------------
          DO 200 I = 1,NNE
            DO 190 J = 1,NNM
              DO 180 K = 1,NDIM
                DO 170 L = 1,NDIM
                  MMAT((2*NDIM)*(I-1)+L,(2*NDIM)*NNE+NDIM*(J-1)+K) =
     &              -COEFCA*PDS*FFE(I)*NORM(L)*FFM(J)*JAC*NORM(K)
 170            CONTINUE
 180          CONTINUE
 190        CONTINUE
 200      CONTINUE
...
C   ---------------------------------------------------------
     ...
     &              -COEFCA*PDS*FFM(I)*NORM(L)*FFE(J)*JAC*NORM(K)
 210            CONTINUE
 220          CONTINUE
 230        CONTINUE
 240      CONTINUE
...
C   --------------------------------------------------------
     ...
     &              COEFCA * PDS * FFM(I) * NORM(L) * FFM(J) * JAC *
     &              NORM(K)
 250            CONTINUE
 260          CONTINUE
 270        CONTINUE
 ...
 301          CONTINUE
          ...
          DO 284 I = 1,NNE
            DO 283 J = 1,NNE
              DO 282 L = 1,NDIM-1
                DO 281 K = 1,NDIM-1
            MMAT((2*NDIM)*(I-1)+NDIM+1+L,(J-1)*(2*NDIM)+NDIM+1+K)
     &         = -PDS*FFE(I)*FFE(J)*JAC*TT(L,K)      
 281            CONTINUE
 282          CONTINUE
 283        CONTINUE
 284      CONTINUE
...
C  0.1  CALCUL DES MATRICES NECESSaIRES
...
C   __________________________________________________________________
     ...
     &                1.D0
C                 ELSE
C                    MMAT(2*NDIM*(I-1)+NDIM+1+L,2*NDIM*(J-1)+NDIM+1+K) =
C     &                0.D0
                 END IF
C                 ELSE
C                    MMAT(2*NDIM*(I-1)+NDIM+1+L,2*NDIM*(J-1)+NDIM+1+K) =
              ...
              DO 666 J = 1,NNM
                DO 667 L = 1,NDIM-1
                  DO 668 K = 1,NDIM
                    MMAT((2*NDIM)*(I-1)+NDIM+1+L,2*NDIM*NNE+(J-1)*NDIM+K
     &                  ) =
     &                LAMBDA * COEFFF * PDS * FFE(I) * FFM(J) * JAC *
     &                B(L,K)
                    MMAT(2*NDIM*NNE+(J-1)*NDIM+K,(2*NDIM)*(I-1)+NDIM+1+L
     &                  ) =
     &                MMAT((2*NDIM)*(I-1)+NDIM+1+L,
     &                     2*NDIM*NNE+(J-1)*NDIM+K)
 668              CONTINUE
 667            CONTINUE
 666          CONTINUE
 665        CONTINUE
            ...
            DO 700 I = 1,NNE
              DO 690 J = 1,NNE
                DO 680 L = 1,NDIM
                  DO 670 K = 1,NDIM
                    MMAT((2*NDIM)*(I-1)+L,(J-1)*(2*NDIM)+K) =
     &                -COEFFA*COEFFF*PDS*LAMBDA*FFE(I)*FFE(J)*JAC*D(L,K)
 670              CONTINUE
 680            CONTINUE
 690          CONTINUE
 700        CONTINUE
