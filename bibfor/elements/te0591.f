...
C - CALCUL DES FORCES INTERIEURES
     ...
     &              ZR(IVF3),IDF1,VU,VG,VP,TYPMOD,ZR(IGEOM),ZR(ICONTM),
     &              ZR(IDDLM),ZR(IVECTU))
      ELSEIF (ZK16(ICOMPO+2) (1:8).EQ.'GDEF_LOG') THEN
        CALL JEVECH('PMATERC','L',IMATE)
        CALL NIFNLG(NDIM,NNO1,NNO2,NNO3,NPG,IW,ZR(IVF1),ZR(IVF2),
     &              ZR(IVF3),IDF1,IDF2,VU,VG,VP,TYPMOD,
     &              ZI(IMATE),ZR(IGEOM),ZR(ICONTM),ZR(IDDLM),ZR(IVECTU))
      ELSEIF (ZK16(ICOMPO+2) (1:10).EQ.'SIMO_MIEHE') THEN
        CALL JEVECH('PMATERC','L',IMATE)
        CALL NIFNSM(NDIM,NNO1,NNO2,NNO3,NPG,IW,ZR(IVF1),ZR(IVF2),
     &              ZR(IVF3),IDF1,IDF2,VU,VG,VP,TYPMOD,
     &              ZI(IMATE),ZR(IGEOM),ZR(ICONTM),ZR(IDDLM),ZR(IVECTU))
      ELSE
