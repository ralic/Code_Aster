      SUBROUTINE XBSIGM (NDIM,NNOP,DDLH,NFE,DDLC,IGEOM,PINTT,CNSET,
     &                   HEAVT,LONCH,BASLOC,SIGMA,NOMTE,BSIGMA)
      
       IMPLICIT NONE
C
       INTEGER       NDIM,NNOP,DDLH,NFE,DDLC,IGEOM
       INTEGER       CNSET(4*32),HEAVT(36),LONCH(7)
       REAL*8        PINTT(3*11),BASLOC(*),SIGMA(*),BSIGMA(*)
       CHARACTER*16  NOMTE
C
...
C          IF (NOMTE(1:8).EQ.'MECA_XH_') THEN
C            CALL XIBSIG(ELREFP,COORSE,IGEOM,HE,DDLH,DDLC,NNOP,NPG,
C     &                  SIGMA(IDEB),BSIGMA)
C          ELSEIF (NOMTE(1:8).EQ.'MECA_XT_') THEN
C            CALL XTBSIG(ELREFP,COORSE,IGEOM,BASLOC,NDIM,NFE,NNOP,NPG,
C     &                  SIGMA(IDEB),BSIGMA)          
C          ELSEIF (NOMTE(1:8).EQ.'MECA_XHT') THEN
            CALL XXBSIG(ELREFP,NDIM,COORSE,IGEOM,HE,DDLH,DDLC,NFE,
     &                  BASLOC,NNOP,NPG,SIGMA(IDEB),BSIGMA)
C          ENDIF    
