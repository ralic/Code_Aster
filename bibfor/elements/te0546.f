      SUBROUTINE TE0546(OPTION,NOMTE)

C            CONFIGURATION MANAGEMENT OF EDF VERSION
C MODIF ELEMENTS  DATE 11/04/2002   AUTEUR CIBHHLV L.VIVAN 
C ======================================================================
C COPYRIGHT (C) 1991 - 2002  EDF R&D                  WWW.CODE-ASTER.ORG
C THIS PROGRAM IS FREE SOFTWARE; YOU CAN REDISTRIBUTE IT AND/OR MODIFY
C IT UNDER THE TERMS OF THE GNU GENERAL PUBLIC LICENSE AS PUBLISHED BY
C THE FREE SOFTWARE FOUNDATION; EITHER VERSION 2 OF THE LICENSE, OR   
C (AT YOUR OPTION) ANY LATER VERSION.                                 
C
C THIS PROGRAM IS DISTRIBUTED IN THE HOPE THAT IT WILL BE USEFUL, BUT 
C WITHOUT ANY WARRANTY; WITHOUT EVEN THE IMPLIED WARRANTY OF          
C MERCHANTABILITY OR FITNESS FOR A PARTICULAR PURPOSE. SEE THE GNU    
C GENERAL PUBLIC LICENSE FOR MORE DETAILS.                            
C
C YOU SHOULD HAVE RECEIVED A COPY OF THE GNU GENERAL PUBLIC LICENSE   
C ALONG WITH THIS PROGRAM; IF NOT, WRITE TO EDF R&D CODE_ASTER,       
C    1 AVENUE DU GENERAL DE GAULLE, 92141 CLAMART CEDEX, FRANCE.      
C ======================================================================

      IMPLICIT NONE
      CHARACTER*16 OPTION,NOMTE
C ......................................................................
C    - FONCTION REALISEE:  CALCUL DES OPTIONS NON-LINEAIRES MECANIQUES
C                          ELEMENTS 3D
C    - ARGUMENTS:
C        DONNEES:      OPTION       -->  OPTION DE CALCUL
C                      NOMTE        -->  NOM DU TYPE ELEMENT
C ......................................................................
      CHARACTER*8 TYPMOD(2),NOMAIL,ELREFE
      CHARACTER*24 CARAC,FF,TRAV

      INTEGER NNO,NPG1,I,IMATUU,ITRAV,LGPG,NDIM,LGPG1,LGPG2
      INTEGER ICARAC,IFF,IPOIDS,IVF,IDFDE,IDFDN,IDFDK,IGEOM,IMATE
      INTEGER ITREF,ICONTM,IVARIM,ITEMPM,ITEMPP,IPHASM,IPHASP
      INTEGER IINSTM,IINSTP,IDEPLM,IDEPLP,ICOMPO,ICARCR
      INTEGER IVECTU,ICONTP,IVARIP,LI,IDEFAM,IDEFAP,JCRET,CODRET
      INTEGER IHYDRM,IHYDRP,ISECHM,ISECHP,IVARIX,IADZI,IAZK24
      LOGICAL DEFANE
      INTEGER NDDL,KK,NI,MJ,JTAB(7),NZ
      REAL*8 MATNS(3*27*3*27)

C --------- DEBUT DECLARATIONS NORMALISEES  JEVEUX ---------------------
      CHARACTER*32 JEXNUM,JEXNOM,JEXR8,JEXATR
      INTEGER ZI
      COMMON /IVARJE/ZI(1)
      REAL*8 ZR
      COMMON /RVARJE/ZR(1)
      COMPLEX*16 ZC
      COMMON /CVARJE/ZC(1)
      LOGICAL ZL
      COMMON /LVARJE/ZL(1)
      CHARACTER*8 ZK8
      CHARACTER*16 ZK16
      CHARACTER*24 ZK24
      CHARACTER*32 ZK32
      CHARACTER*80 ZK80
      COMMON /KVARJE/ZK8(1),ZK16(1),ZK24(1),ZK32(1),ZK80(1)
C --------- FIN  DECLARATIONS  NORMALISEES  JEVEUX ---------------------

      CALL ELREF1(ELREFE)

C - FONCTIONS DE FORMES ET POINTS DE GAUSS

      CARAC = '&INEL.'//ELREFE//'.CARACTE'
      CALL JEVETE(CARAC,'L',ICARAC)
      NDIM = ZI(ICARAC)
      NNO = ZI(ICARAC+1)
      NPG1 = ZI(ICARAC+3)
      IF (NNO.GT.27) CALL UTMESS('F','TE0139','MATNS MAL DIMENSIONNEE')

      FF = '&INEL.'//ELREFE//'.FFORMES'
      CALL JEVETE(FF,'L',IFF)
      IPOIDS = IFF + (NDIM+1)*NNO*NNO
      IVF = IPOIDS + NPG1
      IDFDE = IVF + NPG1*NNO
      IDFDN = IDFDE + 1
      IDFDK = IDFDN + 1

      TRAV = '&INEL.'//ELREFE//'.TRAVAIL'
      CALL JEVETE(TRAV,'L',ITRAV)


C - TYPE DE MODELISATION
      TYPMOD(1) = '3D      '
      TYPMOD(2) = 'GRADVARI'


C - PARAMETRES EN ENTREE

      CALL JEVECH('PGEOMER','L',IGEOM)
      CALL JEVECH('PMATERC','L',IMATE)
      CALL JEVECH('PCONTMR','L',ICONTM)
      CALL JEVECH('PVARIMR','L',IVARIM)
      CALL JEVECH('PDEPLMR','L',IDEPLM)
      CALL JEVECH('PDEPLPR','L',IDEPLP)
      CALL JEVECH('PCOMPOR','L',ICOMPO)
      CALL JEVECH('PCARCRI','L',ICARCR)

      CALL TECACH(.TRUE.,.TRUE.,'PVARIMR',7,JTAB)
      LGPG1 = MAX(JTAB(6),1)*JTAB(7)
      LGPG = LGPG1


C - VARIABLES DE COMMANDE

      CALL JEVECH('PTEREF','L',ITREF)
      CALL JEVECH('PTEMPMR','L',ITEMPM)
      CALL JEVECH('PTEMPPR','L',ITEMPP)
      CALL JEVECH('PINSTMR','L',IINSTM)
      CALL JEVECH('PINSTPR','L',IINSTP)
      CALL JEVECH('PHYDRMR','L',IHYDRM)
      CALL JEVECH('PHYDRPR','L',IHYDRP)
      CALL JEVECH('PSECHMR','L',ISECHM)
      CALL JEVECH('PSECHPR','L',ISECHP)
      CALL TECACH(.TRUE.,.FALSE.,'PDEFAMR',1,IDEFAM)
      CALL TECACH(.TRUE.,.FALSE.,'PDEFAPR',1,IDEFAP)
      DEFANE = IDEFAM .NE. 0
      CALL TECACH(.FALSE.,.FALSE.,'PPHASMR',1,IPHASM)
      CALL TECACH(.FALSE.,.FALSE.,'PPHASPR',1,IPHASP)
      IF (IPHASP.NE.0) THEN
        CALL TECACH(.TRUE.,.TRUE.,'PPHASPR',7,JTAB)
        NZ = JTAB(6)
      END IF

C - PARAMETRES EN SORTIE

      IF (OPTION(1:16).EQ.'RIGI_MECA_TANG' .OR.
     &    OPTION(1:9).EQ.'FULL_MECA') THEN
        CALL JEVECH('PMATUUR','E',IMATUU)
      END IF

      IF (OPTION(1:9).EQ.'RAPH_MECA' .OR.
     &    OPTION(1:9).EQ.'FULL_MECA') THEN
        CALL JEVECH('PVECTUR','E',IVECTU)
        CALL JEVECH('PCONTPR','E',ICONTP)
        CALL JEVECH('PVARIPR','E',IVARIP)

C      ESTIMATION VARIABLES INTERNES A L'ITERATION PRECEDENTE
        CALL JEVECH('PVARIMP','L',IVARIX)
        CALL R8COPY(NPG1*LGPG,ZR(IVARIX),1,ZR(IVARIP),1)

      END IF


      IF (ZK16(ICOMPO+3) (1:9).EQ.'COMP_ELAS') THEN

C - LOIS DE COMPORTEMENT ECRITES EN CONFIGURATION DE REFERENCE
C                          COMP_ELAS

        IF (OPTION(1:14).EQ.'RIGI_MECA_TANG') THEN

C        OPTION RIGI_MECA_TANG :         ARGUMENTS EN T-
          CALL NMEL3D(NNO,NPG1,ZR(IPOIDS),ZR(IVF),ZR(IDFDE),ZR(IDFDN),
     &                ZR(IDFDK),ZR(IGEOM),TYPMOD,OPTION,ZI(IMATE),
     &                ZK16(ICOMPO),LGPG,ZR(ICARCR),ZR(ITEMPM),
     &                ZR(IHYDRM),ZR(ISECHM),ZR(ITREF),ZR(IDEPLM),
     &                ZR(ITRAV),ZR(ITRAV+3*NNO),
     &                ZR(ITRAV+3*NNO* (1+2*NNO)),ZR(ICONTM),ZR(IVARIM),
     &                ZR(IMATUU),ZR(IVECTU),CODRET)

        ELSE

C        OPTION FULL_MECA OU RAPH_MECA : ARGUMENTS EN T+

          DO 10 LI = 1,3*NNO
            ZR(IDEPLP+LI-1) = ZR(IDEPLM+LI-1) + ZR(IDEPLP+LI-1)
   10     CONTINUE

          CALL NMEL3D(NNO,NPG1,ZR(IPOIDS),ZR(IVF),ZR(IDFDE),ZR(IDFDN),
     &                ZR(IDFDK),ZR(IGEOM),TYPMOD,OPTION,ZI(IMATE),
     &                ZK16(ICOMPO),LGPG,ZR(ICARCR),ZR(ITEMPP),
     &                ZR(IHYDRP),ZR(ISECHP),ZR(ITREF),ZR(IDEPLP),
     &                ZR(ITRAV),ZR(ITRAV+3*NNO),
     &                ZR(ITRAV+3*NNO* (1+2*NNO)),ZR(ICONTP),ZR(IVARIP),
     &                ZR(IMATUU),ZR(IVECTU),CODRET)
        END IF

      ELSE

C - LOIS DE COMPORTEMENT ECRITE EN CONFIGURATION ACTUELLE
C                          COMP_INCR
C      PETITES DEFORMATIONS (AVEC EVENTUELLEMENT REACTUALISATION)
        IF (ZK16(ICOMPO+2) (1:5).EQ.'PETIT') THEN
          IF (ZK16(ICOMPO+2) (6:10).EQ.'_REAC') THEN
            DO 20 I = 1,3*NNO
              ZR(IGEOM+I-1) = ZR(IGEOM+I-1) + ZR(IDEPLM+I-1) +
     &                        ZR(IDEPLP+I-1)
   20       CONTINUE
          END IF
          CALL NMPL3D(NNO,NPG1,ZR(IPOIDS),ZR(IVF),ZR(IDFDE),ZR(IDFDN),
     &                ZR(IDFDK),ZR(IGEOM),TYPMOD,OPTION,ZI(IMATE),
     &                ZK16(ICOMPO),LGPG,ZR(ICARCR),ZR(IINSTM),
     &                ZR(IINSTP),ZR(ITEMPM),ZR(ITEMPP),ZR(IHYDRM),
     &                ZR(IHYDRP),ZR(ISECHM),ZR(ISECHP),NZ,ZR(IPHASM),
     &                ZR(IPHASP),ZR(ITREF),ZR(IDEPLM),ZR(IDEPLP),
     &                ZR(IDEFAM),ZR(IDEFAP),DEFANE,ZR(ICONTM),
     &                ZR(IVARIM),ZR(ITRAV),ZR(ITRAV+3*NNO* (1+2*NNO)),
     &                ZR(ICONTP),ZR(IVARIP),ZR(IMATUU),ZR(IVECTU),
     &                CODRET)


C      GRANDES DEFORMATIONS : FORMULATION SIMO - MIEHE

        ELSE IF (ZK16(ICOMPO+2) (1:10).EQ.'SIMO_MIEHE') THEN
          CALL NMGP3D(NNO,NPG1,ZR(IPOIDS),ZR(IVF),ZR(IDFDE),ZR(IDFDN),
     &                ZR(IDFDK),ZR(IGEOM),TYPMOD,OPTION,ZI(IMATE),
     &                ZK16(ICOMPO),LGPG,ZR(ICARCR),ZR(IINSTM),
     &                ZR(IINSTP),ZR(ITEMPM),ZR(ITEMPP),ZR(IHYDRM),
     &                ZR(IHYDRP),ZR(ISECHM),ZR(ISECHP),NZ,ZR(IPHASM),
     &                ZR(IPHASP),ZR(ITREF),ZR(IDEPLM),ZR(IDEPLP),
     &                ZR(ICONTM),ZR(IVARIM),ZR(ITRAV),ZR(ITRAV+3*NNO),
     &                ZR(ICONTP),ZR(IVARIP),MATNS,ZR(IVECTU),CODRET)

C        SYMETRISATION DE MATNS DANS MATUU
          IF (OPTION(1:16).EQ.'RIGI_MECA_TANG' .OR.
     &        OPTION(1:9).EQ.'FULL_MECA') THEN
            NDDL = 3*NNO
            KK = 0
            DO 40 NI = 1,NDDL
              DO 30 MJ = 1,NI
                ZR(IMATUU+KK) = (MATNS((NI-1)*NDDL+MJ)+
     &                          MATNS((MJ-1)*NDDL+NI))/2.D0
                KK = KK + 1
   30         CONTINUE
   40       CONTINUE
          END IF

C 7.3 - GRANDES ROTATIONS ET PETITES DEFORMATIONS        
        ELSE IF (ZK16(ICOMPO+2) (1:5).EQ.'GREEN') THEN
          
          DO 45 LI = 1,3*NNO
            ZR(IDEPLP+LI-1) = ZR(IDEPLM+LI-1) + ZR(IDEPLP+LI-1)
   45     CONTINUE          
          
          CALL NMGR3D(NNO,NPG1,ZR(IPOIDS),ZR(IVF),ZR(IDFDE),ZR(IDFDN),
     &                ZR(IDFDK),ZR(IGEOM),TYPMOD,OPTION,ZI(IMATE),
     &                ZK16(ICOMPO),LGPG,ZR(ICARCR),ZR(IINSTM),
     &                ZR(IINSTP),ZR(ITEMPM),ZR(ITEMPP),ZR(IHYDRM),
     &                ZR(IHYDRP),ZR(ISECHM),ZR(ISECHP),NZ,ZR(IPHASM),
     &                ZR(IPHASP),ZR(ITREF),ZR(IDEPLM),ZR(IDEPLP),
     &                ZR(IDEFAM),ZR(IDEFAP),DEFANE,ZR(ICONTM),
     &                ZR(IVARIM),ZR(ITRAV),ZR(ITRAV+3*NNO),
     &                ZR(ITRAV+3*NNO* (1+2*NNO)),
     &                ZR(ICONTP),ZR(IVARIP),ZR(IMATUU),ZR(IVECTU),
     &                CODRET)
        ELSE
          CALL UTMESS('F','TE0139','COMPORTEMENT:'//ZK16(ICOMPO+2)//
     &                'NON IMPLANTE')
        END IF

      END IF

      IF (OPTION(1:9).EQ.'RAPH_MECA' .OR.
     &    OPTION(1:9).EQ.'FULL_MECA') THEN
        CALL JEVECH('PCODRET','E',JCRET)
        ZI(JCRET) = CODRET
      END IF

      END
