...
C MODIF ELEMENTS  DATE 02/05/2001   AUTEUR ADBHHPM P.MASSIN 
C ======================================================================
...
C --------- FIN  DECLARATIONS  NORMALISEES  JEVEUX ---------------------
     ...
     + (PHENOM.EQ.'ELAS_COQUE')) THEN
        ...
        IF (NOMTE(1:8).EQ.'MEDKTR3 ' .OR. NOMTE(1:8).EQ.'MEGRDKT ') THEN
          CALL DKTCOL(XYZL,OPTION,PGL,IC,INIV,DEPL,SIGM,MULTIC,GRILLE)

        ELSE IF (NOMTE(1:8).EQ.'MEDSTR3 ') THEN
          CALL DSTCOL(XYZL,OPTION,PGL,IC,INIV,DEPL,SIGM)
        ELSE IF (NOMTE(1:8).EQ.'MEDKQU4 ') THEN
          CALL DKQCOL(XYZL,OPTION,PGL,IC,INIV,DEPL,SIGM)
        ELSE IF (NOMTE(1:8).EQ.'MEDSQU4 ') THEN
          CALL DSQCOL(XYZL,OPTION,PGL,IC,INIV,DEPL,SIGM)
        ELSE IF (NOMTE(1:8).EQ.'MEQ4QU4 ') THEN
          CALL Q4GCOL(XYZL,OPTION,PGL,IC,INIV,DEPL,SIGM)
        ELSE
