...
C MODIF ELEMENTS  DATE 22/05/2006   AUTEUR REZETTE C.REZETTE 
C ======================================================================
...
C
      ELSE
         ...
         IF(IFOND.EQ.0)THEN
          CALL UTMESS('F','GVER2D','MOT CLE FOND_FISS OBLIGATOIRE')
         ELSE
           CHFOND = FOND//'.FOND      .NOEU'
           CALL JELIRA(CHFOND,'LONMAX',LNOFF,K8B)
           IF(LNOFF.NE.1)THEN
                 CALL UTMESS('F','GVER2D','LE FOND DE FISSURE NE DOIT '
     +        //'ETRE DEFINI QUE PAR UN NOEUD ( REVOIR FOND_FISS )')
           ELSE
