...
C DECLARATION VARIABLES LOCALES
      CHARACTER*6 NOMPRO
      PARAMETER ( NOMPRO = 'TE0205' )
      INTEGER IPOIDS,IVF,IDFDE,II,IJ,IJ1
      ...
      INTEGER I,J,K,KK,L,M,KP,NDIM,COMPT,JVAL,IRET
      ...
      REAL*8 THET,TREF,TG,TGDM(3),DUDMDE(3),FLAG(3)
      REAL*8 PROD,PROD1,PROD2,DIVT,VALPAR(4),VALRES(3),DEVRES(3)
      ...
      REAL*8 P,ENERGI(2),RHO,OM,OMO,EPSEPS,EPSDEP
...
C
     ...
     &    CALL UTMESS('F','TE0205','SEULE UNE LOI DE  '//
...
C	
     ...
     &      .OR.OPTION.EQ.'CALC_DGG_FORC_F') THEN
      ...
      IF ((ISIGI.NE.0) .AND. EPSINI) THEN
        CALL UTMESS('F','CALC_G_GLOB','UNE DEFORMATION INITIALE EST '//
     &       'PRESENTE DANS LA CHARGE : INCOMPATIBLE AVEC LA CONTRAINTE'
     &              //' INITIALE SIGMA_INIT')
      END IF

      IF (INCR) THEN
        CALL UTMESS('F',NOMPRO//OPTION,'LE CALCUL DE DERIVEE N''A '//
...
C - CALCUL DES CONTRAINTES 
          ...
          CRIT(2) = 0.D0
          CRIT(3) = 1.D-3
          CALL NMELNL(NDIM,TYPMOD,ZI(IMATE),COMPOR,CRIT,TG,TREF,OPRUPT,
     &                EPS,SIGL,RBID,RBID,ENERGI,LPIPO,RPIPO,T1PIPO,
     &                T2PIPO,T3PIPO)
...
C
          TERM5 = -(ALPHA + DALPHA*(TG-TREF))*(TRA+E*TRADE)/NU1M
     &           -(ALPHA*(TG-TREF)/NU1M)*(2.D0*DNU*TRA/NU1M
     &           +(DE+2.D0*E*DNU/NU1M)*TRADE)
          TERM6 = ((3.D0*ALPHA*(TG-TREF))/NU1M)*
