...
C MODIF ELEMENTS  DATE 11/02/2013   AUTEUR SFAYOLLE S.FAYOLLE 
C ======================================================================
...
C     POUR TGVERI
      REAL*8       SDEPL(135),SVECT(135),SCONT(6*27),SMATR(18225)
      REAL*8       EPSILO,EPSILP, EPSILG
...
C - PARAMETRES EN SORTIE
        ...
        ELSE
          IMATUU=1
        END IF
        CALL NIFIPD(NDIM,NNO1,NNO2,NNO3,NPG,IW,ZR(IVF1),ZR(IVF2),
     &              ZR(IVF3),IDF1,VU,VG,VP,ZR(IGEOM),TYPMOD,OPTION,
     &              ZI(IMATE),ZK16(ICOMPO),LGPG,ZR(ICARCR),
...
C - PARAMETRES EN SORTIE
        ...
        ELSE
          IMATUU=1
        END IF
        CALL NIFILG(NDIM,NNO1,NNO2,NNO3,NPG,IW,ZR(IVF1),ZR(IVF2),
     &              ZR(IVF3),IDF1,VU,VG,VP,ZR(IGEOM),TYPMOD,OPTION,
     &              ZI(IMATE),ZK16(ICOMPO),LGPG,ZR(ICARCR),
     &              ZR(IINSTM),ZR(IINSTP),ZR(IDDLM),ZR(IDDLD),ANGMAS,
     &              ZR(ICONTM),ZR(IVARIM),ZR(ICONTP),ZR(IVARIP),
...
C - PARAMETRES EN SORTIE
        ...
        ELSE
          IMATUU=1
        END IF
        CALL NIFISM(NDIM,NNO1,NNO2,NNO3,NPG,IW,ZR(IVF1),ZR(IVF2),
     &              ZR(IVF3),IDF1,IDF2,VU,VG,VP,ZR(IGEOM),TYPMOD,OPTION,
     &              ZI(IMATE),ZK16(ICOMPO),LGPG,ZR(ICARCR),
     &              ZR(IINSTM),ZR(IINSTP),ZR(IDDLM),ZR(IDDLD),ANGMAS,
     &              ZR(ICONTM),ZR(IVARIM),ZR(ICONTP),ZR(IVARIP),
     &              RESI,RIGI,ZR(IVECTU),ZR(IMATUU),CODRET)
...
C       Calcul eventuel de la matrice TGTE par PERTURBATION
     ...
     &              ZR(IVECTU),SVECT,NDIM*2*NPG,ZR(ICONTP),SCONT,
     &              NPG*LGPG,ZR(IVARIP),ZR(IVARIX),ZR(IMATUU),SMATR,
     &              MATSYM,EPSILO,EPSILP,EPSILG,VARIA,IRET)
