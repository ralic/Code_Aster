...
C --- FIN DECLARATIONS NORMALISEES JEVEUX ------------------------------
     ...
     &       DIRE(3),ORIG(3),POINT(3),ANGL(3),LAMBS,PREC,R8PREM,
     &       LAMBOS(3),TRACE,FLUSX,FLUSY,FLUSZ,FLUGLS(3),FLULOS(3),
     &       R8DGRD,ALPHA,BETA
      INTEGER JGANO,IPOIDS,IVF,IDFDE,IDFDN,IDFDK,IGEOM,IMATE,NNO,KP,
     &        NPG1,I,IFLUX,ITEMPS,ITEMPE,IMATSE,ITEMSE,K,L,N1,N2,NDIM,
     &        JVAL,ICAMAS,NUNO,NNOS
...
C PHENOM MAT DERIVEE
      IF (LSENS) THEN
        ...
        IF (PHESEN.NE.PHENOM) CALL UTMESS('F','TE0062',
     &                             '! PB PHESEN.NE.PHENOM !')
      END IF
...
C CALCUL DE SENSIBILITE PART III (ISOTROPE)
        IF (LSENS) THEN
          CALL RCVALA(ZI(IMATSE),PHENOM,1,'INST',ZR(ITEMPS),1,NOMRES,
     &                LAMBS,CODRET,'FM')
...
C CALCUL DE SENSIBILITE PART IV (ORTHOTROPE)
        IF (LSENS) THEN
          CALL RCVALA(ZI(IMATSE),PHENOM,1,'INST',ZR(ITEMPS),3,NOMRES,
     &                VALRES,CODRET,'FM')
C SI SENSIBILITE /RHO_CP OU /AUTRE LAMBDA ON A PAS DE TERME COMPLE
C MENTAIRE A ASSEMBLER
          LAMBOS(1) = VALRES(1)
...
C CALCUL DE SENSIBILITE PART V (THERMIQUE NON-LINEAIRE)
        IF (LSENS) THEN
          TPG = 0.D0
          CALL RCVALA(ZI(IMATSE),PHENOM,1,'TEMP',TPG,1,'LAMBDA',LAMBS,
     &                CODRET,'FM')
          IF (ABS(LAMBS).LT.PREC) LSENS = .FALSE.
        END IF
...
C CALCUL DE SENSIBILITE PART VI : CALCUL DE T ET DE GRAD(T)
   ...
   30     CONTINUE
        ...
        IF (PHENOM.EQ.'THER_NL') CALL RCVALA(ZI(IMATE),PHENOM,1,'TEMP',
     &                                TPG,1,'LAMBDA',LAMBDA,CODRET,'FM')
