...
C
      ...
      REAL*8 DTH1D1,DTH2D2,POIDS,TH1,TH2,TSOM
...
C
      ...
      REAL*8 A(3),E1(3),E2(3),E3(3),P(3,3),INVP(3,3),COURB(3,3,3)
...
C       (A,E1=GRLT,E2=GRLN,E3=E1^E2)
        DO 123 I = 1,3
          ...
          DO 321 INO = 1,NNO
            ...
            A(I) = A(I) + ZR(IBALO-1+9*(INO-1)+I)*FF
...
C       COORDONNEES DE G DANS LA BASE LOCALE
        ...
        IF (ABS(VGL(3)) .GT. 3.0D-2) THEN
          CALL UTMESS('A','TE0311','ZG NON NUL !!')
        END IF
C
C       COORDONNEES POLAIRES DE G
        RG = SQRT(XLG*XLG+YLG*YLG)
        PHIG = ATAN2(YLG,XLG)
C
C       SI R EST NUL (POINT DE GAUSS SUR LE FONFIS)
        IF (RG .LT. 1.0D-9) THEN
          CALL UTMESS('A','TE0297','POINT DE GAUSS SUR FOND_FISS')
          RG = 1.0D-9
        END IF
C
        IF ((ABS(YLG) .LT. 1.0D-8) .AND.
     &      (XLG .LT. 0.0D0)) THEN
C
...
C
        ...
        IF ((CODRET(1).NE.'OK') .OR. (CODRET(2).NE.'OK')) THEN
          CALL UTMESS('F','TE0297',
     &                'IL FAUT AFFECTER LES ELEMENTS DE '//
