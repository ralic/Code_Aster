      SUBROUTINE SHB8PS(IMAT,OPTION,XETEMP,D,PROPEL,WORK,RE,OUT)
...
C               ELEMENT SHB8
      ...
      INTEGER IOP,IBID,IDPLA(5),LAG,IRDC,IPROPE,IMAT
...
C          IMAT = PROPEL(100)
        CALL RCTRAC(ZI(IMAT),'TRACTION','SIGM',0.D0,JPROLP,JVALEP,
     &              LCOURB,EM)
...
C      CALL SHIFTD(OUT((IP-1)*6+1),STOT,6)
     ...
     &                  DSIGP,STOT,STEST,S0,SIGELA,STAR)
...
C
            ...
            CALL PRJSHB(STAR,SIGELA,PENTE,SI,DEPSPL,EPST,EPSTAR,ITER,
     &                  SN,HOOKE,SIGEL,DSIGP,TEST,STOT,ZI(IMAT),ITMAX)
