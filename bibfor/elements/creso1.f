...
C DECLARATION PARAMETRES D'APPELS
      INTEGER      NPREC,NMAXIT,ISTOP,NIREMP,IFM,NIV,NUMSD,NBMA,
     &             NBREOR
...
C MONITORING
     ...
     &    ' ',NIREMP,' ',NBREOR
        ...
        IF (NIV.GE.4) 
     &    CALL UTIMSD('MESSAGE',2,.FALSE.,.TRUE.,SOLVEU(1:19),1,'V')
      ENDIF     

C OBJET TEMPORAIRE PROPRE A CHAQUE SOUS-DOMAINE. VECTEUR DE LOGICAL
C DIMENSIONNE AU NOMBRE DE MAILLES. V(I)='TRUE' SI LA IEME MAILLE
C DU MODELE APPARTIENT AU SOUS-DOMAINE NUMSD
      IF (NUMSD.NE.0) THEN
C CREATION DE L'OBJET TEMPORAIRE 
C &&//SDFETI(1:8)//'.'//NOMSD(1:8)//'.LOGI' QUI CONTIENT 
C LA LISTE DES LOGICAL DESIGNANT L'APPARTENANCE D'UNE MAILLE
C AU SOUS-DOMAINE
        K24B='&&'//SDFETI(1:8)//'.'//SOLVEU(12:19)//'.LOGI'
        CALL WKVECT(K24B,'V V L',NBMA,ILOGI)
   ...
   10   CONTINUE
        DO 20 I=1,NBMASD
          INTBUF=ZI(IFETA+I-1)
          ZL(ILOGI+INTBUF-1)=.TRUE.
   20   CONTINUE
        IF (NIV.GE.4) 
     &    CALL UTIMSD('MESSAGE',2,.FALSE.,.TRUE.,K24B(1:19),1,'V')   
      ENDIF
      IF (NIV.GE.3) THEN 
        WRITE(IFM,*)'DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD'        
        WRITE(IFM,*)
