...
C
      ...
      DO 100 INTE=1,NPGE
        IF(NPGE.EQ.3) THEN
C
C-- TRAITEMENT DE LA PLASTICITE (INTEGRATION PAR COUCHES)
C
          IF (INTE.EQ.1) THEN
            ZIC = ZMIN + (ICOU-1)*HIC
            COEF = 1.D0/3.D0
        ...
        ELSE
C
C-- TRAITEMENT DE L'ELASTICITE (2 POINTS D'INTEGRATION DANS
C-- L'EPAISSEUR)
C
...
C      ON PREND LA VALEUR MOYENNE DES FORCES NODALES DE REFERENCE
      ...
      IF  (OPTION.EQ.'REFE_FORC_NODA') THEN
         ...
         CALL R8AXPY(NB1*5,1.D0/NVAL,FTEMP,1,EFFINT,1)
      ENDIF
