...
C          ELEMENT SHB8PS
      PARAMETER (NBRES=2)
      CHARACTER*8 MODELI,ELREFE,TYPMOD
      CHARACTER*4 FAMI
      CHARACTER*2 CODRET(NBRES)
      CHARACTER*8 NOMRES(NBRES),NOMPAR(4)
      CHARACTER*16 NOMTE,OPTION,PHENOM,SOPTIO,NOMSHB
      CHARACTER*24 CHVAL,CHCTE
      REAL*8 RE(24,24),SIGMA(120),D(36),TMPTAB(3,3)
      REAL*8 DUSDX(180),FSTAB(12),SIGMM(120),SIGMP(120)
      REAL*8 VALRES(NBRES),VALPAR(4),VARINT(40),SIGM(6),SIMP(6)
      REAL*8 R8VIDE,LC,G,ANGMAS(3),INSTM,INSTP,DSIDEP(6,6)
      REAL*8 DEPSLO(120),EPS2D(6),DEPS2D(6),EPSLOC(120)
      INTEGER NBSIGM,IINSTM,IINSTP,IRET
      REAL*8 NU,E,PARA(11),F(3,3),FT(3,3),SIG(6),SIG33(3,3)
      REAL*8 XIDEPP(60),RE6(18,18),RE15(45,45),RE20(60,60)
      REAL*8 CC1,CC2,CC3,DETF,DUDDD(180)
...
C
   ...
   70       CONTINUE
            CALL SH8EPS(ZR(IGEOM),ZR(IDEPLM),EPSLOC,DUSDX)
            CALL SH8EPS(ZR(IGEOM),ZR(IDEPLP),DEPSLO,DUDDD)
          ELSE IF (NOMSHB.EQ.'SHB6') THEN
            CALL SH6EPS(ZR(IGEOM),ZR(IDEPLM),EPSLOC,DUSDX)
            CALL SH6EPS(ZR(IGEOM),ZR(IDEPLP),DEPSLO,DUDDD)
C
          ELSE IF (NOMSHB.EQ.'SHB15') THEN
            CALL SH1EPS(ZR(IGEOM),ZR(IDEPLM),EPSLOC,DUSDX)
            CALL SH1EPS(ZR(IGEOM),ZR(IDEPLP),DEPSLO,DUDDD)
C
          ELSE IF (NOMSHB.EQ.'SHB20') THEN
            CALL SH2EPS(ZR(IGEOM),ZR(IDEPLM),EPSLOC,DUSDX)
            CALL SH2EPS(ZR(IGEOM),ZR(IDEPLP),DEPSLO,DUDDD)
          END IF
C
          DO 80 IPG=1,NPG
C	    			
