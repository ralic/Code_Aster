...
C        CALL TECACH('NNO','PRIGIEL','L',1,IBID,INS)
        CALL TECACH('NNO','PRIGIEL','L',1,IDRIGI,INS)
        IF (INS .EQ. 0) THEN
          CALL TECACH('ONN','PMATUUR','E',5,IDRESU,IRET)
        ELSE
          CALL TECACH('NNN','PMATUNS','E',5,IDRESU,IRNS)
          IF (IRNS.NE.0) CALL TECACH('ONN','PMATUUR','E',5,IDRESU,IRET)
        ENDIF
      ELSE IF (OPTION.EQ.'RIGI_MECA_HYST') THEN
        CALL TECACH('ONN','PMATUUC','E',5,IDRESU,IRET)
      ELSE
        CALL ASSERT(.FALSE.)
      END IF
      NBVAL= IDRESU(2)
C
      ...
      NOMPAR(3)='Z'
C
      CALL TECACH('ONN','PGEOMER','L',5,IDGEO,IRET)
      IGEOM=IDGEO(1)
      IDIMGE=IDGEO(2)/NNO
...
C
      IF (INS .EQ. 0) THEN
        CALL TECACH('ONN','PRIGIEL','L',2,IDRIGI,IRET)
        CALL ASSERT(IDRIGI(2).EQ.NBVAL)
      ELSEIF (IRNS.EQ.0) THEN
        CALL TECACH('ONN','PRIGINS','L',2,IDRIGI,IRET)
        CALL ASSERT(IDRIGI(2).EQ.NBVAL)
      ENDIF
C
C
...
C
      IF (OPTION.EQ.'AMOR_MECA') THEN
C     --------------------------------
        CALL TECACH('ONN','PMASSEL','L',2,IDMASS,IRET)
C
        IF (INS .EQ. 0) THEN
