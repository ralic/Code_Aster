...
C     ------------------------------------------------------------
      ...
      CALL TECACH(.TRUE.,.FALSE.,'PMATUUR',5,IDRESU)
      ELSE IF (OPTION.EQ.'RIGI_MECA_HYST') THEN
      CALL TECACH(.TRUE.,.FALSE.,'PMATUUC',5,IDRESU)
      ELSE
...
C
      ...
      VALPAR(1)=ZR(ITEMPE)
C
      CALL TECACH(.TRUE.,.FALSE.,'PGEOMER',5,IDGEO)
      IDIMGE=IDGEO(2)
      DO 5, K=1,IDIMGE
...
C
      ...
      MATER=ZI(IMATE)
C
      CALL TECACH(.TRUE.,.FALSE.,'PRIGIEL',2,IDRIGI)
...
C
      IF (OPTION.EQ.'AMOR_MECA') THEN
C     --------------------------------
      CALL TECACH(.TRUE.,.FALSE.,'PMASSEL',2,IDMASS)
