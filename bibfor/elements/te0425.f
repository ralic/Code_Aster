...
C CHARGEMENT EN PRESSION SUIVEUSE SUR DES FACES D'ELEMENTS
C ISOPARAMETRIQUES 3D
C
...
C
      ...
      PARAMETER     (MXNOEU=27,MXVECT=3*27,MXMATR=3*3*27*27)
      INTEGER       MXPARA
      PARAMETER     (MXPARA=4)
C
...
C
      DO 100 IPG = 1,NPG
        ...
        DO 105 INO = 1,NNO
          ...
          Z = Z + ZR(JGEOM+NDIM*(INO-1)+3-1) * ZR(JVF+KDEC+INO-1)
 105    CONTINUE
        VALPAR(1) = X
        VALPAR(2) = Y
        VALPAR(3) = Z
