      SUBROUTINE XGELEM(ELREFP,NDIM,COORSE,IGEOM,HE,DDLH,DDLC,DDLM,NFE,
     &                  BASLOC,NNOP,IDEPL,LSN,LST,IGTHET)
...
C TOLE CRP_20
      ...
      CHARACTER*8   ELREFP
      CHARACTER*24  COORSE
      INTEGER       IGEOM,NDIM,DDLH,DDLC,NFE,NNOP,DDLM
      INTEGER       IDEPL
      REAL*8        HE
...
C------------FIN  COMMUNS NORMALISES  JEVEUX  --------------------------
      ...
      INTEGER  IRESE,DDLI,NNOI,INDENI,IBID,NNOPS
...
C       CALCUL DES FF
     ...
     &       DDLH,NFE,DDLS,DDLM,FE,DGDGL,'NON',XE,FF,DFDI,F,EPS,GRAD)
...
C         DDLS HEAVISIDE
          DO 202 I=1,DDLH
            CPT=CPT+1
            DEPLA(I) = DEPLA(I) + HE * FF(IN) * ZR(IDEPL-1+INDENI+CPT)
 202      CONTINUE
...
C       CALCUL DU GRAD DE U AU POINT DE GAUSS
        CALL REEREF(ELREFP,NNOP,NNOPS,IGEOM,XG,IDEPL,GRDEPL,NDIM,HE,
     &       DDLH,NFE,DDLS,DDLM,FE,DGDGL,'OUI',XE,FF,DFDI,F,EPS,GRAD)
