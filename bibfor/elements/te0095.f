...
C
      CALL TECACH('ONN','UPESANR','L',1,IPESAU,IRET)
      CALL TECACH('ONN','UROTATR','L',1,IROTAU,IRET)
      CALL TECACH('ONN','VPESANR','L',1,IPESAV,IRET)
      CALL TECACH('ONN','VROTATR','L',1,IROTAV,IRET)
C
      ...
      IF (OPTION .EQ. 'G_BILI_F') THEN
        ...
        VAPARU(4) = ZR(ITMPSU)
        VAPARV(4) = ZR(ITMPSV)
        CALL TECACH('ONN','UEPSINF','L',1,IEPSRU,IRET)
        CALL TECACH('ONN','VEPSINF','L',1,IEPSRV,IRET)
      ELSE
        FONC = .FALSE.
        CALL JEVECH('UPFRVOL','L',IFORCU)
        CALL JEVECH('VPFRVOL','L',IFORCV)
        CALL TECACH('ONN','UEPSINR','L',1,IEPSRU,IRET)
        CALL TECACH('ONN','VEPSINR','L',1,IEPSRV,IRET)
      END IF
