      SUBROUTINE XPESRO(ELREFP,NDIM,COORSE,IGEOM,JHEAVT,JFISNO,NFH,DDLC,
...
C IN IVECTU   : INDICE DU SECONDE MEMBRE
      ...
      CHARACTER*8  ELRESE(6),FAMI(6)
      LOGICAL GRDEPL,ISELLI,LTEATT, AXI
      INTEGER IRESE
...
C       CALCUL DES FF DE L'ELEMENT DE REFERENCE PARENT AU PG COURANT
        CALL REEREF(ELREFP,AXI,NNOP,NNOPS,ZR(IGEOM),XG,1,GRDEPL,
     &              NDIM,RBID,RBID, RBID,
...
C         TERME HEAVISIDE
          DO 113 IG=1,NFH
            ...
            DO 110 J=1,NDIM
              POS=POS+1
              ZR(IVECTU-1+POS) = ZR(IVECTU-1+POS) +
     &        ZI(JHEAVT-1+(IFISS-1)*NCOMP+ISE)*FORVOL(J)*POIDS*FF(INO)
 110        CONTINUE
 113      CONTINUE
