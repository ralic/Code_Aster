...
C CE CHAMP EST ISSU D UN CHARGEMENT PRE-EPSI
      IF (OPTION.EQ.'CALC_G_F') THEN
        ...
        NOMPAR(3) = 'INST'
        VALPAR(3) = ZR(ITEMPS)
        CALL TECACH('ONN','PEPSINF','L',1,IEPSF,IRET)
        IF (IEPSF.NE.0) EPSINI = .TRUE.
      ELSE
        FONC = .FALSE.
        CALL JEVECH('PFRVOLU','L',IFORC)
        CALL TECACH('ONN','PEPSINR','L',1,IEPSR,IRET)
...
C LOI DE COMPORTEMENT
      ...
      CALL TECACH('ONN','PSIGINR','L',1,ISIGI,IRET)
...
C      WRITE(6,*)'IEPSR',IEPSR
      IF (OPTION.EQ.'CALC_G'.OR.OPTION.EQ.'CALC_G_F') THEN
        CALL TECACH('ONN','PVITESS','L',1,IVITES,IRET)
        CALL TECACH('ONN','PACCELE','L',1,IACCEL,IRET)
      ENDIF
...
435       CONTINUE
     ...
     &                OPRUPT,EPS,SIGL,RBID,RBID,ENERGI)
          ...
          CALL TECACH('NNN','PCONTGR','L',1,ISIGM,IRET)
          IF(IRET.EQ.0)THEN
