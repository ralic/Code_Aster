...
C ----    EPSTH = ALPHA*(T-TREF) :
     ...
     &            ZR(ITEMPE),ZR(ITREF),HYDR,SECH,INSTAN,ZI(IMATE),
...
C --- CALCUL DU JACOBIEN AU POINT D'INTEGRATION COURANT :
          ...
          CALL NMGEOM(3,NNO,AXI,GRAND,ZR(IGEOM),IGAU,ZR(IPOIDS+IGAU-1),
     &                ZR(IVF+ (IGAU-1)*NNO),ZR(IDFDE),ZR(IDFDN),
     &                ZR(IDFDK),ZR(IDEPL),POIDS,DFDBID,F,EPSBID,R)
