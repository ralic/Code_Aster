...
C ON TESTE LA RECUPERATION DU CHAMP DE CONTRAINTES DU PAS PRECEDENT
     ...
     +      .AND.( ZK16(IDCOMP)(1:4).NE.'ELAS')) THEN
             CALL TECACH(.FALSE.,.FALSE.,'PCONTMR',1,IDCONM)
            IF (IDCONM.NE.0) THEN
...
C  CALCUL DE B.U
     ...
     +            NHARM,EPSS)
           IF (IDEPLM.NE.0)THEN
              CALL EPS1MC(MODELI,NNO,NDIM,NBSIG,NPG1,ZR(IVF), ZR(IDFDE),
     +        ZR(IDFDN), ZR(IDFDK), ZR(IPOIDS), ZR(IGEOM),ZR(IDEPMM),
     +        NHARM,EPSSM)
           ENDIF
         ENDIF
...
C --- CALCUL DU JACOBIEN AU POINT D'INTEGRATION COURANT :
     ...
     &              ZR(IDFDE),ZR(IDFDN),
     &              ZR(IDFDK),ZR(IDEPL),POIDS,DFDBID,
     &              F,EPSBID,R)
...
C --- CALCUL DE LA TRACE DES DEFORMATIONS ELASTIQUES BE
     ...
     +   +((Q-(D**(1.D0/2.D0)))**(1.D0/3.D0))
         ...
         ENELAS = -TROISK*ALPHA*(TEMPG-ZR(ITREF))*(JAC-1.D0/JAC)/2.D0
...
C --- TRAVAIL PLASTIQUE 'EQUIVALENT' :
         ...
         CALL RCFONC('V','TRACTION',JPROL,JVALE,NBVAL,
     &               RBID,RBID,RBID,P,RP,RPRIM,AIREP,RBID,RBID)
...
C---             CALCUL DES TERMES A SOMMER
     ...
     &             2.0D0*SIGMM(5)*DELTA(5)+2.0D0*SIGMM(6)*DELTA(6)

                  INTEG2=SIGMA(1)*DELTA(1)+SIGMA(2)*DELTA(2)+
     &                   SIGMA(3)*DELTA(3)+2.0D0*SIGMA(4)*DELTA(4)+
     &             2.0D0*SIGMA(5)*DELTA(5)+2.0D0*SIGMA(6)*DELTA(6)
...
C --- POINT D'INTEGRATION COURANT :
     ...
     +        (EPSDV(1)*EPSDV(1)+EPSDV(2)*EPSDV(2)+EPSDV(3)*EPSDV(3)
     + + DEUX*(EPSDV(4)*EPSDV(4)+EPSDV(5)*EPSDV(5)+EPSDV(6)*EPSDV(6))))
...
C --- CONTRAINTE EQUIVALENTE :
         ...
         CALL RCFONC('E','TRACTION',JPROL,JVALE,NBVAL,RBID,E,NU,
     +               ZERO,RP,RPRIM,AIREP,SIGEQ,P)
...
C ---      CALCUL DU TRAVAIL PLASTIQUE AU POINT D'INTEGRATION COURANT :
            ...
            EPLAST =
     +        SIGMA(1)*EPSPLA(1)+SIGMA(2)*EPSPLA(2)+SIGMA(3)*EPSPLA(3)
     + +DEUX*(SIGMA(4)*EPSPLA(4)+SIGMA(5)*EPSPLA(5)+SIGMA(6)*EPSPLA(6))
