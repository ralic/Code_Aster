...
C --------- FIN  DECLARATIONS  NORMALISEES  JEVEUX ---------------------
     ...
     +        IINSTP,LGPG,IDEPLP,ICARCR,NBVARI,NZ,JCRET,CODRET,ITEMPF
      ...
      INTEGER NNO,KP,NPG1,I,J,K,IMATUU,ITEMPR,ICACO,NDIMV,IVARIX
      INTEGER ICARAC,IFF,IPOIDS,IVF,IDFDK,IGEOM,IMATE,ITER
...
C-- APPEL DE NMCOMP: LOI PLASTIQUE
   ...
   38       CONTINUE
     ...
     +                  HYDRGP,SECHGM,SECHGP,EPS2D,DEPS2D,ZR(ICONTM+K1),
     +                  ZR(IVARIM+K2),OPTION,EPSANM,EPSANP,NZ,PHASM,
     +                  PHASP,LC,ZR(ICONTP+K1),ZR(IVARIP+K2),DSIDEP,
     +                  COD)
...
C           COD=3 : C_PLAN DEBORST SIGZZ NON NUL
           ...
           IF(VECTEU) THEN
C
            IF (NOMTE(3:4).NE.'TC') GOTO 39
            IF(ITER.EQ.1) THEN
               SIGMA = ZR(ICONTP-1+K1+2)
               IF(ABS(SIGMA).LT.1.D-18) GOTO 39
               XM = (XS + XI)/2.D0
               GOTO 38
            ELSE
               IF(ABS(XS-XI).LT.1.D-30) GOTO 39
               IF(ABS((XS-XI)/(XS+XI)).LT.1.D-14) GOTO 39
            ENDIF
...
C-- CONSTRUCTION DE LA MATRICE TANGENTE
          CALL MATTGE(KP,NOMTE,DTILD,SINA,COSA,R,JACP,ZR(IVF+K),DFDX,
     +                RTANGI)
          DO 100 I = 1,9
            DO 90 J = 1,9
