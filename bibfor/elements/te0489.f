...
C ---- RECUPERATION DU NOMBRE DE COMPOSANTES
        CALL TECACH('OOO','PDERAMG','L',7,JTAB,IRET)
        IDERA1 = JTAB(1)
        NBCMP  = JTAB(2)/JTAB(3)
...
C
      CALL TECACH('OOO','PCONTMR','L',3,JTAB,IRET)
      NPG = JTAB(3)
      ISIGTM = JTAB(1)
C
...
C ---- CALCUL DE L'INDICATEUR LOCAL DE PERTE DE RADIALITE ERR_RADI
...
C      -------------------------------------------------------
     ...
     &              0,COMPOR,IMATE,NBVARI,ZR(IDVAR1),ZR(IDVAR2))
C
C ---- CALCUL DE L'INDICATEUR LOCAL IND_DCHA et VAL_DCHA
