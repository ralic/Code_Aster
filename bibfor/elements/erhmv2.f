...
C IN NDDLM   : NOMBRE DE DDLS SUR LES MILIEUX 
...
C IN DEPL    : TABLEAU DES DEPLACEMENTS GENERALISES
...
C AUXILIAIRE :
C ------------
C DFDI     : DERIVEE DES FONCTIONS DE FORME POUR LES VARIABLES P2
C DFDI2    : DERIVEE DES FONCTIONS DE FORME POUR LES VARIABLES P1
C B        : MATRICE PERMETTANT LES OPERATIONS ELEMENTAIRES
C
C SORTIE :
C -------
C
...
C
      ...
      INTEGER       DIMDEP,DIMDEF,NMEC,NP1,NP2
      INTEGER       NBCMP
...
C A P1. DANS LE TABLEAU DEPLA, ELLES COMMENCENT A L'ADRESSE JDEBP1. ON
C LES RETROUVE ENSUITE ESPACEES DE DIMDEP, DIMENSION DU VECTEUR
C DEPLACEMENT. ON TERMINE AU MAX DU NOMBRE DE NOEUDS SOMMETS.
...
C LE RESULTAT SE PLACE A L'ADRESSE DES DEFORMATIONS PHYSIQUES, ADDPP1,
C AUGMENTEE DE LA DIMENSION EN COURS.
C
...
C              JDEBP1 =  3 = 2 + 1 (NDIM + 1)
C              JFIN   = 12 = 3*4 (NNOS*DIMDEP)
C    LA BOUCLE 121 J = JDEBP1, JFIN, DIMDEP, EXPLORE DONC LES
...
C
        ...
        JAUX = NNOS*DIMDEP
C
        DO 22 , N = IAUX, JAUX, DIMDEP
C
          GRADPX = GRADPX + B(ADDEP1+1,N)*DEPL(N)
          GRADPY = GRADPY + B(ADDEP1+2,N)*DEPL(N)
C
 22     CONTINUE
C
C =====================================================================
...
C    LES VALEURS AUX NOEUDS SONT DANS SIELNO
C =====================================================================
C
        DSXXDX = 0.D0
        DSXYDY = 0.D0
...
C      REMARQUE : TOUS CES TERMES SONT DES DERIVEES SECONDES DE TERMES
C                 DISCRETISES EN DEGRE 1, DONC SONT STRUCTURELLEMENT
C                 NULS. ON POURRAIT LES CALCULER QUAND MEME ET
C                 RETROUVER CES VALEURS NULLES MAIS ON NE CALCULERA
C                 RIEN DU TOUT. 
C =====================================================================
C
   20 CONTINUE
