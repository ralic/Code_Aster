...
C     (CAS D'1 ELEMENT PARENT QUADRATIQUE) => NBNAMX=3
      ...
      REAL*8 E,NU,VALRES(2),R8TMP,COORSE(81)
...
C
      CALL UTHK(NOMTE,ZR(IGEOM),HE,NDIM,ITAB,IBID,IBID,IBID,NIV,IFM)
C
...
C
        ...
        IBID = 1
        CALL ERMEV2(NNOP,IGEOM,ZR(IAUX),ZR(ISIGNO),NBCMP,DFDXP,DFDYP,
     &              POIDP,IBID,
     &              R8BID,R8BID,NOR)
...
C
      CALL UTJAC(.TRUE.,ZR(IGEOM),1,IDFDE,0,IBID,NNOP,ORIEN)
C
...
C
        CALL UTHK(NOMTSE,COORSE,HSE,NDIM,ITAB,IBID,IBID,IBID,NIV,IFM)
C
C ----- CALCUL DE L'ORIENTATION DU SOUS-ELEMENT
C
        CALL UTJAC(.TRUE.,COORSE,1,IDFSE,0,IBID,NNO,ORIEN)
C
