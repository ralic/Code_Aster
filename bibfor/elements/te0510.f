...
C --------- FIN  DECLARATIONS  NORMALISEES  JEVEUX --------------------
      ...
      INTEGER       I,J,JJ,NLI,NNOP
      REAL*8        LONGAR,AL,ND(3),GRLT(3),TAU1(3),TAU2(3),NORME,PS
      REAL*8        NORM2,PTREE(3),PTREF(3),RBID,RBID3(3,3)
...
C     ARCHIVAGE DE PINTER, AINTER, GESCLA ET GMAITR
      DO 110 I=1,NINTER
        DO 111 J=1,NDIM
...
C    ON TRANFORME LES COORDONNÉES RÉELES EN COORD. DANS L'ÉLÉMENT DE REF
       CALL REEREF(ELP,NNOP,IGEOM,PTREE,RBID3,LBID,NDIM,RBID,IBID,
     &      IBID,IBID,RBID,RBID3,'NON',PTREF,FF,DFDI,RBID3,RBID,RBID3)
        DO 112 JJ=1,NDIM
...
C     ARCHIVAGE DE CFACE
      DO 120 I=1,NFACE        
        DO 121 J=1,NDIM
          ZI(JOUT3-1+NDIM*(I-1)+J)=CFACE(I,J)
 121    CONTINUE
 120  CONTINUE
...
C       ARCHIVAGE DE BASECO
        DO 134 J=1,NDIM
          ZR(JOUT5-1+NDIM*NDIM*(NLI-1)+J)  =ND(J)
          ZR(JOUT5-1+NDIM*NDIM*(NLI-1)+J+NDIM)=TAU1(J)
          IF (NDIM .EQ. 3)
     &    ZR(JOUT5-1+NDIM*NDIM*(NLI-1)+J+2*NDIM)=TAU2(J)
 134    CONTINUE
