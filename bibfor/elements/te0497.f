...
C      SUR UN ELEMENT ISOPARAMETRIQUE 2D, VIA L'OPTION 'ERME_ELEM'
...
C
      CALL TECACH('ONN','PCONTNO','L',3,ITAB,IRET)
      ISIENP = ITAB(1)
      NBCMP  = ITAB(2)/NNO
      IF ( .NOT. PERMAN ) THEN
        CALL TECACH('ONN','PCONTNM','L',3,ITAB,IRET)
        ISIENM = ITAB(1)
      ELSE
        ISIENM = 1
      ENDIF
...
C
      CALL TECACH('ONN','PPESANR','L',1,ITAB,IRET)
      IF ( ITAB(1).NE.0 ) THEN
      ...
      ELSE
        YAPR = .FALSE.
      ENDIF
      CALL TECACH('ONN','PROTATR','L',1,ITAB,IRET)
      IF ( ITAB(1).NE.0 ) THEN
...
C          VALEURS REELLES ?
      CALL TECACH('ONN','PFRVOLU','L',1,IFOVR,IRET)
C          OU FONCTIONS ?
      IF ( IFOVR.EQ.0 ) THEN
        CALL TECACH('ONN','PFFVOLU','L',1,IFOVF,IRET)
      ELSE
        IFOVF = 0
      ENDIF
...
C--------------------------------------------------------------------
      ...
      POUM='+'
C
      CALL RCVALB (FAMI,KPG,SPT,POUM, ZI(IMATE), ' ', 'THM_DIFFU', 1,
     &              NOMPAR, VALRES,NBRE1, NOMRE1, VALRE1, CODME1, 1)
C
...
C
      CALL RCVALB (FAMI,KPG,SPT,POUM, ZI(IMATE), ' ', 'THM_DIFFU', 1,
     &             NOMPAR, VALRES, NBR11, NOMR11, VALR11, CODM11, 0)
C
      IF ( CODM11(1).EQ.0) THEN
        PERMIN = VALR11(1)
      ELSE IF ( CODM11(1).EQ.1 ) THEN
        CALL RCVALB (FAMI,KPG,SPT,POUM, ZI(IMATE), ' ', 'THM_DIFFU', 1,
     &               NOMPAR, VALRES,NBR12, NOMR12, VALR12, CODM12, 0)
     ...
     &      ( CODM12(3).EQ.0 )) THEN
      ...
      ENDIF
C
      CALL RCVALB (FAMI,KPG,SPT,POUM, ZI(IMATE), ' ', 'THM_LIQU', 1,
     &              NOMPAR, VALRES, NBRE2, NOMRE2, VALRE2, CODME2, 1)
C
...
C
        CALL RCVALB (FAMI,KPG,SPT,POUM, ZI(IMATE), ' ', 'THM_INIT', 1,
     &               NOMPAR, VALRES, NBRE3, NOMRE3, VALRE3, CODME3, 1)
C
...
C
        CALL RCVALB (FAMI,KPG,SPT,POUM, ZI(IMATE), ' ', 'ELAS', 1,
     &               NOMPAR, VALRES,NBRE4, NOMRE4, VALRE4, CODME4, 1)
C
