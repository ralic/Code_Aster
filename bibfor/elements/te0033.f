...
C         ---------------------------------------
     ...
     +        NOMTE(1:8).EQ.'MEGRDKT ') THEN
            CALL DKTCOD(XYZL,OPTION,PGL,IC,INIV,DEPLR,SIGMRL,
     +                  MULTIC,GRILLE)
            CALL DKTCOD(XYZL,OPTION,PGL,IC,INIV,DEPLI,SIGMIL,
     +                  MULTIC,GRILLE)
     ...
     +             NOMTE(1:8).EQ.'MEDKQG4 ') THEN
          ...
          CALL DXSIRO(NP,T2EV,SIGMRL,SIGMR)
          CALL DXSIRO(NP,T2EV,SIGMIL,SIGMI)
...
C
          CALL DXEFRO(NP,T2EV,SIGMRL,SIGMR)
          CALL DXEFRO(NP,T2EV,SIGMIL,SIGMI)
