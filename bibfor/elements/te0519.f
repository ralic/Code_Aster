...
C
      CHARACTER*8    ELREF
      INTEGER       JDEPL,JPINT,JLON,JGEO,JLST
      INTEGER       JGES,JGMA
      INTEGER       IBID,NDIM,NNO,DDLH,SINGU,DDLS,NINTER
      INTEGER       I,J,IPT,NFE,DDLC,NNOM    
...
C --- INITIALISATION DES DIMENSIONS DES DDLS X-FEM
      CALL XTEINI(NOMTE,DDLH,NFE,SINGU,DDLC,NNOM,DDLS,IBID,IBID)
C
...
C
        DO 210 I=1,NNO
          DO 220 J=1,NDIM
            DEPLM(J)=DEPLM(J)+FF(I)*ZR(JDEPL-1+DDLS*(I-1)+J)
            DEPLE(J)=DEPLE(J)+FF(I)*ZR(JDEPL-1+DDLS*(I-1)+J)
 220      CONTINUE
          DO 230 J=1,DDLH
            DEPLM(J)=DEPLM(J)+FF(I)*ZR(JDEPL-1+DDLS*(I-1)+NDIM+J)
            DEPLE(J)=DEPLE(J)-FF(I)*ZR(JDEPL-1+DDLS*(I-1)+NDIM+J)
 230      CONTINUE
          DO 240 J=1,SINGU*NDIM
            DEPLM(J)=DEPLM(J)+R*FF(I)*ZR(JDEPL-1+DDLS*(I-1)+NDIM+DDLH+J)
            DEPLE(J)=DEPLE(J)-R*FF(I)*ZR(JDEPL-1+DDLS*(I-1)+NDIM+DDLH+J)
 240      CONTINUE
 210    CONTINUE
C
