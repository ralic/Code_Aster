...
C       CAS OU LA FORCE EST FOURNIE AUX NOEUDS           
        IF (IFORC.NE.0) THEN
          FONO=.TRUE.
          CALL JEVECH('PNFORCER','L',IFORC)
C       CAS OU LA FORCE EST FOURNIE PAR ELEMENT 
        ELSE
          FONO=.FALSE.
          IF (NDIM.EQ.3) CALL JEVECH('PFR3D3D','L',IFORC)
          IF (NDIM.EQ.2) CALL JEVECH('PFR2D2D','L',IFORC)
        ENDIF
