...
C
      ...
      INTEGER JTAB(7),JCRET,KK,LGPG,IIRRAM,IIRRAP
      ...
      REAL*8       COELMA(12), COEFGR(3),COELEM(7)
...
C
     ...
     +         TEMPP,ZR(ITREF),ALPHAM,ALPHAP,E,EM,NU,ZR(ICONTM),
     +         FL,ZR(ICONTP))
         ENDIF
C
...
C
        ...
        IF(IIRRAM .EQ. 0 .OR. IIRRAP .EQ. 0) THEN
...
C
        CALL NMASSG (ZI(IMATE),COEFGR)     
        CALL NMASSC (ZI(IMATE),COELMA,COELEM,ICAS)
C
...
C
         CALL NMPOEL(KLV,XL,NNO,NC,PGL,ZR(IDEPLP),ITEMP,TEMPM,
     +                  TEMPP,ZR(ITREF),ALPHAM,ALPHAP,E,EM,NU,
     +                  ZR(ICONTM),FL,SIGP)
C
