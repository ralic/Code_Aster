      SUBROUTINE XBSIG3(ELREFP,NDIM,COORSE,IGEOM,HE,NFH,DDLC,NFE,
...
C
      CHARACTER*8   ELREFP
      CHARACTER*24  COORSE
      INTEGER       IGEOM,NDIM,NFH,DDLC,NFE,NNOP,NPG
      INTEGER       NFISS,FISNO(NNOP,NFISS)
      REAL*8        BASLOC(9*NNOP),SIGMA(6,NPG),LSN(NNOP),LST(NNOP)
      REAL*8        HE(NFISS),BSIGMA(NDIM*(1+NFH+NFE)+DDLC,NNOP)
...
C     VARIABLES LOCALES
      ...
      REAL*8     XG(NDIM),RBID1(NDIM*(1+NFH+NFE)+DDLC,NNOP)
      REAL*8     DFDI(NNOP,NDIM),F(3,3),RBID2(6),BASLOG(9),FE(4)
      REAL*8     DGDGL(4,3),RAC2,RBID3(4),XE(NDIM),FF(NNOP)
      REAL*8     RBID4(4),RBID5(4),JAC,RBID6(3,3),LSNG,LSTG
      REAL*8     DEF(6,NNOP,NDIM*(1+NFH+NFE)),SIGN(6)
...
C       JUSTE POUR CALCULER LES FF
        CALL REERE3(ELREFP,NNOP,IGEOM,XG,RBID1,.FALSE.,NDIM,HE,
...
C         DÉRIVÉES DES FONCTION D'ENRICHISSEMENT AU POINT DE GAUSS
