...
C
      ...
      INTEGER NBFIB,KP,ADR,NCOMP,I,CARA,NE,JACF,NCARFI,JTAB(7)
      INTEGER ICHN,ICHG,ICOMPO,NBVAR,LGPG,ICGP,ICONTN,IORIEN,IVECTU
C
      INTEGER IDEPLM,IDEPLP,IGEOM,IRET,ISECT,IMATE,ITEMPE,K,KK
...
C     NOMBRE DE VARIABLES PAR POINT DE GAUSS EN PLUS DU NBFIB
      ...
      NBFIB = ZI(I)
      IF(ZI(I+1).NE.1)CALL U2MESS('F','ELEMENTS4_71')
...
C     -- PASSAGE DE G (CENTRE DE GRAVITE) A C (CENTRE DE TORSION)
        ...
        IF ( IRET .NE. 0 ) THEN
            CALL MATELA(ZI(IMATE),' ',0,0.D0,E,NU,R8BID)
        ELSE
            TEMP = (ZR(ITEMPE)+ZR(ITEMPE+1))/2.D0
