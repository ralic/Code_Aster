...
C
C
C      IF (NOMTE(1:8).EQ.'MEFOTR3 ' .OR.
C     &    NOMTE(1:8).EQ.'MEFOQU4 ') THEN
      CALL ELREF4(' ','GANO',NDIM,NNO,NNOS,NPG,IPOIDS,
     &                   IVF,IDFDE,JGANO)
C      ELSEIF (NOMTE(1:8).EQ.'MEFOTR6 ' .OR.
C     &        NOMTE(1:8).EQ.'MEFOQU8 ' .OR.
C     &        NOMTE(1:8).EQ.'MEFOQU9 ') THEN
C        CALL ELREF4(' ','NOEU_S',NDIM,NNO,NNOS,NPG,IPOIDS,
C     &                   IVF,IDFDE,JGANO)
C      ENDIF
C
...
101   CONTINUE
C
      CALL PPGAN2(JGANO,6,DEPG,ZR(IDEFO))
C
      END
