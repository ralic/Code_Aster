...
C-----------------------------------------------------------------------
      ...
      CHARACTER*24 CONEC,TYPP,NOMMAI,NOMNOE,MESMAI,VALK(2),NOGRP
      LOGICAL BUG
      INTEGER IARG
C DEB-------------------------------------------------------------------
      CALL JEMARQ()
...
C --- LECTURE DU NOM DU NOEUD ORIGINE (S'IL EST FOURNI)
      ...
      CALL GETVTX(MOTFAC,'GROUP_NO_ORIG',IOCC,IARG,0,NOGRP,N2)
      IF (N1.NE.0) THEN
        CALL GETVTX(MOTFAC,'NOEUD_ORIG',IOCC,IARG,1,NDORIG,N1)
      ELSEIF (N2.NE.0) THEN
        CALL GETVTX(MOTFAC,'GROUP_NO_ORIG',IOCC,IARG,1,NOGRP,N2)
        CALL UTNONO(' ',NOMAIL,'NOEUD',NOGRP,NDORIG,IRET)
        IF (IRET.EQ.10) THEN
...
C --- LECTURE DU NOM DU NOEUD EXTREMITE (S'IL EST FOURNI)
      ...
      CALL GETVTX(MOTFAC,'GROUP_NO_EXTR',IOCC,IARG,0,NOGRP,N2)
      IF (N1.NE.0) THEN
        CALL GETVTX(MOTFAC,'NOEUD_EXTR',IOCC,IARG,1,NDEXTR,N1)
      ELSEIF (N2.NE.0) THEN
        CALL GETVTX(MOTFAC,'GROUP_NO_EXTR',IOCC,IARG,1,NOGRP,N2)
        CALL UTNONO(' ',NOMAIL,'NOEUD',NOGRP,NDEXTR,IRET)
        IF (IRET.EQ.10) THEN
          CALL U2MESK('F','ELEMENTS_67',1,NOGRP)
        ELSEIF (IRET.EQ.1) THEN
