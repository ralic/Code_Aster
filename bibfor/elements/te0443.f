...
C
      ...
      INTEGER            IGEOM,IVECTU,K,I,IFORC
...
C
         ...
         FY = 0.0D0
         DO 102 I = 1 , NNO1
            FX = FX + ZR(IFORC-1+2*(I-1)+1)*ZR(IVF1+K+I-1)
            FY = FY + ZR(IFORC-1+2*(I-1)+2)*ZR(IVF1+K+I-1)
 102     CONTINUE
C
...
C
         DO 106 I = 1 , NNO2
            ZR(IVECTU+3*I-3) = ZR(IVECTU+3*I-3)+FX*ZR(IVF1+K+I-1)*POIDS
            ZR(IVECTU+3*I-2) = ZR(IVECTU+3*I-2)+FY*ZR(IVF1+K+I-1)*POIDS
            ZR(IVECTU+3*I-1) = 0.D0
 106     CONTINUE
         DO 108 I = NNO2+1 , NNO1
            ZR(IVECTU+NNO2+2*I-2) = ZR(IVECTU+NNO2+2*I-2)
     &                                + FX*ZR(IVF1+K+I-1)*POIDS
            ZR(IVECTU+NNO2+2*I-1) = ZR(IVECTU+NNO2+2*I-1)
     &                                + FY*ZR(IVF1+K+I-1)*POIDS
 108     CONTINUE
C
 100  CONTINUE
C
