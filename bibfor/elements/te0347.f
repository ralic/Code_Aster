...
C       - COMPORTEMENT ELAS : RECOPIE DES POINTS 1 ET 2 SI NPG=2
...
C
      ...
      INTEGER     KP,NPG,IRET,IPLOUF,KPG,SPT
      CHARACTER*8 NOMRES(2), NOMPAR,FAMIL,POUM
      CHARACTER*4 FAMI
      INTEGER CODRES(2)
      LOGICAL     REAGEO
      CHARACTER*16 COMPOR
      REAL*8       D1B3(2,3),KSI1,FORREF,MOMREF
...
C           - COMPORTEMENT VMIS_POU*
C                 ==> ON CALCULE BT*SIGMA AU 2 NOEUDS
C           - COMPORTEMENT ELAS : RECOPIE DES POINTS 1 ET 2 OU 3
C                 QUI CONTIENNENT DEJA LES EFFORTS AUX NOEUDS
C
...
C
            ...
            NBPAR  = 1
            NOMPAR = 'TEMP'
            CALL RCVALB(FAMIL,KPG,SPT,POUM,ZI(IMATE),' ','ELAS',NBPAR,
     &                  NOMPAR,VALPAR,2,NOMRES, VALRES, CODRES, 0)
            ...
            IF (NC.EQ.7) THEN
               DO 50 KP = 1,3
                  ...
                  DO 52 K = 1,14
                     DO 54 KK = 1,7
                        RESIDU(K)=RESIDU(K) +
     &                     XLS2*ZR(ICGP-1+7*(KP-1)+KK)*D1B(KK,K)*CO(KP)
54                   CONTINUE
52                CONTINUE
50             CONTINUE
C
            ...
            ELSE
               DO 55 KP = 1,3
                  ...
                  DO 56 K = 1,12
                     DO 57 KK = 1,6
                        RESIDU(K)=RESIDU(K) +
     &                     XLS2*ZR(ICGP-1+6*(KP-1)+KK)*D1B6(KK,K)*CO(KP)
57                   CONTINUE
56                CONTINUE
55             CONTINUE
C
