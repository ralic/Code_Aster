...
C
      REAL*8 SIGMAM(32),EFFORM(32),EFFINT(32),EFFORP(32)
C            SIGMAM : EFFORTS DANS LE REPERE UTILISATEUR  A T "-"
C            EFFORM : EFFORTS DANS LE REPERE DE L'ELEMENT A T "-"
C            EFFINT : EFFORTS DANS LE REPERE DE L'ELEMENT A T "+"
C            EFFORP : EFFORTS DANS LE REPERE UTILISATEUR  A T "+"
...
C
      REAL*8 T2EV(4),T2VE(4),T1VE(9), C, S
      ...
      REAL*8 LAMBDA, DEUXMU, DEUMUF, LAMF, GT, GC, GF, SEUIL, ALPHAF
      REAL*8 R8BID,TREF,DTMOY,DTGRA,ALPHAT,DEPSTH,DKHITH,EPSTH,KHITH
      REAL*8 ALPHA,BETA
      REAL*8 R8DGRD
...
C LORS DE L AJOUT DE VARIABLE INTERNE IL FAUT INCREMENTER ECR ET ECRP
      ...
      REAL*8 MATR(50), SIGM(8),ALFMC, TMOY, TGRA
...
C SIGMAM : EFFORTS DANS REPERE UTILISATEUR
...
C
C     T2EV : MATRICE DE PASSAGE (2x2) ; UTILISATEUR -> INTRINSEQUE
C     T2VE : MATRICE DE PASSAGE (2x2) ; INTRINSEQUE -> UTILISATEUR
C
...
C
C --- PASSAGE DES EFFORTS GENERALISES AUX POINTS D'INTEGRATION
C     DU REPERE UTILISATEUR AU REPERE INTRINSEQUE
C
...
C   AIRE DE SURFACE APPARTENANT AU POINT DE G.
 ...
 8510       CONTINUE
     ...
     &     GC,GF,SEUIL,ALPHAF,ALFMC,EP,LRGM,IPG,THER,TREF,DTMOY,DTGRA,
     &     TMOY,TGRA,ALPHAT)

C     CALCUL DE LA DEFORMATION THERMIQUE
          IF(THER) THEN
...
C     ENDOMMAGEMENT SEULEMENT
     ...
     &                  SEUIL,ALPHAF,ALFMC,CRIT,CODRET)
...
C --- PASSAGE DES EFFORTS GENERALISES AUX POINTS D'INTEGRATION
C     DU REPERE INTRINSEQUE AU REPERE LOCAL
C     STOCKAGE DES EFFORTS GENERALISES
C
