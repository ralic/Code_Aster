...
C            DMF:    MATRICE DE RIGIDITE TANGENTE MATERIELLE (COUPLAGE)
      ...
      REAL*8 MEFL(2*NNO*3*NNO), WORK(3*NNO*3*NNO), DULP(6,NNO)
...
C     ------------------ PARAMETRAGE ELEMENT ---------------------------
      INTEGER  NDIM, NNOEL, NNOS, NPG, J1, J2, IAD, I1
      INTEGER  IPOIDS, ICOOPG, IVF, IDFDX, IDFD2, JGANO, KL
      INTEGER  JTAB(7), COD, I, IADZI, IAZK24, KSP,ICOU, ICPG,IER, IGAUH
      INTEGER  INO, IPG, IRET, ISP,IVSP, J, K, NBSP, NBVAR, NDIMV
      REAL*8   DEUX, RAC2, QSI, ETA, CARA(25), JACOB(5), DISTN, LC, CTOR
      REAL*8   DEDT(6),DEDTM(6),DEDTF(6),PGL(3,3),ANGMAS(3), KTAN(300)
      REAL*8   SIGM(6),SIGP(6),SIGMS(6),SIGPS(6),SIPAS(6),KPRIMU(24)
      REAL*8   SIGBID(360),VARBID(120)
...
C --- INITIALISE A R8VIDE (ON NE S'EN SERT PAS)
     ...
     &                  VARMOI(IVSP,IPG),OPTI,ANGMAS,LC,
