...
C     INITIALISATION DES DIMENSIONS DES DDLS X-FEM
      CALL XTEINI(NOMTE,NFH,NFE,IBID,DDLC,NNOM,DDLS,NDDL,DDLM,NFISS)
...
C            DO 50 I = 1,3*NNO
   ...
   50     CONTINUE
          ...
          CALL XNMGR(NNO,NFH,NFE,DDLC,IGEOM,
     &               ZR(IINSTM),ZR(IINSTP),ZR(IDEPLP),ZR(ICONTM),
     &               ZR(IVARIP),TYPMOD,OPTION,ZI(IMATE),
     &               ZK16(ICOMPO),LGPG,ZR(ICARCR),ZR(JPINTT),
     &               ZI(JCNSET),ZI(JHEAVT),ZI(JLONCH),ZR(JBASLO),
     &               ZR(IDEPLM),ZR(JLSN),ZR(JLST),NFISS,JFISNO,
     &               ZR(ICONTP),ZR(IVARIM),ZR(IMATUU),ZR(IVECTU),CODRET)
