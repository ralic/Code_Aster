...
C
      ...
      INTEGER IADEP1,IADEP2,IADED1,IADED2,IAVA1,IAVA2,IAPTM1,IAPTM2
      INTEGER IGD1,IGD2,I
      INTEGER IACMP,NCMPM1,NCMPM2,IATYMA,INO,ITABP(7),ITABD(7)

      REAL*8 DFDX(9),DFDY(9),HE,HF,POIDS
      REAL*8 FORPX,FORPY,FORDX,FORDY,FPPX,FPPY,FPDX,FPDY
      REAL*8 FRPX(9),FRPY(9),FRDX(9),FRDY(9),RHO,NOR,R8BID
      REAL*8 ERREST,SIGCAL, NUEST,COEFF,ORIEN
...
C
        CALL ERE2D1(NOMTE,NNO,IPG,IGEOM,IVF,IADP,NBCMP,DFDX,DFDY,
     &              POIDS,DSPX,DSPY,R8BID)
C
C ------- CALCUL DE LA DIVERGENCE ET DE LA NORME DE SIGMA PB. DUAL -----
C
        CALL ERE2D1(NOMTE,NNO,IPG,IGEOM,IVF,IADD,NBCMP,DFDX,DFDY,
     &              R8BID,DSDX,DSDY,R8BID)
C ------- R8BID AU LIEU DE POIDS SINON ERREUR EN AXI (POIDS*R*R)
C
...
C
          CALL ERE2D2(INO,TYPEMA,TYPMAV,IREFP1,IVOIS,IADP,NBCMP,
     &                SGP11,SGP22,SGP12)
C
C ------- CALCUL DU SAUT DE CONTRAINTE ENTRE ELEMENTS PB. DUAL ---------
C
          CALL ERE2D2(INO,TYPEMA,TYPMAV,IREFD1,IVOIS,IADD,NBCMP,
     &                SGD11,SGD22,SGD12)
C
...
C
           CALL ERE2D3(INO,IREFP1,IREFP2,IVOIS,IGEOM,IADP,TYPEMA,NBCMP,
     &                 INST,NX,NY,TX,TY,SIGP11,SIGP22,SIGP12,CHPX,CHPY)
C
C ------- CALCUL EFFORTS SURFACIQUES ET DES CONTRAINTES PB. DUAL -------
C
            CALL ERE2D3(INO,IREFD1,IREFD2,IVOIS,IGEOM,IADD,TYPEMA,NBCMP,
