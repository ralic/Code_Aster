...
C --------- FIN  DECLARATIONS  NORMALISEES  JEVEUX ---------------------
     ...
     &       DFDY(27),DFDZ(27),TEM,POIDS,VECTT(NBPT),DIRE(3),ORIG(3),
...
C PHENOM MAT DERIVEE
      IF (LSENS) THEN
        ...
        IF (PHESEN.NE.PHENOM) CALL UTMESS('F','TE0061',
     &                             '! PB PHESEN.NE.PHENOM !')
      END IF
...
C IDEM CAS TRANSITOIRE A PARTIR DU SECOND PAS DE TEMPS
          CALL RCVALA(ZI(IMATE),PHENOM,1,'INST',VALPAR,2,NOMRES,VALRES,
     &                CODRET,'FM')
          LAMBDA = VALRES(1)
          CP = VALRES(2)
        END IF
...
C CALCUL STD
     ...
     &                CODRET,'FM')
          ...
          CP = 0.D0
        ELSE
          CALL RCVALA(ZI(IMATE),PHENOM,1,'INST',VALPAR,4,NOMRES,VALRES,
     &                CODRET,'FM')
          LAMBOR(1) = VALRES(1)
          LAMBOR(2) = VALRES(2)
          LAMBOR(3) = VALRES(3)
