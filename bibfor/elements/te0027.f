...
C DECLARATION VARIABLES LOCALES
      ...
      INTEGER I,J,K,KK,L,M,KP,NDIM,COMPT,JVAL,NBVARI,IRET
...
C - RECUPERATION DES CHARGES ET DEFORMATIONS INITIALES ----------------
  ...
  180     CONTINUE
     ...
     &                ZR(IDFDK),ZR(IDEPI),RBID,DFDI,F,EPS,RBID)
          DO 190 I = 1,NCMP
...
C - CALCUL DES ELEMENTS GEOMETRIQUES
        ...
        CALL NMGEOM(NDIM,NNO,.FALSE.,GRAND,ZR(IGEOM),KP,ZR(IPOIDS+KP-1),
     &              ZR(IVF+L),ZR(IDFDE),ZR(IDFDN),ZR(IDFDK),ZR(IDEPL),
...
C -  CONTRAINTES LAGRANGIENNES (SIGL),ENERGIE LIBRE ET DERIVEE / T
        ...
        IF (INCR) THEN
          CALL NMPLRU(NDIM,TYPMOD,ZI(IMATE),COMPOR,TG,TREF,PPG,EPS,EPSP,
     &                RP,ENERGI)
          DO 430 I = 1,3
