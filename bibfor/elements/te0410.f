...
C     LE CPU CAR LES APPELS A WKVECT DANS LES TE SONT COUTEUX.
      ...
      IF ( OPTION(1:9) .EQ. 'DEGE_ELGA'   .OR.
     &      OPTION(1:9) .EQ. 'DEGE_ELNO' ) THEN
         CALL VDXEDG(NOMTE,OPTION,ZR(JGEOM),NB1,NPGSR,EDGPG,DEFGT)
      ELSE 
...
C
C
      ELSEIF ( OPTION(1:9) .EQ. 'DEGE_ELGA') THEN
        CALL TECACH('OOO','PDEFOPG','E',7,ITAB,IRET)
        ICONTR=ITAB(1)

C ---   PASSAGE DES DEFORMATIONS DANS LE REPERE UTILISATEUR 
C       ET STOCKAGE DES DEFORMATIONS:

        CALL VDEFRO(NPGSN,MATEVN,EDGPG,ZR(ICONTR))

      ELSEIF ( OPTION(1:9) .EQ. 'DEGE_ELNO') THEN
        CALL TECACH('OOO','PDEFOGR','E',7,ITAB,IRET)
        ICONTR=ITAB(1)
