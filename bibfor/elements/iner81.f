...
C MODIF ELEMENTS  DATE 09/09/98   AUTEUR ACBHHCD G.DEVESA 
C ======================================================================
...
C
      CALL WKVECT(NOMRES(1:19)//'.VALE',CLASSE//' V R',3*NBDEF,LDRES)
C
...
C
         IAD = (IF-1)*NBDEF
         DO 20 I=1,NBDEF
           CALL R8COPY(NEQ,ZR(IDBASE+(I-1)*NEQ),1,ZR(LTVEC1),1)
           CALL ZERLAG(ZR(LTVEC1),NEQ,ZI(IDDEEQ))
           ZR(LDRES+IAD+I-1) = R8DOT(NEQ,ZR(LTVEC1),1,ZR(LTVEC2),1)
   20    CONTINUE
C
   30 CONTINUE
C
