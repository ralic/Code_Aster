...
C      EFGE_ELNO_DEPL ELEMENT: MET3SEG3 MET6SEG3 MET3SEG4
      ...
      REAL*8 TREF,VPG(4),TEMP
      REAL*8 SIGTH(2),HK(4,4),VNO(4)
      ...
      INTEGER JNBSPI,ITAB(8),NITER,ITER,IRET1,IRET2,IRET3
      INTEGER NDIM,NNOS,JCOOPG,IDFDK,JDFD2,JGANO
      REAL*8 EPSTHE,TMOY1,TINF1,TSUP1,HHK,COE1,ALPHAF,BETAF
...
C PASSAGE DU REPERE LOCAL AU REPERE GLOBAL
  ...
  200   CONTINUE
      ELSE IF (OPTION(1:14).EQ.'EFGE_ELNO_DEPL') THEN
        ...
        NOMRES(2) = 'NU'
        NOMRES(3) = 'ALPHA'
        CALL MOYTEM('RIGI',NPG,1,'+',VALPAR,IRET2)
        NBPAR = 1
        NOMPAR = 'TEMP'
...
C  FONCTIONS DE FORMES AUX POINT DE GAUSS
  ...
  290     CONTINUE
     ...
     &                  VTEMP)
          END IF
          DO 340 IGAU = 1,NPG
            CALL VERIFG('RIGI',IGAU,3,'+',ZI(IMATE),'ELAS', 
     &                  1,EPSTHE,IRET)
