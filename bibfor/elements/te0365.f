...
C MODIF ELEMENTS  DATE 24/08/2005   AUTEUR MABBAS M.ABBAS 
C ======================================================================
...
C
      INTEGER NNE,NNM,NDIM,INDCO,INADH,INDM,DIRLG
      INTEGER IGEOM,IDEPL,IVECT,JPCF,IDEPM,NDDL
      INTEGER MGEOM,MDEPL,TYPFRO,IFORM
...
C
      DO 20 I = 1,NNE
        DO 10 J = 1,NDIM
          ZR(IGEOM+(I-1)*NDIM+J-1) =
     &      ZR(IGEOM+(I-1)*NDIM+J-1) + ZR(IDEPM+(I-1)*(2*NDIM)+J-1)
 10     CONTINUE
 20   CONTINUE
C
      DO 21 I = 1,NNM
...
C
      DO 212 I = 1,NDIM
        GEOMM(I) = 0.D0
        DO 113 J = 1,NNM
          GEOMM(I) = GEOMM(I) + FFM(J)*ZR(IGEOM+NNE*NDIM+(J-1)*NDIM+I-1)
 113    CONTINUE
 212  CONTINUE
C
...
C
      DO 213 I = 1,2*NDIM
        ...
        DEPLME(I) = 0.D0
        DO 114 J = 1,NNE
          DEPLE(I) = DEPLE(I) + FFE(J)*ZR(IDEPL+(J-1)*(2*NDIM)+I-1)
          DEPLME(I) = DEPLME(I) + FFE(J)*ZR(IDEPM+(J-1)*(2*NDIM)+I-1)
 114    CONTINUE
 213  CONTINUE
C
C  --MAITRE------------
C
      DO 214 I = 1,NDIM
        ...
        DO 117 J = 1,NNM
          DEPLM(I) =
     &      DEPLM(I) + FFM(J)*ZR(IDEPL+NNE*(2*NDIM)+(J-1)*NDIM+I-1)
          DEPLMM(I) =
     &      DEPLMM(I) + FFM(J)*ZR(IDEPM+NNE*(2*NDIM)+(J-1)*NDIM+I-1)
 117    CONTINUE
 214  CONTINUE
C
...
C
          IF (IFORM .EQ. 1) THEN
            DO 40 I = 1,NNE
              VTMP((I-1)*(2*NDIM)+NDIM+1) =
     &          -PDS*JAC*DEPLE(NDIM+1)*FFE(I)/COEFCA
 40         CONTINUE
          ELSE
            DO 41 I = 1,NNE
              VTMP((I-1)*(2*NDIM)+NDIM+1) =
     &          -PDS*JAC*DEPLE(NDIM+1)*FFE(I)*DT/COEFCA
 41         CONTINUE
          END IF
          GOTO 220
C
...
C
            DO 70 I = 1,NNE
              DO 60 J = 1,NDIM
                VTMP((I-1)*(2*NDIM)+J) =
     &            -PDS*JAC*(DEPLE(NDIM+1)-COEFCA*JEU)*FFE(I)*NORM(J)
 60           CONTINUE
 70         CONTINUE
C
...
C
            DO 90 I = 1,NNM
              DO 80 J = 1,NDIM
                VTMP(NNE*(2*NDIM)+(I-1)*(NDIM)+J) =
     &            PDS * JAC * (DEPLE(NDIM+1)-COEFCA*JEU) * FFM(I) *
     &            NORM(J)
 80           CONTINUE
 90         CONTINUE
C
...
C
              DO 71 I = 1,NNE
                DO 61 J = 1,NDIM
                  VTMP((I-1)*(2*NDIM)+J) =
     &              -
     &              PDS*JAC*(DEPLE(NDIM+1)-COEFCA*(JDEPP-JDEPM)/DT)*
     &              FFE(I)*NORM(J)
 61             CONTINUE
 71           CONTINUE
C
...
C              
          ELSEIF (NDIM .EQ. 3) THEN
            DO 31 K = 1,NDIM
              TT(1) =
     &          (DEPLE(NDIM+1+1)*TAU1(K)+DEPLE(NDIM+1+2)*TAU2(K))*
     &          TAU1(K) + TT(1)
 31         CONTINUE
            DO 32 K = 1,NDIM
              TT(2) =
...
C
          DO 101 I = 1,NNE
            DO 102 L = 1,NDIM-1
C              IF (INDM .EQ. 3.D0 .AND. L .EQ. DIRLG) THEN
C              VTMP((I-1)*(2*NDIM)+NDIM+1+L) = 0.D0
C              ELSEIF (INDM .EQ. 2.D0 .AND. L .EQ. DIRLG) THEN
C              VTMP((I-1)*(2*NDIM)+NDIM+1+L) = 0.D0
C              ELSEIF (INDM .EQ. 1.D0 .AND. L .EQ. DIRLG) THEN
C              VTMP((I-1)*(2*NDIM)+NDIM+1+L) = 0.D0
C              ELSE
...
C
          DO 190 I = 1,NNE
            DO 180 J = 1,NDIM
              VTMP((I-1)*(2*NDIM)+J) =
     &          -JAC*PDS*COEFFF*LAMBDA*VECTT(J)*FFE(I)
 180        CONTINUE
 190      CONTINUE
C
C       DDL DEPLACEMENT DE LA  SURFACE GEOMETRIQUE
C
          DO 210 I = 1,NNM
            DO 200 J = 1,NDIM
              VTMP(NNE*(2*NDIM)+(I-1)*NDIM+J) =
     &          JAC * PDS * COEFFF * LAMBDA * VECTT(J) * FFM(I)
 200        CONTINUE
 210      CONTINUE
C
C      DDL LAGRANGES FROTTEMENT
          IF (NDIM .EQ. 2) THEN
            ...
            DO 228 I = 1,NDIM
              INTER(1) =
     &          (DEPLE(NDIM+1+1)*TAU1(I)-RESE(I))*TAU1(I) + INTER(1)
 228        CONTINUE
C
          ELSEIF (NDIM .EQ. 3) THEN
            INTER(1) = 0.D00
            ...
            DO 233 I = 1,NDIM
              INTER(1) =
     &          (DEPLE(NDIM+1+1)*TAU1(I)+DEPLE(NDIM+3)*TAU2(I)-RESE(I))*
     &          TAU1(I) + INTER(1)
              INTER(2) =
     &          (DEPLE(NDIM+1+1)*TAU1(I)+DEPLE(NDIM+3)*TAU2(I)-RESE(I))*
     &          TAU2(I) + INTER(2)
