...
C
      ...
      INTEGER            NNO,KP,NPG1,IMATUU,ITEMP,ITEMPF,ICACO
...
C
      ...
      IF ( NOMTE(3:4) .EQ. 'CX' ) THEN
        ...
        ELSE
          ...
          BSS =-((C1+DEUX*CONS-H*COUR)*COSA/(R*COUR**3)+CONS/(COUR**2)
     &         -H/COUR)*COEFF1
          DSS =((CONS-H*COUR)/COUR**3+COSA*(C1+CONS*TROIS-DEUX*H*COUR)
     &         /(COUR**4*R))*COEFF1
     ...
     &          +CONS2*C3**3*(UN+TROIS*C3*COUR))*COEFF1
        ENDIF
        IF ( ABS(COSA) .LE. EPS .OR.
     &       ABS(COUR*R) .LE. EPS .OR.
     &       ABS(COSA-COUR*R) .LE. EPS .OR.
     &       CORREC.EQ.ZERO ) THEN
          CTS = COEFF1 * H
          BTS = ZERO
     ...
     &          /(COSA*(COUR*R-COSA)))*COEFF1
          BTS = -(-H*(UN/COUR+C3)+COSA*CONS/(COUR**2*(COSA-COUR*R))
...
C
     ...
     &                           -JACP*   BTT*SINA*VFI           *VFJ /R
              ...
              ZR(IJ3+2)=ZR(IJ3+2)+JACP*SINA*(DTT*SINA*VFI*VFJ/(R*R)
