...
!
        call utbtab('ZERO', 2, 2, nle, t2iu,&
                    xab, nlc)
        call utbtab('ZERO', 2, 2, mle, t2iu,&
                    xab, mlc)
!
...
!
        edglc(7+8*(i-1)) = edgle( 7+8*(i-1)) * t2iu(1, 1) + edgle(8+8*( i-1)) * t2iu(2, 1 )
        edglc(8+8*(i-1)) = edgle( 7+8*(i-1)) * t2iu(1, 2) + edgle(8+8*( i-1)) * t2iu(2, 2 )
120  end do
end subroutine
