...
C     ------------------------------------------------------------------
      ...
      REAL*8 DEUX,RAD,ANGARC,ANGS2,ZERO
     ...
     &          (NOMTE.EQ.'MECA_POU_D_TGM') ) THEN
      ...
      ELSE
        CH16 = NOMTE
        CALL UTMESS('F','TE0039','"'//CH16//
     &              '"    NOM D''ELEMENT INCONNU.')
      END IF
...
C        MX1, MY1, MZ1 MOMENTS DE TORSION ET FLEXION NOEUDS 1
        ...
        IF (ITSEC.EQ.1) THEN
          CALL UTMESS('A','CALCUL DE PM ET PM+PB DU RCCM G3000',
...
C           --- RECUPERATION DES COORDONNEES DES NOEUDS
     ...
     &           (ZR(LX+6)-ZR(LX+3))**2)
            IF (XL.EQ.ZERO) THEN
              CH16 = ' ?????????'
              CALL UTMESS('F','ELEMENTS DE POUTRE (TE0039)',
