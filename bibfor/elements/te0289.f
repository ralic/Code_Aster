...
C
      ...
      INTEGER NFH,NFE,SINGU,DDLC,NBSIGM,NBSIG,NBSECO(27)
      INTEGER JCNSET,JLONCH,JSIGPG
      INTEGER JOUT1,JOUT2,IDECPG
      INTEGER I,J,NSE,ISE,IN,INO,KPG,IC
...
C     INITIALISATION DES DIMENSIONS DES DDLS X-FEM
      CALL XTEINI(NOMTE,NFH,NFE,SINGU,DDLC,IBID,IBID,IBID,IBID,IBID)
C
...
C       BOUCLE NCMP DES CONTRAINTES
  ...
  121     CONTINUE
          ...
          DO 122 IN=1,NNO
            ZR(JOUT1-1+NBSIG*NNO*(ISE-1)+NBSIG*(IN-1)+IC)=VNO(IN)
 122      CONTINUE
...
C       BOUCLE SUR LES 4/3 SOMMETS DU SOUS-TETRA/TRIA
        DO 211 IN=1,NNO
          ...
          IF (INO.LT.1000) THEN
            ...
            DO 212 IC = 1,NBSIG
              TMP =  ZR(JOUT1-1+NBSIG*NNO*(ISE-1)+NBSIG*(IN-1)+IC)
