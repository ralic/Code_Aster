...
C
C      OPTION : 'G_LAGR_EPSI_R' OU 'G_LAGR_EPSI_F'
C
C
...
C
      ...
      INTEGER       IDEPL,IMATE,IFORC,IFORF,IGTHET
      INTEGER       COMPT,I,J,K,IDEFI,ITEMPS,NNOS,JGANO,IRET
C
      REAL*8        G,C1,C2,C3,R,ALPHA
      REAL*8        CP1,CP2,CP3,DP1,DP2,DP3,EXX,EYY,EXY,EZZ,TEMP
      REAL*8        DGRT,DIVT,EPS,POIDS,VF,ADET,GEYCS,GEXYCS
      REAL*8        DFDM(7),DFDX(9),DFDY(9),GUGT(5),DUDM(7),DEDM(8)
      REAL*8        GUC(4),GUC1(4),GEGT(4),GEC(4),VALPA2(2)
      REAL*8        GEGC(8),GEXC(2),GEYC(2),GEXYC(2),DTDM(7),TGD(2)
...
C
      ...
      IF (OPTION.EQ.'G_LAGR_EPSI_F') THEN
        FONC = .TRUE.
