...
C --- PRE-CALCULS
C
      IF ((PHASE.EQ.'ADHE').OR.(PHASE.EQ.'GLIS')
     &   .OR.(PHASE.EQ.'PGLI')) THEN         
...
C ---   PRODUIT LAGR. FROTTEMENT. PAR MATRICE P
        DO 221 I=1,NDIM
          DO 222 J=1,NDIM
            PLAGFT(I) = MPROJT(J,I)*DLAGFT(I)+PLAGFT(I)
222       CONTINUE
221     CONTINUE
C ---   PRODUIT SEMI MULT. LAGR. FROTTEMENT. PAR MATRICE P  
        IF ((PHASE.EQ.'GLIS').OR.(PHASE.EQ.'PGLI')) THEN 
          DO 228 I=1,NDIM
            DO 229 J=1,NDIM
              PRESE(I) = MPROJT(J,I)*RESE(J)/NRESE+PRESE(I)
229         CONTINUE
228       CONTINUE
        ENDIF
...
C ---   PRODUIT SAUT PAR MATRICE P
        DO 721 I=1,NDIM
          DO 722 J=1,NDIM
            PDVITT(I) = MPROJT(I,J)*DVITET(I)+PDVITT(I)
722       CONTINUE
721     CONTINUE      
      ENDIF
C
   ...
   74   CONTINUE  
      ...
      ELSEIF (PHASE.EQ.'STAF') THEN
        DO 76 INOE = 1,NNE
          DO 66 IDIM = 1,NDIM
            II = NDIM*(INOE-1)+IDIM
C TERME DE STABILISATION NON PRESENT !
C            VECTEE(II) = VECTEE(II)-
C     &                   HPG*FFE(INOE)*JACOBI*PDVITT(IDIM)*
C     &                   LAMBDA*COEFFF*COEFFP
   66     CONTINUE
   76   CONTINUE
