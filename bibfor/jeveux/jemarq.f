...
C MODIF JEVEUX  DATE 16/07/2002   AUTEUR VABHHTS J.PELLET 
C ======================================================================
...
C
        ...
        CALL JJALLS ( LGD*LOIS,'V','I',LOIS,'INIT',IADMA,IADRS,KDESMA )
        ISZON(JISZON+KDESMA-1) = ISTAT(2)
        ISZON(JISZON+ISZON(JISZON+KDESMA-4)-4) = ISTAT(4)
        LGDUTI = 0
        LGP = 50
        CALL JJALLS ( LGP*LOIS,'V','I',LOIS,'INIT',IADMA,IADRS,KPOSMA )
        ISZON(JISZON+KPOSMA-1) = ISTAT(2)
        ISZON(JISZON+ISZON(JISZON+KPOSMA-4)-4) = ISTAT(4)
        LGPUTI = 0
...
C
        LSI = LGP
        LGP = 2*LGP
        CALL JJALLS (LGP*LOIS,'V','I',LOIS,'INIT',IADMA,IADRS,KTEMPO )
        ISZON(JISZON+KTEMPO-1) = ISTAT(2)
        ISZON(JISZON+ISZON(JISZON+KTEMPO-4)-4) = ISTAT(4)
        DO 100 K=1,LSI
          ISZON(JISZON+KTEMPO+K-1) = ISZON(JISZON+KPOSMA+K-1)
 100    CONTINUE
        CALL JJLIBP ( KPOSMA )
        KPOSMA = KTEMPO
      ENDIF
...
C
      ISZON(JISZON + KPOSMA + IPGC ) = LGDUTI
      LGPUTI = LGPUTI + 1
      IPGC = IPGC + 1
