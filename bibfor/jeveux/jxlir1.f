...
C MODIF JEVEUX  DATE 23/06/97   AUTEUR D6BHHJP J.P.LEFEBVRE 
C ======================================================================
...
C     ------------------------------------------------------------------
      INTEGER          LINDEX , NPAR
      PARAMETER      ( LINDEX = 11, NPAR = 11, NP2 = NPAR+3, NP8=8*NP2 )
...
C DEB ------------------------------------------------------------------
      ...
      IF ( .NOT. LEXIST) THEN
        CALL JVMESS ( 'S' , 'JXLIR1_01' ,
     &              ' FICHIER ASSOCIE A LA BASE '//NOMBAS(IC)//
     &              ' INEXISTANT ')
      ENDIF
...
C
      ...
      IF ( IERR .NE. 0 ) THEN
         CALL JVMESS ( 'S' , 'JXLIR1_02' ,
     &            ' ERREUR DE LECTURE DU 1ER BLOC DE '//NOMBAS(IC))
      ENDIF
      CALL CLOSDR ( NOM , IERR )
      IF ( IERR .NE. 0 ) THEN
         CALL JVMESS ( 'S' , 'JXLIR1_02' ,
     &            ' ERREUR LORS DE LA FERMETURE DE '//NOMBAS(IC))
      ENDIF
      DO 1 K=1,NPAR
        CARALU(K) = TAMPON(K+3)
 1    CONTINUE
