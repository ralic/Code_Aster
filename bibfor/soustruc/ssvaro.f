...
C     -------------------------------------------------------------
      ...
      IF (IER.GT.0) THEN
        CALL UTMESS('F','SSVARO','LA GRANDEUR "DEPL_R" DOIT AVOIR LES'
     +             //'COMPOSANTES (1 A 6) : DX,DY, ..., DRZ')
      END IF
      CALL DISMOI('F','NU_CMP_LAGR','DEPL_R','GRANDEUR',NULAG,KBID,IED)
      IF (NULAG.EQ.0) THEN
        CALL UTMESS('F','SSVARO','LA GRANDEUR "DEPL_R" DOIT AVOIR LA'
     +             //'COMPOSANTE "LAGR".')
      END IF
C
C
...
C     ------------------------
 ...
 88     CONTINUE
      ELSE
        CALL UTMESS('F','SSVARO','AUTORISE : "LG" OU "GL" PAS:'//SENS2)
      END IF
C
C
...
C     --------------------------------
      ...
      ELSE
        ...
        ELSE IF (TYPNOE.EQ.'EXTE') THEN
          ...
          IEQDEB=NDDLI+1
        ELSE
          CALL UTMESS('F','SSVARO','ERREUR PROGRAMMEUR')
        END IF
      END IF
C
C
...
C       --ICUMUL COMPTE LA SOMME DES COMPOSANTES PRESENTES SUR LE NOEUD
        ELSE
          CALL UTMESS('F','SSVARO','DDL NON PREVU. ON ATTEND: DX ')
        END IF
C
        DMI=0
...
C             3 = DX + DY
          IF (ICMPP.EQ.2) THEN
            DMI=2
            IF (ICUMUL.NE.3) CALL UTMESS('F','SSVARO','DDLS NON PREVUS')
          END IF
C
C         -- CAS "3D"
C             6 = DX + DY + DZ
          IF (ICMPP.EQ.3) THEN
            DMI=3
            IF (ICUMUL.NE.6) CALL UTMESS('F','SSVARO','DDLS NON PREVUS')
          END IF
C
...
C              9 = DX + DY + DRZ
            ELSE IF (ICUMUL.EQ.9) THEN
              DMI=2
            ELSE
              CALL UTMESS('F','SSVARO','DDLS NON PREVUS')
            END IF
          END IF
 21     CONTINUE
C
