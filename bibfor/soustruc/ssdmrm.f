...
C     -------------------------------------------------------------
     ...
     +               ZK8(IALIKG),N2)
        IF (N1.LT.0) CALL UTMESS('F','SSDMRM', 'LA LISTE DE '
     +    //'MAILLES EST PLUS LONGUE QUE LE NOMBRE TOTAL DE MAILLES.')
        IF (N1.NE.N2) CALL UTMESS('F','SSDMRM','LA LISTE DE MAILLES '
     +    //' N''A PAS LA MEME LONGUEUR QUE LA LISTE DE GROUP_NO.')
        IF (N1.LT.2) CALL UTMESS('F','SSDMRM','LA LISTE DE MAILLES '
     +    //' DOIT ETRE DE DIMENSION AU MOINS 2 POUR LE RECOLLEMENT')
C
        NBSMAR=N1
C
...
C
     ...
     +                 IBI,MAL,IER)
          ...
          CALL JELIRA(JEXNOM(MAL//'.GROUPENO',NOGNOI),'LONMAX',
     +                 NBNGNO,KBID)
...
C           POUR NE PAS LE DETRUIRE A CHAQUE FOIS, ON ALLOUE PLUS GRAND
     ...
     +                   IBI,MAL,IER)
            ...
            CALL JELIRA(JEXNOM(MAL//'.GROUPENO',NOGNOJ),'LONMAX',
     +                   NBNGNO,KBID)
