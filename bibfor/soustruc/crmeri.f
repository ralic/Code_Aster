...
C
      ...
      CHARACTER*16 NOMROU,NOMCHP,NORESU,MAELR
      CHARACTER*19 CHAMNO,CH1S,NU,MODID,MESTIT
      CHARACTER*24 VNOEUD,VRANGE,BASEPR,NOEUMS,BASEIT,VSU,MESINT,RANGMS
...
C RECUPERATION DES ELEMENTS RELATIFS A L'EXPANSION
      NOEUMS = PROMES//'.VNOEUD'
      CALL JELIRA(NOEUMS,'LONUTI',NBMESU,K8BID)
      CALL JELIRA(NOEUMS,'ORIG',IBID,NOMCHP)

      RANGMS = PROMES//'.VRANGE'
      CALL JELIRA(RANGMS,'ORIG',IBID,MODLMS)

      BASEPR = PROMES//'.BASEPR'
      CALL JEVEUO(BASEPR,'L',LRED)
      CALL JELIRA(BASEPR,'ORIG',IBID,BASEMO)
...
C CREATION DE LA BASE RESTREINTE AUX DDL EXTERIEUR
        ...
        DO 220 IMOD = 1,NBVECB
          ...
          IF (IRET .NE. 0) THEN
            ...
            CALL UTIMPI('S', ' NUME_ORDRE ',1, ZI(LORD-1+IMOD) )
            CALL UTFINM
          END IF
...
220     CONTINUE
        ...
        CALL JEECRA (VRANGE,'ORIG',IBID,MODLMS )
        CALL JEECRA (BASEIT,'ORIG',IBID,BASEMO )
...
C SAUVEGARDE TIT*PHIid POUR VERIFICATION ORTHOGONALITE MODES
        CALL WKVECT (MESTIT, 'G V R', NDDLE*NBORD, LTITP)
...
130     CONTINUE
        ...
        CALL JEECRA (VSU,'ORIG',IBID,MODMES )
