...
C MODIF SOUSTRUC  DATE 17/04/96   AUTEUR VABHHTS J.PELLET 
C ======================================================================
...
C
      CHARACTER*1  BASE
      CHARACTER*8  VECEL,KBID
      CHARACTER*14 NU
      CHARACTER*8  LIPARA(5)
      REAL*8       EPSI
      CHARACTER*8  NOMO,MATE,CARA
      INTEGER      NPREC
C --------------- COMMUNS NORMALISES  JEVEUX  --------------------------
      ...
      REAL*8 ZR
      COMPLEX*16 ZC
      LOGICAL ZL,EXISTE,ASS1ER,EXIMAT
      CHARACTER*8 ZK8,VPROF,NOMCAS
      CHARACTER*16 ZK16
      CHARACTER*19 VECAS
      CHARACTER*24 ZK24,CHMATE
      CHARACTER*32 ZK32
      CHARACTER*80 ZK80
...
C
      ...
      CARA  = ZK8(IAREFM-1+4)
      NU= ZK8(IAREFM-1+5)
      IF (NU(1:8).NE.NOMACR) CALL JXABOR()
C
      VECEL = '&&VECEL'
...
C       ------------------------
        CALL GETVTX('CAS_CHARGE','SUIV',IOCC,1,1,KBID,N1)
        IF (KBID(1:3).EQ.'OUI') THEN
          ZK8(IALICH-1+1)= 'OUI_SUIV'
        ELSE
          ZK8(IALICH-1+1)= 'NON_SUIV'
        END IF
        CALL GETVID('CAS_CHARGE','CHARGE',IOCC,1,0,KBID,N1)
        IF (-N1.GT.NCH) CALL UTMESS('F','SSCHGE','LISTE DE CHARGES '
     +                   //'TROP LONGUE')
        CALL GETVID('CAS_CHARGE','CHARGE',IOCC,1,-N1,ZK8(IALICH+1),N2)
C
C       -- INSTANT:
C       -----------
        CALL GETVR8('CAS_CHARGE','INST',IOCC,1,1,TIME,N2)
C
