...
!       -----------------------------------------------------
        call amumpu(1, 'C', kxmps, usersm, ibid,&
                    lbid, k12bid)
...
! ---  ICNTL(14): ON MODIFIE DES PARAMETRES POUR LA NOUVELLE TENTATIVE
                        ...
                        if ((iaux.eq.-9) .or. (ifact.eq.(ifactm-1))) cmpsk%icntl(22)=1
                        if (niv .ge. 2) then
                            vali(1)=cmpsk%icntl(14)/pcentp(2)
                            vali(2)=cmpsk%icntl(14)
                            vali(3)=ifact
                            vali(4)=ifactm
                            if (.not.lpreco) call u2mesi('I', 'FACTOR_58', 4, vali)
                        endif
                    endif
!
...
! ---  ICNTL(23): ON MODIFIE DES PARAMETRES POUR LA NOUVELLE TENTATIVE
                    if (niv .ge. 2) then
                        ...
                        if (.not.lpreco) call u2mesi('I', 'FACTOR_85', 1, vali)
                    endif
                    lpb13=.true.
                    cmpsk%icntl(23)=0
                else
...
!       ------------------------------------------------
        ...
        if (valr(1) .ge. valr(2)) then
            if ((niv.ge.2) .and. (.not.lpreco)) call u2mesr('I', 'FACTOR_73', 2, valr)
        endif
        if (cmpsk%infog(1) .eq. 0) then
!              -- C'EST OK
...
!       ------------------------------------------------
        call amumpu(2, 'C', kxmps, k8bid, nprec,&
                    lresol, k12bid)
!
!       ------------------------------------------------
!        RECUPERATION DU DETERMINANT SI NECESSAIRE:
!       ------------------------------------------------
        call amumpu(4, 'C', kxmps, k8bid, ibid,&
                    lbid, k12bid)
!
