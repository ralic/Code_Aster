...
C ----------------------------------------------------------------------
      ...
      INTEGER NPARIO,NBOBMX,NPARIN,NBSD,IMUMPS
...
C    POUR SAVOIR SI SOLVEUR MUMPS PARALLELE DISTRIBUE. SI C'EST LE CAS
C    ON TARIT LE VOLUME DE DONNEES.
...
C     -----------------------------------------------------------------
      ...
      IF (IRET.NE.0) THEN
        ...
        IF (OPTION(1:9).NE.'NSPG_NBVA') THEN
          LMUMPS=.TRUE.
...
C            '&MUMPS.MAILLE.NUMSD'
     ...
     &              ((IMA.GT.0).AND.(ZI(IMUMPS+IMA).GE.0)))
     &                ZL(IFETI2+IIEL)=.TRUE.
              ENDIF
   68       CONTINUE
C       -- MONITORING
...
C         IMPRESSIONS DE DEBUG POUR DETERMINER LE TEXXXX COUPABLE :
          ...
          IF (LFETMO.OR.LFETTD.OR.LMUMPS) CALL JEDETR(KCAL)
        END IF
   80 CONTINUE
C     ---FIN BOUCLE IGR
