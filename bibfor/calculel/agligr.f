...
C
      ...
      CALL JEDUPO(LIGRCH//'.LIEL', 'G', LIGR1,.FALSE.)
      CALL JEDETR(LIGRCH//'.LIEL')

      CALL JELIRA(LIGRCH//'.NEMA','LONT',LON2,K8BID)
      CALL JELIRA(LIGRCH//'.NEMA','NMAXOC',NMAX2,K8BID)
      CALL JELIRA(LIGRCH//'.NEMA','CLAS',IBID,BASE)
      CALL JEDUPO(LIGRCH//'.NEMA', 'G', LIGR2,.FALSE.)
      CALL JEDETR(LIGRCH//'.NEMA')
C
C --- COPIE DE LIGR1 ET LIGR2 SUR LIGRCH ET LIGRCH.NEMA
C
      LONG1 = 2*LONG
...
C
      CALL COCOPG(LIGR1, LIGRCH//'.LIEL', LONG, LONG1, 'G')
      CALL COCOPG(LIGR2, LIGRCH//'.NEMA', LONG, LONG2, 'G')
C
C --- AGRANDISSEMENT DE LIGRCH.LGNS
C
      CALL JUVECA(LIGRCH//'.LGNS',2*LONG2)
      CALL JEVEUO(LIGRCH//'.LGNS','E',IDLGNS)
C
C --- MENAGE
C
      CALL JEDETR(LIGR1)
