...
C=======================================================================
     ...
     &                    CHHYDR,CHSECH,K24B,COMPOR,CHTESE,CHDESE,
     &                    NOPASE,TYPESE,IRET)
              IF (IRET.GT.0) GO TO 70
...
C     ------------------------------------------------------------------
     ...
     &                    CHEPSA,CHFREQ,CHMASS,CHMETA,ZK8(JCHA),K24B,
     &                    ZERO,CZERO,CHDYNR,SOP,CHEPSP,LIGREL,BASE,
     &                    CHHYDR,CHSECH,CHVARI,COMPOR,CHTESE,CHDESE,
     &                    NOPASE,TYPESE,IRET)
              IF (IRET.GT.0) GO TO 150
              CALL RSNOCH(LERES1,OPTION,IORDR,' ')
  150       CONTINUE
...
C     ------------------------------------------------------------------
     ...
     &                    K24B,ZK8(JCHA),K24B,ZERO,CZERO,K24B,K24B,
     &                    CHELEM,LIGREL,BASE,CHSEQ,CHEEQ,K24B,COMPOR,
     &                    CHTESE,CHDESE,NOPASE,TYPESE,IRET)
...
C PREPARATION DES CALCULS D'INDICATEUR (CONNECTIVITE INVERSE, CHARGE)
            CALL RESTH2(MODELE,LIGREL,ZK8(JCHA),NCHAR,IFM,NIV,MA,CARTEF,
...
C     ------------------------------------------------------------------
     ...
     &                    K24B,COMPOR,CHTESE,CHDESE,NOPASE,TYPESE,IRET)
              IF (IRET.GT.0) GO TO 250
              CALL RSNOCH(LERES1,OPTION,IORDR,' ')
  250       CONTINUE
  260       CONTINUE
...
C     ------------------------------------------------------------------
     ...
     &                    IRET2)
              IF (IRET1.GT.0 .OR. IRET2.GT.0) GO TO 310
...
C     ------------------------------------------------------------------
     ...
     &                      IRET)
                CALL RSEXC2(2,2,RESUCO,'SIEF_ELGA_DEPL',IORDR,CHSIG,
     &                      OPTION,IRET)
                IF (IRET.GT.0) CALL UTMESS('A',NOMCMD,'PAS DE CHAMP'//
...
C     ------------------------------------------------------------------
     ...
     &                    K24B,K24B,K24B,K24B,TYPCOE,ALPHA,CALPHA,K24B,
     &                    SOP,CHELEM,LIGREL,BASE,K24B,CHEEQ,K24B,COMPOR,
     &                    CHTESE,CHDESE,NOPASE,TYPESE,IRET)
              IF (IRET.GT.0) GO TO 390
              CALL RSNOCH(LERES1,OPTION,IORDR,' ')
  390       CONTINUE
...
C     ------------------------------------------------------------------
     ...
     &                    CHTEMP,CHTREF,K24B,CHNUMC,K24B,K24B,K24B,
     &                    K24B,K24B,K24B,K24B,TYPCOE,ALPHA,CALPHA,K24B,
     &                    SOP,CHELEM,LIGREL,BASE,CHSEQ,K24B,K24B,COMPOR,
     &                    CHTESE,CHDESE,NOPASE,TYPESE,IRET)
              IF (IRET.GT.0) GO TO 400
              CALL RSNOCH(LERES1,OPTION,IORDR,' ')
  400      CONTINUE
