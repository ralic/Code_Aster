...
C     ------------------------------------------------------------------
      ...
      CHARACTER*8 SCALAI
      INTEGER LONGT,NCMPEL,MODE,J,IGD,IBD
      REAL*8 RZERO,RBID
      COMPLEX*16 CBID
      CHARACTER*4 CVAL,TYPCH,KMPIC
      CHARACTER*8 SCAL
...
C     -- IL FAUT COMMUNIQUER LE RESULTAT ENTRE LES PROCS :
      ...
      IF (KMPIC.EQ.'NON') THEN
        IF (SCAL(1:1).EQ.'I') THEN
          CALL MPICM1('MPI_SUM','I',LONGT,VI,RBID)
        ELSEIF (SCAL(1:1).EQ.'R') THEN
          CALL MPICM1('MPI_SUM','R',LONGT,IBID,VR)
