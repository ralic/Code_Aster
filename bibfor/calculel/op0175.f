...
C    DIMAKI = DIMENSION MAX DE LA LISTE DES RELMATIONS KIT
      ...
      CHARACTER*24 CHGEOM,CHCARA(18),CHVARC,CHVREF,CHTIM,CHTIMP,CHTIP
...
C    POUR LA PARTIE LOI DE COMPORTEMENT ZAC
      INTEGER JDIDO,JZAC,N1,KK,JPARA,JORDR
...
C     CALL JEVEUO(RESUEL//'           .INST','L',INSTL)
      CALL JEVEUO(RESUEL//'           .ORDR','L',JORDR)
      CALL JELIRA(RESUEL//'           .ORDR','LONUTI',N1,K8BID)
      CALL WKVECT('&&OP0175.INST','V V R',N1,INSTL)
      DO 1, KK=1,N1
        CALL RSADPA(RESUEL,'L',1,'INST',ZI(JORDR-1+KK),0,JPARA,K8BID)
        ZR(INSTL-1+KK)=ZR(JPARA)
 1    CONTINUE
