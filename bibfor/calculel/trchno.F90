...
!
    do 100 iocc = 1, nocc
        ...
        nonoeu = ' '
        noddl = ' '
        call getvid('CHAM_NO', 'CHAM_GD', iocc, iarg, 1,&
                    cham19, n1)
...
!
        call getvtx('CHAM_NO', 'NOM_CMP', iocc, iarg, 1,&
                    noddl, n1)
        if (n1 .ne. 0) then
            nl1 = lxlgut(lign1)
            nl2 = lxlgut(lign2)
        ...
        call getvtx('CHAM_NO', 'VALE_ABS', iocc, iarg, 1,&
                    ssigne, n1)
        ...
        call getvr8('CHAM_NO', 'TOLE_MACHINE', iocc, iarg, 1,&
                    epsi, iret)
        call getvtx('CHAM_NO', 'CRITERE', iocc, iarg, 1,&
                    crit, iret)
!
        call getvr8('CHAM_NO', 'VALE_CALC', iocc, iarg, 0,&
                    r8b, n1)
        call getvis('CHAM_NO', 'VALE_CALC_I', iocc, iarg, 0,&
                    ibid, n2)
        call getvc8('CHAM_NO', 'VALE_CALC_C', iocc, iarg, 0,&
                    c16b, n3)
        if (n1 .ne. 0) then
            nref=-n1
            typres = 'R'
            ...
            call getvr8('CHAM_NO', 'VALE_CALC', iocc, iarg, nref,&
                        zr( irefr), iret)
            ...
            call getvis('CHAM_NO', 'VALE_CALC_I', iocc, iarg, nref,&
                        zi( irefi), iret)
            ...
            call getvc8('CHAM_NO', 'VALE_CALC_C', iocc, iarg, nref,&
                        zc( irefc), iret)
        endif
! ----------------------------------------------------------------------
        lref=.false.
        call getvr8('CHAM_NO', 'PRECISION', iocc, iarg, 1,&
                    epsir, iret)
        if (iret .ne. 0) then
            lref=.true.
            call getvr8('CHAM_NO', 'VALE_REFE', iocc, iarg, 0,&
                        r8b, n1r)
            call getvis('CHAM_NO', 'VALE_REFE_I', iocc, iarg, 0,&
                        ibid, n2r)
            call getvc8('CHAM_NO', 'VALE_REFE_C', iocc, iarg, 0,&
                        c16b, n3r)
            if (n1r .ne. 0) then
...
! ----------------------------------------------------------------------
!
        call getvtx('CHAM_NO', 'TYPE_TEST', iocc, iarg, 1,&
                    typtes, n1)
!
        if (n1 .ne. 0) then
!
            ...
            lign2(nl2+17:nl2+17)='.'
!
            call getvtx('CHAM_NO', 'NOM_CMP', iocc, iarg, 0,&
                        noddl, n4)
            if (n4 .eq. 0) then
                nl1 = lxlgut(lign1)
