...
C MODIF CALCULEL  DATE 03/09/2002   AUTEUR G8BHHXD X.DESROCHES 
C ======================================================================
...
C   IN  NUME     :   NOM DE LA NUMEROTATION
...
C ----------------------- DECLARATIONS --------------------------------
      ...
      CHARACTER*8 BLANC,RESU,TYPE,OPER,MODELE,CHMAT
      CHARACTER*8 K8B,CHSIG,VECEL,KBID,CHTEMP,CHAR
      CHARACTER*8 NOEUBO,MA,NOM,TYPEMA,LICMP(4),LICHA
      CHARACTER*8 VECASS,LIREFE(10),ELREFE
      CHARACTER*14 NUME
      CHARACTER*16 PHEN,MODE,NOMTE
      CHARACTER*19 LIGRMO,CHAM,NOEUB,MO
      CHARACTER*24 LISCHA,INFCHA,SIGNO,SIGEL
      CHARACTER*24 TIME,LISVEC,VECT(6),FOMULT,LCHAR(1)
      CHARACTER*24 NUNUME,PRNO,CARAC,FF
...
C     -- ON VERIFIE QUE LE CHAM_ELEM N'EST PAS TROP DYNAMIQUE :
      ...
      IF (PHEN.NE.'MECANIQUE') THEN
        CALL UTMESS('F','SINOZ2','ERREUR: LE CALCUL DES CONTRAINTES '//
     &              'NE FONCTIONNE QUE POUR LE PHENOMENE MECANIQUE')
      END IF
...
C   L'APPARTENANCE OU NON AU BORD
      ...
      VECASS = '&&VECASS'
      LISVEC = VECEL//'.LISTE_RESU'
      CALL ASSVEC('V',VECASS,1,LISVEC,1.D0,NUME,' ','ZERO',1)

      NOEUB = VECASS
      CALL WKVECT('&&SINOZ2.NOEUBORD','V V L',NBNO,JNOEU)
      CALL JEVEUO(NOEUB//'.REFE','L',JREFE)
      NUNUME = ZK24(JREFE+1)
      CALL DISMOI('F','NB_EC','DEPL_R','GRANDEUR',NBEC,KBID,IER)
      CALL JEVEUO(JEXNUM(NUNUME(1:19)//'.PRNO',1),'L',JPRNO)
   ...
   80 CONTINUE
      IF (INOB.NE.NBNOB) THEN
        CALL UTMESS('F','SINOZ2','ERREUR NUMEROS DES NOEUDS BORDS')
      END IF
...
C    VERIFICATION DES TYPES DE MAILLE
      ...
      DO 90 IMA = 1,NBMA
        ...
        IF (TYPEMA(1:4).EQ.'TRIA') THEN
          ...
          IF (TYPEMA(5:5).NE.'3' .AND. TYPEMA(5:5).NE.'6') THEN
            CALL UTMESS('F','SINOZ2','ERREUR: LES ELEMENTS SUPPORTES'//
     &                  ' SONT TRIA3 OU TRIA6')
          END IF
     ...
     &        TYPEMA(5:5).NE.'9') THEN
            CALL UTMESS('F','SINOZ2','ERREUR: LES ELEMENTS SUPPORTES'//
     &                  ' SONT QUAD4 OU QUAD8 OU QUAD9')
          END IF
        END IF
   90 CONTINUE
      IF (NTRI.NE.0 .AND. NQUA.NE.0) THEN
        CALL UTMESS('F','SINOZ2','MAILLAGE MIXTE TRIA-QUAD NON SUPPORTE'
     &              //' POUR L ESTIMATEUR ZZ2')
      END IF
      IF (NTRI.EQ.0 .AND. NQUA.EQ.0) THEN
        CALL UTMESS('F','SINOZ2','ERREUR: LES MAILLES SUPPORTEES'//
     &              ' SONT TRIA OU QUAD')
      END IF

      MO = MODELE//'.MODELE    '
      CALL JEVEUO(MO//'.REPE','L',IA1)
...
C        RECUPERATION DES FCTS DE FORME DE L'ELEMENT
             ...
             CALL ELREF2(NOMTE,10,LIREFE,NBELR)
              ...
              CALL JEVEUO(CARAC,'L',ICARAC)
              NNO = ZI(ICARAC)
              NPG1 = ZI(ICARAC+2)
              NPG = NPG1

              FF = '&INEL.'//ELREFE//'.FF'
              CALL JEVEUO(FF,'L',IFF)
              IPOIDS = IFF
              IVF = IPOIDS + NPG
...
C    CALCUL DU SECOND MEMBRE B
              ...
              CALL ZZCALB(NUMAV,NPG,NNO,ZR(IVF),ZR(JCELV),X,Y,XMIN,XMAX,
     &                    YMIN,YMAX,B)
  160       CONTINUE
            IANEW = IANOB + NBNOBP
...
C    MOYENNAGE SUR LES NOEUDS BORD
      ...
      DO 260 I = 1,NBNOB
        ...
        IF (ZI(JPA-1+I).EQ.0) THEN
          CALL UTMESS('F','SINOZ2','ERREUR: UN ELEMENT DU MAILLAGE'//
     &            ' POSSEDE TOUS SES SOMMETS SUR UNE FRONTIERE. IL FAUT'
