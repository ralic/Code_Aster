...
C     --- VARIABLES LOCALES ---
      ...
      INTEGER IRET,IRET1,IRET2,IRET3,IERD
      ...
      CHARACTER*13 INPSCO
      CHARACTER*19 PFCHNO
      CHARACTER*16 NOMCMD,OPTION,OPTIO2,OPTIOX,K16B
...
C     ON RECUPERE LE TYPE DE MODE: DYNAMIQUE OU STATIQUE
      ...
      CALL GETVTX(' ','NORME',1,IARG,1,NORME,NOR)
...
C                  RESUCO -> CHDESE
C                  LERES0 -> CHAMGD
C                  VOIR LA "GLUTE" DANS MSCALC.F (_SENS)
...
C          * UTILISATION DU MOT-CLE FACTEUR EXCIT
                IF (NBCHRE.NE.0) THEN
                  ...
                  CALL GETVC8('EXCIT','COEF_MULT_C',IOCC,IARG,1,
     &                        CCOEF,L4)
...
C     POUR LE CALCUL DES OPTIONS SIEF_ELGA ET EFGE_ELNO
     ...
     &                      OPTION,IRET1)
                CALL RSEXC2(2,2,RESUCO,'EFGE_ELNO',IORDR,CHSIG,
     &                      OPTION,IRET2)
     ...
     &                      OPTION,IRET)
              ...
              IF (OPTION.EQ.'SIGM_ELNO') THEN
                CALL RSEXC2(1,1,LERES0,'SIEF_ELGA',IORDR,CHSIG,
     &                      OPTION,IRET)
                IF (IRET.GT.0) THEN
                  CALL ASSERT(.FALSE.)
                ENDIF
              ...
              ELSE
                CHTESE=' '
              ENDIF
              CALL MSCALC(OPTIO2,MODELE,CHAMGD,CHGEOM,MATE,CHCARA,
...
C ---- VERIF SENSIBILITE FIN
     ...
     &            TYSD.EQ.'COMB_FOURIER') THEN
              ...
              IF (TYPE.EQ.'DEPL') THEN
                ...
                FREQ=ZR(LFREQ)
              ENDIF
              CHFREQ='&&'//NOMPRO//'.OMEGA2'
              CALL MECACT('V',CHFREQ,'MAILLA',NOMA,'OME2_R',1,'OMEG2',
     &                    IBID,FREQ,CBID,K8B)
              IF (EXITIM) THEN
                CALL RSADPA(RESUCO,'L',1,'INST',IORDR,0,IAINST,K8B)
                TIME=ZR(IAINST)
...
C          CHAMP D'ENTREE POUR COQUES
              ...
              ENDIF
              CALL RSEXC1(LERES1,OPTION,IORDR,CHELEM)
              CALL MSCALC(OPTION,MODELE,K24B,CHGEOM,MATE,CHCARA,K24B,
...
C ---- VERIF SENSIBILITE FIN
     ...
     &                    OPTION,IRET2)
              ...
              CALL MSCALC(OPTIO2,MODELE,CHAMGD,CHGEOM,MATE,CHCARA,K24B,
...
C     --- OPTION: INDL_ELGA
C     ------------------------------------------------------------------
          ELSEIF (OPTION.EQ.'INDL_ELGA') THEN
C ---- VERIF SENSIBILITE
            IF (TYPESE.NE.0) THEN
              CODSEN=1
            ENDIF
...
C ---- VERIF SENSIBILITE FIN
     ...
     &                    IRET)
              ...
              CALL RSEXCH(RESUCO,'COMPORTEMENT',IORDR,COMPOR,IRET1)
              CALL RSEXC1(LERES1,OPTION,IORDR,CHELEM)
              CALL MSCALC(OPTION,MODELE,CHVAR2,K24B,MATE,K24B,K24B,K24B,
...
C ---- VERIF SENSIBILITE FIN
     ...
     &                    IRET)
              ...
              IF (IRET.GT.0)CHVARI=' '
              CALL RSEXCH(RESUCO,'COMPORTEMENT',IORDR,COMPOR,IRET1)
              CALL MSCALC(OPTION,MODELE,K24B,CHGEOM,MATE,CHCARA,CHTEMP,
