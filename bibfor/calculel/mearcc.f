...
C
      ...
      INTEGER      NBNOMX,NBMAMO,JMOMA,JLIMA,NBMAT
...
C --- ON VERIFIE QUE LES MAILLES FOURNIES SONT 2D (TRIA,QUAD)
      IF (NBMA2D.GT.0) THEN
...
C     -- FICHE 17654 : IL FAUT SE PROTEGER DES MAILLES QUI NE FONT PAS
C        PARTIE DU MODELE :
      CALL WKVECT('&&MEARCC.LIMAIL','V V I',NBMAT,JLIMA)
      CALL JEVEUO(MO//'.MAILLE','L',JMOMA)
      NBMAMO=0
      DO 77, K=1,NBMAT
         IF (ZI(JMOMA-1+K).GT.0) THEN
           NBMAMO=NBMAMO+1
           ZI(JLIMA-1+NBMAMO)=K
         ENDIF
 77   CONTINUE
