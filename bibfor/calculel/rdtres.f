      SUBROUTINE RDTRES(RESU1,RESU2,NOMA1,NOMA2,CORRN,CORRM)
      IMPLICIT NONE
...
C     -----  FIN  COMMUNS NORMALISES  JEVEUX  --------------------------
      ...
      INTEGER NBGREL,IGR,JCORRM,IEL,NBMA1,NBMA2,JCOINV
      INTEGER ITE,JMAIL2
      REAL*8 RBID,PREC
      COMPLEX*16 CBID
      CHARACTER*1 KBID
      CHARACTER*16 TYPRES,NOMSYM(200),NOPARA
      CHARACTER*8 MODEL1,CRIT,TYPE,MODEL2
...
C     1- FABRICATION DU LIGREL REDUIT (LIGREL)
C     -----------------------------------------
      MODEL1=' '
      CALL GETVID('RESTREINT','MODELE',1,1,1,MODEL1,IRET)
      IF (MODEL1.NE.' ') THEN
C       1.1- ON RESTREINT LE LIGREL DU MODELE SUR LES MAILLES RETENUES :
        CALL EXLIMA('RESTREINT','V',MODEL1,LIGREL)
...
C     ------------------------------------
      ...
      CALL RSUTNU(RESU1,'RESU',1,'&&RDTRES.NUME_ORDRE',NBORDR,PREC,CRIT,
     &            IRET)
      IF (IRET.NE.0) THEN
...
C     ------------------------------------
      CALL RSUTC4(RESU1,'RESU',1,200,NOMSYM,NBSYM,ACCENO)
...
C         -- REDUCTION DU CHAMP :
          CALL RDTCHP(CORRN,CORRM,CHP,CHPRE,NOMA1,NOMA2,LIGREL)
