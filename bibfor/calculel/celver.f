...
C     ------------------------------------------------------------------
      ...
      IF (KK.EQ.0) CALL UTMESS('F','CELVER',' LE CHAM_ELEM: '//CEL//
     +                         ' N''EXISTE PAS.')
            ...
            ELSE
              IF (NCDYN.NE.NCDYN1) THEN
                IF (ARRET.NE.'COOL') THEN
                  CALL UTMESS('F','CELVER','LE CHAM_ELEM:'//CEL//
     +                        ' N''A PAS LE MEME NOMBRE DE'//
     +                        ' CMPS DYNAMIQUES SUR TOUS SES ELEMENTS.')
                ELSE
                  IRET = 1
                END IF
              END IF
...
C     --------------------------------
     ...
     +                  ' A DES SOUS-POINTS.')
      ...
      ELSE
        CALL UTMESS('F','CELVER','TYPE DE VERIF. INCONNU.')
      END IF
