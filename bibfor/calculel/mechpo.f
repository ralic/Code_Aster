...
C MODIF CALCULEL  DATE 13/06/96   AUTEUR CIBHHRA R.MEDDOURI 
C ======================================================================
...
C
      ...
      CALL JEEXIN(LCHIN(NBOPT),IRET)
      IF (IRET.EQ.0) THEN
         CALL JEVEUO(CHDEPL(1:19)//'.REFE','L',LREFE)
         CALL JELIRA(CHDEPL(1:19)//'.VALE','LONMAX',NEQ,K8B)
         CALL CODENT(NBOPT,'D0',CH5(2:5))
         LCHIN(NBOPT) = SOUCHE(1:8)//CH5//'.PCHDY'
         IF ( OPTION(15:16) .EQ. '_C' ) THEN
            CALL VTCREA(LCHIN(NBOPT),ZK24(LREFE),'V','C',NEQ)
         ELSE
            CALL VTCREA(LCHIN(NBOPT),ZK24(LREFE),'V','R',NEQ)
         ENDIF
      ENDIF
C
      NBOPT = NBOPT+1
