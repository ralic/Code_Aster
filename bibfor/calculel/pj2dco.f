...
43    CONTINUE
      IF (KK.EQ.0) CALL UTMESS('F','PJ2DCO',
...
C           V(1+4(I-1)+4) : NUMERO DE LA MAILLE MERE DU IEME TRIA3
      ...
      DO 51,IMA=1,NMA1
        ...
        ELSE IF (ITYPM.EQ.NUTM(6)) THEN
          ICO=ICO+2
        ELSE
          CALL UTMESS('F','PJ2DCO','STOP 3')
        END IF
51    CONTINUE
      CALL WKVECT('&&PJXXCO.TRIA3','V V I',1+4*ICO,IATR3)
      ZI(IATR3-1+1)=ICO
      IF (ICO.EQ.0)
     &  CALL UTMESS('F','PJ2DCO','PAS DE MAILLES A PROJETER.')
...
C     ------------------------------------------------
     ...
     &  ZI(IABTCO),IFM,NIV,LDMAX,DISTMA)
        ...
        IF (NBTROU.EQ.0) THEN
          ...
          CALL UTMESS('F','PJ2DCO',NONO2//' PAS TROUVE.')
        END IF
