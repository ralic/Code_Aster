      SUBROUTINE PJ2DCO(MOCLE,MO1,MO2,NBMA1,LIMA1,NBNO2,LINO2,
...
C       ET CEUX DE M2 (DANS LE CAS DE MAILLAGES 2D)
C
...
C
      ...
      CHARACTER*14  BOITE
      PARAMETER    (NBTM=6)
      INTEGER      NUTM(NBTM), IFM, NIV, IBID
      CHARACTER*8  NOTM(NBTM), ELRF(NBTM)
      LOGICAL DBG

      LOGICAL LDMAX,LOIN,LOIN2
      REAL*8  DISTMA,DMIN,DMIN2
C DEB ------------------------------------------------------------------
      CALL JEMARQ()
...
C     ------------------------------------------------
     ...
     &                COBARY,ITR3,NBTROU,
     &  ZI(IABTDI), ZR(IABTVR), ZI(IABTNB), ZI(IABTLC),
     &  ZI(IABTCO),IFM,NIV,LDMAX,DISTMA,LOIN,DMIN)
        IF (NBTROU.EQ.2) ZI(IACOAM-1+INO2)=1
        IF (LOIN) LOIN2=.TRUE.
        IF (DMIN.GT.DMIN2) THEN
          DMIN2=DMIN
          INO2M=INO2
        ENDIF
...
C     -- EMISSION D'UN EVENTUEL MESSAGE D'ALARME:
      IF (LOIN2) THEN
        ...
        IF (ALARME.EQ.'OUI') THEN
          ...
          CALL U2MESG ('A','CALCULEL5_48',1,NONO2,0,0,1,DMIN2)
        ENDIF
      ENDIF
...
C     ----------------------------------------------------------
      CALL PJ2DTR(CORTR3,CORRES,NUTM,ELRF,ZR(IACOO1),ZR(IACOO2))
      DBG=.FALSE.
      IF (DBG) THEN
