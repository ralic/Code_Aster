...
C                         ON VEUT REDUIRE LE CHAM_ELEM_S
C                         LISTE DES COMPOSANTES A OTER DE CES1Z
C
C SI NBCMP > 0 : LES CMPS DU CHAM_ELEM_S REDUIT SONT DANS L'ORDRE
C                DE LICMP.
C SI NBCMP = 0 : LES CMPS DU CHAM_ELEM_S REDUIT SONT DANS L'ORDRE DE
C                CES1Z, TOUTES LES COMPOSANTES SONT PRESENTES.
C SI NBCMP < 0 : LES CMPS DU CHAM_ELEM_S REDUIT SONT DANS L'ORDRE
C                DE CES1Z EN OTANT LES COMPOSANTES PRESENTES DANS LICMP.
C
C REMARQUES :
C  - POUR REDUIRE SUR LES MAILLES (ET GARDER TOUTES LES CMPS)
C        NBCMP=0   LICMP= KBID          NBMA=N   LIMA= LISTE(1..N)
C  - POUR REDUIRE SUR LES CMPS (ET GARDER TOUTES LES MAILLES)
C        NBCMP=+N  LICMP= LISTE(1..N)   NBMA=0   LIMA= IBID
C  - POUR OTER DES CMPS (ET GARDER TOUTES LES MAILLES)
C        NBCMP=-N  LICMP= LISTE(1..N)   NBMA=0   LIMA= IBID
C
...
C
      JCE3C = JCE1C
      IF      (NBCMP.GT.0) THEN
C        CHAM_ELEM_S CES2 AVEC CMPS IMPOSEES DANS L'ORDRE DE LICMP
         NCMP2 = NBCMP
      ELSE IF (NBCMP.EQ.0) THEN
C        CHAM_ELEM_S CES2 AVEC MEME CMPS ET MEME ORDRE QUE CES1
         NCMP2 = NCMP1
      ELSE
C        CHAM_ELEM_S CES2 AVEC CMPS DANS LE MEME ORDRE QUE CES1
C        AUXQUELLES SONT ENLEVEES LES COMPOSANTES PRESENTES DANS LICMP
...
C     ---------------------------------------
      IF (NBCMP.GT.0) THEN
C        ON DEMANDE EXPLICITEMENT UNE LISTE DE COMPOSANTES SUR LES
C        MAILLES. TOUTES LES MAILLES SONT DONC AFFECTEES ET SE
C        RETROUVE AVEC LE MEME NOMBRE DE COMPOSANTES. IL SE PEUT
C        EGALEMENT QUE LE NOMBRE DE COMPOSANTES SUR CERTAINE MAILLES
C        SOIT SUPÉRIEUR A CE QU'IL ETAIT PRECEDEMMENT, ON NE PEUT DONC
C        PAS UTILISER ZI(JNBCMP) QUI EST LE MIN ==> -NBCMP
         CALL CESCRE(BASE,CES2,TYPCES,MA,NOMGD,NCMP2,LICMP,
     &               ZI(JNBPT),ZI(JNBSP),-NBCMP)
      ELSE
C        CHAM_ELEM_S AVEC COMPOSANTES DANS LE MEME ORDRE QUE CES1
         CALL CESCRE(BASE,CES2,TYPCES,MA,NOMGD,NCMP2,ZK8(JCE3C),
