      ...
      IMPLICIT NONE
      REAL*8  COBARY(4),GEOM1(*),GEOM2(*),D2,VOLU
...
C ======================================================================
C     BUT :
C       DETERMINER LA DISTANCE D ENTRE LE NOEUD INO2 ET LE TETR4 I.
...
C  OUT  COBARY(4)  R  : COORDONNEES BARYCENTRIQUES DE INO2 PROJETE SUR I
C  OUT  D2         R  : CARRE DE LA DISTANCE ENTRE I ET INO2
C  OUT  VOLU       R  : VOLUME DU TETR4 I


C ----------------------------------------------------------------------
      INTEGER K
      LOGICAL OK
      REAL*8 R8MAEM ,DP,L1,L2,L3,LA,LB,LC,LD
      REAL*8 A(3),B(3),C(3),D(3),M(3),AB(3),AC(3),AD(3),V(3)
C DEB ------------------------------------------------------------------
...
C     1. ON BOUCLE SUR LES 4 FACETTES :
C     ---------------------------------
      ...
      CALL  PJ3DA3(M,B,C,D,OK,L1,L2,L3,DP)
      IF ((OK).AND.(DP.LT.D2)) THEN
        D2=DP
        LB=L1
        LC=L2
      ...
      CALL  PJ3DA3(M,C,D,A,OK,L1,L2,L3,DP)
      IF ((OK).AND.(DP.LT.D2)) THEN
        D2=DP
        LC=L1
        LD=L2
      ...
      CALL  PJ3DA3(M,D,A,B,OK,L1,L2,L3,DP)
      IF ((OK).AND.(DP.LT.D2)) THEN
        D2=DP
        LD=L1
        LA=L2
...
C     2. ON BOUCLE SUR LES 6 ARRETES :
C     ---------------------------------
