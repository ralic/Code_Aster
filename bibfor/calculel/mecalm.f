...
C     POUR LE CALCUL DES OPTIONS SIEF_ELGA_DEPL ET EFGE_ELNO_DEPL
     ...
     &                    ALPHA,CALPHA,CHDYNR,SOP,CHELEM,K24B,LIGREL,
     &                    BASE,CHVARC,CHVREF,K24B,COMPOR,CHTESE,
...
C      A PARTIR DE SIEF_ELGA_DEPL
     ...
     &                    CHELEM,CHELEX,LIGREL,BASE,CHVARC,CHVREF,K24B,
...
C ---- VERIF SENSIBILITE FIN
     ...
     &                    ZERO,CZERO,CHDYNR,SOP,CHELEM,K24B,LIGREL,BASE,
...
C ---- VERIF SENSIBILITE FIN
     ...
     &                    ZERO,CZERO,CHDYNR,SOP,CHEPSP,K24B,LIGREL,BASE,
...
C    ------------------------------------------------------------------
          ELSE IF (OPTION.EQ.'PROJ_ELEM_SIGM') THEN
C ---- VERIF SENSIBILITE
            IF (TYPESE.NE.0) THEN
               CODSEN = 1
            ENDIF
...
C ---- VERIF SENSIBILITE FIN
     ...
     &                    OPTION,IRET2)
              IF (IRET1.GT.0 .AND. IRET2.GT.0) GO TO 787
...
C       ERZ2_ELEM_SIGM PAR RAPPORT A ERZ1_ELEM_SIGM
              ...
              ELSE
                CALL RSEXCH(RESUCO,'EPOT_ELEM_DEPL',IORDR,CHENEG,IRET4)
              ENDIF
              IF (IRET4.GT.0) THEN
...
C       CES DEUX COMPOSANTES SONT CONSTANTES PAR ELEMENT
     ...
     &                    ZR(JCOOR),PREC,LIGRMO,IORDR,CHELEM,TYPES)
...
C ---- VERIF SENSIBILITE FIN
     ...
     &                    K24B,K24B,K24B,K24B,TYPCOE,ALPHA,CALPHA,K24B,
     &                    SOP,CHELEM,K24B,LIGREL,BASE,CHVARC,CHEEQ,K24B,
...
C ---- VERIF SENSIBILITE FIN
     ...
     &                    CHTEMP,K24B,K24B,CHNUMC,K24B,K24B,K24B,
     &                    K24B,K24B,K24B,K24B,TYPCOE,ALPHA,CALPHA,K24B,
     &                    SOP,CHELEM,K24B,LIGREL,BASE,CHSEQ,K24B,K24B,
     &                    COMPOR,CHTESE,CHDESE,NOPASE,TYPESE,
     &                    CHACSE,IRET)
...
C ---- VERIF SENSIBILITE FIN
     ...
     &                    K24B,LIGREL,BASE,K24B,K24B,K24B,COMPOR,
     &                    CHTESE,CHDESE,NOPASE,TYPESE,CHACSE,IRET)
              IF (IRET.GT.0) GO TO 1410
              CALL RSNOCH(LERES1,OPTION,IORDR,' ')
 1410      CONTINUE
...
C ---- VERIF SENSIBILITE FIN
     ...
     &                    CHEPS,CHFREQ,CHMASS,CHMETA,ZK8(JCHA),' ',ZERO,
     &                    CZERO,CHDYNR,SOP,CHELEM,K24B,LIGREL,BASE,
     &                    CHVARC,CHVREF,CHVARI,COMPOR,CHTESE,CHDESE,
     &                    NOPASE,TYPESE,CHACSE,IRET)
