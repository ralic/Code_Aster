...
C
  ...
  121   CONTINUE
     ...
     >   'LE CALCUL DE L INDICATEUR D ERREUR NE SAIT PAS TRAITER LES '//
     >   'CHARGES DU TYPE DE '//LCHAR(I))
          IRET1 = IRET1 + 1
        ENDIF
   12 CONTINUE
C
...
C
      DO 13 , J = 2 , NTYCHX
        IF ( ITYCHA(J).GT.1 ) THEN
          CALL UTMESS('A',NOMPRO,
     > 'LE CHOIX '//KTYCH(J)//' APPARAIT AU MOINS DANS 2 CHARGES.')
          IRET1 = IRET1 + 1
        ENDIF
   13 CONTINUE
C
...
C
        ...
        IF (IRET1.NE.0) THEN
          ...
          IF (IRET.NE.0) THEN
            CALL UTMESS('F',NOMPRO,'ERREUR DANS ETENCA')
          END IF
