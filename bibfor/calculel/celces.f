...
C         '&&CELCES.CORR_CMP': CONTIENT LA CORRESPONDANCE ENTRE LE
...
C     -----------------------------------------------------------------
      CALL CMPCHA(CEL,'&&CELCES.NOM_CMP','&&CELCES.CORR_CMP',
     &            NCMP1,NCMPMX)
      CALL JEVEUO('&&CELCES.NOM_CMP','L',JNOCMP)
      CALL JEVEUO('&&CELCES.CORR_CMP','L',JNUCMP)
...
C     -------------------------------------------
      ...
      CALL CESCRE(BASE,CES,TYPCES,MA,NOMGD,NCMP1,ZK8(JNOCMP),ZI(JNBPT),
     &            ZI(JNBSPT),ZI(JNBCMP))
