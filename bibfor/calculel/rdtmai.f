...
C --- OBJET COORDO.REFE
      CALL WKVECT(COOREF,BASE//' V K24',2,IAD)
      ZK24(IAD)=NOMARE
...
C       'TOUT_GROUP_MA'='NON'
     ...
     &              NBGMA)
        DO 100 IGM=1,NBGMA
          ...
          CALL JELIRA(JEXNOM(NOMA//'.GROUPEMA',NOMGMA),'LONMAX',NBMA,
     &                K8B)
...
C       TOUT_GROUP_MA='OUI'
        ...
        DO 120 IGM=1,NBGMIN
          ...
          CALL JELIRA(JEXNUM(NOMA//'.GROUPEMA',IGM),'LONMAX',NBMA,K8B)
          NMPG=0
          PREMS=.TRUE.
          DO 110 IMA=1,NBMA
  ...
  120   CONTINUE
     ...
     &              NBGMNV)
        DO 140 IGM=1,NBGMNV
          ...
          CALL JELIRA(JEXNOM(NOMA//'.GROUPEMA',NOMGMA),'LONMAX',NBMA,
     &                K8B)
          CALL JEECRA(JEXNOM(GRPMAI,NOMGMA),'LONMAX',ZI(JNMPG+IGM-1),
     &                K8B)
          CALL JEVEUO(JEXNOM(NOMA//'.GROUPEMA',NOMGMA),'L',JADIN)
          CALL JEVEUO(JEXNOM(GRPMAI,NOMGMA),'E',JADOU)
          K=0
...
C       TOUT_GROUP_NO='OUI'
        ...
        DO 180 IGN=1,NBGNIN
          ...
          CALL JELIRA(JEXNUM(NOMA//'.GROUPENO',IGN),'LONMAX',NBNO,K8B)
          NNPG=0
          PREMS=.TRUE.
          DO 170 INO=1,NBNO
            IF (ZI(JWK1+ZI(JADIN+INO-1)-1).NE.0) THEN
...
C     SI AUCUN GROUPE DE NOEUD N'EST A CREER, ON SORT
      ...
      DO 200 IGN=1,NBGNNV
        ...
        CALL JELIRA(JEXNOM(NOMA//'.GROUPENO',NOMGNO),'LONMAX',NBNO,K8B)
        CALL JEECRA(JEXNOM(GRPNOE,NOMGNO),'LONMAX',ZI(JNNPG+IGN-1),K8B)
