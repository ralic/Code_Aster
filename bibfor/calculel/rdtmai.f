...
C
      ...
      CHARACTER*8 K8B,NOMMA,NOMNO,TTGRMA,TTGRNO,VALK(2)
      CHARACTER*24 NOMMAI,NOMNOE,GRPNOE,COOVAL,COOREF,COODSC
      CHARACTER*24 GRPMAI,CONNEX,TYPMAI,DIMIN,DIMOU,NOMGMA,NOMGNO
      CHARACTER*24 PTNGRN,PTNGRM
      LOGICAL LVIDE,LCAAY
      INTEGER      IARG
...
C       'TOUT_GROUP_MA'='NON'
        ...
        CALL GETVTX('RESTREINT','GROUP_MA',1,IARG,NBGMA,ZK24(JGMA),IRET)
        CALL JECREO(PTNGRM,BASE//' N K24')
        CALL JEECRA(PTNGRM,'NOMMAX',NBGMA,' ')
        CALL JECREC(GRPMAI,BASE//' V I','NO '//PTNGRM,
...
C     --------------------
      ...
      IF (NBGNO.NE.0) THEN
        ...
        CALL GETVTX('RESTREINT','GROUP_NO',1,IARG,NBGNO,ZK24(JNUGN),
     &               IRET)
      ENDIF
...
C       'TOUT_GROUP_MA'='NON' ET 'GROUP_NO' PRESENT
        ...
        NBGNNV=0
        DO 160 IGN=1,NBGNO
          CALL JENONU(JEXNOM(NOMA//'.GROUPENO',ZK24(JNUGN+IGN-1)),
     &                NUMGNO)
          IF(NUMGNO.EQ.0) THEN
          ...
          ENDIF
          CALL JEVEUO(JEXNOM(NOMA//'.GROUPENO',ZK24(JNUGN+IGN-1)),'L',
     &                JADIN)
          CALL JELIRA(JEXNOM(NOMA//'.GROUPENO',ZK24(JNUGN+IGN-1)),
     &                'LONMAX',NBNO,K8B)
          NNPG=0
          LVIDE=.TRUE.
          DO 150 INO=1,NBNO
...
C     SI AUCUN GROUPE DE NOEUD N'EST A CREER, ON SORT
      ...
      CALL JECREC(GRPNOE,BASE//' V I','NO '//PTNGRN,
     &            'DISPERSE','VARIABLE',NBGNNV)
      DO 200 IGN=1,NBGNNV
