...
C
        IFR = IUNIFI( 'RESULTAT' )
        PROFNO = PRCHNO
        CALL JEIMPO(IFR,PROFNO//'.LILI',' ',
     & 'REPERTOIRE DES NOMS DE LIGREL')
        CALL JEIMPA(IFR,PROFNO//'.LILI',
     & 'REPERTOIRE DES NOMS DE LIGREL')
C
C
        CALL JEIMPO(IFR,PROFNO//'.DEEQ',' ',
     & 'NUMERO DU NOEUD ET DU DDL')
        CALL JEIMPA(IFR,PROFNO//'.DEEQ',
     & 'NUMERO DU NOEUD ET DU DDL')
C
        CALL JELIRA(PROFNO//'.PRNO','NMAXOC',NOBJ,K1BID)
C
       DO 1 I=1,NOBJ
        CALL JEIMPO(IFR,JEXNUM(PROFNO//'.PRNO',I),' ',
     & 'PROFIL NOEUD')
        CALL JEIMPA(IFR,JEXNUM(PROFNO//'.PRNO',I),
     & 'PROFIL NOEUD')
1      CONTINUE
C
      END
