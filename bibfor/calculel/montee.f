...
C     ------------------
      ...
      INTEGER DESCGD,JCELD,CODE,DEBUGR,NCMPEL,JRSVI,JCRSVI,LG1
...
C         -- CAS : RESUELEM
          ...
          CALL JELIRA(JEXNUM(CH19//'.RESL',IGR),'LONMAX',LG1,KBID)
          IF (LPARAL) THEN
C           -- POUR L'INSTANT ON N'ACCEPTE PAS EVFINI=1
   ...
   10       CONTINUE
          ELSE
            LGGREL=NBELGR*NCMPEL
            IF (EVFINI.EQ.1) THEN
              JRSVI =ZI(IACHOI-1+3*(JPAR-1)+2)
              IF (JRSVI.NE.0) THEN
                JCRSVI=ZI(IACHOI-1+3*(JPAR-1)+3)
                LGGREL=ZI(JRSVI-1+ZI(JCRSVI-1+IGR)+NBELGR)-1
              ENDIF
            ENDIF
            CALL ASSERT(LGGREL.LE.LG1)
            CALL JACOPO(LGGREL,TYPSCA,IAD1,JRESL)
          ENDIF
