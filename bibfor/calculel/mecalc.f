     ...
     &                  CHDYNR,SUROPT,CHELEM,LIGREL,BASE,CHHYDR,CHSECH,
     &                  CHVARI,COMPOR,CHTESE,CHDESE,NOPASE,TYPESE,
     &                  CODRET)
C ----------------------------------------------------------------------
...
C PARAMETRES D'APPELS
     ...
     &              TYPCOE,LIGREL,BASE,CHHYDR,CHSECH,CHVARI,CHDESE,
     &              CHTESE,NOPASE
      REAL*8 ALPHA
      COMPLEX*16 CALPHA
...
C     ---------------------------------------------------------
        ...
        IF (OPTIO2.EQ.'VARI_ELNO_ELGA') THEN
          ...
          IF (IRET2.NE.1) CALL UTMESS('F',NOMPRO,'COMPOR SVP!')
          CALL EXISD('CHAM_ELEM_S',COMPOR,IRET1)
          IF (IRET1.NE.1) CALL CESVAR(CAREL,COMPOR,LIGREL,COMPOR)
          CALL COPISD('CHAM_ELEM_S','V',COMPOR,CHELE2)
        ...
        ELSE IF (OPTIO2.EQ.'VARI_ELNO_TUYO') THEN
          ...
          IF (IRET1.NE.1) CALL CESVAR(' ',COMPOR,LIGREL,CANBVA)
     ...
     &           OPTIO2.EQ.'EQUI_ELGA_EPME') THEN
        ...
        ELSE
          ...
          ELSE IF (OPTIO2.EQ.'SIEF_ELNO_ELGA') THEN
            LPAOUT(1) = 'PSIEFNOR'
          ELSE IF (OPTIO2.EQ.'HYDR_ELNO_ELGA') THEN
            LPAIN(1) = 'PHYDRPP'
            LCHIN(1) = CHSIG
            LPAOUT(1) = 'PHYDRNOR'
...
C ----------------------------------------------------------------------
     ...
     &              'N')
        ...
        CALL AJCHCA('PDEFAPR',CHEPS,LPAIN,LCHIN,NBIN,MAXIN,'N')
...
C         DE DEPLACEMENT : 'NOMUTILI.C00.000000'
     ...
     &      LPAIN,LCHIN,NBIN,MAXIN,'N')
        ...
        CALL AJCHCA('PSECHER',CHSECH,LPAIN,LCHIN,NBIN,MAXIN,'N')
        CALL AJCHCA('PHYDRER',CHHYDR,LPAIN,LCHIN,NBIN,MAXIN,'N')
...
C OPTION VALE_NCOU_MAXI
        IF (OPTIO2.EQ.'VALE_NCOU_MAXI') THEN
          CALL AJCHCA('PCONTRR',CHDESE,LPAIN,LCHIN,NBIN,MAXIN,'N')
          CALL AJCHCA('PDEFORR',CHDESE,LPAIN,LCHIN,NBIN,MAXIN,'N')
          CALL AJCHCA('PCONTEQ',CHDESE,LPAIN,LCHIN,NBIN,MAXIN,'N')
          CALL AJCHCA('PDEFOEQ',CHDESE,LPAIN,LCHIN,NBIN,MAXIN,'N')
          CALL AJCHCA('PNOMCMP',CHDESE,LPAIN,LCHIN,NBIN,MAXIN,'N')
        END IF
...
C DERIVEE LAGRANGIENNE
        ...
        ELSE IF (TYPESE.EQ.3) THEN
          IF (OPTIO2(1:4).EQ.'EPSI') GOTO 18
          IF (OPTIO2(11:14).EQ.'ELGA') GOTO 18
...
C DETERMINATION DU CHAMP MATERIAU DERIVE NON CODE MATSEN
          ...
          IF (IRET.NE.0) CALL UTMESS('F',NOMPRO,
     &                               'IMPOSSIBLE LIRE '//MATERI)
