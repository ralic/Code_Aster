...
C     BOUCLE SUR LES OCCURRENCES DE CHAM_UTIL
      DO 10 IOC=1, NUTI
        CALL GETVTX(MCFACT, 'NOM_CHAM', IOC, 1, 1, CHIN, IBID)
        CALL GETVIS(MCFACT, 'NUME_CHAM_RESU', IOC, 1, 1, NCHOUT, IBID)
        CALL ASSERT(NCHOUT.GE.1 .AND. NCHOUT.LE.20)
C       CRITERE OU FORMULE ?
        CRIT = ' '
        CALL GETVID(MCFACT, 'FORMULE', IOC, 1, 0, K8B, NF)
        IF (NF.EQ.0) THEN
          CALL GETVTX(MCFACT, 'CRITERE', IOC, 1, 1, CRIT, NC)
        ELSE
          NF = -NF
          CALL WKVECT(LFORM, 'V V K8', NF, JFORM)
          CALL GETVID(MCFACT, 'FORMULE', IOC, 1, NF, ZK8(JFORM), IBID)
        ENDIF
