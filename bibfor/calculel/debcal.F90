...
! ----------------------------------------------------------------------
    ...
    integer :: ier, jpar, igd, nec, ncmpmx, iii, num
...
!     -------------------------------------------------------------
    ...
    call dismoi('F', 'NOM_MAILLA', chin, 'CHAMP', ibid,&
                ma2, ierd)
    if (ma2 .ne. ma) then
        valk(1)=chin
        valk(2)=ligrel
...
!
    call dismoi('F', 'TYPE_CHAMP', chin, 'CHAMP', ibid,&
                tych, ier)
!
...
!           (ET ON MODIFIE SON NOM DANS LCHIN)
    if ((tych(1:2).eq.'EL') .or. (tych.eq.'RESL')) then
        call dismoi('F', 'NOM_LIGREL', chin, 'CHAMP', ibid,&
                    ligre2, ier)
