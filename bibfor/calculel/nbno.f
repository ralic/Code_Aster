...
C
      ...
      ELSE IF (ZI(IADM).EQ.2) THEN
         ISCHN = 1
      ELSE
         CALL UTMESS('F','NBNO','CETTE FONCTION NE MARCHE '//
     +            'QUE POUR DES MODES LOCAUX DE TYPE CHNO, VECT, OU MAT'
     +               )
      END IF
C
      IF (ISCHN.EQ.1) THEN
...
C
      IF (ISMAT.EQ.1) THEN
         ...
         END IF
         IF (N1.NE.N2) THEN
            CALL UTMESS('F','NBNO','LE MODE LOCAL'//
     +                  ' EST DE TYPE MATRICE NON_CARREE')
         END IF
      END IF
      NBNO = N1
 9999 CONTINUE
