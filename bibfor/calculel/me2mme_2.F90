...
!
!   variables de commandes TEMP, HYDR, SECH pouvant donner lieu à un chargement
!   de type dilatation
    call nmvcd2('TEMP', mate, ltemp)
    if (ltemp) call utmess('A', 'CALCULEL3_19', sk='TEMP')
    call nmvcd2('HYDR', mate, ltemp)
    if (ltemp) call utmess('A', 'CALCULEL3_19', sk='HYDR')
    call nmvcd2('SECH', mate, ltemp)
    if (ltemp) call utmess('A', 'CALCULEL3_19', sk='SECH')
!
 60 continue
!
    call jedema()
