      SUBROUTINE PJ6DCO(MOCLE,MO1,MO2,NBMA1,LIMA1,NBNO2,LINO2,
...
C       ET CEUX DE M2 (DANS LE CAS DU MAILLAGE M1 1.5D : LIGNE EN 2/3D)
C
...
43    CONTINUE
      IF (KK.EQ.0) CALL UTMESS('F','PJ6DCO',
...
C           V(1+3(I-1)+3) : NUMERO DE LA MAILLE MERE DU IEME SEG2
     ...
     &        .OR.(ITYPM.EQ.NUTM(3)) ) THEN
          ICO=ICO+1
        ELSE
          CALL UTMESS('F','PJ6DCO','STOP 3')
      IF (ICO.EQ.0)
     &  CALL UTMESS('F','PJ6DCO','PAS DE MAILLES A PROJETER.')
        END IF
51    CONTINUE
...
C     ------------------------------------------------
     ...
     &   ZI(IABTNB), ZI(IABTLC),ZI(IABTCO),IFM,NIV,LDMAX,DISTMA)
        ...
        IF (NBTROU.EQ.0) THEN
          ...
          CALL UTMESS('F','PJ6DCO',NONO2//' PAS TROUVE.')
        END IF
