...
C MODIF CALCULEL  DATE 25/11/98   AUTEUR CIBHHGB G.BERTRAND 
C ======================================================================
...
C
      ...
      IF (TYSCA1.NE.'R') CALL UTMESS('F','CHNUCN','TYPE SCALAIRE DU '
     +   //'CHAM_NO : '//CN1//' NON REEL.')
      IF (TYSCA2.NE.'R') CALL UTMESS('F','CHNUCN','TYPE SCALAIRE DU '
     +   //'NUME_DDL : '//NU2//' NON REEL.')
C
C
      CALL DISMOI('F','NB_EQUA',CN1,'CHAM_NO',NVAL1,REPK,IERD)
      CALL DISMOI('F','NOM_NUME_DDL',CN1,'CHAM_NO',IBID,NU1,IERD)
C
C ------------------------------- REFE --------------------------------
C
...
C
      CALL WKVECT(CN2//'.REFE',BASE2//' V K24',2,I1)
...
C
      ...
      IF (MA.NE.REPK) CALL UTMESS('F','CHNUCN','STOP1')
...
C     ------------------------------------
      ...
      IF (NCORR.EQ.0) THEN
        IF (GD1.NE.GD2) CALL UTMESS('F','CHNUCN','SI NCORR=0, LES'
     +   //' GRANDEURS DOIVENT ETRE IDENTIQUES :'//GD1//','//GD2)
        DO 5,I2=1,NCMMX2
          ZI(ICORR2-1+I2)=I2
 5      CONTINUE
      ELSE
        IF (NCORR.NE.2*(NCORR/2)) CALL UTMESS('F','CHNUCN',
