...
C MODIF MODELISA  DATE 28/06/2005   AUTEUR F1BHHAJ J.ANGLES 
C ======================================================================
...
C    DANS LE CAS OU LE MOT CLE EST MANSON_COFFIN
         ...
         CALL JEEXIN (NOMMAT//'.'//NOMPHE//'.VALR',IRET)
         IF ( IRET .EQ. 0 ) THEN
             CALL UTMESS ('F','LIMEND', 'PB LECTURE COURBE DE '//
     &                                  'MANSON_COFFIN')
         ENDIF
         CALL JEVEUO (NOMMAT//'.'//NOMPHE//'.VALR', 'L', IVALR)
         CALL JELIRA (NOMMAT//'.'//NOMPHE//'.VALR','LONUTI',NBR,K8BID)
...
C        MANSON_COFFIN
               CALL JEVEUO(NOMFON//'           .PROL','L',IPROL)
C        PROLONGEMENT A GAUCHE DE LA COURBE DE MANSON_COFFIN EXCLU
               IF (ZK16(IPROL-1+5)(1:1).EQ.'E') THEN
                  VALLIM=ZR(IVALF)
                  IF (SALT.LT.VALLIM) THEN
                     LIMIT=.TRUE.
            ...
            ENDIF
  30     CONTINUE
         CALL UTMESS ('F','LIMEND','MOT CLE MANSON_COFFIN NON TROUVE')
  40     CONTINUE
