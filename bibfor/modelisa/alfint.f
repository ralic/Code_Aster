...
C MODIF MODELISA  DATE 12/11/2007   AUTEUR PELLET J.PELLET 
C ======================================================================
...
C -----  VARIABLES LOCALES
      ...
      INTEGER NBEC,K,IER,EC1,KK,IGD,NGDMAX,JDESC,JVALE,ITREF,JPROL
...
C        -- SI LA FONCTION EST UNE CONSTANTE, ON NE FAIT RIEN :
         IF (ZK16(JPROL-1+1).EQ.'CONSTANT') THEN
           GOTO 9999
C        -- SI TREF ET TDEF SONT PROCHES (1 DEGRE), ON NE FAIT RIEN :
         ELSEIF (ABS(TREF-TDEF).LT.1.D0) THEN
           GOTO 9999
C        -- SINON ON ARRETE TOUT :
         ELSE
