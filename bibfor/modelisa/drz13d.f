...
C
      CALL UTMESS('F','DRZ13D',
     +              'INCOHERENCE CAR AUCUN NOEUD N''A DE DDL DE'//
     +              'ROTATION DRX ET DRY ET DRZ'//
     +             ' ET LA ROUTINE TRAITE LE CAS 3D OU IL Y A'//
     +             ' AU-MOINS UN NOEUD AYANT CES 3 DDLS')
C
  30  CONTINUE
C
      DO 40 J=1, LONLIS
     ...
     +       -ZR(JCOOR-1+3*(INO1-1)+2)
         Z =  ZR(JCOOR-1+3*(IN-1)+3)
     +       -ZR(JCOOR-1+3*(INO1-1)+3)
C
C ---   PREMIERE RELATION
