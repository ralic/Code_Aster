...
C ---     STOCKAGE DE L'UNITE LOGIQUE
          CALL  GETVIS(NOMMCF,'UNITE_CD',1,IARG,1,UNIT1,IBID1)
          CALL  GETVIS(NOMMCF,'UNITE_CK',1,IARG,1,UNIT2,IBID2)
...
C ---     RECHERCHE DE LA DERNIERE OCCURENCE DES MOTS-CLES FACULTATIFS
            ...
            CALL GETVR8(NOMMCF,'RHO_TUBE'      ,IOCC,IARG,0,RBID,IRHO)
            IF(IRHO.NE.0)   JRHO=IOCC
            CALL GETVTX(NOMMCF,'TYPE_PAS'      ,IOCC,IARG,0,K8BID,ITPAS)
...
C
 ...
 30          CONTINUE
             CALL GETVTX(NOMMCF,'TYPE_PAS',JTPAS,IARG,1,TPAS,IBID)
...
C
C ---------- PAS REDUIT
             CALL GETVR8(NOMMCF,'PAS',JPAS,IARG,1,PAS ,IBID)
             IF (JCM.EQ.0) THEN
...
C ---------------- RESEAU A PAS CARRE
     ...
     &                      ZR(LFSVR),IBID)
             ...
             ZR(LFSVR+1) = PAS
C
             CALL GETVR8(NOMMCF,'RHO_TUBE',JRHO,IARG,1,ZR(LFSVR+2),IRHO)
C
...
C
            ...
            CALL GETVTX(NOMMCF,'GRAPPE_2' ,1,IARG,1,ZK8(LFSVK)  ,IBID)
            CALL GETVTX(NOMMCF,'NOEUD'    ,1,IARG,1,ZK8(LFSVK+1),IBID)
            CALL GETVID(NOMMCF,'CARA_ELEM',1,IARG,1,ZK8(LFSVK+2),IBID)
            CALL GETVID(NOMMCF,'MODELE'   ,1,IARG,1,ZK8(LFSVK+3),IBID)
C
C ----      STOCKAGE DE L'UNITE LOGIQUE
            CALL  GETVIS(NOMMCF,'UNITE_CA',1,IARG,1,UNIT1,IBID1)
            CALL  GETVIS(NOMMCF,'UNITE_KA',1,IARG,1,UNIT2,IBID2)
...
C
            ...
            CALL GETVR8(NOMMCF,'COEF_MASS_AJOU',1,IARG,0,RBID,ICM)
            IF (ICM.NE.0) THEN
     ...
     &                    ZR(LFSVR),IBID)
            ...
            ENDIF
            CALL GETVR8(NOMMCF,'RHO_FLUI',1,IARG,1,ZR(LFSVR+1),IBID)
C
...
C --------    FACTEUR POUR LESQUELLES ON IRA CHERCHER LES INFORMATIONS
          IEQUIV = 1
          IF (NBOCC.EQ.1) THEN
            CALL GETVID(NOMMCF,'CARA_ELEM',1,IARG,0,K8BID,ICAEL)
...
C
          IF (IEQUIV.EQ.0) THEN
C
            CALL GETVTX(NOMMCF,'GROUP_MA',1,IARG,0,K8BID,NBGRMA)
            NBGRMA = ABS(NBGRMA)
            CALL GETVR8(NOMMCF,'PESANTEUR',1,IARG,0,RBID,IPESA)
            IF (IPESA.NE.0) OCPESA = 1
            CALL GETVTX(NOMMCF,'CARA_PAROI',1,IARG,0,K8BID,NBCARA)
            NBCARA = ABS(NBCARA)
            IF (NBCARA.EQ.3) THEN
               IENCE = 1
               NBANGL = 0
            ELSE
               ...
               NBANGL = 1
            ENDIF
            CALL GETVR8(NOMMCF,'COOR_GRILLE',1,IARG,0,RBID,NBGTOT)
            IF (NBGTOT.NE.0) THEN
                OCGRIL = 1
                NBGTOT = ABS(NBGTOT)
                CALL GETVR8(NOMMCF,'LONG_TYPG',1,IARG,0,RBID,NTYPG)
                NTYPG = ABS(NTYPG)
            ELSE
                NTYPG = 0
                NBGTOT = 0
...
C
     ...
     &                     K8BID,IPVIS)
               ...
               CALL GETVR8(NOMMCF,'PESANTEUR',IOCC,IARG,0,RBID,IPESA)
               IF (IPESA.NE.0) OCPESA = IOCC
               CALL GETVR8(NOMMCF,'RUGO_TUBE',IOCC,IARG,0,RBID,IRUGO)
               IF (IRUGO.NE.0) OCRUGO = IOCC
               CALL GETVTX(NOMMCF,'CARA_PAROI',IOCC,IARG,0,K8BID,ICARA)
               IF (ICARA.NE.0) OCCAPA = IOCC
               CALL GETVR8(NOMMCF,'COOR_TUBE',IOCC,IARG,0,RBID,ICOOR)
               ICOOR = ABS(ICOOR)
               ZI(LNBCR+IOCC-1) = ICOOR
               NBCOOR = NBCOOR + ICOOR
               CALL GETVR8(NOMMCF,'COOR_GRILLE',IOCC,IARG,0,RBID,NBGTOT)
               IF (NBGTOT.NE.0) OCGRIL = IOCC
  60        CONTINUE
C
            CALL GETVTX(NOMMCF,'CARA_PAROI',OCCAPA,IARG,0,K8BID,NBCARA)
            NBCARA = ABS(NBCARA)
            IF (NBCARA.EQ.3) THEN
               IENCE = 1
               NBANGL = 0
...
C --------3.2.1.OBJET .FSVI
          ZI(LFSVI) = IEQUIV
C
          CALL GETVR8(NOMMCF,'VECT_X',OCVECT,IARG,3,VECT(1),IBID)
...
C --------3.2.3.OBJET .FSVR
          IF (OCPESA.NE.0) THEN
            CALL GETVR8(NOMMCF,'PESANTEUR',OCPESA,IARG,4,ZR(LFSVR),IBID)
          ELSE
            ZR(LFSVR)   =  9.81D0
            ZR(LFSVR+1) =  0.D0
            ZR(LFSVR+2) =  0.D0
            ZR(LFSVR+3) = -1.D0
          ENDIF
          CALL GETVR8(NOMMCF,'RUGO_TUBE',OCRUGO,IARG,1,ZR(LFSVR+4),IBID)
C
...
C
          ZI(LFSIC+1) = 1
          CALL WKVECT(FSVI,'G V I',2,LFSVI)
          CALL GETVTX(NOMMCF,'MASS_AJOU',1,IARG,1,OUINON,IBID)
          IF (OUINON.EQ.'OUI') THEN
            ZI(LFSVI) = 1
          ELSE
            ZI(LFSVI) = 0
...
C
          ...
          CALL GETVID(NOMMCF,'CARA_ELEM',1,IARG,1,ZK8(LFSVK)  ,IBID)
          CALL GETVID(NOMMCF,'MATER_INT',1,IARG,1,ZK8(LFSVK+1),IBID)
          CALL GETVID(NOMMCF,'MATER_EXT',1,IARG,1,ZK8(LFSVK+2),IBID)
C
          CALL WKVECT(FSVR,'G V R',7,LFSVR)
          CALL GETVR8(NOMMCF,'RHO_FLUI' ,1,IARG,1,ZR(LFSVR)  ,IBID)
          CALL GETVR8(NOMMCF,'VISC_CINE',1,IARG,1,ZR(LFSVR+1),IBID)
          CALL GETVR8(NOMMCF,'RUGOSITE' ,1,IARG,1,ZR(LFSVR+2),IBID)
          CALL GETVR8(NOMMCF,'PDC_MOY_1',1,IARG,1,ZR(LFSVR+3),IBID)
          CALL GETVR8(NOMMCF,'PDC_DYN_1',1,IARG,1,ZR(LFSVR+4),IBID)
