...
C ---     STOCKAGE DE L'UNITE LOGIQUE
          ...
          CALL JEEXIN ('&&OP0143.UNIT_FAISCEAU',IRET)
          IF (IRET .EQ. 0) THEN
             CALL WKVECT('&&OP0143.UNIT_FAISCEAU','G V I',2,IUNIT)
          ELSE
             CALL JEVEUO('&&OP0143.UNIT_FAISCEAU','L',IUNIT)
          ENDIF
          ZI(IUNIT-1+1) = UNIT1
          ZI(IUNIT-1+2) = UNIT2
C
...
C
            ...
            CALL GETVID(NOMMCF,'NOEUD'    ,1,1,1,ZK8(LFSVK+1),IBID)
...
C ----      STOCKAGE DE L'UNITE LOGIQUE
            ...
            CALL JEEXIN ('&&OP0143.UNIT_GRAPPES',IRET)
            IF (IRET .EQ. 0) THEN
              CALL WKVECT('&&OP0143.UNIT_GRAPPES','G V I',2,IUNIT)
            ELSE
              CALL JEVEUO('&&OP0143.UNIT_GRAPPES','L',IUNIT)
            ENDIF
C
            ZI(IUNIT-1+1) = UNIT1
            ZI(IUNIT-1+2) = UNIT2
...
C
          IF (IEQUIV.EQ.0) THEN
C
            CALL GETVID(NOMMCF,'GROUP_MA',1,1,0,K8BID,NBGRMA)
            NBGRMA = ABS(NBGRMA)
...
C
     ...
     &                     ZR(LFSVR+IDECVR+IOCC-1),IBID)
               CALL GETVID(NOMMCF,'GROUP_MA',IOCC,1,1,
...
C
            CALL GETVID(NOMMCF,'CARA_ELEM',1,1,1,ZK8(LFSVK+2),IBID)
            IF (NBGRMA.NE.0) THEN
              CALL GETVID(NOMMCF,'GROUP_MA',1,1,NBGRMA,ZK8(LFSGM),IBID)
            ELSE
...
C
          ...
          CALL GETVID(NOMMCF,'GROUP_MA_INT',1,1,1,ZK8(LFSGM)  ,IBID)
          CALL GETVID(NOMMCF,'GROUP_MA_EXT',1,1,1,ZK8(LFSGM+1),IBID)
C
        ENDIF
C=======================================================================
C ----- IMPRESSION -----
