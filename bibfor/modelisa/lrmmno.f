...
C          LEUR NUMERO
C
      CALL WKVECT('&&'//NOMPRO//'.NOMNOE','V V K8',NBNOEU,JNOMNO)
      CALL EFNOML ( FID, NOMAMD, ZK8(JNOMNO), NBNOEU,
     >              EDNOEU, TYPNOE, CODRET)
C
      IF ( CODRET.NE.0 ) THEN
        DO 11 , IAUX = 1 , NBNOEU
          CALL CODENT (IAUX,'G',SAUX07)
          ZK8(JNOMNO+IAUX-1) = 'N'//SAUX07
   11   CONTINUE
        CODRET = 0 
      ENDIF
C
...
C
      ...
      DO 12 , IAUX = 1 , NBNOEU
        CALL JECROC (JEXNOM(NOMNOE,ZK8(JNOMNO+IAUX-1)))
   12 CONTINUE
C
C====
...
C
     ...
     >              TYPREP, NOMCOO, UNICOO, CODRET )
      IF ( CODRET.NE.0 ) THEN
        ...
        CALL UTMESS ('F',NOMPRO,'MED: ERREUR EFCOOL NUMERO '//SAUX08)
      ENDIF
C
...
C -   OBJET REFE COORDONNEES DES NOEUDS
      CALL WKVECT (COOREF,'G V K24',2,JCOORF)
      ZK24(JCOORF) = NOMU
C
C====
C 3. LA FIN
