...
C ------- 1.1.1 S1 ET S2 DE PAR ET D'AUTRE DU PLAN DEFINI PAR F1 ?
          ...
          R1 = R8DOT(3,FQ(1,F1),1,SC(1,S1),1) + FQ(4,F1)
          R2 = R8DOT(3,FQ(1,F1),1,SC(1,S2),1) + FQ(4,F1)
...
C ------- 1.1.3 INTERSECTION APPARTIENT A F1 ?
          ...
          IF (R8DOT(3,FQ(1,F1),1,V,1).LT.0.D0) GOTO 20
...
C ------- 1.2.1 S1 ET S2 DE PAR ET D'AUTRE DU PLAN DEFINI PAR F2 ?
          ...
          R1 = R8DOT(3,FQ(1,F2),1,SC(1,S1),1) + FQ(4,F2)
          R2 = R8DOT(3,FQ(1,F2),1,SC(1,S2),1) + FQ(4,F2)
          IF (((R1.LT.0.D0).EQV.(R2.LT.0.D0)).OR.(R1.EQ.0.D0)
...
C ------- 1.2.3 INTERSECTION APPARTIENT A F2 ?
          ...
          IF (R8DOT(3,FQ(1,F2),1,V,1).LT.0.D0) GOTO 40
...
C --- 4. COMPOSANTES CONNEXES ET TRIANGULATION DES FACES
 ...
 220  CONTINUE
      ...
      IF (NC.GT.NCMAX) CALL UTMESS('A','PLINT3','NOMBRE DE COMPOSANTES '
     & //'CONNEXES MAXIMAL PREVU INSUFFISANT')
