...
C MODIF MODELISA  DATE 04/10/2005   AUTEUR REZETTE C.REZETTE 
C ======================================================================
...
C     VERIFICATIONS
      IF(TYPCHS.EQ.'NOEU')THEN
        ...
        IF(N1.EQ.0)
     &       CALL UTMESS('F','U195TB','VOUS DEVEZ FOURNIR LE '//
     &       'MOT CLE MAILLAGE POUR UN CHAMP AUX NOEUDS')
        OPTION=' '
        MO=' '
     ...
     &       CALL UTMESS('F','U195TB','VOUS DEVEZ FOURNIR LES '//
     &       'MOTS CLES MODELE ET OPTION POUR UN CHAMP ELEMENTAIRE')
        CALL JEVEUO(MO//'.MODELE    .NOMA','L',JNOMA)
        MA=ZK8(JNOMA)
      ENDIF

C     CREATION DU CHAMP SIMPLE
      CALL TABCHS(TABIN,TYPCHS,BASE,NOMGD,MA,MO,OPTION,CHS)
