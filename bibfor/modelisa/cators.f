...
C       ---------------
        ...
        IF (IRET1.NE.0) THEN
C
C ---     SI LA CARTE EXISTE DEJA, ON L'AGRANDIT DU NOMBRE TOTAL DES
C ---     MAILLES CONSTITUANT TOUS LES CONTOURS INTERIEURS  :
C         ------------------------------------------------
          CALL JELIRA(CARFLU//'.VALV','LONMAX',LONG,K1BID)
          LONG = LONG + NBMA + 1
          CALL AGCART(LONG, CARFLU)
        ELSE
C
C ---     SI LA CARTE N'EXISTE PAS, ON LA CREE :
C         ------------------------------------
          LONG = NBMA + 1
          CALL ALCART('G', CARFLU, NOMA, 'FLUN_R', LONG , LONG)
        ENDIF
C
