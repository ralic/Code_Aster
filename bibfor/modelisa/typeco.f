...
C
      ...
      INTEGER      POSMAM,POSMAE,NUMMAE,ISUMAE,INDMAE
      INTEGER      INO,IMA,POSNO,POSMA,CODRET
      INTEGER      TYPBAR,TYPRAC,NDEXFR,TYPINT,NNINT     
      CHARACTER*24 DEFICO
C
C ----------------------------------------------------------------------
C
...
C
      ...
      DO 150 IZONE = 1,NZOCO
        CALL CFZONE(DEFICO,IZONE ,'MAIT',ISURFM) 
        CALL CFNBSF(DEFICO,ISURFM,'MAIL',NBMAM ,JDECMM)
        CALL CFZONE(DEFICO,IZONE ,'ESCL',ISURFE) 
        CALL CFNBSF(DEFICO,ISURFE,'MAIL',NBMAE ,JDECME)     
C      
        DO 160 IMAM = 1,NBMAM
          POSMAM = JDECMM + IMAM
...
C 
     ...
     &                NUMMAE,CODRET)
          ...
          IF (IFORM.EQ.2) THEN     
            ...
            CALL MMFOND(NOMA  ,DEFICO,IZONE ,POSMAE,TYPBAR)  
            CALL MMSSFR(NOMA  ,DEFICO,POSMAE,NDEXFR) 
            CALL MMRACC(NOMA  ,DEFICO,IZONE ,POSMAE,TYPRAC)
          ELSE
            NNINT  = 0
            TYPBAR = 0
            NDEXFR = 0
          ...
          ZI(JMAESC+ZMAES*(INDMAE-1)+3-1) = NNINT
          ZI(JMAESC+ZMAES*(INDMAE-1)+4-1) = TYPBAR
          ZI(JMAESC+ZMAES*(INDMAE-1)+5-1) = NDEXFR
          ZI(JMAESC+ZMAES*(INDMAE-1)+6-1) = TYPRAC   
 20     CONTINUE
 30   CONTINUE 
C
C --- VERIFS
