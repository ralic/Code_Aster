...
C                MAILLES A CREER
C       NOMNGF : BASE DU NOM DE L'OBJET QUI CONTIENT LES INFOS SUR LES
C                GROUPES PAR FAMILLE
...
C                        MILLIERS DE NOEUDS OU DE MAILLES ET QUELQUES
C                        CENTAINES DE GROUPES
...
C
      DO 30 , IFAM = 1 , NBFAM
C
        NOMJ = JEXNUM(NOMNGF//'.FANOMG',IFAM)
...
C
          NOMJ = JEXNUM(NOMNGF//'.FANUMG',IFAM)
          CALL JEVEUO(NOMJ,'L',JFNUMG)
C
          NOMJ = JEXNUM(NOMNGF//'.FANUM',IFAM)
...
C
     ...
     >'IMPOSSIBLE DE RETROUVER L''ADRESSE ASSOCIEE AU GROUPE '//NOMGRP)
C
 3221        CONTINUE
C
C 3.2.3. ==> TRANSFERT
