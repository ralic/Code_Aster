...
C
      ...
      INTEGER      IRGNO,IRGTO,ISYM,ITBMP,ITBNO,IUNIFI,IV
      ...
      REAL*8        ETA, VALE(NBVAL),R8BID
      CHARACTER*1  KMA(3)
      CHARACTER*8  NOMNOE,NOMMAI,K8BID,NOMU,CAR(NBCAR),LAMASS
      CHARACTER*16 REP,REPDIS(NRD),CONCEP,CMD
      CHARACTER*19 CART(3),CARTDI
      CHARACTER*24 TMPND(3),TMPVD(3),K24BID,NOGP
...
C
            IF ( TRANSL ) THEN
               LAMASS = 'K_T_D_N'
               CALL MASREP(NOMA,IOC,VALE,LVALE,NG,ZK24(JDLS),NBNO,
