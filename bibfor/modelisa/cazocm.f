...
C
      ...
      INTEGER      JJFO1,JJFO2     
      CHARACTER*16 APPA,NORM,APTY,TYPNOR,TYPM
      REAL*8       NOOR,R8PREM
      REAL*8       DIR(3),TOLJ,TOLA
      CHARACTER*8  JEUF1,JEUF2
      LOGICAL      LLISS,CFDISL
C
C ----------------------------------------------------------------------
C
      CALL JEMARQ
...
C 
      CALL GETVTX(MOTFAC,'DIST_POUTRE',IZONE,1,1,TYPM  ,NOC   )
      IF (TYPM(1:3) .EQ. 'OUI') THEN
        ZI(JMETH+ZMETH*(IZONE-1)+4-1) = 1
      ENDIF
      CALL GETVTX(MOTFAC,'DIST_COQUE' ,IZONE,1,1,TYPM  ,NOC   )
      IF (TYPM(1:3) .EQ. 'OUI') THEN
...
C     
      ...
      IF (APTY(1:6) .EQ. 'PROCHE') THEN
        ZI(JMETH+ZMETH*(IZONE-1)+9-1) = 0
      ELSEIF (APTY(1:4) .EQ. 'FIXE') THEN
        ZI(JMETH+ZMETH*(IZONE-1)+9-1) = 1
...
C
      IF (NORM(1:4) .EQ. 'MAIT') THEN
        IF (NORM(5:9).EQ.'_ESCL') THEN
          ZI(JMETH+ZMETH*(IZONE-1)+8-1) = 1
        ELSE 
          ZI(JMETH+ZMETH*(IZONE-1)+8-1) = 0
        ENDIF  
      ELSEIF (NORM(1:4) .EQ. 'ESCL') THEN
        ZI(JMETH+ZMETH*(IZONE-1)+8-1) = 2  
      ELSE
        CALL ASSERT(.FALSE.)
      ENDIF
C 
...
C      
      ...
      ELSEIF  (TYPNOR.EQ.'FIXE') THEN 
        IF (ZI(JMETH+ZMETH*(IZONE-1)+8-1).NE.0) THEN
...
C      
      IF (TYPNOR.EQ.'AUTO') THEN
        ZI(JMETH+ZMETH*(IZONE-1)+10-1) = 0
      ELSEIF  (TYPNOR.EQ.'FIXE') THEN 
        IF (ZI(JMETH+ZMETH*(IZONE-1)+8-1).NE.2) THEN
          CALL U2MESS('F','CONTACT3_52')
        ENDIF  
        IF (LLISS) THEN
          CALL U2MESS('F','CONTACT3_54')
        ENDIF            
        ZI(JMETH+ZMETH*(IZONE-1)+10-1) = 1
