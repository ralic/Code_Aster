...
C        ARRET = '  ' : ON REMPLIT CODRET ET ON SORT SANS MESSAGE.
...
C
      ...
      IF (K8MAT.NE.' ') THEN
        DO 5 KMAT =1,NBMAT
          INOM=ZI(JMAT+KMAT)
          NOMI=ZK8(INOM)
          IF ( NOMI .EQ. K8MAT ) THEN
            IMAT = JMAT+ZI(JMAT+NBMAT+KMAT)
            GOTO 9
          ENDIF
 5      CONTINUE
        CALL UTMESS ('F','RCVALA_01','MATERIAU :'//K8MAT//' NON TROUVE')
      ELSE
...
C
 999  CONTINUE
C
      CALL RCVALS( ARRET, CODRET, NBRES, NOMRES )
C
      END
