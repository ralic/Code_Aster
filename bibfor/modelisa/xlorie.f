      SUBROUTINE XLORIE(FISS,NDIM,GEOFIS,NOEUD,VECT1,VECT2)
C
...
C
      IMPLICIT NONE
      INTEGER     NDIM
      REAL*8      NOEUD(3),VECT1(3),VECT2(3)
      CHARACTER*8 FISS
      CHARACTER*16 GEOFIS
C
C ----------------------------------------------------------------------
...
C     ORIENTATION AUTOMATIQUE
      IF (IOCC .EQ. 0) THEN
        ...
        IF (NDIM .EQ. 3) THEN
          CALL GETVTX('DEFI_FISS','FORM_FISS',1,IARG,1,K16BID,IOCC2)
          IF ( IOCC2 .EQ. 0 ) THEN
            CALL U2MESS('F','XFEM_20')
          ELSE
C           CONSTRUCTION AUTO DES DONNEES ORIENTATION DU FOND
            CALL XLORIA(ZR(JCARAF-1+3),GEOFIS,NOEUD,VECT1,VECT2)
          ENDIF
        ENDIF
...
C     ORIENTATION UTILISATEUR
      ELSEIF (IOCC .EQ. 1) THEN
        ...
        CALL GETVR8('ORIE_FOND','PFON_INI' ,1,IARG,3,PFI,IBID)
        CALL GETVR8('ORIE_FOND','VECT_ORIE',1,IARG,3,VOR,IBID)
        CALL GETVR8('ORIE_FOND','POINT_ORIG',1,IARG,3,ORI,IBID)
