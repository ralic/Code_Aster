...
C --- NOM DU MAILLAGE :
C     ---------------
      CALL JEVEUO(LIGRET//'.NOMA','L',IDNOMA)
C
...
C       -------------------------------
        ...
        ENDIF
C
C ---   CREATION DE L'OBJET LIGREL.NOMA :
C       -------------------------------
        CALL JEEXIN(LIGREL//'.NOMA',IRET)
        IF (IRET.EQ.0) THEN
          CALL WKVECT(LIGREL//'.NOMA',BASE//' V K8',1,JDNOMA)
...
C     ---------------------------------
      CALL ADALIG(LIGREL)
C
C --- CREATION ET AFFECTATION DE L'OBJET LIGREL.REPE QUI EST TEL QUE
C ---  LIGREL.REPE(2*(I-1)+1) = NUMERO DU GREL CONTENANT LA MAILLE I
C ---  LIGREL.REPE(2*(I-1)+2) = NUMERO LOCAL DE LA MAILLE DANS LE GREL:
C      ---------------------------------------------------------------
      CALL CORMGI(BASE,LIGREL)
C
C --- CREATION DE L'OBJET LIGREL.PRNM QUI EST LE NOM (K24) DU
C --- VECTEUR DES ENTIERS CODES DECRIVANT LA NATURE DES DDLS DES
C --- NOEUDS PHYSIQUES
C --- ET CREATION DE L'OBJET LIGREL.PRNS QUI EST LE NOM (K24) DU
C --- VECTEUR DES DDLS PORTES PAR LES NOEUDS TARDIFS DU LIGREL :
C     --------------------------------------------------------
      CALL CREPRN(LIGREL,MOLOC,BASE,LIGREL//'.PRNM',LIGREL//'.PRNS')
C
      CALL JEDEMA()
C
      END
