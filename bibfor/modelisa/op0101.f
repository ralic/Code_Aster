...
C --------- FIN  DECLARATIONS  NORMALISEES  JEVEUX ---------------------
      PARAMETER   ( NBMEC = 10 , NBTHE = 3 , NBACO = 1 )
      CHARACTER*8 CHCINE,MO,NOMMEC(NBMEC),NOMTHE(NBTHE),NOMACO(NBACO)
      CHARACTER*16 TYPE,OPER
      CHARACTER*24 CNOMO,CTYPE
      INTEGER IOCMI,IOCTI,IOCAI
      DATA CTYPE/'        .TYPE           '/
      DATA CNOMO/'        .CI  .MODEL.NOMO'/
C --- ATTENTION LA LISTE DES DDLS IMPOSABLES DOIT ETRE TELLE QUE
C     NUMERO D'UN DDL DANS LA LISTE = NUMERO D'1 DDL DANS LA GRANDEUR
      DATA NOMMEC/'DX','DY','DZ','DRX','DRY','DRZ','GRX','PRES',
     +            'TEMP','PHI'/
      DATA NOMTHE/'TEMP','TEMP_INF','TEMP_SUP'/
      DATA NOMACO/'PRES'/
C --- DEBUT
C --- INITIALISATION DU NOMBRE D'ERREURS
C
      CALL JEMARQ()
C
      CALL INFMAJ
...
C
      ...
      IF (IOCMI.NE.0) THEN
         ZK8(ITYPE) = 'CIME_RE'
         CALL CHARCI(CHCINE,'MECA_IMPO',IOCMI,NBMEC,NOMMEC,'R')
      ENDIF
C
      CALL GETFAC('THER_IMPO',IOCTI)
      IF (IOCTI.NE.0) THEN
         ZK8(ITYPE) = 'CITH_RE'
         CALL CHARCI(CHCINE,'THER_IMPO',IOCTI,NBTHE,NOMTHE,'R')
      ENDIF
C
      CALL GETFAC('ACOU_IMPO',IOCAI)
      IF (IOCAI.NE.0) THEN
         ZK8(ITYPE) = 'CIAC_CX'
         CALL CHARCI(CHCINE,'ACOU_IMPO',IOCAI,NBACO,NOMACO,'C')
      ENDIF
C
      CALL JEDEMA()
      END
