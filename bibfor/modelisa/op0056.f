...
C     --- VERIFICATION DES MATERIAUX ---
      ...
      THER = .FALSE.
      DO 20 ICOU = 1,NBCOU
        CALL GETVID('COUCHE','MATER',ICOU,IARG,1,MATER,N)
        ...
        DO 10 I = 1,NBAD
          ...
          ELSE
        VALK (1) = MATER
            CALL U2MESG('F', 'MODELISA8_71',1,VALK,0,0,0,0.D0)
          END IF
   10   CONTINUE
   20 CONTINUE
      IF (THER .AND. ELAS) THEN
        VALK (1) = 'MECANIQUE'
        VALK (2) = 'THERMIQUE'
        CALL U2MESG('F', 'MODELISA8_72',2,VALK,0,0,0,0.D0)
      END IF
      ...
      IF (IMPR.NE.0) THEN
         ...
         IFR    = 0
         FICHIE = ' '
         CALL GETVIS ( 'IMPRESSION', 'UNITE'  , 1,IARG,1, IFR   , N1 )
...
C     ------ CARACTERISTIQUES MECANIQUES ------
   ...
   30   CONTINUE
        DO 50 ICOU = 1,NBCOU
          CALL GETVR8('COUCHE','EPAIS',ICOU,IARG,1,EPAIS,N)
          CALL GETVID('COUCHE','MATER',ICOU,IARG,1,MATER,N)
          CALL GETVR8('COUCHE','ORIENTATION',ICOU,IARG,1,ORIEN,N)
...
C        -- INVERSION DE HF -- POUR CALCULER HC PAGE 242
   ...
   90   CONTINUE
        DO 100 I = 1,3
          HFINV(I,I) = 1.D0
  100   CONTINUE
        CALL MGAUSS('NFVD',HF,HFINV,3,3,3,DET,IRET)
        DO 120 K = 1,2
          DO 110 J = 1,2
            DA1I(K,J) = 0.D0
            C11(K,J) = 0.D0
