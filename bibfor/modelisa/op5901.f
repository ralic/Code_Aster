...
C MODIF MODELISA  DATE 25/06/2012   AUTEUR PROIX J-M.PROIX 
C ======================================================================
...
C     COMMANDE:  DEFI_COMPOR MONOCRISTAL
      ...
      CHARACTER*16 NOMPAR(5), ECOULE, ECROIS, ECROCI,ELASTI,NOMVAR(100)
      CHARACTER*16 FASYGL, NOMS(6),COMDES,ROTA,TBINTE,SYSTGL
      CHARACTER*19 LISTR
      REAL*8 MS(6),NG(3),Q(3,3),LG(3),PGL(3,3)
      COMPLEX*16 CBID
      INTEGER IARG,IFM,NIV,NBTBSG, NUMS(2),INDROT
      INTEGER IOCC, NBMAT, NBECOU, NBECRO, NBCINE, NBELAS, NBFASY
      INTEGER I,J,NBELA1, NBSYS, NVI,IMK,IMI,IPR,ITAB,ITSG,IRRA,IDD
...
C        CAS DES LOIS DD_CC_IRRA
         IF (ECOULE.EQ.'MONO_DD_CC_IRRA')  THEN
            IRRA=IRRA+1
         ENDIF
C        CAS DES LOIS DD_CC
         IF (ECOULE(1:10).EQ.'MONO_DD_CC')  THEN
            IDD=IDD+1
         ENDIF
 ...
 557     CONTINUE
      ...
      IF (IROTA.GT.0) THEN
         DO 556 I=1,16
            CALL CODENT(I,'G',CHAINE)
            INDROT=6+3*NBSYST
            IF (IRRA.GT.0) INDROT=INDROT+12*NBOCCM
            NOMVAR(INDROT+I)='ROTA_'//CHAINE
 556     CONTINUE
      ENDIF
...
C     A FAIRE : IMPRESSION Du NOM DES VARIABLES INTERNES.
