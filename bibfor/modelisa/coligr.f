...
C     ---------------
      ...
      LIGOUT = LIGOUZ
C
      LIGR1W = '&&COLIGR.LIGRE1.WRK'
      LIGR2W = '&&COLIGR.LIGRE2.WRK'
      LIGREL = 'LIGREL'
C
...
C --- A CONCATENER :
C     ------------
      CALL JEVEUO(LIGR1W//'.LGRF','L',IDNOM1)
      CALL JEVEUO(LIGR2W//'.LGRF','L',IDNOM2)
C
      IF (ZK8(IDNOM1).NE.ZK8(IDNOM2)) THEN
        CALL U2MESS('F','MODELISA4_34')
      ENDIF
C
C --- AFFECTATION DU NOM DU MAILLAGE AU LIGREL RESULTANT :
C     --------------------------------------------------
      CALL WKVECT(LIGOUT//'.LGRF',BASE//' V K8',2,IDNOMA)
      ZK8(IDNOMA) = ZK8(IDNOM1)
      CALL JELIRA(LIGR1W//'.LGRF','DOCU',IB,PHENO)
      CALL JEECRA(LIGOUT//'.LGRF','DOCU',IB,PHENO)
C
