      SUBROUTINE SPECT1(CASINT,NOMU,SPECTR,ISPECT,BASE,VITE,FREQ,NUOR,
     +                  IMODI,IMODF,NBM,NBPF,NPV,NOMZON,VMOYZI,VMOYTO)
      IMPLICIT REAL*8 (A-H,O-Z)
C-----------------------------------------------------------------------
C            CONFIGURATION MANAGEMENT OF EDF VERSION
C MODIF MODELISA  DATE 08/08/2002   AUTEUR CIBHHLV L.VIVAN 
C ======================================================================
...
C
      ...
      REAL*8       VITE(NPV),FREQ(2,NBM,*),VMOYZI,VMOYTO
      INTEGER      NUOR(NBM)
C
      INTEGER      DIM
...
C
C
C --- 5.RECUPERATION DE LA LONGUEUR DE CORRELATION PHYSIQUE ---
C
      VALR = SPECTR//'.VARE'
...
C
C
C --- 6.CALCUL DES LONGUEURS DE CORRELATION GENERALISEES ---
C
C --- 6.1.RECHERCHE POUR L INTEGRALE DOUBLE DES BORNES
...
C
  ...
  61  CONTINUE
C
C --- 6.2.CREATION D UN PROFIL DE VITESSE NORMALISE POUR
...
C
  ...
  71  CONTINUE
C
C --- 6.3.CREATION D UN VECTEUR DE TRAVAIL POUR STOCKER
...
C
      DO 90 JM = IMODI,IMODF
        ...
        DO 80 IM = IDEB,JM
          ...
          IMB = IM - IMODI + 1
          KK = (JMB* (JMB-1))/2 + IMB
          ZR(ILC2+KK-1) = SPECT2(X1,X2,XLC,ZR(IVITN),ZR(IRHOE),
     +                           ZR(IDEFM),SPECT4,TOL,IER,R1,ERR,
     +                           NBP,IM,JM)
C
          IF (IER.NE.0) THEN
...
C
C
C --- 7.CALCUL DES INTERSPECTRES D'EXCITATIONS MODALES ---
C
C --- 7.1.CREATION D UN VECTEUR DE TRAVAIL POUR STOCKER
C ---     LES VALEURS DU SPECTRE
C
      CALL WKVECT('&&SPECT1.TEMP.SWR ','V V R',NBPF,LWR)
C
C --- 7.2.BOUCLE POUR CHAQUE VITESSE
C
      DO 180 IV = 1,NPV
      VITEZI = VITE(IV)*VMOYZI/VMOYTO
C
C --- 7.2.1.RECUPERATION DE LA DISCRETISATION FREQUENTIELLE
...
C
C --- 7.2.2.CALCUL DES VALEURS DU SPECTRE
C
        IF (ISPECT.EQ.1) THEN
C
          XNU = ZR(IRSP+1)
...
C
        CALL JELIBE(VALE)
C
C --- 7.2.3.CALCUL DES INTERSPECTRES
C
