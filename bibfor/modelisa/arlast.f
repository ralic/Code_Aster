...
C
      ...
      CHARACTER*8   K8BID,TYGEO1,TYGEO2
      INTEGER       NMA,NMA1,NMA2
      INTEGER       JCOLM,JNORM,JTANG
      INTEGER       VALI(2),AIMA1,IMA1,AIMA2,IMA2,NUMMA1,NUMMA2
      INTEGER       NNDC,NDML1,NDML2
      INTEGER       IAS,JGRMA1,JGRMA2,JTYP,JTYPMM
      INTEGER       IMA
      INTEGER       PTMT1(NNM*NNM),PTMT2(NNM*NNM)
      INTEGER       JCOOR,JCONX,JCUMU
      INTEGER       IRET
      INTEGER       JCINO
      INTEGER       JMINO1,JMCUM1,JMVAL1,LCOQE1,LCOQE2
      INTEGER       JMINO2,JMCUM2,JMVAL2
      INTEGER       JQMAMA
      INTEGER       IFM,NIV
      INTEGER       ICPL,IALIEL,ILLIEL,IND1
      CHARACTER*19  LIGRMO,ARLMT1,ARLMT2,NGRMA1,NGRMA2
      INTEGER       IGREL,IEL,JTABCP,IMATU1,IMATU2,IEXI
...
C
      ...
      CALL JELIRA(NGRMA1(1:19),'LONMAX',NMA1,K8BID)
      CALL JEVEUO(NGRMA2(1:19),'L',JGRMA2)
      CALL JELIRA(NGRMA2(1:19),'LONMAX',NMA2,K8BID)

C
C --- LECTURE DONNEES TYPE MAILLES
C
...
C
      DO 210 IGREL = 1,NBGREL(LIGRMO)
        ...
        IND1  = IALIEL-1+ZI(ILLIEL-1+IGREL)-1

        ARLMT1 = MARLEL(1:8)//'.ARLMT1'
        CALL JAEXIN(JEXNUM(ARLMT1(1:19)//'.RESL',IGREL),IEXI)
        IF (IEXI.EQ.0) GOTO 210
        CALL JEVEUO(JEXNUM(ARLMT1(1:19)//'.RESL',IGREL),'L',IMATU1)
        ARLMT2 = MARLEL(1:8)//'.ARLMT2'
        CALL JAEXIN(JEXNUM(ARLMT2(1:19)//'.RESL',IGREL),IEXI)
        IF (IEXI.EQ.0) GOTO 210
...
C
          ...
          AIMA1 = ABS(IMA1)
          IF (AIMA1.GT.NMA1) THEN
            CALL ASSERT(.FALSE.)
          ENDIF
          NUMMA1 = ZI(JGRMA1-1+AIMA1)

          TYGEO1 = ZK8(JTYPMM+ZI(JTYP-1+NUMMA1)-1)
          NDML1  = ZI(JCUMU+NUMMA1) - ZI(JCUMU+NUMMA1-1)
          CALL TMACOQ(TYGEO1    ,NDIM  ,LCOQE1)
          ...
          AIMA2 = ABS(IMA2)
          IF (AIMA2.GT.NMA2) THEN
            CALL ASSERT(.FALSE.)
          ENDIF
          NUMMA2 = ZI(JGRMA2-1+AIMA2)

          TYGEO2 = ZK8(JTYPMM+ZI(JTYP-1+NUMMA2)-1)
          NDML2  = ZI(JCUMU+NUMMA2) - ZI(JCUMU+NUMMA2-1)
          CALL TMACOQ(TYGEO2    ,NDIM  ,LCOQE2)
...
C
          CALL ARLAS9(NDIM,NDML1,NDML1,PTMT1,IDEB,IMATU1,ZR(JMVAL1))
          CALL ARLAS9(NDIM,NDML1,NDML2,PTMT2,IDEB,IMATU2,ZR(JMVAL2))
  220   CONTINUE
