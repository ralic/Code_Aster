...
C
      ELSE
         IF ( NBMA .EQ. 0 ) THEN
             CALL UTMESS('F','MATLOC','IL FAUT FOURNIR DES MAILLES')
         ENDIF
         IF ( NBMA .NE. 1 ) THEN
             CALL UTMESS('F','MATLOC','ON ATTEND 1 ET 1 SEULE MAILLE')
         ENDIF
         DO 20 I = 1 , NBMA
            NUMA = LISTMA(I)
            DO 22 J = 1 , NBM
 ...
 22         CONTINUE
 20      CONTINUE
         CALL UTMESS('F','MATLOC','ON N''A PAS TROUVE LA MAILLE')
 24      CONTINUE
      ENDIF
C
      CALL JENUNO(JEXNUM('&CATA.TM.NOMTM',ZI(IATYMA+NUMA-1)),TYPM)
      IF ( TYPM(1:3) .NE. 'SEG' ) THEN
         CALL UTMESS('F','MATLOC','QUE DES MAILLES DE TYPE "SEG"')
      ENDIF
