...
C
      CALL WKVECT('&&DEFFEN.TEMP.PROL','V V K8',5,IPROL)
      ZK8(IPROL)   = 'FONCTION'
      ZK8(IPROL+1) = 'LIN LIN '
      ZK8(IPROL+2) =  NOMPAR
      ZK8(IPROL+3) = 'TOUTRESU'
      ZK8(IPROL+4) = 'CC      '
C
...
C
        IMR = IMOD - IMODI + 1
        CALL FOINTR(' ',ZK8(IPROL),NBNFEN,DISCFE,ZR(IVALE1),NBP1,DISCFF,
     &              DEFM(1,IMR),IER1)
        CALL FOINTR(' ',ZK8(IPROL),NBNFEN,ZR(IFEN2),ZR(IVALE2),NBP2,
     &              DISCFF(NBP1+1),DEFM(NBP1+1,IMR),IER2)
        IF (IER1.NE.0 .OR. IER2.NE.0) THEN
          CALL UTMESS('F','DEFFEN','PROBLEME RENCONTRE LORS DE L '//
     &         'INTERPOLATION D UNE DES DEFORMEES MODALES')
        ENDIF
C
  20  CONTINUE
C
