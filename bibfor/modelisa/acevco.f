...
C
         IF (IOC.EQ.1 .AND. NE.EQ.0) THEN
            CALL UTMESS('E',CMD,'COQUE : OCCURENCE 1 : LE MOT CLE '//
     +                                       '"EPAIS" EST OBLIGATOIRE.')
            IER = IER + 1
         ENDIF
C
         IF ( NEX.NE.0  .AND.  NIN.NE.0 ) THEN
            ...
            IF ( INT .EQ. 'NON' ) THEN
              CALL UTMESS('E',CMD,'COQUE : AVEC UN EXCENTREMENT, LA '//
     +        'PRISE EN COMPTE DES TERMES D''INERTIE DE ROTATION '//
     +        'EST OBLIGATOIRE.')
              IER = IER + 1
            ENDIF
         ENDIF
