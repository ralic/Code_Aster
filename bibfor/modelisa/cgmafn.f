...
C     ---------------------------
     ...
     +                   'FACE_NORMALE DE CREA_GROUP_MA')
         ...
         IF ( ABS(NV) .NE. NDIM ) THEN
           ...
           CALL UTIMPK('S',' VALEURS POUR LE MOT CLE ',1,'VECT_NORMALE')
...
C
      IF (XNORM2.EQ.ZERO) THEN
          CALL UTMESS('F','CGMAFN','ERREUR DANS LA DONNEE DU '//
     +                   'VECTEUR NORMAL SELON LEQUEL ON VEUT '//
     +                   'SELECTIONNER DES MAILLES : '//
     +                   'CE VECTEUR EST NUL.')
      ENDIF
C
      XNORM = SQRT(XNORM2)
