...
C                            (CERC+RECT) ET AFFECTATIONS DANS LE TAMPON
      DO 62 I = 1 ,NPOAFF
         ...
         CALL JENONU(JEXNOM(MLGNMA,NOMMAI),NUMMAI)
         NUTYEL = ZI(JDME+NUMMAI-1)
         CALL AFFGEN(TMPGEN,NOMMAI,NUTYEL,NTYELE)
 62   CONTINUE
C
...
C --- ALLOCATION DES CARTES
      CALL ALCAR2('G',CARTPO,NOMA,'CAGNPO')
      CALL ALCAR2('G',CARTGE,NOMA,'CAGEPO')
...
C     POUR LA CARTE DE VENT ==> FCXP
 ...
 73      CONTINUE
         ...
         CALL NOCAR2(CARTPO ,3,' ','NOM',1,NOMMAI,0,' ',31)
         CALL NOCAR2(CARTPF,3,' ','NOM',1,NOMMAI,0,' ',1)
 70   CONTINUE
C
...
C ---       GENERALE
            DO 78 I = 1 , 13
               ZR (JDVGE+I-1) = 0.D0
 78         CONTINUE
            CALL NOCAR2(CARTGE,3,' ','NOM',1,NOMMAI,0,' ',13)
         ELSE
