...
C
C IN  NNS    : NOMBRE DE NOUEDS D'ELEMENT (DE LA MAILLE SUPPORT)
...
C
      ...
      REAL*8  R8BID,PREC
      REAL*8  PGREEL(3*NG),PGPARA(3*NG)
...
C
        CALL MGAUSS('TCVD',JACOBA,DFA(P3),DIME,DIME,
     &              NNA   ,VOLMAI,IRET)
...
C
        CALL MGAUSS('TFVP',JACOBB,DFB(P4),DIME,DIME,
     &              NNB   ,R8BID ,IRET)
C
        P0 = P0 + NNS
        P1 = P1 + NNS*DIME
