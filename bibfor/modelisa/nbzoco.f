      SUBROUTINE NBZOCO(CHAR  ,MOTFAC,NOMA  ,IZONE ,JZONE ,
     &                  NSUCO ,NGTOT)
C
...
C MAILLES ET DE GROUPES DE MAILLES
...
C      
C --- NOMBRE TOTAL DE GROUPES DE MAILLES ET DE MAILLES
C --- IMPLIQUES POUR CETTE ZONE DE CONTACT
C
      NGTOT = ABS(NGMAIT) + ABS(NGESCL) + ABS(NMMAIT) + ABS(NMESCL)
      IF (NGTOT.GT.2) THEN
        WRITE(6,*) 'ERREUR: ',NGTOT,NGMAIT,NGESCL,NMMAIT,NMESCL
        CALL ASSERT(.FALSE.)
      ENDIF
C      
