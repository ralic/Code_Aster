...
C     ---------------------------------------
   ...
   10   CONTINUE
        IF (TYPVAL.EQ.'REEL') THEN
          WRITE (IFM,*) '_RELA = ',BETAR
        ELSE IF (TYPVAL.EQ.'COMP') THEN
          WRITE (IFM,*) '_RELA = ',BETAC
...
C     EN ATTENDANT QUE CISI CORRIGE L'ANOMALIE EMISE LE 8/12/97 :
...
C --- SI L'OBJET L  ISREL N'EXISTE PAS, ON LE CREE :
C     ------------  ------------------------------
      CALL JEEXIN(LISREL//'.RLCO',IRET)
      IF (IRET.EQ.0) CALL CRELRL(TYPCO2,TYPVAL,'V',LISREL)

C --- NOMBRE DE RE  LATIONS DE LA LISTE_DE_RELATIONS :
C     ------------  --------------------------------
...
C --- DOUBLANT LA TAILLE DE CES OBJETS  SI LEUR LONGUEUR S'AVERE
C --- INSUFFISANTE :
C     ------------
      IF (LONUTI+NBTERR.GE.LVECLR) THEN
        ...
        CALL JEVEUO(LISREL//'.RLCO','E',IDCOEF)
        CALL JUVECA(LISREL//'.RLDD',2*LVECLR)
        CALL JEVEUO(LISREL//'.RLDD','E',IDDL)
        CALL JUVECA(LISREL//'.RLNO',2*LVECLR)
        CALL JEVEUO(LISREL//'.RLNO','E',IDNOEU)
      END IF
...
C --- EN DOUBLANT LA TAILLE DE CES OBJETS  SI LEUR LONGUEUR
...
C     --------------------
      IF (NBRELA.GE.NBRMAX) THEN
        ...
        CALL JUVECA(LISREL//'.RLBE',2*NBRMAX)
        CALL JEVEUO(LISREL//'.RLBE','E',IDBETA)
        CALL JUVECA(LISREL//'.RLNT',2*NBRMAX)
        CALL JEVEUO(LISREL//'.RLNT','E',IDTERM)
        CALL JUVECA(LISREL//'.RLPO',2*NBRMAX)
        CALL JEVEUO(LISREL//'.RLPO','E',IDPOIN)
        CALL JUVECA(LISREL//'.RLSU',2*NBRMAX)
        CALL JEVEUO(LISREL//'.RLSU','E',IDSURC)
        CALL JUVECA(LISREL//'.RLLA',2*NBRMAX)
        CALL JEVEUO(LISREL//'.RLLA','E',IDLAGR)
      END IF
