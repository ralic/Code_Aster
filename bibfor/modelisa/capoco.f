...
C
      ...
      INTEGER      ICESC,ICESD,ICESL,ICESV,ICMP,NPMAX,ISEC,JMAIL,NUMMA
...
C 
      RANGR0 = INDIK8(ZK8(ICMP),'TSEC    ',1,NPMAX)
      RANGR1 = INDIK8(ZK8(ICMP),'R1      ',1,NPMAX)
      RANGR2 = INDIK8(ZK8(ICMP),'R2      ',1,NPMAX)
C
...
C
            ...
            IF (INO1.EQ.0) CALL UTMESS('F','CAPOCO','BUG NOEUD 1 ')
            ZR(IJPOU-1+INO1) = R1
C
            NUMNO = ZI(IACNEX-1+NBNMA)
            INO2 = INDIIS(ZI(JNOCO),NUMNO,1,NNOCO)
            IF (INO2.EQ.0) CALL UTMESS('F','CAPOCO','BUG NOEUD 2 ')
            ZR(IJPOU-1+INO2) = R2
C
 30      CONTINUE
C
