...
C
C SD EN SORTIE (NOMBOI) :
C =======================
C
C .DIME   : DIMENSIONS ET INDEX DE LA SD BOITE
...
C .MINMAX : BOITES ENGLOBANT LES MAILLES
C           SUIVANT X, Y, [Z]
C           (X1MIN,X1MAX,Y1MIN,Y1MAX,[Z1MIN],[Z1MAX],X2MIN,...)
C .PAN    : PANS (2D = ARETES, 3D = FACES) DES CONVEXES
C           ENGLOBANTS ET INSCRITS DES MAILLES
C           (A1,B1,[C1],D1,E1,A2...)
C           TELS QUE AX+BY+[CZ]+D<=0 (CONVEXE ENGLOBANT)
C           ET       AX+BY+[CZ]+E<=0 (CONVEXE INSCRIT)
C .SOMMET : SOMMETS DES CONVEXES ENGLOBANT LES MAILLES
C           (X1,Y1,[Z1],X2,...)
C .MMGLOB : BOITE ENGLOBANT TOUTES LES MAILLES DE NGRMA
C           (XMIN,XMAX,YMIN,YMAX,[ZMIN,ZMAX])
C .H      : TAILLE MOYENNE DES MAILLES
C           (H1,H2,...)
C
C
