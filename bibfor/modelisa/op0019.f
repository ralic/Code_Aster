...
C MODIF MODELISA  DATE 18/10/2004   AUTEUR LEBOUVIE F.LEBOUVIER 
C ======================================================================
...
C     -----------------
     ...
     &            NOCAGB,NMTGGB,JDLM,JDLN,LMAX,IRET)
      IF (IRET.NE.0) THEN
     ...
     &              )
      END IF
      DO 110 I = 1,3
        NOCADI(I) = MAX(1,NOCADI(I))
        NMTGDI(I) = MAX(1,NMTGDI(I))
  110 CONTINUE
      NBCACO = NOCACO
      NBCAGB = NOCAGB
      NOCACO = MAX(1,NOCACO)
...
C     CORRECTION DE LA DIMENSION DES RAIDEURS REPARTIES :
C     -------------------------------------------------
      IF (NBOCC(10).NE.0) THEN
        DO 120 I = 1,3
          NOCADI(I) = NOCADI(I) + LXRP
...
C     FABRICATION DE LA CARTE COMMUNE A TOUS LES ELEMENTS LINEIQUE
      ...
      IF (NBCART.GT.0) THEN
        ...
        CALL ALCART('G',CARTCF,NOMA,'VENTCX_F',NBCART,1)
      END IF
...
C     -----------------------------------------------------------------
      IF (NBOCC(2).NE.0) THEN
        CALL ACEACO(NOMU,NOMA,LMAX,NOCACO,NMTGCO,NBCAGB,NOCAGB,NMTGGB,
     &              NBOCC(2))
      END IF
...
C     --------------------------------------------------------
      IF (NBOCC(5).NE.0) THEN
        CALL ACEAPC(NOMU,NOMA,LMAX,NOCAPC,NMTGPC,NBOCC(5))
      END IF

C --- AFFECTATION DES SECTIONS AUX ELEMENTS CABLE :
C     ---------------------------------------------
      IF (NBOCC(6).NE.0) THEN
        CALL ACEACA(NOMU,NOMA,LMAX,NOCACA,NMTGCA,NBOCC(6))
      END IF
...
C     -----------------------------------------------------------------
      IF (NBOCC(8).NE.0) THEN
        CALL ACEAMA(NOMU,NOMA,LMAX,NOCAMA,NMTGMA,NBOCC(8))
      END IF

C --- AFFECTATION DES REPERES AUX ELEMENTS POUTRE_FLUI  :
C     ---------------------------------------------------
      IF (NBOCC(9).NE.0) THEN
        CALL ACEAPF(NOMU,NOMA,LMAX,NOCAPF,NMTGPF,NBOCC(9))
      END IF

C --- AFFECTATION DES MATRICES AUX RAIDEURS REPARTIES :
C     ------------------------------------------------
      IF (NBOCC(10).NE.0) THEN
        CALL ACEARP(NOMA,NOMO,LMAX,NOEMAF,NOCADI,NMTGDI,NBOCC(10),IVR,
     &              IFM)
      END IF
...
C     -----------------------------------------------------------
      IF (NBOCC(11).NE.0) THEN
        CALL ACEAGR(NOMU,NOMA,LMAX,NOCAGR,NMTGGR,NBOCC(11))
      END IF
