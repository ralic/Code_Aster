...
C
           ...
           IF (NBGTOT.LT.NTYPG) THEN
              CALL UTMESS('E',CMD,'FAISCEAU_AXIAL : IL Y A PLUS DE '//
     &          'TYPES DE GRILLES QUE DE GRILLES')
           ENDIF
           CALL GETVIS(NOMMCF,'TYPE_GRILLE',OCGRIL,1,0,IBID,NTOT2)
           IF (ABS(NTOT2).NE.NBGTOT) THEN
              CALL UTMESS('E',CMD,'FAISCEAU_AXIAL : IL FAUT AUTANT '//
     &          'D''ARGUMENTS POUR LES OPERANDES <TYPE_GRILLE> ET '//
     &          '<COOR_GRILLE>')
           ENDIF
     ...
     &            (ZI(IVECI+IGRIL-1).GT.NTYPG)) THEN
                 CALL UTMESS('E',CMD,'FAISCEAU_AXIAL, OPERANDE '//
     &             '<TYPE_GRILLE> : DETECTION D''UNE VALEUR ILLICITE')
              ENDIF
 100       CONTINUE
     ...
     &         (ABS(NTYPG6).NE.NTYPG)) THEN
              CALL UTMESS('E',CMD,'FAISCEAU_AXIAL : IL FAUT AUTANT '//
     &          'D''ARGUMENTS POUR LES OPERANDES <LONG_TYPG>, '//
     &          '<LARG_TYPG>, <EPAI_TYPG>, <RUGO_TYPG>, '//
     &          '<COEF_TRAI_TYPG> ET <COEF_DPOR_TYPG>')
           ENDIF
