...
C      
      ...
      ELSEIF (INTEG.EQ.'NOEUD') THEN
        ZR(JCMCF+ZCMCF*(IZONE-1)+1) = 9.D0
      ELSE
        VALK(1) = INTEG
        VALK(2) = 'INTEGRATION'
...
C-----SI LE FROTTEMENT A ETE DECLARE POUR LES GRANDS GLISS, ON SORT
      IF (REACBG .GT. 0) THEN
        IF (FROT(1:4) .NE. 'SANS') THEN
          CALL U2MESS('F','XFEM_25')
        ENDIF
      ENDIF
C        
