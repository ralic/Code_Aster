...
C
      IF (ISPECT.EQ.21) THEN
        ...
        IF (IINTER.NE.0) THEN
          ...
          CALL GETVID(NOMMCF,'NOEUD' ,1,1,0,K8BID,INOEUD)
          IF ( INATUR.NE.IANGL .OR. INATUR.NE.INOEUD
     &                         .OR. INOEUD.NE.IANGL ) THEN
            CALL UTMESS('F',CMD,'SPEC_EXCI_POINT : SI INTE_SPEC '//
     &        'ALORS AUTANT D ARGUMENTS POUR NATURE, ANGL ET NOEUD')
          ENDIF
        ELSE
          CALL GETVID(NOMMCF,'NOEUD',1,1,0,K8BID,INOEUD)
          IF (ABS(INOEUD).NE.1) THEN
            CALL UTMESS('F',CMD,'SPEC_EXCI_POINT : SI GRAPPE_2 '//
     &        'ALORS UN SEUL NOEUD')
          ENDIF
        ENDIF
      ENDIF
...
C
     ...
     +              'DIMENSION', K8B, DIM, R8B, C16B, K8B, IRET )
          IF ( IRET .NE. 0 ) CALL UTMESS('F','OP0145','Y A UN BUG 1' )
          IF (ISPECT.EQ.11) THEN
            CALL GETVID(NOMMCF,'FONCTION',1,1,0,K8BID,IFONCT)
            IF ( ABS(IFONCT) .NE. DIM ) THEN
              CALL UTMESS('F',CMD,'SPEC_FONC_FORME : LE NOMBRE DE '//
     &         'FONCTIONS FOURNIES DOIT ETRE EGAL A LA DIMENSION DE '//
     &         'LA MATRICE INTERSPECTRALE')
            ENDIF
          ELSE
            CALL GETVID(NOMMCF,'NOEUD',1,1,0,K8BID,INOEUD)
            IF ( ABS(INOEUD) .NE. DIM ) THEN
              CALL UTMESS('F',CMD,'SPEC_EXCI_POINT : LE NOMBRE D '//
     &         'ARGUMENTS POUR NATURE, ANGL ET NOEUD DOIT ETRE EGAL '//
     &         'A LA DIMENSION DE LA MATRICE INTERSPECTRALE')
            ENDIF
          ENDIF
        ENDIF
      ENDIF
...
C
        ...
        CALL GETVID(NOMMCF,'NOEUD',1,1,0,K8BID,NNAP)
        NNAP = ABS(NNAP)
        ZI(LVAIN) = ISPECT
        IF (IINTER.EQ.0) THEN
          ZI(LVAIN+1) = 1
...
C
        ...
        CALL GETVID(NOMMCF,'NOEUD',1,1,NNAP,ZK8(LNNOE),IBID)
C
