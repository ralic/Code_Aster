...
C IN NOEARE  : ARETES LINEAIRES ASSOCIEES AUX FACES
C              VOIR ARLPAN
...
C
      IF (DIME.EQ.2) THEN
C
C --- CHAQUE ARETE EST DECOUPEE EN NECH SEGMENTS
C
...
C
        ...
        IF (NIV.GE.2) THEN
          DO 300 , ISEG = 1, NSEG
            VALI(1)=ISEG
            VALI(2)=TRAVI(PFS+2*OFFSEG+2*(ISEG-1))
            VALI(3)=TRAVI(PFS+2*OFFSEG+2*(ISEG-1)+1)
