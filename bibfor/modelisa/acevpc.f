...
C
         IF (NR.NE.0) THEN
            IF (XRC.LE.0.D0) THEN
            CALL UTMESS('E',CMD,'DEFI_ARC: LE RAYON DE COURBURE '//
     +                                            'DOIT ETRE POSITIF.')
            IER = IER + 1
            ENDIF
         ENDIF
         IF (NC.NE.0 .AND. NC.NE.-3) THEN
            CALL UTMESS('E',CMD,'DEFI_ARC: IL FAUT 3 REELS POUR '//
     +                                'DEFINIR LE CENTRE DE COURBURE.')
            IER = IER + 1
         ENDIF
         IF (NP.NE.0 .AND. NP.NE.-3) THEN
            CALL UTMESS('E',CMD,'DEFI_ARC: IL FAUT 3 REELS POUR '//
     +                   'DEFINIR LE POINT DE CONCOURS DES TANGENTES.')
            IER = IER + 1
         ENDIF
         IF (NF.NE.0) THEN
            IF (XFL.LE.0.D0) THEN
            CALL UTMESS('E',CMD,'DEFI_ARC: LE COEFFICIENT DE '//
     +                                'FLEXIBILITE DOIT ETRE POSITIF.')
            IER = IER + 1
            ENDIF
         ENDIF
         IF (NS.NE.0) THEN
            IF (XSI.LE.0.D0) THEN
            CALL UTMESS('E',CMD,'DEFI_ARC: L''INDICE DE CONTRAINTE '//
     +                                            'DOIT ETRE POSITIF.')
            IER = IER + 1
            ENDIF
         ENDIF
