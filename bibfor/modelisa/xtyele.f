...
C
      ...
      REAL*8      CMIN(NDIM),LONGAR,M(NDIM),PADIST,RBID(NDIM)
...
C --- BOUCLE SUR LES FACES
                  DO 450 IFQ=1,NBF
                    CALL INTFAC(NOMA,IMA,IFQ,FA,NNO,ZR(ILST),ZR(ILSN),
     &          NDIM,'NON',IBID,IBID,IGEOM,M,INDPTF,RBID,RBID,CODRET)
