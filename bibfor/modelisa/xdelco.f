...
C
        DO 50 IMA = 1,NBMA
C
C --- NUMERO ABSOLU DE LA MAILLE TRAIT…E
C
          NMAABS = ZI(JMA-1+IMA)
...
C         MAILLE LINEAIRE ?
          LLIN = ISMALI(TYPMA)
C           PRESENCE DE MAILLES LINEAIRES, ON SORT
          IF (LLIN) GOTO 999
   50   CONTINUE
   60 CONTINUE
C
...
C --- RECUPÈRATION DU TYPE D'ÈLÈMENT
...
C
 ...
 130        CONTINUE
     ...
     &                                       ZXAIN*(I-1)+2,IAD)
              ...
              IF (IA.EQ.0) THEN
C                INTERSECTION SUR UN NOEUD SOMMET, UN SEUL ACTIV…
                LINACT(IN) = .FALSE.
              ELSE
