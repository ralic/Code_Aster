...
C
      IF (METHOD(1:3) .EQ. 'GCP') THEN
        ...
        ZI(JCONV+ZCONV*(IWRITE-1)+6) = GCPREA
        CALL GETVIS(MOTFAC,'ITER_GCP_MAXI',IREAD,1,1,GCPREA,NOC)
        ZI(JCONV+ZCONV*(IWRITE-1)+7) = GCPREA
        ...
        ELSE IF (PREC.EQ.'DIRICHLET') THEN
          ...
          ZR(JTOLE+ZTOLE*(IWRITE-1)+5) = RELA
          CALL GETVIS(MOTFAC,'ITER_PRE_MAXI',IREAD,1,1,GCPREA,NOC)
          ZI(JCONV+ZCONV*(IWRITE-1)+8) = GCPREA
        ELSE
          VALK(1) = PREC
          VALK(2) = 'PRE_COND'
          CALL U2MESK('F','CONTACT3_3',2,VALK)
