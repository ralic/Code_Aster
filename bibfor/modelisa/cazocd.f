...
C
      CALL GETVR8(MOTFAC,'TOLE_PROJ_INT',IZONE,1,1,LAMB,NOC)
      ZR(JTOLE+ZTOLE*(IZONE-1)+1) = LAMB      
C
C --- MOT-CLE VECT_Y
C
      IF (LFROT) THEN
...
C
      CALL GETVR8(MOTFAC,'VECT_NORM_ESCL',IZONE,1,3,DIR,NOC)
      IF (NOC .NE. 0) THEN
        IF (APPA(1:5) .NE. 'NODAL') THEN
          CALL U2MESS('F','CONTACT_54')
        END IF
        ZI(JMETH+ZMETH*(IZONE-1)+1) = 4
        ZR(JDIR+3*(IZONE-1))   = DIR(1)
        ZR(JDIR+3*(IZONE-1)+1) = DIR(2)
        ZR(JDIR+3*(IZONE-1)+2) = DIR(3)
      END IF
C
C --- RECUPERATION DU TYPE DE PROJECTION
C
      CALL GETVTX(MOTFAC,'PROJECTION',IZONE,1,1,PROJ,NOC)
C
      IF (PROJ .EQ. 'LINEAIRE') THEN
        ZI(JMETH+ZMETH*(IZONE-1)+4) = 1
      ELSEIF (PROJ .EQ. 'QUADRATIQUE') THEN
        ZI(JMETH+ZMETH*(IZONE-1)+4) = 2
      ELSE
        VALK(1) = PROJ
        VALK(2) = 'PROJECTION'
        CALL U2MESK('F','CONTACT3_3',2,VALK)
...
C
      CALL GETVTX(MOTFAC,'LISSAGE',IZONE,1,1,LISS,NOC)
C
