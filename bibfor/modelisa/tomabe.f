...
C                    BETON, EN % DE LA TENSION INITIALE
C  OUT    : REGL   : CHARACTER*4 TYPE DE REGLEMENT UTILISE :
C                    BPEL OU ETCC
...
C     MATERIAUX BETON.
C     ( LA LOI BPEL_BETON OU ETCC_BETON DOIT ETRE LA MEME 
C      POUR TOUTES LES MAILLES )
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
C.... RELATION DE COMPORTEMENT <BPEL_BETON> OU  <ETCC_BETON>
C      ON TESTE D'ABORD SI BPEL_BETON EST DONNE (ON A IMPOSE DANS
C     LE CATALOGUE QU'UN SEUL COMPORTEMENT ETAIT POSSIBLE)
      ...
      IF ( IRET.EQ.0 ) THEN
C       ON TESTE SI ETCC_BETON EST RENSEIGNE      
        REGL='ETCC'
...
C     RECUPERATION DES PERTES PAR FLUAGE OU RETRAIT POUR BPEL_BETON
...
C PAR L UTILISATEUR DANS LA RELATION BPEL_BETON 
C QUI DOIT ETRE UNIQUE A TOUT LE BETON
...
C        CALL JELIRA(RCVALR,'LONMAX',NBCSTE,K1B)
C        write(6,*)'tomabe',RCVALK,RCVALR,NBCSTE        
C        CALL JEVEUO(RCVALR,'L',JVALR)  
      ENDIF
