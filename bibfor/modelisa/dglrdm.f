...
C MODIF MODELISA  DATE 16/10/2012   AUTEUR ALARCON A.ALARCON 
C ======================================================================
...
C       OMY     : SECTION D'ACIER D'UN LIT DE CABLES SUIVANT Y (M²/ML)
C       OMX     : SECTION D'ACIER D'UN LIT DE CABLES SUIVANT X (M²/ML)
...
C       NYT     : SEUIL D'ENDOMMAGEMENT EN TRACTION
C       NYC     : SEUIL D'ENDOMMAGEMENT EN COMPRESSION
C       MYF     : SEUIL D'ENDOMMAGEMENT EN FLEXION
C ----------------------------------------------------------------------
      INCLUDE 'jeveux.h'
      LOGICAL ULEXIS
      ...
      REAL*8  PENDT,PENDF,NYC,GT,GF,GC
      REAL*8  NUM,NUF,EM,EF,NYT,DXD,MYF,DXP,DRP,MP,RHO,DRD
...
C - RECUPERATION DU PARAMETRE DE COMPRESSION SAISI PAR L'UTILISATEUR
      ...
      IF (COMPR .EQ. 'GAMMA') THEN
        CALL GETVR8(' ','GAMMA_C',1,IARG,1,GC,IBID1)
      ELSE IF (COMPR .EQ. 'SEUIL') THEN
        CALL GETVR8(' ','NYC',1,IARG,1,NYC,IBID1)
      ENDIF
...
C          ENDIF
     ...
     &        (RX(ILIT) .NE. RY(ILIT))) THEN
          ...
          ENDIF
C Mise en coh�rence avec GLRC_DAMAGE
C D�veloppement fait pour -0.5<RX<0.5
C or pour la coh�rence avec GLRC_DAMAGE
...
C - RECUPERATION DE LA DEFORMATION MAXIMALE EN FLEXION (COUR_MAX_F)
        ...
        CALL GETVR8(' ','KAPPA_FLEX',1,IARG,1,EMAXF,IBID1)
...
C - DETERMINATION DES POINTS DE FISSURATION (DXD,NYT) ET (DRD,MYF)
C   ET DES PENTES ELASTIQUES
      CALL DGSEUI(EM,NUM,EF,NUF,EB,NUB,SYTB,H,ICISAI,
     &            NYT,NYC,DXD,MYF,DRD,PELAST,PELASF,ICOMPR)
...
C - DETERMINATION DES PARAMETRES D ENDOMMAGEMENT
      CALL DGENDO(EM,EF,H,NYT,NYC,NUM,NUF,PENDT,
