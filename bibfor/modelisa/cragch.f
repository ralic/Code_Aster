...
C MODIF MODELISA  DATE 20/06/2000   AUTEUR ACBHHCD G.DEVESA 
C ======================================================================
...
C
         IF (TYPCOE.EQ.'REEL'.OR. TYPCOE.EQ.'FONC') THEN
             CALL ALCART('G',CA1, NOMA, 'DDLM_R', LONG, LONG)
         ELSE IF (TYPCOE.EQ.'COMP') THEN
             CALL ALCART('G',CA1, NOMA, 'DDLM_C', LONG, LONG)
         ELSE
            CALL UTMESS('F','CRAGCH','VALEUR INATTENDUE: '//TYPCOE )
         ENDIF
C
         IF (TYPVAL.EQ.'REEL') THEN
            CALL ALCART('G',CA2, NOMA, 'DDLI_R',LONG, LONG)
         ELSE IF (TYPVAL.EQ.'FONC') THEN
            CALL ALCART('G',CA2, NOMA, 'DDLI_F',LONG, LONG)
         ELSE IF (TYPVAL.EQ.'COMP') THEN
            CALL ALCART('G',CA2, NOMA, 'DDLI_C',LONG, LONG)
         ELSE
...
C
      CALL JEVEUO(CA1(1:19)//'.DESC','L',IDESC)
      NGDMX = ZI(IDESC-1+2)
      NEDIT = ZI(IDESC-1+3)
      NDISP = NGDMX - NEDIT
