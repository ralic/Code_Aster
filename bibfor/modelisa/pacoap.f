...
C MODIF MODELISA  DATE 17/02/99   AUTEUR CIBHHLV L.VIVAN 
C ======================================================================
...
20      CONTINUE
C
        IF (J2.EQ.0) CALL UTMESS('F','PACOAP','ON NE TROUVE PAS DE'
...
C
      IF ( IER .NE. 0 ) THEN
         CALL UTMESS('F','PACOAP',' ERREURS DANS LES DONNEES')
      ENDIF
C
      DO 30 I = 1, LONLIS
...
C       CALL JENONU(JEXNOM(NOEUMA,NOMNO2),NUNO2)
        NUNO2=ZI(JNULI2-1+I2)
C       CALL PACOOR(NOMA,NUNO2,0,X2)
...
C         CALL JENONU(JEXNOM(NOEUMA,NOMO1),INO1)
          INO1=ZI(JNULI1-1+I1)
...
50      CONTINUE
C
        IF (J1.EQ.0) CALL UTMESS('F','PACOAP','ON NE TROUVE PAS DE'
