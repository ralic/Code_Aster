...
C
      CHARACTER*8   CHARGE
      CHARACTER*8   K8BID, MOTCLE, MOD, NOMA, NOMNOE, NOMAIL
      CHARACTER*8   MONOEU, MOGRNO, MOMAIL, MOGRMA
      CHARACTER*16  MOTFAC
      CHARACTER*19  LISREL
      CHARACTER*19 LIGRMO
      CHARACTER*24 TRAV, NOEUMA, GRNOMA, MAILMA, GRMAMA, LISNOE
      CHARACTER*1 K1BID
C ----------------------------------------------------------------------
C
      CALL JEMARQ()
...
C        -------------------------------------------------------
      CALL GETVTX (MOTFAC,MOGRNO,IOCC,1,0,K8BID,NG)
      IF (NG.NE.0) THEN
          NG = -NG
...
C        -------------------------------------------------------
      CALL GETVTX (MOTFAC,MONOEU,IOCC,1,0,K8BID,NBNO)
      IF (NBNO.NE.0) THEN
          NBNO = -NBNO
...
C        -------------------------------------------------------
      CALL GETVTX (MOTFAC,MOGRMA,IOCC,1,0,K8BID,NG)
      IF (NG.NE.0) THEN
          NG = -NG
          CALL WKVECT ('&&MALINO.TRAV','V V K8',NG,JJJ)
...
C        -------------------------------------------------------
      CALL GETVTX (MOTFAC,MOMAIL,IOCC,1,0,K8BID,NBMA)
      IF (NBMA.NE.0) THEN
          NBMA = -NBMA
...
C
      ...
      IF (NG.NE.0) THEN
          NG = -NG
          CALL GETVTX (MOTFAC,MOGRNO,IOCC,1,NG,ZK8(JJJ),NGR)
          DO 50 IGR = 1, NGR
...
C
      ...
      IF (NBNO.NE.0) THEN
          NBNO = -NBNO
          CALL GETVTX (MOTFAC,MONOEU,IOCC,1,NBNO,ZK8(JJJ),NNO)
          DO 70 INO = 1, NNO
                  INDNOE = INDNOE + 1
...
C
      ...
      IF (NG.NE.0) THEN
          NG = -NG
          CALL GETVTX (MOTFAC,MOGRMA,IOCC,1,NG,ZK8(JJJ),NGR)
          DO 80 IGR = 1, NGR
               CALL JEVEUO (JEXNOM(GRMAMA,ZK8(JJJ+IGR-1)),'L',JGRO)
               CALL JELIRA (JEXNOM(GRMAMA,ZK8(JJJ+IGR-1)),'LONMAX',
     +                      NBMAIL,K1BID)
...
C
      ...
      IF (NBMA.NE.0) THEN
          NBMA = -NBMA
          CALL GETVTX (MOTFAC,MOMAIL,IOCC,1,NBMA,ZK8(JJJ),NMAI)
          DO 110 IMA = 1, NMAI
                CALL JENONU(JEXNOM(NOMA//'.NOMMAI',ZK8(JJJ+IMA-1)),IBID)
