...
C
      ...
      NDIME  = 2
C
C --- 1.2. LECTURE DONNEES MAILLAGE
C
...
C
C --- 1.3. LECTURE DONNEES NORMALES
C
      ...
      ENDIF
C
C --- 1.4. LECTURE DONNEES BOITES APPARIEMENT
C
...
C
C --- 1.5. LECTURE DONNEES GROUPE DE MAILLES
C
      CALL JEVEUO(NGRMA,'L',JGRMA)
      CALL JELIRA(NGRMA,'LONMAX',NMA,K8BID)
C
C --- 1.6. QUELQUES VERIFICATIONS
C
...
C
C====
C 2. EXPLORATION DES MAILLES
C====
C
      DO 20 , IMA = 1, NMA

C ==> 2.1. CARACTERISTIQUES DE LA MAILLE
C ---  NUMERO ET TYPE
C
        NUMA   = ZI(JGRMA + IMA - 1)
...
C --- 2.2.3. ERREUR
          ELSE
            CALL U2MESK('F','ARLEQUIN_20',1,TYPEMA)
          ENDIF
C
C --- 2.2.4. TROIS SOMMETS EN 2D: +6
          JDCSOM = JDCSOM + 6
C
C ==> 2.3. MAILLE QUADRANGULAIRE
C
...
C --- 2.3.3. ERREUR
          ...
          ENDIF
C
C --- 2.3.4. QUATRE SOMMETS EN 2D: +8
          JDCSOM = JDCSOM + 8
        ELSE
          CALL U2MESK('F','ARLEQUIN_20',1,TYPEMA)
        ENDIF
C
C ==> 2.4. CALCUL DE LA BOITE GLOBALE
C
        SURF   = 1.D0
        JDCMMG = 0
        DO 24 , IDIME = 1,NDIME
   ...
   24   CONTINUE
C
C ==> 2.5. EQUATIONS DES PANS :
C ---  * NORMALISATION PAR LA SURFACE DE LA BOITE
C        EN EFFET, EN 2D L'EQUATION D'UNE DROITE EST UN PRODUIT
C        VECTORIEL, DONC HOMOGENE A UNE SURFACE.
C ---  * CALCUL DES DEUX BOITES (ENGLOBANT ET INSCRIT) PAR LA PRECISION
C        COMME ON VIENT DE NORMALISER, ON UTILISE LE CHIFFRE BRUT
C
        DO 25 , IPAN = 1, NPAN
          DO 251 , L = 1, 4
  ...
  251     CONTINUE
   ...
   25   CONTINUE
C
C ==> 2.6. INFORMATIONS SUR DIMENSIONS
C
        LCPAN = LCPAN + NPAN
        LCSOM = LCSOM + NSOM
        ...
        JDCDIM = JDCDIM + 2
C
C ==> 2.7. DIMENSION CARACTERISTIQUE DE LA BOITE
C
        DIMEH = SQRT(SURF)
