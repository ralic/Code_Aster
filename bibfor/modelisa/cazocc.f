...
C MODIF MODELISA  DATE 13/09/2011   AUTEUR MASSIN P.MASSIN 
C ======================================================================
...
C
      ...
      ELSE IF (INTEG(1:5) .EQ. 'GAUSS') THEN
        CALL GETVIS(MOTFAC,'ORDRE_INT',IZONE,1,1,PARING,NOC)
...
C
      CALL GETVTX(MOTFAC,'SANS_GROUP_MA'   ,IZONE,1,1,SGRNO,NOC)
      CALL GETVTX(MOTFAC,'SANS_MAILLE'     ,IZONE,1,1,SGRNO,NOCC)
...
C
      ...
      ENDIF
C
      CALL GETVTX(MOTFAC,'SANS_GROUP_NO_FR',IZONE,1,1,SGRNO,NOC)
      CALL GETVTX(MOTFAC,'SANS_NOEUD_FR'   ,IZONE,1,1,SGRNO,NOCC)
...
C
      IF (LSSFRO) THEN
        CALL GETVR8(MOTFAC,'DIRE_EXCL_FROT',IZONE,1,3,DIR,NOC)
        LEXDIR = (NOC .NE. 0)
        IF (.NOT.LEXDIR) THEN
...
C
      CALL GETVTX(MOTFAC,'EXCLUSION_PIV_NUL',IZONE,1,1,PIVOT,NOC)
