...
C
      ...
      ELSE IF (INTEG(1:5) .EQ. 'GAUSS') THEN
        ...
        ZR(JCMCF+ZCMCF*(IZONE-1)+1-1) = 10.D0*PARINT + 2.D0
      ELSE IF (INTEG(1:7) .EQ. 'SIMPSON') THEN
        CALL GETVIS(MOTFAC,'ORDRE_INT',IZONE,IARG,1,PARINT,NOC)
        ZR(JCMCF+ZCMCF*(IZONE-1)+1-1) = 10.D0*PARINT + 3.D0
      ELSE IF (INTEG(1:6) .EQ. 'NCOTES') THEN
        CALL GETVIS(MOTFAC,'ORDRE_INT',IZONE,IARG,1,PARINT,NOC)
        ZR(JCMCF+ZCMCF*(IZONE-1)+1-1) = 10.D0*PARINT + 4.D0
      ELSE
        CALL ASSERT(.FALSE.)
      END IF
C
...
C
        IF (COEFFF.EQ.0.D0) THEN
          ZR(JCMCF+ZCMCF*(IZONE-1)-1+ 3) = 0.D0
          ZR(JCMCF+ZCMCF*(IZONE-1)-1+29) = 0.D0
        ENDIF
      END IF
C
