...
C
      ...
      INTEGER      NBNO,NBNOT ,NBMAT,NCTOT,NTRG
      INTEGER      IRET
      ...
      INTEGER      NBVLI,NBVLR,NBVLK,NBVLI1,NBVLI2
      PARAMETER   ( NBVLI = 5 , NBVLR = 1 , NBVLK = 1, NBVLI1 = 1 ,
     &             NBVLI2 = 2 )
      INTEGER      VALI(NBVLI)
      REAL*8       VALR(NBVLR)
...
C --- AFFICHAGE
C
      CALL ARLDBG(NOMPRO,NIV,IFM,1,NBVLI1,VALI,NBVLR,VALR,NBVLK,VALK)
C
...
C
      ...
      VALI(1)=NBMAT
      CALL ARLDBG(NOMPRO,NIV,IFM,2,NBVLI1,VALI,NBVLR,VALR,NBVLK,VALK)
C
C --- ESTIMATION GROSSIERE DE LA LONGUEUR CUMULEE DES CONNECTIVITES
C
      NCTOT = NNOIN1 + NNOIN2 + NMASS*NBSS*NNEL
      VALI(1)=NCTOT
      CALL ARLDBG(NOMPRO,NIV,IFM,3,NBVLI1,VALI,NBVLR,VALR,NBVLK,VALK)
C
...
C
      ...
      VALI(1)=NBNOT
      CALL ARLDBG(NOMPRO,NIV,IFM,4,NBVLI1,VALI,NBVLR,VALR,NBVLK,VALK)
C
...
C
      CALL JEVEUO(MAIL(1:8)// '.COORDO    .VALE','L',JCOORO)
      DO 10 IAUX = 1,3*NBNO
        ZR(JCOOR+IAUX-1) = ZR(JCOORO+IAUX-1)
  10  CONTINUE
C
C --- RECOPIE DES NOMS DES ANCIENS NOEUDS
C
      DO 11 IAUX = 1,NBNO
        CALL JENUNO(JEXNUM(MAIL(1:8)//'.NOMNOE',IAUX),NOM)
...
C
     ...
     &                NTRG    ,NUMMA2,CXCUMU)
          DO 35 IAUX = 1,NTRG
            ZI(JGRP3 +IGRM3-1)   = IMAIL + IAUX - 1
            ZI(JTABCO+IMAIL+IAUX-2) = -NUMMA2
            ZI(JTABCP+IMAIL+IAUX-2) = ICPL
            IGRM3 = IGRM3 + 1
 35       CONTINUE
C
          IMAIL = IMAIL + NTRG
          INOEU = INOEU + NTRG*NNEL
          NBMAR = NBMAR + NTRG
        ENDIF
 30   CONTINUE
C
...
C
      ...
      VALI(4)=NMAIN2
      VALI(5)=NBMAR
      CALL ARLDBG(NOMPRO,NIV,IFM,5,NBVLI,VALI,NBVLR,VALR,NBVLK,VALK)
C
...
C
      ...
      VALI(1)=NBNOT
      VALI(2)=INOEU-1
      CALL ARLDBG(NOMPRO,NIV,IFM,6,NBVLI2,VALI,NBVLR,VALR,NBVLK,VALK)
C
...
C
      ...
      VALI(1)=NCTOT
      VALI(2)=CXCUMU
      CALL ARLDBG(NOMPRO,NIV,IFM,7,NBVLI2,VALI,NBVLR,VALR,NBVLK,VALK)
C
C --- CREATION DES GROUPES
C
      CALL ARLDBG(NOMPRO,NIV,IFM,8,NBVLI1,VALI,NBVLR,VALR,NBVLK,VALK)
      NMAGR = 0
      IF (NMAIN1.NE.0) THEN
        NMAGR  = NMAGR + 1
      ENDIF
      ...
      ENDIF
      VALI(1)=NMAGR
      CALL ARLDBG(NOMPRO,NIV,IFM,9,NBVLI1,VALI,NBVLR,VALR,NBVLK,VALK)
      CALL JECREC(GRPMA,'V V I','NOM','DISPERSE','VARIABLE',NMAGR)
      IF (NMAIN1.NE.0) THEN
        VALI(1)=NMAIN1
        CALL ARLDBG(NOMPRO,NIV,IFM,10,NBVLI1,VALI,NBVLR,VALR,NBVLK,VALK)
        CALL JECROC(JEXNOM(GRPMA,'INCLU1'))
        CALL JEECRA(JEXNOM(GRPMA,'INCLU1'),'LONMAX',
     &        MAX(1,NMAIN1),K8BID)
        CALL JEECRA(JEXNOM(GRPMA,'INCLU1'),'LONUTI',NMAIN1,K8BID)
        CALL JEVEUO(JEXNOM(GRPMA,'INCLU1'),'E',IDA)
        DO 333 IAUX = 1,NMAIN1
          ZI(IDA+IAUX-1) = ZI(JGRP1+IAUX-1)
 333    CONTINUE
      ENDIF
      IF (NMAIN2.NE.0) THEN
        VALI(1)=NMAIN2
        CALL ARLDBG(NOMPRO,NIV,IFM,11,NBVLI1,VALI,NBVLR,VALR,NBVLK,VALK)
        CALL JECROC(JEXNOM(GRPMA,'INCLU2'))
        CALL JEECRA(JEXNOM(GRPMA,'INCLU2'),'LONMAX',
     &        MAX(1,NMAIN2),K8BID)
        CALL JEECRA(JEXNOM(GRPMA,'INCLU2'),'LONUTI',NMAIN2,K8BID)
        CALL JEVEUO(JEXNOM(GRPMA,'INCLU2'),'E',IDA)
        DO 334 IAUX = 1,NMAIN2
          ZI(IDA+IAUX-1) = ZI(JGRP2+IAUX-1)
 334    CONTINUE
      ENDIF
      IF (NBMAR.NE.0) THEN
        VALI(1)=NBMAR
        CALL ARLDBG(NOMPRO,NIV,IFM,12,NBVLI1,VALI,NBVLR,VALR,NBVLK,VALK)
        CALL JECROC(JEXNOM(GRPMA,'SOUS'))
        CALL JEECRA(JEXNOM(GRPMA,'SOUS'),'LONMAX',
     &        MAX(1,NBMAR),K8BID)
        CALL JEECRA(JEXNOM(GRPMA,'SOUS'),'LONUTI',NBMAR,K8BID)
        CALL JEVEUO(JEXNOM(GRPMA,'SOUS'),'E',IDA)
        DO 335 IAUX = 1,NBMAR
          ZI(IDA+IAUX-1) = ZI(JGRP3+IAUX-1)
 335    CONTINUE
      ENDIF
C
...
C
      IF (ULPMAI.GT.0) THEN
        CALL ARLDBG(NOMPRO,NIV,IFM,13,NBVLI1,VALI,NBVLR,VALR,NBVLK,VALK)
        CALL IRMGMS(ULPMAI,NDIM,NBNOT,MAILAR,NMAGR,'        ',.FALSE.,2)
      ENDIF
C
C --- AFFICHAGE
C
      CALL ARLDBG(NOMPRO,NIV,IFM,14,NBVLI1,VALI,NBVLR,VALR,NBVLK,VALK)
C
C --- MENAGE
C
      CALL JEDETR(VTRAV)
