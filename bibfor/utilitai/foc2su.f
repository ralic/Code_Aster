      SUBROUTINE FOC2SU (SORTIE,NBPARA,NBFON,NOMFON,COEF,NOMPAR,BASE)
      IMPLICIT REAL*8 (A-H,O-Z)
      INTEGER                   NBPARA, NBFON
      CHARACTER*(*)      SORTIE,             NOMFON(*),  NOMPAR
      REAL*8                                        COEF(*)
      CHARACTER*1                                               BASE
C     ----------------------------------------------------------------
...
C
      ...
      PROL(20:24) = '.PROL'
C
      CALL WKVECT('&&FOC2SU.IEME  ','V V I'  ,NBFON,LIEME)
      CALL WKVECT('&&FOC2SU.NOMFON','V V K24',NBPARA,LNOMF)
      DO 100 IPARA = 1, NBPARA
         ZK24(LNOMF+IPARA-1)(1:10) = '&&FOC2SU.F'
         CALL CODENT(IPARA,'G',ZK24(LNOMF+IPARA-1)(11:19))
         DO 110 J = 1, NBFON
            ZI(LIEME+J-1) = IPARA
 110     CONTINUE
         CALL FOC3SU(ZK24(LNOMF+IPARA-1),NBFON,NOMFON,COEF,NOMPAR,
     +                                                   ZI(LIEME),'V')
 100  CONTINUE
C
...
C     --- DESTRUCTION DES OJETS DE TRAVAIL ---
      CALL JEDETR('&&FOC2SU.IEME')
      DO 999 IPARA = 1, NBPARA
         CALL JEDETR( ZK24(LNOMF+IPARA-1) )
 999  CONTINUE
      CALL JEDETR('&&FOC2SU.NOMFON')
C
      CALL JEDETC('V','&&FOC2SU',1)
      CALL JEDEMA()
      END
