...
C     --- CREATION ET REMPLISSAGE DE L'OBJET NOMFON.PROL ---
      NPROL = 6 + 2*NBFONC
      CALL WKVECT(NOMFON//'.PROL','G V K8',NPROL,LPRO)
      ZK8(LPRO  ) = 'NAPPE   '
      CALL GETVTX(' ','INTERPOL'   ,0,1,2,INTERP,L1)
      IF ( L1 .EQ. 1 ) INTERP(2) = INTERP(1)
      ZK8(LPRO+1) = INTERP(1)//INTERP(2)
      CALL GETVTX(' ','NOM_PARA'   ,0,1,1,ZK8(LPRO+2),L)
      CALL GETVTX(' ','NOM_RESU'   ,0,1,1,ZK8(LPRO+3),L)
      CALL GETVTX(' ','PROL_GAUCHE',0,1,1,ZK8(LPRO+4)(1:1),L)
      CALL GETVTX(' ','PROL_DROITE',0,1,1,ZK8(LPRO+4)(2:2),L)
C
...
C
      ...
      IF ( DEFONC ) THEN
         CALL GETVTX(' ','NOM_PARA_FONC',0,1,1,ZK8(LPRO+5),L)
         MXVA = 0
