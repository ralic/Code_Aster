...
C
      IF ( BASE.NE.'V' .AND. BASE.NE.'G' ) THEN
         CALL UTMESS('F','TBEXVE','TYPE BASE INCONNU :'//BASE)
      ENDIF
C
...
C
      ...
      IF ( IRET .EQ. 0 ) THEN
         CALL UTMESS('F','TBEXVE','LA TABLE N''EXISTE PAS') 
      ENDIF
C
      ...
      NBLIGN = ZI(JTBNP+1)
      IF ( NBPARA .EQ. 0 ) THEN
         CALL UTMESS('F','TBEXVE','PAS DE PARAMETRES DEFINIS') 
      ENDIF
      IF ( NBLIGN .EQ. 0 ) THEN
         CALL UTMESS('F','TBEXVE','PAS DE LIGNES DEFINIES') 
      ENDIF
C
