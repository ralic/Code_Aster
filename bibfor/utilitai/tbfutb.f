...
C
      IF ( BASE.NE.'V' .AND. BASE.NE.'G' ) THEN
         CALL UTMESS('F','TBFUTB','TYPE BASE INCONNU :'//BASE)
      ENDIF
C
...
C
      ...
      DO 10 I = 1 , NTAB
         ...
         IF ( IRET .EQ. 0 ) THEN
            CALL UTMESS('F','TBFUTB','LA TABLE N''EXISTE PAS') 
         ENDIF
C
         ...
         IF ( NBPARA .EQ. 0 ) THEN
            CALL UTMESS('F','TBFUTB','PAS DE PARAMETRES DEFINIS') 
         ENDIF
         IF ( NBLIGN .EQ. 0 ) THEN
            CALL UTMESS('F','TBFUTB','PAS DE LIGNES DEFINIS') 
         ENDIF
C
