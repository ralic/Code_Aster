...
C     INSERTION DU PARAMETRE 'RT' DANS LA TABLE 'RESU'
      ...
      IF ( NT .EQ. 0 ) THEN
        CALL GETVTX ( 'CARA_POUTRE', 'GROUP_MA', 1,1,0, K8B, NGM )
        IF (NGM.NE.0) THEN
          NGM = 1
          CALL GETVTX('CARA_POUTRE','GROUP_MA',1,1,NGM,NOGRMA,NGM)
          NOMA=NOGRMA
...
C     -------------------
        CALL GETVTX('CARA_POUTRE','GROUP_MA_INTE',1,1,0,K8B,NGI)
        IF (NGI.NE.0) THEN
          NGI = -NGI
          CALL WKVECT('&&PECAPO.GRMA_INTE','V V K8',NGI,IDGRMI)
          CALL GETVTX('CARA_POUTRE','GROUP_MA_INTE',1,1,
     &                NGI,ZK8(IDGRMI),NGRI)
        ELSE
