...
C               12   345678   9012345678901234
      ...
      CALL PSNOSD ( NOMD2, NBSTSE, NOSTDE, IRET )
      IF ( IRET.NE.0 ) THEN
         GOTO 9999
      ENDIF
      IF ( NBSTSE.NE.0 ) THEN
        CALL JEVEUO ( NOSTDE, 'L', ADSTDE )
      ENDIF
      NBSTRU = MAX(NBSTSE,0)
C
C====
...
C
      ENDIF
C
      IF ( NRSTRU.NE.0 .OR. NBSTSE.LT.0 ) THEN
...
C
      CALL WKVECT('&&'//NOMPRO//'.TIRET','V V K16',INOMSY,LTIRT)
      CALL WKVECT('&&'//NOMPRO//'.POINT','V V K16',INOMSY,LPOIN)
      DO 242 I = 1,INOMSY
C                          1234567890123456
        ZK16(LTIRT+I-1) = '----------------'
        ZK16(LPOIN+I-1) = '      ...       '
...
C
         ...
         FORM2 = '(1X,''!'',1X,I10,1X,''!'',A'//NOMB1//')'
C
         CALL WKVECT('&&'//NOMPRO//'.TIRET','V V K16',IPAR,LTIRT)
         CALL WKVECT('&&'//NOMPRO//'.POINT','V V K16',IPAR,LPOIN)
         DO 262 I = 1,IPAR
            ZK16(LTIRT+I-1) = '----------------'
            ZK16(LPOIN+I-1) = '      ...       '
  262    CONTINUE
