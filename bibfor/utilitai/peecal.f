      SUBROUTINE PEECAL(RESU,NOMCHA,LIEU,NOMLIE,MODELE,
...
C     -----  FIN  COMMUNS NORMALISES  JEVEUX  --------------------------
      ...
      INTEGER NUCMP,JCESK,JCMPGD,NCMPMX,IBID,IAD,JINTR,JINTK,INDMA
...
C     LES MAILLES DU POST TRAITEMENT
      CALL WKVECT('&&PEECAL_IND.MAILLE','V V I',NBMA,INDMA)
...
C      CESOUT: CHAMP ELGA CORRESPONDANT AU CHAMP CHPOST (SIMPLE) PONDERE
C              PAR LE POIDS*JACOBIEN. 
C      CESPOI: CHAMP ELGA CORRESPONDANT AU POIDS*JACOBIEN
      CESOUT='&&PEECAL_CESOUT'
      CESPOI='&&PEECAL_POIDS_'//NOMCHA(1:4)
      CALL CHPOND(CHPOST,CESOUT,CESPOI)
...
C     (POUR LA GRANDEUR VARI_* , IL FAUT CONSTITUER :(V1,V2,...,VN))
      ...
      IF (NOMGD(1:5).NE.'VARI_') THEN
        CALL DISMOI('F','NB_CMP_MAX',NOMGD,'GRANDEUR',NCMPMX,K8B,IBID)
        CALL JEVEUO(JEXNOM('&CATA.GD.NOMCMP',NOMGD),'L',JCMPGD)
      ELSE 
        CALL JELIRA(CESOUT//'.CESC','LONMAX',NCMPMX,K8B)
        CALL WKVECT('&&PEECAL.LIST_CMP','V V K8',NCMPMX,JCMPGD)
        DO 25 I = 1,NCMPMX
          NOMVA = 'V'
...
C --- CALCUL DE L'INTEGRALE ET DE LA MOYENNE(=INTEGRALE/VOLUME):
      DO 30 ICMP = 1 , NBCMP
         NUCMP=INDIK8(ZK8(JCMPGD),NOMCMP(ICMP),1,NCMPMX)
         VAL=0.D0
         VOL=0.D0
         DO 35 IMA=1,NBMA
...
C --- ON REMPLIT LA TABLE
      ...
      CALL JEDETR('&&PEECAL_IND.MAILLE')
      CALL JEDETR('&&PEECAL_CESOUT')
      CALL JEDETR('&&PEECAL_CESPOI')
