...
C ------ ON RECUPERE LES TABLEAUX CONSTRUITS DANS CREACO
      ...
      CALL JEVEUO('RENUM2','L',RENUM2)
      CALL JEVEUO('RENUM3','L',RENUM3)
      CALL JEVEUO('CO    ','L',CO)
      CALL JEVEUO('IDCO  ','L',IDCO)
      CALL JEVEUO('NBMAMA','L',NBMAMA)
      CALL JEVEUO('MABORD','L',MABORD)
...
C ------- CREATION DES GROUP_MA
      ...
      NBMATO=NBMABO
      CALL CREAGM(NBMATO,NBPART,SDB,RENUM,NBMASD,IDMASD,MA,
     &                             NUMSDM,SDBORD,MASD,NOMSDM)

C ------- ON RECUPERE LES TABLEAUX CREES DANS CREAGM

      CALL JEVEUO('MASD  ','L',MASD)
      CALL JEVEUO('IDMASD','L',IDMASD)
      CALL JEVEUO('NOMSDM','L',NOMSDM)

C ------- ON EFFECTUE LE VOISINAGE DE CHAQUE SD 
      
      IF (VAL.EQ.1) THEN 
        CALL WKVECT('TEMP  ','V V I',NBPART,TEMP)
        DO 166 ISD=1,NBPART
          ID=0
