...
C
      ...
      ELSE
         CALL UTMESS('F','TBACCE','ACCES INCONNU '//MODACC)
      ENDIF
C
      NOMTAB = NOMTA
      CALL JEEXIN ( NOMTAB//'.TBBA', IRET )
      IF ( IRET .EQ. 0 ) THEN
         CALL UTMESS('F','TBACCE','LA TABLE N''EXISTE PAS')
      ENDIF
C
      ...
      NBLIGN = ZI(JTBNP+1)
      IF ( NBPARA .EQ. 0 ) THEN
         CALL UTMESS('F','TBACCE','PAS DE PARAMETRES DEFINIS')
      ENDIF
      IF ( NBLIGN .EQ. 0 ) THEN
         CALL UTMESS('F','TBACCE','PAS DE LIGNES DEFINIS')
      ENDIF
      IF ( NUMELI .GT. NBLIGN ) THEN
         CALL UTMESS('F','TBACCE','MAUVAIS NUMERO DE LIGNE')
      ENDIF
C
