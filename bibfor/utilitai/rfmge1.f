...
C
      ...
      IF (IRET.NE.0) THEN
         CALL UTMESS('F','RFMGE1','PROBLEME(S) RENCONTRE(S) LORS DE '//
...
C
         ...
         IF(IRET.NE.0) THEN
          CALL UTMESS('F','RFMGE1','PB LORS DE L''EXTRACTION DU CHAMP')
         ENDIF
         CALL RSADPA(MODGEN,'L',1,'FREQ',ZI(JORDR+IORD-1),0,IAD,K8B)
         ZR(LVAR+IORD) = ZR(IAD)
C
         ...
         IF ( K16B(1:1) .NE. 'R') THEN
            CALL UTMESS('F','RFMGE1','ON NE TRAITE QUE LE TYPE REEL')
         ENDIF
C
 ...
 110        CONTINUE
          CALL UTMESS('F','RFMGE1','COMPOSANTE GENERALISEE NON TROUVEE')
 114        CONTINUE
            IM = I
         ELSE
            IM = NCMP
