...
C
      IF ( NBVAR .EQ. 1 ) THEN
         IF ( NBRES .NE. 1  .AND.  CHPROL(5)(1:2) .NE. 'CC' ) THEN
            CALL UTMESS('F','FOINTR','ERREUR_1 FONCTION')
         ENDIF
 ...
 10         CONTINUE
         ELSE
            ...
            ELSE
               CALL UTMESS('F','FOINTR','ERREUR_2 FONCTION.')
            ENDIF
         ENDIF
         GOTO 9999
      ENDIF
...
C     ------------------------------------------------------------------
 ...
 1100    CONTINUE
      ELSEIF (CHPROL(1) .EQ.'INTERPRE' ) THEN
         ...
         IF ( LONUTI .NE. 1 ) THEN
            CALL UTMESS('F','FOINTR (ERREUR 02)',
 ...
 1200    CONTINUE
      ELSE
 2000    CONTINUE
         CALL UTMESS('F','FOINTR (ERREUR 01)',
     +               'INTERPOLATION-EXTRAPOLATION RESERVEE A '//
     +               'DES FONCTIONS DE TYPE "FONCTION" '//
     +               'OU DE TYPE "CONSTANT" '//
     +               'OU DE TYPE "INTERPRE".')
      ENDIF
 9999 CONTINUE
