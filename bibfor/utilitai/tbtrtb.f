...
C
      IF ( BASE.NE.'V' .AND. BASE.NE.'G' ) THEN
         CALL UTMESS('F','TBTRTB','TYPE BASE INCONNU :'//BASE)
      ENDIF
C
...
C
      ...
      IF ( IRET .EQ. 0 ) THEN
         CALL UTMESS('F','TBTRTB','LA TABLE N''EXISTE PAS') 
      ENDIF
C
      ...
      NBLIGN = ZI(JTBNP+1)
      IF ( NBPARA .EQ. 0 ) THEN
         CALL UTMESS('F','TBTRTB','PAS DE PARAMETRES DEFINIS') 
      ENDIF
      IF ( NBLIGN .EQ. 0 ) THEN
         CALL UTMESS('F','TBTRTB','PAS DE LIGNES DEFINIS') 
      ENDIF
C
...
C
      ...
      ELSEIF ( NPARA .GT. 2 ) THEN
         CALL UTMESS('F','TBTRTB','ON NE TRIE QUE 1 OU 2 PARAMETRES')
      ENDIF
      I = 1
      INPAR = LIPARA(I)
      DO 102 J = 1 , NBPARA
