...
C MODIF UTILITAI  DATE 15/10/2007   AUTEUR LEBOUVIER F.LEBOUVIER 
C ======================================================================
...
C     -----  FIN  COMMUNS NORMALISES  JEVEUX  --------------------------
     ...
     &        MXVAL2,NBPAR2,IORIG,NRE,ICAGE,IRET1
      PARAMETER (MXVAL1=16,NBPAR1=18)
      PARAMETER (MXVAL2=25,NBPAR2=27)
      REAL*8 ALPHA,ZERO,ORIG(3),R8B
      CHARACTER*8 K8B,NOMA,LPAIN(15),LPAOUT(5),NOMLIG,TYPARR(NBPAR2),
     &            VALEK(2)
      CHARACTER*16 NOPARR(NBPAR2)
      CHARACTER*19 CHELEM,CHDEF,COMPOR
      CHARACTER*24 LCHIN(15),LCHOUT(1),MLGGMA,MLGNMA
      CHARACTER*24 CHGEOM,CHGEO2,CHCARA(15),CHHARM,LIGREL
      COMPLEX*16 C16B
...
C --- RECUPERATION DU NIVEAU D'IMPRESSION
     ...
     &            CHHARM,IRET)
      ...
      NOMLIG = '&&PEMAIN'
      CALL EXLIMA('MASS_INER','V',MODELE,NOMLIG,LIGREL)
...
C     --- IMPRESSION DU CHAMELEM ---
      ...
      NBPARR = NBPAR1
      DO 10 IOCC = 1,NBOCC
        CALL GETVR8('MASS_INER','ORIG_INER',IOCC,1,0,R8B,NR)
        IF (NR.NE.0) THEN
          MXVALE = MXVAL2
          NBPARR = NBPAR2
          GO TO 20
...
C     --- CREATION DE LA TABLE ---
      ...
      DO 50 IOCC = 1,NBOCC
        ...
        ORIG(2) = ZERO
        ORIG(3) = ZERO
        CALL GETVTX('MASS_INER','TOUT',IOCC,1,0,K8B,NT)
        CALL GETVEM(NOMA,'GROUP_MA','MASS_INER','GROUP_MA',IOCC,1,0,K8B,
     &              NG)
        CALL GETVEM(NOMA,'MAILLE','MASS_INER','MAILLE',IOCC,1,0,K8B,NM)
        CALL GETVR8('MASS_INER','ORIG_INER',IOCC,1,0,R8B,NR)
        IF (NR.NE.0) THEN
          IORIG = 1
          NRE = -NR
          CALL GETVR8('MASS_INER','ORIG_INER',IOCC,1,NRE,ORIG,NR)
        END IF
        IF (NT.NE.0) THEN
        ...
        IF (NG.NE.0) THEN
          ...
          CALL GETVEM(NOMA,'GROUP_MA','MASS_INER','GROUP_MA',IOCC,1,
     &                NBGRMA,ZK8(JGR),NG)
          VALEK(2) = 'GROUP_MA'
          DO 30 IG = 1,NBGRMA
   ...
   30     CONTINUE
        ...
        IF (NM.NE.0) THEN
          ...
          CALL GETVEM(NOMA,'MAILLE','MASS_INER','MAILLE',IOCC,1,NBMAIL,
     &                ZK8(JMA),NM)
          VALEK(2) = 'MAILLE'
          DO 40 IM = 1,NBMAIL
