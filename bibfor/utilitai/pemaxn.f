...
C     -----  FIN  COMMUNS NORMALISES  JEVEUX  --------------------------
      ...
      INTEGER NUCMP,JCESK,JCMPGD,NCMPM,IBID,IAD,JINTR,INDMA
      INTEGER JMESMA,IPT,NBSP,NBPT,ICMP,IMA,NBPARA,JFAM,INDIK8,NBNO
      INTEGER INO,NMIN,NMAX,NPARA,JINTN,PMAX,PMIN,JINTI,JDEB
      REAL*8  VMIN,VMAX,INST,R8MAEM
      COMPLEX*16 CBID
      CHARACTER*8  NOMA,K8B,TYPMCL(2),NOMGD,MOCLES(2),NOMVA,KNMIN,KNMAX
      CHARACTER*16 MOTCLE(2),VALK(2)
      CHARACTER*19 LIGREL,CESOUT
      CHARACTER*24 MESMAI,NOMNOE,NOMMAI
      LOGICAL EXIST
...
C     (POUR LA GRANDEUR VARI_* , IL FAUT CONSTITUER :(V1,V2,...,VN))
   ...
   25    CONTINUE
      ...
      DO 30 ICMP = 1 , NBCMP
         ...
         DO 36 INO=1,NBNO
            IF(ZL(JCESL+(INO-1)*NBCMP+ICMP-1)) THEN
              IF (VMAX.LT.ZR(JCESV+(INO-1)*NBCMP+ICMP-1)) THEN
                 VMAX=ZR(JCESV+(INO-1)*NBCMP+ICMP-1)
                 NMAX=INO
              ENDIF
              IF (VMIN.GT.ZR(JCESV+(INO-1)*NBCMP+ICMP-1)) THEN
                 VMIN=ZR(JCESV+(INO-1)*NBCMP+ICMP-1)
                 NMIN=INO
              ENDIF
            ENDIF
 36      CONTINUE
         ...
         NOMAX(2+2*(ICMP-1)+1)=KNMAX
         NOMAX(2+2*(ICMP-1)+2)=KNMIN

         NOMPAR(4+4*(ICMP-1)+1)='MAX_'//NOMCMP(ICMP)
         NOMPAR(4+4*(ICMP-1)+2)='NO_MAX_'//NOMCMP(ICMP)
         NOMPAR(4+4*(ICMP-1)+3)='MIN_'//NOMCMP(ICMP)
         NOMPAR(4+4*(ICMP-1)+4)='NO_MIN_'//NOMCMP(ICMP)

C ---    ON AJOUTE LES PARAMETRES MANQUANTS DANS LA TABLE:
         CALL TBEXIP(RESU,NOMPAR(4+4*(ICMP-1)+1),EXIST,K8B)
         IF(.NOT.EXIST)THEN
            CALL TBAJPA(RESU,1,NOMPAR(4+4*(ICMP-1)+1),'R' )
            CALL TBAJPA(RESU,1,NOMPAR(4+4*(ICMP-1)+2),'K16' )
            CALL TBAJPA(RESU,1,NOMPAR(4+4*(ICMP-1)+3),'R' )
            CALL TBAJPA(RESU,1,NOMPAR(4+4*(ICMP-1)+4),'K16' )
         ENDIF
