...
C     --- RECUPERATION DU RESULTAT ET DU NUMERO D'ORDRE
      ...
      CALL GETVID ( ' ', 'CHAM_GD'   ,1,1,1, CHAMG,  ND)
...
C         --- CALCUL ET STOCKAGE DES MOYENNES : MOT-CLE 'GROUP_MA'
          CALL GETVTX('INTEGRALE','GROUP_MA',IOCC,1,NZERO,K8B,N1)
          IF(N1.NE.0)THEN
            NBGMA=-N1
            CALL WKVECT('&&PEEINT_GMA','V V K8',NBGMA,JGMA)
            CALL GETVTX('INTEGRALE','GROUP_MA',IOCC,1,NBGMA,
     &                   ZK8(JGMA),N1)
            DO 20 IGM=1,NBGMA
...
C         --- CALCUL ET STOCKAGE DES MOYENNES : MOT-CLE 'MAILLE'
          CALL GETVTX('INTEGRALE','MAILLE',IOCC,1,NZERO,K8B,N1)
          IF(N1.NE.0)THEN
            NMA=-N1
            CALL WKVECT('&&PEEINT_MAIL','V V K8',NMA,JMA)
            CALL GETVTX('INTEGRALE','MAILLE',IOCC,1,NMA,
     &                   ZK8(JMA),N1)
            DO 30 IM=1,NMA
