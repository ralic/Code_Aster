...
C     ----- FIN COMMUNS NORMALISES  JEVEUX  ----------------------------
     ...
     +              RAIDE, NOMGR, NOMA, INTERP, PROLGD
      CHARACTER*16  NOMCMD, FILE , FORMAT, PARAX, PARAY, NOMCON
...
C
C        ON STOCKE LES NOMS DE FONCTIONS DE L'UTILISATEUR
         CALL JEEXIN(NOMCON,IRET)
         IF(IRET.NE.0) CALL JEDETR(NOMCON)
         CALL WKVECT(NOMCON,'V V K16',2,JNOMF)
...
C
         ...
         ELSEIF ( FORMAT .EQ. 'SEISME'   ) THEN
            CALL FOECFD(NOMCON,NOMFON,IUL,IND,LISTR)
