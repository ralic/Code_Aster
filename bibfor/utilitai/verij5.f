...
C     ------------------------------
        ...
        CALL VERIJ2('O','MAILLAGE',ZK8(JNOMA-1+1) (1:8),M80,NVU,NOBJ)
...
C     ------------------------------
        CH8 = NOMSD
        CALL VERIJ2('O','LIGREL',CH8//'.MODELE',M80,NVU,NOBJ)
...
C     ------------------------------
        ...
        IF (TYCH.EQ.'NOEU') THEN
          CALL VERIJ2('O','CHAM_NO',CH19,M80,NVU,NOBJ)

        ELSEIF (TYCH.EQ.'CART') THEN
          CALL VERIJ2('O','CARTE',CH19,M80,NVU,NOBJ)

        ELSEIF (TYCH(1:2).EQ.'EL') THEN
          CALL VERIJ2('O','CHAM_ELEM',CH19,M80,NVU,NOBJ)

        ELSEIF (TYCH.EQ.'RESL') THEN
          CALL VERIJ2('O','RESUELEM',CH19,M80,NVU,NOBJ)

        ELSEIF (TYCH.EQ.'VGEN') THEN
          CALL VERIJ2('O','CHAM_GENE',CH19,M80,NVU,NOBJ)
...
C     ------------------------------
        ...
        CALL VERIJ2('O','MAILLAGE',ZK24(JREFE-1+1),M80,NVU,NOBJ)
C       -- SI CHAM_NO "CONSTANT" : PAS DE PROF_CHNO:
        IF (ZI(JDESC-1+2).EQ.0) THEN
          CALL VERIJ2('O','PROF_CHNO',ZK24(JREFE-1+2),M80,NVU,NOBJ)
        ENDIF
...
C         -- PARFOIS, CERTAINS RESUELEM N'EXISTENT PAS (ADLV100A)
          ...
          CALL VERIJ2('O','RESUELEM',CHAMP,M80,NVU,NOBJ)
   10   CONTINUE
   20   CONTINUE
...
C         -- PARFOIS ON CREE DES FONCTIONS EN SOUS-TERRAIN :
          ...
          DO 30,K2 = 1,N2
            ...
            CALL VERIJ2('O','FONCTION',ZK24(J2-1+K2),M80,NVU,NOBJ)
   30     CONTINUE
   40     CONTINUE
...
C       -- MATR_ASSE OU MATR_ASSE_GENE ? :
        ...
        IF (ZK24(JREFA-1+10).EQ.'NOEU') THEN
          CALL VERIJ2('O','MATR_ASSE_GD',CH19,M80,NVU,NOBJ)

        ELSEIF (ZK24(JREFA-1+10).EQ.'GENE') THEN
          CALL VERIJ2('O','MATR_ASSE_GENE',CH19,M80,NVU,NOBJ)
...
C     -----------------------------------
        ...
        CALL VERIJ2('O','MAILLAGE',ZK24(JREFA-1+1),M80,NVU,NOBJ)
        CALL VERIJ2('O','NUME_DDL',ZK24(JREFA-1+2),M80,NVU,NOBJ)
     ...
     &              (ZK24(JREFA-1+3).EQ.'ELIMF'))
        ...
        IF (ZK24(JREFA-1+7).NE.' ') THEN
          CALL VERIJ2('O','SOLVEUR',ZK24(JREFA-1+7),M80,NVU,NOBJ)
        ENDIF
...
C       CALL VERIJ2('O','????',ZK24(JREFA-1+1),M80,NVU,NOBJ)
        CALL VERIJ2('O','NUME_DDL',ZK24(JREFA-1+2),M80,NVU,NOBJ)
...
C     -----------------------------------
        CH19 = NOMSD
        CALL VERIJ2('O','PROF_CHNO',CH19,M80,NVU,NOBJ)


      ELSEIF (TYP2SD.EQ.'NUME_EQGE') THEN
C     -----------------------------------
        CH19 = NOMSD
        CALL VERIJ2('O','PROF_VGEN',CH19,M80,NVU,NOBJ)
...
C     -----------------------------------
        ...
        CALL VERIJ2('O','SOLVEUR',ZK24(JNSLV),M80,NVU,NOBJ)
C       -- SI FETI, IL N'Y A PAS DE STOCKAGE :
        CALL JEVEUO(ZK24(JNSLV) (1:19)//'.SLVK','L',JSLVK)
        IF (ZK24(JSLVK).NE.'FETI') CALL VERIJ2('O','STOCKAGE',CH14,M80,
     &      NVU,NOBJ)
...
C       IL ARRIVE (CALC_MATR_AJOU) QUE NUME_EQGE SOIT VIDE :
        ...
        IF (IEXI.EQ.0) THEN
          CALL VERIJ2('O','NUME_EQUA',CH14//'.NUME',M80,NVU,NOBJ)

        ELSE
          CALL VERIJ2('O','NUME_EQGE',CH14//'.NUME',M80,NVU,NOBJ)
        ENDIF
   70   CONTINUE
...
C     -----------------------------------
        ...
        CALL ASSERT(I1.GT.0)
        CALL VERIJ2('O','STOC_MORSE',CH14//'.SMOS',M80,NVU,NOBJ)
        IF (I2.GT.0) CALL VERIJ2('O','STOC_LCIEL',CH14//'.SLCS',M80,NVU,
     &                           NOBJ)
        IF (I3.GT.0) CALL VERIJ2('O','STOC_MLTF',CH14//'.MLTF',M80,NVU,
     &                           NOBJ)
