...
C           LES PARAMETRES NE SONT PAS CROISSANTS (SENS LARGE)
            CALL ORDONP(NOMFON)
            CALL UTMESS('A',NOMCMD,'LES PARAMETRES DE LA NAPPE'
     +      //' ONT ETE REORDONNEES.')
         ENDIF
...
C     --------------------------------------------
      ELSE
         CALL UTMESS('F',NOMCMD,'TYPE DE FONCTION NON CONNU (ORDONN)')
      ENDIF
C
 999  CONTINUE
      IF(IMES.EQ.4)THEN
         CALL UTDEBM('F',NOMCMD,'LES ABSCISSES '//MESS)
         CALL UTIMPK('S',' ',1,NOMFON)
         CALL UTIMPK('L','NE SONT PAS MONOTONES.',0,CHBID)
         CALL UTFINM()
      ELSEIF(IMES.EQ.2)THEN
         CALL UTDEBM('A',NOMCMD,'LES ABSCISSES '//MESS)
         CALL UTIMPK('S',' ',1,NOMFON)
         CALL UTIMPK('L','ONT ETE REORDONNEES.',0,CHBID)
         CALL UTFINM()
      ELSEIF(IMES.EQ.1)THEN
         CALL UTDEBM('A',NOMCMD,'L ORDRE DES ABSCISSES '//MESS)
         CALL UTIMPK('S',' ',1,NOMFON)
