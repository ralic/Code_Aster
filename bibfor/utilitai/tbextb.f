...
C
      IF ( BASE.NE.'V' .AND. BASE.NE.'G' ) THEN
         CALL UTMESS('F','TBEXTB','TYPE BASE INCONNU :'//BASE)
      ENDIF
C
...
C
      ...
      IF ( IRET .EQ. 0 ) THEN
         CALL UTMESS('F','TBEXTB','LA TABLE N''EXISTE PAS') 
      ENDIF
C
      ...
      NBLIGN = ZI(JTBNP+1)
      IF ( NBPARA .EQ. 0 ) THEN
         CALL UTMESS('F','TBEXTB','PAS DE PARAMETRES DEFINIS') 
      ENDIF
      IF ( NBLIGN .EQ. 0 ) THEN
         CALL UTMESS('F','TBEXTB','PAS DE LIGNES DEFINIS') 
      ENDIF
C
