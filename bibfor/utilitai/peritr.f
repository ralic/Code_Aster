...
C     -----  FIN  COMMUNS NORMALISES  JEVEUX  --------------------------
      ...
      INTEGER NUMOR,IORD,IAINST,IBID,LVALE,NUMON,NBIN,IOCC,NT,NM,NC
      INTEGER NG,KK,NBGRMA,JGR,IG,NBMA,JAD,NBMAIL,JMA,IM,NUME
     ...
     &             NOPARR(NBPARR),NOPARD(NBPARD)
      CHARACTER*19 CHELEM,KNUM,KINS,VARNUL
      CHARACTER*24 CHGEOM,CHCARA(15),CHHARM,LIGREL,LCHIN(7)
...
C --- RECUPERATION DU NIVEAU D'IMPRESSION
      ...
      INST = 0.D0
      CALL GETVID(' ','CHAM_GD',1,1,1,CONTG,ND)
      CALL GETVID(' ','RESULTAT',1,1,1,RESUL,NR)
      CALL GETVR8(' ','INST',1,1,1,INST,NI)
      CALL GETVTX('RICE_TRACEY','OPTION',1,1,1,OPTCAL(1),NP)
      CALL GETVTX('RICE_TRACEY','LOCAL',1,1,1,OPTCAL(2),NQ)
      IF (NBOCC.GT.1) THEN
        DO 10 I = 2,NBOCC
...
C        --- IMPRESSION DU CHAMELEM ---
        ...
        DO 80 IOCC = 1,NBOCC
          ...
          CALL GETVEM(NOMA,'MAILLE',OPTION(1:11),'MAILLE',IOCC,1,0,K8B,
     &                NM)
          CALL GETVEM(NOMA,'GROUP_MA',OPTION(1:11),'GROUP_MA',IOCC,1,0,
     &                K8B,NG)
   ...
   30         CONTINUE
     ...
     &                 EXP(1.5D0*ABS(TRIAX))*ZR(LVALE+4)
          ...
          IF (NG.NE.0) THEN
            ...
            CALL GETVEM(NOMA,'GROUP_MA',OPTION(1:11),'GROUP_MA',IOCC,1,
     &                  NBGRMA,ZK8(JGR),NG)
            DO 50 IG = 1,NBGRMA
              NOMMAI = ZK8(JGR+IG-1)
   ...
   50       CONTINUE
          ...
          IF (NM.NE.0) THEN
            ...
            CALL GETVEM(NOMA,'MAILLE',OPTION(1:11),'MAILLE',IOCC,1,
     &                  NBMAIL,ZK8(JMA),NM)
            DO 70 IM = 1,NBMAIL
              NOMMAI = ZK8(JMA+IM-1)
