...
C
        ...
        IF (NBINS.NE.NBOLD) THEN
           CALL UTMESS('F','OP0197','IL EXISTE DES DOUBLONS DANS LA '
     +          //'LISTE D''INSTANTS DE RUPTURE')
        END IF        
        ZI(IRENT+IRESU-1) = NBINS
        NRUPT = NRUPT + NBINS  
        CALL JEECRA(JEXNUM(COLLEC,IRESU),'LONUTI',NBINS,' ')
        IF (NBINS.LE.1) THEN
           CALL UTMESS('F','OP0197','IL FAUT DONNER PLUS D''UN '
     +          //'INSTANT DE RUPTURE')
        END IF
C        
...
C
       IF (NTEMP.NE.NBRESU) THEN
         CALL UTMESS('F','OP0197','IL MANQUE DES TEMPERATURES '
     +               // ' ASSOCIEES AUX BASES DE RESULTATS (MOT-CLE'
     +               // ' TEMPE)')
       ELSE         
         IF (NBRESU.GT.1) THEN
           DEPT = .FALSE.
...
C
     ...
     +                    'LONMAX',NBMTCM,K8BID)
        CALL WKVECT('&&OP0197.L_NOM_MAT','V V K8',NBMTCM,ANOMMT)
        CALL CHMRCK(ZK8(ICHMAT-1+IRESU),NOMRC,NBMTCM,ZK8(ANOMMT),NBMTRC)
        ZK24(IKVAK-1+IRESU)(1:8)  = ZK8(ANOMMT)
        ZK24(IKVAK-1+IRESU)(9:24) = '.WEIBULL   .VALK'
        ZK24(IKVAL-1+IRESU)(1:8)  = ZK8(ANOMMT)
...
117     CONTINUE
        ...
        IF (IRESU.GT.1) THEN
          IF (MINI.NE.MINIP) THEN
           CALL UTMESS('F','OP0197','LE PARAMETRE M DE WEIBULL DOIT'
     +           //' ETRE LE MEME POUR TOUTES LES BASES RESULTATS !')
          END IF
          IF (ZR(ISIGI-1+IRESU).NE.ZR(ISIGI-2+IRESU)) THEN
           CALL UTMESS('F','OP0197','LE PARAMETRE SIGM_REFE DE WEIBULL' 
     +         //' DOIT ETRE LE MEME POUR TOUTES LES BASES RESULTATS !')
          END IF
        END IF
        MINIP = MINI
C
115   CONTINUE 
      ...
      CALL UTFINM
C
      CALL WKVECT('&&OP0197.NOM_TABLPE','V V K8',NBRESU,ITABW)
      CALL WKVECT('&&OP0197.NOM_TABLIN','V V K8',NBRESU,ITABR)         
...
C
        CALL COPISD(' ','G',ZK8(ICHMAT-1+IRESU),CHCOP1)
        CALL JEVEUO(CHCOP1//'.CHAMP_MAT .VALE','E',ICHCO)
        DO 301 I=1,NBMTCM
          IF (ZK8(ICHCO+I-1).EQ.ZK8(ANOMMT)) THEN
            CALL COPISD(' ','G',ZK8(ANOMMT),CHCOP2)
...
C
        CALL GCNCON ( '.' , ZK8(ITABW-1+IRESU) )
        CALL GCNCON ( '.' , ZK8(ITABR-1+IRESU) )
C
        NCHAR = 0
...
C
        CALL PEWEIB(ZK8(ITABW-1+IRESU),ZK8(IMOD-1+IRESU),MATE,
...
C
        CALL TBEXVE(ZK8(ITABW-1+IRESU),'SIGMA_WEIBULL',
     +                  '&&OP0197.NOM_VECSIG','V',NBVAL,K8BID)
        CALL TBEXVE(ZK8(ITABW-1+IRESU),'PROBA_WEIBULL',
     +                  '&&OP0197.NOM_VECPRO','V',NBVAL,K8BID) 
        CALL TBEXVE(ZK8(ITABW-1+IRESU),'INST',
...
C
        CALL TBCRSD(ZK8(ITABR-1+IRESU),'G')
        CALL TBAJPA (ZK8(ITABR-1+IRESU), NBPARR, NOPARR, TYPARR )
C 
     ...
     +                          ' / TEMPS = ',ZR(IINST+IT-1)
         ...
         VALR(2) = SIGINT
         VALR(3) = PROINT
         CALL TBAJLI (ZK8(ITABR-1+IRESU), NBPARR, NOPARR,
...
C 
        CALL JEDETC('G',CHCOP1,1)     
        CALL JEDETC('G',CHCOP2,1)        
C          
300   CONTINUE
C         
...
C
      CALL TBFUTB ( TABOUT, 'G', NBRESU, ZK8(ITABR),' ',' ',
...
C  
      ...
      IF (NBITE.EQ.NITMAX) THEN
        CALL UTMESS('F','OP0197','NOMBRE MAX D''ITERATIONS ATTEINT')
      END IF
C
