...
C MODIF UTILITAI  DATE 26/02/2013   AUTEUR LADIER A.LADIER 
C ======================================================================
...
C TOLE CRP_20
      ...
      CHARACTER*19 KNUM,KINS,LISINS,CHAM2,CHAMTM,CELMOD,LIGREL,LISCOE
...
C     ==================================================
      ...
      CALL GETVR8( 'NORME', 'COEF_MULT' ,1,IARG,0, R8B, NCO)
...
C     -- VERIFICATION DE L'UTILISATION DE COEF_MULT
      IF (NR.EQ.0.AND.NCO.NE.0) THEN
          CALL DISMOI('C','NOM_GD',TMPCHA,'CHAMP',IBID,NOMGD,IRET)
...
C             (UTILE POUR LES TENSEURS) --
 ...
 23         CONTINUE
     ...
     &               ZK8(JLICM1+I-1).EQ.'SIYZ    ')THEN
              ...
              ENDIF
 24         CONTINUE
          ELSEIF(NOMGD(1:6).EQ.'NEUT_R')THEN
            IF (NCO.NE.0)THEN
             NBCOEF=-NCO
             CALL WKVECT(LISCOE,'V V R',NBCOEF,JNO)
             CALL GETVR8('NORME','COEF_MULT',1,IARG,NBCOEF,ZR(JNO),IRET)
             DO 25 I=1,NBCOEF
