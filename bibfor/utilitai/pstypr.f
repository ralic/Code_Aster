...
C 0.1. ==> ARGUMENTS
C
      INTEGER NBPASE, NBMOCL
C
      CHARACTER*(*) NOPASE, TYPEPS, LIMOCL, LIVALE
      CHARACTER*(*) INPSCO
C
C 0.2. ==> COMMUNS
C
...
C
      ...
      INTEGER ADMOCL, ADMOMM, ADVALE, ADVAVV, NBMCMX, NBMCPA
      INTEGER LGTYPS
...
C            ON VERIFIE QUE C'EST TOUJOURS LE MEME TYPE SI LA
C            SENSIBILITE EST ISSUE DE PLUSIEURS CAUSES. PAR EXEMPLE, UNE
C            DONNEE SENSIBLE DANS PLUSIEURS CHARGEMENTS. IL FAUT QUE CE
C            SOIT TOUJOURS LE MEME TYPE.
C
        ADTSKK = ADTSKK + 1
     ...
     >      'ON A '//SAUX80(1:LGTYPS)//' ET '//ZK80(ADTSKK)(1:JAUX))
        ...
        ENDIF
C
C 2.3.2. ==> LE NOMBRE DE MOTS-CLES POUR CETTE CONTRIBUTION
C
        ADTSII = ADTSII + 1
        NBMCPA = ZI(ADTSII+IAUX-1)
C
C 2.3.3. ==> LES MOTS-CLES ASSOCIES
C 2.3.3.1. ==> ALLOCATION DES STRUCTURES
...
C
          ...
          NBMCMX = 5*NBMOCL + 1
C
          CALL WKVECT ( LIMOCL,'V V K80',NBMCMX,ADMOCL )
...
C
        ELSE
C
          NBMOCL = NBMOCL + NBMCPA
          IF ( NBMOCL.GT.NBMCMX ) THEN
            NBMCMX = 2*NBMCMX
            CALL JUVECA ( LIMOCL,NBMCMX )
            CALL JEVEUO ( LIMOCL, 'E', ADMOCL )
            ADMOMM = ADMOCL + NBMOCL - NBMCPA - 1
