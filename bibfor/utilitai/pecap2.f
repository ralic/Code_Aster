...
C     ---------------------------------------------------
      ...
      IF (TYPRE1.NE.'EVOL_THER') THEN
        CALL UTMESS('F','PECAP2','ON DOIT DONNER UN RESULTAT '//
     &              'DE TYPE "EVOL_THER" APRES LE MOT-CLE '//
     &              '"LAPL_PHI_Y"  DU MOT-FACTEUR "CARA_POUTRE"'//
     &              ' DANS LA COMMANDE POST_ELEM '//
     &              'POUR CALCULER LES COEFFICIENTS DE CISAILLEMENT'//
     &              ' ET LES COORDONNEES DU CENTRE DE TORSION.')
      END IF
      ...
      IF (TYPRE2.NE.'EVOL_THER') THEN
        CALL UTMESS('F','PECAP2','ON DOIT DONNER UN RESULTAT '//
     &              'DE TYPE "EVOL_THER" APRES LE MOT-CLE '//
     &              '"LAPL_PHI_Z"  DU MOT-FACTEUR "CARA_POUTRE"'//
     &              ' DANS LA COMMANDE POST_ELEM '//
     &              'POUR CALCULER LES COEFFICIENTS DE CISAILLEMENT'//
     &              ' ET LES COORDONNEES DU CENTRE DE TORSION.')
      END IF
...
C     ---------------------------------------------
      ...
      IF (NBORDR.NE.1) THEN
        CALL UTMESS('F','PECAP2','LE NOMBRE D''ORDRES DU '//
     &              'RESULTAT '//TEMPE1//' NECESSAIRE POUR '//
     &              'CALCULER LES COEFFICIENTS DE CISAILLEMENT'//
     &              ' ET LES COORDONNEES DU CENTRE DE TORSION '//
     &              'DOIT ETRE EGAL A 1.')
      END IF
...
C     -----------------------------------------------------
      ...
      IF (IRET.GT.0) THEN
        CALL UTMESS('F','PECAP2','ON N''ARRIVE PAS A RECUPERER'//
     &              ' LE CHAMP DE TEMPERATURES DU RESULTAT '//TEMPE1)
      END IF

      CALL RSEXCH(TEMPE2,'TEMP',0,CHTEM2,IRET)
      IF (IRET.GT.0) THEN
        CALL UTMESS('F','PECAP2','ON N''ARRIVE PAS A RECUPERER'//
     &              ' LE CHAMP DE TEMPERATURES DU RESULTAT '//TEMPE2)
      END IF

C --- RECUPERATION DU NUME_DDL ASSOCIE AU CHAMP DE TEMPERATURES :
C     ---------------------------------------------------------
      CALL DISMOI('F','NOM_NUME_DDL',CHTEM1,'CHAM_NO',IBID,NUMDDL,IERD)
