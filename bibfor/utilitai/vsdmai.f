...
C ----------------------------------------------------------------------
C  BUT : FAIRE UN ENSEMBLE DE VERIFICATIONS SUR UNE SD MAILLAGE
...
C -DEB------------------------------------------------------------------
      ...
      MA = NOMSD
      CALL VERIOB(MA//'.DIME','EXIS',0)
      CALL VERIOB(MA//'.NOMNOE','EXIS',0)
C     -- JP: ON NE PEUT PAS VERIFIER .COORDO
C        CAR CELA ENTRAINE UNE BOUCLE INFINIE :
C        CHAM_NO -> MAILLAGE -> CHAM_NO ...
C     CALL VERIS2('CHAM_NO',MA//'.COORDO')
      CALL JEEXIN(MA//'.NOMMAI',I1)
      CALL JEEXIN(MA//'.NOMACR',I2)
      CALL ASSERT(I1.GT.0 .OR. I2.GT.0)
      IF (I1.GT.0) THEN
         CALL VERIOB(MA//'.NOMMAI','EXIS',0)
         CALL VERIOB(MA//'.CONNEX','EXIS',0)
         CALL VERIOB(MA//'.TYPMAIL','EXIS',0)
      ENDIF
      IF (I2.GT.0) THEN
         CALL VERIOB(MA//'.NOMACR','EXIS',0)
         CALL VERIOB(MA//'.PARA_R','EXIS',0)
         CALL VERIOB(MA//'.SUPMAIL','EXIS',0)
      ENDIF
...
C     -- VERIFICATION SSS :
      ...
      IF (NBSM.GT.0) THEN
         ...
         CALL JELIRA(MA//'.SUPMAIL','LONMAX',N1,KBID)
         CALL ASSERT(N1.EQ.NBSM)
         CALL JELIRA(MA//'.NOMACR','NOMUTI',N1,KBID)
         CALL ASSERT(N1.EQ.NBSM)
      ENDIF
      IF (NBNL.GT.0) THEN
