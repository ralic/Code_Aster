...
C
     ...
     &                ,IRET,I,LPRTMP,LVATMP
      CHARACTER*1     K1BID
      CHARACTER*8     NOPAR1,NORES1,NOPAR2,NORES2,NOMF1,NOMF2
      CHARACTER*19    FONTMP
...
C
      ...
      ZK8(LPROS  ) = 'FONCTION'
      ZK8(LPROS+1) = ZK8(LPRO1+1)
      ZK8(LPROS+2) = NOPAR2
      ZK8(LPROS+3) = NORES1
      ZK8(LPROS+4) = ZK8(LPRO1+4)
C
...
C     FONCTION TEMPORAIRE QUI CONTIENT t(X)
      ...
      CALL WKVECT(FONTMP//'.PROL',BASE//' V K8',5,LPRTMP)
      ZK8(LPRTMP-1+1) = ZK8(LPRO2-1+1)
      ZK8(LPRTMP-1+2) = ZK8(LPRO2-1+2)
      ZK8(LPRTMP-1+3) = ZK8(LPRO2-1+3)
      ZK8(LPRTMP-1+4) = ZK8(LPRO2-1+4)
      ZK8(LPRTMP-1+5) = ZK8(LPRO2-1+5)
C
      DO 10 I = 0 , NBINST-1
...
C     REORDONNE LES VALEURS DE X
      CALL UTTRIF(ZR(LVATMP),NBINST,ZK8(LPRTMP))
C
      DO 11 I = 0 , NBINST-1
         ZR(LRES+I) = ZR(LVATMP+NBINST+I)
 11   CONTINUE
C
      CALL FOINTR(NOMFO1,ZK8(LPRO1),NBPT1,ZR(LVAR1),ZR(LVAR1+NBPT1),
