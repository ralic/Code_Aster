...
C        VERIF QUE LES PARAMETRES SONT STRICT CROISSANTS
         IF(VERIF.EQ.'CROISSANT')THEN
           ...
           IF(IRET.NE.2) CALL UTMESS('F',NOMCMD,'STOP')
         ENDIF
         CALL JEDETR('&&OP0003.TEMP.PARA')
         CALL JEDETR('&&OP0003.TEMP.PAR2')
...
C
         ...
         IF ( NBLV .NE. NBLN) THEN
           CALL UTMESS('F',NOMCMD, 'NOMBRE DE VALEURS DIFFERENT '//
...
C     A VALEURS COMPLEXES, MAIS AUTANT LE LAISSER POSSIBLE
     ...
     +.OR.(INTERP(2).EQ.'LOG'.AND.(MIN2.LE.0.D0.OR.MIN3.LE.0.D0))) THEN
         CALL UTMESS('F',NOMCMD,'INTERPOLATION "LOG" ET VALEURS '//
     +                          'NEGATIVES SONT INCOMPATIBLES !')
      ENDIF
C
C     --- MENAGE ---
      CALL JEDETC('V','&&OP0003',1)
C
