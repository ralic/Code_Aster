...
C
      ...
      CALL GETVR8(' ','VALE'      ,0,IARG,0,R8BID,N2)
      CALL GETVR8(' ','VALE_C'    ,0,IARG,0,R8BID,N3)
      CALL GETVR8(' ','ABSCISSE'  ,0,IARG,0,R8BID,NBLA)

      CALL GETVTX(' ','NOEUD_PARA',0,IARG,0,K8B  ,NBLN)
      CALL GETVID(' ','VALE_PARA' ,0,IARG,0,K8B  ,NBLR)
      CALL GETVR8(' ','VALE_Y'    ,0,IARG,0,R8BID,NBLV)
      NBVR = ABS(N2)
      NBVC = ABS(N3)
      NBLA = ABS(NBLA)
      NBLN = ABS(NBLN)
...
C
      IF ( NBVC.NE.0 ) THEN
         ...
         CALL GETVR8(' ','VALE_C',0,IARG,NBVC,ZR(LPARA),L)
         DO 2 I = 0,NBCOUP-1
...
C        VERIF QUE LES PARAMETRES SONT STRICT CROISSANTS
      ...
      ELSEIF ( NBVR.NE.0 .AND. NBLN.EQ.0 ) THEN
         ...
         CALL WKVECT('&&OP0003.TEMP.PAR2','V V R',NBCOUP,LPAR2)
         CALL GETVR8(' ','VALE',0,IARG,NBVR,ZR(LPARA),L)
         DO 4 I = 0,NBCOUP-1
...
C        VERIF QUE LES PARAMETRES SONT STRICT CROISSANTS
      ...
      ENDIF
C
      CALL GETVTX(' ','NOM_PARA'   ,0,IARG,1,NOMPAR,N1)
      CALL GETVTX(' ','NOM_RESU'   ,0,IARG,1,NOMRES,N2)
      CALL GETVTX(' ','INTERPOL'   ,0,IARG,2,INTERP,N3)
      IF ( N3 .EQ. 1 ) INTERP(2) = INTERP(1)
      CALL GETVTX(' ','PROL_GAUCHE',0,IARG,1,PROLGD(1:1),N4)
      CALL GETVTX(' ','PROL_DROITE',0,IARG,1,PROLGD(2:2),N5)
...
C
      IF ( NBLN .NE. 0 ) THEN
         CALL GETVID(' ','MAILLAGE',0,IARG,1,NOMMAI,IBID)
...
C
         ...
         ENDIF
         CALL GETVR8(' ','VALE_Y',0,IARG,NBLV,ZR(JVAL) ,N)
         CALL GETVEM(NOMMAI,'NOEUD',' ','NOEUD_PARA',
     &         0,IARG,NBLV,ZK8(JNOE),N)
         NBVAL = 2*(NBRMA+1)
...
C
 ...
 09      CONTINUE
      ELSEIF ( NBVC.NE.0 ) THEN
         ...
         CALL GETVR8(' ','VALE_C',0,IARG,NBVC,ZR(JVAL),N)
...
C
      ELSEIF ( NBVR.NE.0 .AND. NBLN.EQ.0 ) THEN
         ...
         CALL GETVR8(' ','VALE',0,IARG,NBVR,ZR(JVAL),N)
...
C
      ELSEIF ( NBLR.NE.0 ) THEN
         CALL GETVID(' ','VALE_PARA',1,IARG,1,LISTPA,N1)
         CALL GETVID(' ','VALE_FONC',1,IARG,1,LISTFO,N1)
 ...
 30      CONTINUE
      ELSEIF ( NBLA.NE.0 ) THEN
         ...
         CALL GETVR8(' ','ABSCISSE',0,IARG,NBLA,ZR(LVAL),N)
         CALL GETVR8(' ','ORDONNEE',0,IARG,NBLA,ZR(LVAL+NBLA),N)
C
      ENDIF
C
