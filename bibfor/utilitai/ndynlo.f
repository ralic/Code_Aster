      LOGICAL FUNCTION NDYNLO(SDDYNA,CHAINE)
C      
C            CONFIGURATION MANAGEMENT OF EDF VERSION
C MODIF UTILITAI  DATE 04/04/2007   AUTEUR ABBAS M.ABBAS 
C ======================================================================
C COPYRIGHT (C) 1991 - 2007  EDF R&D                  WWW.CODE-ASTER.ORG
C THIS PROGRAM IS FREE SOFTWARE; YOU CAN REDISTRIBUTE IT AND/OR MODIFY  
C IT UNDER THE TERMS OF THE GNU GENERAL PUBLIC LICENSE AS PUBLISHED BY  
C THE FREE SOFTWARE FOUNDATION; EITHER VERSION 2 OF THE LICENSE, OR     
C (AT YOUR OPTION) ANY LATER VERSION.                                   
C                                                                       
C THIS PROGRAM IS DISTRIBUTED IN THE HOPE THAT IT WILL BE USEFUL, BUT   
C WITHOUT ANY WARRANTY; WITHOUT EVEN THE IMPLIED WARRANTY OF            
C MERCHANTABILITY OR FITNESS FOR A PARTICULAR PURPOSE. SEE THE GNU      
C GENERAL PUBLIC LICENSE FOR MORE DETAILS.                              
C                                                                       
C YOU SHOULD HAVE RECEIVED A COPY OF THE GNU GENERAL PUBLIC LICENSE     
C ALONG WITH THIS PROGRAM; IF NOT, WRITE TO EDF R&D CODE_ASTER,         
C   1 AVENUE DU GENERAL DE GAULLE, 92141 CLAMART CEDEX, FRANCE.         
C ======================================================================
C RESPONSABLE ABBAS M.ABBAS
C
      IMPLICIT NONE
      CHARACTER*19  SDDYNA
      CHARACTER*(*) CHAINE
C 
C ----------------------------------------------------------------------
C
C ROUTINE MECA_NON_LINE (SDDYNA - UTILITAIRE)
C
C INTERROGE SDDYNA POUR RENVOYER UN BOOLEEN
C      
C ----------------------------------------------------------------------
C
C
C OUT NDYNLO : TRUE SI <CHAINE> FAIT PARTIE DES PROPRIETES DE LA SD
C               FALSE SINON
C IN  SDDYNA : NOM DE LA SD DEDIEE A LA DYNAMIQUE
C IN  CHAINE : PROPRIETE EVENTUELLE DE LA SD DYNAC
C                    = 'DIFF_CENT','TCHAMWA','NEWMARK','THETA_METHODE',
C                      'HHT_NON_MODIFIE','HHT_MODIFIE',
C                      'IMPLICITE', 'EXPLICITE',
C                       'STATIQUE', 'DYNAMQIUE'
C                      'MAT_AMORT','MASS_DIAG',
C                      'MULTI_APPUI','AMOR_MODAL','PROJ_MODAL',
C
C -------------- DEBUT DECLARATIONS NORMALISEES  JEVEUX ----------------
C      
      INTEGER ZI
      COMMON /IVARJE/ZI(1)
      REAL*8 ZR
      COMMON /RVARJE/ZR(1)
      COMPLEX*16 ZC
      COMMON /CVARJE/ZC(1)
      LOGICAL ZL
      COMMON /LVARJE/ZL(1)
      CHARACTER*8 ZK8
      CHARACTER*16 ZK16
      CHARACTER*24 ZK24
      CHARACTER*32 ZK32
      CHARACTER*80 ZK80
      COMMON /KVARJE/ZK8(1),ZK16(1),ZK24(1),ZK32(1),ZK80(1)
C
C -------------- FIN  DECLARATIONS  NORMALISEES  JEVEUX ----------------
C
      CHARACTER*24 TSCH,LOSD
      INTEGER      JTYSCH,JISD
      CHARACTER*19 VALK(1)
C
C ----------------------------------------------------------------------
C
      CALL JEMARQ()
C
C --- INITIALISATIONS
C
      NDYNLO = .FALSE.
      TSCH   = SDDYNA(1:15)//'.TYPE_SCH'
      LOSD   = SDDYNA(1:15)//'.INFO_SD'      
C      
      CALL JEVEUO(TSCH,'L',JTYSCH)
C  
      IF (ZK16(JTYSCH+1-1)(1:8).EQ.'STATIQUE') THEN
        IF (CHAINE(1:8).EQ.'STATIQUE') THEN
          NDYNLO = .TRUE.
        ELSE
          NDYNLO = .FALSE.           
        ENDIF 
        GOTO 9999
      ELSE
        IF (CHAINE(1:9).EQ.'DYNAMIQUE') THEN
          NDYNLO = .TRUE.
          GOTO 9999          
        ENDIF 
      ENDIF
C
      CALL JEVEUO(LOSD,'L',JISD)
C
      IF (CHAINE(1:8).EQ.'STATIQUE')THEN
        NDYNLO =.FALSE.
      ELSEIF(CHAINE(1:9).EQ.'DIFF_CENT')THEN
        IF(ZK16(JTYSCH+7-1)(1:12).EQ.'DIFF_CENTREE')NDYNLO=.TRUE.
      ELSE IF(CHAINE(1:7).EQ.'TCHAMWA')THEN
        IF(ZK16(JTYSCH+8-1)(1:7).EQ.'TCHAMWA')NDYNLO=.TRUE.
      ELSE IF(CHAINE(1:7).EQ.'NEWMARK')THEN
        IF(ZK16(JTYSCH+2-1)(1:7).EQ.'NEWMARK')NDYNLO=.TRUE.
      ELSE IF(CHAINE(1:13).EQ.'THETA_METHODE')THEN
        IF(ZK16(JTYSCH+4-1)(1:13).EQ.'THETA_METHODE')NDYNLO=.TRUE.
      ELSE IF(CHAINE(1:11).EQ.'HHT_COMPLET')THEN
        IF(ZK16(JTYSCH+5-1)(1:11).EQ.'HHT_COMPLET')NDYNLO=.TRUE.
      ELSE IF(CHAINE(1:3).EQ.'HHT')THEN
        IF(ZK16(JTYSCH+3-1)(1:3).EQ.'HHT')NDYNLO=.TRUE.
      ELSE IF(CHAINE(1:9).EQ.'IMPLICITE')THEN
          IF(ZK16(JTYSCH+2-1)(1:7) .EQ.'NEWMARK'       .OR.
     &       ZK16(JTYSCH+4-1)(1:13).EQ.'THETA_METHODE' .OR.
     &       ZK16(JTYSCH+5-1)(1:11).EQ.'HHT_COMPLET'   .OR.
     &       ZK16(JTYSCH+3-1)(1:3).EQ.'HHT') NDYNLO=.TRUE.
      ELSE IF(CHAINE(1:9).EQ.'EXPLICITE')THEN
           IF(ZK16(JTYSCH+7-1)(1:12).EQ.'DIFF_CENTREE' .OR.
     &        ZK16(JTYSCH+8-1)(1:7) .EQ.'TCHAMWA')NDYNLO=.TRUE.
      ELSE IF(CHAINE(1:9).EQ.'MAT_AMORT')THEN 
          NDYNLO=ZL(JISD+1-1)
      ELSE IF(CHAINE(1:11).EQ.'MULTI_APPUI')THEN
          NDYNLO=ZL(JISD+2-1)
      ELSE IF(CHAINE(1:10).EQ.'AMOR_MODAL')THEN
          NDYNLO=ZL(JISD+3-1)
      ELSE IF(CHAINE(1:9).EQ.'MASS_DIAG')THEN
          NDYNLO=ZL(JISD+4-1)
      ELSE IF(CHAINE(1:10).EQ.'PROJ_MODAL')THEN
          NDYNLO=ZL(JISD+5-1)
      ELSE
        VALK(1)=CHAINE
        CALL U2MESK('F','UTILITAI7_7',1,VALK)
      ENDIF
C
9999  CONTINUE
C
      CALL JEDEMA()

      END
