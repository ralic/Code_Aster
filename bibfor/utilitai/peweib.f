...
C     -----  FIN  COMMUNS NORMALISES  JEVEUX  --------------------------
      ...
      INTEGER NBMAIL,JMA,IM,NUME,IMC
...
C     --- VERIF D'HOMOGENEITE WEIBULL ---
      ...
      NOMRC = 'WEIBULL         '
      CALL CHMRCK(CHMAT,NOMRC,NBMTCM,ZK8(ANOMMT),NBMTRC)
      IF (NBMTRC.GT.1) THEN
        CALL UTDEBM('A','PEWEIB',
     &              'HOMOGENEITE DU CHAMP DE MATERIAUX POUR WEIBULL')
        CALL UTIMPI('L','NOMBRE DE RC WEIBULL TROUVEES = ',1,NBMTRC)
        CALL UTIMPK('L','LES CALCULS SONT VALABLES POUR '//
     &              'UN SEUL COMPORTEMENT WEIBULL',0,K8B)
        CALL UTIMPK('L',
     &              'ON CHOISIT LA PREMIERE RELATION DU TYPE WEIBULL',0,
     &              K8B)
        CALL UTFINM
      END IF
...
C CAS WEIBULL_FO
   ...
   40   CONTINUE
        CALL UTDEBM('I','PEWEIB','PARAMETRES DE LA RC WEIBULL_FO')
        CALL UTIMPR('L','EXPOSANT DE LA LOI      =',1,MREF)
        CALL UTIMPR('L','VOLUME DE REFERENCE     =',1,VREF)
        CALL UTIMPR('L',
...
C CAS WEIBULL
      ELSE
        CALL UTDEBM('I','PEWEIB','PARAMETRES DE LA RC WEIBULL')
        CALL UTIMPR('L','EXPOSANT DE LA LOI      =',1,MREF)
        CALL UTIMPR('L','VOLUME DE REFERENCE     =',1,VREF)
        CALL UTIMPR('L','CONTRAINTE DE REFERENCE =',1,SREF)
        CALL UTFINM
      END IF
...
C  EN ENTREE : CHELEM DES SIGI MAX ATTEINTE AU COURS DU TEMPS
        ...
        LPAIN(7) = 'PDOMMAG'
C  EN ENTREE : CHELEM DES TEMPERATURES
        LCHIN(8) = CHTEMP
        LPAIN(8) = 'PTEMPER'
        LCHIN(9) = COMPOR
        LPAIN(9) = 'PCOMPOR'
