...
C
         ...
         IF(IRET.NE.0) THEN
            CALL UTMESS('F',NOMCMD,'PB LORS DE L''EXTRACTION DU CHAMP')
         ENDIF
...
C
         ...
         IF ( K16B(1:1) .NE. 'C') THEN
            CALL UTMESS('F',NOMCMD,'ON NE TRAITE QUE LE TYPE COMPLEXE')
         ENDIF
C
 ...
 110        CONTINUE
            CALL UTMESS('F',NOMCMD,'COMPOSANTE GENERALISEE NON TROUVEE')
 114        CONTINUE
            IM = I
         ELSE
            IM = NCMP
