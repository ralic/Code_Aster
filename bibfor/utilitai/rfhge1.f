...
C
      ...
      IF (IRET.NE.0) THEN
         CALL UTMESS('F','RFHGE1','PROBLEME(S) RENCONTRE(S) LORS DE '//
...
C
         ...
         IF(IRET.NE.0) THEN
          CALL UTMESS('F','RFHGE1','PB LORS DE L''EXTRACTION DU CHAMP')
         ENDIF
...
C
         ...
         IF ( K16B(1:1) .NE. 'C') THEN
          CALL UTMESS('F','RFHGE1','ON NE TRAITE QUE LE TYPE COMPLEXE')
         ENDIF
C
 ...
 110        CONTINUE
          CALL UTMESS('F','RFHGE1','COMPOSANTE GENERALISEE NON TROUVEE')
 114        CONTINUE
            IM = I
         ELSE
            IM = NCMP
