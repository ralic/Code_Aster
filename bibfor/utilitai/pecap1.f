...
C     ------------------------------------------------
      ...
      IF (TYPRES.NE.'EVOL_THER') THEN
        CALL UTMESS('F','PECAP1','ON DOIT DONNER UN RESULTAT '//
     &              'DE TYPE "EVOL_THER" APRES LE MOT-CLE '//
     &              '"LAPL_PHI"  DU MOT-FACTEUR "CARA_POUTRE"'//
     &              ' DANS LA COMMANDE POST_ELEM'//
     &              ' POUR CALCULER LA CONSTANTE DE TORSION.')
      END IF
...
C     -------------------------------------------
      ...
      IF (NBORDR.NE.1) THEN
        CALL UTMESS('F','PECAP1','LE NOMBRE D''ORDRES DU '//
     &              'RESULTAT '//TEMPER//' NECESSAIRE POUR '//
     &              'CALCULER LA CONSTANTE DE TORSION DOIT '//
     &              'ETRE EGAL A 1.')
      END IF
...
C     -------------------------------------------------
      ...
      IF (IRET.GT.0) THEN
        CALL UTMESS('F','PECAP1','ON N''ARRIVE PAS A RECUPERER'//
     &              ' LE CHAMP DE TEMPERATURES DU RESULTAT '//TEMPER)
      END IF
...
C       -------------------------------
        CALL JEVEUO(LIGRTH//'.NOMA','L',IDNOMA)
        NOMA = ZK8(IDNOMA)
...
C       -------------------------------------------------------
     ...
     &                'X_MIN',K8BID,IBID,XMIN,CBID,K8BID,IRET2)
          IF (IRET2.NE.0) CALL UTMESS('F','PECAP1',
     ...
     &                                )
        ELSE
          CALL UTMESS('F','PECAP1','LA TABLE "CARA_GEOM" N''EXISTE'//
     &                ' PAS.')
        END IF
...
C       --------------------------------------------
        NEC = NBEC(GD)
        CALL JENUNO(JEXNUM(NUMDDL//'.NUME.LILI',1),NOLILI)
        CALL JELIRA(JEXNUM(NUMDDL//'.NUME.PRNO',1),'LONMAX',IBID,K1BID)

C ---   RECUPERATION DU PRNO DU CHAMP DE TEMPERATURES :
C       ---------------------------------------------
        CALL JEVEUO(JEXNUM(NUMDDL//'.NUME.PRNO',1),'L',IAPRNO)

C ---   RECUPERATION DU TABLEAU DES NUMEROS D'EQUATION :
C       ----------------------------------------------
        CALL JEVEUO(NUMDDL//'.NUME.NUEQ','L',IANUEQ)
...
C       ---------------------------------------------
          CALL JELIRA(JEXNOM(NOMA//'.GROUPEMA',LISGMA(IGR)),'LONMAX',
     &                NBMAIL,K1BID)
