...
C MODIF UTILITAI  DATE 16/12/2003   AUTEUR DURAND C.DURAND 
C ======================================================================
...
C====
C
      CALL GETMJM (MOTFA2, IOCC-1, 0, K16BID, K16BID, NBARG )
      NBARG = ABS(NBARG)
      CALL WKVECT ('&&'//NOMPRO//'.LMOCLE','V V K16',
     >             MAX(1,NBARG),JLMCLE)
      CALL WKVECT ('&&'//NOMPRO//'.LTYP','V V K16',
     >             MAX(1,NBARG),JLTYP)
      CALL GETMJM (MOTFA2,IOCC-1,NBARG,ZK16(JLMCLE),ZK16(JLTYP),IBID)
C
      TYPE2 = '???'
      DO 21, IAUX = 1 , NBARG
