...
C MODIF UTILITAI  DATE 21/07/2003   AUTEUR NICOLAS O.NICOLAS 
C ======================================================================
...
C
     ...
     &             )
     &           )
     &   ) CALL UTMESS('F','IREDCA','IL Y A UN BUG ANGLES NAUTIQUES')
C
        CAS = 0
C
        IF (
...
C
 ...
 22      CONTINUE
         CALL UTMESS('F','IREDCA','IL Y A UN BUG')
 24      CONTINUE
         INO = ZI(JNOE2+IMA-1)
...
C
      ...
      DO 100 IM = 0 , NBT-1
         ...
         ENDIF
 100  CONTINUE
      IF ( IER .NE. 0 ) CALL UTMESS('F','IREDCA','VRAIMENT DESOLE')
