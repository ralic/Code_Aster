subroutine load_neut_mtan(stop_calc , model     , time , load_name, load_nume,&
...
! 
! Elementary (on one load) - Tangent matrix
!
! --------------------------------------------------------------------------------------------------
!
...
!
        call load_neut_spec('STAT'    , 'MTAN'     , model       , time       , load_name  ,&
                            load_nume , i_type_neum, nb_type_neum, nb_in_maxi , nb_in_prep ,&
                            lchin     , lpain      , nb_in_add   , load_ligrel, mtan_option)
!
        if (mtan_option .ne. 'No_Load') then
!
...
! --------- Computation
!
            call calcul(stop_calc, mtan_option, load_ligrel, nbin  , lchin,&
