...
! 
! Elementary (on one load) - Rigidity matrix
!
! --------------------------------------------------------------------------------------------------
!
...
!
        call load_neut_spec('STAT'    , 'MRIG'     , model       , time       , load_name  ,&
                            load_nume , i_type_neum, nb_type_neum, nb_in_maxi , nb_in_prep ,&
                            lchin     , lpain      , nb_in_add   , load_ligrel, mrig_option)
!
        if (mrig_option .ne. 'No_Load') then
!
...
! --------- Computation
!
            call calcul(stop_calc, mrig_option, load_ligrel, nbin  , lchin,&
