...
!      print*,'Av hydr_xmat endo3d',e00,e0,hydra1,hydras,0.66d0,erreur
    call hydr_xmat(e00, e0, hydra1, hydras, 0.33d0,&
                   erreur)
...
!       recuperation de l endo thermique et effet de l'hydratation
        call hydr_vari(var0(28), dth0, hydra0, hydra1, hydras,&
                       erreur)
...
!       recuperation des parametres d endo et de la def vpl
        do i = 1, 6
            call hydr_vari(var0(1+i), ssg6(i), hydra0, hydra1, hydras,&
                           erreur)
            call hydr_vari(var0(7+i), epsplg6(i), hydra0, hydra1, hydras,&
                           erreur)
            call hydr_vari(var0(18+i), ssw6(i), hydra0, hydra1, hydras,&
                           erreur)
            if (i .le. 3) then
                dg3(i)=var0(13+i)
                dw3(i)=var0(24+i)
...
!       print*,'on fait le calcul poro meca2'
        call b3d_sigd(bg1, pg1, bw1, pw1, sfld,&
                      ssg6, e0, rt0, ept0, mfr,&
                      erreur, dg3, dw3, xnu0, sigaf6,&
                      dflu0, sigef6, xmg0, sigat6, vplg33,&
...
!***********************************************************************
    local=.true.
    call b3d_stock_tail(xmat, nmat, ifour, mfr, nmat0,&
                        nmat1, t33, n33, local, vt33,&
                        var0, varf, nvari, erreur, gft00,&
                        fr10, rt00, ept00, beta1, gama1,&
                        nvar1)
!
!***********************************************************************
...
!            limitation du volume charge mini a celui de l element
                call b3d_l3(local, t33, n33, vt33, vt33,&
                            l3)
                xvef=l3(1)*l3(2)*l3(3)
                xli=max(l3(1),l3(2),l3(3))
                dpict00=1.d0-cnlt1**(-1)
!            donnees pour endo de Poisson sans effet d echelle
                call b3d_nosnap(0, fr10, rt0, dpict00, Gft0,&
                                e0, beta1, gama1, xli, vref0,&
                                weib0, vref0, rtw2, errmesh, coeffGf,&
                                1.d0)
                gftw2=coeffgf*gft0
                eptw2=cnlt1*(rtw2/e0)
...
!             print*,'istep 2 coefft=',coefft
                call b3d_nosnap(istep, fr10, rt0, dpict00, Gft0,&
                                e0, beta1, gama1, xli, vref0,&
                                weib0, veq, rteff, errmesh, coeffGf,&
                                coefft)
...
!            donnees pour endo de Poisson sans effet d echelle
                call b3d_nosnap(istep, fr10, rt0, dpict00, Gft0,&
                                e0, beta1, gama1, xli, vref0,&
                                weib0, vref0, rtw2, errmesh2, coeffGf2,&
                                1.d0)
                gftw2=coeffgf2*gft0
                eptw2=cnlt1*(rtw2/e0)
            end if
...
!          on verifie la condition de non snap back local
            ...
            call b3d_nosnap(istep, fr10, rt0, dpict00, Gft0,&
                            e0, beta1, gama1, xli, vref0,&
                            weib0, veq, rtw2, errmesh, coeffGf,&
                            coefft)
...
!     recuperation du seuil et de l endo de Poisson du pas precedent
    do i = 1, 6
        call hydr_vari(var0(nvar0+6+i), ssp6(i), hydra0, hydra1, hydras,&
                       erreur)
    end do
...
!      print*,sigaf6
    call b3d_partition(sigaf6, sigef3, vsigef33, vsigef33t, sigeft6,&
...
!     seul le coeff de non snap back deterministe est pris en compte ici
    call b3d_sd(ssp6, t33, n33, l3, vt33,&
                e0, xnu0, gftw2, fr10, rtw2,&
                eptw2, beta1, gama1, regl0, erreur,&
                d03, df3, st3, vss33, vss33t,&
...
!     matrice d endommagement pour l attenuation du coeff de Poisson
    call b3d_d66(xnu0, st3, d66, e0, .false.,&
                 .false.)
!
!********************************************************************************
...
!     on poursuit les calcul avec ces nouvelles contraintes effectives
    call b3d_partition(siget6, sigef3, vsigef33, vsigef33t, sigeft6,&
                       sigefc6, sigefc3, sigeft3)
!
!********************************************************************************
...
!     calcul des contraintes locales (sigi) incluant les autocontraintes eventuelles
    call b3d_covs(aleas0, vsigef33, vsigef33t, sigeft3, sigefc3,&
                  sigi6, sigit3, sigic3)
!
!******************************************************************************
...
!******************************************************************************
    do i = 1, 6
        call hydr_vari(var0(nvar0+15+i), ssl6(i), hydra0, hydra1, hydras,&
                       erreur)
    end do
!     mise a jour des seuils
    ...
    dldd=.false.
    nfid1=1.d0
    call b3d_sd(ssl6, t33, n33, l3, vt33,&
                e0, xnu0, Gft0, fr10, rt0,&
                ept0, beta1, gama1, regl0, erreur,&
                d03, dfl3, st3, vss33, vss33t,&
    ...
    call b3d_d66(xnu0, st3, dl66, e0, .false.,&
                 .true.)
!
!***************************************************************************
...
!***************************************************************************
    do i = 1, 6
        call hydr_vari(var0(nvar0+24+i), spl6(i), hydra0, hydra1, hydras,&
                       erreur)
        call hydr_vari(var0(nvar0+30+i), wfm6(i), hydra0, hydra1, hydras,&
                       erreur)
    end do
    if (fl3d) then
...
!      et des vitesses de deformation du pas precedent
        do i = 1, 6
            call hydr_vari(var0(nvar0+44+i), eve6(i), hydra0, hydra1, hydras,&
                           erreur)
            call hydr_vari(var0(nvar0+58+i), vve6(i), hydra0, hydra1, hydras,&
                           erreur)
            call hydr_vari(var0(nvar0+64+i), vm6(i), hydra0, hydra1, hydras,&
                           erreur)
        end do
    else
...
!     recup de l indicateur de localisation
    xloc=var0(nvar0+74)
    call b3d_srf(sigat6, vss33, spl6, l3, e2l3,&
                 wfeff3, wfm6, vss33t, sigarf6, xnu0,&
                 e0, dpic0, rc0, wref0, tphi0,&
                 dl66, erreur, dfl3, ssl6, ept0,&
                 siget6, sref1, fl3d, eve6, vve6,&
                 sve6, y1sy, tau1, tau2, cc2,&
                 teta12, eafluage, dt, vm6, maj0,&
                 depst6, sigaf6, xloc)
...
!     calcul sur la partie effective des contraintes dans le solide
    call b3d_partition(siga6, siga3, vsiga33, vsiga33t, sigat6,&
                       sigac6, sigac3, sigat3)
!     calcul des longueurs des elements dans les directions principales
!     des contraintes effectives de compression
    vrai=.true.
    call b3d_l3(vrai, t33, n33, vt33, vsiga33,&
                long3)
!     recuperation du seuil et de l endo de compression du pas precedent
    call hydr_vari(var0(nvar0+40), suc0, hydra0, hydra1, hydras,&
                   erreur)
    call hydr_vari(var0(nvar0+41), dc0, hydra0, hydra1, hydras,&
                   erreur)
!     actualisation de l endommagement de compression cisaillement
!     l aleas n est repercute que si cg0=aleas0 au debut d endo3d
    call b3d_dc(siga3, long3, dcf, dc0, e0,&
                delta0, gfc0, rc0, epc0, beta1,&
                gama1, regl0, fr2, suc0, ifour,&
                cg0, bw1, pw1)
