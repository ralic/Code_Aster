...
!      endommagement de fluage minimal et resistance à la compression b
...
!      contrainte effectives endommagees
    ...
    real(kind=8) :: sigaf6(6), sigaf33(3, 3)
...
!     au debut du pas 
      ...
      call hydr_xmat(xk0,xk0i,hydra1,hydras,0.33d0,erreur)
      call hydr_xmat(xk1,xk1i,hydra1,hydras,0.33d0,erreur)
      call hydr_xmat(x2mu0,x2mu0i,hydra1,hydras,0.33d0,erreur) 
      call hydr_xmat(x2mu1,x2mu1i,hydra1,hydras,0.33d0,erreur) 
      call hydr_xmat(hs1,hs1i,hydra1,hydras,0.33d0,erreur)
      call hydr_xmat(hd1,hd1i,hydra1,hydras,0.33d0,erreur)
      call hydr_xmat(hs2,hs2iref,hydra1,hydras,0.33d0,erreur)
      call hydr_xmat(hd2,hd2iref,hydra1,hydras,0.33d0,erreur)
!     on suppose que le potentiel de consolidation depend  de l hydratation 
!     via son inverse, il est constant sur le pas
      unsek0=epsk0**(-1)
      call hydr_xmat(unsek0,unsek01,hydra1,hydras,0.33d0,erreur)
      epsk01=unsek01**(-1)
...
!     en fin du pas
      ...
      call hydr_xmat(xk0,xk0f,hydra2,hydras,0.33d0,erreur)
      call hydr_xmat(xk1,xk1f,hydra2,hydras,0.33d0,erreur)
      call hydr_xmat(x2mu0,x2mu0f,hydra2,hydras,0.33d0,erreur) 
      call hydr_xmat(x2mu1,x2mu1f,hydra2,hydras,0.33d0,erreur) 
      call hydr_xmat(hs1,hs1f,hydra2,hydras,0.33d0,erreur)
      call hydr_xmat(hd1,hd1f,hydra2,hydras,0.33d0,erreur)
      call hydr_xmat(hs2,hs2fref,hydra2,hydras,0.33d0,erreur)
      call hydr_xmat(hd2,hd2fref,hydra2,hydras,0.33d0,erreur)
      call hydr_xmat(cohet1,cohet2,hydra2,hydras,0.4d0,erreur)
      call hydr_xmat(cohec1,cohec2,hydra2,hydras,0.4d0,erreur)  
      call hydr_xmat(e0,e1,hydra1,hydras,0.33d0,erreur)
      rt2=rt1*e1/e0       
...
!     sur les dissipation d energie par fluage    
      call hydr_vari(phis0,phisref1,hydra1,hydra2,hydras,erreur)
      call hydr_vari(phid0,phidref1,hydra1,hydra2,hydras,erreur) 

!     endommagement effectif de fluage fin de pas precedent
      dflu00=max(var0(nvhydra+47),0.d0)
!     correction par l hydratation      
      call hydr_vari(dflu00,dflu0,hydra1,hydra2,hydras,erreur)
...
!     chargement des variables viscoplastique
      do i=1,6
       call hydr_vari(var0(1+i),ssg6(i),hydra1,hydra2,hydras,erreur)
       call hydr_vari(var0(7+i),epsplg6(i),hydra1,hydra2,hydras,erreur)
       call hydr_vari(var0(18+i),ssw6(i),hydra1,hydra2,hydras,erreur)       
      end do       
!     recuperation des endommagement viscoplastique du pas precedent      
      do i=1,3 
       call hydr_vari(var0(13+i),dg3(i),hydra1,hydra2,hydras,erreur)       
      end do
...
!     prise en compte de l ouverture de fissure dans le calcul de pression
     ...
     epsvpw,vplg33,vplg33t,e1,rt2,ept1,erreur,xwnsat,mfr,pw0,dpw,vw1) 
...
!      ve2si=ve2si/coeff_s2      
      call flu_inc3d(e0si,e1si,e2si,ve1si,ve2si,xk0i,xk1i,hs1i,&
     hs2i,vk0,vk1,vhs1,vhs2,depsv,dt,e0sf,e1sf,e2sf,dsigs,ve1sf,ve2sf,&
     dphis1)
!      print*,'spherique'
...
!       ve2di=ve2di/coeff_d2  
       call flu_inc3d(e0di,e1di,e2di,ve1di,ve2di,x2mu0i,x2mu1i,&
      hd1i,hd2i,v2mu0,v2mu1,vhd1,vhd2,depsd6(i),dt,e0df6(i),e1df,&
      e2df,dsigd6(i),ve1df,ve2df,dphid1)
...
!      print*,'sigef6',sigef6
      call b3d_partition(sigef6,sigef3,vsigef33,vsigef33t,&
...
!     recuperation de l endo thermique et effet de l'hydratation
      call hydr_vari(var0(28),dth0,hydra1,hydra2,hydras,erreur)
...
!     print*,'on fait le calcul poro meca 2 ds fluage 3d'    
      call b3d_sigd(bg1,pg1,bw1,pw1,sfld,ssg6,e1,rt2,ept1,&
     mfr,erreur,dg3,dw3,xnu0,sigaf6,dflu01,sigef6,xmg0,&
