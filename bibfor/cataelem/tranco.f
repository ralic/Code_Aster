      SUBROUTINE TRANCO(INIT,FIN,IVAL,CVAL)
...
C
C      -------------------------------------------------
C      !MODES-  ! MOT     ! ENTIER ! SEPARAT ! FIN/EOF !
C      !PARTAGES!         !        !         !         !
C      !        !         !        !         !         !
C-------------------------------------------------------
C  0   !  1     !         !        !         !         !
...
C
      IF (INIT.EQ.0) THEN
        CALL WCLASS(ICLASS,IVAL,CVAL,4,MP,FIN,1,IRTET)
        IF ( IRTET.GT.0 ) GOTO (901), IRTET
        GOTO 9999

      ELSE
        GO TO (1,2,3,4,5,6,7,8),INIT
...
C
    3 CONTINUE
      CALL WCLASS(ICLASS,IVAL,CVAL,3,' ',FIN,4,IRTET)
      IF ( IRTET.GT.0 ) GOTO (901), IRTET
      GOTO 9999
C
    4 CONTINUE
...
C
    7 CONTINUE
      CALL WMEOF(ICLASS,IVAL,CVAL,FIN,3,11,IRTET)
      IF ( IRTET.GT.0 ) GOTO (901), IRTET
      GOTO 9999
C
    8 CONTINUE
...
C
  900 CONTINUE
      WRITE (IULMES,9000)
      CALL UTMESS('F','TRANCO','PB. LECTURE CATALOGUES.')
C
  901 CONTINUE
      ...
      CALL UTMESS('F','TRANCO','PB. LECTURE CATALOGUES.')
C
 9000 FORMAT (' ************* LECTURE CATALOGUE COMMUNICATION  **',/,
     +       ' ************* ERREUR LEXICALE *************',/)
 9001 FORMAT (' ************* LECTURE CATALOGUE COMMUNICATION  **',/,
     +       ' ************* ERREUR GRAMMAIRE ************',/,
