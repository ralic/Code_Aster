...
C    ON A UNE FONCTION REELLE
        ...
        ELSE
          CALL GETFTX(' ','ENTIER',1,1,NOMBRE,FTEXT,LL)
          IF(LL.EQ.1)THEN
C    ON A UNE FONCTION ENTIERE
            CALL FIPREP('FONCTION_I',RESUFI,'L',NOMRES,IERUSR)
          ELSE
...
C
         NOMPAR = '  '
         NOMRES = 'TOUTRESU'
         CALL WKVECT(NOMFON//'.PROL','G V K8',5,LPROL)
         ZK8(LPROL)   = 'INTERPRE'
         ZK8(LPROL+1) = 'INTERPRE'
         ZK8(LPROL+2) =  NOMPAR
         ZK8(LPROL+3) =  NOMRES
         ZK8(LPROL+4) = 'II      '
C
