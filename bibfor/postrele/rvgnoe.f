...
C        VERIFICATIONS PRELIMINAIRES
     ...
     +      (NBNEUD.EQ.0.AND.NBRGPN.EQ.1)) THEN
          ...
          IF(NBRGPN.EQ.1) THEN
            ...
            CALL JELIRA(JEXNOM(NREPGN,NOMGRN),'LONMAX',NBN,K1B)
            IF(NBN.LT.2)  CALL UTMESS('F','POST_RELEVE_T',
     +                             'AVEC VECT_Y LE GROUPE DE NOEUDS '//
     +                             'DOIT CONTENIR PLUSIEURS NOEUDS')
          ENDIF
        ELSE
          CALL UTMESS('F','POST_RELEVE_T',
     +                   'AVEC VECT_Y IL FAUT PRECISER SOIT UN SEUL '//
     +                      'GROUPE DE NOEUDS SOIT PLUSIEURS NOEUDS')
        ENDIF
...
C       VERIFICATION QUE LES POINTS SONT ALIGNES
     ...
     +              TOLE,IERA,IRET)
        IF (IRET.NE.0) CALL UTMESS('F','RVGNOE','ARRET SUR ERREURS')
