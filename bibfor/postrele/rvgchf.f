...
C MODIF POSTRELE  DATE 25/09/2001   AUTEUR GNICOLAS G.NICOLAS 
C ======================================================================
...
C
      ...
      INTEGER       N1,N2,I,J,NBORDR,IBID,IORDR,NBTROU
            ...
            ELSE
               IF ( CA .EQ. 'N' ) THEN
                  IF ( CHPSYM(6:14) .EQ. 'ELNO_DEPL' ) THEN
                     CALL RVGCH1 ( ZK24(ALSCHP), CHPSYM )
                  ENDIF
               ENDIF
            ENDIF
100      CONTINUE
...
700         CONTINUE
     ...
     >                     CBID,EPSI,CRITER,ZI,0,N1)
               N1 = -N1
               CALL JECROC(JEXNUM(NLSCHP,I))
               CALL JEECRA(JEXNUM(NLSCHP,I),'LONMAX',MAX(N1,1),' ')
               CALL JEVEUO(JEXNUM(NLSCHP,I),'E',ALSCHP)
               IF ( N1 .EQ. 0 ) THEN
                  ZK24(ALSCHP + 1-1) = '&&CHAMP_EFF_NON_EXISTANT'
               ELSE
     ...
     >                           ZK24(ALSCHP + J-1),N2)
                     ...
                     ELSE
                        IF ( CA .EQ. 'N' ) THEN
                           IF ( CHPSYM(6:14) .EQ. 'ELNO_DEPL' ) THEN
                              CALL RVGCH1 ( ZK24(ALSCHP+J-1), CHPSYM )
                           ENDIF
                        ENDIF
                     ENDIF
810               CONTINUE
