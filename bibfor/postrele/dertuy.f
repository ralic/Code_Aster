...
C
        CALL U2MESS('A','POSTRELE_2')
        CALL U2MESS('A','POSTRELE_3')
        CALL U2MESS('A','POSTRELE_4')
        GOTO 9999
      END IF
C
...
C
      ...
      IF ( NLRET .GT. 100 ) THEN
        CALL U2MESS('F','POSTRELE_6')
      END IF
C
      LFIDIN = LFIS/4.D0
      LFIDSU = 2.D0*LFIS
      IF ( LFIS .GT. 0.3D0*EPAI ) THEN
        CALL U2MESS('F','POSTRELE_7')
      END IF
      NLFID = 50
      NLFID1 = NLFID+1
      DLFID = (LFIDSU-LFIDIN)/NLFID
      IF ( NLFID .GT. 100 ) THEN
        CALL U2MESS('F','POSTRELE_8')
      END IF
C
      DO 100 I=1, NLRET1
        ...
        IF ( LRIET(I) .LE. LRIC(1) ) THEN
          CALL U2MESS('F','POSTRELE_9')
        ELSE IF ( LRIET(I) .GE. LRIC(NBP3) ) THEN
          CALL U2MESS('F','POSTRELE_10')
        ELSE
          DO 60 K=2, NBP3
...
C -- CALCUL DU KR* DU DEFAUT DEBOUCHANT --------------------------------
          IF ( LRDET(J) .LE. LRDC(1) ) THEN
            CALL U2MESS('F','POSTRELE_11')
          ELSE IF ( LRDET(J) .GE. LRDC(NBP3) ) THEN
            CALL U2MESS('F','POSTRELE_12')
          ELSE
            DO 70 K=2, NBP3
