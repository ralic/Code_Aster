...
! --  ETAPE 0.0: INITIALISATIONS PROPRES A CET OPERATEUR
    call vpini0(compex,  modes, typcon, solveu, eigsol, matpsc, matopa, veclag,&
                vecblo, vecrig, vecrer, vecrei, vecrek, vecvp)

! --  ETAPE 0.1: PARALLELISME MULTI-NIVEAUX STEP 1
    call vpmpi(1, k19bid,&
               icom1, icom2, lcomod, mpicou, mpicow, ibid, ibid, rangl, rbid, rbid,&
               rbid, rbid)
...
! --             DETERMINATION TAILLE DE L'ESPACE DE PROJECTION)
    call vpini1(eigsol, modes, solveu, typcon, vecblo, veclag, vecrig,&
                matpsc, matopa, iret, nblagr, neqact, npivot, nstoc, omemax, omemin, omeshi, sigma)
    if (iret.ne.0) goto 999

! --  ETAPE 1.2: PARALLELISME MULTI-NIVEAUX STEP 2
    call vpmpi(2, eigsol,&
               ibid, ibid, lcomod, mpicou, mpicow, nbvecg, nfreqg, rangl, rbid, rbid,&
               rbid, rbid)
...
! --------------------------------------------------------------------------------------------------
    ...
    select case(method)
    case('SORENSEN')
        call vpcals(eigsol, vecrer, vecrei, vecrek, vecvp, matopa, mxresf, neqact, nblagr,&
                    omemax, omemin, omeshi, solveu, vecblo, veclag, sigma,&
                    npivot, flage, nconv, vpinf, vpmax)
    case('TRI_DIAG')
        call vpcalt(eigsol, vecrer, vecrei, vecrek, vecvp, matopa, matpsc, mxresf,&
                    nblagr, nstoc, omemax, omemin, omeshi, solveu, vecblo, veclag, vecrig, sigma,&
                    npivot, flage, nconv, vpinf, vpmax)
    case('JACOBI')
        call vpcalj(eigsol, vecrer, vecrei, vecrek, vecvp, matopa, matpsc, mxresf,&
                    nblagr, omemax, omemin, omeshi, solveu, vecblo, npivot,&
                    flage, nconv, vpinf, vpmax)
    case('QZ')
        call vpcalq(eigsol, vecrer, vecrei, vecrek, vecvp, mxresf, neqact,&
...
! --------------------------------------------------------------------------------------------------
    call vppost(vecrer, vecrei, vecrek, vecvp, nbpark, nbpari, nbparr, mxresf,&
...
! --------------------------------------------------------------------------------------------------
    call vpmpi(5, k19bid,&
               ibid, ibid, lcomod, mpicou, mpicow, ibid, ibid, ibid, rbid, rbid,&
               rbid, rbid)
