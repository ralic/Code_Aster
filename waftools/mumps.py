...
def check_mumps_version(self):
    ...
    dict_vers = { '5.1.0consortium' : '5.1.0' }
    fragment = r'''
#include <stdio.h>
#include "smumps_c.h"
...
int main(void){
    ...
    try:
        ...
        if dict_vers.get(ret, ret) != '5.1.0' and dict_vers.get(ret, ret) != '5.1.0consortium':
            raise Errors.ConfigurationError("expected versions: {0}".format('5.1.0'))
    except:
        self.end_msg('no', 'YELLOW')
        raise
    else:
