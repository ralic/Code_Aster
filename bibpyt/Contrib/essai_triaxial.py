...
def Triaxial_DR(K,G,phi,psi,cohes=0.,sigma0=0.,depzz=-.000001,epzz_max=-0.0003,):
    ...
    for i,ep1 in enumerate(epsi1[1:]):
        ...
        while not Newton:
            ...
            seuil =si*(sigma1[i+1]-sigma3[i+1])+(sigma1[i+1]+sigma3[i+1])*s-cohes
            ...
            if not Newton:
                if not ielas:
                    ddepsi3 = (sigma0-sigma3[i+1]-dsigma3)/(1.-B*(t-si+C/D*(t+si))/BB)/D
                else:
                ...
                f[i+1]=si*(sigma1[i+1]-sigma3[i+1])+(sigma1[i+1]+sigma3[i+1])*s-cohes

                # Matrice tangente consistente
                # ----------------------------
                DSDE.append(
                      [[ E-A*(E*(t+si)+C*(t-si))/BB, C-B*(E*(t+si)+C*(t-si))/BB,],
