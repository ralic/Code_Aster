...
MACRO_BASCULE_SCHEMA = MACRO(
    ...
    EXCIT =FACT(statut='f', max='**',
                ...
                FONC_MULT=SIMP(
                statut='f', typ=(fonction_sdaster, nappe_sdaster, formule)),
        DEPL=SIMP(
            statut='f', typ=(fonction_sdaster, nappe_sdaster, formule)),
        ACCE=SIMP(
            statut='f', typ=(fonction_sdaster, nappe_sdaster, formule)),
        ...
        NOEUD=SIMP(
            statut='f', typ=no, validators=NoRepeat(), max='**'),
        GROUP_NO=SIMP(
    ...
    SOUS_STRUC=FACT(statut='f', min=01, max='**',
                    ...
                    SUPER_MAILLE=SIMP(
                    statut='f', typ=ma, validators=NoRepeat(), max='**',),
                    FONC_MULT=SIMP(
                    statut='f', typ=(fonction_sdaster, nappe_sdaster, formule)),
                    ),
    AMOR_RAYL_RIGI=SIMP(
    ...
    PROJ_MODAL=FACT(statut='f', max='**',
                    ...
                    NB_MODE=SIMP(statut='f', typ='I', defaut=9999),
                    regles=(
                    PRESENT_PRESENT('MASS_GENE', 'RIGI_GENE'),),
    ...
    b_reuse=BLOC(
        ...
        ETAT_INIT=FACT(statut='o',
                       regles=(
                       AU_MOINS_UN(
                       'EVOL_NOLI', 'DEPL', 'VITE', 'ACCE', 'SIGM', 'VARI',),
    ...
    #-------------------------------------------------------------------
    METHODE=SIMP(
        statut='d', typ='TXM', defaut="NEWTON", into=("NEWTON", "NEWTON_KRYLOV")),
    NEWTON=C_NEWTON(),
    #-------------------------------------------------------------------
    ...
    CRIT_STAB=FACT(statut='f', min=1, max=1,
                   NB_FREQ=SIMP(
                   statut='f', typ='I', max=1, val_min=1, defaut=3),
                   COEF_DIM_ESPACE=SIMP(
                   statut='f', typ='I', max=1, val_min=2, defaut=5),
                   CHAR_CRIT=SIMP(
                   statut='f', typ='R', min=2, max=2, defaut=(-10.0, 10.),
                   ...
                   statut='f', typ='TXM', defaut="NON", into=("OUI", "NON")),
                   TYPE=SIMP(
                   statut='f', typ='TXM', defaut="FLAMBEMENT", into=("FLAMBEMENT", "STABILITE")),
                   PREC_INSTAB=SIMP(statut='f', typ='R', defaut=1.E-6, max=1,),
                   SIGNE=SIMP(statut='f', typ='TXM', defaut=(
                              "POSITIF_NEGATIF"), into=("NEGATIF", "POSITIF", "POSITIF_NEGATIF"), max=1,),
                   bloc_rigi_geom=BLOC(
                   condition="(RIGI_GEOM=='NON'or MODI_RIGI=='OUI')",
                   DDL_EXCLUS=SIMP(
                   statut='f', typ='TXM', validators=NoRepeat(), max=40,
                   into=(
                   ...
                   DDL_STAB=SIMP(
                   statut='o', typ='TXM', validators=NoRepeat(), min=1, max=40,
                   into=(
                   'DX', 'DY', 'DZ', 'DRX', 'DRY', 'DRZ', 'GRX', 'PRES', 'PHI',
                   'TEMP', 'PRE1', 'PRE2', 'UI2', 'UI3', 'VI2', 'VI3', 'WI2', 'WI3', 'UO2',
                   'UO3', 'VO2', 'VO3', 'WO2', 'WO3', 'UI4', 'UI5', 'VI4', 'VI5', 'WI4',
                   ...
                   INST=SIMP(
                   statut='f', typ='R', validators=NoRepeat(), max='**'),
    ...
    INCR_IMPL=FACT(statut='o',
                   ...
                   OPTI_LIST_INST=SIMP(
                   statut='f', typ='TXM', into=("INCR_MAXI",),),
    ...
    SCHEMA_TEMPS_IMPL=FACT(statut='o',
                           SCHEMA=SIMP(
                           statut='o', min=1, max=1, typ='TXM',
                           into=(
                           "NEWMARK", "HHT", "THETA_METHODE"),),
                           b_newmark=BLOC(condition="SCHEMA=='NEWMARK'",
                                          ALPHA=SIMP(
                                          statut='f', typ='R', defaut=0.25),
                                          DELTA=SIMP(
                                          statut='f', typ='R', defaut=0.5),),
                           b_hht=BLOC(condition="SCHEMA=='HHT'",
                                      ALPHA=SIMP(
                                      statut='f', typ='R', defaut=-0.3),
                                      MODI_EQUI=SIMP(
                                      statut='f', typ='TXM', defaut="NON", into=("OUI", "NON"),),),
                           b_theta=BLOC(condition="SCHEMA=='THETA_METHODE'",
                                        THETA=SIMP(
                                        statut='f', typ='R', defaut=1., val_min=0.5, val_max=1.),),
                           b_implicit=BLOC(
                           condition="SCHEMA!='TCHAMWA'and SCHEMA!='DIFF_CENT'",
                           FORMULATION=SIMP(
                           statut='o', max=1, typ='TXM', into=("DEPLACEMENT", "VITESSE",),),),
                           ),
    #
    ...
    regles=(AU_MOINS_UN('COMPORTEMENT_IMPL', 'COMPORTEMENT_EXPL',),
            PRESENT_PRESENT(
            'COMPORTEMENT_IMPL', 'COMPORTEMENT_EXPL',),),
    #
    LIST_INST_BASCULE=SIMP(
        statut='o', typ=listr8_sdaster),
    SCHEMA_INIT=SIMP(
        statut='o', typ='TXM', into=("IMPLICITE", "EXPLICITE")),
