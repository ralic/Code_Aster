...
def calc_modes_simult(self, TYPE_RESU, OPTION, SOLVEUR_MODAL,
    ...
    if SOLVEUR_MODAL['DIM_SOUS_ESPACE'] != None :  motcles_calc_vp['DIM_SOUS_ESPACE'] = SOLVEUR_MODAL['DIM_SOUS_ESPACE']
    if SOLVEUR_MODAL['COEF_DIM_ESPACE'] != None :  motcles_calc_vp['COEF_DIM_ESPACE'] = SOLVEUR_MODAL['COEF_DIM_ESPACE']
    if SOLVEUR_MODAL['APPROCHE']        != None :  motcles_calc_vp['APPROCHE']        = SOLVEUR_MODAL['APPROCHE']
    ...
    if sturm in ('GLOBAL','LOCAL'):
        # for MODE_ITER_SIMULT, value for STURM can be only OUI or NON. Other values are equivalent to OUI
        sturm = 'OUI'
