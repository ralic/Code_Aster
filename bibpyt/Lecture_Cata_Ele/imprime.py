...
#-----------------------------------------
    ...
    if cata:
        if seq == "non":
            file = open(file2 + "/type_maille__.cata", "w")
        file.write(cata.cmodif + "\n")
        imprime_copyright(file)
        file.write("TYPE_MAILLE__\n")
        for k in range(len(cata.ltm)):
            catak = cata.ltm[k]
            file.write("\n\nMAILLE__ %-8s   %-5s   DIM__  %-1s   CODE__  %-5s" %
                       (catak[0], str(catak[1]), catak[2], catak[3]))
...
#--------------------------------------------------------
    ...
    if cata:
        if seq == "non":
            file = open(file2 + "/phenomene_modelisation__.cata", "w")
        file.write(cata.cmodif + "\n")
        imprime_copyright(file)
...
#-------------------------------------------
    for cata in capy.te:
        ...
        if seq == "non":
            file = open(file2 + "/" + string.lower(entete[0]) + ".cata", "w")
        l_elref1 = entete[2]
        lattrib = entete[3]
        l_decl_en = entete[4]
        file.write(cata.cmodif + "\n")
...
def imprime_copyright(file):
    ...
    file.write('% COPY')
    file.write(
        'RIGHT (C) 1991 - 2002  EDF R&D                  WWW.CODE-ASTER.ORG    ' + "\n")
    file.write(
        '% THIS PROGRAM IS FREE SOFTWARE; YOU CAN REDISTRIBUTE IT AND/OR MODIFY      ' + "\n")
    file.write(
        '% IT UNDER THE TERMS OF THE GNU GENERAL PUBLIC LICENSE AS PUBLISHED BY      ' + "\n")
...
def imprime_lk8(format, file, liste, ncol, decal1=0, decaln=0, retour=1):
    ...
    for k in range(nblig - 1):
        ...
        for j in range(nbcol):
            file.write("%-8s " % liste[k * nbcol + j])
        if ilig < nblig - 1:
            file.write("\n")
        if ilig == nblig - 1 and reste > 0:
            file.write("\n")
    if reste == 0 and retour:
        file.write("\n")
    if reste > 0:
        ilig = ilig + 1
        if ilig == 1:
            ...
            file.write(" " * decaln)
        for j in range(len(liste) % nbcol):
            file.write("%-8s " % liste[(nblig - 1) * nbcol + j])
        if retour:
            file.write("\n")
    ...
    for moloc in MLOCs:
        if moloc[2] == "ELEM__":
            ...
            file.write("    %-8s = %-8s %-6s %-4s__ " %
                       (moloc[0], moloc[1], moloc[2], moloc[4]))
            if moloc[4] == "IDEN":
                file.write(" (")
                imprime_lk8(
                    format, file, moloc[5], 6, decal1=0, decaln=40, retour=0)
                file.write(")\n")
            else:
                file.write("\n")
...
#----------------------------------------------------------
    ...
    if not capy.tm:
        ERR.mess('E', 'il faut un catalogue de TYPE_MAILLE__.')
    if not capy.gd:
        ERR.mess('E', 'il faut un catalogue de GRANDEUR__.')
    if not capy.ph:
        ERR.mess('E', 'il faut un catalogue de PHENOMENE_MODELISATION__.')
    if len(capy.op) == 0:
        ERR.mess('E', "il faut des catalogues d'OPTION__.")
    if len(capy.te) == 0:
        ERR.mess('E', "il faut des catalogues de TYPE_ELEM__.")
    ...
    NBNO = ut.cree_co(d, nom='&CATA.TM.NBNO', tsca='I',
                      tsca_pn='K8', contig='CONTIG', acces='NO', longv=1)
    TMDIM = ut.cree_co(d, nom='&CATA.TM.TMDIM', tsca='I',
                       tsca_pn='K8', contig='CONTIG', acces='NO', longv=1)
    nbtm = len(cata.ltm)
    ...
    NOMCMP = ut.cree_co(d, nom='&CATA.GD.NOMCMP', tsca='K8',
                        tsca_pn='K8', contig='CONTIG', acces='NO', longv=0)
    DESCRIGD = ut.cree_co(d, nom='&CATA.GD.DESCRIGD', tsca='I',
        ...
        if gd.union:
            ...
            for nogd in gd.lgd:
                ...
                if gd.tscal != x[nogd].tscal:
                    ERR.mess(
                        'E', " type scalaire différent entre: " + gd.nom + " et: " + nogd)
    ...
    for gd in cata.l_gdsimp:
        ...
        DESCRIGD.ecri_co(
            nom=nogd, indice=3, valeur=(len(gd.lcmp) - 1) / 30 + 1)
    ...
    for gd in cata.l_gdelem:
        ...
        if len(gd.gdelem) == 3:
            ...
            if tsca1 != tsca2:
                raise Exception("Erreur types incompatibles.")
            tscal = tsca1
    ...
    DESCOPT = ut.cree_co(d, nom='&CATA.OP.DESCOPT', tsca='I',
                         tsca_pn='K16', contig='CONTIG', acces='NU', longv=0)
    OPTPARA = ut.cree_co(d, nom='&CATA.OP.OPTPARA', tsca='K8',
                         tsca_pn='K16', contig='CONTIG', acces='NU', longv=0)
    LOCALIS = ut.cree_co(d, nom='&CATA.OP.LOCALIS', tsca='K24',
                         tsca_pn='K16', contig='CONTIG', acces='NU', longv=0)
    ...
    # retourne le nombre total de "familles locales de PG" référencées dans
    # les type_elem :
    def nb_loc_fpg(capy):
        nb = 0
        for cata in capy.te:
...
#   -- calcul de 2 dictionnaires qui seront utilises plus loin :
    ...
    OPTMOD = ut.cree_co(d, nom='&CATA.TE.OPTMOD', tsca='I',
                        tsca_pn='K8', contig='CONTIG', acces='NU', longv=0)
    OPTNOM = ut.cree_co(d, nom='&CATA.TE.OPTNOM', tsca='K8',
                        tsca_pn='K8', contig='CONTIG', acces='NU', longv=0)
    CTE_ATTR = ut.cree_co(d, nom='&CATA.TE.CTE_ATTR', tsca='K16',
                          tsca_pn='K16', contig='CONTIG', acces='NU', longv=0)
    ...
    for cata in capy.te:
        ...
        for elref1 in entete[2]:
            ...
            if elref1[1][0]:   # famille de PG ordinaire
                for attr_val in elref1[1][0]:
                    iflpg = iflpg + 1
                    noflpg = note2 + \
                        txtpad(8, elref1[0]) + txtpad(8, attr_val[0])
            ...
            if elref1[1][1]:   # famille de PG "liste"
                if num_elref1 != 1:
                    ERR.mess(
                        'E', "Utilisation d'une famille de PG 'liste' pour un ELREFE__ non principal: " + elref1[0])
        ...
        for moloc in MLOCs:
            ...
            if typept == "ELGA__":
                nbpt = -999
                nofpg1 = moloc[3]
                # on cherche les caractéristiques de la famille nofpg1 :
                # nbpt,ifpg

                # on cherche d'abord dans les familles "simples" définies dans
                # capy.tm :
                for decl in l_elref1[0][1][0]:
                    if decl[0] == nofpg1:
                        (nbpt, ifpg) = elrefe_npg(
                            capy.tm, NOFPG, entete[2][0][0], decl[1])
                ...
                if nbpt == -999:
                    if l_elref1[0][1][1]:
                        for decl_l in l_elref1[0][1][1]:
                            if decl_l[0] == nofpg1:
                                # on parcourt les familles "simples" de la
                                # liste :
                                nbpt_l = 0
            ...
            if nbpt == -999:
                ERR.mess(
                    'E', "Utilisation d'un nombre de points de Gauss indéfini pour le mode_local: " + nomolo)
                assert 0
    ...
    for (ph, lmod, codph) in cata.l_pheno:
        ...
        MODELI = ut.cree_co(d, nom='&CATA.' + ph, tsca='I',
                            tsca_pn='K16', contig='CONTIG', acces='NU', longv=(nbtm + 2))
        NOMMODELI = ut.cree_pn(
...
def verif_type_gene(capy):
    ...
    for cata in capy.tg:
        ...
        # 1. On vérifie qu'un catalogue "générique" a bien un nom de la forme
        # GENER_XXXX:
        if nogene[0:6] != "GENER_":
            ERR.mess('E', 'Le TYPE_GENE__: ' + nogene +
                     ' doit avoir un nom de la forme : GENER_XXXX')
            ...
            if diff == "DIFF":
                for (en, point) in moloc[5]:
                    if en not in liste_EN:
                        ERR.mess('E', "L' ensemble de noeuds " + en +
                                 " n'est pas utilisé dans le catalogue : " + nogene)
        ...
        for entete in l_entete:
            ...
            if note in capy.dicte:
                ...
                if note in dicte2:
                    ...
                    dicte2[note] = 'b'
                    ERR.mess(
                        'I', 'Le TYPE_ELEM__: ' + note + ' est défini en tant que TYPE_GENE__')
            else:
    ...
    for (ph, lmod, codph) in capy.ph.l_pheno:
        for (mod, laffe, codmod, (d1, d2), lattrib) in lmod:
            ...
            if trouve:
                ...
                # On ajoute les attributs définis AUTOMATIQUEMENT (ceux de
                # lattr_AUTO) :
                ...
                if not 'ALIAS8' in dicattr:
                    ...
                    if dicattr['ALIAS8'][5:] != str(codtma)[1:4]:
                        ERR.mess(
                ...
                # le cas d1 == -1 est particulier : il est réservé aux
                # modélisations discrètes DIS_xxx
                if d1 == -1:
                    dicattr['DISCRET'] = 'OUI'
                ...
                if lattrib:
                    for k in range(len(lattrib)):
                        ...
                        if no_attr in lattr_AUTO:
                            ERR.mess(
                                'E', "Il est interdit de redéfinir l'attribut:" + no_attr)
