...
#-------------------------------------------------
    ...
    # on détruit les modes locaux de type vecteur/matrice inutilisés dans les
    # options :
    for moloc in MLVEs:
        if not utilise.has_key(moloc[0]):
            MLVEs.remove(moloc)
    for moloc in MLMAs:
        if not utilise.has_key(moloc[0]):
            MLMAs.remove(moloc)
    ...
    for moloc in MLOCs:
        ...
        if (not utilise.has_key(moloc[0])) and (not moloc[0][0:4] == "DDL_"):
            MLOCs.remove(moloc)
...
class ERREUR:
    ...
    def veri_appartient_liste(self, code, element, liste):
        try:
            ...
            self.mess(
                code, str(element) + " n'appartient pas à  la liste: " + str(liste))
...
def cree_os(dicobj, nom, tsca, long):
    if dicobj.has_key(nom):
        ERR.mess('F', "Erreur objet déjà  déclaré:" + nom)
...
class JV_COLLEC:
    ...
    def cree_oc(self, nom, long):
        ...
        if num > 0:
            ERR.mess(
                'F', "Erreur : nom existant déjà : " + nom + " dans: " + self.nom)
    ...
    with open(os.path.abspath(unigest)) as fid:
        for line in fid:
            ...
            if type == "OPTIONS":
                ...
                assert type in (
                    "COMMANDE", "COMMUN", "ENTETE", "COMPELEM"), type
                continue
            if nom in dico:
                num = dico[nom]
                del dico[nom]
