    ...
    def condensation(self, resolution = None, nomcham = 'DEPL'):
        ...
        # cas ES, il faut re-calculer la base a partir des noeuds choisis
            ...
            for num in modes_expansion_retenus:
                node,comp = self.calc_base_es().get_modes_data()['NOEUD_CMP'][num-1].split()
        ...
        self.x_mailcond = None
        __MAILCD = DEFI_MAILLAGE( DEFI_SUPER_MAILLE = _F( MACR_ELEM = self.x_ssexp,
...
##        acouple = cpl.mat_amor[0]
        ...
        self.modes_couple = ModeMeca(self.objects, _MODCPL.nom, _MODCPL, self.mess)
        ...
        try:
            __MDRETR=DEPL_INTERNE(DEPL_GLOBAL=_MODCPL,SUPER_MAILLE=self.sumail)
        except aster.error,err :
            message = "ERREUR ASTER : " + mess.GetText('I',err.id_message, err.valk, err.vali, err.valr)
            self.mess.disp_mess( message)
