...
from code_aster.Cata.Commands import DETRUIRE, CREA_TABLE, RECU_FONCTION, IMPR_TABLE
...
class InterfaceCalcSpec(Frame):
    ...
    def InterfaceCalcSpec(self):
        ...
        self.list_mes = Listbox(
            fl, selectmode='extended', exportselection=False, background='white')
        self.list_mes.grid(row=r, column=2)

        self.list_ref = Listbox(
            fl, selectmode='extended', exportselection=False, background='white')
        ...
        r = r + 1
        self.but_vis = Button(
            fl, text="Visualiser temporels", command=self.visu_tempo)
        ...
        windows = ["Naturelle", "Hanning",
                   "Hamming", "Exponentielle", "Utilisateur"]
        for win in range(len(windows)):
            Radiobutton(fp, text=windows[win], variable=self.radio_win, value=win).grid(
                row=r, column=win)
        r = r + 1
        Label(
            fp, text="Parametres (cas des fenetres Exponentielle et Utilisateur) :",
        ...
        self.view_spec = Button(
            fp, text="Interspectres", command=self.calc_intespec)
        self.view_spec.grid(row=r, column=1)

        self.view_spec = Button(
            fp, text="Transferts", command=self.calc_transfert)
        ...
        Radiobutton(fp, text="H1", variable=self.radio_h1h2, value=0).grid(
            row=r, column=2)
        r = r + 1
        Radiobutton(fp, text="H2", variable=self.radio_h1h2, value=1).grid(
            row=r, column=2)
    ...
    def frame_visu(self, type_data='R'):
        ...
        self.var_visu_resu = StringVar()
                                       # variable du choix de visu (non utilise
                                       # pour calc_spec)
        self.label_visu = StringVar()
                                    # variable pour le label de la colonne de
                                    # visu
...
# self.curve_list.delete()
    ...
    def display_curve(self):
        ...
        data = {
            'I': [self.Spec, 'FONCTION_C', 'Fréquence', 'Hz', 'Inter-spectre', 'unite^2/Hz', 'Inter-spectre'],
    ...
    def calc_intespec(self):
        ...
        recouvrement = [
            "RECOUVREMENT_DUREE", "RECOUVREMENT_NB_PTS", "RECOUVREMENT_POURCENT"]
        ...
        if len(lon) > 0:
            mcfact_tabechant.update(
                {longueur[self.radio_long.get()]: float(self.entry_long.get())})
        else:
            mcfact_echant.update({'LONGUEUR_POURCENT': 100.})

        if len(rec) > 0:
            mcfact_tabechant.update(
                {recouvrement[self.radio_rec.get()]: float(self.entry_rec.get())})
        else:
        ...
        self.Spec = InterSpectre(
            'Inter-spectre', __Spec, None, [], self.mess, None)
        self.is_tab_spec = 1
...
# self.curve_list.delete()
    ...
    def calc_coherence(self):
        ...
        # recuperation des points de reference. Pour les transfert, une seule
        # reference.
...
# self.curve_list.delete()
    ...
    def calc_transfert(self):
        ...
        self.curve_list.set_values(lst)
        self.label_visu.set(
            "Transferts (H" + str(self.radio_h1h2.get() + 1) + ") :")

    def crea_tab_fonc(self):

        from code_aster.Cata.Commands import CREA_TABLE
        ...
        for mes in ind_sel:
            ...
            for i1 in range(1, list_temp.nume_ordr.count(mes)):
                ind_new.append(
                    list_temp.nume_ordr[ind_new[-1] + 1:].index(mes) + ind_new[-1] + 1)
        ...
        tempo = Tempo('__tab_temp', __tab_temp,
                      0, None, None, [], self.mess, None)
    ...
    def refresh(self, list_tempo, list_tab, list_mes, list_ref):
        ...
        # pour suivre la table fonction sur laquelle on travaille (celle qui a
        # ete exportee)
        if type(self) != type([]):
        ...
        c = []
        for i1 in range(l_ord[-1] + 1):
            c.append([])
        for mes in l_ord:
            c[mes].append(list_temp.nume_ordr.index(mes))
            for i1 in range(1, list_temp.nume_ordr.count(mes)):
                c[mes].append(
                    list_temp.nume_ordr[c[mes][-1] + 1:].index(mes) + c[mes][-1] + 1)
