...
def interp(typ_i, val, x1, x2, y1, y2, tol=1.e-6):
    ...
    if typ_i == ['LIN', 'LOG']:
        return exp(log(y1) + (val - x1) * (log(y2) - log(y1)) / (x2 - x1))
    if typ_i == ['LOG', 'LOG']:
...
class t_fonction:
    ...
    def __add__(self, other):
        ...
        if isinstance(other, t_fonction):
            ...
            vale_x, para['PROL_GAUCHE'], para[
                'PROL_DROITE'] = self.homo_support(other)
            fff = self.evalfonc(vale_x)
            ggg = other.evalfonc(vale_x)
            if isinstance(self, t_fonction_c):
                return t_fonction_c(vale_x, fff.vale_y + ggg.vale_y, para)
            else:
                return t_fonction(vale_x, fff.vale_y + ggg.vale_y, para)
    ...
    def homo_support(self, other):
        ...
        if other.vale_x[0] > self.vale_x[0]:
            if other.para['PROL_GAUCHE'] != 'EXCLU':
                f_g = self
            else:
                f_g = other
        else:
        ...
        if self.vale_x[-1] > other.vale_x[-1]:
            if other.para['PROL_DROITE'] != 'EXCLU':
                f_d = self
            else:
                f_d = other
        else:
    ...
    def cut(self, rinf, rsup, prec, crit='RELATIF', nom=''):
        ...
        if crit == 'ABSOLU':
            rinf_tab = NP.greater(abs(self.vale_x - rinf), prec)
        elif crit == 'RELATIF':
        ...
        vale_y = NP.array(
            [self(rinf), ] + self.vale_y.tolist()[i:j] + [self(rsup), ])
    ...
    def cat(self, other, surcharge):
        ...
        if surcharge == 'GAUCHE':
            ...
            if i != len(f2.vale_x):
                para['PROL_DROITE'] = f2.para['PROL_DROITE']
            else:
                para['PROL_DROITE'] = f1.para['PROL_DROITE']
    ...
    def trapeze(self, coef):
        ...
        """
        n = len(self.vale_y)
                # [1:n] car pb sur calibre 5 (etch, python2.5 x86_64)
        trapz = NP.zeros(n, float)
        trapz[0] = coef
        trapz[1:n] = (self.vale_y[1:n] + self.vale_y[:-1]) / 2 * (
            self.vale_x[1:n] - self.vale_x[:-1])
        prim_y = NP.cumsum(trapz)
        para = copy.copy(self.para)
        para['PROL_GAUCHE'] = 'EXCLU'
        para['PROL_DROITE'] = 'EXCLU'
        if para['NOM_RESU'][:4] == 'VITE':
            para['NOM_RESU'] = 'DEPL'
        elif para['NOM_RESU'][:4] == 'ACCE':
            para['NOM_RESU'] = 'VITE'
    ...
    def simpson(self, coef):
        ...
        for i in range(2, len(self.vale_x)):
            ...
            tabl[iperm] = tabl[iperm] + \
                (bma / 6.) * (ct[0] * fa + ct[1] * fm + ct[2] * fb)
    ...
    def derive(self):
        ...
        pentes = (self.vale_y[1:n] - self.vale_y[:-1]) / (
            self.vale_x[1:n] - self.vale_x[:-1])
        derive = (pentes[1:n - 1] * pas[1:n - 1] + pentes[:-1] * pas[:-1]) / (
            pas[1:n - 1] + pas[:-1])
        derv_y = [pentes[0]] + derive.tolist() + [pentes[-1]]
        para = copy.copy(self.para)
        para['PROL_GAUCHE'] = 'EXCLU'
        para['PROL_DROITE'] = 'EXCLU'
        if para['NOM_RESU'][:4] == 'DEPL':
            para['NOM_RESU'] = 'VITE'
        elif para['NOM_RESU'][:4] == 'VITE':
            para['NOM_RESU'] = 'ACCE'
    ...
    def inverse(self):
        ...
        if para['PROL_GAUCHE'] == 'CONSTANT':
            para['PROL_GAUCHE'] = 'EXCLU'
        if para['PROL_DROITE'] == 'CONSTANT':
            para['PROL_DROITE'] = 'EXCLU'
        vale_x = self.vale_y
        vale_y = self.vale_x
    ...
    def abs(self):
        ...
        """
        para = copy.copy(self.para)
        if para['PROL_GAUCHE'] == 'LINEAIRE':
            para['PROL_GAUCHE'] = 'EXCLU'
        if para['PROL_DROITE'] == 'LINEAIRE':
            para['PROL_DROITE'] = 'EXCLU'
    ...
    def fft(self, methode):
        ...
        if methode == 'TRONCATURE':
            ...
            if len(self.vale_y) > 2 ** n:
                ...
                vale_y = NP.concatenate(
                    (vale_y, NP.zeros(2 ** (n + 1) - len(self.vale_x))))
...
class t_fonction_c(t_fonction):
    ...
    def __repr__(self):
        ...
        texte = []
        for i in range(len(self.vale_x)):
            texte.append('%f %f + %f .j' %
                         (self.vale_x[i], self.vale_y[i].real, self.vale_y[i].imag))
        return '\n'.join(texte)
    ...
    def fft(self, methode, syme):
        ...
        if syme == 'OUI':
            ...
            if NP.remainder(len(part1), 2) == 0:
                ...
                # pour la symetrie. On identifie a, b et c, pour calculer
                # Y(N+1)
                middle = []
                middle.append(
                    (4 * part1[-1].real - part1[len(part1) - 2].real) / 3)
                part2 = NP.conjugate(fonc_temp[1:len(part1)])
                part2 = part2.tolist()
                part2.reverse()
                vale_fonc = NP.array(part1 + middle + part2)
            else:
                # Si le dernier point est effectivement reel, on reconstruit
                # theoriquement
                ...
                # Sinon, on approxime comme dans le cas ou N est pair
                    part1[-1] = (
                        4 * part1[len(part1) - 2].real - part1[len(part1) - 3].real) / 3
...
class t_nappe:
    ...
    def __call__(self, val1, val2, tol=1.e-6):
        ...
        # i = 0, val1 <= val_min
        if i == 0:
            eps_g = (val1 - self.vale_para[0]) / (
                self.vale_para[1] - self.vale_para[0])
            if abs(eps_g) <= tol:
                return self.l_fonc[0](val2)
            else:
    ...
    def homo_support(self, other):
        ...
        for val in vale_para:
            if val in self.vale_para:
                l_fonc.append(
                    self.l_fonc[NP.searchsorted(self.vale_para, val)])
            elif val in other.vale_para:
                other_fonc = other.l_fonc[
                    NP.searchsorted(other.vale_para, val)]
    ...
    def extreme(self):
        ...
        for i in range(len(self.vale_para)):
            for j in range(len(self.l_fonc[i].vale_y)):
                ...
                if y == val_min:
                    vm['min'].append(
                        [y, self.l_fonc[i].vale_x[j], self.vale_para[i]])
                if y == val_max:
                    vm['max'].append(
                        [y, self.l_fonc[i].vale_x[j], self.vale_para[i]])
        vm['min'].sort()
...
def fractile(l_f, fract):
    ...
    if fract >= 1.:
        ...
        for l_val in tab_val:
            vale_y.append(
                l_val[indice] * (1 - reste) + l_val[indice + 1] * reste)
    return t_fonction(vale_x, vale_y, para)
