...
def test_temps_ops(self, RESU, INFO, **args):
    ...
    try:
        ...
        assert len(
            l_export) > 0, "<TEST_TEMPS> pas de fichier export dans le repertoire de travail."
    ...
    # liste des timers par ordre d'apparition : dict_cmde['commande'] =
    # [timer1, timer2, ...]
    dict_cmde = {}
    ...
    for res_i in RESU:
        ...
        tres = (tab.COMMANDE == dres['COMMANDE']) & (
            tab.NUME_ORDRE == dres['NUME_ORDRE'])
        if len(tres) != 1:
            UTMESS('E', 'TEST0_7', valk=dres[
                   'COMMANDE'], vali=dres['NUME_ORDRE'])
            error = True
