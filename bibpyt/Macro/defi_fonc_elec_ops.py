...
def FcompletGR1(T, I1, I2, FR, TR, PHI1, PHI2, TAU1, TAU2):
    fxt = 4.E-7 * I1 * I2
    fxt = fxt * \
        (cos(2 * pi * FR * (T - TR) + PHI1 * pi / 180.)
         - exp(-(T - TR) / TAU1) * cos(PHI1 * pi / 180.))
    fxt = fxt * \
        (cos(2 * pi * FR * (T - TR) + PHI2 * pi / 180.)
         - exp(-(T - TR) / TAU2) * cos(PHI2 * pi / 180.))
    return fxt
...
def FcompletGR2R(T, I1R, I2R, FR, TRR, PHIR1, PHI2R, TAU1R, TAU2R, D):
    fxt = 4.E-7 * I1R * I2R / D
    fxt = fxt * \
        (cos(2 * pi * FR * (T - TRR) + PHI1R * pi / 180.) -
         exp(-(T - TRR) / TAU1R) * cos(PHI1R * pi / 180.))
    fxt = fxt * \
        (cos(2 * pi * FR * (T - TRR) + PHI2R * pi / 180.) -
         exp(-(T - TRR) / TAU2R) * cos(PHI2R * pi / 180.))
    return fxt
...
#
        ...
        for k_cour in COUR:
            ...
            if SIGNAL == 'CONTINU':
                for t in t_k_cour:
                    fff.append(
                        FcontinuGR1(t, I1, I2, TR, PHI1, PHI2, TAU1, TAU2))
            elif SIGNAL == 'COMPLET':
                for t in t_k_cour:
                    fff.append(
                        FcompletGR1(t, I1, I2, FREQ, TR, PHI1, PHI2, TAU1, TAU2))
#
    elif COUR_PRIN:
