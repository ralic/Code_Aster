...
#         on ne prend en compte que ARCHIVAGE / LIST_INST
            if ARCHIVAGE['LIST_INST'] != None:
                ...
                REPONSE = CALC_TABLE(TABLE=__REP1, TITRE=Titre,
                                     ...
                                     VALE=lr, PRECISION=ARCHIVAGE[
                                     'PRECISION']),
                                     )
            else:
                REPONSE = CALC_POINT_MAT(
                    INFO=INFO, MATER=MATER, ANGLE=ANGLE, **motscles)
        else:
            REPONSE = CALC_POINT_MAT(
                INFO=INFO, MATER=MATER, ANGLE=ANGLE, **motscles)
...
# imposées.
            if ANGLE != 0.:
                __MA = MODI_MAILLAGE(
                    reuse=__MA, MAILLAGE=__MA, ROTATION=_F(POIN_1=(0., 0.), ANGLE=ANGLE),)
                c = math.cos(ANGLE * math.pi / 180.)
                s = math.sin(ANGLE * math.pi / 180.)
                __C_RIGIDE = AFFE_CHAR_MECA(MODELE=__MO,
                                            DDL_IMPO=_F(
                                                NOEUD='P0', DX=0, DY=0., DZ=0.),
                                            LIAISON_DDL=(
                                            _F(NOEUD=('P1', 'P1', 'P2', 'P2'), DDL=('DX', 'DY', 'DX', 'DY'),
                                               COEF_MULT=(
                                                   s, -c, c, s), COEF_IMPO=0),
                                                _F(NOEUD=('P1', 'P3', 'P3'), DDL=('DZ', 'DX', 'DY'),
                                                   COEF_MULT=(
                                                       -1.0, c, s), COEF_IMPO=0),
...
#     -- Mouvement de corps rigide
                __C_RIGIDE = AFFE_CHAR_MECA(MODELE=__MO,
                                            DDL_IMPO=_F(
                                                NOEUD='P0', DX=0, DY=0, DZ=0),
                                            LIAISON_DDL=(
                                            _F(NOEUD=('P2', 'P1'), DDL=('DX', 'DY'),
                                               COEF_MULT=(1, -1), COEF_IMPO=0),
                                                _F(NOEUD=('P3', 'P1'), DDL=('DX', 'DZ'),
                                                   COEF_MULT=(
                                                       1, -1), COEF_IMPO=0),
            ...
            if args['AFFE_VARC'] != None:
                ...
                for ivarc in range(nbvarc):
                    ...
                    if(str(lvarc[ivarc]['NOM_VARC']) == 'SECH'):
                        ...
                        __EVOV = CREA_RESU(
                            OPERATION='AFFE', TYPE_RESU='EVOL_VARC', NOM_CHAM='TEMP',
                            AFFE=l_affe_cham)
...
#     --contraintes initiales
        if SIGM_INIT:
            ...
            __SIG_INIT = CREA_CHAMP(
                MAILLAGE=__MA, OPERATION='AFFE', TYPE_CHAM='CART_SIEF_R',
...
#     -- Deroulement du calcul
        ...
        if etatinit == 1:
            ...
            if MASSIF:
                __EVOL1 = STAT_NON_LINE(
                    INFO=INFO, CARA_ELEM=__CARA, MODELE=__MO, CHAM_MATER=__CHMAT,
                    ETAT_INIT=init_dico, EXCIT=l_char, **motscles)
            else:
                __EVOL1 = STAT_NON_LINE(
                    INFO=INFO, MODELE=__MO, CHAM_MATER=__CHMAT,
                    ETAT_INIT=init_dico, EXCIT=l_char, **motscles)
            ...
            if MASSIF:
                __EVOL1 = STAT_NON_LINE(
                    INFO=INFO, MODELE=__MO, CARA_ELEM=__CARA, CHAM_MATER=__CHMAT,
        ...
        if MODELISATION == "3D":
            ...
            __EVOL = MODI_REPERE(RESULTAT=__EVOL1, MODI_CHAM=(
                _F(NOM_CHAM='DEPL', NOM_CMP=(
                    'DX', 'DY', 'DZ'), TYPE_CHAM='VECT_3D',),
                _F(NOM_CHAM='SIGM_ELNO', NOM_CMP=(
                    'SIXX', 'SIYY', 'SIZZ', 'SIXY', 'SIXZ', 'SIYZ'), TYPE_CHAM='TENS_3D',),
                _F(NOM_CHAM=nomepsi, NOM_CMP=(
            ...
            __EVOL = MODI_REPERE(RESULTAT=__EVOL1, MODI_CHAM=(
                                 _F(NOM_CHAM='DEPL', NOM_CMP=(
                                    'DX', 'DY'), TYPE_CHAM='VECT_2D',),
                                 _F(NOM_CHAM='SIGM_ELNO', NOM_CMP=(
                                    'SIXX', 'SIYY', 'SIZZ', 'SIXY'), TYPE_CHAM='TENS_2D',),
                                 _F(NOM_CHAM=nomepsi, NOM_CMP=(
