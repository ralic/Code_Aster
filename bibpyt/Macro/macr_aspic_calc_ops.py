...
def macr_aspic_calc_ops(
    self, TYPE_MAILLAGE, TUBULURE, MAILLAGE, MODELE, CHAM_MATER, CARA_ELEM,
...
#------------------------------------------------------------------
# data
    GRMAIL = ('EQUERRE', 'PEAUINT', 'EXCORP1',
...
#
    ...
    if (PRES_REP['PRES_LEVRE'] == 'OUI') and (TYPE_MAILLAGE[-4:] == '_DEB'):
        ...
        mcfact.append(
            _F(GROUP_MA=GRMAIL[:-2], PHENOMENE='MECANIQUE', MODELISATION='3D'))
    mcfact.append(
        _F(GROUP_MA='P1_CORP', PHENOMENE='MECANIQUE', MODELISATION='DIS_TR'))
    mcfact.append(
        _F(GROUP_MA='P2_CORP', PHENOMENE='MECANIQUE', MODELISATION='DIS_TR'))
    mcfact.append(
        _F(GROUP_MA='P_TUBU', PHENOMENE='MECANIQUE', MODELISATION='DIS_TR'))
...
#
    ...
    indther = 0
    if ECHANGE != None and RESU_THER != None:
        indther = 1
    mcfact = []
    mcfac2 = []
    for mater in mc_AFFE_MATERIAU:
...
#
    if COMPORTEMENT != None:
        mcfci = []
        mcfci.append(
            _F(GROUP_MA=GRMAIL[:-2], RELATION=COMPORTEMENT['RELATION']))
...
#
        ...
        if TYPE_MAILLAGE in ('FISS_COUR_DEB', 'FISS_LONG_DEB', 'FISS_AXIS_DEB', 'FISS_COUR_NONDEB'):
            ...
            if TYPE_MAILLAGE in ('FISS_COUR_DEB', 'FISS_LONG_DEB'):
                FERME = 0
            else:
                FERME = 1
...
#       NOMGRE.append(('P_FON2' ,'P_FIS2' ),)
            ...
            if TYPE_MAILLAGE == 'FISS_LONG_NONDEB':
                FERME = 0
            else:
                FERME = 1
...
#
        ...
        for j in range(NBFIS):
            if FOND_FISS_1 != None:
                self.DeclareOut('_fd3d_0', FOND_FISS_1)
            if FOND_FISS_2 != None:
                self.DeclareOut('_fd3d_1', FOND_FISS_2)
#
...
#
                    montit = 'G_THETA AVEC R_INF = ' + \
                        str(tht3d['R_INF']) + ' ET R_SUP = ' + str(
                            tht3d['R_SUP'])
                    motscles = {}
                    if COMPORTEMENT != None:
                        motscles['COMPORTEMENT'] = _F(TOUT='OUI',
...
#
    if IMPRESSION != None:
        ...
        if IMPRESSION['FORMAT'] in ('IDEAS', 'CASTEM'):
            ncham = []
            if IMPRESSION['NOM_CHAM'] != None:
                if type(IMPRESSION['NOM_CHAM']) in (types.TupleType, types.ListType):
                    ncham = IMPRESSION['NOM_CHAM']
                else:
                    ncham = [IMPRESSION['NOM_CHAM'], ]
            if len(ncham) == 3:
                motscles['NOM_CHAM'] = ('DEPL', 'SIEQ_ELNO')
