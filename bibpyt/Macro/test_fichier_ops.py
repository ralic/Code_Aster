...
def test_fichier_ops(self, FICHIER, NB_VALE, VALE_CALC, VALE_CALC_K, TYPE_TEST,
    ...
    nbval, valeur, chksum = test_iter(
        fileobj, function=dict_func_test[TYPE_TEST], verbose=(INFO > 1))
    fileobj.close()
    ...
    if VALE_CALC is not None:
        ...
        sVALE = 'non testé'
    UTMESS('I', 'TEST0_4', vali=(nbval, NB_VALE),
           valr=valeur, valk=(chksum, refsum, FICHIER, sVALE))
...
if __name__ == '__main__':
    ...
    nbv2, sumv2, chksum2 = test_iter(
        fileobj, function=dict_func_test[opts.type_test], verbose=opts.verbose)
