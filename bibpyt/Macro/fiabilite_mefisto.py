...
#
        f_menu = fiabilite_fichier.fiabilite_fichier(
            self, Rep_Calc_LOGICIEL_global, fic_dataMenu, INFO)
...
#
        f_stoch = fiabilite_fichier.fiabilite_fichier(
            self, Rep_Calc_LOGICIEL_global, fic_dataStoch, INFO)
...
#
            ...
            if m["LOI"] == "UNIFORME":
                f_stoch.Ecrit_Valeurs(
                    [m["NOM"],  1, d["v_min_loi"], d["v_max_loi"]])
            elif m["LOI"] == "NORMALE":
                f_stoch.Ecrit_Valeurs(
                    [m["NOM"],  2, d["v_moy_loi"], d["sigma_loi"]])
            elif m["LOI"] == "LOGNORMALE":
                f_stoch.Ecrit_Valeurs(
                    [m["NOM"],  3, d["v_moy_loi"], d["sigma_loi"], d["v_min_loi"]])
            elif m["LOI"] == "NORMALE_TRONQUEE":
                f_stoch.Ecrit_Valeurs(
                    [m["NOM"],  4, d["v_moy_loi"], d["sigma_loi"], d["v_min_loi"], d["v_max_loi"]])
            else:
                erreur = 50
#
        if erreur:
...
#
        f_num = fiabilite_fichier.fiabilite_fichier(
            self, Rep_Calc_LOGICIEL_global, fic_dataNum, INFO)
...
#
# remarque : il faut mettre une valeur plausible en aux1, sinon plantage
# violent ...
            aux1 = 1
            aux2 = 1789
#
...
#
        f_num.Ecrit_Titre(
            "Parameter Importance Samplings : Simulation Number")
...
#
        f_grad = fiabilite_fichier.fiabilite_fichier(
            self, Rep_Calc_LOGICIEL_global, fic_dataGrad, INFO)
