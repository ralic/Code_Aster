...
def essai_TD(self,str_n_essai,DicoEssai,MATER,COMPORTEMENT,CONVERGENCE,INFO):
  ...
  for i in xrange(len(PRES_CONF)):
    ...
    remplir_graphique(DicoEssai,Courbes,list(p),list(eps_vol),str_leg,"P-EPS_VOL")
...
def essai_CISA_C(self,str_n_essai,DicoEssai,MATER,COMPORTEMENT,CONVERGENCE,INFO):
  ...
  for i in xrange(len(PRES_CONF)):
    ...
    # ---
    Gs_max = Calc_Gs_max(self,GAMMA_ELAS,PRES_CONF[i],KZERO,MATER,COMPORTEMENT,CONVERGENCE)
    ...
    for j in xrange(len(GAMMA_IMPOSE)):
      ...
      affiche_infos_essai(str_n_essai,"CISA_C",PRES_CONF[i],GAMMA_IMPOSE[j])
      ...
      if not( Gs <= 1. or abs(Gs-1.) <= 1.e-8 ) :
        UTMESS('F','COMPOR2_36',valk=('CISA_C','cisaillement','EPSI_ELAS'),valr=(GAMMA_ELAS))
      Gs_list[i].append(Gs)

      # taux d'amortissement D (aire delta_W : methode des trapezes)
      I=NP.sum(0.5*(sig_xy[ind1:ind2]+sig_xy[ind1+1:ind2+1])*(gamma[ind1+1:ind2+1]-gamma[ind1:ind2]))  # 1ere moitié de la boucle 
      J=NP.sum(0.5*(sig_xy[ind2:ind3]+sig_xy[ind2+1:ind3+1])*(gamma[ind2+1:ind3+1]-gamma[ind2:ind3]))  # 2eme moitié de la boucle
      delta_W=abs(I+J)
    
      W = 0.5*abs(sig_xy[ind3]*GAMMA_IMPOSE[j])  # aire du triangle, énergie élastique sur le 1/4 du cycle
      ...
      remplir_graphique(DicoEssai,Courbes_niv2,list(gamma),list(sig_xy),str_leg2,"GAMMA-SIGXY")

      DETRUIRE(CONCEPT=_F(NOM = (__CHAR1,__CHAR2,__CHAR3,__EVOL),), INFO=1)
    # ---
    ...
    remplir_graphique(DicoEssai,Courbes_niv1,GAMMA_IMPOSE,Gs_list[i],str_leg1,"GAMMA-G")
    remplir_graphique(DicoEssai,Courbes_niv1,GAMMA_IMPOSE,D_list[i] ,str_leg1,"GAMMA-D")
    remplir_graphique(DicoEssai,Courbes_niv1,Gs_list[i],D_list[i] ,str_leg1,"G-D")
...
def essai_TND_C(self,str_n_essai,DicoEssai,MATER,COMPORTEMENT,CONVERGENCE,INFO):
  ...
  for i in xrange(len(PRES_CONF)):
    ...
    for j in xrange(len(SIGM_IMPOSE)):
      ...
      remplir_graphique(DicoEssai,Courbes_niv2,list(eps_zz),list(pre_eau),str_leg2,"EPS_AXI-PRE_EAU")
      remplir_graphique(DicoEssai,Courbes_niv2,list(eps_zz),list(q),str_leg2,"EPS_AXI-Q")
...
def essai_TD_A(self,str_n_essai,DicoEssai,MATER,COMPORTEMENT,CONVERGENCE,INFO):
  ...
  for i in xrange(len(PRES_CONF)):
    ...
    # ---
    Es_max = Calc_Es_max_TA(self,EPSI_ELAS,PRES_CONF[i],KZERO,MATER,COMPORTEMENT,CONVERGENCE)
    ...
    for j in xrange(len(EPSI_IMPOSE)):
      ...
      affiche_infos_essai(str_n_essai,"TD_A",PRES_CONF[i],EPSI_IMPOSE[j])
      ...
      if not( Es <= 1. or abs(Es-1.) <= 1.e-8 ) :
        UTMESS('F','COMPOR2_36',valk=('TD_A','young','EPSI_ELAS'),valr=(EPSI_ELAS))
      Es_list[i].append(Es)

#      # taux d'amortissement D (aire delta_W : methode des trapezes)

#      I=NP.sum(0.5*(q[ind1:ind2]+q[ind1+1:ind2+1])*(eps_zz[ind1+1:ind2+1]-eps_zz[ind1:ind2]))  # 1ere moitié de la boucle 
#      J=NP.sum(0.5*(q[ind2:ind3]+q[ind2+1:ind3+1])*(eps_zz[ind2+1:ind3+1]-eps_zz[ind2:ind3]))  # 2eme moitié de la boucle
...
#      D_list[i].append(D)
      ...
      str_leg2 = "PRES_CONF = " + str("%E"%(PRES_CONF[i])) + ", EPSI_IMPOSE = "\
                                + str("%E"%(EPSI_IMPOSE[j]))
      remplir_graphique(DicoEssai,Courbes_niv2,list(p),list(q),str_leg2,"P-Q")
      remplir_graphique(DicoEssai,Courbes_niv2,list(eps_zz),list(eps_vol),str_leg2,"EPS_AXI-EPS_VOL")
      remplir_graphique(DicoEssai,Courbes_niv2,list(eps_zz),list(q),str_leg2,"EPS_AXI-Q")
      remplir_graphique(DicoEssai,Courbes_niv2,list(eps_vol),list(q),str_leg2,"EPS_VOL-Q")
      remplir_graphique(DicoEssai,Courbes_niv2,list(p),list(eps_vol),str_leg2,"P-EPS_VOL")
    ...
    remplir_graphique(DicoEssai,Courbes_niv1,EPSI_AXI_DA,Es_list[i],str_leg1,"EPSI-E")
...
def essai_TD_NA(self,str_n_essai,DicoEssai,MATER,COMPORTEMENT,CONVERGENCE,INFO):
  ...
  for i in xrange(len(PRES_CONF)):
    ...
    # ---
    Es_max = Calc_Es_max_TNA(self,EPSI_ELAS,PRES_CONF[i],KZERO,MATER,COMPORTEMENT,CONVERGENCE)
    ...
    for j in xrange(len(EPSI_IMPOSE)):
      ...
      affiche_infos_essai(str_n_essai,"TD_NA",PRES_CONF[i],EPSI_IMPOSE[j])
      ...
      __CHAR1 = DEFI_FONCTION(INFO = INFO, NOM_PARA = 'INST',
                              ABSCISSE = [20.*k for k in xrange(2*(NB_CYCLE+1))],
                              ORDONNEE = [0., EPSI_IMPOSE[j]]*(NB_CYCLE+1) ,
                              )
      ...
      Es = Es/Es_max
      if not( Es <= 1. or abs(Es-1.) <= 1.e-8 ) :
        UTMESS('F','COMPOR2_36',valk=('TD_NA','Young','EPSI_ELAS'),valr=(EPSI_ELAS))
      Es_list[i].append(Es)
    ...
    remplir_graphique(DicoEssai,Courbes_niv1,EPSI_AXI,Es_list[i],str_leg1,"EPSI-E")
...
def essai_OEDO_C(self,str_n_essai,DicoEssai,MATER,COMPORTEMENT,CONVERGENCE,INFO):
  ...
  for i in xrange(len(PRES_CONF)):
    ...
    for j in xrange(len(SIGM_IMPOSE)):
      ...
      affiche_infos_essai(str_n_essai,"OEDO_C",PRES_CONF[i],SIGM_IMPOSE[j],SIGM_DECH[i])
      ...
      # ---
      inst_ini += long_cyc
      L_inst    = [inst_ini + k*pas_inst for k in xrange(2*NB_INST+1)]
      ...
      if j==0:
        ...
        __CHAR1 = DEFI_FONCTION(INFO = INFO, NOM_PARA = 'INST',
                                VALE = (inst_ini               , PRES_CONF[i]        ,
                                        inst_ini+long_cyc/2.   , PRES_CONF[i]   +SIGM_IMPOSE[j],
                                        inst_ini+long_cyc      , SIGM_DECH[i]           ,))
                                        
      else:
        __CHAR1 = DEFI_FONCTION(INFO = INFO, NOM_PARA = 'INST',
                                VALE = (inst_ini               , SIGM_DECH[i]        ,
                                        inst_ini+long_cyc/2.   , SIGM_DECH[i]   +SIGM_IMPOSE[j],
                                        inst_ini+long_cyc      , SIGM_DECH[i]           ,))
      ...
      DETRUIRE(CONCEPT=_F(NOM = (__RLIST,__DLIST,__CHAR1,__EVOL)), INFO=1)
...
def essai_ISOT_C(self,str_n_essai,DicoEssai,MATER,COMPORTEMENT,CONVERGENCE,INFO):
  ...
  for i in xrange(len(PRES_CONF)):
    ...
    for j in xrange(len(SIGM_IMPOSE)):
      ...
      affiche_infos_essai(str_n_essai,"ISOT_C",PRES_CONF[i],SIGM_IMPOSE[j],SIGM_DECH[i])
      ...
      if j==0:
        ...
        __CHAR2 = DEFI_FONCTION(INFO = INFO, NOM_PARA = 'INST',
                                VALE = (inst_ini               , KZERO*PRES_CONF[i]        ,
                                        inst_ini+long_cyc/2.   , KZERO*PRES_CONF[i]   +SIGM_IMPOSE[j],
        ...
        __CHAR2 = DEFI_FONCTION(INFO = INFO, NOM_PARA = 'INST',
                                VALE = (inst_ini               , KZERO*SIGM_DECH[i]        ,
                                        inst_ini+long_cyc/2.   , KZERO*SIGM_DECH[i]   +SIGM_IMPOSE[j],
                                        inst_ini+long_cyc      , KZERO*SIGM_DECH[i]           ,))
