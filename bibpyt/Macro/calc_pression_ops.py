...
def calc_pression_ops(self, MAILLAGE, RESULTAT,GROUP_MA, INST, **args):
    ...
    # BLINDAGE : on poursuit le calcul uniquement que si le modèle n'est pas un élément de structure
    iret,ibid,test_structure = aster.dismoi('EXI_RDM',__model.nom,'MODELE','F')  
...
# Champ de contraintes de Cauchy aux noeuds
    ...
    # Formule en dimension 3 :
        __Pression = FORMULE(VALE='(SIXX*X*X+SIYY*Y*Y+SIZZ*Z*Z+2*SIXY*X*Y+2*SIXZ*X*Z+2*SIYZ*Y*Z)',
