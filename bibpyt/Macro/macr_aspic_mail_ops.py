...
def write_file_dgib_ASPID0(
    nomFichierDATG, UNITD, EPT1, DET1, D1, D2, EPT2, DET2, ZMAX, H,
...
def write_file_dgib_ASPID2(
    nomFichierDATG, UNITD, EPT1, DET1, D1, D2, EPT2, DET2, ZMAX,
...
#    PIQUAGE INCLINE
            ...
            PHI = (EPSIL * ZA) - YA
            DELTA = pow(PHI, 2) - ((1 + pow(EPSIL, 2)) * (
                pow(PHI, 2) - (pow((DEC / 2.0), 2) * pow(EPSIL, 2))))
            if (STHETA > 0):
            ...
            if (TFISS == 'DEB_EXT'):
                ...
                XN = XN0 * CTHETA
                YN = XN0 * STHETA
                DNXY = sqrt(pow(XD, 2) + pow(YD, 2)) - sqrt(
                    pow(XN, 2) + pow(YN, 2))
                DNXY0 = XD0 - XN0
                RAPP = DNXY / DNXY0
...
#    PIQUAGE DROIT
            ...
            EPSIL = STHETA * TGALP
            PHI = (EPSIL * ZA) - YA
            DELTA = pow(PHI, 2) - (1.0 + pow(EPSIL, 2)) * (
                pow(PHI, 2) - pow((DEC / 2.0), 2) * pow(EPSIL, 2))
            if (STHETA > 0):
                YD = (sqrt(DELTA) - PHI) / (1.0 + pow(EPSIL, 2))
            else:
...
#
    ...
    message = ' MACR_ASPIC_MAIL : X_MAX CALCULEE : %.2f  X_MAX FOURNI : %.2f\n' % (
        XMAXC, XMAX)
    message = message + \
        ' MACR_ASPIC_MAIL : Z_MAX CALCULEE : %.2f  Z_MAX FOURNI : %.2f\n' % (
            ZMAXC, ZMAX)
    aster.affiche('MESSAGE', message)
#
...
#
    ...
    if FISS_SOUDURE == None:
        ...
        if FISS_SOUDURE['TYPE'] == 'LONGUE':
            FISLON = 1
        elif FISS_SOUDURE['TYPE'] == 'COURTE':
            FISCOU = 1
        ...
        if TFISS not in ('DEB_INT', 'DEB_EXT'):
            ...
            if POSI == 'DROIT':
                ...
                if ITYPSO == 1:
                    ZETA = (A + LIGA) * cos(ALPHA * pi / 180.0) / EPC
                else:
                    ZETA = (A + LIGA) * cos(ALPHA * pi / 180.0) / EPT1
            if ZETA < 0.1:
                UTMESS('F', 'ASPIC0_16')
            if ZETA > 0.9:
                UTMESS('F', 'ASPIC0_17')
...
# Nom du fichier de maillage GIBI
    ...
    if SAIN:
        write_file_dgib_ASPID0(
            nomFichierDATG, UNITD, EPT1, DET1, D1, D2, EPT2, DET2, ZMAX, H,
            ALPHA, JEU, EPC, DEC, XMAX, TYPMAI, THETA, TYPELE,
            ITYPSO, DPENE, NIVMAG, loc_datg)
    if FISLON:
        write_file_dgib_ASPID1(
            nomFichierDATG, UNITD, EPT1, DET1, D1, D2, EPT2, DET2, ZMAX, H,
            ALPHA, JEU, EPC, DEC, XMAX, TYPMAI, THETA,
            A, C, EPS, RC0, NS, NC, NT, POSI, NDT, FETIRF, FETIRP,
            TFISS, ZETA, ITYPSO, DPENE, NIVMAG, loc_datg)
    if FISCOU:
        write_file_dgib_ASPID2(
            nomFichierDATG, UNITD, EPT1, DET1, D1, D2, EPT2, DET2, ZMAX,
            H, ALPHA, JEU, EPC, DEC, XMAX, TYPMAI,
            THETA, A, C, EPS, RC0, RC1, RC2, RC3,
            ALP, BETA, NS, NC, NT, POSI, NDT, NSDT, TFISS,
...
#
    ...
    if not SAIN:
        motscles['ORIE_PEAU_3D'] = _F(
            GROUP_MA=('PEAUINT', 'EXCORP1', 'EXCORP2', 'EXTUBU', 'LEVRTUBU', 'LEVRCORP'),)
    else:
        motscles['ORIE_PEAU_3D'] = _F(
            GROUP_MA=('PEAUINT', 'EXCORP1', 'EXCORP2', 'EXTUBU',),)
    __MAPROV = MODI_MAILLAGE(reuse=__MAPROV,
                             MAILLAGE=__MAPROV,
                             **motscles
                             )
