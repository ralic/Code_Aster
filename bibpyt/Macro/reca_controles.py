...
def erreur_dimension(PARAMETRES, REPONSES):
    ...
    for i in range(len(PARAMETRES)):
        if (len(PARAMETRES[i]) != 4):
            txt = txt + "\nLa sous-liste de la variable paramètre numéro " + \
                str(i + 1) + " n'est pas de longueur 4"
    for i in range(len(REPONSES)):
        if (len(REPONSES[i]) != 3):
            txt = txt + "\nLa sous-liste de la variable réponse numéro " + \
                str(i + 1) + " n'est pas de longueur 3"
    return txt
...
def compare__dim_rep__dim_RESU_EXP(REPONSES, RESU_EXP):
    ...
    if(len(REPONSES) != len(RESU_EXP)):
        txt = "\nVous avez entré " + str(len(REPONSES)) + " réponses et " + str(
            len(RESU_EXP)) + " expériences ; On doit avoir autant de réponses que de résultats expérimentaux"
    return txt
...
def compare__dim_poids__dim_RESU_EXP(POIDS, RESU_EXP):
    ...
    if(len(POIDS) != len(RESU_EXP)):
        txt = "\nVous avez entré " + str(len(POIDS)) + " poids et " + str(
            len(RESU_EXP)) + " expériences ; On doit avoir autant de poids que de résultats expérimentaux"
    return txt
...
def verif_valeurs_des_PARAMETRES(PARAMETRES):
    ...
    for i in range(len(PARAMETRES)):
        if(PARAMETRES[i][2] > PARAMETRES[i][3]):
            txt = txt + "\nLa borne inférieure " + str(PARAMETRES[i][2]) + " de  " + PARAMETRES[
                i][0] + "est plus grande que sa borne supérieure" + str(PARAMETRES[i][3])
    # verification de l'encadrement de val_init
    for i in range(len(PARAMETRES)):
        if((PARAMETRES[i][1] < PARAMETRES[i][2]) or (PARAMETRES[i][1] > PARAMETRES[i][3])):
            txt = txt + "\nLa valeur initiale " + str(PARAMETRES[i][1]) + " de " + PARAMETRES[i][
                0] + " n'est pas dans l'intervalle [borne_inf,born_inf]=[" + str(PARAMETRES[i][2]) + " , " + str(PARAMETRES[i][3]) + "]"
    # verification que val_init !=0
    for i in range(len(PARAMETRES)):
        if (PARAMETRES[i][1] == 0.):
            txt = txt + "\nProblème de valeurs initiales pour le paramètre " + \
                PARAMETRES[i][
                    0] + " : ne pas donner de valeur initiale nulle mais un ordre de grandeur."
    # verification que borne_sup !=0
    for i in range(len(PARAMETRES)):
        if (PARAMETRES[i][3] == 0.):
            txt = txt + "\nProblème de borne supérieure pour le paramètre " + \
                PARAMETRES[i][
                    0] + " : ne pas donner de valeur strictement nulle."
    # verification que borne_inf !=0
    for i in range(len(PARAMETRES)):
        if (PARAMETRES[i][2] == 0.):
            txt = txt + "\nProblème de borne inférieure pour le paramètre " + \
                PARAMETRES[i][
                    0] + " : ne pas donner de valeur strictement nulle."
    return txt
...
def gestion(UL, PARAMETRES, REPONSES, RESU_EXP, POIDS, GRAPHIQUE, UNITE_RESU, METHODE):
    ...
    # verification des valeurs des PARAMETRES entrées par l'utilisateur (pour
    # fmin* les bornes ne sont pas prises en compte)
