...
#       raise Exception("Le mode INCLUSION doit etre lance depuis Aster")
    ...
    try:
        ...
        if endpos != -1:
            newtxt = newtxt[endpos+1:]
        if newtxt[0] == ';':
            newtxt = newtxt[1:]  # Bug dans find_command si la commande se termine par un ";"
...
def Ecriture_Fonctionnelle(output_file, type_fonctionnelle, fonctionnelle):
    ...
    if type_fonctionnelle == 'vector':
        try:
            fonctionnelle = fonctionnelle.tolist()
        except:
            pass
...
def Ecriture_Derivees(output_file, derivees):
    ...
    # On cherche a imprimer la matrice des sensibilite (A ou A_nodim)
        ...
        raise Exception("Wrong type for gradient !")
...
#          if not reponse in liste_reponses: liste_reponses.append(reponse)
        ...
        for i in range(len(list_val)):
            ...
            if i == 0:
                UTMESS('I', 'RECAL0_67', valk=tpara)
            else:
                UTMESS('I', 'RECAL0_68', valk=(tpara, list_params[i-1]) )
    ...
    def run_distrib(self, list_val):
        ...
        for i in range(len(reponses)):
            ...
            try:
                os.remove( 'tmp_macr_recal'+os.sep+"REPE_TABLE"+os.sep+"fort."+_ul )
            except:
                pass
...
class CALC_ERROR:
    ...
    def __init__(self, experience, X0, calcul, poids=None, objective_type='vector', info=0, unite_resu=None):
        ...
        try:
            ...
            self.Dim   = reca_algo.Dimension(copy.copy(self.X0), None)  # gere l'ancienne version de MACR_RECAL
        # self.Dim   = reca_algo.Dimension(copy.copy(self.X0))
    ...
    def CalcError(self, Lcalc):
        ...
        if self.L_J_init is None:
            self.L_J_init = copy.copy(self.L_J)
