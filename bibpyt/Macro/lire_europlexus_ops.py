...
le mot-clé %s"""%mc_cara)
    ...
    def lire_champs_noeud(self, resu):
        ...
        if self.CARA_ELEM is not None: lire_resu['CARA_ELEM'] = self.CARA_ELEM
        if self.CHAM_MATER is not None: lire_resu['CHAM_MATER'] = self.CHAM_MATER
...
présentes%s"""%compo)
    ...
    def asse_champs_gauss(self, dic_champ_cont, dic_champ_var_int, dic_transfo,
        ...
        for i in xrange(itot):
            ...
            if self.CHAM_MATER is not None: dic['CHAM_MATER'] = self.CHAM_MATER
            if self.CARA_ELEM is not None: dic['CARA_ELEM'] = self.CARA_ELEM
