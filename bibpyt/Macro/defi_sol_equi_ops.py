...
# coefficient de variabilite sur le profil de modules Emax : 2/3 ou 3/2 par exemple
...
# et on calcule le signal reconvolue au Rocher Affleurant defini au toit du substratum
   if args['LIEU_SIGNAL'] == 'AFFLEURANT' :
     input = 'RA' ;
   elif args['LIEU_SIGNAL'] == 'CHAMP_LIBRE' :
     input = 'CL' ;

# Possibilite de faire une verification en temporel avec coeff Rayleigh : veriftmp ='OUI' ou 'NON'
...
# coefficient pour calcul gamma_eff / gamma_max
   geff = args['COEF_GAMMA'] ; #0.65

# tolerance pour convergence sur evolution module E
   tole = args['RESI_RELA'] ;#0.05;
   nmaxit = args['NMAX_ITER'] ; #10; # nombre maximal d'iteration
#  unites tables resu
...
# definition des frequences de calcul
   __lfreqc = DEFI_LIST_REEL(DEBUT=0.,INTERVALLE=_F(JUSQU_A=fcoup, PAS=0.1),);
...
# Definition de la fonction unite et du filtre en frequence 
   ...
   if (fmax+df) > fcoup :
     __FILTRE = DEFI_FONCTION(NOM_PARA='FREQ',
                     VALE_C=(0.,0.,0.,df,1.,0.,(fcoup-df),1.,0.,(fcoup),0.,0.,(fmax+df),0.,0.),
                     INTERPOL='LIN',PROL_DROITE = 'CONSTANT',PROL_GAUCHE = 'CONSTANT',);
   else :
     __FILTRE = DEFI_FONCTION(NOM_PARA='FREQ',
                     VALE_C=(0.,0.,0.,df,1.,0.,(fcoup-df),1.,0.,(fcoup),0.,0.),
                     INTERPOL='LIN',PROL_DROITE = 'CONSTANT',PROL_GAUCHE = 'CONSTANT',);   
...
# Lecture du maillage
   ...
   __MODELE=AFFE_MODELE(MAILLAGE=__mailla,
                   AFFE=(
                         _F(GROUP_MA=grma_colon, PHENOMENE='MECANIQUE',MODELISATION='D_PLAN',),
                         _F(GROUP_MA=grma_subst, PHENOMENE='MECANIQUE',MODELISATION='D_PLAN_ABSO',),
                            ),);
...
#for a in range (1,nbacc+1):
   ...
   __ACCEX = CALC_FONCTION ( COMB =_F ( FONCTION = fonc_acce, COEF = coefu*coefzpa ) ) ;

   __SAX = CALC_FONCTION( SPEC_OSCI=_F( FONCTION=__ACCEX,AMOR_REDUIT=0.05,NORME = 9.81))
            
   ifsax.append(_F(FONCTION = __SAX,MARQUEUR = 0, LEGENDE='SAXi_'+str(a)),);
...
#    for a in range (1,nbacc+1):
   ...
   deltaE = 100; # initialisation de l erreur sur E
   iter = 0; # indicateur du nombre d iterations
   etat = 'debut';
   ...
   # 2. dictionnaires des COUCHES
     ...
     for j in range(1,nbmat+1) :
       if j==1:      
         __tabfgg = CREA_TABLE(FONCTION=_F(FONCTION = __GG[j],PARA=('EPSI','GG'+str(j))));
         __tabfdg = CREA_TABLE(FONCTION=_F(FONCTION = __DG[j],PARA=('EPSI','DG'+str(j))));
      
       else:
         __tabtmpgg = CREA_TABLE(FONCTION=_F(FONCTION = __GG[j],PARA=('EPSI','GG'+str(j))));
         __tabtmpdg = CREA_TABLE(FONCTION=_F(FONCTION = __DG[j],PARA=('EPSI','DG'+str(j))));
   ...
   # para/typ pre-trie les colonnes
     ...
     for couche in l_couche:
       ...
       tabini.append({'RHO':couche["RHO"],'NU':couche["NU"],'Emax':couche["E"],
   ...
   if args['NIVE_COUCH_ENFO'] != None :
     ...
     NCOU2 = NCOU 
      ...
      # On genere la liste initiale pour Emax AH et ratio G/Gmax et des profondeurs
   E=[[]]*nmaxit;
   AH=[[]]*nmaxit;
   rat=[[]]*nmaxit;
   ...
   while etat <> 'fin' : 
        ...
        if iter == 0 :
          ...
          for j in range (1,NCOU+1):
            ...
            __SOLH[j] = DEFI_MATERIAU(ELAS=_F(E=__TMAT[('E'+str(iter)),j], 
                                       RHO = __TMAT['RHO',j], 
                                       NU=__TMAT['NU',j],
      ...
      # l'amortissement des FA (rho.Vs) est calcule avec l'option AMOR_MECA 
      ...
      # MATRICE D AMORTISSEMENT ELEMENTAIRE (pour prise en compte amortissement frontiere absorbante)
      ...
      #Chargement sismique monoappui sur la base du modele
        __VECASX=CALC_CHAR_SEISME(MATR_MASS =__MASSEH,DIRECTION =(1., 0., 0.,),MONO_APPUI='OUI' );
                                            
      #######################################
      # CALCUL HARMONIQUE SUR BASE PHYSIQUE #
      #######################################
      
      #On excite la base de la colonne avec un bruit blanc (dans la direction X )
        ...
        __DYNHARM=DYNA_LINE_HARM(MODELE=__MODELE,
                              ...
                              MATR_AMOR=__AMORTIH,
                              LIST_FREQ=__lfreqc,
                              EXCIT=(_F(VECT_ASSE = __VECASX,FONC_MULT=__UN,),
                                       ),
      ...
      #
        ...
        if iter==0:
         ...
         __mailla=DEFI_GROUP(reuse=__mailla,
                    MAILLAGE=__mailla,
                    CREA_GROUP_NO=_F(NOM='PN0',GROUP_NO=__TMAT['M',1],
                                     POSITION='INIT'),
                    );
                    
         __mailla=DEFI_GROUP(reuse=__mailla,
                    MAILLAGE=__mailla,
                    CREA_GROUP_NO=_F(GROUP_MA=__TMAT['M',NCOU+1],),
                    );
                    
         __mailla=DEFI_GROUP(reuse=__mailla,
                    MAILLAGE=__mailla,
                    CREA_GROUP_NO=_F(NOM='PNRA',GROUP_NO=__TMAT['M',NCOU+1],
                                     POSITION='INIT'),
                    );
                    
      #########################################################################
      
      #On recupere le signal en Champ Libre (CL) et au substratum (Bore Hole)       
        ...
        __mAHuXrCL =  CALC_FONCTION(EXTRACTION=_F(FONCTION=__AHuXrCL,PARTIE='MODULE'));
        __mAHuXrBH =  CALC_FONCTION(EXTRACTION=_F(FONCTION=__AHuXrBH,PARTIE='MODULE'));
      
      #######################################################
      #Definition de la fonction de transfert entre RA et CL #
      #######################################################
      
        self.update_const_context({'FILTRE': __FILTRE, 'AHuXrCL': __AHuXrCL, 'AHuXrBH': __AHuXrBH }) 
        __formFDT=FORMULE(NOM_PARA='FREQ',VALE_C='FILTRE(FREQ)*(1.+0.j )/(1.+0.j + AHuXrCL(FREQ))') ;
        __formFDT2=FORMULE(NOM_PARA='FREQ',VALE_C='FILTRE(FREQ)*(1.+0.j + AHuXrCL(FREQ))/(1.+0.j + AHuXrBH(FREQ))') ;
        ...
        __mFDTRACL =  CALC_FONCTION(EXTRACTION=_F(FONCTION=__FDT_RACL,PARTIE='MODULE'));
        __mFDTCLBH =  CALC_FONCTION(EXTRACTION=_F(FONCTION=__FDT_CLBH,PARTIE='MODULE'));
      ...
      #################################################################
        ...
        if input == 'CL':      
          __AHX_RA= CALC_FONCTION(MULT=(_F(FONCTION=__AHX,),_F(FONCTION=__FDT_RACL,),_F(FONCTION=__FILTRE,)),LIST_PARA=__lfreq,NOM_PARA='FREQ',);         
      
        if input == 'RA':          
          __AHX_RA= CALC_FONCTION(MULT=(_F(FONCTION=__AHX,),_F(FONCTION=__FILTRE,)),LIST_PARA=__lfreq,NOM_PARA='FREQ',);      
      ...
      # Calcul des accelerogrammes et spectres de sol au RA
        ...
        __AX_RA= CALC_FONCTION(COMB =_F( FONCTION = __AX_RAf , COEF = 1.),LIST_PARA=__linst, 
                              INTERPOL='LIN',
                              PROL_DROITE='CONSTANT', PROL_GAUCHE='CONSTANT', );
        DETRUIRE (CONCEPT=_F(NOM = __AX_RAf,));
      
        __SAX_RA = CALC_FONCTION( SPEC_OSCI=_F( FONCTION=__AX_RA,AMOR_REDUIT=0.05,NORME = 9.81));
                                    
      
      # Calcul des accelerogrammes et spectres de sol en CL
      
        __AHXrCL = CALC_FONCTION(MULT=(_F(FONCTION=__AHX_RA,),_F(FONCTION=__AHuXrCL,),_F(FONCTION=__FILTRE,)),
                LIST_PARA=__lfreq,NOM_PARA='FREQ',);
                
        __AXrCL = CALC_FONCTION(FFT=_F(FONCTION=__AHXrCL,METHODE='COMPLET',SYME='NON',));
          
        __AX_CL = CALC_FONCTION(COMB=(_F(FONCTION=__AXrCL,COEF=1.,),_F(FONCTION=__AX_RA,COEF=1.,),),LIST_PARA=__linst,);
                                   
        __SAX_CL = CALC_FONCTION( SPEC_OSCI=_F( FONCTION=__AX_CL,AMOR_REDUIT=0.05,NORME = 9.81));
      
      # Calcul des contraintes et deformations dans tous les elements de la colonne
      ...
      # Boucle de postraitement
        ...
        for k in range(1,NCOU+1):
          if iter==0:
           ...
           __mailla=DEFI_GROUP(reuse=__mailla,
                    MAILLAGE=__mailla,
                    CREA_GROUP_NO=_F(NOM=('PN'+str(k)),GROUP_NO=__TMAT['M',k+1],
                                     POSITION='INIT'),
                    );              
                                  
          #print ('legende = '+legende + 'deltaE ='+str(deltaE));
          
          __fthr = RECU_FONCTION( GROUP_NO=('PN'+str(k)), RESULTAT=__DYNHARM,NOM_CHAM='ACCE',NOM_CMP='DX',INTERPOL='LIN',);
       
          __axhr = CALC_FONCTION(MULT=(_F(FONCTION=__AHX_RA,),_F(FONCTION=__fthr,),_F(FONCTION=__FILTRE,)),LIST_PARA=__lfreq,NOM_PARA='FREQ',);
          
          __axr = CALC_FONCTION(FFT=_F(FONCTION=__axhr,METHODE='COMPLET',SYME='NON',));
                    
          __axa[k] = CALC_FONCTION(COMB=(_F(FONCTION=__axr,COEF=1.,),_F(FONCTION=__AX_RA,COEF=1.,),),LIST_PARA=__linst,);
          
       
          __ftep = RECU_FONCTION( GROUP_MA=__TMAT['M',k], RESULTAT=__DYNHARM,NOM_CHAM='EPSI_ELGA',POINT=1,NOM_CMP='EPXY',INTERPOL='LIN',);
          __eph = CALC_FONCTION(MULT=(_F(FONCTION=__AHX_RA,),_F(FONCTION=__ftep,),_F(FONCTION=__FILTRE,)),LIST_PARA=__lfreq,NOM_PARA='FREQ',);
          __epxy[k] = CALC_FONCTION(FFT=_F(FONCTION=__eph,METHODE='COMPLET',SYME='NON',));
          
                     
         # Calcul de la distorsion : gamma = 2*epxy ; tau = sixy
          __gam[k] = CALC_FONCTION(LIST_PARA=__linst,COMB=(_F(FONCTION=__epxy[k],COEF=(2),),
                                                       ),);
          # Calcul des max
...
#       
        ...
        for k in range(1,NCOU+1):  
          ...
          diff.append(abs(((E[iter-1][k])-(E[iter][k])))/(E[iter-1][k]));
        ...
        if deltaE > tole : 
          ...
          if iter == (nmaxit-1) :
            ...
            __TMAT = CALC_TABLE(reuse = __TMAT, TABLE = __TMAT,
                  ACTION=(_F(OPERATION = 'COMB', TABLE = __TEnew,NOM_PARA='Y'),
                          _F(OPERATION = 'COMB', TABLE = __TAHnew,NOM_PARA='Y'),
                       ));
            ...
            for k in range(1,NCOU+1):          
              DETRUIRE (CONCEPT=_F(NOM = (__SOLH[k],__axa[k],__epxy[k],__gam[k]),),INFO = 1);
        ...
        if etat == 'fin' :
          ...
          self.update_const_context({'ca': ca, 'aamult': aamult, 'abmult': abmult })
          __fAB = FORMULE(NOM_PARA = ('AHfin'),VALE = 'ca*abmult*AHfin')
          __fAA = FORMULE(NOM_PARA = ('AHfin'),VALE = 'ca*aamult*AHfin')
      
          __TMAT = CALC_TABLE(reuse = __TMAT, TABLE = __TMAT,
                  ACTION=(
                          _F(OPERATION = 'COMB', TABLE = __tabred,NOM_PARA='Y'),
                          _F(OPERATION = 'COMB', TABLE = __tabgam,NOM_PARA='Y'),
                          _F(OPERATION = 'COMB', TABLE = __tabacc,NOM_PARA='Y'),
                          _F(OPERATION = 'RENOMME', NOM_PARA = (('E'+str(iter-1)),'Efin'),),
                          _F(OPERATION = 'RENOMME', NOM_PARA = (('AH'+str(iter-1)),'AHfin'),),
                          _F(OPERATION = 'OPER',FORMULE=__fAA, NOM_PARA = 'AAfin',),
                          _F(OPERATION = 'OPER',FORMULE=__fAB, NOM_PARA = 'ABfin',),
                        ));
        
          DETRUIRE (CONCEPT=_F(NOM = (__tabred,__tabgam,__tabacc,__fAA,__fAB),));
      
      # Recuperation des historiques d acceleration et deformation dans chaque couche
          ...
          if v == 1:
            if s == 1 :
              if a == 1:
                __tabaccx = CREA_TABLE(FONCTION=_F(FONCTION = __AX_CL,PARA=('INST','AX_CL')),);
                __tabAXRA = CREA_TABLE(FONCTION=_F(FONCTION = __AX_RA,PARA=('INST','AX_RA')),);
  
                __tabadec = CALC_TABLE(TABLE = __tabaccx,
                  ACTION=(_F(OPERATION = 'COMB', TABLE = __tabAXRA,NOM_PARA='INST'),
                          _F(OPERATION = 'RENOMME', NOM_PARA = ('AX_CL','CL'+legendeT),),
                          _F(OPERATION = 'RENOMME', NOM_PARA = ('AX_RA','RA'+legendeT),),
                        ));
          ...
          for k in range(1,NCOU+1) :
            if k==1:
              ...
              __tabgam = CREA_TABLE(FONCTION=_F(FONCTION = __gam[k],PARA=('INST','GAM'+str(k))));
  
              __tabatmp = CREA_TABLE(FONCTION=_F(FONCTION = __axa[k],PARA=('INST','ACCE'+str(k))));
              __tabaccx = CALC_TABLE(TABLE = __tabaccx,reuse=__tabaccx,
                          ACTION=_F(OPERATION = 'COMB', TABLE = __tabatmp,NOM_PARA='INST'),);
              
              __SPEC[k] = CALC_FONCTION( SPEC_OSCI=_F( FONCTION=__axa[k],AMOR_REDUIT=0.05,NORME = 9.81))

              DETRUIRE (CONCEPT=_F(NOM = (__tabatmp),));
      
            else: 
              __tabatmp = CREA_TABLE(FONCTION=_F(FONCTION = __axa[k],PARA=('INST','ACCE'+str(k))));
              __tabgtmp = CREA_TABLE(FONCTION=_F(FONCTION = __gam[k],PARA=('INST','GAM'+str(k))));

              __SPEC[k] = CALC_FONCTION( SPEC_OSCI=_F( FONCTION=__axa[k],AMOR_REDUIT=0.05,NORME = 9.81))
  
                        
              __tabaccx = CALC_TABLE(reuse = __tabaccx, TABLE = __tabaccx,
                        ACTION=_F(OPERATION = 'COMB', TABLE = __tabatmp,NOM_PARA='INST'));
          ...
          IMPR_FONCTION(  UNITE = 8,
             ...
             COURBE = (
             _F(FONCTION = __mFDTRACL   ,MARQUEUR = 0.,LEGENDE ='FdT RA/CL'+legende+ 'deltaE ='+str(deltaE),),
             _F(FONCTION = __mFDTCLBH   ,MARQUEUR = 0.,LEGENDE ='FdT CL/BH'+legende+ 'deltaE ='+str(deltaE),),
              ))
          ...
          ifspec.append(_F(FONCTION = __SAX_CL,MARQUEUR = 0, LEGENDE='SAX_CL'+legende+ 'deltaE ='+str(deltaE)),);
          ifspec.append(_F(FONCTION = __SAX_RA,MARQUEUR = 0, LEGENDE='SAX_RA'+legende+ 'deltaE ='+str(deltaE)),);
          
          for d in range(1,NCOU+1):
            ifspec.append(_F(FONCTION = __SPEC[d],MARQUEUR = 0, LEGENDE='SAX_'+str(d)+legende+ 'deltaE ='+str(deltaE)),);         
          #print ifspec;
      ...
      # Destruction de tous les concepts pour permettre une utilisation en INCLUDE dans boucle
          ...
          if veriftmp == 'OUI' :
            ...
            for j in range (1,NCOU+1):
              ...
              __SOLR[j] = DEFI_MATERIAU(ELAS=_F(E=__TMAT['Efin',j], 
                                       RHO = __TMAT['RHO',j], 
                                       NU=__TMAT['NU',j],
                                       AMOR_ALPHA=__TMAT['AAfin',j],
                                       AMOR_BETA =__TMAT['ABfin',j],
                              ));
                              
            __SOLR[NCOU+1] = DEFI_MATERIAU(ELAS=_F(E=__TMAT['E0',NCOU+1], 
                                       RHO = __TMAT['RHO',NCOU+1], 
                                       NU=__TMAT['NU',NCOU+1],
                              ));
      ...
      # Boucle pour affectation du materiau j au GROUP_MA 'Mj'
            ...
            __RIGIDITE=ASSE_MATRICE(MATR_ELEM=__RIGI_ELR,NUME_DDL=__NUMEDDLR,);
            ...
            __MASSER=ASSE_MATRICE(MATR_ELEM=__MASS_ELR,NUME_DDL=__NUMEDDLR,);
            ...
            __AMORTIR=ASSE_MATRICE(MATR_ELEM=__AMOR_ELR,NUME_DDL=__NUMEDDLR,);                   
                              
            __VECASXR=CALC_CHAR_SEISME(MATR_MASS =__MASSER,DIRECTION =(1., 0., 0.,),MONO_APPUI='OUI' );
            ...
            __AX_CLv = CALC_FONCTION(COMB=(_F(FONCTION=__AXrCLv,COEF=1.,),_F(FONCTION=__AX_RA,COEF=1.,),),LIST_PARA=__linst,);
                                   
            __SAX_CLv = CALC_FONCTION( SPEC_OSCI=_F( FONCTION=__AX_CLv,AMOR_REDUIT=0.05,NORME = 9.81))
          ...
          for k in range(1,NCOU+1):
            DETRUIRE (CONCEPT=_F(NOM = (__SPEC[k],__axa[k],__epxy[k],__gam[k],)),INFO = 1);
   ...
   if SURF == 'OUI' :
      ...
      for k in range(1,NCOU+1):
        if k > 1 :
          ic = ic+1
          tab.append({'RHO':__TMAT['RHO',k],'NU':__TMAT['NU',k],'E':__TMAT['Efin',k],
      ...
      ic = ic+1
      tab.append({'RHO':__TMAT['RHO',ic],'NU':__TMAT['NU',ic],'E':__TMAT['E0',ic],
      ...
      if NCOU2 < NCOU :
        ...
        ic = ic+1
        k = NCOU+1
        tab.append({'RHO':__TMAT['RHO',k],'NU':__TMAT['NU',k],'E':__TMAT['E0',k],
                    'NUME_COUCHE':ic,'NUME_MATE':k,
                    'EPAIS':(__TMAT['Y',k]-__TMAT['Y',k-1]),'SOURCE':'NON',
