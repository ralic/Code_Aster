...
def macr_spectre_ops(self,MAILLAGE,PLANCHER,NOM_CHAM,CALCUL,RESU,IMPRESSION=None,
    ...
    if NOM_CHAM=='DEPL' : dico_glob={'DX_max':[], 'DY_max':[], 'DZ_max':[], 'DH_max':[], }
    #
    # ---------------------------------------------------------------------------------------------
        ...
        for node in planch_nodes[plancher] :
            ...
            for dd in ('X','Y','Z') :
                ...
                if NOM_CHAM=='ACCE' :
                    ...
                    mcfCMBz=[]
                    for spo in l_fonc :
                        mcfCMBx.append(_F(FONCTION=spo, COEF=1./float(nbresu),))
                        mcfCMBy.append(_F(FONCTION=spo, COEF=1./float(nbresu),))
                        mcfCMBz.append(_F(FONCTION=spo, COEF=1./float(nbresu),))
                    motscles={}
            ...
            if NOM_CHAM=='ACCE' and IMPRESSION!=None :
                if IMPRESSION['TOUT']=='OUI' :
                    ...
                    for i in range(len(AMOR_SPEC)) :
                        amor = AMOR_SPEC[i]
                        __moyxa[i]=RECU_FONCTION(NAPPE = __moy_x[indexn], VALE_PARA_FONC = AMOR_SPEC[i] )
                        __moyya[i]=RECU_FONCTION(NAPPE = __moy_y[indexn], VALE_PARA_FONC = AMOR_SPEC[i] )
                        __moyza[i]=RECU_FONCTION(NAPPE = __moy_z[indexn], VALE_PARA_FONC = AMOR_SPEC[i] )
                    motscles={}
                    ...
                    if IMPRESSION['TRI']=='AMOR_SPEC' :
                        for i in range(len(AMOR_SPEC)) :
                            TITRE   ='Spectres / Plancher = '+plancher+' / amor='+str(AMOR_SPEC[i])+' / noeud='+node
                            IMPR_FONCTION(
                        ...
                        lfonc=[]
                        for dd in ('X','Y','Z') :
                            TITRE ='Spectres / Plancher = '+plancher+' / direction = '+dd+' / noeud = '+node
                            legende = 'amor='+str(AMOR_SPEC[i])
                            if dd=='X' :
                                l_fonc=[_F(FONCTION=__moyxa[i], LEGENDE = legende,) for i in range(len(AMOR_SPEC)) ]
                            if dd=='Y' :
                                l_fonc=[_F(FONCTION=__moyya[i], LEGENDE = legende,) for i in range(len(AMOR_SPEC)) ]
                            if dd=='Z' :
                                l_fonc=[_F(FONCTION=__moyza[i], LEGENDE = legende,) for i in range(len(AMOR_SPEC)) ]
                            IMPR_FONCTION(
                                FORMAT=IMPRESSION['FORMAT'],
                                UNITE =IMPRESSION['UNITE' ],
                                COURBE=l_fonc,
        ...
        if   NOM_CHAM=='ACCE' and IMPRESSION!=None :
            ...
            if IMPRESSION['TRI']=='AMOR_SPEC' :
                ...
                for dd in ('X','Y','Z','H'):
                    ...
                    l_fonc  =[]
                    if dd=='X' :
                        l_fonc=[_F(FONCTION=__snxa[i], LEGENDE = legende,) for i in range(len(AMOR_SPEC)) ]
                    if dd=='Y' :
                        l_fonc=[_F(FONCTION=__snya[i], LEGENDE = legende,) for i in range(len(AMOR_SPEC)) ]
                    if dd=='Z' :
                        l_fonc=[_F(FONCTION=__snza[i], LEGENDE = legende,) for i in range(len(AMOR_SPEC)) ]
                    if dd=='H' :
                        l_fonc=[_F(FONCTION=__snha[i], LEGENDE = legende,) for i in range(len(AMOR_SPEC)) ]
                    IMPR_FONCTION(
                        FORMAT=IMPRESSION['FORMAT'],
                        UNITE =IMPRESSION['UNITE' ],
                        COURBE=l_fonc,
