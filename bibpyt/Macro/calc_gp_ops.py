...
def Crea_grp_ma(maya,C_k):
   ...
   if C_k==0:
       ...
       dicma={'NOM':'Mai_Plan', 'OPTION':'APPUI', 'GROUP_NO':'Cop_Pl', 'TYPE_APPUI':'TOUT', 'TYPE_MAILLE':'2D'}
       ...
       dicma={'NOM':'Mai_Pla1', 'OPTION':'APPUI', 'GROUP_NO':'Cop_Pl', 'TYPE_APPUI':'TOUT'}
       DEFI_GROUP(reuse =maya, MAILLAGE=maya, CREA_GROUP_MA=dicma),

       dicma={'NOM':'Mai_Plan'}
       DEFI_GROUP(reuse =maya, MAILLAGE=maya, DETR_GROUP_MA=dicma),

       dicma={'NOM':'Mai_Plan', 'DIFFE':('Mai_Pla1','Mai_Pla2'), 'TYPE_MAILLE':'2D'}
       DEFI_GROUP(reuse =maya, MAILLAGE=maya, CREA_GROUP_MA=dicma),
...
def Calcul_mesure_3D(maya,nbcop,l_copo_tot,ltyma,nd_fiss,normale):
   ...
   dicno={'NOM':'Nds_Plan', 'OPTION':'PLAN', 'VECT_NORMALE':normale,'PRECISION':1e-6, 'NOEUD_CENTRE':nd_fiss}
   DEFI_GROUP(reuse=maya, MAILLAGE=maya, CREA_GROUP_NO=dicno);
   ...
   for C_k,Copeau_k in enumerate(l_copo_tot) :
       ...
       Coord_nds = [(COOR[(connexe[x]-1)*3],COOR[(connexe[x]-1)*3+1],COOR[(connexe[x]-1)*3+2]) for x in range(4)]
