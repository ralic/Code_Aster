...
def CLASS_MODES(self, L_MODES, NFREQ, NFREQ_CAMP, L_GR_NOEUD, VITE_ROTA):
    ...
    RESULT = [NFREQ_f, NFREQ_t, NFREQ_l, __Mf, __Mt, __Ml,
              NVT, NVT_f, NVT_t, NVT_l, NFREQ_fc, NFREQ_tc, NFREQ_lc]
...
def EXTR_FREQ(self, L_MODES, L_MODEf, L_MODEt, L_MODEl, NFREQ, NFREQ_f, NFREQ_t, NFREQ_l):
    ...
    for ii in range(NBV):
        ...
        # frequences totales
        __tfreq = RECU_TABLE(
            CO=L_MODES[ii], NOM_PARA=('NUME_MODE', 'FREQ', 'AMOR_REDUIT'),)
        ...
        if NFREQ_f > 0:
            ...
            __tfr_f = RECU_TABLE(
                CO=L_MODEf[ii], NOM_PARA=('FREQ', 'AMOR_REDUIT'),)
...
def sup_redon_listv(LS):
    ...
    # Liste de vitesses initiale sans rangement
    LS.sort()
    LS_min = min(LS)
    LS_max = max(LS)
    if LS_min < 0:
