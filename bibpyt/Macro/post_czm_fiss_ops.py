...
# ---------------------------------------------------------------------------
#  POST_CZM_FISS CALCUL DE LA LONGUEUR DES FISSURES COHESIVES 2D
#
...
#########################################################################################
def post_czm_fiss_ops(self,RESULTAT,GROUP_MA,POINT_ORIG,VECT_TANG,**args):
  """Corps de POST_CZM_FISS"""
  
  MasquerAlarme('CALCULEL3_40')
  ier=0
...
# Vecteur des points de gauss (qui va du point min au point max) et sa norme
  vx = xmax-xmin
  vy = ymax-ymin
  nv = (vx**2 + vy**2)**0.5
...
# angle entre le vecteur des points de gauss et le vecteur donne par l'utilisateur
  alpha=acos((xdir*vx + ydir*vy)/(ndir*nv))

# petit parametre de tolerence
  eps = 0.0001
# cas ou le point de reference n'est pas aligne avec les points de Gauss
  if (abs(yref - A*xref - B)>=eps):
      UTMESS('F','POST0_45',valk=list(GROUP_MA),valr=(xmin,xmax,ymin,ymax))
       
# cas ou le vecteur n'est pas colineaire a la droite des points de Gauss
  if (abs(alpha)>=eps) and (abs(alpha-pi)>=eps):
      UTMESS('F','POST0_46',valk=list(GROUP_MA),valr=(xmin,xmax,ymin,ymax))

# Calcul de la distance signee des points de Gauss au point de reference
...
# Verification qu'il y a bien des points de Gauss sur la demi-droite definie par l'utilisateur
      if (cpt0 + cpt1 + cpt2 == 0):
