...
class Message:
    ...
    def affiche_result_iter(self, iter, J, val, residu, Act=[], ecart_para=None, ecart_fonc=None):
        ...
        if ecart_para:
            UTMESS('I', 'RECAL0_37', valr=ecart_para,
                   files=self.get_filename())
        if ecart_fonc:
            UTMESS('I', 'RECAL0_38', valr=ecart_fonc,
                   files=self.get_filename())
    ...
    def affiche_calcul_etat_final(self, para, Hessien, valeurs_propres, vecteurs_propres, sensible, insensible):
        ...
        """
        UTMESS('I', 'RECAL0_60', valk=str(
            valeurs_propres), files=self.get_filename())
        UTMESS('I', 'RECAL0_61', valk=str(
            vecteurs_propres), files=self.get_filename())
        ...
        if (len(sensible) != 0):
            ...
            for i in sensible:
                ...
                UTMESS('I', 'RECAL0_65', valk=(
                    txt, str(valeurs_propres[i])), files=self.get_filename())
