...
#
    if DSF:
        ...
        if (GEOM == 'COUDE'):
            ...
            SFP = SF
    if (GEOM == 'COUDE'):
        echo_mess.append('COTE AXIALE CENTRE FISSURE SUR COUDE : %.2f \n' % SF)
    if (GEOM == 'TUBE'):
        echo_mess.append('COTE AXIALE CENTRE FISSURE SUR TUBE  : %.2f \n' % SF)
...
#
    if (GEOM == 'COUDE'):
        DIST = ALPHA * 2.0 * pi / 360.0 * (RC + X * cos(AZIMR))
    else:
        DIST = ALPHA * 2.0 * pi / 360.0 * RC
    BCOUD = 0.0
    BEMB = 0.0
    if abs(ORIEN) < 0.01:
...
# -- FISSURE LONGITUDINALE (0 DEGRE)
        if GEOM == 'COUDE':
            AXECP = BCOUD / (1.0 + X / RC * cos(AZIMR)) + BEMB
        else:
            AXECP = BCOUD + BEMB
    elif abs(ORIEN - 90.) < 0.01:
# -- FISSURE CIRCONFERENTIELLE (90 DEGRES)
...
#
    if GEOM == 'COUDE':
        echo_mess.append(
            'TAILLE GRAND AXE COUDE DONNE : %.2f \n' % (2. * AXEC))
    elif GEOM == 'TUBE':
        echo_mess.append(
            'TAILLE GRAND AXE TUBE  DONNE : %.2f \n' % (2. * AXEC))
    ...
    if GEOM == 'COUDE' and BEMB > 0. and BSUP > DIST:
        ...
        echo_mess.append(
            'ABSC. CURV. AXIALE CENTRE FISSURE SUR PLAQUE : %.2f \n' % SFP)
...
#
    ...
    if (AZIM >= 120.) and (AZIM <= 240.):
        ...
        if (SFP >= ZSUR1) and (SFP <= ZSUR2):
            ...
            MU = YFISS * SFP / (pi / 6. * RM * ZSUR1)
        elif (SFP > ZSUR2):
            MU = YFISS * (ALPHAR * RC - SFP) / (pi / 6. * RM * ZSUR1)
    elif (AZIM >= 240.) and (AZIM <= 270.):
        if (SFP >= ZSUR1) and (SFP <= ZSUR2):
            MU = (YFISS - 5. * pi / 6. * RM) / (pi / 6. * RM)
        elif (SFP <= ZSUR1):
            MU = (YFISS - 5. * pi / 6. * RM) * SFP / (pi / 6. * RM * ZSUR1)
        elif (SFP > ZSUR2):
            MU = (YFISS - 5. * pi / 6. * RM) * \
                (ALPHAR * RC - SFP) / (pi / 6. * RM * ZSUR1)
#
    if SUREP != 0.:
...
#
        if ssep['TYPE'] == 'AXIS':
            ...
            echo_mess.append(
                'CALCUL DE LA TAILLE LONGI ENVELOPPE EN INTRADOS (AZIMUT PI)\n')
            ssep.ICIRP = 2. * pi * RM
            ssep.ISCP = pi * RM
            ssep.IPHIC = 180.
            AZIMC = pi
...
#
            if ssep['POSI_CURV_CIRC'] != None:
                ...
                AZIMC = ssep.ISCP / RM
                ssep.IPHIC = ssep['POSI_CURV_CIRC'] / (
                    RM + EP / 2.) * 180. / pi
                echo_mess.append(
                    'AZIMUT CENTRE SOUS-EPAISSEUR (DEGRES) : %.2f \n' % ssep.IPHIC)
            else:
                ssep.ISCP = ssep['AZIMUT'] * pi * RM / 180.
                AZIMC = ssep['AZIMUT'] * pi / 180.
                echo_mess.append(
                    'ABSC. CURV. CIRCONF. CENTRE SOUS-EPAISSEUR SUR COUDE : %.2f \n' % (AZIMC * (RM + EP / 2.)))
#
#    PASSAGE DANS LE REPERE PLAQUE (0,2PI) AVEC ORIGINE FLANC DROIT
#    CAR L ORIGINE DES DONNEES CIRCONF. EST EN EXTRADOS
#
            if ssep.ISCP > (3. * pi * RM / 2.):
                ssep.ISCP = ssep.ISCP - 3. * pi * RM / 2.
            else:
                ssep.ISCP = ssep.ISCP + pi * RM / 2.
            echo_mess.append(
                'ABSC. CURV. CIRCONF. CENTRE SOUS-EPAISSEUR SUR PLAQUE : %.2f \n' % ssep.ISCP)
#
...
#
            ...
            if ssep.ICIRP > (2. * pi * RM):
                texte_final = string.join(echo_mess)
                aster.affiche("MESSAGE", texte_final)
                UTMESS('F', 'ASCOUF0_6', vali=[
                       i], valr=[ssep.ICIRP, 2 * pi * RM])
            echo_mess.append(
                'TAILLE CIRCONFERENTIELLE SOUS-EPAISSEUR SUR PLAQUE : %.2f \n' % ssep.ICIRP)
        ...
        if ssep['POSI_CURV_LONGI'] != None:
            if GEOM == 'COUDE':
                ssep.ISLP = ssep['POSI_CURV_LONGI'] / (
                    1. + (RM + EP / 2.) / RC * cos(AZIMC))
                AZIML = ssep.ISLP / RC
                echo_mess.append(
                    'ANGLE COUDE CENTRE SOUS-EPAISSEUR (DEGRES) : %.2f \n' % (AZIML * 180. / pi))
            else:
                ssep.ISLP = ssep['POSI_CURV_LONGI']
            ...
            if GEOM == 'COUDE':
                ...
                echo_mess.append(
                    'ABSC. CURV. AXIALE CENTRE SOUS-EPAISSEUR SUR TUBE  : %.2f \n' % ((ssep.BETA) * CG * RC))
            ...
            if (SYME in ('QUART', 'DEMI')) and (ssep.BETA != ALPHA / 2.):
                ...
                UTMESS('F', 'ASCOUF0_7')
        echo_mess.append(
            'ABSC. CURV. AXIALE CENTRE SOUS-EPAISSEUR SUR PLAQUE : %.2f \n' % ssep.ISLP)
#
...
#
        if GEOM == 'COUDE':
            DIST = ALPHA * CG * (RC + (RM + EP / 2.) * cos(AZIMC))
        else:
            DIST = ALPHA * CG * RC
        if ssep['POSI_CURV_LONGI'] != None:
            ...
            if GEOM == 'COUDE':
                BSUP = ssep.BETA * CG * \
                    (RC + (RM + EP / 2.) * cos(AZIMC)) + ssep['AXE_LONGI'] / 2.
                BINF = ssep.BETA * CG * \
                    (RC + (RM + EP / 2.) * cos(AZIMC)) - ssep['AXE_LONGI'] / 2.
            else:
        ...
        BEMB = BEMB1 + BEMB2
        if GEOM == 'COUDE':
            BPLAQ = BCOUD / (1. + (RM + EP / 2.) / RC * cos(AZIMC))
        else:
            BPLAQ = BCOUD
        ...
        if BEMB1 > 0.:
            ...
            echo_mess.append(
                'CORRECTION CENTRE : SOUS-EP. A CHEVAL SUR EMBOUT \n')
            echo_mess.append(
                'ABSC. CURV. AXIALE CENTRE SOUS-EPAISSEUR SUR PLAQUE : %.2f \n' % ssep.ISLP)
        if BEMB2 > 0.:
        ...
        if ssep.ISLP < 0.:
            ssep.ISLP = 0.
        if (ssep.ISLP > ALPHA * CG * RC):
            ssep.ISLP = ALPHA * CG * RC
#
...
#
        echo_mess.append(
            'TAILLE LONGITUDINALE SOUS-EPAISSEUR SUR PLAQUE : %.2f \n' % ssep.ILONP)
...
#
    ...
    for ssep in MCL_SOUS_EPAIS:
        ...
        if (ssep.ISCP > 2. * pi * RM) or (ssep.ISCP < 0.):
            texte_final = string.join(echo_mess)
            aster.affiche("MESSAGE", texte_final)
            UTMESS('F', 'ASCOUF0_9', vali=[
                   MCL_SOUS_EPAIS.index(ssep)], valr=[ssep.ISCP, 2. * pi * RM])
...
#
    ...
    for ssep in MCL_SOUS_EPAIS:
        ...
        if (ssep.ISLP > ALPHAR * RC) or (ssep.ISLP < 0.):
            texte_final = string.join(echo_mess)
            aster.affiche("MESSAGE", texte_final)
            UTMESS('F', 'ASCOUF0_10', vali=[
                   MCL_SOUS_EPAIS.index(ssep)], valr=[ssep.ISLP, ALPHAR * RC])
...
#
    ...
    for bid in TAMPON:
        ...
        if fabs(bid[0]) < EPS:
            ...
            YS = MCL_SOUS_EPAIS[bid[1] - 1].ILONP - (
                MCL_SOUS_EPAIS[bid[1] - 1]['AXE_LONGI']) / 2.
        if fabs(bid[0] - ALPHAR * RC) < EPS:
            YI = ALPHAR * RC - \
                (MCL_SOUS_EPAIS[bid[1] - 1].ILONP - (
                    MCL_SOUS_EPAIS[bid[1] - 1]['AXE_LONGI']) / 2.)
        ...
        if YS > (ALPHAR * RC + LTCLIM):
            texte_final = string.join(echo_mess)
            aster.affiche("MESSAGE", texte_final)
            UTMESS('F', 'ASCOUF0_11', vali=[
                   bid[1]], valr=[YI, ALPHAR * RC + LTCLIM])
        COORYI.append(YI)
        COORYS.append(YS)
#
# tri des bornes d'intervalles en abscisse
...
def ASCNBE(
    MCL_SOUS_EPAIS, COORXG, COORXD, COORYI, COORYS, BD, BG, BS, BI, DNX, DNY, RM, RC,
...
#
    ...
    echo_mess.append('\n')
    echo_mess.append(
        'DETERMINATION DU NOMBRE D''ELEMENTS DANS LES SOUS-EPAISSEURS :\n')
    echo_mess.append(
        '------------------------------------------------------------\n')
...
#      calcul au passage du nombre d'elements sur chaque zone circonf.
            ...
            if RNBEL2 <= 0.5:
                NBEL = int(floor(RNBEL))
            else:
                NBEL = int(floor(RNBEL)) + 1
            if NBEL <= 1:
                NBEL = 2
# calcul au passage du nombre d'elements sur chaque sous-epaisseur
# circonf.
            for i in range(len(MCL_SOUS_EPAIS)):
                ...
                if ((COORXG[l] < COORXD[l] and BG[j] >= COORXG[l] and BD[j] <= COORXD[l])
                   or (COORXG[l] >= COORXD[l] and (BG[j] <= COORXG[l] or BD[j] >= COORXD[l]))):
                    NLX[i] = NLX[i] + NBEL
                    echo_mess.append(
                        'SOUS-EP NO %d ZONE CIRC. NO %d NB ELEM. = %d \n' % (i + 1, j + 1, NBEL))
...
# circonf.
            ...
            for i in range(len(MCL_SOUS_EPAIS)):
                ...
                if (BI[j] >= COORYI[l] and BS[j] <= COORYS[l]):
                    NLY[i] = NLY[i] + NBEL
                    echo_mess.append(
                        'SOUS-EP NO %d ZONE LONGI. NO %d NB ELEM. = %d \n' % (i + 1, j + 1, NBEL))

    for j in range(len(NLX)):
        echo_mess.append(
            'SOUS-EP NO %d NBE TOTAL ELEMENTS CIRCONF. : %d \n' % (j + 1, NLX[j]))
        echo_mess.append(
            'SOUS-EP NO %d NBE TOTAL ELEMENTS LONGI.   : %d \n' % (j + 1, NLY[j]))
...
# tri des donnees sous-epaisseurs en axial
    TAMPON, IORDO1, IORDO2, COORYI, COORYS = ASCTLO(
        MCL_SOUS_EPAIS, RC, ALPHA, LTCHAR, LTCLIM)
...
# calcul du nombre d'elements longi. et circonf. dans les soue-ep
    NLX, NLY = ASCNBE(
        MCL_SOUS_EPAIS, COORXG, COORXD, COORYI, COORYS, BD, BG, BS, BI,
...
#  un centre de sous-ep.
    if fabs(BG[NZONEX - 1] - BD[NZONEX - 1]) < EPSI:
        NZONEX = NZONEX - 1
...
def write_file_dgib_ASCFDO(
    nomFichierDATG, RM, RC, ALPHA, NBTRAN, EP1, EP2, EPI, TETA1,
    ...
    texte = texte + \
        open(os.path.join(loc_datg, 'ascouf_fiss_v4.datg'), 'r').read()
...
def write_file_dgib_ASCSQO(
    nomFichierDATG, TYPELE, RM, RC, ALPHA, NBTRAN, EP1, EP2,
    ...
    texte = texte + ' option dime 3 elem ' + \
        TYPELE + ' nive nivmag echo 0' + POIVIR
    texte = texte + '*\n'
    ...
    texte = texte + ' azimc = ' + \
        str(AZIMC) + POIVIR
    texte = texte + ' tysep = ' + \
        str(ssep.ICIRP) + POIVIR
    texte = texte + ' tzsep = ' + \
        str(ssep.ILONP) + POIVIR
    texte = texte + ' prof .                      1  = ' + \
        str(ssep['PROFONDEUR']) + POIVIR
    texte = texte + ' ycsep = ' + \
        str(SCP - pi * RM) + POIVIR
    texte = texte + ' theta = ' + \
        str(ssep.IPHIC) + POIVIR
    texte = texte + ' zcsep = ' + \
        repr(ssep.ISLP) + POIVIR

    texte = texte + " posit .                      1  = '" + \
        str(ssep['SOUS_EPAIS'].lower()) + "'" + POIVIR
    texte = texte + ' nby   = ' + \
        str(int(NLX[0])) + POIVIR
    texte = texte + ' nbz   = ' + \
        str(int(NLY[0])) + POIVIR
    texte = texte + ' nbxse = ' + \
        str(ssep['NB_ELEM_RADI']) + POIVIR
    texte = texte + ' axelonc .                      1  = ' + \
        str(ssep['AXE_LONGI']) + POIVIR
    if ssep['POSI_CURV_LONGI'] != None:
        texte = texte + ' coorzc .                      1  = ' + \
            repr(ssep['POSI_CURV_LONGI']) + POIVIR
    else:
        DZC = ssep.BETA * pi * (RC + (RM + EP1 / 2.) * cos(pi / 2.)) / 180.
        texte = texte + \
            ' coorzc .                      1  = ' + repr(DZC) + POIVIR
    if ssep['TYPE'] == 'AXIS':
    ...
    texte = texte + '* FIN PARAMETRES UTILISATEUR \n'
    texte = texte + \
        open(os.path.join(loc_datg, 'ascouf_ssep_mono_v1.datg'), 'r').read()
    fdgib = open(nomFichierDATG, 'w')
    fdgib.write(texte)
    fdgib.close()
#
...
def write_subpart_file_pgib_POST(MCL_SOUS_EPAIS, NLX, NLY):
    ...
    for ssep in MCL_SOUS_EPAIS:
        ...
        if ssep['TYPE'] == 'ELLI':
            ...
            texte = texte + '*\n'
            texte = texte + 'pcirc' + \
                str(issep) + ' = circo .' + str(issep).rjust(4) + POIVIR
            texte = texte + 'plong' + \
                str(issep) + ' = longi .' + str(issep).rjust(4) + POIVIR
            texte = texte + 'pcent' + \
                str(issep) + ' = centr .' + str(issep).rjust(4) + POIVIR
        texte = texte + '*\n'
        texte = texte + \
            '* ligaments tous les 45 degres a epaisseur minimale \n'
        texte = texte + '*\n'
        ...
        for k in range(8):
            ...
            texte = texte + \
                str(CAR3[k]) + str(issep) + ' = lig45 . rlig' + POIVIR
        if ssep['TYPE'] == 'ELLI':
            texte = texte + '*\n'
            texte = texte + \
                "* ligaments circonferentiels a l'epaisseur minimale\n"
            texte = texte + '*\n'
            texte = texte + 'isep = ' + str(issep) + POIVIR
            for k in range(2 * NLX[issep - 1] + 1):
                ...
                texte = texte + 'cir' + \
                    str(issep) + '_' + str(k + 1) + ' = ligcir . rlig' + POIVIR
            texte = texte + '*\n'
            texte = texte + \
                "* ligaments longitudinaux a l'epaisseur minimale\n"
            texte = texte + '* \n'
            for k in range(2 * NLY[issep - 1] + 1):
                texte = texte + 'ilig = ' + str(k + 1) + POIVIR
                texte = texte + 'rlig = ilig/100. + isep' + POIVIR
                texte = texte + 'lon' + \
                    str(issep) + '_' + str(k + 1) + ' = liglon . rlig' + POIVIR
...
def write_file_dgib_ASCSP1(nomFichierDATG, TYPELE, MCL_SOUS_EPAIS, NIVMAG, loc_datg):
    ...
    texte = texte + '* \n'
    texte = texte + \
        open(os.path.join(loc_datg, 'ascouf_ssep_mult_v1.datg'), 'r').read()
    fdgib = open(nomFichierDATG, 'w')
    fdgib.write(texte)
    fdgib.close()
...
# a l'angle du coude
    ...
    for j in range(NZONEX):
        texte = texte + 'bg .'   + \
            str(j + 1).rjust(23) + ' = ' + str(BG[j] - pi * RM) + POIVIR
        texte = texte + 'bd .'   + \
            str(j + 1).rjust(23) + ' = ' + str(BD[j] - pi * RM) + POIVIR
        texte = texte + 'indbg .' + \
            str(j + 1).rjust(23) + ' = ' + str(INDBG[j]) + POIVIR
        texte = texte + 'indbd .' + \
            str(j + 1).rjust(23) + ' = ' + str(INDBD[j]) + POIVIR
        texte = texte + 'deny .' + \
            str(j + 1).rjust(23) + ' = ' + str(DNX[2 * j]) + POIVIR
        texte = texte + 'nbely .' + \
            str(j + 1).rjust(23) + ' = ' + str(int(DNX[2 * j + 1])) + POIVIR
        texte = texte + '*\n'
    ...
    for j in range(NZONEY):
        texte = texte + 'bi .'   + \
            str(j + 1).rjust(23) + ' = ' + str(BI[j]) + POIVIR
        texte = texte + 'bs .'   + \
            str(j + 1).rjust(23) + ' = ' + str(BS[j]) + POIVIR
        texte = texte + 'indbi .' + \
            str(j + 1).rjust(23) + ' = ' + str(INDBI[j]) + POIVIR
        texte = texte + 'indbs .' + \
            str(j + 1).rjust(23) + ' = ' + str(INDBS[j]) + POIVIR
        texte = texte + 'denz .' + \
            str(j + 1).rjust(23) + ' = ' + str(DNY[2 * j]) + POIVIR
        texte = texte + 'nbelz .' + \
            str(j + 1).rjust(23) + ' = ' + str(int(DNY[2 * j + 1])) + POIVIR
        texte = texte + '*\n'
    ...
    for ssep in MCL_SOUS_EPAIS:
        issep = issep + 1
        texte = texte + 'axecir .' + \
            str(issep).rjust(23) + ' = ' + str(ssep.ICIRP) + POIVIR
        texte = texte + 'axelon .' + \
            str(issep).rjust(23) + ' = ' + str(ssep.ILONP) + POIVIR
        texte = texte + 'prof .'   + \
            str(issep).rjust(23) + ' = ' + str(ssep['PROFONDEUR']) + POIVIR
        texte = texte + 'coory .'  + \
            str(issep).rjust(23) + ' = ' + str(ssep.ISCP - pi * RM) + POIVIR
        texte = texte + 'coorz .'  + \
            str(issep).rjust(23) + ' = ' + str(ssep.ISLP) + POIVIR
        texte = texte + 'posit .'  + \
            str(issep).rjust(23) + " = '" + \
            str(ssep['SOUS_EPAIS'].lower()) + "'" + POIVIR
        texte = texte + 'axelonc .' + \
            str(issep).rjust(23) + ' = ' + str(ssep['AXE_LONGI']) + POIVIR
        if ssep['POSI_CURV_LONGI'] != None:
            texte = texte + 'coorzc .' + \
                str(issep).rjust(23) + ' = ' + str(
                    ssep['POSI_CURV_LONGI']) + POIVIR
        else:
            DZC = ssep.BETA * pi * (RC + (RM + EP / 2.) * cos(pi / 2.)) / 180.
            texte = texte + 'coorzc .' + \
                str(issep).rjust(23) + ' = ' + str(DZC) + POIVIR
        if ssep['TYPE'] == 'AXIS':
            texte = texte + 'axisym .' + \
                str(issep).rjust(23) + " = 'oui'" + POIVIR
        else:
            texte = texte + 'axisym .' + \
                str(issep).rjust(23) + " = 'non'" + POIVIR
        if ssep['EMPREINTE'] == 'OUI':
            texte = texte + 'sousep .' + \
                str(issep).rjust(23) + " = 'oui'" + POIVIR
        else:
...
def write_file_pgib_ASCSP2(MCL_SOUS_EPAIS, NLX, NLY):
    ...
    texte = '*\n'
    texte = texte + \
        'coude extube bord1 clgv  bord2 peauext peauint placoude platube\n'
    texte = texte + \
        'plagv  longi  circo centr bou1  bou3  ligmed  ligtub liggv lig45\n'
    texte = texte + 'ligcir liglon bordtu\n'
    texte = texte + '= PLAQSEP bg bd bi bs indbg indbd indbi indbs rm rc\n'
    texte = texte + \
        'alphac pirad epc lt lgv coory coorz axecir axelon prof zsyme posit\n'
    texte = texte + \
        'lcoude nxep sousep deny nbely denz nbelz axelonc coorzc axisym\n'
...
def write_file_dgib_ASCRDO(
    nomFichierDATG, TYPELE, NIVMAG, TYPBOL, ALPHA, RC, RM, EP, SUREP,
    ...
    texte = texte + '* FIN PARAMETRES UTILISATEUR\n'
    texte = texte + \
        open(os.path.join(loc_datg, 'ascouf_regl_v1.datg'), 'r').read()
    fdgib = open(nomFichierDATG, 'w')
    fdgib.write(texte)
    fdgib.close()
...
# La macro compte pour 1 dans la numerotation des commandes
    ...
    PRECIS = 0.01
    CRITER = 'RELATIF'
    CAR3 = ('IFDRO', 'IEXDR', 'IEXTR',
            'IEXGA', 'IFGAU', 'IINGA', 'IINTR', 'IINDR')
    CAR4 = ('NOFDRO', 'NOEXDR', 'NOEXTR',
            'NOEXGA', 'NOFGAU', 'NOINGA', 'NOINTR', 'NOINDR')
    CAR5 = ('NEFDRO', 'NEEXDR', 'NEEXTR',
            'NEEXGA', 'NEFGAU', 'NEINGA', 'NEINTR', 'NEINDR')
...
#
    if FISS_COUDE != None:
        ...
        if FAXI == 'OUI' and FISS_COUDE['LONGUEUR'] != None:
            UTMESS('A', 'ASCOUF0_20')
        if FISS_COUDE['LONGUEUR'] != None:
            FLONG = FISS_COUDE['LONGUEUR']
        if FAXI == 'OUI':
# on prend une marge de securite a cause des modifs dans ascfis
        ...
        if FISS_COUDE['RAYON_TORE'] != None:
            RC0 = FISS_COUDE['RAYON_TORE']
        else:
            RC0 = 0.
        if FISS_COUDE['COEF_MULT_RC2'] != None:
            RC2 = FISS_COUDE['COEF_MULT_RC2']
        else:
            RC2 = 0.
        if FISS_COUDE['COEF_MULT_RC3'] != None:
            RC3 = FISS_COUDE['COEF_MULT_RC3']
        else:
            RC3 = 0.
...
#
    ...
    if SOUS_EPAIS_MULTI != None:
        MCL_SOUS_EPAIS = SOUS_EPAIS_MULTI
    if SOUS_EPAIS_COUDE != None:
        MCL_SOUS_EPAIS = SOUS_EPAIS_COUDE
    ...
    if MCL_SOUS_EPAIS != None:
        AZIM = 90.0
        if MCL_SOUS_EPAIS.__class__.__name__ != 'MCList':
            MCL_SOUS_EPAIS = [MCL_SOUS_EPAIS, ]
        ...
        for ssep in MCL_SOUS_EPAIS:
            ...
            if ssep['POSI_CURV_LONGI'] != None:
                if ssep['POSI_CURV_LONGI'] > (ALPHA * RC * pi / 180.0):
                    UTMESS('F', 'ASCOUF0_30', vali=[isep], valr=[
                           ssep['POSI_CURV_LONGI'], ALPHA * RC * pi / 180.0])
...
#
            if ssep['POSI_CURV_CIRC'] != None:
                if ssep['POSI_CURV_CIRC'] > (2 * pi * RM):
                    UTMESS('F', 'ASCOUF0_32', vali=[isep], valr=[
                           ssep['POSI_CURV_CIRC'], 2 * pi * RM])
...
#      procedure coude sous-ep.:(MOT-CLE SOUS_EPAIS_MULTI)
            write_file_dgib_ASCSP1(
                nomFichierDATG, TYPELE, MCL_SOUS_EPAIS, NIVMAG, loc_datg)
            write_file_pgib_ASCSDO(
                RM, RC, ALPHA, EP1, LTCLIM, LTCHAR, NBEP, SUREP,
                NZONEX, NZONEY, BG, BD, BI, BS, INDBG, INDBD, INDBI, INDBS,
                DNX, DNY, MCL_SOUS_EPAIS, GEOM, SYME)
            write_file_pgib_ASCSP2(MCL_SOUS_EPAIS, NLX, NLY)
    else:
#   procedure coude regle
        write_file_dgib_ASCRDO(
            nomFichierDATG, TYPELE, NIVMAG, TYPBOL, ALPHA, RC, RM, EP1, SUREP,
            LTCLIM, LTCHAR, NBEP, loc_datg)
            ...
            if ssep['TYPE'] == 'ELLI':
                for k in range(2 * NLX[issep - 1] + 1):
                    ...
                    motscles['CREA_GROUP_NO'].append(_F(NOM=ch2,
                                                        GROUP_MA=ch1,),)
                motscles[
                    'CREA_GROUP_NO'].append(_F(NOM='IPCEN' + str(issep),
                ...
                motscles[
                    'CREA_GROUP_NO'].append(_F(NOM='OPCEN' + str(issep),
                                               INTERSEC=('PEAUEXT', 'IPCEN' + str(issep),),),)
                motscles[
                    'CREA_GROUP_NO'].append(_F(NOM='EPCEN' + str(issep),
                ...
                for k in range(2 * NLX[issep - 1] + 1):
                    ...
                    ch4 = 'ECI' + chtmp
                    motscles[
                        'CREA_GROUP_NO'].append(_F(OPTION='SEGM_DROI_ORDO',
                                                   NOM=ch1,
                                                   GROUP_NO=ch2,
                                                   GROUP_NO_ORIG=ch3,
                                                   GROUP_NO_EXTR=ch4,
...
#        des sections: TU,MI,GV et sous-ep.
            for k in range(8):
                motscles[
                    'CREA_GROUP_NO'].append(_F(NOM=CAR3[k] + str(issep),
        ...
        for k in range(3):
            if SYME == 'ENTIER' or k != 2:
                for j in range(8):
                    motscles[
                        'CREA_GROUP_NO'].append(_F(NOM=CAR3[j] + SECT[k],
...
#      representant les ligaments de la ou des sections: sous-ep.
        ...
        for ssep in MCL_SOUS_EPAIS:
            ...
            for k in range(8):
                motscles[
                    'CREA_GROUP_NO'].append(_F(NOM=CAR4[k] + str(issep),
                                               INTERSEC=('PEAUEXT', CAR3[k] + str(issep),),),)
                motscles[
                    'CREA_GROUP_NO'].append(_F(NOM=CAR5[k] + str(issep),
...
#      representant les ligaments des sections: TU,MI,GV
        for k in range(3):
            if SYME == 'ENTIER' or k != 2:
                for j in range(8):
                    motscles[
                        'CREA_GROUP_NO'].append(_F(NOM=CAR4[j] + SECT[k],
                                                   INTERSEC=('PEAUEXT', CAR3[j] + SECT[k],),),)
                    motscles[
                        'CREA_GROUP_NO'].append(_F(NOM=CAR5[j] + SECT[k],
                                                   INTERSEC=('PEAUINT', CAR3[j] + SECT[k],),),)
# 5/ nommage final des groupes de noeuds representant les ligaments des
# sections: TU,MI,GV
                for j in range(8):
                    motscles[
                        'CREA_GROUP_NO'].append(_F(OPTION='SEGM_DROI_ORDO',
...
# MODI_MAILLAGE  2
    ...
    if FISS_COUDE != None:
        if FISS_COUDE['FISSURE'] == 'DEB_INIT':
            motscles['ORIE_PEAU_3D'] = _F(
                GROUP_MA=('PEAUINT', 'EXTUBE', 'FACE1', 'FACE2'),)
    __nomres = MODI_MAILLAGE(reuse=__nomres,
                             MAILLAGE=__nomres,
                             **motscles)
...
# IMPRESSSION
    if IMPRESSION != None:
        if IMPRESSION.__class__.__name__ != 'MCList':
            IMPRESSION = [IMPRESSION, ]
        for impr in IMPRESSION:
#
            motscles = {}
            if impr['FORMAT'] == 'IDEAS':
                motscles['VERSION'] = impr['VERSION']
            if impr['FORMAT'] == 'CASTEM':
                motscles['NIVE_GIBI'] = impr['NIVE_GIBI']
            if impr['UNITE'] != None:
                motscles['UNITE'] = impr['UNITE']
