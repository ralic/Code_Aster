#@ MODIF post_gp_ops Macro  DATE 09/04/2013   AUTEUR PELLET J.PELLET 
# -*- coding: iso-8859-1 -*-
...
def post_gp_ops(self, **args):
   ...
   if self['TRAC_COMP']=='OUI':
      ...
      l_mailles_last_gm = maya.sdj.GROUPEMA.get()[l_copo_tot[-1].ljust(24)]
   ...
   for i in range(0,nb_tranches):
      ...
      if self['TRAC_COMP']=='OUI':
         ...
         for group_ma in l_copo:
            d_groupma[group_ma] = maya_GROUPEMA[group_ma.ljust(24)]
...
def Calcul_mesure_2D(is_2D,maya,nbcop,num_ord,l_copo_tot,ltyma,resu,type_def):
   ...
   for C_k in range(nbcop) :
        ...
        tab_noeud=[]
        tab_maille=[]
        for maille_courante in maya.sdj.GROUPEMA.get()[mon_nom.ljust(24)]:
...
def Calcul_mesure_3D(is_2D,maya,nbcop,num_ord,l_copo_tot,ltyma,resu,type_def):
   ...
   l_noeud_fond = [noeud_courant for noeud_courant in maya.sdj.GROUPENO.get()['Nds_Floc'.ljust(24)]]
   tab_DEP_el   = Coord_Recup([maya.sdj.NOMNOE.get()[x-1] for x in maya.sdj.GROUPENO.get()['Nds_Floc'.ljust(24)]] ,None,resu)
   ...
   for C_k in range(len(l_copo_tot)) :
       ...
       coord_nds = []
       dist_moy=0.
       for noeud_courant in maya.sdj.GROUPENO.get()['Nds_Delt'.ljust(24)]:
       ...
       l_ep_copeaux_tot_3D.append(dist_moy/len(maya.sdj.GROUPENO.get()['Nds_Delt'.ljust(24)]))
