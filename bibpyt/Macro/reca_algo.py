...
def Levenberg_bornes(val, Dim, val_init, borne_inf, borne_sup, A, erreur, l, ul_out):
    ...
    while done < 1:
        ...
        if (len(Act) != 0):
            ...
            sa = -NP.take(d, Act) - NP.dot(
                NP.take(Q, Act, axis=0), Dim.adim(dval))
        ...
        done = (max(val + dval - borne_sup) <= 0) & (
            min(val + dval - borne_inf) >= 0) & (min(s) >= 0.0)
        ...
        if (k > 50):
            try:
                ...
                res.write('\n\nQ = \n' + NP.array2string(
                    Q - l * id, array_output=1, separator=','))
                res.write('\n\nd = ' + NP.array2string(
                    d, array_output=1, separator=','))
                res.write('\n\nval = ' + NP.array2string(
                    val, array_output=1, separator=','))
                res.write('\n\nval_ini= ' + NP.array2string(
                    val_init, array_output=1, separator=','))
                res.write('\n\nborne_inf= ' + NP.array2string(
                    borne_inf, array_output=1, separator=','))
                res.write('\n\nborne_sup= ' + NP.array2string(
                    borne_sup, array_output=1, separator=','))
                UTMESS('F', 'RECAL0_18')
                return
...
def actualise_lambda(l, val, new_val, A, erreur, new_J, old_J):
    ...
    old_Q = old_J
    new_Q = old_J + 0.5 * \
        NP.dot(NP.transpose(new_val - val), NP.dot(Q, new_val - val)) + \
...
def test_convergence(gradient_init, erreur, A, s):
    ...
    try:
        epsilon = NP.dot(gradient, gradient) / \
            NP.dot(gradient_init, gradient_init)
    except:
        UTMESS('F', "RECAL0_19")
        return
    epsilon = epsilon ** 0.5
...
#   if ((iter < max_iter) or (residu < prec)):
    if 1 == 1:
        ...
        vecteurs_propres = NP.transpose(
            vecteurs_propres)  # numpy et Numeric n'ont pas la meme convention
        sensible = NP.nonzero(
            NP.greater(abs(valeurs_propres / max(abs(valeurs_propres))), 1.E-1))[0]
        insensible = NP.nonzero(
            NP.less(abs(valeurs_propres / max(abs(valeurs_propres))), 1.E-2))[0]
        # Reactive les FPE
        aster_core.matfpe(1)
