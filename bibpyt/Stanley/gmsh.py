...
#  Mode post-traitement
        ...
        # Verifications
            ...
            if ((param['machine_gmsh_exe'].strip() != '') and (param['machine_visu'].strip() != '')):
                ex_gmsh = param['machine_gmsh_exe'] + \
                    ' -display ' + param['machine_visu']
            else:
                ex_gmsh = None

            # Protocole de recopie et d'execution distante
            copie = param['protocole'].split('/')[
                0]                                    # rcp ou scp
            execution = param['protocole'].split('/')[
                1]                                    # rsh ou ssh
            ...
            # Fichier distant
            fic_distant = param['tmp'] + '/' + \
                fichier                                      # /tmp/fort.33.pos

            if param['machine_gmsh_login'].strip() != '':
                mdis  = '-l ' + \
                    param['machine_gmsh_login'] + ' ' + \
                        mdis                     # -l assire cli75ca
                rep_distant = param['machine_gmsh_login'] + '@' + \
                    rep_distant                # assire@cli75ca:/tmp/

            # Liste des fichiers a copier sur la machine distante
            l_fichier = [fichier]

            # Creation du script d'affichage sur la peau et recopie sur le
            # serveur Gmsh
            ...
            # On lance Gmsh sur le serveur distant sur le premier fichier de la
            # liste l_fichier
            if ex_gmsh:
                fic = l_fichier[0]
                txt = execution + " " + mdis + " '" + \
                    ex_gmsh + " " + param['tmp'] + '/' + fic + "'"
                self.Commande(txt)
            else:
                UTMESS('A', 'STANLEY_7')
...
#  Mode post-traitement
        ...
        if mode == 'POST':
            ...
            if partage_win_login == '':
                txt = smbclient + " '\\\\" + machine_win + "\\" + _nom_partage + \
                    "' -N -c 'cd " + sous_rep + " ; rm " + \
                    fichier + ".pos ; put " + fichier + ".pos'"
                UTMESS('I', 'STANLEY_9', valk=[txt])
                os.system(txt)
            else:
                txt = smbclient + " '\\\\" + machine_win + "\\" + _nom_partage + "' " + "****" + " -U " + \
                    partage_win_login + " -c 'cd " + sous_rep + \
                    " ; rm " + fichier + ".pos ; put " + fichier + ".pos '"
                UTMESS('I', 'STANLEY_9', valk=[txt])
                txt = smbclient + " '\\\\" + machine_win + "\\" + _nom_partage + "' " + partage_win_pass + " -U " + \
                    partage_win_login + " -c 'cd " + sous_rep + \
                    " ; rm " + fichier + ".pos ; put " + fichier + ".pos '"
                os.system(txt)
            ...
            if param['SKIN'].lower() in ['oui', 'yes']:
                ...
                if partage_win_login == '':
                    txt = smbclient + " '\\\\" + machine_win + "\\" + _nom_partage + \
                        "' -N -c 'cd " + sous_rep + \
                        " ; rm skin.pos ; put skin.pos'"
                    UTMESS('I', 'STANLEY_9', valk=[txt])
                    os.system(txt)
                else:
                    txt = smbclient + " '\\\\" + machine_win + "\\" + _nom_partage + "' " + "****" + \
                        " -U " + partage_win_login + " -c 'cd " + \
                        sous_rep + " ; rm skin.pos ; put skin.pos'"
                    UTMESS('I', 'STANLEY_9', valk=[txt])
                    txt = smbclient + " '\\\\" + machine_win + "\\" + _nom_partage + "' " + partage_win_pass + \
                        " -U " + partage_win_login + " -c 'cd " + \
                        sous_rep + " ; rm skin.pos ; put skin.pos'"
                    os.system(txt)
                UTMESS('I', 'STANLEY_12')
