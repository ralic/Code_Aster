...
# height=hauteur)
        ...
        frame_1.place(in_=self.fenetre2, x=x_1, y=y_1,
                      anchor='nw', width=largeur_1, height=hauteur_1)
...
#    hauteur_2 = 340
        ...
        frame_2.place(in_=self.fenetre2, x=x_2, y=y_2,
                      anchor='nw', width=largeur_2, height=hauteur_2)
        ...
        frame_3.place(in_=self.fenetre2, x=x_3, y=y_3,
                      anchor='nw', width=largeur_3, height=hauteur_3)
        ...
        frame_4.place(in_=self.fenetre2, x=x_4, y=y_4,
                      anchor='nw', width=largeur_4, height=hauteur_4)
        ...
        frame_5.place(in_=self.fenetre2, x=x_5, y=y_5,
                      anchor='nw', width=largeur_5, height=hauteur_5)
        ...
        # Liste des sections
        self.liste_section = [['MODE_GRAPHIQUE', frame_1, 10], ['CONFIG', frame_2, 10], [
            'PARAMETRES', frame_4, 10], ['VISUALISATION', frame_5, 10]]
...
# h=hauteur_3-40, l=largeur_3-40, anchor='nw')
        ...
        # Aide
        self.case_aide = objets.CASE_TEXTE(
            ...
            for cle in self.dparam.keys():
                ...
                if section2 == section:
                    if self.dparam[cle]['type'] == 'liste':
                        self.liste_cle[section].append(cle)
                    else:
                        self.liste_cle[section].insert(0, cle)
            ...
            for cle in self.liste_cle[section]:
                ...
                if (type_para in ['texte', types.FloatType, 'fichier']):
                    ...
                    self.objet[cle] = objets.ENTRY(
                        frame, self.dparam, self.dvar, section, cle, x=x1, y=y, largeur=15, fonte=self.fonte, bulle=self.bulle)
                    y += dy
                elif (self.dparam[cle]['type'] == 'liste'):
                    y += 0.2 * dy
                    self.label[cle] = objets.LABEL(
                        frame, self.dparam, self.dvar, section, cle, x=x, y=y + 2, txt=label, fonte=self.fonte, bulle=self.bulle)
                    self.objet[cle] = objets.LISTE(
                        frame, self.dparam, self.dvar, section, cle, x=x1, y=y, largeur=15, commande=None, fonte=self.fonte, bulle=self.bulle)
                    y += 1.4 * dy
                else:
                    pass
            y += dy
...
#    self.bulle.destroy()
    ...
    def Raffraichir_Affichage(self):
        ...
        # Si les 2 choix mode_graphique et mode n'ont pas changés on n'a pas
        # besoin de raffraichir
        if (self.old_mode_graphique == mode_graphique) and (self.old_mode == mode):
            return
...
# self.valeur = Tk.Entry(frame, textvariable = self.dvar[section][cle],
# font=fonte, width=largeur, justify="center")
        self.valeur = Tk.Entry(frame)
        self.valeur.place(in_=frame, x=x, y=y)
        self.valeur.configure(width=largeur)
        self.valeur.configure(justify="center")
...
#     self.Case_Texte = Tk.Text(self.frame)
        self.Case_Texte = Tk.Text(
            self.frame, wrap=Tk.WORD, yscrollcommand=scrollbar.set)
    ...
    def Dessine(self, image):
        ...
        if image:
            ...
            self.canvas.create_image(
                0.5 * self.lcanv, 0.5 * self.hcanv, anchor='center', image=self.image)
