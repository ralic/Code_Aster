#@ MODIF xmgrace Stanley  DATE 20/12/2011   AUTEUR COURTOIS M.COURTOIS 
# -*- coding: iso-8859-1 -*-
...
class Xmgr :
  ...
  def __init__(self, gr_max = 10, options=None, xmgrace=os.path.join(aster.repout(), 'xmgrace')) :
    ...
    if options != None :
       cmd = self.xmgrace + ' -noask ' + options + ' -graph ' + repr(gr_max-1) + ' -npipe ' + self.nom_pipe
    else :
       cmd = self.xmgrace + ' -noask ' +           ' -graph ' + repr(gr_max-1) + ' -npipe ' + self.nom_pipe
