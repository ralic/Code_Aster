    ...
    try:
        ...
        for para, pval in args.items():
            ...
            if pval:
                ...
                pval = "''"
            if not txt.find(para) != -1:
                UTMESS('I', 'EXECLOGICIEL0_19', valk=para)
...
def RunScript(SALOMESCRIPT, CHOIX, INPUTFILE):
    ...
    """
    tmpfile = MakeTempScript(
        SALOMESCRIPT=SALOMESCRIPT, CHOIX=CHOIX, INPUTFILE=INPUTFILE, OUTPUTFILE=False, STUDY=False)
...
if __name__ == '__main__':
    ...
    # Remplacer les deux chemins ci-dessous par vos surcharges, sinon le
    # script tentera de detecter l'installation d'Aster

#    root_astest  = '/home/assire/DEV/PYLOTAGE/astest'
# salomeScript =
# '/home/assire/DEV/PYLOTAGE/bibpyt/Templates/salomeScript.py'
    ...
    lst_vers = ['NEW11', 'NEW10', 'NEW9', 'STA11.3',
                'STA11.2', 'STA11.1', 'STA10.3', 'STA10.2', 'STA10.1']
    ...
    if not salomeScript or not os.path.isfile(salomeScript):
        ...
        for rep in lst_aster:
            for vers in lst_vers:
                ...
                script = os.path.join(
                    p, 'bibpyt', 'Templates', 'salomeScript.py')
    ...
    if not root_astest:
        raise Exception(
            "Impossible de detecter le repertoire Aster. Remplir la variable root_astest.")

    if not salomeScript:
        raise Exception(
            "Impossible de detecter le script Salome. Remplir la variable salomeScript.")
    ...
    script = os.path.join(
        os.path.dirname(salomeScript), 'salomeGetStudies.py')
