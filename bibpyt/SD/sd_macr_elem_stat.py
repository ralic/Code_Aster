...
#----------------------------------------------
    ...
    LICH = Facultatif(
        AsColl(acces='NO', stockage='CONTIG', modelong='CONSTANT', type='K', ltyp=8))
    ...
    #------------------------------------
        ...
        nbnoe, nbnoi, nddle, nddli, nbchar, nbcas, nlage, nlagl, nlagi = desm[
            1:10]
        assert nbnoe > 0, desm
        ...
        if self.MAEL_RAID_VALE.exists:
            ...
            assert nlagl >= 0, desm
            assert nlagi >= 0, desm
            assert self.CONX.lonmax == 3 * \
                (nbnoe + nlage + nlagl), (desm, self.CONX.get())
