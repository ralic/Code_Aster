#@ MODIF B_SENSIBILITE_JDC Build  DATE 03/09/2002   AUTEUR GNICOLAS G.NICOLAS 
...
#
          ...
          codret0 = self.commandes_sensibles.add_classe_para_sensi(etape.sd.__class__)
...
#               s'il fait partie des paramètres sensibles définisprécédemment dans le jdc
            ...
            if etape.nom in l_commandes_a_deriver_ensemble :
              ...
              for param in self.l_param_sensible :
                for sd_u in l_sd_utilisees :
                  if sd_u is param :
                    codret = memo_nom_sensi.add_commande(param,etape.nom)
#
...
#
      if poursuite :
        ...
        context_ini = None
#
      new_jdc = self.jdc.__class__(definition=self.jdc.definition,cata=self.jdc.cata,appli=self.jdc.appli,procedure="#\n",context_ini=context_ini)
#
      new_jdc.compile()
...
#
      for etape in self.jdc.etapes :
        ...
        if etape.isactif() :
     # 3.1. pour un parametre : on recopie tel quel
          if etape.nature == 'PARAMETRE':
#            print "Lancement de la copie de ",etape.nom
            new_etape = copy.copy(etape)
            new_etape.reparent(new_jdc)
            new_jdc.register_parametre(new_etape)
            new_jdc.register(new_etape)
#            print "Fin de la copie de ",etape.nom
     # 3.2. pour une commande : on dérive éventuellement et on recopie tel quel
          elif hasattr(etape,'sd') :
        # 3.2.1. Copie telle quelle de l'étape initiale
#            print "Lancement de la copie de ",etape.nom
            new_etape = etape.copy()
            new_etape.copy_reuse(etape)
            new_etape.copy_sdnom(etape)
            new_etape.reparent(new_jdc)
#            print "Fin de la copie de ",etape.nom
            if etape.sd :
              new_sd = etape.sd.__class__(etape=new_etape)
              new_etape.sd = new_sd
              if etape.reuse == None :
                new_jdc.NommerSdprod(new_sd,etape.sd.nom)
              else :
                new_sd.nom = etape.sd.nom
            if self.DEBUG :
              print ".. L'étape a été recopiée à l'identique."

        # 3.2.2. On repère si c'est une commande sensible ou non :
...
#              print ". enregistrement immediat de  = ",etape.nom
        ...
        # 3.2.3. Si l'étape produit une sd, on va la dériver dans chacun des cas suivants :
...
#              print "type(etape.definition.sd_prod) = ",type(etape.definition.sd_prod)
          ...
          # 3.2.3.1. Dérivation d'une définition de paramètre sensible
              for param in self.l_param_sensible :
                if param is etape.sd :
                  if self.DEBUG :
                    print ".. Lancement de la dérivation"
                  codret = derivation.derivation_para_sensi(etape,param,new_jdc)
                  a_faire = 0

          # 3.2.3.2. Dérivation d'une définition de paramètres autres : on ne fait
          #          qu'enregistrer un nom composé pour activer les dépendances ultérieures
              if a_faire :
                for param in self.l_param_autres :
                  if param is etape.sd :
                    if self.DEBUG :
                      print ".. Création d'un nom composé"
                    nom_sd_derivee = derivation.get_nouveau_nom_sd()
                    codret = memo_nom_sensi.add_nom_compose(etape.sd,param,nom_sd_derivee)
                    a_faire = 0

          # 3.2.3.3. Dérivation d'un autre type de commande
          #          Remarques :
          ...
          # 3.2.3.3.1. Récupération du dictionnaire des couples (nom simple,nom composé) existant pour ce paramètre
                    codret,d_nom_s_c = memo_nom_sensi.get_d_nom_s_c(param)

          # 3.2.3.3.2. Doit-on effectivement dériver par rapport au paramètre param ?
          ...
          #           qu'une a été trouvée car la dérivation les prendra toutes en compte
                    on_derive = 0
                    if etape.nom in l_commandes_a_deriver_ensemble :
                      codret, l_commandes = memo_nom_sensi.get_l_commandes(param)
                      if etape.nom in l_commandes :
                        on_derive = 1
                    else :
                      l_sd_utilisees = etape.get_sd_utilisees()
#                      print ". l_sd_utilisees = ",l_sd_utilisees
                      for sd_u in l_sd_utilisees :
                        for sd_d in d_nom_s_c.keys() :
                          if sd_u is sd_d :
                            on_derive = 1

          # 3.2.3.3.3. On doit dériver par rapport au paramètre param ; alors on y va ...
          #            Si la commande est à dériver, on y va
          #            Sinon, il sufit de créer et de mémoriser un nouveau nom de sd. Quand le nom est
          #            nouveau, il faut l'enregistrer dans le JDC via la commande MEM0_NON_SENSI
                    if on_derive :
                      if self.DEBUG :
                        print ".. Lancement de la dérivation"
                      if etape.nom in l_commandes_a_deriver + l_commandes_a_deriver_ensemble :
                        codret = derivation.derivation_commande(etape,param,new_jdc,d_nom_s_c)
                      else :   
                        nom_sd_derivee = derivation.get_nouveau_nom_sd()
                        codret = memo_nom_sensi.add_nom_compose(etape.sd,param,nom_sd_derivee)
#                        print "... code de retour de memo_nom_sensi.add_nom_compose = ",codret
                        if codret == 0 : 
                          txt = derivation.get_texte_memo_nom_sensi_compose(etape.sd.nom,param.nom,nom_sd_derivee,None)
                          exec txt in new_jdc.g_context

        # 3.2.4. Enregistrement final si cela n'a pas été fait
