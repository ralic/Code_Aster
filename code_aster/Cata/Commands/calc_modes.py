          ...
          b_bande       =BLOC(condition = """equal_to("TYPE_RESU", 'DYNAMIQUE') and equal_to("OPTION", 'BANDE')""",
                   ...
                   CALC_FREQ  =FACT(statut='d',
                       FREQ       =SIMP(statut='o',typ='R',min=2,max='**', validators=AndVal((OrdList('croissant'), NoRepeat())),
                                      fr=tr("Valeurs des fréquences délimitant la (les) bande(s) de recherche")),
                       TABLE_FREQ =SIMP(statut= 'f',typ=table_sdaster),
                       b_multi_bande =BLOC( condition= """length('FREQ') > 2""",
...
#        Catalogue commun SOLVEUR
         b_solveur_lin_simult =BLOC(condition = """is_in("OPTION", ('PLUS_PETITE','PLUS_GRANDE','BANDE','CENTRE','TOUT'))""",
                 SOLVEUR =C_SOLVEUR('MODE_ITER_SIMULT'),
         ),
         b_solveur_lin_inv    =BLOC(condition = """is_in("OPTION", ('SEPARE','AJUSTE','PROCHE'))""",
